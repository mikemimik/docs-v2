---
title: "Autenticar a un usuario"
description: "Autentica a un usuario con su número de teléfono o correo electrónico y un código de verificación sin contraseña."
---
...
* El `username` debe ser el número de teléfono del usuario (si `realm=sms`) o el correo electrónico del usuario (si `realm=email`).
...
* El `redirect_uri` es una URL de callback obligatoria que debe estar registrada en la sección **Allowed Callback URLs** de tu aplicación.
...
<ParamField body="redirect_uri" type="string" required>
  Una URL de callback que se haya registrado en las Allowed Callback URLs de tu aplicación.
</ParamField>
