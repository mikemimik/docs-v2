...
Este es el tipo de concesión de OAuth 2.0 que utilizan las aplicaciones web para acceder a una API.
...
* El valor de `redirect_uri` debe especificarse como una URL de devolución de llamada válida en [Settings de la aplicación](https://manage.auth0.com/dashboard).
* Si `response_type=token`, después de que el usuario se autentique con el proveedor, se le redirigirá a la URL de devolución de llamada de su aplicación y se enviará el `access_token` en `location.hash` de la URL. Esto se usa en aplicaciones de página única y en SDK móviles nativos.
...
* [Mitiga los ataques de repetición al usar la concesión implícita](https://auth0.com/docs/get-started/authentication-and-authorization-flow/implicit-flow-with-form-post/mitigate-replay-attacks-when-using-the-implicit-flow)
...
<ParamField query="scope" type="string">
  Los scopes para los que quieres solicitar autorización. Deben separarse con un espacio.
</ParamField>

<ParamField query="response_type" type="string" required>
  Esto especifica el tipo de token que recibirás al final del flujo. Usa `token` para obtener solo un token de acceso, `id_token` para obtener solo un token de identidad, o `id_token token` para obtener ambos.
...
<ParamField query="nonce" type="string">
  Un valor de cadena incluido en la respuesta del token de identidad para evitar ataques de repetición de tokens.
</ParamField>
