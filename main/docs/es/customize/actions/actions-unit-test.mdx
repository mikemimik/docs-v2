---
description: El paquete **Actions NPM (`@auth0/actions`)** permite el desarrollo con TypeScript en proyectos externos, lo que ayuda a los desarrolladores a seguir las prácticas recomendadas y a mejorar sus **pruebas unitarias** a partir de las definiciones de TypeScript.
...
El **paquete Actions NPM (`@auth0/actions`)** permite desarrollar con TypeScript en proyectos externos, lo que ayuda a los desarrolladores a seguir las prácticas recomendadas y a mejorar sus pruebas unitarias a partir de las definiciones de TypeScript.
...
Para hacer pruebas unitarias de una Action, debes simular los objetos `event` y `api` que recibe la función de tu Action. Puedes crear estos mocks con las definiciones de TypeScript incluidas en `auth0/actions`, lo que garantiza que tus pruebas reflejen con precisión el entorno de producción. Las herramientas de pruebas, como Jest, son ideales para gestionar los mocks y comprobar la funcionalidad.
...
Los siguientes ejemplos sirven para validar una serie de escenarios simulando los objetos necesarios.

<Note>
  Los ejemplos usan **Jest** (`https://www.npmjs.com/package/jest`), pero puede utilizarse cualquier biblioteca de pruebas.
</Note>
...
<Tabs>
  ...
  <Tab title="TypeScript">
    ...
    En tu `jest.config.js`, define los ajustes preestablecidos del entorno de `Jest`:
...
<div id="pre-user-registration-access-control-and-user-metadata-setup">
  ## Control de acceso del registro previo de usuario y configuración de los metadatos del usuario
</div>

La siguiente Action de ejemplo comprueba si el correo electrónico del usuario tiene un dominio prohibido y llama a `api.access.deny()` si coincide. En caso contrario, comprueba si se ha proporcionado el nombre completo mediante los campos adicionales de los prompts personalizados y, si es así, lo establece en `user_metadata` del perfil de usuario; de lo contrario, envía un error de validación a Universal Login.
...
<Tabs>
  <Tab title="Javascript">
    ...
    /**
    * Manejador que se llamará durante la ejecución de un flujo de PreUserRegistration.
    *
...
La prueba unitaria realiza algunas validaciones para maximizar la cobertura del código mediante la simulación de los objetos `event` y `api`.
...
La siguiente Action de ejemplo intenta enviar un mensaje mediante una solicitud HTTP a un servicio externo y, si se produce un error en la solicitud, descarta la notificación. Utiliza secrets para la URL del servicio externo y la API Key de autorización.
...
<Tabs>
  <Tab title="Javascript">
    ...
    /**
    * Manejador que se ejecutará durante el envío de una notificación por correo electrónico
    *
...
La prueba unitaria realiza algunas validaciones para maximizar la cobertura del código mediante mocks de los objetos event y api, además de la función fetch.
