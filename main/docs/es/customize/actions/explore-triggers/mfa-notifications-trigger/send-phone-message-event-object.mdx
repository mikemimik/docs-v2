...
#
description: Obtenga información sobre el objeto `event` del trigger de Action send-phone-message, que proporciona contexto sobre la ejecución del trigger.
title: 'Triggers de Actions: send-phone-message - Objeto `event`'
validatedOn: 2026-04-28
---

El objeto `event` del trigger de Actions send-phone-message proporciona contexto sobre la ejecución del trigger.
...
<ResponseField name="event.message_options" type="object">
  ...
  <Expandable title="propiedades de message_options" defaultOpen>
    ...
    <ResponseField name="message_type" type="string">
      Cómo se entregará el mensaje, ya sea por &#39;sms&#39; o por &#39;voice&#39;.
...
<ResponseField name="event.request" type="dictionary">
  ...
  <Expandable title="propiedades de la solicitud" defaultOpen>
    ...
    <ResponseField name="hostname" type="string" post={["optional"]}>
      El hostname que se utiliza en el flujo de autenticación.
    </ResponseField>
...
<ResponseField name="event.security_context" type="object" post={["optional"]}>
  Un objeto que contiene firmas de huella digital. Solo estará disponible si el cliente usa Cloudflare. La huella digital JA3/JA4 puede ser nula o estar vacía en algunos casos. El caso más común se da en las solicitudes HTTP, porque JA3 y JA4 se calculan en TLS. También puede estar vacía si el Worker envía solicitudes dentro de la misma zona o a una zona que no pasa por proxy (o a un tercero).
...
<ResponseField name="event.user" type="dictionary">
  ...
  <Expandable title="propiedades del usuario" defaultOpen>
    <ResponseField name="app_metadata" type="dictionary">
      Campos personalizados que almacenan información sobre un usuario y que influyen en su acceso, como el plan de soporte, los roles de seguridad o los grupos de control de acceso.
    </ResponseField>
