...
#
description: Obtén información sobre el objeto de evento del trigger de Action send-phone-message, que proporciona información contextual sobre la ejecución del trigger.
title: 'Actions Triggers: send-phone-message - Objeto de evento'
validatedOn: 2026-04-28
---

El objeto `event` del trigger de Actions send-phone-message proporciona información contextual sobre la ejecución del trigger.
...
<ResponseField name="event.message_options" type="object">
  ...
  <Expandable title="propiedades de message_options" defaultOpen>
    <ResponseField name="action" type="string">
      El flujo que desencadenó esta Action.
...
<ResponseField name="event.request" type="dictionary">
  ...
  <Expandable title="propiedades de la solicitud" defaultOpen>
    ...
    <ResponseField name="user_agent" type="string" post={["optional"]}>
      El valor del header `User-Agent` recibido al iniciar la transacción.
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="event.security_context" type="object" post={["optional"]}>
  Un objeto que contiene firmas de huella digital. Esto solo estará disponible si el cliente usa cloudflare. La huella digital JA3/JA4 puede ser nula o estar vacía en algunos casos. Lo más habitual ocurre con las solicitudes HTTP, ya que JA3 y JA4 se calculan en TLS. También puede estar vacía porque el Worker envía solicitudes dentro de la misma zona o a una zona que no está detrás del proxy (o a un tercero).
...
<ResponseField name="event.user" type="dictionary">
  ...
  <Expandable title="propiedades del usuario" defaultOpen>
    ...
    <ResponseField name="user_metadata" type="dictionary">
      Campos personalizados que almacenan información sobre un usuario que no influye en aquello a lo que puede o no puede acceder, como la dirección del trabajo, la dirección de casa o las preferencias del usuario.
    </ResponseField>
