...
#
description: Conozca el objeto event del trigger credentials-exchange de Actions, que proporciona información contextual sobre la ejecución del trigger.
...
<ResponseField name="event.resource_server" type="object">
  Información sobre el Resource Server que emite el token de acceso.

  <Expandable title="Propiedades de resource_server" defaultOpen>
    <ResponseField name="identifier" type="string">
      El identificador del resource server. Por ejemplo: `https://your-api.example.com`.
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="event.transaction" type="object">
  ...
  <Expandable title="propiedades de la transacción" defaultOpen>
    ...
    <ResponseField name="target_scopes" type="array of strings" post={["optional"]}>
      &#91;acceso anticipado&#93; El conjunto de scopes de destino en tiempo real para el token de acceso. Se inicializa a partir de las autorizaciones de cliente y se actualiza de inmediato mediante los métodos de target scope de api.transaction en las Actions actuales y posteriores. Una vez que se completan todas las Actions, estos scopes se intersectan con la autorización de cliente. Los scopes que no estén presentes en la concesión se omiten silenciosamente del token de acceso final.
    </ResponseField>
  </Expandable>
</ResponseField>