...
#
description: Obtenga información sobre el objeto `event` del trigger de Action credentials-exchange, que proporciona información contextual sobre la ejecución del trigger.
...
<ResponseField name="event.accessToken" type="dictionary">
  Información sobre el token de acceso que se emitirá.
...
<ResponseField name="event.transaction" type="object">
  ...
  <Expandable title="propiedades de la transacción" defaultOpen>
    ...
    <ResponseField name="target_scopes" type="array of strings" post={["optional"]}>
      &#91;Acceso anticipado&#93; El conjunto actual de scopes de destino para el token de acceso. Se inicializa a partir de las autorizaciones de cliente y se actualiza inmediatamente mediante los métodos de scope de destino de api.transaction en las Actions actuales y posteriores. Una vez que se completan todas las Actions, estos scopes se intersectan con la autorización de cliente. Los scopes que no estén presentes en la autorización se omiten silenciosamente del token de acceso final.
    </ResponseField>
  </Expandable>
</ResponseField>