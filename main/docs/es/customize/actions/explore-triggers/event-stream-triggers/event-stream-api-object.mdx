...
#
description: Obtén información sobre el objeto de API del trigger de Action event-stream.
...
Los valores almacenados en esta caché se aplican únicamente al trigger en el que
...
disponibles en transacciones posteriores, incluso si todavía están dentro del tiempo de vida indicado.
...
<ResponseField name="options" type="cachesetoptions" post={["optional"]}>
  ...
  <Expandable title="propiedades de options" defaultOpen>
    <ResponseField name="expires_at" type="number" post={["optional"]}>
      El momento de expiración absoluto en milisegundos desde la época Unix.
