...
#
description: Obtén información sobre el objeto de API del trigger custom-token-exchange de Actions.
...
Marca como inválido el token de sujeto proporcionado en la request. Esto hará que la request sea
rechazada con un código de error &quot;invalid&#95;request&quot;.

Esto indicará a las funciones de Attack Protection que se ha proporcionado un token de sujeto no válido,
para que se puedan aplicar protecciones que eviten ataques de fuerza bruta contra el token de sujeto.
...
<ResponseField name="user_attributes" type="customtokenexchangesetuserbyconnectionuserattributes">
  ...
  se usará para determinar si el usuario existe o si debe crearse. En el caso de usuarios existentes, este `user_id`
  puede encontrarse al inspeccionar el arreglo `identities` del perfil de usuario normalizado.
  ...
  <Expandable title="propiedades de user_attributes" defaultOpen>
    ...
    <ResponseField name="email_verified" type="boolean" post={["optional"]}>
      Indica si esta dirección de email está verificada (`true`) o no (`false`).
    </ResponseField>
    ...
    <ResponseField name="verify_email" type="boolean" post={["optional"]}>
      Indica si el usuario recibirá un email de verificación después de su creación (`true`) o no recibirá ningún email (`false`).
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="organization_id_or_name" type="string">
  El ID o el nombre de la organización que se asignará al usuario.
</ResponseField>
...
Establece el actor para el intercambio de tokens para representar a la entidad que actúa en nombre del subject.
...
`ttl` o `expires_at` especificados. Si no se especifica ningún tiempo de vida, se usará un tiempo de vida predeterminado
de 15 minutos. Los tiempos de vida no pueden superar la duración máxima
