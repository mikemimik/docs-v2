...
#
description: Más información sobre el objeto de API del trigger custom-token-exchange de Actions.
title: 'Actions Triggers: custom-token-exchange - Objeto de API'
...
para distinguir entre intentos de fuerza bruta con el token de sujeto y otros motivos para denegar la solicitud.
...
<ResponseField name="reason" type="string">
  Una explicación en lenguaje claro del motivo del rechazo de la solicitud de intercambio de tokens.
</ResponseField>
...
exports.onExecuteCustomTokenExchange = async (event, api) => {
  ...
  // si el usuario está autorizado, continuar como se indica aquí
...
Marca como no válido el token de sujeto proporcionado en la solicitud. Esto hará que la solicitud se
rechace con el código de error &quot;invalid&#95;request&quot;.
...
<ResponseField name="reason" type="string">
  Una explicación en lenguaje claro para rechazar la solicitud de intercambio de tokens.
</ResponseField>
...
Nota: La Action de Custom Token Exchange debe llamar exactamente a una de estas dos opciones: api.authentication.setUserByConnection o api.authentication.setUserById.
...
La propiedad user&#95;id del user&#95;profile se usará para determinar si el usuario ya existe.

Nota: La Action de Custom Token Exchange debe llamar exactamente a una de estas funciones: api.authentication.setUserByConnection o api.authentication.setUserById.
...
<ResponseField name="user_attributes" type="customtokenexchangesetuserbyconnectionuserattributes">
  ...
  se usará para determinar si el usuario existe o si debe crearse. En usuarios existentes, este user&#95;id
  ...
  Si estos no coinciden con el usuario existente, se devolverá un error.
  ...
  <Expandable title="propiedades de user_attributes" defaultOpen>
    ...
    <ResponseField name="email_verified" type="boolean" post={["optional"]}>
      Indica si esta dirección de correo electrónico está verificada (true) o no (false).
    </ResponseField>
    ...
    <ResponseField name="verify_email" type="boolean" post={["optional"]}>
      Indica si el usuario recibirá un email de verificación después de crearse (true) o no recibirá ningún email (false).
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="organization_id_or_name" type="string">
  El ID o nombre de la organización que se establecerá para el usuario.
</ResponseField>
...
<ResponseField name="actor" type="actorparams">
  Un objeto anidado que representa una cadena de delegación. Se permiten hasta 4 niveles act adicionales
...
Solicita modificaciones del usuario correspondiente al token de sujeto.

<div id="apiusersetappmetadatakey-value">
  ### `api.user.setAppMetadata(key, value)`
</div>

Asigna metadatos específicos de la aplicación al usuario correspondiente al token de sujeto.
...
Los valores almacenados en este caché quedan delimitados al trigger en el que
...
de `ttl` o `expires_at` especificados. Si no se especifica ninguna vigencia, se usará de forma
predeterminada una vigencia de 15 minutos. Las vigencias no pueden superar la duración máxima
...
disponibles en transacciones posteriores, incluso si todavía se encuentran dentro de la vigencia proporcionada.
