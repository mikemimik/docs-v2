...
#
description: Obtén información sobre el objeto `event` del trigger de Action de Registro previo de usuario, que proporciona información contextual sobre la ejecución del trigger.
title: 'Triggers de Actions: Registro previo de usuario - Objeto `event`'
validatedOn: 2026-05-25
---

El objeto `event` del trigger de Actions de Registro previo de usuario proporciona información contextual sobre la ejecución del trigger.
...
<ResponseField name="event.client" type="dictionary" post={["optional"]}>
  ...
  <Expandable title="propiedades del cliente" defaultOpen>
    ...
    <ResponseField name="metadata" type="dictionary">
      Objeto que contiene otras propiedades de la aplicación.
    </ResponseField>
...
<ResponseField name="event.request" type="dictionary">
  ...
  <Expandable title="propiedades de la solicitud" defaultOpen>
    <ResponseField name="body" type="dictionary">
      El cuerpo de la solicitud POST. Estos datos solo estarán disponibles durante los flujos de token de actualización y de Client Credential Exchange, y en la Action PreUserRegistration.
    </ResponseField>
...
<ResponseField name="event.secrets" type="secrets">
  Valores de Secret almacenados de forma segura y asociados a esta Action.
</ResponseField>
...
<ResponseField name="event.security_context" type="object" post={["optional"]}>
  Un objeto que contiene firmas de huella digital. Solo estará disponible si el cliente usa cloudflare. La huella digital JA3/JA4 puede ser null o estar vacía en algunos casos. El caso más común es el de las solicitudes HTTP, porque JA3 y JA4 se calculan en TLS. También puede estar vacía si el Worker envía solicitudes dentro de la misma zona o a una zona que no está detrás de un proxy (o a un tercero).
...
<ResponseField name="event.transaction" type="dictionary" post={["optional"]}>
  ...
  <Expandable title="propiedades de la transacción" defaultOpen>
    ...
    <ResponseField name="locale" type="string">
      El locale que se usará para esta transacción, determinado al comparar los idiomas solicitados por el navegador con la configuración de idioma del tenant.
    </ResponseField>
    ...
    <ResponseField name="prompt" type="array of strings" post={["optional"]}>
      Lista de instrucciones que indican si se le puede solicitar al usuario reautenticación y consentimiento.
    </ResponseField>
