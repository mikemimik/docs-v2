...
#
description: Conozca el objeto event del trigger Registro previo de usuario de Actions, que proporciona información contextual sobre la ejecución del trigger.
title: 'Triggers de Actions: Registro previo de usuario - Objeto event'
validatedOn: 2026-05-25
---

El objeto `event` del trigger Registro previo de usuario de Actions proporciona información contextual sobre la ejecución del trigger.
        ...
        <ResponseField name="supplemental" type="dictionary" post={["optional"]}>
          ...
          <Expandable title="propiedades de supplemental">
            <ResponseField name="akamai" type="dictionary" post={["optional"]}>
              &#91;Acceso anticipado&#93; Evaluación de riesgo suplementaria. Esto está disponible solo si Akamai Account Protector está habilitado y Akamai reenvía los encabezados de la transacción.
...
<ResponseField name="event.request" type="dictionary">
  ...
  <Expandable title="propiedades de la solicitud" defaultOpen>
    <ResponseField name="body" type="dictionary">
      El cuerpo de la solicitud POST. Estos datos solo estarán disponibles durante los flujos de token de actualización, de intercambio de credenciales de cliente y en la Action de Registro previo de usuario.
    </ResponseField>
    ...
    <ResponseField name="hostname" type="string" post={["optional"]}>
      El nombre de host que se está usando para el flujo de autenticación.
    </ResponseField>
...
<ResponseField name="event.security_context" type="object" post={["optional"]}>
  Un objeto que contiene firmas de huella digital. Esto solo estará disponible si el cliente usa cloudflare. La huella digital JA3/JA4 puede ser nula o estar vacía en algunos casos. El caso más común es el de las solicitudes HTTP, porque JA3 y JA4 se calculan en TLS. También puede estar vacía porque el Worker envía solicitudes dentro de la misma zona o a una zona que no pasa por proxy (o a un tercero).
...
<ResponseField name="event.transaction" type="dictionary" post={["optional"]}>
  ...
  <Expandable title="propiedades de la transacción" defaultOpen>
    ...
    <ResponseField name="prompt" type="array of strings" post={["optional"]}>
      Lista de instrucciones que indican si se puede pedir al usuario que realice la reautenticación y otorgue su consentimiento.
    </ResponseField>
    ...
    <ResponseField name="response_mode" type="string" post={["optional"]}>
      Informa al servidor de autorización qué mecanismo se usará para devolver parámetros desde el endpoint de autorización.
