...
#
description: Conozca el objeto `event` del trigger de Action post-user-registration, que proporciona información contextual sobre la ejecución del trigger.
...
<ResponseField name="event.request" type="dictionary" post={["optional"]}>
  ...
  <Expandable title="propiedades de la solicitud" defaultOpen>
    ...
    <ResponseField name="hostname" type="string" post={["optional"]}>
      El hostname que se está utilizando para el flujo de autenticación.
    </ResponseField>
    ...
    <ResponseField name="method" type="string">
      El método HTTP usado en la solicitud
    </ResponseField>
...
<ResponseField name="event.security_context" type="object" post={["optional"]}>
  Un objeto que contiene firmas de huella digital. Solo estará disponible si el cliente usa cloudflare. La huella digital JA3/JA4 puede ser null o estar vacía en algunos casos. El caso más común se da con las peticiones HTTP, porque JA3 y JA4 se calculan en TLS. También puede estar vacía porque el Worker envía peticiones dentro de la misma zona o a una zona que no pasa por proxy (o a un tercero).
...
<ResponseField name="event.transaction" type="dictionary" post={["optional"]}>
  ...
  <Expandable title="propiedades de la transacción" defaultOpen>
    ...
    <ResponseField name="locale" type="string">
      El locale que se utilizará para esta transacción, determinado al comparar los idiomas solicitados por el navegador con la configuración de idioma del tenant.
    </ResponseField>
    ...
    <ResponseField name="prompt" type="array of strings" post={["optional"]}>
      Lista de instrucciones que indica si se puede solicitar al usuario una reautenticación y consentimiento.
    </ResponseField>
...
<ResponseField name="event.user" type="dictionary">
  ...
  <Expandable title="propiedades del usuario" defaultOpen>
    ...
    <ResponseField name="user_metadata" type="dictionary">
      Campos personalizados que almacenan información sobre un usuario que no afecta a lo que puede o no puede hacer, como la dirección del trabajo, la dirección del hogar o las preferencias del usuario.
    </ResponseField>
