---
#
# THIS FILE IS AUTO-GENERATED.
# DO NOT EDIT MANUALLY.
#
description: Obtén información sobre el objeto `event` del trigger post-user-registration de Actions, que proporciona información contextual sobre la ejecución del trigger.
title: 'Actions Triggers: post-user-registration - Objeto `event`'
validatedOn: 2026-05-25
---

El objeto `event` del trigger post-user-registration de Actions proporciona información contextual sobre la ejecución del trigger.

<div id="eventauthentication">
  ## `event.authentication`
</div>

<ResponseField name="event.authentication" type="dictionary" post={["optional"]}>
  Detalles sobre las señales complementarias de autenticación obtenidas durante el flujo de registro.

  <Expandable title="propiedades de autenticación" defaultOpen>
    <ResponseField name="riskAssessment" type="dictionary" post={["optional"]}>
      Detalles sobre la información de las evaluaciones de riesgo para distintos flujos.

      <Expandable title="propiedades de riskAssessment" defaultOpen>
        <ResponseField name="supplemental" type="dictionary" post={["optional"]}>
          Señales complementarias enviadas por proveedores externos para ayudar con las evaluaciones de riesgo.

          <Expandable title="propiedades de supplemental">
            <ResponseField name="akamai" type="dictionary" post={["optional"]}>
              &#91;Acceso anticipado&#93; Evaluación complementaria del riesgo. Solo está disponible si Akamai Account Protector está habilitado y Akamai reenvía los encabezados de la transacción.

              <Expandable title="propiedades de akamai">
                <ResponseField name="akamaiBot" type="object" post={["optional"]}>
                  Los resultados de detección de bots reenviados por Akamai Bot Manager.

                  <Expandable title="propiedades de akamaiBot">
                    <ResponseField name="type" type="string" post={["optional"]}>
                      El tipo de resultado de Akamai Bot Manager.
                    </ResponseField>

                    <ResponseField name="action" type="string" post={["optional"]}>
                      La acción del resultado de Akamai Bot Manager.
                    </ResponseField>

                    <ResponseField name="botCategory" type="array of strings" post={["optional"]}>
                      La categoría de bots del resultado de Akamai Bot Manager.
                    </ResponseField>

                    <ResponseField name="botScore" type="number" post={["optional"]}>
                      La puntuación de bots del resultado de Akamai Bot Manager.
                    </ResponseField>

                    <ResponseField name="botScoreResponseSegment" type="string" post={["optional"]}>
                      El segmento de respuesta de la puntuación de bots del resultado de Akamai Bot Manager.
                    </ResponseField>

                    <ResponseField name="botnetId" type="string" post={["optional"]}>
                      El ID de la botnet del resultado de Akamai Bot Manager.
                    </ResponseField>
                  </Expandable>
                </ResponseField>

                <ResponseField name="akamaiUserRisk" type="dictionary" post={["optional"]}>
                  Los resultados de detección de riesgo del usuario reenviados por Akamai Account Protector.

                  <Expandable title="propiedades de akamaiUserRisk">
                    <ResponseField name="action" type="string" post={["optional"]}>
                      La acción de la evaluación del riesgo del usuario de Akamai.
                    </ResponseField>

                    <ResponseField name="allow" type="number" post={["optional"]}>
                      El estado de autorización de la evaluación del riesgo del usuario de Akamai.
                    </ResponseField>

                    <ResponseField name="emailDomain" type="string" post={["optional"]}>
                      El dominio de correo electrónico del usuario.
                    </ResponseField>

                    <ResponseField name="general" type="dictionary" post={["optional"]}>
                      El riesgo general de la evaluación del riesgo del usuario de Akamai.
                    </ResponseField>

                    <ResponseField name="ouid" type="string" post={["optional"]}>
                      El OUID del usuario.
                    </ResponseField>

                    <ResponseField name="requestid" type="string" post={["optional"]}>
                      El ID de solicitud del usuario.
                    </ResponseField>

                    <ResponseField name="risk" type="dictionary" post={["optional"]}>
                      El riesgo de la evaluación del riesgo del usuario de Akamai.
                    </ResponseField>

                    <ResponseField name="score" type="number" post={["optional"]}>
                      La puntuación de la evaluación del riesgo del usuario de Akamai.
                    </ResponseField>

                    <ResponseField name="status" type="number" post={["optional"]}>
                      El estado de la evaluación del riesgo del usuario de Akamai.
                    </ResponseField>

                    <ResponseField name="trust" type="dictionary" post={["optional"]}>
                      El nivel de confianza de la evaluación del riesgo del usuario de Akamai.
                    </ResponseField>

                    <ResponseField name="username" type="string" post={["optional"]}>
                      El nombre de usuario del usuario.
                    </ResponseField>

                    <ResponseField name="uuid" type="string" post={["optional"]}>
                      El UUID de la evaluación del riesgo del usuario de Akamai.
                    </ResponseField>
                  </Expandable>
                </ResponseField>
              </Expandable>
            </ResponseField>
          </Expandable>
        </ResponseField>
      </Expandable>
    </ResponseField>
  </Expandable>
</ResponseField>

<div id="eventconnection">
  ## `event.connection`
</div>

<ResponseField name="event.connection" type="dictionary">
  Detalles de la conexión utilizada para registrar al usuario.

  <Expandable title="propiedades de la conexión" defaultOpen>
    <ResponseField name="id" type="string">
      El identificador único de la conexión.
    </ResponseField>

    <ResponseField name="metadata" type="dictionary" post={["optional"]}>
      Metadato asociada a la conexión.
    </ResponseField>

    <ResponseField name="name" type="string">
      El nombre de la conexión utilizada para autenticar al usuario (como `twitter` o `some-g-suite-domain`).
    </ResponseField>

    <ResponseField name="strategy" type="string">
      El tipo de conexión. En las conexiones sociales, `event.connection.strategy === event.connection.name`. En las conexiones empresariales, la estrategia es `waad` (Windows Azure AD), `ad` (Active Directory/LDAP), `auth0` (conexiones de base de datos), etc.
    </ResponseField>
  </Expandable>
</ResponseField>

<div id="eventcustom_domain">
  ## `event.custom_domain`
</div>

<ResponseField name="event.custom_domain" type="dictionary" post={["optional"]}>
  Información sobre el dominio personalizado asociado a la transacción actual.

  <Expandable title="propiedades de custom_domain" defaultOpen>
    <ResponseField name="domain" type="string">
      El nombre del dominio personalizado.
    </ResponseField>

    <ResponseField name="domain_metadata" type="dictionary">
      Metadatos del dominio personalizado en pares clave-valor.
    </ResponseField>
  </Expandable>
</ResponseField>

<div id="eventrequest">
  ## `event.request`
</div>

<ResponseField name="event.request" type="dictionary" post={["optional"]}>
  Detalles de la solicitud que inició la transacción.

  <Expandable title="propiedades de la solicitud" defaultOpen>
    <ResponseField name="geoip" type="dictionary">
      <Expandable title="propiedades de geoip" defaultOpen>
        <ResponseField name="cityName" type="string" post={["optional"]} />

        <ResponseField name="continentCode" type="string" post={["optional"]} />

        <ResponseField name="countryCode" type="string" post={["optional"]} />

        <ResponseField name="countryCode3" type="string" post={["optional"]} />

        <ResponseField name="countryName" type="string" post={["optional"]} />

        <ResponseField name="latitude" type="number" post={["optional"]} />

        <ResponseField name="longitude" type="number" post={["optional"]} />

        <ResponseField name="subdivisionCode" type="string" post={["optional"]} />

        <ResponseField name="subdivisionName" type="string" post={["optional"]} />

        <ResponseField name="timeZone" type="string" post={["optional"]} />
      </Expandable>
    </ResponseField>

    <ResponseField name="hostname" type="string" post={["optional"]}>
      El nombre de host que se usa para el flujo de autenticación.
    </ResponseField>

    <ResponseField name="ip" type="string">
      La dirección IP de origen de la solicitud.
    </ResponseField>

    <ResponseField name="language" type="string" post={["optional"]}>
      El idioma solicitado por el navegador.
    </ResponseField>

    <ResponseField name="method" type="string">
      El método HTTP utilizado para la solicitud
    </ResponseField>

    <ResponseField name="user_agent" type="string" post={["optional"]}>
      El valor del encabezado `User-Agent` recibido al iniciar la transacción.
    </ResponseField>
  </Expandable>
</ResponseField>

<div id="eventsecrets">
  ## `event.secrets`
</div>

<ResponseField name="event.secrets" type="secrets">
  Valores secretos asociados de forma segura a esta Action.
</ResponseField>

<div id="eventsecurity_context">
  ## `event.security_context`
</div>

<ResponseField name="event.security_context" type="object" post={["optional"]}>
  Un objeto que contiene firmas de huellas digitales. Esto solo estará disponible si el cliente usa Cloudflare. La huella digital JA3/JA4 puede ser nula o estar vacía en algunos casos. El caso más habitual se da en las solicitudes HTTP, porque JA3 y JA4 se calculan en TLS. También puede estar vacía porque el Worker envía solicitudes dentro de la misma zona o a una zona que no pasa por proxy (o a un tercero).

  <Expandable title="propiedades de security_context" defaultOpen>
    <ResponseField name="ja3" type="string" post={["optional"]}>
      Firma de huella digital JA3. Esto solo estará disponible si el cliente usa una conexión TLS.
    </ResponseField>

    <ResponseField name="ja4" type="string" post={["optional"]}>
      Firma de huella digital JA4. Esto solo estará disponible si el cliente usa una conexión TLS.
    </ResponseField>
  </Expandable>
</ResponseField>

<div id="eventtenant">
  ## `event.tenant`
</div>

<ResponseField name="event.tenant" type="object">
  Detalles del Tenant asociado a la transacción actual.

  <Expandable title="propiedades del tenant" defaultOpen>
    <ResponseField name="id" type="string">
      El nombre del tenant.
    </ResponseField>
  </Expandable>
</ResponseField>

<div id="eventtransaction">
  ## `event.transaction`
</div>

<ResponseField name="event.transaction" type="dictionary" post={["optional"]}>
  Detalles sobre la transacción actual.

  <Expandable title="propiedades de la transacción" defaultOpen>
    <ResponseField name="acr_values" type="array of strings">
      Cualquier `acr_values` proporcionado en la solicitud de autenticación original.
    </ResponseField>

    <ResponseField name="locale" type="string">
      La configuración regional que se usará para esta transacción, determinada al comparar los idiomas solicitados por el navegador con la configuración de idioma del tenant.
    </ResponseField>

    <ResponseField name="login_hint" type="string" post={["optional"]}>
      Indicación para el servidor de autorización sobre el identificador de inicio de sesión que el usuario final podría usar para iniciar sesión (si es necesario).
    </ResponseField>

    <ResponseField name="prompt" type="array of strings" post={["optional"]}>
      Lista de instrucciones que indica si se le puede solicitar al usuario una reautenticación y consentimiento.
    </ResponseField>

    <ResponseField name="protocol" type="string" post={["optional"]}>
      Valores posibles:

      * `oidc-basic-profile`
      * `oidc-ciba`
      * `oidc-ciba-web-link`
      * `oidc-implicit-profile`
      * `oauth2-device-code`
      * `oauth2-resource-owner`
      * `oauth2-resource-owner-jwt-bearer`
      * `oauth2-password`
      * `oauth2-webauthn`
      * `oauth2-access-token`
      * `oauth2-refresh-token`
      * `oauth2-token-exchange`
      * `oidc-hybrid-profile`
      * `samlp`
      * `wsfed`
      * `wstrust-usernamemixed`
    </ResponseField>

    <ResponseField name="redirect_uri" type="string" post={["optional"]}>
      La URL a la que Auth0 redirigirá el navegador después de que se complete la transacción.
    </ResponseField>

    <ResponseField name="requested_scopes" type="array of strings">
      Los scopes solicitados (si los hay) al iniciar este flujo de autenticación.
    </ResponseField>

    <ResponseField name="response_mode" type="string" post={["optional"]}>
      Informa al servidor de autorización del mecanismo que se usará para devolver parámetros desde el endpoint de autorización.

      Valores permitidos: `query`, `fragment`, `form_post`, `web_message`
    </ResponseField>

    <ResponseField name="response_type" type="array of strings" post={["optional"]}>
      Indica el tipo de credencial que Auth0 devolverá.
    </ResponseField>

    <ResponseField name="state" type="string" post={["optional"]}>
      Una cadena alfanumérica arbitraria y opaca que tu aplicación agrega a la solicitud inicial y que Auth0 incluye al redirigir de vuelta a tu aplicación.
    </ResponseField>

    <ResponseField name="ui_locales" type="array of strings">
      Los `ui_locales` proporcionados en la solicitud de autenticación original.
    </ResponseField>
  </Expandable>
</ResponseField>

<div id="eventuser">
  ## `event.user`
</div>

<ResponseField name="event.user" type="dictionary">
  Un objeto que describe al usuario en cuyo nombre se inició la transacción actual.

  <Expandable title="propiedades del usuario" defaultOpen>
    <ResponseField name="app_metadata" type="dictionary">
      Campos personalizados que almacenan información sobre un usuario que influye en su acceso, como el plan de soporte, los roles de seguridad o los grupos de control de acceso.
    </ResponseField>

    <ResponseField name="created_at" type="string">
      Marca de tiempo que indica cuándo se creó por primera vez el perfil del usuario.
    </ResponseField>

    <ResponseField name="email" type="string" post={["optional"]}>
      (único) Dirección de correo electrónico del usuario.
    </ResponseField>

    <ResponseField name="email_verified" type="boolean">
      Indica si el usuario ha verificado su dirección de correo electrónico.
    </ResponseField>

    <ResponseField name="family_name" type="string" post={["optional"]}>
      Apellido del usuario.
    </ResponseField>

    <ResponseField name="given_name" type="string" post={["optional"]}>
      Nombre de pila del usuario.
    </ResponseField>

    <ResponseField name="last_password_reset" type="string" post={["optional"]}>
      Marca de tiempo que indica la última vez que se restableció o cambió la contraseña del usuario. Al crear el usuario, este campo no existe. Esta propiedad solo está disponible para las conexiones de base de datos.
    </ResponseField>

    <ResponseField name="name" type="string" post={["optional"]}>
      Nombre completo del usuario.
    </ResponseField>

    <ResponseField name="nickname" type="string" post={["optional"]}>
      Apodo del usuario.
    </ResponseField>

    <ResponseField name="phone_number" type="string" post={["optional"]}>
      Número de teléfono del usuario.
    </ResponseField>

    <ResponseField name="phone_verified" type="boolean" post={["optional"]}>
      Indica si el usuario ha verificado su número de teléfono.
    </ResponseField>

    <ResponseField name="picture" type="string" post={["optional"]}>
      URL que apunta a la [foto de perfil del usuario](https://auth0.com/docs/users/change-user-picture).
    </ResponseField>

    <ResponseField name="updated_at" type="string">
      Marca de tiempo que indica cuándo se actualizó o modificó por última vez el perfil del usuario.
    </ResponseField>

    <ResponseField name="user_id" type="string">
      (único) Identificador único del usuario.
    </ResponseField>

    <ResponseField name="user_metadata" type="dictionary">
      Campos personalizados que almacenan información sobre un usuario que no afecta a aquello a lo que puede acceder, como la dirección del trabajo, la dirección de casa o las preferencias del usuario.
    </ResponseField>

    <ResponseField name="username" type="string" post={["optional"]}>
      (único) Nombre de usuario.
    </ResponseField>
  </Expandable>
</ResponseField>