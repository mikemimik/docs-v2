...
#
description: Obtén información sobre el objeto `event` del trigger custom-phone-provider de Actions, que proporciona información contextual sobre la ejecución del trigger.
title: 'Actions Triggers: custom-phone-provider - Objeto `event`'
validatedOn: 2026-04-28
---

El objeto `event` del trigger custom-phone-provider de Actions proporciona información contextual sobre la ejecución del trigger.
...
<ResponseField name="event.connection" type="dictionary" post={["optional"]}>
  ...
  <Expandable title="propiedades de la conexión" defaultOpen>
    ...
    <ResponseField name="metadata" type="dictionary" post={["optional"]}>
      Metadata asociada a la conexión.
    </ResponseField>
    ...
    <ResponseField name="strategy" type="string">
      El tipo de conexión. En las conexiones sociales, `event.connection.strategy === event.connection.name`. En las conexiones empresariales, la estrategia es `waad` (Windows Azure AD), `ad` (Active Directory/LDAP), `auth0` (Database Connections), entre otras.
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="event.tenant" type="object">
  <Expandable title="propiedades del tenant" defaultOpen>
    ...
    <ResponseField name="friendly_name" type="string" post={["optional"]}>
      El nombre descriptivo del tenant, normalmente una versión del ID más legible para las personas.
    </ResponseField>
...
<ResponseField name="event.transaction" type="object" post={["optional"]}>
  Detalles de la transacción actual para fines de trazabilidad.

  <Expandable title="propiedades de la transacción" defaultOpen>
    <ResponseField name="correlation_id" type="string" post={["optional"]}>
      Un identificador único para correlacionar esta solicitud en varios servicios con fines de trazabilidad distribuida.
    </ResponseField>
  </Expandable>
</ResponseField>
