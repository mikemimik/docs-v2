---
description: Aprende a usar componentes personalizados para crear campos que requieren lógica o una interfaz de usuario específicas con JavaScript, HTML y CSS.
title: Componentes de campo personalizado
---
...
Con el campo personalizado, puedes ampliar la apariencia y el comportamiento de tus formularios, y añadir lógica potente con JavaScript, HTML y CSS.

El campo personalizado tiene métodos internos para facilitar el envío de datos al formulario, añadir validaciones de frontend y backend, y manejar eventos comunes como el foco o la pérdida de foco.
...
<Warning>
  Para usar campos personalizados, debes habilitar [Custom Domains](/docs/es/customize/custom-domains). Si renderizas un formulario con un campo personalizado fuera de un dominio personalizado, se mostrará un error.
</Warning>
...
Agrega pares clave-valor para usarlos como referencia en el código fuente del campo personalizado. Los pares clave-valor pueden incluir [variables](/docs/es/customize/forms/variables) de campos del formulario.

<Frame>![](/docs/images/cdy7uua7fh8z/6VEtliKd20ZF4MJKhvB5Z0/e1b3b5df4860d013d1e3eb79d1174b35/params.png)</Frame>

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Los valores de los params solo están disponibles después de que el formulario llame al método `init()`.
</Callout>
...
Para cubrir requisitos de validación complejos, puede usar un [flujo](/docs/es/customize/forms/flows).
...
Se invoca cuando el foco sale del elemento HTML del campo personalizado.

<div id="getvalue">
  ### getValue()
</div>

Se invoca cuando el formulario necesita obtener el valor del campo personalizado una o varias veces. Normalmente, esto ocurre cuando el usuario envía el paso del formulario. Si necesitas realizar validaciones del lado del cliente, puedes lanzar un error para mostrar un mensaje de error personalizado al usuario.
...
Se invoca cuando se debe desbloquear el campo personalizado. Normalmente se ejecuta después de que el usuario envía el paso del formulario o de que los datos dejan de procesarse en nuestro back-end debido a un error de validación.

<div id="getscripts">
  ### getScripts()
</div>

Devuelve una lista de URL cuya carga el formulario garantiza que se habrá completado antes de que se invoque el método `init()`.
...
Al pasar un objeto `context`, puedes usar estos métodos para gestionar la lógica de tu formulario y tus componentes.
...
Obtiene el valor del campo personalizado actual.
...
Devuelve un identificador único para el campo personalizado actual.
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Los valores de los parámetros solo están disponibles una vez que el formulario invoca el método `init()`.
</Callout>
...
Devuelve un valor booleano si el formulario supera todas las validaciones en el cliente.

<div id="contextformvalidate">
  #### context.form.validate()
</div>

Valida los valores existentes de los campos mediante validación del lado del cliente antes de continuar. Si un campo no supera la validación, aparecerá un mensaje de error.
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  `setRequired()` solo marca o desmarca el campo como obligatorio en el cliente. Por ejemplo, si desmarcas un campo como obligatorio, pero en la configuración del campo está marcado como obligatorio, el formulario devolverá un error si el campo no tiene ningún valor.
</Callout>
...
A continuación, se muestran ejemplos de campos personalizados que puedes agregar a tus formularios:

<div id="range-input-custom-field">
  ### Campo personalizado de entrada de intervalo
</div>

Un campo personalizado que devuelve un valor dentro de un intervalo predeterminado.
...
<div id="autocomplete-input-custom-field-using-values-from-api">
  ### Campo personalizado de entrada con autocompletado que usa valores de la API
</div>
...
<div id="dynamic-dropdown-custom-field-using-values-from-api">
  ### Campo personalizado de lista desplegable dinámica con valores de una API
</div>
...
<div id="dynamic-input-custom-field-with-a-button-to-add-more-fields">
  ### Campo personalizado dinámico con un botón (+) para añadir más campos
</div>
