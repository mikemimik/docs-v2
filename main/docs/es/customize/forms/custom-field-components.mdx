---
description: Aprende a usar componentes personalizados para crear campos que requieran lógica específica o una UI con JavaScript, HTML y CSS.
...
Con el campo personalizado, puedes ampliar el aspecto y el comportamiento de tus formularios y añadir lógica avanzada con JavaScript, HTML y CSS.

El campo personalizado tiene métodos internos para facilitar el envío de datos al formulario, añadir validaciones en el frontend y el backend, y gestionar eventos comunes como foco o blur.
...
Agrega pares clave-valor para usarlos como referencia en el código fuente del campo personalizado. Los pares clave-valor pueden incluir [variables](/docs/es/customize/forms/variables) de los campos del formulario.
...
En el siguiente ejemplo, la configuración de parámetros del campo personalizado se rellena con los pares clave-valor `symbol={{fields.symbol}}` y `separator=,`
...
Puedes usar estos controladores para agregar comportamiento personalizado al campo:
...
Se invoca cuando el formulario necesita obtener el valor del campo personalizado una o varias veces. Esto suele ocurrir cuando el usuario envía el paso del formulario. Si necesitas realizar validaciones del lado del cliente, puedes generar un error para mostrarle al usuario un mensaje de error personalizado.
...
Se invoca cuando el campo personalizado debe bloquearse. Normalmente se ejecuta cuando el usuario envía el paso del formulario y los datos se procesan en nuestro backend.
...
Devuelve una lista de URL que el form garantiza que se habrán cargado por completo antes de que se invoque el método `init()`.
...
Al pasar un objeto `context`, puedes usar estos métodos para controlar la lógica del formulario y sus componentes.
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Los valores de los parámetros solo están disponibles una vez que el formulario haya invocado el método `init()`.
</Callout>
...
Cuando necesites interactuar con el form para obtener valores de otros campos o navegar a otros pasos del form, puedes usar los siguientes métodos de Form:
...
Evalúa los valores existentes de los campos mediante validación del lado del cliente antes de continuar. Si un campo no supera la validación, aparecerá un mensaje de error.
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  `setRequired()` solo marca o desmarca el campo como obligatorio en el cliente. Por ejemplo, si desmarcas un campo como obligatorio, pero en la configuración del campo estaba marcado como obligatorio, el formulario devolverá un error si el campo no tiene ningún valor.
</Callout>
...
Las siguientes secciones incluyen ejemplos de campos personalizados que puedes agregar a tus formularios:
...
Un campo personalizado que devuelve un valor hexadecimal de un color.
