...
El cambio flexible entre conexiones es una funcionalidad opcional que permite a los usuarios elegir su método de autenticación al iniciar sesión en una aplicación. Cuando se implementa, la pantalla de inicio de sesión de tu aplicación presenta a los usuarios la opción de autenticarse con credenciales tradicionales de base de datos o con una conexión <Tooltip tip="" cta="Ver glosario" href="/docs/es/glossary?term=passwordless">passwordless</Tooltip>. Los usuarios que seleccionan una [conexión passwordless](/docs/es/authenticate/passwordless) reciben una contraseña de un solo uso (OTP) por correo electrónico o SMS que pueden usar para iniciar sesión en tu aplicación.

El cambio flexible entre conexiones usa [prompts personalizados de Universal Login](/docs/es/customize/login-pages/universal-login/customize-signup-and-login-prompts) para ofrecer a los usuarios una experiencia de autenticación con más autonomía.

<div id="before-you-begin">
  ## Antes de comenzar
</div>

Antes de implementar el cambio flexible entre conexiones, asegúrate de cumplir los siguientes requisitos:
...
* Configura un [custom domain](/docs/es/customize/custom-domains).
* Habilita lo siguiente para tu aplicación:
  ...
  * [autenticación passwordless con Email o SMS](/docs/es/authenticate/login/auth0-universal-login/passwordless-login/email-or-sms)
...
| ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- |
| `state`      | Renderiza el valor de `state` de la página actual, que es opaco y se utiliza con fines de seguridad.<br /><br />Para obtener más información sobre la información de la pantalla actual, consulta [Personalizar las plantillas de página de Universal Login](/docs/es/customize/login-pages/universal-login/customize-templates#current-screen-information). | `<input type='hidden' name='state' value='{{state}}'>`  |
...
Puedes configurar el prompt `signup-password` mediante el endpoint [Establecer partials para un prompt](https://auth0.com/docs/api/management/v2/prompts/put-partials):
...
Como resultado, se añade un **botón Enviar un código seguro por correo electrónico** a la pantalla `signup-password`. Cuando un usuario selecciona este botón, se envía el cuerpo del formulario, que contiene el parámetro state del inicio de sesión y el nombre de la conexión deseada.

<div id="configure-login-prompts">
  ### Configurar prompts de inicio de sesión
</div>

Para obtener mejores resultados, se recomienda configurar el prompt de inicio de sesión tanto para conexiones con contraseña como para conexiones passwordless.

Puede configurar el prompt `login-password` mediante el endpoint [Establecer partials para un prompt](https://auth0.com/docs/api/management/v2/prompts/put-partials):
...
Como resultado, se agrega un **botón Enviar un código seguro por correo electrónico** a la pantalla `login-password`. Cuando un usuario selecciona este botón, se envía un form body que contiene el parámetro state del inicio de sesión y el nombre de la conexión deseada.
