---
description: Aprende cómo implementar el cambio flexible entre conexiones con Universal Login
...
* Habilite lo siguiente para su aplicación:
  ...
  * [Identifier First authentication](/docs/es/authenticate/login/auth0-universal-login/identifier-first)
...
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- |
| `state`      | Renderiza el valor `state` de la página actual, que es opaco y se usa con fines de seguridad.<br /><br />Para obtener más información sobre la pantalla actual, consulta [Personaliza las plantillas de página de Universal Login](/docs/es/customize/login-pages/universal-login/customize-templates#current-screen-information). | `<input type='hidden' name='state' value='{{state}}'>`  |
| `connection` | Nombre y tipo de la conexión.<br /><br />En las conexiones passwordless, el valor es `email` o `sms`.                                                                                                                                                                                                                           | `<input type='hidden' name='connection' value='email'>` |

<Warning>
  En los ejemplos de código a continuación, asegúrate de reemplazar los marcadores de posición por los valores correspondientes:
...
Puedes configurar el prompt `signup-password` usando el endpoint [Establecer partials para un prompt](https://auth0.com/docs/api/management/v2/prompts/put-partials):
...
Como resultado, se añade un **botón Enviar un código seguro por correo electrónico** a la pantalla `signup-password`. Cuando un usuario selecciona este botón, se envía un cuerpo del formulario que contiene el parámetro state del inicio de sesión y el nombre de la conexión deseada.

<div id="configure-login-prompts">
  ### Configurar los prompts de inicio de sesión
</div>

Para obtener los mejores resultados, se recomienda configurar el prompt de inicio de sesión tanto para las conexiones con contraseña como para las conexiones passwordless.

Puedes configurar el prompt `login-password` mediante el endpoint [Establecer partials para un prompt](https://auth0.com/docs/api/management/v2/prompts/put-partials):
...
Como resultado, se añade un **botón Enviar un código seguro por correo electrónico** a la pantalla `login-password`. Cuando el usuario selecciona este botón, se envía el cuerpo del formulario, que contiene el parámetro state del inicio de sesión y el nombre de la conexión deseada.
