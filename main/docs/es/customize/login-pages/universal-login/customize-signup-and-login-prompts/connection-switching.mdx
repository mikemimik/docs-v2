...
* Utilice [Universal Login](/docs/es/authenticate/login/auth0-universal-login).

  * Esta función no está disponible para las páginas de inicio de sesión personalizadas.
* Configure un [dominio personalizado](/docs/es/customize/custom-domains).
* Habilite lo siguiente en su aplicación:
...
Para implementar esta función, usa la [Auth0 Management API](https://auth0.com/docs/api/management/v2) para configurar partials personalizados para los prompts de Signup y login. Los partials son fragmentos de código personalizado que se insertan en un punto de entrada dentro de una pantalla de prompt, como la pantalla de inicio de sesión. Para obtener más información, consulta [Personalizar los prompts de Signup y login](/docs/es/customize/login-pages/universal-login/customize-signup-and-login-prompts).
...
<div id="configure-signup-prompt">
  ### Configurar el prompt de registro
</div>
...
Como resultado, se agrega un **botón Enviar un código seguro por correo electrónico** a la pantalla `login-password`. Cuando un usuario selecciona este botón, se envía un cuerpo del formulario que contiene el parámetro state del inicio de sesión y el nombre de la conexión deseada.

De forma similar, puedes configurar el `prompt` `login-passwordless` mediante el endpoint [Establecer partials para un prompt](https://auth0.com/docs/api/management/v2/prompts/put-partials):
