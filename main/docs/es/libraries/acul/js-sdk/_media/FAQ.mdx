...
A continuación, se presentan una serie de preguntas o problemas que han surgido al usar el SDK.
...
1. **Método `getErrors` para errores específicos de la pantalla**
   Se puede usar una función getter de nivel superior, `getErrors`, para recuperar errores detallados de autenticación cuando falla la validación del backend.
   ```javascript
      import Login from "@auth0/auth0-acul-js/login";
...
2. **Errores específicos de la pantalla**
   Cada pantalla incluye una propiedad `transaction.errors`, que devuelve una lista de errores específicos de esa pantalla. Estos errores se generan después de la validación final de la petición, lo que te ayuda a identificar problemas que requieren atención.
...
<div id="can-i-build-a-reactjs-application-with-this-sdk">
  ## ¿Puedo crear una aplicación en ReactJS con este SDK?
</div>

Sí, este SDK puede integrarse fácilmente en una aplicación en ReactJS.
