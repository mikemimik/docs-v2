...
A continuación, se presentan varias preguntas o problemas que han surgido al usar el SDK.
...
1. **Método `getErrors` para errores específicos de la pantalla**
   Se puede usar una función de nivel superior, `getErrors`, para recuperar errores detallados de autenticación cuando falla la validación del backend.
   ```javascript
      import Login from "@auth0/auth0-acul-js/login";
...
Actualmente, este SDK solo está disponible para instalarse a través del registro de npm y no se puede usar directamente desde una CDN.
...
<div id="can-i-build-a-reactjs-application-with-this-sdk">
  ## ¿Puedo desarrollar una aplicación en ReactJS con este SDK?
</div>

Sí, este SDK se puede integrar fácilmente en una aplicación de ReactJS.

Aunque el SDK está escrito en JavaScript puro, es totalmente compatible con todos los frameworks basados en JavaScript, como React, Angular, Vue y otros. Puedes usar el SDK sin problemas dentro de tus componentes o hooks de React.