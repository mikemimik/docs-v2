...
Este documento forma parte del escenario de arquitectura Mobile + API y explica cómo implementar la aplicación móvil para Android. Consulta el escenario para obtener información sobre la solución implementada.
...
Descarga un proyecto de ejemplo para este tutorial y empieza de inmediato.

[Descargar](./timesheets-mobile_android.zip)
[Hacer un fork en GitHub](https://github.com/auth0-samples/auth0-pnp-exampleco-timesheets/tree/master/timesheets-mobile/android)
...
* [JWTDecode.Android](https://github.com/auth0/JWTDecode.Android): este paquete ayudará a decodificar <Tooltip tip="JSON Web Token (JWT): formato estándar de ID Token (y, a menudo, formato de token de acceso) que se utiliza para representar claims de forma segura entre dos partes." cta="Ver glosario" href="/docs/es/glossary?term=JWTs">JWTs</Tooltip>.
* AppCompat: este paquete nos permite usar el widget de barra de herramientas para navegar entre nuestras actividades.
...
Abre el archivo `AndroidManifest.xml` de la aplicación y añade el permiso de Internet:
...
<div id="set-configuration-values">
  ### Configurar los valores de configuración
</div>

Configura el <Tooltip tip="Client ID: Valor de identificación proporcionado por Auth0 a tu recurso registrado." cta="Ver glosario" href="/docs/es/glossary?term=Client+ID">Client ID</Tooltip>, el dominio de Auth0 y la URL de la API en el recurso `strings.xml`, ubicado en `/res/values/strings.xml`:
...
Para esta implementación, crea directorios para actividades, modelos y utilidades dentro del paquete de la aplicación.
...
Abre el archivo `AndroidManifest.xml` de la app y añade `LoginActivity`:
...
Antes de inicializar `WebAuthProvider` en el método `login()`, podemos crear `CredentialsManager`. Al pasar un `AuthenticationAPIClient` a `CredentialsManager`, este puede renovar los <Tooltip tip="Token de acceso: credencial de autorización, en forma de una cadena opaca o JWT, utilizada para acceder a una API." cta="Ver glosario" href="/docs/es/glossary?term=Access+Tokens">tokens de acceso</Tooltip> si han expirado.
...
A continuación, actualiza el método `login()` en `LoginActivity` para recuperar el <Tooltip tip="ID Token: Credencial destinada al propio cliente, en lugar de para acceder a un recurso." cta="Ver glosario" href="/docs/es/glossary?term=ID+Token">ID Token</Tooltip> y obtener el perfil del usuario a partir de este con la biblioteca JWTDecode.Android. Después, almacena el perfil del usuario con `UserProfileManager`.
...
<div id="step-4-display-ui-elements-conditionally-based-on-scope">
  ## Paso 4. Mostrar elementos de la IU de forma condicional según el scope
</div>
...
Primero, podemos actualizar la clase `User` para almacenar los scopes otorgados y, a continuación, proporcionar un método auxiliar, `hasScope()`, que puede usarse para determinar si los scopes otorgados incluyen un scope específico:
...
Recuerda también actualizar el `UserProfileManager` para que almacene el campo adicional:
...
A continuación, actualiza `LoginActivity` para pasar también el `scope` y que pueda almacenarse en el objeto `User`:
...
Ahora podemos mostrar ciertos elementos de la interfaz de usuario según si al usuario se le concedió un scope concreto. Por ejemplo, tenemos un elemento del menú de aprobación que solo debería ser visible para los usuarios a quienes se les haya concedido el scope `approve:timesheets`.

A continuación, puedes ver el código de la clase `BaseActivity` que comprueba si un usuario tiene el scope `approve:timesheets` y, en función de eso, establece la visibilidad del elemento de menú que muestra la actividad de aprobación:
...
<div id="create-the-timesheets-activity-layouts">
  ### Crea los diseños de actividad de Timesheets
</div>

A continuación, crea `timesheet_activity.xml`, el diseño de `TimeSheetsActivity`:
...
Y, para la navegación de la barra de herramientas en `TimeSheetActivity`, crearemos el recurso de menú `timesheet_action_menu.xml` (`/res/menu/`):
...
Crea un modelo para trabajar con los datos del registro de horas en nuestras vistas:
...
`TimeSheetAdapter` es una clase de utilidad que toma un array de entradas de registro de horas y las aplica a la `ListView` de la `TimeSheetActivity`.
...
La `TimeSheetActivity` muestra las entradas del registro de horas del usuario que ha iniciado sesión y que están almacenadas en el servidor.
...
* El método `callAPI()` recupera los registros de horas desde la API de Node.JS.
...
Para mostrar el perfil del usuario que ha iniciado sesión, crearemos `UserActivity`, el diseño `user_activity.xml` correspondiente y `user_action_menu.xml` para la navegación de la barra de herramientas. La vista mostrará el nombre, el correo electrónico y la foto de perfil del usuario.
...
<div id="create-the-user-activity-layouts">
  #### Crea los diseños de actividad de usuario
</div>
...
Y crea el `user_actions_menu.xml` para la barra de herramientas de `UserActivity`:
...
En el método `onCreate()` recuperaremos la información del usuario desde `UserProfileManager` y estableceremos los valores en la interfaz. Como antes, los métodos `onCreateOptionsMenu()` y `onOptionsItemSelected()` se encargan de la funcionalidad de navegación del widget Toolbar.
...
A continuación, crea `FormActivity` y el diseño para añadir nuevos registros de horas.
...
<div id="create-the-form-activity-layouts">
  ### Crear los diseños de la actividad de formulario
</div>

Crea el diseño `form_activity.xml` con `EditText` para los campos de nombre del proyecto y horas trabajadas, y un `DatePicker` para la fecha trabajada.
...
Y crea el `form_actions_menu.xml` para la barra de herramientas de `FormActivity`:
...
* `@string/api_url` está configurado como `http://10.0.2.2:8080/timesheets` para que el emulador de Android pueda conectarse a la API de Node.JS que se ejecuta en `http://localhost:8080`.
...
1. Inicia la API: ve al directorio de la API en tu terminal e introduce el comando `node server`.
2. Abre la aplicación móvil en Android Studio y pulsa el botón **Run**.
3. Selecciona el dispositivo virtual Nexus 5X API 23.
4. Una vez que el emulador haya cargado la aplicación móvil, puedes iniciar sesión y luego crear y ver registros de horas desde la API de Node.JS en ejecución.
