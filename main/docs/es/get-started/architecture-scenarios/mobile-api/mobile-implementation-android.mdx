...
* [Auth0.Android](https://github.com/auth0/Auth0.Android): este package permite la integration con Auth0 para autenticar a los usuarios.
* [OkHttp](http://square.github.io/okhttp/): este package proporciona un cliente HTTP para hacer solicitudes a la API de Node.JS.
* [JWTDecode.Android](https://github.com/auth0/JWTDecode.Android): este package ayudará a decodificar <Tooltip tip="JSON Web Token (JWT): formato estándar de token de ID (y, con frecuencia, de token de acceso) que se usa para representar claims de forma segura entre dos partes." cta="Ver glosario" href="/docs/es/glossary?term=JWTs">JWT</Tooltip>.
* AppCompat: este package nos permite usar el widget Toolbar para la navegación en nuestras actividades.
...
Configura tu <Tooltip tip="ID de cliente: valor de identificación otorgado a tu recurso registrado por Auth0." cta="Ver glosario" href="/docs/es/glossary?term=Client+ID">ID de cliente</Tooltip>, el dominio de Auth0 y la URL de la API en el recurso `strings.xml` ubicado en `/res/values/strings.xml`:
...
Para esta implementación, cree los directorios de actividades, modelos y utilidades en el paquete de la aplicación.
...
Abre el archivo `AndroidManifest.xml` de la app y agrega la `LoginActivity`:
...
`LoginActivity` se encargará de la autorización del usuario y será la pantalla inicial que verán los usuarios. Crearemos un método `login()` para inicializar un `WebAuthProvider` e iniciar la autenticación. Asegúrate de proporcionar el scheme, la <Tooltip tip="Audience: Identificador único de la audiencia de un token emitido. Se denomina aud en un token; su valor contiene el ID de una aplicación (ID de cliente) para un token de ID o de una API (identificador de API) para un token de acceso." cta="Ver glosario" href="/docs/es/glossary?term=audience">audience</Tooltip> y el scope correctos al `WebAuthProvider`. Para esta implementación usaremos:

* **scheme**: `demo`
* **audience**: `https://api.exampleco.com/timesheet`s (la API de Node.JS)
* **response&#95;type**: `code`
* **scope**: `create:timesheets read:timesheets openid profile email offline_access`. Estos scopes nos permitirán hacer solicitudes `POST` y `GET` a la API de Node.JS, así como recuperar el perfil del usuario y un <Tooltip tip="Refresh Token: Token utilizado para obtener un token de acceso renovado sin obligar a los usuarios a volver a iniciar sesión." cta="Ver glosario" href="/docs/es/glossary?term=Refresh+Token">token de actualización</Tooltip>.
...
En el método `login()`, si la autenticación se realiza correctamente, redirigiremos al usuario a `TimeSheetActivity`.
...
Antes de inicializar `WebAuthProvider` en el método `login()`, podemos crear `CredentialsManager`. Al pasar un `AuthenticationAPIClient` a `CredentialsManager`, este puede renovar los <Tooltip tip="Token de acceso: Credencial de autorización, en forma de una cadena opaca o JWT, que se usa para acceder a una API." cta="Ver glosario" href="/docs/es/glossary?term=Access+Tokens">tokens de acceso</Tooltip> si han expirado.
...
<div id="step-3-get-the-user-profile">
  ## Paso 3. Obtén el perfil del usuario
</div>
...
Cree un modelo de usuario sencillo que utilizarán `UserProfileManager` y `UserActivity`.
...
Para encargarnos de guardar la información del perfil del usuario, crearemos una clase `UserProfileManager`. `UserProfileManager` usará [SharedPreferences](https://developer.android.com/reference/android/content/SharedPreferences.html) para almacenar los datos.
...
A continuación, actualiza el método `login()` de `LoginActivity` para recuperar el <Tooltip tip="token de ID: credencial destinada al propio cliente, en lugar de para acceder a un recurso." cta="Ver glosario" href="/docs/es/glossary?term=ID+Token">token de ID</Tooltip> y obtener el perfil de usuario a partir del token con la biblioteca JWTDecode.Android. Luego, almacena el perfil de usuario con `UserProfileManager`.
...
Para determinar si un usuario tiene permisos para realizar ciertas acciones, podemos fijarnos en el `scope` que se le concedió durante el proceso de autenticación. El `scope` contendrá una cadena con todos los scopes concedidos al usuario, así que, para saber si se concedió un scope concreto, solo tenemos que comprobar si la cadena de scopes contiene la subcadena correspondiente a ese scope.

<div id="store-the-scope">
  ### Guardar el scope
</div>

Primero, podemos actualizar la clase `User` para almacenar los `scopes` concedidos y, luego, proporcionar un método auxiliar, `hasScope()`, que puede usarse para determinar si los `scopes` concedidos contienen un `scope` concreto:
...
Recuerda también actualizar `UserProfileManager` para guardar el campo adicional:
...
A continuación, actualiza `LoginActivity` para pasarle también el `scope`, de modo que pueda almacenarse en el objeto `User`:
...
Ahora podemos mostrar ciertos elementos de la UI en función de si al usuario se le concedió un `scope` determinado. Por ejemplo, tenemos un elemento del menú de aprobación que solo debería ser visible para los usuarios a quienes se les haya concedido el `scope` `approve:timesheets`.

A continuación puedes ver el código de la clase `BaseActivity` que comprueba si un usuario tiene el `scope` `approve:timesheets` y, en función de ello, establece la visibilidad del elemento del menú que muestra la actividad de aprobación:
...
Abre el `AndroidManifest.xml` de la aplicación y agrega `TimeSheetActivity`:
...
<div id="create-the-timesheets-activity-layouts">
  ### Cree los layouts de la actividad de hojas de hora
</div>

A continuación, cree `timesheet_activity.xml`, el layout de la `TimeSheetsActivity`:
...
El widget `ListView` contendrá entradas individuales representadas por el archivo de layout `item_entry.xml`:
...
Crea un modelo para trabajar con los datos de hojas de hora en nuestras vistas:
...
<div id="create-the-timesheet-adapter">
  ### Crear el adaptador de hoja de hora
</div>

`TimeSheetAdapter` es una clase de utilidad que toma un arreglo de entradas de hoja de hora y las aplica a la `ListView` de la `TimeSheetActivity`.
...
<div id="create-the-timesheet-activity">
  ### Crear la actividad de hoja de hora
</div>

La `TimeSheetActivity` muestra las entradas de la hoja de hora del usuario que ha iniciado sesión y que están almacenadas en el servidor.
...
* El método `callAPI()` recupera las hojas de hora desde la API de Node.JS.
...
Para mostrar el perfil del usuario que ha iniciado sesión, crearemos la `UserActivity`, el archivo de diseño `user_activity.xml` correspondiente y el `user_action_menu.xml` para la navegación de la Toolbar. La vista mostrará el nombre, el correo electrónico y la foto de perfil del usuario.
...
<div id="create-the-user-activity-layouts">
  #### Crea los layouts de la actividad de usuario
</div>

A continuación, crea `user_activity.xml`, el layout de `UserActivity`, con un `ImageView` para la foto de perfil y `TextViews` para el nombre y el correo electrónico del usuario.
...
Y cree el `user_actions_menu.xml` para la Toolbar de `UserActivity`:
...
En el método `onCreate()` obtendremos la información del usuario de `UserProfileManager` y estableceremos los valores en la vista. Como antes, los métodos `onCreateOptionsMenu()` y `onOptionsItemSelected()` gestionan la funcionalidad de navegación de la Toolbar.
...
A continuación, crea la `FormActivity` y el layout para gestionar la creación de nuevas entradas de hojas de hora.

<div id="update-the-manifest">
  ### Actualiza el manifiesto
</div>

Abre el archivo `AndroidManifest.xml` de la app y agrega `FormActivity`:
...
<div id="create-the-form-activity-layouts">
  ### Crea los layouts de la Form actividad
</div>

Crea el layout `form_activity.xml` con `EditText` para introducir el nombre del proyecto y las horas trabajadas, y un `DatePicker` para el día de trabajo.
...
<menu xmlns:android="http://schemas.android.com/apk/res/android"
    ...
    <item
        android:id="@+id/action_view"
        android:title="Ver hojas de hora"
        app:showAsAction="always" />
</menu>
```
...
* `@string/api_url` se configura como `http://10.0.2.2:8080/timesheets` para que el emulador de Android pueda conectarse a la API de Node.JS que se ejecuta en `http://localhost:8080`.
* El método `onCreate()` inicializa el formulario y recopila los datos de entrada para el método `postAPI()` cuando se pulsa el botón de envío.
* El método `postAPI()` enviará a la API de Node.JS, en formato JSON, los datos introducidos por el usuario en el formulario.
* El método `clearForm()` borra los datos introducidos en el formulario.
* Los métodos `onCreateOptionsMenu()` y `onOptionsItemSelected()` gestionan la funcionalidad de navegación del widget Toolbar.
...
4. Una vez que el emulador haya cargado la aplicación móvil, puedes iniciar sesión y luego crear y ver entradas de la hoja de hora desde la API de Node.JS en ejecución.
