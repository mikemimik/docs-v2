...
Descarga un proyecto de ejemplo de este tutorial para empezar.

[Descargar](./timesheets-mobile_android.zip)
[Crear un fork en Github](https://github.com/auth0-samples/auth0-pnp-exampleco-timesheets/tree/master/timesheets-mobile/android)
...
* [Auth0.Android](https://github.com/auth0/Auth0.Android): este paquete permite la integración con Auth0 para autenticar usuarios.
...
Configura tu <Tooltip tip="ID de cliente: valor de identificación asignado por Auth0 a tu recurso registrado." cta="Ver glosario" href="/docs/es/glossary?term=Client+ID">ID de cliente</Tooltip>, el dominio de Auth0 y la URL de la API en el recurso `strings.xml` ubicado en `/res/values/strings.xml`:
...
Para esta implementación, crea directorios para actividades, modelos y utilidades en el paquete de la aplicación.
...
Abre el `AndroidManifest.xml` de la aplicación y añade la `LoginActivity`:
...
Para almacenar las credenciales recibidas después de iniciar sesión, usaremos `CredentialsManager` de la biblioteca Auth0.Android y [SharedPreferences](https://developer.android.com/reference/android/content/SharedPreferences.html) como almacenamiento.

Antes de inicializar `WebAuthProvider` en el método `login()`, podemos crear `CredentialsManager`. Al pasar un `AuthenticationAPIClient` a `CredentialsManager`, este podrá renovar los <Tooltip tip="Token de acceso: Credencial de autorización, en forma de cadena opaca o JWT, que se utiliza para acceder a una API." cta="Ver glosario" href="/docs/es/glossary?term=Access+Tokens">tokens de acceso</Tooltip> si han expirado.
...
Ahora actualiza el método `login()` para que las credenciales se almacenen mediante `CredentialsManager` después de completar correctamente la autenticación.
...
<div id="step-3-get-the-user-profile">
  ## Paso 3. Obtén el perfil de usuario
</div>
...
Para almacenar la información del perfil del usuario, crearemos una clase llamada `UserProfileManager`. `UserProfileManager` usará [SharedPreferences](https://developer.android.com/reference/android/content/SharedPreferences.html) para almacenar los datos.
...
A continuación, actualiza el método `login()` en `LoginActivity` para obtener el <Tooltip tip="token de ID: credencial destinada al propio cliente, en lugar de acceder a un recurso." cta="Ver glosario" href="/docs/es/glossary?term=ID+Token">token de ID</Tooltip> y extraer el perfil del usuario a partir del token con la biblioteca JWTDecode.Android. Luego, almacena el perfil del usuario con `UserProfileManager`.
...
Para determinar si un usuario tiene permisos para realizar ciertas acciones, podemos revisar el `scope` que se le concedió durante el proceso de autenticación. El `scope` contendrá una cadena con todos los scopes concedidos al usuario, así que, para determinar si se concedió un scope concreto, solo tenemos que comprobar si la cadena de scopes contiene la subcadena correspondiente a ese scope.

<div id="store-the-scope">
  ### Almacenar los scopes
</div>

Primero, podemos actualizar la clase `User` para almacenar los scopes concedidos y, luego, proporcionar un método auxiliar, `hasScope()`, que puede usarse para determinar si los scopes concedidos contienen un scope en particular:
...
Recuerda también actualizar el `UserProfileManager` para almacenar el campo adicional:
...
A continuación, actualiza `LoginActivity` para pasar el `scope` y que pueda almacenarse en el objeto `User`:
...
Ahora podemos mostrar ciertos elementos de la UI en función de si al usuario se le concedió un scope concreto. Por ejemplo, tenemos un elemento del menú de aprobación que solo debe ser visible para los usuarios a quienes se les haya concedido el scope `approve:timesheets`.
...
Abre el archivo `AndroidManifest.xml` de la app y agrega `TimeSheetActivity`:
...
<div id="create-the-timesheets-activity-layouts">
  ### Crea los layouts de la actividad hojas de hora
</div>

A continuación, crea `timesheet_activity.xml`, que es el layout de `TimeSheetsActivity`:
...
El widget `ListView` contendrá entradas individuales representadas en el layout `item_entry.xml`:
...
Crea un modelo para trabajar con los datos de la hoja de hora en nuestras vistas:
...
`TimeSheetAdapter` es una clase de utilidad que toma un arreglo de entradas de hoja de hora y las carga en el `ListView` de la `TimeSheetActivity`.
...
La `TimeSheetActivity` muestra las entradas de la hoja de hora del usuario que ha iniciado sesión, las cuales están almacenadas en el servidor.
...
* El método `callAPI()` recupera hojas de hora desde la API de Node.JS.
...
Para mostrar el perfil del usuario que ha iniciado sesión, crearemos `UserActivity`, el `layout` correspondiente `user_activity.xml` y `user_action_menu.xml` para la navegación de la Toolbar. La vista mostrará el nombre, el correo electrónico y la foto de perfil del usuario.

<div id="update-the-manifest">
  ### Actualiza el manifiesto
</div>

Abre el `AndroidManifest.xml` de la aplicación y agrega la `UserActivity`:
...
<div id="create-the-user-activity-layouts">
  #### Crear los layouts de la actividad de usuario
</div>
...
Y crea `user_actions_menu.xml` para la Toolbar de `UserActivity`:
...
<div id="load-the-profile-picture-from-url">
  ### Cargar la foto de perfil desde una URL
</div>

Para cargar la foto de perfil del usuario desde una URL, cree una tarea que herede de `AsyncTask` y se ejecute en segundo plano.
...
En el método `onCreate()` recuperaremos la información del usuario desde `UserProfileManager` y asignaremos los valores en la vista. Como antes, los métodos `onCreateOptionsMenu()` y `onOptionsItemSelected()` se encargan de la funcionalidad de navegación del widget Toolbar.
...
A continuación, crea `FormActivity` y el `layout` para gestionar la creación de nuevas entradas de hoja de hora.
...
<div id="create-the-form-activity-layouts">
  ### Crea los layouts de la actividad Form
</div>

Crea el layout `form_activity.xml` con `EditText` para introducir el nombre del proyecto y las horas trabajadas, y un `DatePicker` para el día trabajado.
...
Y crea `form_actions_menu.xml` para la Toolbar de `FormActivity`:
...
1. Inicia la API; para ello, ve al directorio de la API en tu terminal e introduce el comando `node server`.
2. Abre la aplicación móvil en Android Studio y pulsa el botón **Run**.
3. Selecciona el dispositivo virtual Nexus 5X API 23.
4. Cuando el emulador haya cargado la aplicación móvil, podrás iniciar sesión y luego crear y ver entradas de hoja de hora desde la API de Node.JS en ejecución.
