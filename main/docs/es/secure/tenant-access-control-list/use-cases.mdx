...
description: Implemente casos de uso comunes de la ACL del tenant con estos ejemplos de código para solicitudes a la Management API, SDK y CLI de Auth0, y el proveedor Terraform de Auth0.
---
...
Este ejemplo de rule de ACL del tenant bloquea el tráfico entrante procedente de un país específico según su código de geolocalización.

<Tabs>
  <Tab title="Management API">
    Para crear esta rule de ACL del tenant con la Management API:
...
Este ejemplo de rule de ACL del tenant permite tráfico solo desde un código de país de geolocalización específico.
...
<Tabs>
  <Tab title="Management API">
    ...
    2. Llama al [endpoint Create access control list de la Management API](https://auth0.com/docs/api/management/v2/network-acls/post-network-acls) con el siguiente cuerpo:
...
Este ejemplo de rule de ACL del tenant redirige todo el tráfico procedente de un código de país de geolocalización específico.
...
Puedes combinar los operadores `match` y `not_match` en una sola rule de ACL del tenant para aplicar políticas de acceso granulares.

Este ejemplo de rule de ACL del tenant evalúa las señales `geo_country_code` y `geo_subdivision_code` para bloquear todo el tráfico de un país determinado, salvo el de un estado, región o provincia específicos dentro de ese país.
...
<div id="enforce-traffic-through-specific-infrastructure">
  ## Forzar que el tráfico pase por una infraestructura específica
</div>
...
Este ejemplo de rule de ACL del tenant bloquea el acceso a tus dominios canónicos y personalizados, a menos que la solicitud se origine en un conjunto específico de direcciones IP que se conectan directamente al edge de Auth0. Esto evita que los usuarios eludan tus controles de seguridad accediendo directamente a los nombres de host de tu tenant desde internet pública.
