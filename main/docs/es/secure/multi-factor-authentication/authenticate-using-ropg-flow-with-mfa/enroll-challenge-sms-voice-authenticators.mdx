...
title: Inscribir y verificar autenticadores de SMS y voz
---
...
Según cuándo actives la inscripción, puedes obtener un <Tooltip tip="Token de acceso: credencial de autorización, en forma de una cadena opaca o JWT, utilizada para acceder a una API." cta="Ver glosario" href="/docs/es/glossary?term=access+token">token de acceso</Tooltip> para usar la API de MFA de distintas maneras:
...
* Si quieres permitir que el usuario registre un factor en cualquier momento, consulta [Administrar las inscripciones de factores de MFA](/docs/es/secure/multi-factor-authentication/manage-mfa-auth0-apis/manage-authenticator-factors-mfa-api).
...
Para inscribirse con SMS o voz, se inscribe un número de teléfono que puede recibir una verificación por SMS o voz. Especifica los parámetros a continuación para llamar al endpoint. El parámetro `oob_channels` indica cómo quieres enviar el código al usuario (SMS o voz).
...
Si se completa correctamente, recibes una respuesta como esta:
...
Si recibes un error `User is already enrolled`, el usuario ya tiene registrado un factor de MFA. Antes de asociar otro factor con el usuario, debes solicitarle una verificación con el factor existente.

Si es la primera vez que el usuario asocia un autenticador, notarás que la respuesta incluye `recovery_codes`. Los códigos de recuperación se usan para acceder a la cuenta del usuario en caso de que pierda el acceso a la cuenta o al dispositivo que usa para la autenticación de segundo factor. Estos códigos son de un solo uso, y se generan otros nuevos cuando es necesario.
...
<div id="retrieve-enrolled-authenticators">
  ### Recuperar los autenticadores inscritos
</div>

Para solicitar la verificación del usuario, necesitas el `authenticator_id` del factor que quieres usar. Puedes enumerar todos los autenticadores inscritos con el endpoint de autenticadores de MFA:
...
<div id="challenge-user-with-otp">
  ### Verificar al usuario con OTP
</div>

Para iniciar un desafío, envía una solicitud `POST` al endpoint de desafío MFA con el `authenticator_id` correspondiente y el `mfa_token`.
...
Tu aplicación debe solicitar al usuario el código de 6 dígitos enviado en el mensaje y establecerlo en el parámetro `binding_code`. Puedes verificar el código y obtener tokens de autenticación usando el endpoint de token de OAuth0, especificando `binding_code` y `oob_code`, devueltos por la llamada anterior:
...
**Nota:** El envío de SMS y los intentos con códigos no válidos están sujetos a límite de frecuencia. Los códigos SMS pueden enviarse 10 veces y se restablecen una vez por hora, mientras que los códigos no válidos pueden introducirse 10 veces y se restablecen una vez cada seis minutos.
...
La MFA personalizable permite a los usuarios inscribir y verificar con los factores de su elección compatibles con tu aplicación.
...
Usa el `mfa_token` para llamar al endpoint [`mfa/authenticator`](/docs/es/api/authentication/muti-factor-authentication/list-authenticators) y obtener una lista de todos los factores que el usuario ha inscrito, y haz coincidir el tipo con el que admite tu aplicación. También necesitas obtener el `authenticator_type` correspondiente para emitir desafíos:
...
* [Verificación con códigos de recuperación](/docs/es/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa/challenge-with-recovery-codes)
* [Inscribir y verificar autenticadores de correo electrónico](/docs/es/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa/enroll-and-challenge-email-authenticators)
* [Inscribir y verificar autenticadores OTP](/docs/es/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa/enroll-and-challenge-otp-authenticators)
* [Inscribir y verificar autenticadores push](/docs/es/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa/enroll-and-challenge-push-authenticators)