...
title: Registrar y desafiar autenticadores de Email
---
...
<Card title="La disponibilidad varía según el plan de Auth0">
  Tanto tu implementación específica de inicio de sesión como tu plan de Auth0 o tu acuerdo personalizado influyen en si esta funcionalidad está disponible. Para obtener más información, consulta [Precios](https://auth0.com/pricing).
</Card>
...
Para permitir que los usuarios registren direcciones de correo electrónico además de su correo electrónico verificado en su identidad principal, debes completar los siguientes pasos.

### Obtener token de MFA

Según el momento en que actives la inscripción, puedes obtener un <Tooltip tip="Access Token: credencial de autorización, en forma de una cadena opaca o JWT, que se usa para acceder a una API." cta="Ver glosario" href="/docs/es/glossary?term=access+token">access token</Tooltip> para usar la API de MFA de distintas maneras:
...
Haz una solicitud `POST` al endpoint MFA Associate para registrar el autenticador del usuario. El bearer token requerido por este endpoint es el token de MFA obtenido en el paso anterior.
...
Si obtienes un error `User is already enrolled`, el usuario ya tiene un factor de MFA registrado. Antes de asociar otro factor con el usuario, debes pedirle que complete la verificación con el factor existente.

Si es la primera vez que el usuario asocia un autenticador, notarás que la respuesta incluye `recovery_codes`. Los códigos de recuperación se usan para acceder a la cuenta del usuario en caso de que pierda acceso a la cuenta o al dispositivo utilizado para la autenticación de segundo factor. Estos códigos son de un solo uso y se generan nuevos cuando es necesario.
...
En este punto, el autenticador está completamente asociado y listo para usarse, y tienes los tokens de autenticación del usuario.
...
Opcionalmente, puedes personalizar los correos electrónicos que reciben los usuarios. Consulta [Personalizar las plantillas de correo electrónico](/docs/es/customize/email/email-templates) para obtener más detalles.
...
Obtén un token de MFA siguiendo los pasos descritos en [Autentícate mediante la concesión de contraseña del propietario del recurso y MFA](/docs/es/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa).

<div id="retrieve-enrolled-authenticators">
  ### Obtener autenticadores registrados
</div>

Para enviar un desafío al usuario, necesitas el `authenticator_id` del factor que quieres usar para el desafío. Puedes listar todos los autenticadores registrados mediante el endpoint de autenticadores de MFA:
...
Para iniciar un desafío por correo electrónico, envía una solicitud `POST` al endpoint de desafío MFA con el `authenticator_id` correspondiente y el `mfa_token`.
...
Tu aplicación debe pedirle al usuario el código y enviarlo como parte de la solicitud, en el parámetro `binding_code`, en la siguiente llamada al endpoint `oauth``/token`:
...
La MFA personalizable permite a los usuarios registrar los factores que elijan y usarlos para completar desafíos, siempre que sean compatibles con tu aplicación.
...
Usa el `mfa_token` para llamar al endpoint [`mfa/authenticator`](/docs/es/api/authentication/muti-factor-authentication/list-authenticators) y obtener una lista de todos los factores que el usuario ha registrado, y compárala con el mismo tipo que admite tu aplicación.  También necesitas obtener el `authenticator_type` correspondiente para emitir desafíos:
