...
<Card title="Antes de empezar">
  * Configura una [aplicación máquina a máquina (M2M)](/docs/es/get-started/auth0-overview/create-applications/machine-to-machine-apps) y [concédele acceso a la Auth0 Management API](/docs/es/secure/tokens/access-tokens/management-api-access-tokens/get-management-api-access-tokens-for-production) con los siguientes scopes:
...
La [Auth0 Management API](https://auth0.com/docs/api/management/v2) proporciona varios endpoints que puedes usar para gestionar los métodos de autenticación de <Tooltip tip="Autenticación multifactor (MFA): proceso de autenticación de usuario que utiliza un factor adicional al nombre de usuario y la contraseña, como un código enviado por SMS." cta="Ver glosario" href="/docs/es/glossary?term=MFA">MFA</Tooltip> de tus usuarios.

Este método requiere autenticarse mediante una aplicación confidencial. Para obtener más información sobre las aplicaciones confidenciales y las públicas, consulta [Aplicaciones confidenciales y públicas](/docs/es/get-started/applications/confidential-and-public-applications).

<div id="get-all-authentication-methods">
  ## Obtener todos los métodos de autenticación
</div>

Usa el endpoint [Obtiene una lista de métodos de autenticación](https://auth0.com/docs/api/management/v2#!/Users/get_authentication_methods) para obtener una lista de todos los métodos de autenticación en los que un usuario se ha inscrito completa o parcialmente.

Este endpoint requiere el scope: `read:authentication_methods`.

### Ejemplos

La siguiente solicitud devuelve una lista de todos los métodos de autenticación para un usuario específico.
...
Usa el endpoint [Obtener un método de autenticación por ID](https://auth0.com/docs/api/management/v2#!/Users/get_authentication_methods_by_authentication_method_id) para obtener un método de autenticación de un usuario a partir del ID del método de autenticación.

Este endpoint requiere el scope: `read:authentication_methods`.

### Ejemplos

La siguiente solicitud devuelve un solo método de autenticación de un usuario según el ID del método de autenticación especificado.
...
Envía a los usuarios un OTP por SMS para que luego lo introduzcan antes de completar la autenticación.
...
Envía a los usuarios un OTP por Email, que luego deberán introducir antes de poder completar la autenticación. El factor Email solo se admite cuando un usuario no tiene ningún otro factor disponible.
...
Permita a los usuarios usar una aplicación de autenticación, como Google Authenticator, en su dispositivo personal para generar un OTP que cambia periódicamente y que deben introducir antes de completar la autenticación.
...
Permite a los usuarios usar claves de seguridad compatibles con FIDO (por ejemplo, [Yubikey](https://www.yubico.com/) o [Google Titan](https://cloud.google.com/titan-security-key)) para realizar autenticación multifactor.
...
Usa el endpoint [Actualiza todos los métodos de autenticación sustituyéndolos por los indicados](https://auth0.com/docs/api/management/v2#!/Users/put_authentication_methods) para reemplazar todos los métodos de autenticación existentes por los indicados.
...
Usa el endpoint [Actualizar un método de autenticación](https://auth0.com/docs/api/management/v2#!/Users/patch_authentication_methods_by_authentication_method_id) para actualizar un único método de autenticación de un usuario.

Este endpoint requiere el scope: `update:authentication_methods`.

### Ejemplos

La siguiente solicitud actualiza un método de autenticación específico de un usuario según el ID de ese método de autenticación.
...
Usa el endpoint [Deletes an authentication method by ID](https://auth0.com/docs/api/management/v2#!/Users/delete_authentication_methods_by_authentication_method_id) para eliminar un único método de autenticación de un usuario.

### Ejemplos

La siguiente solicitud elimina un solo método de autenticación de un usuario en función del ID del método de autenticación especificado.
...
Para cada solicitud válida, la Management API devolverá una respuesta con un `204` como código de estado y un cuerpo vacío.
