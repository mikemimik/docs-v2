...
title: Diccionario de idiomas del tema de MFA
---
...
`languageDictionary` se puede usar para personalizar el texto en muchas partes del proceso de <Tooltip tip="Multi-factor authentication (MFA): Proceso de autenticación de usuario que utiliza un factor además del nombre de usuario y la contraseña, como un código por SMS." cta="Ver glosario" href="/docs/es/glossary?term=MFA">MFA</Tooltip> en la experiencia de Classic Login. Esto se hace con el siguiente formato:
...
Aquí tienes un ejemplo de cómo modificar el atributo `headerText` en `smsEnrollmentConfirm`:
...
...
  ...
  languageDictionary:{ // Usa la opción del diccionario de idiomas
...
Consulta a continuación la lista de categorías y opciones disponibles.
...
smsEnrollmentAddPhoneNumber: {
  ...
  // countryCodes: { 'US': 'Traducción de Estados Unidos', ... '<country code>': '<traducción>' }
},
