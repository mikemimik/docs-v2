...
El `languageDictionary` se puede usar para sobrescribir el texto en muchas partes del proceso de <Tooltip tip="Autenticación multifactor (MFA): Proceso de autenticación de usuario que usa un factor además del username y la contraseña, como un código por SMS." cta="Ver glosario" href="/docs/es/glossary?term=MFA">MFA</Tooltip> en la experiencia de Classic Login. Para ello, utiliza el siguiente formato:
...
Este es un ejemplo de cómo modificar el atributo `headerText` en `smsEnrollmentConfirm`:
