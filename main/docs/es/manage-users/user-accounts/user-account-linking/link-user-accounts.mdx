---
description: Comprenda cómo vincular cuentas de usuario de varios proveedores de identidad para que sus usuarios puedan autenticarse con cualquiera de sus cuentas y su aplicación los reconozca.
...
Puedes vincular cuentas de usuario mediante diversos métodos:

* Action con aplicación externa para vincular cuentas
...
<div id="action-with-external-linking-application">
  ## Action con aplicación externa de vinculación
</div>
...
<Warning>
  **Auth0 no cambia automáticamente al** **usuario** **principal correcto** después de la vinculación de cuentas, por lo que este cambio debe realizarse en el código de Actions cuando la vinculación de cuentas se complete correctamente.

  **Toda vinculación manual de cuentas debe pedir al usuario que introduzca sus credenciales**. Su tenant debe solicitar autenticación para ambas cuentas antes de que se produzca la vinculación, para evitar que actores maliciosos accedan a cuentas de usuario legítimas.
</Warning>
...
2. La Action redirige al usuario a una aplicación externa de vinculación con un token payload que contiene las identidades de usuario candidatas:
...
4. La aplicación externa de vinculación redirige al usuario de vuelta a la Action con un token payload que contiene las identidades principal y secundaria del usuario:
...
Puedes usar el endpoint [Vincular una cuenta de usuario](https://auth0.com/docs/api/v2#!/Users/post_identities) de la Management API de dos formas:

* vinculación de cuentas del lado del cliente iniciada por el usuario mediante <Tooltip tip="Token de acceso: credencial de autorización, en forma de una cadena opaca o JWT, utilizada para acceder a una API." cta="Ver glosario" href="/docs/es/glossary?term=access+tokens">tokens de acceso</Tooltip> con el scope `update:current_user_identities`.
...
Para la vinculación de cuentas del lado del cliente iniciada por el usuario, necesitas un token de acceso que contenga los siguientes elementos en el payload:

* `update:current_user_identites` scope
* `user_id` de la cuenta principal como parte de la URL
* <Tooltip tip="Token de identidad: credencial destinada al propio cliente, en lugar de usarse para acceder a un recurso." cta="Ver glosario" href="/docs/es/glossary?term=ID+token">token de identidad</Tooltip> de la cuenta secundaria, firmado con RS256 e incluyendo una claim `aud` que identifique al cliente y coincida con el valor de la claim `azp` del token de acceso con el que se realiza la solicitud.

Un token de acceso que contiene el `update:current_user_identities` scope solo puede usarse para actualizar la información del usuario que ha iniciado sesión. Por lo tanto, este método es adecuado para situaciones en las que el usuario inicia el proceso de vinculación.
...
* token de identidad de la cuenta secundaria firmado con RS256 que incluya un `aud` claim que identifique al cliente y coincida con el valor del `azp` claim del token de acceso solicitante.

Los tokens de acceso que contienen el `update:users` scope pueden usarse para actualizar la información de cualquier usuario. Por lo tanto, este método está pensado para usarse únicamente en código del lado del servidor.
...
El `user_id` y el `provider` de la cuenta secundaria del usuario pueden inferirse a partir de su identificador único. Por ejemplo, para el identificador `google-oauth2|108091299999329986433`:

* `provider` es `google-oauth2`
* `user_id` es `108091299999329986433`

Como alternativa, puedes enviar el token de ID de la cuenta secundaria en lugar de `provider` y `user_id`:
...
Puedes usar la biblioteca Auth0.js para realizar la vinculación de cuentas del lado del cliente. Consulta [Referencia de Auth0.js &gt; Gestión de usuarios](/docs/es/libraries/auth0js#user-management) para obtener más información.
