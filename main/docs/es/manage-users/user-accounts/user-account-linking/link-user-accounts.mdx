---
description: Aprende a vincular cuentas de usuario de varios proveedores de identidad para que tus usuarios puedan autenticarse con cualquiera de ellas y tu aplicación los reconozca.
...
<Card title="La disponibilidad varía según el plan de Auth0">
  Tanto tu implementación específica de inicio de sesión como tu plan de Auth0 o tu acuerdo personalizado determinan si esta función está disponible. Para obtener más información, consulta [Precios](https://auth0.com/pricing).
</Card>

Puedes vincular cuentas de usuario mediante diversos métodos:

* Action con una aplicación externa para vincular cuentas
...
<Warning>
  **Auth0 no cambia automáticamente al** **usuario** **principal correcto** después de la vinculación de cuentas, por lo que este cambio debe hacerse en el código de Actions cuando la vinculación de cuentas se complete correctamente.

  **Toda vinculación manual de cuentas debe pedir al usuario que introduzca sus credenciales**. Su tenant debe solicitar authentication para ambas cuentas antes de que se produzca la vinculación, para evitar que actores maliciosos accedan a cuentas de usuario legítimas.
</Warning>
...
2. La Action redirige al usuario a una aplicación externa de vinculación con un payload de token que contiene posibles identidades de usuario:
...
3. La aplicación externa de vinculación pide al usuario que se autentique con las credenciales de la cuenta que desea vincular.
...
/**
 * Completar propiedades de perfil faltantes a partir de identidades vinculadas
 */
...
exports.onExecutePostLogin = async (event, api) => {
  ...
  if (
    ...
    console.log('Falta configuración requerida - omitiendo vinculación de cuentas');
    return;
  }
...
// No procesaremos usuarios para la vinculación de cuentas hasta que hayan verificado su dirección de correo electrónico.
  ...
  try {
    ...
    console.error('Error de vinculación de cuentas:', err.message);
...
* Vinculación de cuentas del lado del cliente iniciada por el usuario mediante <Tooltip tip="Token de acceso: credencial de autorización, en forma de una cadena opaca o JWT, que se usa para acceder a una API." cta="Ver glosario" href="/docs/es/glossary?term=access+tokens">tokens de acceso</Tooltip> con el scope `update:current_user_identities`.
...
* <Tooltip tip="token de ID: Credencial pensada para el propio cliente, en lugar de para acceder a un recurso." cta="Ver glosario" href="/docs/es/glossary?term=ID+token">token de identidad</Tooltip> de la cuenta secundaria, firmado con RS256 e incluido con un claim `aud` que identifique al cliente y coincida con el valor del claim `azp` del token de acceso solicitante.

Un token de acceso que contiene el scope `update:current_user_identities` solo puede usarse para actualizar la información del usuario que tiene la sesión iniciada en ese momento. Por lo tanto, este método es adecuado para escenarios en los que el usuario inicia el proceso de vinculación.
...
Para la vinculación de cuentas del lado del servidor, necesitas un token de acceso que incluya los siguientes elementos en el payload:

* scope `update:users`
* `user_id` de la cuenta principal como parte de la URL
* `user_id` de la cuenta secundaria
* token de identidad de la cuenta secundaria, firmado con RS256, que incluya un claim `aud` que identifique al cliente y coincida con el valor del claim `azp` del token de acceso de la solicitud.
...
Puedes usar la biblioteca Auth0.js para realizar la vinculación de cuentas del lado del cliente. Consulta [Referencia de Auth0.js &gt; Administración de usuarios](/docs/es/libraries/auth0js#user-management) para obtener más información.
