...
Puede usar una Action junto con una aplicación externa de vinculación para vincular cuentas de usuario mediante la Management API.

<Warning>
  **Auth0 no cambia automáticamente al** **usuario** **principal correcto** después de la vinculación de cuentas, por lo que este cambio debe realizarse en el código de Actions una vez que la vinculación de cuentas se haya completado correctamente.

  **Toda vinculación manual de cuentas debe solicitar al usuario que introduzca sus credenciales**. Su tenant debe solicitar autenticación para ambas cuentas antes de que se produzca la vinculación, a fin de evitar que actores maliciosos accedan a cuentas de usuario legítimas.
</Warning>
...
2. La Action redirige al usuario a una aplicación externa de vinculación con una carga útil de token que contiene posibles identidades de usuario para vincular:
...
4. La aplicación externa de vinculación redirige al usuario de vuelta a la Action con una carga útil de token que contiene las identidades del usuario principal y del secundario:
...
Puedes usar el endpoint [Vincular una cuenta de usuario](https://auth0.com/docs/api/v2#!/Users/post_identities) de la Management API de estas dos maneras:

* Vinculación de cuentas del lado del cliente iniciada por el usuario mediante <Tooltip tip="Token de acceso: credencial de Authorization, en forma de cadena opaca o JWT, que se usa para acceder a una API." cta="Ver glosario" href="/docs/es/glossary?term=access+tokens">tokens de acceso</Tooltip> con el scope `update:current_user_identities`.
...
* <Tooltip tip="token de ID: Credencial destinada al propio cliente, no para acceder a un recurso." cta="Ver glosario" href="/docs/es/glossary?term=ID+token">token de identidad</Tooltip> de la cuenta secundaria, firmado con RS256, que incluya un claim `aud` que identifique al cliente y coincida con el valor del claim `azp` del token de acceso de la solicitud.
...
Para la vinculación de cuentas del lado del servidor, necesitas un token de acceso que contenga los siguientes elementos en la carga útil:

* scope `update:users`
* `user_id` de la cuenta principal como parte de la URL
* `user_id` de la cuenta secundaria
* token de identidad de la cuenta secundaria, firmado con RS256, que incluya un claim `aud` que identifique al cliente y coincida con el valor del claim `azp` del token de acceso solicitante.
...
El `user_id` y el `provider` de la cuenta de usuario secundaria pueden inferirse a partir de su identificador único. Por ejemplo, para el identificador `google-oauth2|108091299999329986433`:
...
Puedes usar la biblioteca Auth0.js para vincular cuentas del lado del cliente. Consulta [Referencia de Auth0.js &gt; Administración de usuarios](/docs/es/libraries/auth0js#user-management) para obtener más información.
