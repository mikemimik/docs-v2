...
title: Administrar metadatos mediante la Management API
---
...
Haz la siguiente llamada `POST` al endpoint <Tooltip tip="Management API: Un producto que permite a los clientes realizar tareas administrativas." cta="Ver glosario" href="/docs/es/glossary?term=Management+API">Management API</Tooltip> [`/post_users`](https://auth0.com/docs/api/management/v2#!/Users/post_users) para crear el usuario y establecer los valores de las propiedades:
...
Puedes actualizar los metadatos de un usuario haciendo una solicitud `PATCH` al endpoint [`/patch_users_by_id`](https://auth0.com/docs/api/management/v2#!/Users/patch_users_by_id) de la Management API.
...
Para actualizar `user_metadata` y añadir la dirección particular del usuario como una propiedad de segundo nivel:
...
<Warning>
  Cuando envías una llamada `PATCH` en la que has establecido el valor de una propiedad en `null` (por ejemplo, `{user_metadata: {color: null}}`), Auth0 **elimina** la propiedad o su valor de la base de datos. Además, si haces un parche de los propios metadatos con un objeto vacío, los metadatos se eliminan por completo.
</Warning>

<div id="merge-user-metadata">
  ### Fusionar los metadatos del usuario
</div>

Solo se fusionan en el objeto las propiedades del nivel raíz. Todas las propiedades de niveles inferiores se reemplazarán.

Por ejemplo, para añadir la dirección de trabajo de un usuario como una propiedad interna adicional, tendrías que incluir todo el contenido de la propiedad `addresses`. Como el objeto `addresses` es una propiedad del nivel raíz, se fusionará en el objeto JSON final que representa al usuario, pero sus subpropiedades no.
...
Por tanto, la llamada `PATCH` correspondiente a la API sería:
...
Se puede incluir un objeto `clientMetadata` al crear una nueva aplicación mediante el endpoint `POST /api/v2/` de aplicaciones.

<div id="read-client-metadata">
  ### Leer metadatos del cliente
</div>
...
Los metadatos del cliente se pueden actualizar mediante el endpoint [`PATCH /api/v2/clients/{id}`](https://auth0.com/docs/api/management/v2#!/Users/patch_users_by_id), proporcionando un objeto de aplicación con la propiedad `clientMetadata property`, cuyo valor consiste en un objeto que contiene los metadatos que te gustaría cambiar.
...
Las claves de los metadatos del cliente pueden eliminarse mediante una solicitud PATCH, como se describe anteriormente en &quot;Update app&#95;metadata&quot;, pero proporcionando `null` como valor de la clave. Este comportamiento coincide con el de las propiedades `user_metadata` y `app_metadata` en el endpoint `PATCH` [/api/v2/users/{id}](https://auth0.com/docs/api/management/v2#!/Users/patch_users_by_id).
