...
Realiza la siguiente solicitud `POST` al <Tooltip tip="Management API: Un producto que permite a los clientes realizar tareas administrativas." cta="Ver glosario" href="/docs/es/glossary?term=Management+API">Management API</Tooltip> endpoint [`/post_users`](https://auth0.com/docs/api/management/v2#!/Users/post_users) para crear el usuario y establecer los valores de las propiedades:
...
Puedes actualizar los metadatos de un usuario realizando una llamada `PATCH` al endpoint [`/patch_users_by_id`](https://auth0.com/docs/api/management/v2#!/Users/patch_users_by_id) de la Management API.

Suponiendo que creaste un usuario con los siguientes valores de metadatos:
...
Para actualizar `user_metadata` y agregar la dirección particular del usuario como una propiedad de segundo nivel:
...
<Warning>
  Cuando envías una solicitud `PATCH` en la que has establecido el valor de una propiedad en `null` (por ejemplo, `{user_metadata: {color: null}}`), Auth0 **elimina** la propiedad y su valor de la base de datos. Además, si aplicas el parche a los propios metadatos con un objeto vacío, los metadatos se eliminan por completo.
</Warning>
...
Por ejemplo, para añadir la dirección de trabajo de un usuario como una propiedad interna adicional, tendrías que incluir todo el contenido de la propiedad `addresses`. Como el objeto `addresses` es una propiedad del nivel raíz, se combinará en el objeto JSON final que representa al usuario, pero sus subpropiedades no.
...
Actualizar los metadatos con un objeto vacío los elimina por completo. Por ejemplo, al enviar este cuerpo se elimina todo lo que hay en `app_metadata`:
...
Se puede incluir un objeto `clientMetadata` al crear una nueva aplicación mediante el endpoint `applications` de `POST /api/v2/`.

<div id="read-client-metadata">
  ### Consultar metadatos del cliente
</div>
...
Los metadatos del cliente se pueden actualizar mediante el endpoint [`PATCH /api/v2/clients/{id}`](https://auth0.com/docs/api/management/v2#!/Users/patch_users_by_id), proporcionando un objeto de aplicación con la propiedad `clientMetadata property`, cuyo valor es un objeto que contiene los metadatos que quieres cambiar.
...
Petición: `PATCH /api/v2/client/myclientid123` con el cuerpo:
...
<div id="delete-client-metadata-properties-and-values">
  ### Eliminar propiedades y valores de metadato del cliente
</div>

Las claves de metadato del cliente se pueden eliminar mediante un PATCH, como se describe en &quot;Actualizar app&#95;metadata&quot; más arriba, pero proporcionando `null` como valor de la clave. Este comportamiento coincide con el de las propiedades `user_metadata` y `app_metadata` en el endpoint `PATCH` [/api/v2/users/{id}](https://auth0.com/docs/api/management/v2#!/Users/patch_users_by_id).
