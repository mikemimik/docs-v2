...
Cuando crees la tarea, tendrás que proporcionar:

* El [ID de la conexión](/docs/es/authenticate/identity-providers/locate-the-connection-id) desde la que quieres exportar usuarios (opcional)
...
* Los campos relacionados con el usuario (como el user ID o el nombre) que quieres incluir en la exportación
...
<Warning>
  No puedes exportar todo `app_metadata` ni `user_metadata` en CSV. Debes especificar explícitamente los campos de los objetos de metadatos.
  ...
  Como solo puedes exportar hasta 30 campos, se recomienda usar el formato JSON si los datos de usuario tienen muchos campos.
</Warning>

Por ejemplo, para metadatos estructurados así:
...
La solicitud de exportación (para los tres campos) tendría este aspecto:
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  En los archivos CSV de exportación de usuarios, escapamos los tipos de datos String de conformidad con los estándares de OWASP para mitigar la inyección de CSV:

  * A los caracteres de comillas dobles se les antepone un carácter de comillas dobles.
...
Si exporta los datos en formato compatible con JSON, solo necesita proporcionar la propiedad raíz; no hace falta nombrar cada propiedad interna por separado, ya que se incluirán automáticamente.

Los archivos de exportación de Auth0 usan el formato [NDJSON](https://github.com/ndjson/ndjson-spec) debido a su gran tamaño, mientras que la funcionalidad de importación espera un archivo JSON.
...
Una vez que hayas creado la tarea para exportar tus usuarios, puedes consultar su estado mediante el [endpoint Get a Job](https://auth0.com/docs/api/management/v2#!/Jobs/get_jobs_by_id).

Proporciona el ID de la tarea (que recibiste en la respuesta al crearla). Si estás usando la solicitud de ejemplo a continuación, reemplaza el marcador `{yourJobId}` por el valor del ID.
...
<div id="find-export-data">
  ## Buscar los datos exportados
</div>

Puedes acceder a tus archivos de exportación usando la URL proporcionada como valor del parámetro **location**. El nombre de tu tenant también es el nombre de tu archivo. Por ejemplo, si el nombre de tu tenant es `auth0docs`, entonces tu archivo será `auth0docs.csv` o `auth0docs.json`. Cuando accedas a la URL, la descarga del archivo comenzará automáticamente.

El enlace de descarga es válido durante 60 segundos. Si ese plazo vence, tendrás 24 horas para volver a solicitarlo antes de que caduque la tarea.

<div id="job-cleanup">
  ## Eliminación de tareas
</div>

Todos los datos relacionados con las tareas se eliminan automáticamente después de 24 horas y ya no se puede acceder a ellos. Por lo tanto, **recomendamos encarecidamente que almacene los resultados de las tareas con el mecanismo de almacenamiento que prefiera**.
...
1. Haga una llamada `POST` al endpoint [Create export user&#39;s job](https://auth0.com/docs/api/management/v2/jobs/post-users-exports) de Management API para exportar usuarios a un archivo `.csv`:
