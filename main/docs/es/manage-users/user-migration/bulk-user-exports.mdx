...
Para ver una lista de los campos del perfil de usuario que se pueden exportar, consulta [User Profile Structure](/docs/es/manage-users/user-accounts/user-profiles/user-profile-structure).

Cuando crees tu tarea, tendrás que proporcionar:
...
* Los campos relacionados con el usuario (como el ID de usuario o el nombre) que quieres incluir en la exportación
...
Si exporta datos de usuario en formato CSV y quiere incluir metadatos, especifique cada campo de metadatos que quiera exportar. Puede exportar hasta 30 campos.

<Warning>
  No puede exportar la totalidad de `app_metadata` o `user_metadata` en CSV. Debe especificar de forma explícita los campos de los objetos de metadatos.
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  En los archivos CSV de exportación de usuarios, aplicamos caracteres de escape a los tipos de datos string de conformidad con los estándares de OWASP para mitigar la inyección de CSV:
  ...
  * Cada string se encierra entre comillas dobles.
...
Si exporta los datos en un formato compatible con JSON, solo necesita proporcionar la propiedad raíz; no es necesario indicar cada propiedad interna por separado, ya que se incluirán automáticamente.
...
Antes de poder importar usuarios con una exportación generada por Auth0, deberá convertir el archivo de NDJSON a JSON usando la biblioteca que prefiera (como [jq](https://stedolan.github.io/jq/)).

En este caso, para el mismo ejemplo que usamos antes, la solicitud se vería así:
...
Una vez que hayas creado la tarea para exportar tus usuarios, puedes consultar su estado usando el [endpoint Get a Job](https://auth0.com/docs/api/management/v2#!/Jobs/get_jobs_by_id).

Proporciona el ID de la tarea (que recibiste en la respuesta al crearla). Si estás usando la solicitud de ejemplo a continuación, reemplaza el marcador de posición `{yourJobId}` por el valor del ID.
...
El enlace de descarga es válido durante 60 segundos. Si este período vence, tendrás 24 horas para volver a usarlo antes de que la tarea expire.

<div id="job-cleanup">
  ## Limpieza de tareas
</div>

Todos los datos relacionados con las tareas se eliminan automáticamente después de 24 horas y ya no se puede acceder a ellos. Por ello, **recomendamos encarecidamente almacenar los resultados de las tareas mediante el mecanismo de almacenamiento que prefiera**.
...
1. Haga una llamada `POST` al endpoint [Create export user&#39;s job](https://auth0.com/docs/api/management/v2/jobs/post-users-exports) de la Management API para exportar usuarios a un archivo `.csv`:
...
3. Use una herramienta de línea de comandos para añadir un filtro. En este ejemplo, usamos [Miller](https://miller.readthedocs.io/en/latest/installing-miller/):
