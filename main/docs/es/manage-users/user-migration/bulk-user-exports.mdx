...
Puedes usar el endpoint `POST /api/v2/jobs/users-exports` para crear una tarea que exporte todos los usuarios asociados a una [conexión](/docs/es/authenticate/identity-providers) o todos los usuarios del tenant.

Para obtener una lista de los campos del perfil de usuario que se pueden exportar, consulta [Estructura del perfil de usuario](/docs/es/manage-users/user-accounts/user-profiles/user-profile-structure).

Cuando crees tu tarea, deberás proporcionar:

* [ID de la conexión](/docs/es/authenticate/identity-providers/locate-the-connection-id) desde la que quieres exportar usuarios (opcional)
* Formato del archivo de exportación (CSV o compatible con JSON)
* Número máximo de registros de usuario que se exportarán (opcional; si se omite, se exportarán todos los registros)
* Campos relacionados con el usuario (como el ID de usuario o el nombre) que quieras incluir en la exportación

También necesitarás un [token de acceso de la Management API](https://auth0.com/docs/api/management/v2/tokens).

<div id="create-a-request-body">
  ## Crea el cuerpo de una solicitud
</div>

Si lo deseas, busca el `connection_id` y el nombre de dominio de tu tenant de Auth0 en el <Tooltip tip="Auth0 Dashboard: el producto principal de Auth0 para configurar tus servicios." cta="Ver glosario" href="/docs/es/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip>. Crea un archivo de texto con el cuerpo de la solicitud que aparece a continuación:
...
Si exporta datos de usuario en formato CSV y quiere incluir metadatos, especifique cada campo de metadatos que desea exportar. Puede exportar hasta 30 campos.

<Warning>
  No puede exportar todo el contenido de `app_metadata` ni de `user_metadata` en CSV. Debe especificar explícitamente los campos de los objetos de metadatos.
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  En los archivos CSV de exportación de usuarios, aplicamos caracteres de escape a los tipos de datos String de conformidad con los estándares de OWASP para mitigar la inyección en CSV:

  * A los caracteres de comillas dobles se les antepone otro carácter de comillas dobles.
  * A cada cadena se le antepone un carácter de comilla simple.
...
Si exporta los datos en formato compatible con JSON, solo necesita proporcionar la propiedad raíz; no hace falta indicar cada propiedad interna por separado, ya que se incluirán automáticamente.

Los archivos de exportación de Auth0 usan el formato [NDJSON](https://github.com/ndjson/ndjson-spec) debido al gran tamaño de estos archivos, mientras que la funcionalidad de importación espera un archivo JSON.

Antes de poder importar usuarios mediante una exportación generada por Auth0, deberá convertir el archivo de NDJSON a JSON usando la biblioteca que prefiera (como [jq](https://stedolan.github.io/jq/)).
...
Una vez que hayas creado la tarea para exportar usuarios, puedes consultar su estado mediante el [endpoint Get a tarea](https://auth0.com/docs/api/management/v2#!/Jobs/get_jobs_by_id).

Proporciona el ID de la tarea (que recibiste en la respuesta al crearla). Si estás usando la solicitud de ejemplo que aparece a continuación, sustituye el marcador de posición `{yourJobId}` por el valor del ID.
...
<div id="find-export-data">
  ## Encontrar los datos de exportación
</div>

Puedes acceder a tus archivos de exportación mediante la URL proporcionada como valor del parámetro **location**. El nombre de tu tenant también será el nombre de tu archivo. Por ejemplo, si el nombre de tu tenant es `auth0docs`, tu archivo será `auth0docs.csv` o `auth0docs.json`. Cuando accedas a la URL, la descarga del archivo comenzará automáticamente.

El enlace de descarga es válido durante 60 segundos. Si ese período vence, tendrás 24 horas para volver a solicitarlo antes de que la tarea expire.

<div id="job-cleanup">
  ## Limpieza de tareas
</div>

Todos los datos relacionados con sus tareas se eliminan automáticamente después de 24 horas y ya no se podrá acceder a ellos. Por ello, **recomendamos encarecidamente guardar los resultados de las tareas mediante el mecanismo de almacenamiento que prefiera**.

<div id="filter-example">
  ## Ejemplo de filtro
</div>

Es posible que quiera filtrar el archivo `.csv` exportado para obtener un subconjunto específico de sus datos, como la fecha en que los usuarios iniciaron sesión por última vez en su aplicación.

1. Haga una llamada `POST` al endpoint [Create export user&#39;s tarea](https://auth0.com/docs/api/management/v2/jobs/post-users-exports) de Management API para exportar usuarios a un archivo `.csv`:
...
2. Vaya al archivo en la carpeta de descargas de su equipo local.

3. Use una herramienta de línea de comandos para agregar un filtro. En este ejemplo, usamos [Miller](https://miller.readthedocs.io/en/latest/installing-miller/):
