---
description: Describe cómo implementar la autenticación Passwordless con inicio de sesión integrado en aplicaciones tradicionales.
...
1. Captura el identificador de usuario en tu aplicación (el correo electrónico o número de teléfono del usuario) e invoca el endpoint `/passwordless/start` para iniciar el flujo passwordless. El usuario recibirá un correo electrónico, un SMS con un código de un solo uso o un Magic Link.
2. Si no enviaste un Magic Link, pídele al usuario el código de un solo uso y llama al endpoint `/oauth/token` para obtener tokens de autenticación.

Ten en cuenta que, al usar Magic Links, no necesitas llamar a `/oauth/token`. El usuario hará clic en el Magic Link y será redirigido a la URL de devolución de llamada de la aplicación.

A continuación, presentamos algunos fragmentos de código que pueden usarse para llamar a estos endpoints de API en distintos escenarios.
...
**Autenticar a un usuario de Email**
...
**Autenticar a un usuario mediante un Magic Link**
...
<div id="setting-the-auth0-forwarded-for-header-for-rate-limit-purposes">
  ## Configuración del encabezado auth0-forwarded-for a efectos del límite de frecuencia
</div>

El endpoint `/passwordless/start` tiene un [límite de frecuencia](/docs/es/troubleshoot/customer-support/operational-policies/rate-limit-policy) de 50 solicitudes por hora por IP. Si llamas a la API desde el servidor, la IP de tu backend puede alcanzar fácilmente estos límites de frecuencia. Para saber cómo solucionar este problema, consulta la sección Limitación de frecuencia en endpoints de Passwordless de [Uso de las API de Passwordless](/docs/es/authenticate/passwordless/implement-login/embedded-login/relevant-api-endpoints).

<div id="customize-mfa">
  ## Personaliza MFA
</div>

<ReleaseStageNotice feature="MFA personalizable con los flujos de contraseña del propietario del recurso, integrado o de token de actualización" stage="ea" terms="true" contact="equipo de soporte de Auth0" />

Personaliza <Tooltip tip="Autenticación multifactor (MFA): proceso de autenticación de usuario que usa un factor además de nombre de usuario y contraseña, como un código por SMS." cta="Ver glosario" href="/docs/es/glossary?term=MFA">MFA</Tooltip> con flujos integrados. Usa la API de MFA para permitir que los usuarios registren y verifiquen con los factores de su elección que sean compatibles con tu aplicación.
...
Usa el `mfa_token` para llamar al endpoint [`mfa/authenticator`](https://auth0.com/docs/api/authentication/muti-factor-authentication/list-authenticators), listar todos los factores que el usuario tiene registrados y hacer coincidir el mismo `type` que admite tu aplicación. También necesitas obtener el `authenticator_type` correspondiente para emitir desafíos:
