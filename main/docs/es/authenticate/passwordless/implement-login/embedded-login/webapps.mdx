...
Para usar las API de <Tooltip tip="Passwordless: Forma de authentication que no depende de una contraseña como primer factor." cta="Ver glosario" href="/docs/es/glossary?term=Passwordless">Passwordless</Tooltip> integrado en aplicación web clásica, asegúrate de habilitar la concesión **Passwordless OTP** en [Auth0 Dashboard &gt; Applications &gt; Applications](https://manage.auth0.com/#/applications), en la configuración de tu aplicación, en **Advanced Settings** &gt; **Grant Types**.

La autenticación passwordless para aplicación web clásica consta de dos pasos:

1. Captura el identificador del usuario en tu aplicación (el correo electrónico o el número de teléfono del usuario) e invoca el endpoint `/passwordless/start` para iniciar el flujo passwordless. El usuario recibirá un correo electrónico, un SMS con un código de un solo uso o un Magic Link.
...
A continuación, mostramos algunos fragmentos de código que pueden usarse para llamar a estos endpoints de la API en diferentes escenarios.
...
**Enviar una contraseña de un solo uso mediante SMS**
...
Cuando envías un Magic Link, no necesitas llamar a ninguna API para autenticar al usuario. Los usuarios harán clic en el enlace y serán redirigidos a la URL de devolución de llamada.

<div id="setting-the-auth0-forwarded-for-header-for-rate-limit-purposes">
  ## Configuración del header `auth0-forwarded-for` a efectos del límite de frecuencia
</div>

El endpoint `/passwordless/start` tiene un [límite de frecuencia](/docs/es/troubleshoot/customer-support/operational-policies/rate-limit-policy) de 50 solicitudes por hora y por IP. Si llamas a la API desde el servidor, la IP de tu backend puede alcanzar fácilmente estos límites de frecuencia. Para saber cómo abordar esta incidencia, consulta la sección Límites de frecuencia en endpoints de Passwordless de [Uso de las API de Passwordless](/docs/es/authenticate/passwordless/implement-login/embedded-login/relevant-api-endpoints).
...
Personaliza <Tooltip tip="Autenticación multifactor (MFA): proceso de autenticación de usuario que usa un factor además del nombre de usuario y la contraseña, como un código por SMS." cta="Ver glosario" href="/docs/es/glossary?term=MFA">MFA</Tooltip> con flujos integrados. Usa la API de MFA para permitir que los usuarios registren y usen los factores que elijan, siempre que tu aplicación los admita.

Al usar [Lock for Web](/docs/es/libraries/lock#2-authenticating-and-getting-user-info), el endpoint `oauth/token` devuelve el error `mfa_required` e incluye el `mfa_token` que necesitas para usar la API de MFA y el parámetro `mfa_requirements`, con una lista de los autenticadores que tu aplicación admite actualmente:
...
Usa el `mfa_token` para llamar al endpoint [`mfa/authenticator`](https://auth0.com/docs/api/authentication/muti-factor-authentication/list-authenticators), listar todos los factores que el usuario tiene registrados y encontrar el mismo `type` que admite tu aplicación. También debes obtener el `authenticator_type` correspondiente para emitir desafíos:
...
A continuación, aplique el desafío MFA llamando al endpoint [`request/mfa/challenge`](https://auth0.com/docs/api/authentication/muti-factor-authentication/request-mfa-challenge).

Personalice aún más el flujo MFA con Auth0 Actions. Para obtener más información, consulte [Actions Triggers: post-challenge - objeto de API](/docs/es/customize/actions/explore-triggers/password-reset-triggers/post-challenge-trigger/post-challenge-api-object).