...
Para usar las API de <Tooltip tip="Passwordless: Forma de autenticación que no depende de una contraseña como primer factor." cta="Ver glosario" href="/docs/es/glossary?term=Passwordless">Passwordless</Tooltip> integrado en aplicaciones nativas, asegúrate de habilitar la concesión de **Passwordless OTP** en [Auth0 Dashboard &gt; Applications &gt; Applications](https://manage.auth0.com/#/applications), dentro de la configuración de tu aplicación, en **Advanced Settings** &gt; **Grant Types**.

La autenticación passwordless para aplicaciones nativas consta de dos pasos:

* Captura el identificador del usuario en tu aplicación (el correo electrónico o número de teléfono del usuario) e invoca el endpoint `/passwordless/start` para iniciar el flujo passwordless. El usuario recibirá un correo electrónico o un SMS con una contraseña de un solo uso.
...
Si lo prefieres, puedes usar los SDKs de Android o iOS, que encapsulan estas APIs de una manera adaptada a la plataforma:
...
<ReleaseStageNotice feature="MFA personalizable con los flujos Resource Owner Password Grant, integrado o de token de actualización" stage="ea" terms="true" contact="equipo de soporte de Auth0" />

Personaliza <Tooltip tip="Autenticación multifactor (MFA): proceso de autenticación de usuario que usa un factor además del nombre de usuario y la contraseña, como un código por SMS." cta="Ver glosario" href="/docs/es/glossary?term=MFA">MFA</Tooltip> con flujos integrados. Usa la API de MFA para permitir que los usuarios se registren y verifiquen su identidad con los factores de su elección compatibles con tu aplicación.

Cuando tu aplicación solicita un <Tooltip tip="Token de acceso: credencial de autorización, en forma de una cadena opaca o JWT, utilizada para acceder a una API." cta="Ver glosario" href="/docs/es/glossary?term=access+token+at+the">token de acceso en el</Tooltip> endpoint `oauth/token`, la respuesta incluye `mfa_token` para usar la API de MFA y el parámetro `mfa_requirements` con una lista de autenticadores que tu aplicación admite actualmente:
