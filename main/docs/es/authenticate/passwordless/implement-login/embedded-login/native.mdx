...
Para usar las API de <Tooltip tip="Passwordless: Forma de autenticación que no se basa en una contraseña como primer factor." cta="Ver glosario" href="/docs/es/glossary?term=Passwordless">Passwordless</Tooltip> integradas en aplicaciones nativas, asegúrate de habilitar la concesión **Passwordless OTP** en [Auth0 Dashboard &gt; Applications &gt; Applications](https://manage.auth0.com/#/applications), en la configuración de tu aplicación, en **Advanced Settings** &gt; **Grant Types**.

La autenticación passwordless para aplicaciones nativas consta de dos pasos:

* Captura el identificador del usuario en tu aplicación (el correo electrónico o el número de teléfono del usuario) e invoca el endpoint `/passwordless/start` para iniciar el flujo passwordless. El usuario recibirá un correo electrónico o un SMS con una contraseña de un solo uso.
* Solicita al usuario el código de un solo uso y llama al endpoint `/oauth/token` para obtener tokens de autenticación.

A continuación, enumeramos algunos fragmentos de código que pueden usarse para llamar a estos endpoints de la API en distintos escenarios.
...
Si lo prefieres, puedes usar los SDKs de Android o iOS, que encapsulan estas APIs de una manera adaptada a cada plataforma:
...
<div id="customize-mfa-with-embedded">
  ## Personaliza MFA con inicio de sesión integrado
</div>

<ReleaseStageNotice feature="MFA personalizable con los flujos de contraseña del propietario del recurso, de inicio de sesión integrado o token de actualización" stage="ea" terms="true" contact="equipo de soporte de Auth0" />

Personaliza <Tooltip tip="Autenticación multifactor (MFA): proceso de autenticación de usuario que utiliza un factor adicional además del nombre de usuario y la contraseña, como un código enviado por SMS." cta="Ver glosario" href="/docs/es/glossary?term=MFA">MFA</Tooltip> con flujos integrados. Usa la API de MFA para permitir que los usuarios registren y utilicen los factores compatibles con tu aplicación que prefieran.

Cuando tu aplicación solicita un <Tooltip tip="Token de acceso: credencial de autorización, en forma de una cadena opaque o JWT, que se utiliza para acceder a una API." cta="Ver glosario" href="/docs/es/glossary?term=access+token+at+the">token de acceso en el</Tooltip> endpoint `oauth/token`, la respuesta incluye `mfa_token` para usar la API de MFA y el parámetro `mfa_requirements` con una lista de autenticadores que tu aplicación admite actualmente:
...
Usa el `mfa_token` para llamar al endpoint [`mfa/authenticator`](https://auth0.com/docs/api/authentication/muti-factor-authentication/list-authenticators) y listar todos los factores en los que el usuario se ha inscrito, y hacer coincidir el mismo `type` que admite tu aplicación. También necesitas obtener el `authenticator_type` correspondiente para emitir desafíos:
