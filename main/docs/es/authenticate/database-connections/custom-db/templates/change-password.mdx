...
Este script se usa solo en un [escenario de autenticación heredada](/docs/es/authenticate/database-connections/custom-db/overview-custom-db-connections) y es necesario si desea cambiar la contraseña de un usuario en la base de datos externa. Se ejecutará cuando un usuario realice un [flujo de restablecimiento de contraseña](/docs/es/customize/login-pages/classic-login/customize-password-reset-page), o cuando se inicie un flujo de cambio de contraseña desde el <Tooltip tip="Auth0 Dashboard: el producto principal de Auth0 para configurar sus servicios." cta="Ver glosario" href="/docs/es/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip> o la Auth0 <Tooltip tip="Management API: un producto que permite a los clientes realizar tareas administrativas." cta="Ver glosario" href="/docs/es/glossary?term=Management+API">Management API</Tooltip>.
...
| `newPassword` | String   | Valor que se establecerá como la nueva contraseña del usuario en la base de datos externa. Este valor se envía en texto sin formato a la función y [debe cifrarse](#Encryption) antes de enviarlo a la base de datos externa. |
| `callback`    | Function | Se usa para pasar datos o el resultado de la operación a través del pipeline.                                                                                                                                                 |

### Ejemplo

Este es un ejemplo de seudocódigo en JavaScript de cómo podrías implementar la función `changePassword`. Para ver ejemplos en otros lenguajes, consulta [Language-specific script examples](#language-specific-script-examples).
...
function changePassword(email, newPassword, callback) {
  ...
  send(options, err => {
    if (err && err.id == "FAIL_CHANGE_PASSWORD") {
      ...
      // Devolver true en la devolución de llamada si la operación de cambio de contraseña se realizó correctamente
...
<Warning>
  Evita registrar, almacenar o transmitir la credencial de contraseña en ningún lugar sin cifrar.
</Warning>

Para evitar posibles filtraciones de datos, cifra el valor de la contraseña con una biblioteca criptográfica de hash como `bcrypt`.
...
La función `callback` acepta dos parámetros y se usa para devolver datos de error o indicar el resultado de la operación.
...
| ----------------- | ------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `error`           | Object  | Requerido | Contiene información del error.                                                                                                                                          |
| `operationResult` | Boolean | Opcional  | Indica el resultado de la operación de cambiar la contraseña.                                                                                                            |
| `resultObj`       | Object  | Opcional  | Indica que la operación de cambiar la contraseña se realizó correctamente. Si la propiedad `last_password_reset` está presente, se actualizará en el perfil del usuario. |

<div id="return-a-success">
  ### Devolver un resultado correcto
</div>

Si la operación de cambio de contraseña se realizó correctamente, devuelve mediante la función de devolución de llamada `callback` un valor `null` como parámetro `error` y un valor `true` como parámetro `operationResult`.
...
Si la operación de cambiar la contraseña se realizó correctamente, invoca la función de devolución de llamada `callback` y pasa un valor `null` como parámetro `error` y un valor de tipo `object` como parámetro `profile`. Si el atributo `last_password_reset` se proporciona en el objeto, se actualizará en el perfil del usuario.
...
Si la operación de cambio de contraseña falla, llame a la función de devolución de llamada `callback` y pase un valor `null` en el parámetro `error` y un valor `false` en el parámetro `operationResult`.
...
Si se produce un error, llame a la función de devolución de llamada `callback` y pase la información pertinente del error en el parámetro `error`.
...
function changePassword(email, newPassword, callback) {
  ...
  // hashearse/saltearse para coincidir con lo que esté almacenado en tu base de datos.
  //
...
* [Plantillas de scripts Create](/docs/es/authenticate/database-connections/custom-db/templates/create)
