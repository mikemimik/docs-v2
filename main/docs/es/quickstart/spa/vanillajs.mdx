---
mode: wide
description: Esta guía muestra cómo integrar Auth0, añadir autenticación y mostrar información del perfil del usuario en una aplicación de página única (SPA) que utiliza JavaScript puro, con Auth0 SPA SDK.
sidebarTitle: JavaScript
title: Añade el inicio de sesión a tu aplicación de JavaScript
---
...
<Accordion title="Prompt de IA" defaultOpen icon="microchip-ai" iconType="sharp-solid">
  **¿Usas IA para integrar Auth0?** Añade este prompt a Cursor, Windsurf, Copilot, Claude Code o tu IDE favorito con IA para acelerar el desarrollo.
  ...
  # Solo ejecutar esto si creaste un nuevo proyecto y NO estás ya en auth0-vanillajs:
  cd auth0-vanillajs
  ...
  # Configurar la app de Auth0 y generar el archivo .env
  ...
  - Asegúrate de usar importaciones de módulos ES6 correctamente
  ...
  Si el archivo style.css existente es grande o está mal formado, crea primero un archivo CSS temporal (por ejemplo, style-new.css) y luego reemplaza el original con comandos de terminal como `mv style-new.css style.css` para evitar la corrupción del archivo.
  ...
  * {
  ...
  1.  NUNCA escribas directamente en el código el dominio de Auth0 o el Client ID. Esto constituye una vulnerabilidad de seguridad grave.
  ...
  El directorio de trabajo del terminal puede desincronizarse de la ruta mostrada, por lo que es necesario navegar de forma explícita para garantizar que los comandos npm se ejecuten en la ubicación correcta.
...
<Note>
  ...
  **Herramienta de compilación:** Esta guía de inicio rápido usa **Vite** para el desarrollo. También puedes usar el SDK a través de un CDN para una configuración sin herramientas de compilación.
</Note>
...
Esta guía de inicio rápido muestra cómo añadir la autenticación de Auth0 a una aplicación en JavaScript sin frameworks. Crearás una aplicación web de página única moderna con un inicio de sesión seguro usando JavaScript puro y el Auth0 SPA SDK.
...
<Steps>
  <Step title="Crear un nuevo proyecto" stepNumber={1}>
    Crea un nuevo proyecto de JavaScript para este Quickstart

    ```shellscript
    mkdir auth0-vanillajs && cd auth0-vanillajs
    ```

    Inicializa el proyecto, instala un servidor de desarrollo local y configura scripts
  ...
  <Step title="Configura tu Auth0 App" stepNumber={3}>
    ...
    <Tabs>
      ...
      <Tab title="CLI">
        ...
        <CodeGroup>
          ```shellscript Mac
          # Instala Auth0 CLI (si todavía no está instalado)
          brew tap auth0/auth0-cli && brew install auth0
        ...
        <Note>
          ...
          1. Comprobará si ya iniciaste sesión (y te pedirá que inicies sesión si hace falta)
      ...
      <Tab title="Dashboard">
        ...
        **Allowed Web Origins:**
        ...
        <Info>
          ...
          **Allowed Web Origins** es fundamental para la autenticación silenciosa. Sin esta configuración, se cerrará la sesión de los usuarios cuando actualicen la página o vuelvan a tu aplicación más tarde.
        </Info>
      </Tab>
    </Tabs>
  </Step>
  ...
  <Step title="Ejecuta tu aplicación" stepNumber={5}>
    ...
    <Info>
      Si el puerto 5173 ya está en uso, ejecuta: `npm run dev -- --port 5174` y actualiza las URL de devolución de llamada de tu Auth0 App a `http://localhost:5174`
    </Info>
  </Step>
</Steps>
...
<Accordion title="Obtén un token de acceso para llamadas a la API">
  Si necesitas llamar a una API protegida, puedes obtener un token de acceso:
...
<Accordion title="Gestiona el inicio de sesión con popup">
  Para ofrecer una experiencia de usuario más fluida, puedes usar el inicio de sesión mediante popup:
...
<Accordion title="Agrega compatibilidad con Organizations">
  ...
  auth0Client = await createAuth0Client({
    ...
    authorizationParams: {
      redirect_uri: window.location.origin,
      organization: 'YOUR_ORGANIZATION_ID' // o haz que el usuario seleccione una
    }
  });
  ```
</Accordion>