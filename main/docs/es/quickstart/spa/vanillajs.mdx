---
mode: wide
description: Esta guía muestra cómo integrar Auth0, añadir autenticación y mostrar la información del perfil del usuario en una aplicación de página única (SPA) que usa JavaScript puro con el Auth0 SPA SDK.
...
<Accordion title="Prompt de IA" defaultOpen icon="microchip-ai" iconType="sharp-solid">
  **¿Usas IA para integrar Auth0?** Añade este prompt a Cursor, Windsurf, Copilot, Claude Code o tu IDE favorito con IA para acelerar el desarrollo.
  ...
  echo "   - Allowed Web Origins: http://localhost:5173 (CRÍTICO para la autenticación silenciosa)"
  ...
  echo ""
  echo "📝 NOTA: Asegúrate de que tu aplicación de Auth0 esté configurada como 'Single Page Application'"
  echo "   en el panel de Auth0. Otros tipos de aplicación no funcionarán con este SDK."
  ...
  async function initAuth0() {
    try {
      ...
      // Comprobar si el usuario regresa tras el inicio de sesión
  ...
  ⚠️ VERIFICACIÓN DEL PUNTO DE CONTROL:
  ...
  Reemplaza todo el contenido de style.css con este estilo moderno con la identidad visual de Auth0:
  ...
  * {
  ...
  2.5: Actualiza package.json con el script de desarrollo
  Actualiza tu package.json para incluir los scripts de desarrollo, compilación y vista previa. Reemplaza todo el contenido de package.json:
  ...
  2.6: Inicia el servidor de desarrollo

  ⚠️ PROBLEMA CRÍTICO CON EL DIRECTORIO DE TRABAJO DE LA TERMINAL:
  El directorio de trabajo de la terminal puede confundirse en ocasiones durante el proceso de integración. Antes de iniciar el servidor de desarrollo, SIEMPRE:

  1. Verifica que estás en el directorio correcto con un cambio de ruta explícito:

  cd /full/absolute/path/to/project && pwd

  2. Comprueba que package.json existe en el directorio actual:

  ls -la package.json

  3. Si los comandos npm fallan con "package.json not found", usa el método de ruta absoluta:

  cd /Users/[username]/path/to/auth0-vanillajs && npm run dev

  ⚠️ IMPORTANTE: Aunque `pwd` muestre el directorio correcto, el directorio de trabajo de la terminal para la ejecución de comandos puede ser distinto. SIEMPRE usa `cd` explícito con la ruta absoluta antes de ejecutar comandos npm para garantizar que se ejecutan en la ubicación correcta.

  Paso 2.6a: Comprueba si se necesita un puerto personalizado

  grep -q "VITE_DEV_PORT" .env.local 2>/dev/null && echo "Custom port detected" || echo "Using default port"

  Paso 2.6b: Inicia el servidor de desarrollo con el puerto adecuado
  ...
  Si ves errores de "Auth0 configuration missing" aunque .env.local exista:
  ...
  Incidencia 1: Confusión con el directorio de creación del proyecto
  ...
  Incidencia 2: Problemas con el directorio de trabajo del terminal
  Problema: Los comandos npm fallan con "package.json not found" aunque estés en el directorio correcto
  Solución: Usa cambios de ruta absoluta explícitos: `cd /ruta/absoluta/completa/al/proyecto`

  Incidencia 3: Problemas de carga de variables de entorno
  ...
  Incidencia 4: Corrupción del archivo CSS
  Problema: Los reemplazos de CSS de gran tamaño pueden causar corrupción del archivo
  Solución: Crea primero un archivo CSS temporal y luego usa el comando `mv` para reemplazar el original

  Incidencia 5: Validación de la configuración de Auth0
  ...
  Incidencia 6: El directorio de trabajo del terminal no está en la raíz del proyecto
  Problema: El agente de IA no puede ejecutar `npm run dev` porque el terminal no está en el directorio auth0-vanillajs, aunque pwd muestre la ruta correcta
  ...
  El directorio de trabajo del terminal puede desincronizarse de la ruta mostrada, por lo que es necesaria una navegación explícita para garantizar que los comandos npm se ejecuten en la ubicación correcta.

  IMPLEMENTACIÓN DE FUNCIONES AVANZADAS

  ⚠️ NOTA SOBRE LA FUNCIONALIDAD DEL SDK:
  La función isAuthenticated() del SDK permite la representación condicional de los botones de inicio/cierre de sesión y el contenido del usuario. La implementación anterior demuestra este patrón mostrando diferentes secciones de la interfaz según el estado de autenticación.
...
<Note>
  ...
  * **[npm](https://docs.npmjs.com/downloading-and-installing-node-js-and-npm)** 10+ o **[yarn](https://classic.yarnpkg.com/lang/en/docs/install/)** 1.22+ o **[pnpm](https://pnpm.io/installation)** 8+
  ...
  Comprueba la instalación: `node --version && npm --version`

  **Herramienta de compilación:** Este Quickstart usa **Vite** para el desarrollo. También puedes usar el SDK mediante CDN para una configuración sin herramienta de compilación.
</Note>
...
Este Quickstart muestra cómo añadir autenticación de Auth0 a una aplicación JavaScript sin frameworks. Crearás una aplicación web de página única moderna con una funcionalidad de inicio de sesión segura utilizando JavaScript puro y el Auth0 SPA SDK.
...
<Steps>
  <Step title="Crear un nuevo proyecto" stepNumber={1}>
    ...
    Inicializa el proyecto, instala un servidor local de desarrollo y configura scripts
  ...
  <Step title="Instala el SDK de JavaScript de Auth0 para SPA" stepNumber={2}>
    ```shellscript
    npm install @auth0/auth0-spa-js
    ```
  </Step>
  ...
  <Step title="Configura tu Auth0 App" stepNumber={3}>
    A continuación, debes crear una nueva app en tu tenant de Auth0 y agregar las variables de entorno a tu proyecto.

    Tienes tres opciones para configurar tu app de Auth0: usar la herramienta Quick Setup (recomendado), ejecutar un comando de CLI o configurarla manualmente desde el Dashboard:
    ...
    <Tabs>
      ...
      <Tab title="CLI">
        Ejecuta el siguiente comando en el directorio raíz de tu proyecto para crear una app de Auth0 y generar un archivo `.env`:
        ...
        <CodeGroup>
          ...
          # Configura la app de Auth0 y genera el archivo .env
          auth0 qs setup --app --type spa --framework vanilla-javascript --build-tool vite --name "My App" --port 5173
          ```
        ...
        <Note>
          ...
          1. Comprobará si ya has iniciado sesión (y mostrará el prompt de inicio de sesión si es necesario)
          2. Creará una aplicación web de página única de Auth0 configurada para `http://localhost:5173`
          3. Generará un archivo `.env` con `VITE_AUTH0_DOMAIN` y `VITE_AUTH0_CLIENT_ID`
        </Note>
      </Tab>

      <Tab title="Dashboard">
        Antes de comenzar, crea un archivo `.env.local` en el directorio raíz de tu proyecto
        ...
        5. Reemplaza `YOUR_AUTH0_APP_DOMAIN` y `YOUR_AUTH0_APP_CLIENT_ID` en el archivo `.env.local` por los valores de **Domain** e **ID de cliente** del Dashboard
        ...
        **Allowed Web Origins:**
        ...
        <Info>
          ...
          **Allowed Web Origins** es fundamental para la autenticación silenciosa. Sin esta configuración, los usuarios cerrarán sesión al recargar la página o al volver a tu app más tarde.
        </Info>
      </Tab>
    </Tabs>
  </Step>

  <Step title="Crear la estructura HTML y la lógica de la aplicación" stepNumber={4}>
...
<Check>
  **Punto de control**
  ...
  Ya deberías tener una página de inicio de sesión de Auth0 totalmente funcional en tu [localhost](http://localhost:5173/)
</Check>
...
<Accordion title="Gestionar el inicio de sesión con popup">
  Para una experiencia de usuario más fluida, puedes usar el inicio de sesión con popup:

  ```javascript
  // Reemplaza la función login en app.js
  async function login() {
    try {
      await auth0Client.loginWithPopup();
      await updateUI();
...
<Accordion title="Agregar soporte para Organizations">
  ...
  auth0Client = await createAuth0Client({
    ...
    authorizationParams: {
      redirect_uri: window.location.origin,
      organization: 'YOUR_ORGANIZATION_ID' // o permite que el usuario seleccione una
    }
  });
  ```
</Accordion>