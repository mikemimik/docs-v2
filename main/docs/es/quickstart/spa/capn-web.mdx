---
mode: wide
description: En esta guía se muestra cómo integrar Auth0, añadir autenticación y mostrar información del perfil del usuario en una aplicación de página única (SPA) que usa Cap'n Web RPC con Auth0 SPA SDK.
...
<Accordion title="Prompt de IA" defaultOpen icon="microchip-ai" iconType="sharp-solid">
  **¿Usas IA para integrar Auth0?** Añade este prompt a Cursor, Windsurf, Copilot, Claude Code o tu IDE favorito con IA para acelerar el desarrollo.
  ...
  1. VERIFICAR PROYECTO EXISTENTE PRIMERO: Antes de crear un nuevo proyecto, comprueba si el directorio actual ya contiene un proyecto Cap'n Web (package.json con dependencias de capnweb).
  ...
  3. SIN PLANIFICACIÓN: NO propongas una estructura de directorios. Tu primera acción debe ser ejecutar el comando correspondiente.
  4. SECUENCIA ESTRICTA: Sigue el flujo de ejecución en el orden exacto especificado.
  5. CONSTRUIR RPC SEGURO: Implementa la validación adecuada de tokens JWT tanto en el cliente como en el servidor para la comunicación RPC.
  ...
  Paso 3: Configurar la aplicación de Auth0 (usa el comando de CLI del Paso 3 en la guía de inicio rápido)

  Paso 4: Configurar la aplicación y la API de Auth0
  - Crear una aplicación de Auth0 (tipo SPA)
  - Crear una API de Auth0 con los scopes necesarios
  - Configurar las URLs de devolución de llamada y los orígenes

  Paso 5: Implementar el servidor con validación JWT
  - Crear el servidor WebSocket con Cap'n Web RPC
  ...
  - Crear una interfaz moderna con estado de autenticación

  Paso 7: Ejecutar la aplicación
  npm run start

  REQUISITOS DE SEGURIDAD
  - NUNCA aceptar llamadas RPC no autenticadas
  - SIEMPRE validar las firmas JWT usando JWKS
  - Implementar un manejo adecuado de errores para tokens expirados
  - Usar conexiones WebSocket seguras en producción
  ...
  1. NUNCA ejecutar comandos `cd` automáticamente sin confirmación explícita del usuario
  2. SIEMPRE verificar el directorio actual con `pwd` antes de continuar
  3. Si se trabaja con un proyecto existente: permanecer en el directorio actual
  4. Si se creó un nuevo proyecto: el usuario debe navegar manualmente al directorio capnweb-auth0-app primero
  ...
  Paso 3.2: Crear la plantilla manual de .env (si la configuración automática falla)
  ...
  Reemplaza todo el contenido con la implementación del servidor WebSocket seguro:
    ...
    // - Manejo y caché automático de JWKS
    // - Soporte integrado para tokens JWT/JWE
    ...
    wss.on('connection', (ws) => {
      ...
      ws.on('message', async (message) => {
        try {
          ...
          // Llamar al método correspondiente según la solicitud
  ...
  Usa newWebSocketRpcSession de capnweb y el SDK de Auth0 con las importaciones de módulos ES adecuadas:
    ...
    async function fetchProfile() {
      try {
        ...
        // Llamar al método RPC directamente sobre el stub de profileApi
  ...
  7.3: Probar el flujo de autenticación completo:
    ...
    - Actualizar la página: debería permanecer con la sesión iniciada (gracias a los tokens de actualización)
  ...
  - Asegurarse de que la configuración de la aplicación Auth0 coincida con las URLs locales
...
En este Quickstart, se muestra cómo añadir la autenticación de Auth0 a una aplicación de Cap&#39;n Web. Crearás una aplicación web moderna basada en RPC con una funcionalidad de inicio de sesión segura usando el framework JavaScript de Cap&#39;n Web y el Auth0 SPA SDK.
...
<Steps>
  ...
  <Step title="Instalar dependencias" stepNumber={2}>
    Instala Cap&#39;n Web y sus dependencias principales:

    ```shellscript
    npm install capnweb ws dotenv
    ```

    Instala los Auth0 SDKs para la autenticación y la verificación de tokens:
    ...
    <Info>
      **@auth0/auth0-spa-js** se usa del lado del cliente para gestionar la autenticación de usuario, los flujos de inicio de sesión y los tokens en el navegador.

      **@auth0/auth0-api-js** se usa del lado del servidor para verificar los tokens de acceso y validar las firmas JWT mediante el JWKS de Auth0.
    </Info>
  ...
  <Step title="Configura tu Auth0 App" stepNumber={3}>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        5. Reemplaza `YOUR_AUTH0_APP_DOMAIN` y `YOUR_AUTH0_APP_CLIENT_ID` en el archivo `.env` con los valores de **Domain** e **ID de cliente** del Dashboard

        Por último, en la pestaña **Settings** de la página Application Details de tu aplicación, configura las siguientes URL:
        ...
        **Orígenes web permitidos:**
        ...
        <Info>
          ...
          **Allowed Web Origins** es fundamental para la autenticación silenciosa. Sin esta configuración, se cerrará la sesión de los usuarios cuando actualicen la página o vuelvan más tarde a tu aplicación.
        </Info>

        **Obligatorio: Crear una API de Auth0**
        ...
        4. En la pestaña **Scopes**, agrega estos scopes:
        ...
        <Warning>
          Si no creas una API de Auth0, obtendrás un error &quot;access&#95;denied&quot; durante el inicio de sesión. El identificador de la API debe coincidir exactamente con tu variable de entorno `AUTH0_AUDIENCE`.
        </Warning>
      </Tab>
    </Tabs>
  </Step>
  ...
  <Step title="Crear la interfaz del cliente" stepNumber={6}>
    ...
    <AuthCodeGroup>
      ...
      // Configuración de Auth0 - Se carga dinámicamente desde el servidor
      let AUTH0_CONFIG = null;
...
<Accordion title="Seguridad de RPC en el servidor">
  Refuerza la seguridad agregando validación adicional y límites de frecuencia a tus métodos RPC:
  ...
  class RpcConnectionManager {
    ...
    async connect() {
      ...
      try {
        ...
        // Crea una nueva sesión de RPC por WebSocket
...
<Accordion title="Integración con base de datos">
  Sustituye el almacenamiento en memoria por una base de datos para usarla en producción:
