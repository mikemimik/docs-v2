---
mode: wide
description: "Esta guía muestra cómo integrar Auth0, añadir autenticación y mostrar la información del perfil del usuario en una aplicación de página única (SPA) que usa Cap'n Web RPC con Auth0 SPA SDK."
...
<Accordion title="Prompt de IA" defaultOpen icon="microchip-ai" iconType="sharp-solid">
  **¿Usas IA para integrar Auth0?** Añade este prompt a Cursor, Windsurf, Copilot, Claude Code o tu IDE favorito con IA para acelerar el desarrollo.
  ...
  Usa newWebSocketRpcSession de capnweb y el SDK de Auth0 con las importaciones de módulos ES correctas:
...
Esta Quickstart muestra cómo añadir autenticación de Auth0 a una aplicación de Cap&#39;n Web. Crearás una aplicación web moderna basada en RPC con una función de inicio de sesión seguro utilizando el framework de Javascript de Cap&#39;n Web y Auth0 SPA SDK.
...
<Steps>
  ...
  <Step title="Instalar dependencias" stepNumber={2}>
    ...
    Instala los SDKs de Auth0 para la autenticación y verificación de tokens:
    ...
    <Info>
      **@auth0/auth0-spa-js** se utiliza del lado del cliente para gestionar la autenticación de usuario, los flujos de inicio de sesión y los tokens en el navegador.

      **@auth0/auth0-api-js** se utiliza del lado del servidor para verificar los tokens de acceso y validar las firmas JWT con el JWKS de Auth0.
    </Info>
  ...
  <Step title="Configura tu Auth0 App" stepNumber={3}>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        3. En la ventana emergente, escribe un nombre para tu aplicación, selecciona `Single Page Web Application` como tipo de aplicación y haz clic en **Create**
  ...
  <Step title="Crear el servidor" stepNumber={5}>
    Crea el servidor Cap&#39;n Web con la integración de Auth0:
    ...
    // Servicio de perfil - objetivo RPC de Cap'n Web
    class ProfileService extends RpcTarget {
      async getProfile(accessToken) {
        const decoded = await verifyToken(accessToken);
        const userId = decoded.sub;
...
<Accordion title="Seguridad de RPC del servidor">
...
<Accordion title="Gestión de conexiones WebSocket">
  Implementa una gestión adecuada de las conexiones WebSocket con reconexión automática:
  ...
  // Conéctate cuando te autentiques
  if (isAuthenticated) {
