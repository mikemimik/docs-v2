---
title: Añade el inicio de sesión a tu aplicación React
sidebarTitle: React
description: Esta guía muestra cómo integrar Auth0, añadir autenticación y mostrar información del perfil de usuario en una aplicación de página única (SPA) que usa React mediante el SDK de Auth0 para React.
mode: wide
---
...
<Accordion title="Usa IA para integrar Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **Luego, pídele a tu asistente de IA:**
  ...
  ```text
  Agrega autenticación con Auth0 a mi app de React
  ```
...
<Note>
  ...
  **Compatibilidad de versiones de React:** Este Quickstart funciona con **React 18.x** y versiones posteriores.
</Note>
...
Este Quickstart muestra cómo añadir autenticación de Auth0 a una aplicación de React. Crearás una aplicación web de página única segura con funcionalidades de inicio de sesión, cierre de sesión y perfil de usuario mediante el SDK de Auth0 para React.
...
<Steps>
  <Step title="Crear un nuevo proyecto" stepNumber={1}>
    Crea un proyecto nuevo de React para este Quickstart
    ...
    <Tabs>
      ...
      <Tab title="CLI">
        ...
        <Note>
          ...
          1. Comprobará si ya te has autenticado (y te pedirá iniciar sesión si es necesario)
...
<Check>
  **Verificación**
  ...
  Ahora deberías tener una página de inicio de sesión de Auth0 completamente funcional en [localhost](http://localhost:5173/)
</Check>
...
<Accordion title="Llamar a APIs protegidas">
  Configura tu Auth0Provider para incluir la audience de una API y usar el método `getAccessTokenSilently`:
...
<Accordion title="Usar Auth0 con hooks personalizados">
  Crea hooks personalizados reutilizables para patrones de autenticación habituales:
