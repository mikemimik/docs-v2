...
sidebarTitle: React
description: Esta guía muestra cómo integrar Auth0, añadir autenticación y mostrar la información del perfil del usuario en una aplicación de página única (SPA) creada con React mediante el SDK de Auth0 para React.
mode: wide
---
...
<Accordion title="Usa IA para integrar Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **Luego, pídele a tu asistente de IA:**
  ...
  Tu asistente de IA creará automáticamente tu aplicación de Auth0, obtendrá las credenciales, instalará `@auth0/auth0-react`, configurará el Auth0Provider y creará todos los componentes necesarios. [Documentación completa de Agent Skills →](/docs/es/quickstart/agent-skills)
</Accordion>
...
Esta guía de inicio rápido muestra cómo agregar autenticación de Auth0 a una aplicación de React. Crearás una aplicación web de página única segura con funcionalidades de inicio de sesión, cierre de sesión y perfil de usuario usando el SDK de Auth0 para React.
...
<Steps>
  ...
  <Step title="Instalar el SDK de Auth0 para React" stepNumber={2}>
    ```shellscript
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        **Allowed Web Origins:**
        ...
        <Info>
          **Allowed Callback URLs** son una medida de seguridad esencial para garantizar que los usuarios regresen de forma segura a tu aplicación después de la autenticación. Sin una URL coincidente, el proceso de inicio de sesión fallará y los usuarios verán una página de error de Auth0 en lugar de acceder a tu aplicación.

          **Allowed Logout URLs** son esenciales para ofrecer una experiencia de usuario fluida al cerrar sesión. Sin una URL coincidente, los usuarios no serán redirigidos de vuelta a tu aplicación después del cierre de sesión, sino que se quedarán en una página genérica de Auth0.
  ...
  <Step title="Agregar el proveedor" stepNumber={4}>
  ...
  <Step title="Ejecuta tu aplicación" stepNumber={7}>
    ...
    <Info>
      Si el puerto 5173 está en uso, ejecuta: `npm run dev -- --port 5174` y actualiza las URL de devolución de llamada de tu app de Auth0 a `http://localhost:5174`
    </Info>
  </Step>
</Steps>
...
<Check>
  **Comprobación**
  ...
  Ahora deberías tener una Auth0 login page completamente funcional ejecutándose en [localhost](http://localhost:5173/)
</Check>
...
<Accordion title="Protección de rutas con React Router">
  Usa el estado de autenticación de Auth0 para proteger rutas específicas en tu aplicación:
...
<Accordion title="Calling Protected APIs">
  Configura tu Auth0Provider para incluir una audience de la API y usar el método `getAccessTokenSilently`:
  ...
  Luego, haz llamadas a la API autenticadas:
