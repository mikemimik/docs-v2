...
description: Esta guía muestra cómo integrar Auth0, añadir autenticación y mostrar información del perfil del usuario en cualquier aplicación de Angular con el SDK de Auth0 para Angular.
mode: wide
---
...
Esta guía de inicio rápido muestra cómo agregar la autenticación de Auth0 a una aplicación Angular. Crearás una aplicación segura de una sola página con funciones de inicio y cierre de sesión mediante el sistema de inyección de dependencias de Angular y el SDK de Auth0 para Angular.
...
<Steps>
  <Step title="Crear un proyecto nuevo" stepNumber={1}>
    Crea un nuevo proyecto de Angular para este Quickstart
  ...
  <Step title="Configura tu Auth0 App" stepNumber={3}>
    ...
    <Tabs>
      <Tab title="Setup rápido (recomendado)">
        Crea una Auth0 App y copia el archivo `.env` ya completado con los valores de configuración adecuados.
  ...
  <Step title="Configurar el módulo de Auth0" stepNumber={4}>
    Con el archivo de entorno listo del paso anterior, agrega `provideAuth0` al arreglo de proveedores en tu `app.config.ts`:
    ...
    <Note>
      Si configuras tu aplicación de Auth0 manualmente desde el Dashboard, crea `src/environments/environment.ts` con tu dominio y el ID de cliente del Dashboard.

      Los proyectos de Angular 20+ también incluyen `provideBrowserGlobalErrorListeners()` en este archivo; mantenlo en el arreglo junto con `provideAuth0`. No necesitas hacer cambios en `main.ts`.
    </Note>
  </Step>

  <Step title="Crear componentes de inicio de sesión, cierre de sesión y perfil" stepNumber={5}>
    Usa el Angular CLI para crear los archivos del componente:
    ...
    Ahora actualiza el componente principal de la aplicación y agrega estilos:
...
<Check>
  **Verificación**
  ...
  Ahora ya deberías tener una página de inicio de sesión de Auth0 completamente funcional en tu [localhost](http://localhost:4200/)
</Check>
...
<Accordion title="Uso del enfoque tradicional de NgModule">
  Si creaste tu proyecto con `--standalone=false` o prefieres usar NgModules en lugar de componentes independientes, así es como puedes configurar el SDK en proyectos generados con Angular CLI 20+:
  ...
  <Note>
    ...
    **Angular 20+** genera `app.ts`, `app-module.ts` y `app-routing-module.ts` (sin puntos en los nombres de archivo), y `main.ts` usa `platformBrowser`, como se muestra arriba.
    ...
    En todos los casos, los proyectos con NgModule se inician con `bootstrapModule()` en lugar del enfoque `bootstrapApplication()` que se muestra en el quickstart principal.
  </Note>
</Accordion>
