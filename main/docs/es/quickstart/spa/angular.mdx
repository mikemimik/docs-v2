...
description: Esta guía muestra cómo integrar Auth0, agregar autenticación y mostrar información del perfil del usuario en cualquier aplicación Angular mediante el Auth0 Angular SDK.
mode: wide
---
...
<Accordion title="Usa IA para integrar Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  Si usas un AI coding assistant como Claude Code, Cursor o GitHub Copilot, puedes agregar autenticación de Auth0 automáticamente en minutos con [Agent Skills](https://agentskills.io/home).
...
<Note>
  ...
  * **[jq](https://jqlang.org/)** - Obligatorio para configurar Auth0 CLI
...
Esta guía de Quickstart muestra cómo añadir la autenticación de Auth0 a una aplicación Angular. Crearás una aplicación web de página única segura con funciones de inicio y cierre de sesión mediante el sistema de inyección de dependencias de Angular y el Auth0 Angular SDK.
...
<Steps>
  <Step title="Crea un nuevo proyecto" stepNumber={1}>
    Crea un nuevo proyecto de Angular para esta guía de Quickstart
  ...
  <Step title="Instala el SDK de Auth0 para Angular" stepNumber={2}>
    ```shellscript
  ...
  <Step title="Configura tu Auth0 App" stepNumber={3}>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        5. Sustituye `YOUR_AUTH0_APP_DOMAIN` y `YOUR_AUTH0_APP_CLIENT_ID` en el archivo `src/environments/environment.ts` por los valores de **Domain** e **ID de cliente** del Dashboard
  ...
  <Step title="Configurar el módulo de Auth0" stepNumber={4}>
    ...
    <Note>
      ...
      Los proyectos de Angular 20+ también incluyen `provideBrowserGlobalErrorListeners()` en este archivo; mantenlo en el arreglo junto con `provideAuth0`. No es necesario hacer cambios en `main.ts`.
    </Note>
  </Step>
  ...
  <Step title="Crear componentes de Login, cierre de sesión y perfil" stepNumber={5}>
    ...
    <Tabs>
      <Tab title="Componente de la app">
  ...
  <Step title="Ejecuta tu aplicación" stepNumber={6}>
    ...
    <Info>
      Si el puerto 4200 está en uso, ejecuta: `ng serve --port 4201` y actualiza las URL de devolución de llamada de tu aplicación de Auth0 a `http://localhost:4201`
    </Info>
  </Step>
</Steps>
...
<Check>
  **Verificación**
  ...
  Ahora ya deberías tener funcionando correctamente la página de inicio de sesión de Auth0 en tu [localhost](http://localhost:4200/)
</Check>
...
<Accordion title="Uso del enfoque tradicional de NgModule">
  Si creaste tu proyecto con `--standalone=false` o prefieres usar NgModules en lugar de componentes independientes, así es como puedes configurar el SDK en proyectos generados con Angular CLI de Angular 20+:
