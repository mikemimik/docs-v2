...
description: Esta guía muestra cómo integrar Auth0, añadir autenticación y mostrar información del perfil del usuario en cualquier aplicación Angular mediante el SDK de Auth0 para Angular.
mode: wide
---
...
<Note>
  ...
  **Compatibilidad de versiones de Angular:** Esta guía de inicio rápido funciona con **Angular 19 a 21** usando Angular CLI. El SDK `@auth0/auth0-angular` es compatible con Angular 13 y versiones posteriores.
</Note>
...
Esta guía de inicio rápido muestra cómo añadir la autenticación de Auth0 a una aplicación de Angular. Crearás una aplicación web de página única segura con funciones de inicio y cierre de sesión mediante el sistema de inyección de dependencias de Angular y el SDK de Auth0 para Angular.
...
<Steps>
  ...
  <Step title="Instalar el SDK de Angular de Auth0" stepNumber={2}>
    ```shellscript
  ...
  <Step title="Configura tu Auth0 App" stepNumber={3}>
    ...
    <Tabs>
      <Tab title="Setup rápido (recomendado)">
        Crea una Auth0 App y copia el archivo `.env` prerrellenado con los valores de configuración adecuados.
      ...
      <Tab title="Dashboard">
        ...
        5. Reemplaza `YOUR_AUTH0_APP_DOMAIN` y `YOUR_AUTH0_APP_CLIENT_ID` en el archivo `src/environments/environment.ts` por los valores de **Domain** e **ID de cliente** del Dashboard
        ...
        **Orígenes web permitidos:**
        ...
        <Info>
          ...
          **Allowed Web Origins** es fundamental para la autenticación silenciosa. Sin esto, los usuarios cerrarán sesión al actualizar la página o al volver más tarde a tu aplicación.
        </Info>
      </Tab>
    </Tabs>
  </Step>
  ...
  <Step title="Configurar el módulo de Auth0" stepNumber={4}>
    Con tu archivo de entorno ya configurado en el paso anterior, agrega `provideAuth0` al arreglo `providers` de tu `app.config.ts`:
    ...
    <Note>
      Si configuras manualmente tu aplicación de Auth0 desde el Dashboard, crea `src/environments/environment.ts` con tu dominio y tu ID de cliente del Dashboard.

      Los proyectos de Angular 20+ también incluyen `provideBrowserGlobalErrorListeners()` en este archivo; mantenlo en el arreglo junto a `provideAuth0`. No es necesario hacer cambios en `main.ts`.
    </Note>
  </Step>

  <Step title="Crear componentes de Login, cierre de sesión y perfil" stepNumber={5}>
    ...
    <Tabs>
      <Tab title="Componente de la aplicación">
        Reemplaza el contenido del archivo del componente principal de tu aplicación (`src/app/app.ts` en Angular 20+, o `src/app/app.component.ts` en Angular 19):
...
<Check>
  **Verificación**
  ...
  Ahora deberías tener una página de inicio de sesión de Auth0 completamente funcional en tu [localhost](http://localhost:4200/)
</Check>
...
<Accordion title="Uso del enfoque tradicional de NgModule">
  ...
  <Note>
    ...
    **Angular 20+** genera `app.ts`, `app-module.ts` y `app-routing-module.ts` (sin puntos en los nombres de archivo), y `main.ts` usa `platformBrowser`, como se muestra arriba.
    ...
    En todos los casos, los proyectos con NgModule arrancan con `bootstrapModule()` en lugar del enfoque `bootstrapApplication()` que se muestra en la guía de inicio rápido principal.
  </Note>
</Accordion>
...
<Accordion title="Proteger rutas con AuthGuard">
  ...
  Luego, agrega `provideAuth0` y el router a tu `app.config.ts` (si todavía no lo hiciste en el Paso 4):
...
<Accordion title="Llamar a APIs protegidas">
  Configura el interceptor HTTP para adjuntar automáticamente tokens a las llamadas a la API. Agrega `provideHttpClient` con el interceptor de Auth0 y un `audience` a tu `app.config.ts`:
