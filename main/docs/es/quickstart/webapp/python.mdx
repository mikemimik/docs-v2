...
<Note>
  **Prerrequisitos:** Antes de comenzar, asegúrate de tener instalados los siguientes elementos:
  ...
  **Compatibilidad de versiones de Flask:** Esta guía de inicio rápido usa **Flask 2.0+** con el complemento `[async]` para habilitar la compatibilidad asíncrona.
</Note>
...
Esta guía de inicio rápido muestra cómo añadir autenticación de Auth0 a una aplicación Flask. Crearás una aplicación web segura con funcionalidad de inicio de sesión, rutas protegidas y acceso al perfil del usuario mediante el [Auth0 WebApp Python SDK](https://github.com/auth0/auth0-server-python).

<Steps>
  <Step title="Configura tu entorno" stepNumber={1}>
    Crea un nuevo directorio para tu proyecto Flask:
  ...
  <Step title="Instala las dependencias" stepNumber={2}>
    ...
    El archivo `requirements.txt` contiene todas las dependencias del proyecto. Para instalarlas, ejecuta:
  ...
  <Step title="Configura tu Auth0 App" stepNumber={3}>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        3. Ponle un nombre a tu aplicación (por ejemplo, &quot;My Flask App&quot;) y selecciona **Regular Web Application**
        ...
        9. Reemplaza `YOUR_CLIENT_SECRET` en `.env` por tu **secreto de cliente**
  ...
  <Step title="Crear la configuración de Auth, las rutas y las plantillas" stepNumber={4}>
    ...
    <Info>
      **Solo para desarrollo:** Este ejemplo usa clases sencillas de almacenamiento en memoria (`MemoryStateStore` y `MemoryTransactionStore`) con fines de demostración. Estos almacenamientos perderán todos los datos de la sesión cuando la aplicación se reinicie y no funcionarán en implementaciones con varias instancias.
...
<Check>
  **Punto de control**
  ...
  Ahora deberías tener una página de inicio de sesión de Auth0 totalmente funcional en tu [localhost](http://localhost:5000/)
</Check>
...
<Accordion title="Acceso a tokens de API">
  ...
  <Info>
    ...
    2. Incluir `offline_access` en tus scopes (para los tokens de actualización)
...
<Accordion title="Configuración de almacenamiento personalizado">
  De forma predeterminada, el SDK usa almacenamiento en cookies. Para entornos de producción con necesidades específicas (escalado horizontal, sesiones compartidas entre servicios), puedes configurar backends de almacenamiento personalizados, como Redis o PostgreSQL.
  ...
  * Necesitas una gestión centralizada de sesiones para backchannel logout
  ...
  **Ejemplo con Redis:**
  ...
  # Inicializa un almacenamiento de state personalizado
  ...
  <Info>
    Para la mayoría de las aplicaciones, el almacenamiento predeterminado en cookies es suficiente. El almacenamiento personalizado requiere implementar la interfaz `StateStore`. Consulta los [ejemplos del SDK](https://github.com/auth0/auth0-server-python/blob/main/examples/ConfigureStore.md) para ver implementaciones detalladas.
  </Info>
</Accordion>
...
<AccordionGroup>
  <Accordion title="MissingRequiredArgumentError">
    ...
    **Solución:**
    ...
    3. Genera una nueva clave secreta si es necesario: `openssl rand -hex 64`
    4. Agrégala a `.env`: `AUTH0_SECRET=your_generated_secret`
    5. Reinicia tu aplicación Flask
  </Accordion>

  <Accordion title="Error de URL de devolución de llamada inválida">
    **Problema:** Aparece el error &quot;Callback URL mismatch&quot; o &quot;invalid&#95;request&quot; durante el inicio de sesión

    **Causa:** La URI de redirección de tu código no coincide con la que está registrada en Auth0 Dashboard.

    **Solución:**

    1. Comprueba tu archivo `.env`: `AUTH0_REDIRECT_URI=http://localhost:5000/callback`
    2. Ve a [Auth0 Dashboard](https://manage.auth0.com/dashboard/) → Applications → Your App → Settings
    3. Añade `http://localhost:5000/callback` a **Allowed Callback URLs**
    4. Haz clic en **Save Changes**
    5. Reinicia tu aplicación de Flask
  </Accordion>

  <Accordion title="Errores del bucle de eventos de AsyncIO">
    **Problema:** Ves &quot;RuntimeError: This event loop is already running&quot; u otros errores asíncronos similares
  ...
  <Accordion title="Errores de «módulo no encontrado»">
    **Problema:** Ves &quot;ModuleNotFoundError: No module named &#39;auth0&#95;server&#95;python&#39;&quot; o un mensaje similar
  ...
  <Accordion title="ClaimDecodingFailed Error">
    **Problema:** Ves el error &quot;ClaimDecodingFailed&quot; o &quot;No se pudieron decodificar los claims&quot; durante la autenticación

    **Causa:** El token de identidad o el access token recibido de Auth0 no se pudo decodificar correctamente, a menudo por alguna de estas causas:

    * Formato de JWT no válido
    * Datos de sesión dañados
    * Incompatibilidad entre algoritmos de firma
    * Desfase horario entre tu servidor y Auth0

    **Solución:**

    1. Asegúrate de que `AUTH0_CLIENT_SECRET` sea correcto en el archivo `.env`
    2. Verifica que la hora de tu sistema esté sincronizada (NTP):
       ```bash
       # macOS
    ...
    5. Comprueba que en Auth0 Dashboard → Applications → Your App → Settings → Advanced → OAuth → JsonWebToken Signature Algorithm coincida con la configuración de tu SDK
  </Accordion>

  <Accordion title="Token expirado o no válido">
    **Problema:** Ves errores de &quot;El token ha expirado&quot; o &quot;invalid&#95;token&quot;

    **Causa:** El access token o token de ID ha superado su tiempo de validez, o la sesión ha expirado.
    ...
    **Solución:**
    ...
    2. Para las API, asegúrate de solicitar tokens nuevos:
       ...
       ```
    3. Ajusta la duración de los tokens en Auth0 Dashboard → Applications → Your App → Settings → Advanced → OAuth
    4. Implementa un manejo adecuado de errores para redirigir a los usuarios a iniciar sesión cuando los tokens expiren
  </Accordion>
  ...
  <Accordion title="CORS y errores de origen cruzado">
    ...
    **Solución:**
    ...
    1. Agrega el origen de tu aplicación en Auth0 Dashboard → Applications → Your App → Settings:
       ...
       * **Allowed Callback URLs**: `http://localhost:5000/callback`
       * **Allowed Logout URLs**: `http://localhost:5000`
    2. Para producción, agrega tus URL de producción:
       ```
       https://yourdomain.com
       https://yourdomain.com/callback
       ```
    3. Asegúrate de que tu aplicación Flask tenga una configuración de CORS adecuada si llamas a las APIs de Auth0 desde el frontend en JavaScript:
       ```python
  ...
  <Accordion title="Límite de frecuencia excedido">
    ...
    **Solución:**
    ...
    1. Revisa Auth0 Dashboard → Monitoreo → Registros para ver los detalles de los límites de frecuencia
    2. Implementa reintentos con retroceso exponencial:
       ```python
       import time
