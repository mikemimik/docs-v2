---
mode: wide
description: 'Añade autenticación de Auth0 a una aplicación web de Flask con inicio de sesión, rutas protegidas y perfiles de usuario'
sidebarTitle: Flask
...
<Note>
  ...
  **Compatibilidad de versiones de Flask:** En este Quickstart se usa **Flask 2.0+** con el extra `[async]` para compatibilidad con funciones asíncronas.
</Note>
...
En esta Quickstart se muestra cómo añadir la autenticación de Auth0 a una aplicación Flask. Crearás una aplicación web segura con funcionalidad de inicio de sesión, rutas protegidas y acceso al perfil de usuario mediante el [SDK de Python para aplicaciones web de Auth0](https://github.com/auth0/auth0-server-python).
...
<Steps>
  ...
  <Step title="Instala las dependencias" stepNumber={2}>
    ...
    El archivo `requirements.txt` recopila todas las dependencias del proyecto. Para instalarlas, ejecuta:
  ...
  <Step title="Crea la configuración, las rutas y las plantillas de Auth" stepNumber={4}>
    Crear archivos
    ...
    <AuthCodeGroup>
      ...
      <meta charset="UTF-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>Perfil de usuario - Flask + Auth0</title>
      <link rel="stylesheet" href="{{ url_for('static', filename='style.css') }}">
      </head>
      <body>
      <div class="container">
          <div class="card">
              ...
              <div class="profile-details">
                  {% if user.picture %}
                  <img src="{{ user.picture }}" alt="Perfil" class="profile-pic-large">
                  {% endif %}
...
<Accordion title="Acceso a tokens de API">
  Si necesitas hacer una llamada a una API protegida, obtén un token de acceso:
...
<Accordion title="Configuración de almacenamiento personalizado">
  De forma predeterminada, el SDK usa almacenamiento basado en cookies. Para entornos de producción con necesidades específicas (escalado horizontal o sesiones compartidas entre servicios), puedes configurar backends de almacenamiento personalizados como Redis o PostgreSQL.
...
<AccordionGroup>
  <Accordion title="MissingRequiredArgumentError">
    ...
    **Causa:** La variable de entorno `AUTH0_SECRET` falta o no se carga correctamente.
  ...
  <Accordion title="Errores del bucle de eventos de AsyncIO">
    ...
    **Causa:** La compatibilidad async de Flask 2.0+ puede dar problemas con determinadas configuraciones.

    **Solución:**

    Instala Flask con compatibilidad async:
  ...
  <Accordion title="ClaimDecodingFailed Error">
    **Problema:** Aparece el error &quot;ClaimDecodingFailed&quot; o &quot;Failed to decode claims&quot; durante la autenticación

    **Causa:** El token de identidad o token de acceso recibido de Auth0 no se pudo decodificar correctamente, a menudo por alguna de estas causas:
    ...
    **Solución:**
    ...
    5. Verifica que Auth0 Dashboard → Applications → Tu aplicación → Settings → Advanced → OAuth → JsonWebToken Signature Algorithm coincida con la configuración de tu SDK
  </Accordion>

  <Accordion title="Token caducado o no válido">
    **Problema:** Aparecen los errores &quot;Token has expired&quot; o &quot;invalid&#95;token&quot;

    **Causa:** El token de acceso o el token de identidad ha superado su duración, o la sesión ha expirado.
    ...
    **Solución:**
    ...
    1. El SDK gestiona automáticamente la renovación del token si incluyes el scope `offline_access`:
       ...
       ```
    2. Para las API, asegúrate de solicitar tokens actualizados:
       ```python
       access_token = await auth0.get_access_token(
           audience='https://your-api.example.com',
       ...
       ```
    3. Ajusta la duración de los tokens en Auth0 Dashboard → Applications → Tu aplicación → Settings → Advanced → OAuth
    4. Implementa un manejo de errores adecuado para redirigir a los usuarios al inicio de sesión cuando los tokens expiren
  </Accordion>
  ...
  <Accordion title="CORS y errores de origen cruzado">
    ...
    **Solución:**
    ...
    1. Agrega el origen de tu aplicación en Auth0 Dashboard → Applications → Your App → Settings:
       ...
       * **Allowed Callback URLs**: `http://localhost:5000/callback`
       * **Allowed Logout URLs**: `http://localhost:5000`
    2. Para Production, agrega las URL de producción:
       ```
       https://yourdomain.com
       https://yourdomain.com/callback
       ```
    3. Asegúrate de que tu aplicación Flask tenga una configuración de CORS adecuada si llamas a las API de Auth0 desde el JavaScript del frontend:
       ```python
  ...
  <Accordion title="Se excedió el límite de frecuencia">
    **Problema:** Ves errores de &quot;Too many requests&quot; o &quot;Rate limit exceeded&quot;

    **Causa:** Tu aplicación ha superado los límites de frecuencia de Auth0 para las solicitudes de autenticación.

    **Solución:**

    1. Revisa Auth0 Dashboard → Monitoreo → Logs para consultar los detalles del límite de frecuencia
           ...
           for attempt in range(max_retries):
               try:
                   ...
                   if e.status_code == 429 and attempt < max_retries - 1:
                       ...
                       raise
       ```
    3. Revisa los límites del plan de tu suscripción de Auth0
    4. Optimiza el flujo de autenticación para reducir las solicitudes de tokens innecesarias
    5. Usa la caché de tokens de forma adecuada en lugar de solicitar nuevos con frecuencia
  </Accordion>
</AccordionGroup>