---
mode: wide
description: Esta guía muestra cómo integrar Auth0, añadir autenticación y mostrar información del perfil de usuario en una aplicación de Python con el Auth0 FastAPI SDK.
...
<Accordion title="Prompt de IA" defaultOpen icon="microchip-ai" iconType="sharp-solid">
  **¿Usas IA para integrar Auth0?** Añade este prompt a Cursor, Windsurf, Copilot, Claude Code o tu IDE favorito con IA para acelerar el desarrollo.
  ...
  Replace or create main.py with this complete, production-ready code:
    ...
    # Vincular al estado de la aplicación FastAPI
        ...
        if session:
            ...
            <body>
                <div class="container">
                    ...
                    <h1>Bienvenido a Auth0 FastAPI</h1>
                    <div class="success">✅ ¡Autenticado correctamente!</div>
                    <h2>Tu perfil</h2>
                    <div class="profile">
                    ...
                    <a href="/auth/logout" class="button logout">Cerrar sesión</a>
                </div>
            </body>
            </html>
            """
            ...
            <body>
                <div class="container">
                    ...
                    <div class="action-card">
                        <p class="action-text">Comienza iniciando sesión en tu cuenta</p>
                        <a href="/auth/login" class="button login">Iniciar sesión</a>
                    </div>
                </div>
            </body>
            </html>
  ...
  1.  NUNCA incluyas credenciales de Auth0 directamente en el código. Usa siempre variables de entorno.
  ...
  4.  NUNCA uses uvicorn[standard] sin comillas en comandos de shell. Provocará errores de expansión de glob.
  ...
  Problema 1: ModuleNotFoundError: No module named 'itsdangerous'
  ...
  Problema 2: Error de expansión glob en shell con uvicorn[standard]
  Problema: Los corchetes se interpretan como un patrón glob
  Solución: Escribir el paquete entre comillas: pip install "uvicorn[standard]"

  Problema 3: Las sesiones no persisten
  Problema: Falta SessionMiddleware o el secret es incorrecto
  Solución: Añadir SessionMiddleware con el secret_key adecuado desde el entorno

  Problema 4: Se requiere HTTPS en producción
...
<Steps>
      ...
      <Tab title="Dashboard">
        ...
        **Orígenes web permitidos:**
        ...
        <Info>
          ...
          **Allowed Logout URLs** son esenciales para ofrecer una experiencia de usuario fluida al cerrar sesión. Sin una URL que coincida, los usuarios no serán redirigidos de vuelta a su aplicación después del cierre de sesión, sino que se quedarán en una página genérica de Auth0.

          **Allowed Web Origins** es fundamental para la autenticación silenciosa. Sin esta configuración, los usuarios cerrarán sesión al actualizar la página o al volver más tarde a su aplicación.
        </Info>
      </Tab>
    </Tabs>
  </Step>
  ...
  <Step title="Configurar el Auth0 FastAPI SDK" stepNumber={4}>
    ...
    # Adjuntar al estado de la aplicación de FastAPI
    app.state.config = config
    app.state.auth_client = auth_client
    ...
    <Info>
      ...
      **SessionMiddleware** debe agregarse antes de usar el SDK. Sin él, FastAPI no puede leer ni establecer cookies, y todos los intentos de autenticación fallarán sin mostrar ningún error.

      **HTTPS en Production** es obligatorio para las cookies seguras (`secure=True`). Sin HTTPS, los navegadores no enviarán las cookies de sesión y los usuarios tendrán que volver a iniciar sesión después de cada request.
    </Info>
  </Step>
  ...
  <Step title="Crear rutas y mostrar el perfil del usuario" stepNumber={5}>
    ...
    * Una página de inicio (`/`) que muestra un botón de inicio de sesión cuando la sesión no está iniciada, o el perfil del usuario cuando sí lo está
...
<Accordion title="Llamar a Protected APIs con Access Tokens">
  ...
  # Configurar Auth0 con audience de la API
  config = Auth0Config(
      domain=os.getenv("AUTH0_DOMAIN"),
      client_id=os.getenv("AUTH0_CLIENT_ID"),
      client_secret=os.getenv("AUTH0_CLIENT_SECRET"),
  ...
  async def get_external_data(
      ...
      # Hacer una llamada autenticada a la API
...
<Accordion title="Uso de Redis para el almacenamiento de sesiones con estado">
  ...
  * **Sin límites de tamaño de cookies** - Almacene una cantidad ilimitada de datos de sesión
  * **Invalidación inmediata** - Elimine sesiones del lado del servidor
  * **Compatibilidad con backchannel logout** - Gestione eventos de logout de Auth0
...
<Accordion title="Las sesiones no se mantienen / a los usuarios se les cierra la sesión repetidamente">
  **Problema**: Los usuarios inician sesión, pero las sesiones no se mantienen entre solicitudes.
...
<Accordion title="Error de Auth0: la URL de devolución de llamada no coincide">
  ...
  **Solución**:
  ...
  2. Agrega tu URL de devolución de llamada a **Allowed Callback URLs**:
     ...
     http://localhost:3000/auth/callback
     ```
  3. Para producción, agrega tu URL de producción:
     ```
...
<Accordion title="ImportError o problemas con async/await">
  **Problema**: Errors relacionados con funciones async o bucles de eventos.

  **Causa**: FastAPI es un framework async y todos los métodos del SDK deben usar `await`.
  ...
  **Solución**: Asegúrate de que todas las funciones de ruta sean async y de que los métodos del SDK usen `await` correctamente:
  ...
  @app.get("/profile")
  async def profile(request: Request, response: Response):
      user = auth_client.client.get_user(...)  # Devuelve una coroutine, no datos
      return {"user": user}
  ```
</Accordion>
...
<Accordion title="HTTPS obligatorio en producción">
  ...
  **Causa**: Las cookies seguras requieren HTTPS en producción. El indicador `secure=True` impide que las cookies se envíen a través de conexiones HTTP no cifradas.
...
Para ver funcionalidades y opciones de configuración más avanzadas, consulta la [documentación de Auth0 FastAPI SDK](https://github.com/auth0/auth0-fastapi/blob/main/README.md).