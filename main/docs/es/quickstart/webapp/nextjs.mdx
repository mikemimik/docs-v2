---
title: Añade el inicio de sesión a tu aplicación de Next.js
sidebarTitle: Next.js
description: Esta guía muestra cómo integrar Auth0 en cualquier aplicación nueva o existente de Next.js mediante el SDK de Auth0 para Next.js v4.
mode: wide
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  ...
  * **[Node.js](https://nodejs.org/en/download)** 20 LTS o superior
...
Esta guía de inicio rápido muestra cómo agregar autenticación de Auth0 a una aplicación de Next.js 16. Crearás una aplicación web full stack con renderización en el servidor, funcionalidad de inicio de sesión segura y rutas protegidas con el SDK de Auth0 para Next.js.
...
<Steps>
    ...
    <Tabs>
      ...
      <Tab title="CLI">
        ...
        <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
          ...
          1. Comprobará si ya estás autenticado (y mostrará el prompt de inicio de sesión si hace falta)
      ...
      <Tab title="Dashboard">
        ...
        **Genera un AUTH0&#95;SECRET seguro:**
        ...
        <Warning>
          **Importante:** Si cambias el `AUTH0_SECRET` después de que la aplicación haya estado ejecutándose, debes borrar las cookies de tu navegador para localhost. Las session cookies antiguas cifradas con el secreto anterior provocarán un error `JWEDecryptionFailed`.
        </Warning>
        ...
        **Allowed Web Origins:**
        ...
        <Info>
          ...
          **Allowed Web Origins** es fundamental para la autenticación silenciosa. Sin esta configuración, la sesión de los usuarios se cerrará cuando actualicen la página o vuelvan más tarde a tu aplicación.
        </Info>
      </Tab>
    </Tabs>
  </Step>

  <Step title="Crea la configuración de Auth0" stepNumber={5}>
    Agrega el código del cliente de Auth0 a `src/lib/auth0.ts`:
  ...
  <Step title="Añadir proxy" stepNumber={6}>
    Añade el código del proxy en `src/proxy.ts`:
    ...
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      Como estamos usando un directorio `src/`, el archivo `proxy.ts` se crea dentro de `src/`. Si no estás usando un directorio `src/`, créalo en la raíz del proyecto.
    </Callout>
    ...
    <Info>
      ...
      * `/auth/backchannel-logout` - Ruta de cierre de sesión por canal secundario
    </Info>
  </Step>

  <Step title="Crea componentes para Login, cierre de sesión y perfil" stepNumber={7}>
    Añade el código del componente a los archivos creados en el Paso 3:
  ...
  <Step title="Actualiza el layout con Auth0Provider" stepNumber={9}>
    Actualiza `src/app/layout.tsx` para cargar la fuente Inter y envolver tu aplicación en `Auth0Provider`:
  ...
  <Step title="Configurar Tailwind CSS" stepNumber={10}>
    Sustituye el contenido de `src/app/globals.css` por:
  ...
  <Step title="Ejecutar la aplicación" stepNumber={11}>
    ```shellscript
    npm run dev
    ```
...
<Check>
  **Comprobación**
  ...
  Ahora deberías tener una Auth0 login page completamente funcional ejecutándose en [localhost](http://localhost:3000/)
</Check>
...
<Accordion title="Error JWEDecryptionFailed">
  Si ves el error `JWEDecryptionFailed: decryption operation failed`, se debe a que `AUTH0_SECRET` no es válido o a que hay una cookie de sesión antigua cifrada con un secreto diferente.
  ...
  **Solución:**
  ...
  3. **Borra las cookies de tu navegador** para `localhost:3000`:
     * Chrome/Edge: Pulsa `F12` → pestaña Application → Cookies → Delete all cookies for localhost
     * Firefox: Pulsa `F12` → pestaña Storage → Cookies → Delete all cookies for localhost
  ...
  El secreto debe tener exactamente 32 bytes (64 caracteres hexadecimales). El error se produce cuando la aplicación intenta descifrar una cookie de sesión existente que fue cifrada con un secreto distinto.
</Accordion>
...
<Accordion title="Error 404 en /auth/login">
  ...
  3. **Reinicia el servidor:** Después de crear el archivo de proxy, reinicia el servidor de desarrollo
...
<Accordion title="Errores de Module Not Found">
  Si ves &quot;Cannot find module &#39;@/components/LoginButton&#39;&quot; u otros errores similares:

  1. **Verifica que los archivos existan:** Comprueba que se hayan creado todos los archivos del Paso 3
  2. **Comprueba las rutas:** Asegúrate de que los componentes estén en el directorio `src/components/`
  3. **Reinicia TypeScript:** Pulsa `Cmd+Shift+P` (Mac) o `Ctrl+Shift+P` (Windows) y ejecuta &quot;TypeScript: Restart TS Server&quot;
  4. **Verifica las importaciones:** Asegúrate de que estás usando `@/components/*` (no `~/components/*`)
</Accordion>
...
<Accordion title="Cambios importantes de la v4">
  Este Quickstart usa **SDK de Auth0 para Next.js v4**, que incorpora cambios importantes con respecto a la v3:

  * **No se necesitan manejadores de rutas dinámicas** - Las rutas de autenticación se montan automáticamente a través del proxy
  ...
  * **Usa etiquetas `<a>`** - La navegación debe usar `<a href="/auth/login">` en lugar de botones con `onClick`
  ...
  | `/auth/access-token`       | Obtener access token                               |
  | `/auth/backchannel-logout` | Gestionar el cierre de sesión por canal secundario |

  <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
    Si ves errores 404 en estas rutas, asegúrate de lo siguiente:

    1. El archivo `proxy.ts` está en la ubicación correcta (la raíz del proyecto o dentro de `src/` si usas un directorio `src/`)
    2. El proxy está configurado correctamente con el patrón `matcher` que se muestra en el Paso 6
    3. Reiniciaste el servidor de desarrollo después de crear el archivo del proxy
  </Callout>
</Accordion>

<Accordion title="Autenticación del lado del servidor">
  SDK de Auth0 para Next.js v4 admite tanto los patrones de App Router como los de Pages Router. Aquí tienes algunos patrones comunes del lado del servidor:
...
<Accordion title="Protección de rutas de API">
  Para proteger una ruta de API, usa el método `withApiAuthRequired`:
...
<Accordion title="Uso de tokens de Auth0 con backends de terceros">
  Si usas un servicio de backend de terceros (como Convex, Supabase o Firebase) que requiere tokens de autenticación de Auth0, tendrás que pasar el access token desde tu aplicación de Next.js al cliente del backend.

  ### Obtención del access token

  **del lado del servidor (App Router):**
  ...
  **del lado del cliente:**
  ...
  export function ConvexClientProvider({ children }: { children: React.ReactNode }) {
    return (
      <ConvexProviderWithAuth0
        client={convex}
        useAuth0={() => ({
          // Obtén el access token desde tu ruta de API de Next.js
  ...
  La mayoría de los servicios de terceros necesitan tu domain y audience de Auth0 para verificar los tokens. En la configuración de tu backend:
  ...
  <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
    Asegúrate de que tu aplicación de Auth0 esté configurada con un audience de API si tu backend lo requiere. Puedes configurarlo en el Auth0 Dashboard, en Applications → APIs, o agregar `AUTH0_AUDIENCE` a tu `.env.local` y configurar el SDK según corresponda.
  </Callout>
  ...
  1. **Verifica que se esté pasando el token:** Revisa la pestaña Network de las DevTools del navegador para confirmar que el token se incluye en las requests
  2. **Comprueba el formato del token:** Asegúrate de estar pasando el `accessToken`, no el `idToken`
  3. **Verifica la configuración del backend:** Confirma que tu backend tenga el domain de Auth0 y el ID de cliente correctos
  4. **Comprueba el audience:** Si usas una Auth0 API, asegúrate de que `AUTH0_AUDIENCE` esté configurado y coincida con tu API identifier
