...
description: Esta guía muestra cómo integrar Auth0 en cualquier aplicación nueva o existente de Next.js con el SDK v4 de Auth0 para Next.js.
mode: wide
...
Esta guía de inicio rápido muestra cómo añadir la autenticación de Auth0 a una aplicación de Next.js 16. Crearás una aplicación web full-stack con renderización del lado del servidor, funcionalidad de inicio de sesión segura y rutas protegidas con el Auth0 Next.js SDK.
...
<Steps>
  <Step title="Crea un proyecto nuevo" stepNumber={1}>
    Crea un nuevo proyecto en Next.js para este Quickstart
  ...
  <Step title="Instalar el SDK de Auth0 para Next.js" stepNumber={2}>
  ...
  <Step title="Configura tu Auth0 App" stepNumber={4}>
    ...
    Tienes tres opciones para configurar tu Auth0 app: usar la herramienta Quick Setup (recomendado), ejecutar un comando de CLI o configurar manualmente a través del Dashboard:
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        **Genera un AUTH0&#95;SECRET seguro:**
        ...
        <Warning>
          **Importante:** Si cambias `AUTH0_SECRET` después de que la aplicación haya estado ejecutándose, debes borrar las cookies de tu navegador para localhost. Las cookies de sesión antiguas cifradas con el secreto anterior provocarán un error `JWEDecryptionFailed`.
        </Warning>
  ...
  <Step title="Crea la configuración de Auth0" stepNumber={5}>
    Añade el código del cliente de Auth0 en `src/lib/auth0.ts`:
  ...
  <Step title="Crea componentes de inicio de sesión, cierre de sesión y perfil" stepNumber={7}>
  ...
  <Step title="Actualiza el layout con Auth0Provider" stepNumber={9}>
    ...
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      En v4, Auth0Provider es opcional. Solo lo necesitas si quieres pasar un usuario inicial durante el renderizado en el servidor para que esté disponible en el hook useUser().
    </Callout>
  </Step>
  ...
  <Step title="Ejecuta tu aplicación" stepNumber={11}>
    ...
    <Info>
      Tu aplicación estará disponible en http://localhost:3000. El SDK de Auth0 v4 monta automáticamente las rutas de autenticación en `/auth/*` (no `/api/auth/*` como en la v3).

      Si el puerto 3000 está ocupado, ejecuta: `npm run dev -- --port 3001` y actualiza las URL de devolución de llamada de tu aplicación de Auth0 a `http://localhost:3001`
    </Info>
  </Step>
</Steps>
...
<Accordion title="Error JWEDecryptionFailed">
  Si ves un error `JWEDecryptionFailed: decryption operation failed`, se debe a un `AUTH0_SECRET` no válido o a una cookie de sesión antigua cifrada con un secreto diferente.

  **Solución:**

  1. Genera un nuevo secreto con:
  ...
  El secreto debe tener exactamente 32 bytes (64 caracteres hexadecimales). El error se produce cuando la aplicación intenta descifrar una cookie de sesión existente que fue cifrada con un secreto diferente.
</Accordion>
...
<Accordion title="Error 404 en /auth/login">
  ...
  4. **Revisa las importaciones:** Asegúrate de que la ruta `import { auth0 } from "./lib/auth0"` sea correcta
</Accordion>

<Accordion title="Errores de módulo no encontrado">
  Si ves errores como &quot;Cannot find module &#39;@/components/LoginButton&#39;&quot; o similares:
  ...
  4. **Verifica las importaciones:** Asegúrate de usar `@/components/*` (no `~/components/*`)
</Accordion>
...
<Accordion title="Cambios importantes de v4">
  Este guía de inicio rápido usa **Auth0 Next.js SDK v4**, que incorpora cambios importantes con respecto a v3:

  * **No se necesitan manejadores de rutas dinámicas** - Las rutas de autenticación se montan automáticamente mediante el proxy
  ...
  * **Usa etiquetas `<a>`** - La navegación debe usar `<a href="/auth/login">` en lugar de botones con onClick
  ...
  El SDK monta automáticamente estas rutas mediante el proxy:
...
<Accordion title="Autenticación del lado del servidor">
  Auth0 Next.js SDK v4 admite los patrones App Router y Pages Router. Aquí tienes algunos patrones habituales del lado del servidor:
...
<Accordion title="Uso de tokens de Auth0 con backends de terceros">
  Si usas un servicio de backend de terceros (como Convex, Supabase o Firebase) que requiere tokens de autenticación de Auth0, tendrás que pasar el token de acceso de tu app de Next.js al cliente del backend.
  ...
  **En el cliente:**
  ...
  La mayoría de los servicios de terceros necesitan tu dominio de Auth0 y el audience para verificar tokens. En la configuración de tu backend:
  ...
  1. **Verifica que se esté pasando el token:** Revisa la pestaña Network de las herramientas para desarrolladores del navegador para confirmar que el token esté incluido en las solicitudes
  ...
  4. **Verifica el audience:** Si usas una API de Auth0, asegúrate de que `AUTH0_AUDIENCE` esté configurado y coincida con el identificador de tu API
