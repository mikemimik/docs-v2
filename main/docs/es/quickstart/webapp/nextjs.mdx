...
description: Esta guía muestra cómo integrar Auth0 en cualquier aplicación nueva o existente de Next.js con el SDK de Auth0 para Next.js v4.
mode: wide
...
<Accordion title="Usa IA para integrar Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **Después, pídele a tu asistente de IA:**
  ...
  Tu asistente de IA creará automáticamente tu aplicación de Auth0, obtendrá las credenciales, instalará `@auth0/nextjs-auth0`, creará las rutas de API y configurará las variables de entorno. [Documentación completa de Agent Skills →](/docs/es/quickstart/agent-skills)
</Accordion>
...
Esta guía de inicio rápido muestra cómo añadir autenticación de Auth0 a una aplicación de Next.js 16. Crearás una aplicación web full stack con renderización del lado del servidor, funcionalidad de inicio de sesión segura y rutas protegidas con el SDK de Auth0 para Next.js.
...
<Steps>
    ...
    <Tabs>
      ...
      <Tab title="CLI">
        ...
        <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
          ...
          1. Comprobar si ya has iniciado sesión (y mostrar el prompt de inicio de sesión si hace falta)
          2. Crear una Auth0 Regular Web Application configurada para `http://localhost:3000`
          3. Generar un archivo `.env` con `AUTH0_DOMAIN`, `AUTH0_CLIENT_ID`, `AUTH0_CLIENT_SECRET`, `AUTH0_SECRET` y `APP_BASE_URL`
        </Callout>
      </Tab>
      ...
      <Tab title="Dashboard">
        ...
        **Genera un AUTH0&#95;SECRET seguro:**
        ...
        <Warning>
          **Crítico:** Si cambias el `AUTH0_SECRET` después de que la aplicación haya estado ejecutándose, debes borrar las cookies de tu navegador para localhost. Las cookies de sesión antiguas cifradas con el secreto anterior provocarán un error `JWEDecryptionFailed`.
        </Warning>
  ...
  <Step title="Añadir proxy" stepNumber={6}>
    ...
    <Info>
      ...
      * `/auth/profile` - Ruta de perfil de usuario
      * `/auth/access-token` - Ruta de token de acceso
      * `/auth/backchannel-logout` - Ruta de cierre de sesión por canal secundario
    </Info>
  </Step>

  <Step title="Crear componentes para inicio de sesión, cierre de sesión y perfil" stepNumber={7}>
    Añade el código de los componentes a los archivos creados en el Paso 3:
  ...
  <Step title="Actualizar el layout con Auth0Provider" stepNumber={9}>
    Actualiza `src/app/layout.tsx` para cargar la fuente Inter y envolver tu aplicación con `Auth0Provider`:
    ...
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      En la versión 4, Auth0Provider es opcional. Solo lo necesitas si quieres pasar un usuario inicial durante la renderización en el servidor para que esté disponible en el hook `useUser()`.
    </Callout>
  </Step>
  ...
  <Step title="Ejecuta tu app" stepNumber={11}>
    ...
    <Info>
      Tu aplicación estará disponible en http://localhost:3000. El SDK de Auth0 v4 monta automáticamente las rutas de autenticación en `/auth/*` (no en `/api/auth/*`, como en v3).
...
<Check>
  **Checkpoint**
  ...
  Ahora deberías tener una página de inicio de sesión de Auth0 completamente funcional ejecutándose en [localhost](http://localhost:3000/)
</Check>
...
<Accordion title="Error JWEDecryptionFailed">
  Si ves el error `JWEDecryptionFailed: decryption operation failed`, suele deberse a que `AUTH0_SECRET` no es válido o a una cookie de sesión antigua cifrada con otro secreto.
...
<Accordion title="Error 404 en /auth/login">
  ...
  4. **Comprueba las importaciones:** Asegúrate de que la ruta de `import { auth0 } from "./lib/auth0"` sea correcta
</Accordion>

<Accordion title="Errores de módulo no encontrado">
  Si ves errores como &quot;Cannot find module &#39;@/components/LoginButton&#39;&quot; u otros similares:
...
<Accordion title="Cambios importantes en v4">
  Esta guía de inicio rápido usa **SDK de Auth0 para Next.js v4**, que incorpora cambios importantes con respecto a la v3:

  * **No se necesitan controladores de ruta dinámicos** - Las rutas de autenticación se montan automáticamente mediante el proxy
  * **Configuración simplificada del cliente** - `new Auth0Client()` lee las variables de entorno automáticamente
  ...
  * **Usa etiquetas `<a>`** - La navegación debe hacerse con `<a href="/auth/login">` en lugar de botones con `onClick`
  ...
  | `/auth/logout`             | Cierre de sesión del usuario                       |
  ...
  | `/auth/backchannel-logout` | Gestionar el cierre de sesión por canal de retorno |

  <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
    Si tienes errores 404 en estas rutas, asegúrate de que:
...
<Accordion title="Autenticación del lado del servidor">
  El SDK de Auth0 para Next.js v4 es compatible con los patrones App Router y Pages Router. Estos son algunos patrones comunes del lado del servidor:
...
<Accordion title="Uso de tokens de Auth0 con backends de terceros">
  ...
  **Del lado del cliente:**
  ...
  export function ConvexClientProvider({ children }: { children: React.ReactNode }) {
    return (
      <ConvexProviderWithAuth0
        client={convex}
        useAuth0={() => ({
          // Obtén el token de acceso desde tu ruta de API de Next.js
  ...
  ### Solución de problemas con los tokens

  Si `ctx.auth.getUserIdentity()` devuelve `null` en tu backend:

  1. **Verifica que se esté pasando el token:** Revisa la pestaña Network de las DevTools del navegador para confirmar que el token se incluya en las requests
  2. **Verifica el formato del token:** Asegúrate de estar pasando el `accessToken`, no el `idToken`
  3. **Verifica la configuración del backend:** Confirma que tu backend tenga el dominio de Auth0 y el ID de cliente correctos
  4. **Revisa el audience:** Si usas una Auth0 API, asegúrate de que `AUTH0_AUDIENCE` esté configurado y coincida con tu identificador de API
