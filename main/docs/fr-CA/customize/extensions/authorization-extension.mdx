...
2. [Configurer](/docs/fr-CA/customize/extensions/authorization-extension/configure-authorization-extension) le comportement de l’extension pendant la transaction de connexion.
...
Vous pouvez facilement importer ou exporter des données de l’extension à l’aide d’un fichier JSON. Pour en savoir plus, consultez [Importation et exportation des données d’Authorization Extension](/docs/fr-CA/customize/extensions/authorization-extension/import-and-export-authorization-extension-data).

<div id="add-functionality">
  ## Ajouter des fonctionnalités
</div>

Une fois l’extension installée et en cours d’exécution, vous pouvez lui ajouter des fonctionnalités supplémentaires.
...
* [Explorer les endpoints de l’API d’Authorization Extension](https://auth0.com/docs/api/authorization-extension): Découvrez les endpoints de l’API d’Authorization Extension et comment les utiliser.
...
<div id="rotate-the-extensions-api-key">
  #### Remplacer la clé API de l&#39;extension
</div>
...
3. Repérez la règle `auth0-authorization-extension` et faites-la glisser à la position située sous la règle `auth0-authz`.
4. Vérifiez que la règle `auth0-authz` :

   * A été créée par l’Authorization Extension et n’a pas été modifiée manuellement
   * Ne modifiera pas le processus d’autorisation de façon à accorder l’accès ou des privilèges à des utilisateurs non désirés si elle est supprimée
