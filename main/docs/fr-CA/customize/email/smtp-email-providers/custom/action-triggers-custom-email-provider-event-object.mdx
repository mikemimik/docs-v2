...
#
description: Découvrez l'objet event du déclencheur Action custom-email-provider, qui fournit des informations contextuelles sur l'exécution du déclencheur.
title: 'Déclencheurs Actions : custom-email-provider - Objet Event'
validatedOn: 2026-04-28
---

L&#39;objet `event` du déclencheur Action custom-email-provider fournit des informations contextuelles sur l&#39;exécution du déclencheur.
...
<ResponseField name="event.client" type="dictionary">
  Informations sur le client avec lequel cette transaction de connexion a commencé.
...
<ResponseField name="event.connection" type="dictionary" post={["optional"]}>
  Détails sur la connexion ayant servi à authentifier l’utilisateur.
...
<ResponseField name="event.notification" type="object">
  <Expandable title="propriétés de la notification" defaultOpen>
    ...
    <ResponseField name="locale" type="string">
      La locale dans laquelle le message a été généré, par exemple `en_US`, telle que définie dans la spécification BCP-47.
    </ResponseField>
...
<ResponseField name="event.organization" type="dictionary" post={["optional"]}>
  Renseignements sur l’Organisation associée à la transaction en cours.
...
<ResponseField name="event.transaction" type="object" post={["optional"]}>
  Détails sur la transaction en cours aux fins de traçage.
...
<ResponseField name="event.user" type="dictionary">
  ...
  <Expandable title="propriétés de l’utilisateur" defaultOpen>
    ...
    <ResponseField name="user_metadata" type="dictionary">
      Champs personnalisés qui stockent des renseignements sur un utilisateur et qui n’ont aucune incidence sur ce à quoi il peut accéder, comme son adresse professionnelle, son adresse résidentielle ou ses préférences.
    </ResponseField>
