---
description: Découvrez comment utiliser des composants personnalisés pour créer des champs qui nécessitent une logique précise ou une interface utilisateur particulière à l’aide de JavaScript, HTML et CSS.
...
Grâce au champ personnalisé, vous pouvez enrichir l’apparence de vos formulaires et y ajouter une logique puissante avec JavaScript, HTML et CSS.

Le champ personnalisé comprend des méthodes internes qui facilitent la transmission de données au formulaire, l’ajout de validations côté client et côté serveur, ainsi que la gestion d’événements courants comme focus ou blur.

Vous pouvez utiliser le champ personnalisé pour créer :

* Des champs avec une structure de données personnalisée.

  * **Exemple** : objets, tableaux de chaînes de caractères
* Des champs qui utilisent des widgets tiers.

  * **Exemple** : autocomplete d’adresse Google
...
Ajoutez des paires clé-valeur à utiliser dans le code source du champ personnalisé. Les paires clé-valeur peuvent inclure des [variables](/docs/fr-CA/customize/forms/variables) de champ de formulaire.
...
function CustomComponent(context) {
  ...
  function mountComponent() {
    /** La méthode getParams() retourne les params que vous avez configurés dans votre entrée */
...
Par défaut, le champ personnalisé accepte les valeurs dans n’importe quel format. Cependant, vous pouvez utiliser [schéma JSON](https://json-schema.org/) pour valider les valeurs côté serveur.
...
Appelée une seule fois lorsque le champ est créé; elle reçoit les valeurs de params que vous configurez dans les paramètres Params.

Renvoie un élément HTML, une chaîne de caractères ou aucune valeur.
...
Cette option est utile lorsque vous devez réafficher la logique de l’interface utilisateur ou actualiser les valeurs de `params` qui ont peut-être changé.

<div id="onfocus">
  ### onFocus()
</div>

Appelée lorsque le focus est placé sur l’élément HTML du champ personnalisé.

<div id="onblur">
  ### onBlur()
</div>

Est appelée lorsque le focus quitte l’élément HTML du champ personnalisé.

<div id="getvalue">
  ### getValue()
</div>

Invoquée lorsque le formulaire doit récupérer la valeur du champ personnalisé une ou plusieurs fois. Cela se produit généralement lorsque l’utilisateur soumet l’étape du formulaire. Si vous devez effectuer des validations côté client, vous pouvez générer une erreur pour afficher un message d’erreur personnalisé à l’utilisateur.
...
Appelée lorsque le champ personnalisé doit être débloqué. Elle est généralement exécutée après que l’utilisateur a soumis l’étape du formulaire ou que le traitement des données par notre serveur principal a cessé en raison d’une erreur de validation.

<div id="getscripts">
  ### getScripts()
</div>

Retourne une liste d’URL que le formulaire garantit d’avoir entièrement chargées avant l’appel de la méthode `init()`.
...
Renvoie une valeur booléenne indiquant si le formulaire satisfait à toutes les validations côté client.

<div id="contextformvalidate">
  #### context.form.validate()
</div>

Vérifie les valeurs de champ existantes au moyen de la validation côté client avant de poursuivre. Si un champ échoue à la validation, un message d’erreur s’affiche.

<div id="contextformgetallhiddenfields">
  #### context.form.getAllHiddenFields()
</div>

Retourne un objet contenant toutes les valeurs des champs masqués.
...
Renvoie un objet contenant toutes les valeurs des champs et des champs masqués.
...
* `setRequired(boolean)` Définit ou retire le caractère obligatoire du champ.
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  `setRequired()` définit ou retire uniquement le caractère obligatoire du champ côté client. Par exemple, si vous retirez le caractère obligatoire d’un champ, mais qu’il est marqué comme obligatoire dans les paramètres du champ, le formulaire renverra une erreur si le champ n’a aucune valeur.
</Callout>
...
<div id="autocomplete-input-custom-field-using-values-from-api">
  ### Champ personnalisé de saisie avec autocomplétion à partir de valeurs provenant d’une API
</div>

Un champ personnalisé qui renvoie une valeur d’autocomplétion au moyen d’une API tierce.
...
Un champ personnalisé qui renvoie une valeur d’une liste déroulante dynamique au moyen d’une API tierce.
