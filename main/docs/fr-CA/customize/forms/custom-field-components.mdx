---
description: Découvrez comment utiliser des composants personnalisés pour créer des champs qui nécessitent une logique ou une UI particulières à l’aide de JavaScript, HTML et CSS.
...
Le champ personnalisé comprend des méthodes internes pour faciliter la transmission de données au formulaire, l’ajout de validations côté client et côté serveur, ainsi que la gestion d’événements courants comme le focus ou le blur.
...
<Warning>
  Pour utiliser des champs personnalisés, vous devez activer les [Domaines personnalisés](/docs/fr-CA/customize/custom-domains). Le rendu d’un formulaire contenant un champ personnalisé à l’extérieur d’un domaine personnalisé affichera une erreur.
</Warning>
...
Ajoutez des paires clé-valeur que vous pourrez référencer dans le code source du champ personnalisé. Ces paires clé-valeur peuvent inclure des [variables](/docs/fr-CA/customize/forms/variables) de champ de formulaire.

<Frame>![](/docs/images/cdy7uua7fh8z/6VEtliKd20ZF4MJKhvB5Z0/e1b3b5df4860d013d1e3eb79d1174b35/params.png)</Frame>

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Les valeurs des paramètres sont disponibles uniquement après l’appel de la méthode `init()` par le formulaire.
</Callout>

**Exemple** :

Dans l’exemple ci-dessous, les paramètres du champ personnalisé sont renseignés avec les paires clé-valeur `symbol={{fields.symbol}}` et `separator=,`
...
Par défaut, le champ personnalisé accepte des valeurs de tout format. Toutefois, vous pouvez utiliser [JSON Schema](https://json-schema.org/) pour valider les valeurs côté serveur.
...
Pour prendre en charge des exigences de validation complexes, vous pouvez utiliser un [flow](/docs/fr-CA/customize/forms/flows).

<div id="css">
  ### CSS
</div>

Ajoutez vos styles CSS à votre champ personnalisé.
...
Vous pouvez utiliser ces gestionnaires pour ajouter un comportement personnalisé à ce champ :

<div id="init">
  ### init()
</div>

Est appelée une seule fois lors de la création du champ et reçoit les valeurs `params` que vous configurez dans les paramètres Params.
...
Cette option est utile lorsque vous devez réafficher la logique de l’interface utilisateur ou actualiser les valeurs de `params` qui ont pu changer.

<div id="onfocus">
  ### onFocus()
</div>

Invoquée lorsque le focus passe dans l’élément HTML du champ personnalisé.
...
Appelé lorsque le formulaire doit récupérer la valeur du champ personnalisé, une ou plusieurs fois. Cette fonction est généralement exécutée lorsque l’utilisateur soumet l’étape du formulaire. Si vous devez effectuer des validations côté client, vous pouvez générer une erreur pour afficher un message d’erreur personnalisé à l’utilisateur.
...
Appelée lorsque le champ personnalisé doit être bloqué. Cette fonction est généralement exécutée lorsque l’utilisateur soumet l’étape du formulaire et que les données sont en cours de traitement par notre backend.

<div id="unblock">
  ### unblock()
</div>

Appelée lorsque le champ personnalisé doit être débloqué. Cette opération s’exécute généralement après que l’utilisateur a soumis l’étape du formulaire, ou lorsque les données ont cessé d’être traitées par notre back-end en raison d’une erreur de validation.

<div id="getscripts">
  ### getScripts()
</div>

Retourne une liste d’URL que le formulaire garantit avoir fini de charger avant l’appel de la méthode `init()`.
...
Lorsque vous passez un objet de contexte, vous pouvez utiliser ces méthodes pour gérer la logique dans votre formulaire et vos composants.
...
Retourne les paramètres des réglages du champ personnalisé actuel.

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Les valeurs des paramètres sont accessibles seulement après que le formulaire a appelé la méthode `init()`.
</Callout>
...
Lorsque vous devez interagir avec le formulaire pour récupérer les valeurs d’autres champs ou passer à d’autres étapes du formulaire, vous pouvez utiliser les méthodes suivantes :
...
Retourne un booléen indiquant si le formulaire satisfait à toutes les validations côté client.

<div id="contextformvalidate">
  #### context.form.validate()
</div>

Vérifie les valeurs existantes des champs au moyen d’une validation côté client avant de poursuivre. Si un champ ne passe pas la validation, un message d’erreur s’affiche.
...
Renvoie un objet contenant les valeurs de tous les champs, y compris les champs masqués.
...
console.log(fullNameValue); // Jean Tremblay
```

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  `setRequired()` ne fait que définir ou retirer l’état obligatoire du champ côté client. Par exemple, si vous retirez l’état obligatoire d’un champ, mais qu’il est marqué comme obligatoire dans les paramètres du champ, le formulaire renverra une erreur si le champ n’a aucune valeur.
</Callout>
...
<div id="range-input-custom-field">
  ### Champ personnalisé de plage de valeurs
</div>

Un champ personnalisé qui renvoie une valeur dans une plage prédéterminée.
...
<div id="color-input-custom-field">
  ### Champ personnalisé de sélection de couleur
</div>

Un champ personnalisé qui renvoie une valeur de couleur hexadécimale.
...
<div id="autocomplete-input-custom-field-using-values-from-api">
  ### Champ personnalisé de saisie semi-automatique utilisant des valeurs provenant d’une API
</div>
...
<div id="dynamic-dropdown-custom-field-using-values-from-api">
  ### Champ personnalisé de liste déroulante dynamique utilisant des valeurs d’API
</div>

Un champ personnalisé qui renvoie une valeur provenant d’une liste déroulante dynamique alimentée par une API tierce.
