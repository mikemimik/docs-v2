...
Lorsqu’une erreur d’autorisation survient et que votre URL de rappel est valide, le <Tooltip tip="Serveur d’autorisation : serveur centralisé qui contribue à définir les limites de l’accès d’un utilisateur. Par exemple, votre serveur d’autorisation peut contrôler les données, les tâches et les fonctionnalités accessibles à un utilisateur." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Authorization+Server">serveur d’autorisation</Tooltip> renvoie à votre URL de rappel les paramètres d’erreur et d’état appropriés. Si votre URL de rappel n’est pas valide, votre application affichera la [page d’erreur Auth0 par défaut](/docs/fr-CA/authenticate/login/auth0-universal-login/error-pages).

Votre application peut également afficher la page d’erreur Auth0 par défaut pour d’autres raisons qu’une URL de rappel invalide, par exemple :
...
* L’utilisateur ouvre un lien expiré de réinitialisation du mot de passe (lorsqu’il utilise l’expérience Classic Login).
...
Les paramètres fournis varient selon le type d’erreur et sont propres à la requête. Par exemple, si la requête à l’origine de l’erreur ne contenait pas de `client_id`, le serveur d’autorisation ne renverra pas le paramètre `client_id`.
...
2. Configurer Auth0 pour qu’il affiche une page d’erreur personnalisée en votre nom au moyen de la Management API.
...
4. Saisissez l’URL de la page d’erreur que vous souhaitez afficher à vos utilisateurs, puis sélectionnez **Save**.
...
Utilisez le point de terminaison <Tooltip tip="Management API : un produit permettant aux clients d’effectuer des tâches administratives." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Management+API">Management API</Tooltip> [Update Tenant Settings](https://auth0.com/docs/api/management/v2/#!/Tenants/patch_settings). Remplacez la valeur d’espace réservé `{mgmtApiAccessToken}` par votre jeton d’accès à la Management API <Tooltip tip="Access Token : justificatif d’autorisation, sous la forme d’une chaîne opaque ou d’un JWT, utilisé pour accéder à une API." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Access+Token">Access Token</Tooltip>, puis mettez à jour la valeur du champ `url` dans le corps de la requête JSON pour qu’elle pointe vers l’emplacement de la page d’erreur.
...
| `show_log_link`         | Indique si un lien vers l’erreur doit être affiché dans les journaux de votre tenant. Les valeurs valides sont `true` et `false`. |
| `url`                   | Adresse de la page d’erreur personnalisée vers laquelle vous souhaitez rediriger l’utilisateur.                                   |
...
Utilisez le point de terminaison [Update Tenant Settings](https://auth0.com/docs/api/management/v2/#!/Tenants/patch_settings) de la Management API. Remplacez la valeur de l’espace réservé `{mgmtApiAccessToken}` par votre Management API jeton d’accès, puis mettez à jour la valeur du champ `html` dans le corps JSON avec une chaîne contenant le code HTML de votre page.
...
| `html`                  | Le HTML de la page d’erreur personnalisée que vous souhaitez afficher.                                                           |

Pour éviter les vulnérabilités XSS, nettoyez votre gabarit personnalisé à l’aide des filtres Liquid [escape](https://shopify.github.io/liquid/filters/escape/) et [strip&#95;html](https://shopify.github.io/liquid/filters/strip_html/).
