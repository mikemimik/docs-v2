...
Chaque écran Universal Login a sa propre configuration de rendu, un objet JSON qui contrôle la façon dont Auth0 affiche cet écran. La configuration de rendu définit le mode de rendu, les assets JavaScript et CSS qui composent votre interface utilisateur personnalisée, ainsi que les données de contexte accessibles à votre application à l’exécution.

Vous pouvez gérer la configuration de rendu à partir de l’Auth0 Dashboard, d’outils d’infrastructure-as-code ou de l’Auth0 Management API. Pour savoir comment faire, consultez [Configurer ACUL](/docs/fr-CA/customize/login-pages/advanced-customizations/configure/overview). Pour obtenir le schéma complet de l’API, consultez le point de terminaison [`PATCH /v2/prompts/{prompt}/screen/{screen}/rendering`](/docs/fr-CA/api/management/v2/prompts/patch-rendering).

La configuration de rendu comprend les propriétés suivantes :

| **Propriété**                | **Type** | **Description**                                                                                                                                                             |
| ---------------------------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `rendering_mode`             | string   | Contrôle le mode de rendu de l’écran. Accepte `standard` ou `advanced`.                                                                                                     |
...
| `default_head_tags_disabled` | boolean  | Lorsque `true`, supprime les balises `<head>` par défaut d’Universal Login (comme le favicon). La valeur par défaut est `false`.                                            |
...
| `use_page_template`          | boolean  | Lorsque `true`, affiche l’écran ACUL dans le modèle de page personnalisé du tenant, s’il est défini. La valeur par défaut est `false`.                                      |
| `filters`                    | object   | Restreint le rendu ACUL à des applications, des organisation ou des domaine personnalisé précis. Lorsqu’elle est omise, la configuration ACUL s’applique à l’ensemble du tenant. |

<div id="rendering-modes">
  ## Modes de rendu
</div>

La propriété `rendering_mode` détermine si un écran utilise l’interface utilisateur par défaut de Universal Login ou votre implémentation ACUL personnalisée.

* **Standard** : L’écran est rendu à l’aide de l’interface [Universal Login](/docs/fr-CA/customize/login-pages/universal-login) par défaut. Vous pouvez utiliser des [partials](/docs/fr-CA/customize/login-pages/universal-login/customize-signup-and-login-prompts) et des [template variables](/docs/fr-CA/customize/login-pages/universal-login/customize-templates#page-template-variables) pour personnaliser l’écran.
* **Advanced** : L’écran est rendu à l’aide de vos ressources JavaScript et CSS personnalisées chargées via `head_tags`. Les partials ne s’appliquent pas. Par défaut, le mode avancé s’applique à toutes les applications et organisations du tenant.
* **Advanced (filtered)** : L’écran est rendu avec ACUL pour les requêtes qui correspondent aux critères définis dans `filters` (applications, organisations ou domaines personnalisés précis). Les requêtes qui ne correspondent pas aux filtres retombent sur le rendu standard, où les partials s’appliquent.

Pour utiliser le mode filtré, définissez `rendering_mode` sur `advanced` et fournissez un objet `filters` avec un `match_type` (`includes_any` ou `excludes_any`) et au moins un tableau d’entités (`clients`, `organizations` ou `domains`). Chaque entité peut être identifiée par `id` ou par une paire clé-valeur `metadata`.

<div id="head-tags">
  ## Balises head
</div>

Les balises head sont le principal mécanisme permettant de charger votre UI personnalisée dans la page hôte ACUL. Quand Auth0 affiche un écran en mode avancé, il sert une page HTML minimale et injecte dans le `<head>` de la page les éléments définis dans `head_tags`. Ces balises chargent les lots de ressources JavaScript et CSS qui composent votre écran d’authentification personnalisé.
...
L’attribut `integrity` est obligatoire pour les balises `script` et les feuilles de style. Auth0 utilise [Subresource Integrity (SRI)](https://developer.mozilla.org/en-US/docs/Web/Security/Subresource_Integrity) pour vérifier qu’aucune modification n’a été apportée aux ressources.

<div id="dynamic-url-segments">
  ### Segments d’URL dynamiques
</div>

Les URL des balises Head prennent en charge des segments dynamiques à l’aide de la syntaxe de modèle Liquid, ce qui vous permet de servir différents lots de ressources selon le contexte d’authentication actuel. Par exemple, vous pouvez charger un lot Javascript différent pour chaque client ou chaque organisation afin de prendre en charge le multi-branding.
...
Lorsque vous utilisez des segments d’URL dynamiques, fournissez un hachage SRI pour chaque variante possible de ressource. L’attribut `integrity` accepte plusieurs hachages séparés par des espaces; le navigateur charge la ressource si l’un des hachages correspond.
...
Vous pouvez également utiliser des balises head pour charger des scripts d’analyse, des polices personnalisées ou des balises meta.

Pour remplacer les balises head par défaut de Universal Login (comme le favicon) par vos propres balises, définissez `default_head_tags_disabled` sur `true`.
...
Chaque valeur que vous souhaitez rendre disponible dans votre écran doit être explicitement indiquée dans le tableau `context_configuration`. Auth0 supprime du payload les clés dont la valeur est vide ou nulle.
...
| `tenant.enabled_locales`                          | Langues régionales activées sur le tenant                  |
...
| `untrusted_data.authorization_params.ui_locales`  | Langues de l’interface demandées                           |
