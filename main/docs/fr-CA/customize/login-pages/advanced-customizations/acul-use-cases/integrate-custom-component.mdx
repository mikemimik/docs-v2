---
description: Découvrez comment intégrer une bibliothèque de composants personnalisés avec ACUL
...
2. Démarrez le serveur de développement local ACUL pour modifier et prévisualiser les mises à jour de votre écran.
...
5. Ajoutez les fichiers de composant à `src/components/ui/input-otp.tsx` :
...
6. Intégrez le composant :
   ...
   b. Importez les composants InputOTP et remplacez le champ de saisie actuel. Vous devrez aussi gérer l’état du code OTP et utiliser le hook approprié du SDK.
...
7. Lancez l’écran en local avec l’ACUL Context Inspector pour voir votre nouveau composant :

```bash
auth0 acul dev -s  login-passwordless-email-code
```

8. Connectez votre environnement de développement local à votre tenant de test pour essayer le nouvel écran dans un flux d’authentification en conditions réelles :
