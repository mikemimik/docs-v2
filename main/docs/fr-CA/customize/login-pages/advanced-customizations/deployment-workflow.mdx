...
* Créer les lots de ressources de vos écrans
  ...
  * Configurer votre tenant Auth0
    ...
    * Il met à jour les paramètres de personnalisation des écrans Auth0 afin qu’ils pointent vers vos ressources sur le CDN CloudFront.
...
Créez une application [Machine-to-Machine](/docs/fr-CA/get-started/auth0-overview/create-applications/machine-to-machine-apps) pour accorder au flux de travail GitHub les autorisations nécessaires à la mise à jour des paramètres de votre tenant.
...
4. Sous **Name**, saisissez un nom descriptif, par exemple : `GitHub ACUL Deployment`.
...
Pour téléverser et diffuser vos ressources, vous avez besoin d’un bucket AWS S3 et de configurer CloudFront comme CDN.

* **Créez un bucket AWS S3**. Bloquez tout accès public afin que ce bucket demeure privé. Ce bucket privé stocke vos ressources.
* **Configurez une distribution CloudFront**. Ce CDN diffuse vos ressources de façon sécurisée. Il doit être configuré pour utiliser votre bucket S3 comme `Origin` au moyen d’Origin Access Control (OAC).
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  ...
  * Le rôle nécessite une politique accordant les permissions `s3:PutObject`, `s3:DeleteObject` et `s3:ListBucket` sur votre bucket S3.
</Callout>
...
* [`config/screen-to-prompt-mapping.js`](https://github.com/auth0-samples/auth0-acul-samples/blob/6cb4dcc43defd1a9cf586da9307d7d2348e696ae/react/.github/config/screen-to-prompt-mapping.js) : Fait correspondre les noms de répertoire internes de vos écrans. Exemple : `mfa-sms-challenge` avec le nom de prompt Auth0 officiel `mfa-sms`.
