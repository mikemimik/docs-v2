---
description: Découvrez le flux de travail de déploiement ACUL
...
Créez une application [Machine-to-Machine](/docs/fr-CA/get-started/auth0-overview/create-applications/machine-to-machine-apps) pour accorder au flux de travail GitHub les permissions nécessaires à la mise à jour des paramètres de votre tenant.
...
4. Sous **Name**, entrez un nom descriptif, par exemple : `GitHub ACUL Deployment`.
...
Pour téléverser et diffuser vos ressources, vous avez besoin d’un bucket S3 AWS et de configurer CloudFront comme CDN.

* **Créez un bucket S3 AWS**. Désactivez tout accès public afin que ce bucket demeure privé. Ce bucket privé stocke vos ressources.
* **Configurez une distribution CloudFront**. Ce CDN diffuse vos ressources de façon sécuritaire. Elle doit être configurée pour utiliser votre bucket S3 comme `Origin` au moyen d’Origin Access Control (OAC).

<div id="3-create-an-aws-iam-role-for-github-action">
  ### 3. Créez un rôle AWS IAM pour GitHub Action
</div>

Créez un **rôle IAM** dans AWS pour permettre à GitHub Actions de téléverser des fichiers vers AWS S3.

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  * Ce rôle IAM doit utiliser l’identité Web (OIDC) et faire confiance à `token.actions.githubusercontent.com`.
  * Le rôle doit être associé à une politique accordant les autorisations `s3:PutObject`, `s3:DeleteObject` et `s3:ListBucket` sur votre bucket S3.
</Callout>
...
Vous pouvez déterminer quels écrans seront déployés en modifiant les fichiers de configuration situés dans le répertoire `.github/config/` :

* [`config/deploy_config.yml`](https://github.com/auth0-samples/auth0-acul-samples/blob/6cb4dcc43defd1a9cf586da9307d7d2348e696ae/react/.github/config/deploy_config.yml) : Votre panneau de contrôle principal. Attribuez la valeur `true` à un écran pour le déployer, ou `false` pour l’ignorer.
* [`config/screen-to-prompt-mapping.js`](https://github.com/auth0-samples/auth0-acul-samples/blob/6cb4dcc43defd1a9cf586da9307d7d2348e696ae/react/.github/config/screen-to-prompt-mapping.js) : Fait correspondre les noms de répertoire internes de vos écrans. Exemple : `mfa-sms-challenge` au nom officiel du prompt Auth0 `mfa-sms`.
