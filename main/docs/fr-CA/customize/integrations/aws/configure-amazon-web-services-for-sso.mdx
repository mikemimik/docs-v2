...
5. Mappez le rôle AWS à un utilisateur.
...
2. À la page suivante, vous devrez sélectionner le type d’entité approuvée. Sélectionnez **SAML 2.0 Federation**.
3. Lorsque demandé, définissez le fournisseur créé ci-dessus comme **SAML provider**. Sélectionnez **Allow programmatic and AWS Management Console access**. Cliquez sur **Next** pour continuer.
4. À la page **Attach Permission Policies**, sélectionnez les stratégies appropriées à associer au rôle. Elles définissent les permissions que les utilisateurs auxquels ce rôle est attribué auront dans AWS. Par exemple, pour accorder à vos utilisateurs un accès en lecture seule à IAM, filtrez et sélectionnez la stratégie `IAMReadOnlyAccess`. Une fois terminé, cliquez sur **Next Step**.
...
6. À la page **Review**, définissez le **Role Name** et passez vos paramètres en revue. Fournissez des valeurs pour les paramètres suivants :

   | Champ                   | Description                             |
   | ----------------------- | --------------------------------------- |
   | **Nom du rôle**         | Nom descriptif de votre rôle.           |
   | **Description du rôle** | Description de l’utilité de votre rôle. |
7. Passez en revue les renseignements sur les **Trusted entities** et les **stratégies**, puis cliquez sur **Create Role**.
...
Les **rôles AWS** indiqués seront associés à une **stratégie IAM** qui définit le type d’accès autorisé à une ressource, y compris aux consoles AWS. Pour en savoir plus sur les rôles et les stratégies, consultez [Creating IAM Roles](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_create.html).
...
Pour prolonger le délai autorisé avant l’expiration de la session AWS (3 600 secondes), utilisez une [règle personnalisée](/docs/fr-CA/customize/rules). Cette règle définit l’[attribut **SessionDuration**](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_providers_create_saml_assertions.html), ce qui modifie la durée de la session.
...
export const codeExample = `function (user, context, callback) {
  ...
  user.time = 1000; // temps restant avant l’expiration, en secondes
...
3. Dans l’onglet **Usage**, repérez l’**Identity Provider Login URL**. Vous devriez être redirigé vers la page de connexion de votre compte Auth0. Si vous réussissez à vous connecter, vous serez redirigé vers AWS.
