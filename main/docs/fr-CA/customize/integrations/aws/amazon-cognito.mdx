---
description: Comment intégrer Auth0 à Amazon Cognito au moyen d’un fournisseur OpenID Connect (OIDC).
title: Intégrer Amazon Cognito
---

Amazon Cognito est un service backend qui vous permet de vous concentrer sur le développement de l’expérience utilisateur de votre application. Pour en savoir plus et vous inscrire à ces services, consultez [Amazon Cognito](http://aws.amazon.com/cognito/) ainsi que la marche à suivre pour créer un [fournisseur OpenID Connect (OIDC) au moyen d’AWS CLI](https://docs.aws.amazon.com/cli/latest/reference/iam/create-open-id-connect-provider.html).
...
D’abord, vous devrez obtenir certaines informations au sujet de votre application dans Auth0 :

1. Accédez à [Auth0 Dashboard &gt; Applications &gt; Applications](https://manage.auth0.com/#/applications).
2. Sélectionnez votre application, puis passez à la section **Settings**.
...
1. Ouvrez la [console IAM](https://console.aws.amazon.com), sélectionnez **Identity Providers** dans la barre latérale de gauche, puis **Add Provider**.
...
Créez un groupe d’identités dans AWS pour permettre à Cognito d’utiliser le fournisseur d’identité OIDC Auth0 pour l’authentification :
...
4. Sous **Authentication Providers**, sélectionnez l’onglet **OpenID**, puis le nom du fournisseur que vous avez créé précédemment.
...
Une fois que l’utilisateur s’est connecté avec Auth0, l’étape suivante consiste à transmettre ses identifiants à Cognito. Pour en savoir plus, consultez [les fournisseurs OpenID Connect (groupes d’identités) dans la documentation AWS](http://docs.aws.amazon.com/cognito/latest/developerguide/open-id.html).

Cognito utilise le <Tooltip tip="ID Token : information d’authentification destinée au client lui-même plutôt qu’à l’accès à une ressource." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=ID+token">ID token</Tooltip> qu’un utilisateur reçoit d’Auth0 pour générer des IDs Cognito uniques. Lorsque l’utilisateur se connecte à Cognito par l’intermédiaire d’Auth0, vous pouvez y stocker des renseignements auxquels lui seul peut accéder.
...
Si le message d’erreur « Jeton de connexion invalide » s’affiche, vérifiez que l’application Auth0 est configurée pour utiliser l’algorithme de signature RS256 :
