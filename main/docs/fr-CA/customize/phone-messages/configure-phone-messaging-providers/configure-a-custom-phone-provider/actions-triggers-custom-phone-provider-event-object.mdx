...
<ResponseField name="event.notification" type="object">
  <Expandable title="propriétés de notification" defaultOpen>
    ...
    <ResponseField name="code" type="string" post={["optional"]}>
      L’OTP que nous avons généré pour ce message pour certains types (par ex. `otp_verify`, `otp_enroll`). S’il est fourni, il est important de le communiquer à l’utilisateur final.
    </ResponseField>

    <ResponseField name="delivery_method" type="string">
      Le mode de livraison du message. Il peut s’agir de `text` ou de `voice`.
...
<ResponseField name="event.transaction" type="object" post={["optional"]}>
  ...
  <Expandable title="propriétés de la transaction" defaultOpen>
    <ResponseField name="correlation_id" type="string" post={["optional"]}>
      Un identifiant unique permettant de corréler cette requête à travers plusieurs services pour le traçage distribué.
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="event.user" type="dictionary">
  Un objet qui décrit l&#39;utilisateur au nom duquel la transaction en cours a été lancée.
  ...
  <Expandable title="propriétés de l'utilisateur" defaultOpen>
    ...
    <ResponseField name="identities" type="array of objects" post={["optional"]}>
      Contient des renseignements récupérés auprès du fournisseur d&#39;identité avec lequel l&#39;utilisateur s&#39;authentifie à l&#39;origine. Les utilisateurs peuvent aussi lier leur profil à plusieurs fournisseurs d&#39;identité; ces identités apparaîtront alors également dans ce tableau. Le contenu de chaque objet fournisseur d&#39;identité varie selon le fournisseur.
