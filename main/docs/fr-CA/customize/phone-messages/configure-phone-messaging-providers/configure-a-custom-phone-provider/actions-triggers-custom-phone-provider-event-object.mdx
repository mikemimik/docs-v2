...
#
description: Découvrez l’objet `event` du déclencheur custom-phone-provider d’Actions, qui fournit des renseignements contextuels sur l’exécution du déclencheur.
title: 'Déclencheurs Actions : custom-phone-provider - objet Event'
validatedOn: 2026-04-28
---

L’objet `event` du déclencheur custom-phone-provider d’Actions fournit des renseignements contextuels sur l’exécution du déclencheur.
...
<ResponseField name="event.notification" type="object">
  <Expandable title="propriétés de la notification" defaultOpen>
    ...
    <ResponseField name="code" type="string" post={["optional"]}>
      L’OTP que nous avons généré pour ce message pour certains types (p. ex. `otp_verify`, `otp_enroll`). S’il est fourni, il est important de le communiquer à l’utilisateur final.
    </ResponseField>
    ...
    <ResponseField name="from" type="string" post={["optional"]}>
      Le numéro de téléphone de l’expéditeur conforme à E.164.
    </ResponseField>

    <ResponseField name="locale" type="string" post={["optional"]}>
      La locale dans laquelle nous avons généré le message, par exemple `en_US`, telle que définie dans la spécification BCP-47.
    </ResponseField>
    ...
    <ResponseField name="recipient" type="string">
      Le numéro de téléphone du destinataire conforme à E.164.
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="event.tenant" type="object">
  <Expandable title="propriétés du tenant" defaultOpen>
    ...
    <ResponseField name="friendly_name" type="string" post={["optional"]}>
      Le nom convivial du tenant, généralement une version plus compréhensible de l&#39;ID.
    </ResponseField>

    <ResponseField name="home_url" type="string" post={["optional"]}>
      L&#39;URL d&#39;accueil du tenant, si elle est définie et telle qu&#39;indiquée dans ses paramètres.
    </ResponseField>

    <ResponseField name="logo_url" type="string" post={["optional"]}>
      L&#39;URL du logo du tenant, si elle est définie et telle qu&#39;indiquée dans ses paramètres.
    </ResponseField>

    <ResponseField name="support_email" type="string" post={["optional"]}>
      L&#39;adresse courriel du service de soutien du tenant, si elle est définie et telle qu&#39;indiquée dans ses paramètres.
    </ResponseField>

    <ResponseField name="support_url" type="string" post={["optional"]}>
      L&#39;URL du service de soutien du tenant, si elle est définie et telle qu&#39;indiquée dans ses paramètres.
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="event.user" type="dictionary">
  ...
  <Expandable title="propriétés de l’utilisateur" defaultOpen>
    ...
    <ResponseField name="last_password_reset" type="string" post={["optional"]}>
      Horodatage indiquant la dernière fois où le mot de passe de l’utilisateur a été réinitialisé ou modifié. Lors de la création de l’utilisateur, ce champ n’existe pas. Cette propriété est disponible uniquement pour les connexions de base de données.
    </ResponseField>
    ...
    <ResponseField name="user_metadata" type="dictionary">
      Champs personnalisés qui stockent des renseignements sur un utilisateur et qui n’ont pas d’incidence sur ce à quoi il a accès, comme l’adresse professionnelle, l’adresse du domicile ou les préférences de l’utilisateur.
    </ResponseField>
    ...
    <ResponseField name="identities" type="array of objects" post={["optional"]}>
      ...
      <Expandable title="propriétés des identités" defaultOpen>
        ...
        <ResponseField name="profileData" type="dictionary" post={["optional"]}>
          Renseignements sur l’utilisateur associés à la connexion. Lorsque des profils sont liés, cette propriété est remplie avec les renseignements associés aux comptes secondaires.
        </ResponseField>
