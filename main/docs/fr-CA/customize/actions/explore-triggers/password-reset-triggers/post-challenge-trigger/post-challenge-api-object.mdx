...
#
description: Découvrez l’objet d’API du déclencheur d’Action password-reset-post-challenge.
...
Marquez la tentative actuelle de réinitialisation du mot de passe comme rejetée. Cela empêchera
l’utilisateur final de terminer le processus de réinitialisation du mot de passe. Cela *N’ANNULERA PAS* les autres effets secondaires liés à l’utilisateur
...
<ResponseField name="reason" type="string">
  Une explication en langage clair du rejet de la réinitialisation du mot de passe. Elle peut être affichée
...
1. Il termine avec succès la vérification pour le facteur par défaut.
2. Il termine avec succès la vérification pour l’un des facteurs facultatifs décrits dans `additionalFactors`.
...
Lorsqu’une vérification multifacteur est demandée, les Actions suivantes ne seront pas exécutées tant que l’utilisateur n’y aura pas répondu. Un utilisateur aura satisfait à la vérification dans l’une ou l’autre des situations suivantes :

1. Il termine avec succès la vérification pour l’un des facteurs.

Si l’un des facteurs demandés a déjà fait l’objet d’une vérification réussie dans la transaction en cours, il sera ignoré.

Si un facteur demandé n’est pas activé sur le tenant, il sera ignoré. Si un facteur demandé n’a pas déjà été configuré par l’utilisateur, il sera ignoré. Si aucun des facteurs demandés n’est activé ou configuré, la transaction d’authentification échouera (c.-à-d. que la connexion ne se terminera pas).

<Note>
  Cette méthode affichera l’écran de sélection des facteurs si l’utilisateur n’a pas déjà satisfait aux exigences de la vérification. S’il existe un facteur de préférence, la méthode `api.authentication.challengeWith()` est à privilégier. L’écran de sélection des facteurs ne sera pas affiché si un seul facteur est transmis ou est valide.
</Note>
...
Crée un jeton de session pouvant servir de cible de redirection sous la forme d’un paramètre de chaîne de requête (via `sendUserTo`) et
...
<ResponseField name="options" type="tokencreationoptions">
  ...
  <Expandable title="propriétés des options" defaultOpen>
    ...
    <ResponseField name="secret" type="string">
      ...
      valeur du secret doit être stockée comme **secret** et récupérée dans
      `event.secrets['<secret_name>']`.
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="options" type="validatesessiontokenoptions">
  ...
  <Expandable title="propriétés des options" defaultOpen>
    ...
    <ResponseField name="tokenParameterName" type="string" post={["optional"]}>
      Le nom du paramètre de requête ou du corps de la requête qui a été envoyé au point de terminaison `/continue`.
    </ResponseField>
  </Expandable>
</ResponseField>
...
maximale indiquée dans [Actions Cache Limits](https://auth0.com/docs/customize/actions/limitations).
...
Définit l’URL vers laquelle l’utilisateur sera redirigé après la réinitialisation du mot de passe.
