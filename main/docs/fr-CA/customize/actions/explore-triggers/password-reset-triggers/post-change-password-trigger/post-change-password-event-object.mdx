...
#
description: Découvrez l’objet `event` du déclencheur Actions post-change-password, qui fournit des informations contextuelles sur l’exécution du déclencheur.
...
<ResponseField name="event.request" type="dictionary">
  Renseignements sur la requête qui a déclenché la transaction.
...
<ResponseField name="event.transaction" type="object" post={["optional"]}>
  ...
  <Expandable title="propriétés de la transaction" defaultOpen>
    <ResponseField name="correlation_id" type="string" post={["optional"]}>
      L’ID de corrélation peut être fourni dans la requête d’authentification initiale lorsque l’application redirige vers Universal Login. Vous pouvez utiliser cette valeur pour établir une corrélation entre les logs et les requêtes de votre code Action et le flux utilisateur.
    </ResponseField>
  </Expandable>
</ResponseField>
