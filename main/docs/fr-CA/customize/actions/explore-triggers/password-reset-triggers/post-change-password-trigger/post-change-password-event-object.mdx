...
#
description: Découvrez l’objet Event du déclencheur Action post-change-password, qui fournit des informations contextuelles sur l’exécution du déclencheur.
title: 'Déclencheurs Actions : post-change-password - objet Event'
validatedOn: 2026-05-25
---

L’objet `event` du déclencheur Actions post-change-password fournit des informations contextuelles sur l’exécution du déclencheur.
...
<ResponseField name="event.authentication" type="dictionary" post={["optional"]}>
  ...
  <Expandable title="propriétés d’authentification" defaultOpen>
    <ResponseField name="riskAssessment" type="dictionary" post={["optional"]}>
      Détails sur les renseignements relatifs aux évaluations du risque pour différents flux.
        ...
        <ResponseField name="supplemental" type="dictionary" post={["optional"]}>
          ...
          <Expandable title="propriétés de supplemental">
            <ResponseField name="akamai" type="dictionary" post={["optional"]}>
              &#91;Accès anticipé&#93; Évaluation supplémentaire du risque. Celle-ci est offerte seulement si Akamai Account Protector est activé et qu’Akamai transmet les en-têtes de la transaction.
...
<ResponseField name="event.transaction" type="object" post={["optional"]}>
  ...
  <Expandable title="propriétés de la transaction" defaultOpen>
    <ResponseField name="correlation_id" type="string" post={["optional"]}>
      Le Correlation ID peut être fourni dans la requête d’authentification initiale lorsque l’application redirige vers Universal Login. Vous pouvez utiliser cette valeur pour corréler les logs et les requêtes de votre code Action au parcours utilisateur.
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="event.user" type="object">
  ...
  <Expandable title="propriétés de l’utilisateur" defaultOpen>
    ...
    <ResponseField name="last_password_reset" type="string" post={["optional"]}>
      Horodatage indiquant la dernière réinitialisation ou modification du mot de passe de l’utilisateur. Lors de la création de l’utilisateur, ce champ n’existe pas. Cette propriété n’est disponible que pour les connexions de base de données.
    </ResponseField>
