...
#
description: Découvrez l’objet d’événement du déclencheur post-user-registration d’Actions, qui fournit des renseignements contextuels sur l’exécution du déclencheur.
title: 'Déclencheurs Actions : post-user-registration - objet d’événement'
...
<ResponseField name="event.authentication" type="dictionary" post={["optional"]}>
  ...
  <Expandable title="propriétés d’authentification" defaultOpen>
    <ResponseField name="riskAssessment" type="dictionary" post={["optional"]}>
      Détails sur les renseignements d’évaluation du risque pour différents flux.

      <Expandable title="propriétés de riskAssessment" defaultOpen>
        <ResponseField name="supplemental" type="dictionary" post={["optional"]}>
          Signaux supplémentaires envoyés par des fournisseurs tiers pour faciliter l’évaluation du risque.

          <Expandable title="propriétés de supplemental">
            <ResponseField name="akamai" type="dictionary" post={["optional"]}>
              &#91;Accès anticipé&#93; Évaluation supplémentaire du risque. Disponible seulement si Akamai Account Protector est activé et qu’Akamai transfère les en-têtes pour la transaction.

              <Expandable title="propriétés d’akamai">
                <ResponseField name="akamaiBot" type="object" post={["optional"]}>
                  Les résultats de détection de bot transmis par Akamai Bot Manager.
...
<ResponseField name="event.transaction" type="dictionary" post={["optional"]}>
  ...
  <Expandable title="propriétés de la transaction" defaultOpen>
    ...
    <ResponseField name="response_type" type="array of strings" post={["optional"]}>
      Indique le type d’information d’identification qu’Auth0 retournera.
    </ResponseField>
...
<ResponseField name="event.user" type="dictionary">
  Un objet décrivant l’utilisateur pour le compte duquel la transaction en cours a été amorcée.
  ...
  <Expandable title="propriétés de l’utilisateur" defaultOpen>
    ...
    <ResponseField name="last_password_reset" type="string" post={["optional"]}>
      Horodatage indiquant la dernière réinitialisation ou modification du mot de passe de l’utilisateur. À la création de l’utilisateur, ce champ n’existe pas. Cette propriété est disponible uniquement pour les Connexions de base de données.
    </ResponseField>
    ...
    <ResponseField name="updated_at" type="string">
      Horodatage indiquant la dernière mise à jour du profil de l’utilisateur.
    </ResponseField>
