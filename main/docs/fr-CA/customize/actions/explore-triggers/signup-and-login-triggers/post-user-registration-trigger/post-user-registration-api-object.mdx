...
title: 'Déclencheurs d’Actions : post-user-registration - Objet d’API'
...
Supprime l’enregistrement associé à une valeur en cache à la clé fournie, s’il existe.
...
Les valeurs stockées dans ce cache sont propres au déclencheur dans lequel elles
...
spécifiée par `ttl` ou `expires_at`. Si aucune durée de vie n’est spécifiée, une durée de vie par
