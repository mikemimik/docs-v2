...
Modifiez les droits d’accès de l’utilisateur qui tente de se connecter, par exemple en rejetant la tentative de connexion.

<div id="apiaccessdenyreason-usermessage">
  ### `api.access.deny(reason, userMessage)`
</div>

Empêche l’utilisateur de s’inscrire. Le flux d’inscription s’arrêtera immédiatement après
l’exécution de cette action et aucune autre Action ne sera exécutée.

<ResponseField name="reason" type="string">
  Une raison interne expliquant pourquoi cette tentative d’inscription est refusée. Cette valeur
  apparaîtra dans le journal du tenant.
</ResponseField>

<ResponseField name="userMessage" type="string">
  Une explication claire du rejet de la tentative d’inscription. Elle peut être présentée
...
Remarque : cette méthode ne doit pas être utilisée dans les callbacks. Son appel ne met pas immédiatement les métadonnées à jour.
...
Récupère l’enregistrement décrivant une valeur en cache pour la clé fournie,
...
spécifiées par `ttl` ou `expires_at`. Si aucune durée de vie n’est spécifiée, une durée de vie
...
action, et aucune autre Action ne sera exécutée.

<ResponseField name="errorCode" type="string">
  Un code d’erreur défini par le client qui décrit pourquoi cette tentative d’inscription est refusée. Cette valeur
  apparaîtra dans les journaux du tenant.
</ResponseField>

<ResponseField name="errorMessage" type="string">
  Une explication définie par le client pour justifier le refus de la tentative d’inscription. Celle-ci peut être affichée
  directement dans les interfaces destinées à l’utilisateur final.
</ResponseField>