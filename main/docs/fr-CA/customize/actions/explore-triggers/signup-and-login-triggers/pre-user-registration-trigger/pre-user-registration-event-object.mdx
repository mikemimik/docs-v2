...
#
description: Découvrez l’objet Event du déclencheur pre-user-registration d’Actions, qui fournit des renseignements contextuels sur l’exécution du déclencheur.
title: 'Déclencheurs d’Actions : pre-user-registration - Objet Event'
validatedOn: 2026-05-25
---

L’objet `event` du déclencheur pre-user-registration d’Actions fournit des renseignements contextuels sur l’exécution du déclencheur.
...
<ResponseField name="event.authentication" type="dictionary" post={["optional"]}>
  Détails sur l’authentification obtenus pendant le flux Pre User Registration.
        ...
        <ResponseField name="supplemental" type="dictionary" post={["optional"]}>
          ...
          <Expandable title="propriétés de supplemental">
            <ResponseField name="akamai" type="dictionary" post={["optional"]}>
              &#91;Early Access&#93; Évaluation supplémentaire du risque. Cette fonctionnalité est offerte uniquement si Akamai Account Protector est activé et qu’Akamai transmet les headers de la transaction.
                ...
                <ResponseField name="akamaiUserRisk" type="dictionary" post={["optional"]}>
                  ...
                  <Expandable title="propriétés de akamaiUserRisk">
                    ...
                    <ResponseField name="uuid" type="string" post={["optional"]}>
                      Le UUID de l’évaluation du risque utilisateur d’Akamai.
                    </ResponseField>
                  </Expandable>
                </ResponseField>
              </Expandable>
...
<ResponseField name="event.client" type="dictionary" post={["optional"]}>
  Renseignements sur le client à partir duquel cette transaction a été initiée.

  <Expandable title="propriétés du client" defaultOpen>
    <ResponseField name="client_id" type="string">
      L’ID du client de l’application à laquelle l’utilisateur se connecte.
    </ResponseField>
...
<ResponseField name="event.request" type="dictionary">
  ...
  <Expandable title="propriétés de la requête" defaultOpen>
    <ResponseField name="body" type="dictionary">
      Le corps de la requête POST. Ces données sont disponibles uniquement dans les flux refresh token et Client Credential Exchange, ainsi que dans l’Action PreUserRegistration.
    </ResponseField>
...
<ResponseField name="event.secrets" type="secrets">
  Valeurs de secrets associées de manière sécurisée à cette Action.
</ResponseField>
...
<ResponseField name="event.transaction" type="dictionary" post={["optional"]}>
  ...
  <Expandable title="propriétés de la transaction" defaultOpen>
    ...
    <ResponseField name="prompt" type="array of strings" post={["optional"]}>
      Liste d’instructions indiquant si l’utilisateur peut être invité à se ré-authentifier et à donner son consentement.
    </ResponseField>
    ...
    <ResponseField name="correlation_id" type="string" post={["optional"]}>
      L’ID de corrélation peut être fourni dans la requête d’authentification initiale lorsque l’application redirige vers Universal Login. Vous pouvez utiliser cette valeur pour corréler les logs et les requêtes de votre code Action avec le flux utilisateur.
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="event.user" type="dictionary">
  ...
  <Expandable title="propriétés de l’utilisateur" defaultOpen>
    ...
    <ResponseField name="user_metadata" type="dictionary" post={["optional"]}>
      Champs personnalisés qui stockent des renseignements sur un utilisateur sans incidence sur ce à quoi il a accès, comme son adresse professionnelle, son adresse domiciliaire ou ses préférences.
    </ResponseField>
