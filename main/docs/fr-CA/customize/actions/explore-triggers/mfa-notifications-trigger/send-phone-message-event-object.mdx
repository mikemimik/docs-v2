...
<ResponseField name="event.security_context" type="object" post={["optional"]}>
  Un objet contenant des signatures d’empreinte. Il est disponible uniquement si le client utilise cloudflare. L’empreinte JA3/JA4 peut être null ou vide dans certains cas. Le cas le plus courant concerne les requêtes HTTP, car les empreintes JA3 et JA4 sont calculées au niveau de TLS. Elle peut aussi être vide si le Worker envoie des requêtes dans la même zone ou vers une zone qui n’est pas derrière un proxy (ou vers un tiers).
...
<ResponseField name="event.user" type="dictionary">
  ...
  <Expandable title="propriétés de l’utilisateur" defaultOpen>
    ...
    <ResponseField name="user_metadata" type="dictionary">
      Champs personnalisés qui stockent des renseignements sur un utilisateur sans incidence sur ce à quoi il a accès, comme l’adresse professionnelle, l’adresse domiciliaire ou les préférences de l’utilisateur.
    </ResponseField>
    ...
    <ResponseField name="identities" type="array of objects" post={["optional"]}>
      Contient les renseignements récupérés auprès du fournisseur d’identité avec lequel l’utilisateur s’est authentifié initialement. Les utilisateurs peuvent aussi lier leur profil à plusieurs fournisseurs d’identité; ces identités apparaîtront alors également dans ce tableau. Le contenu de chaque objet de fournisseur d’identité varie selon le fournisseur.
      ...
      <Expandable title="propriétés des identités" defaultOpen>
        ...
        <ResponseField name="profileData" type="dictionary" post={["optional"]}>
          Renseignements sur l’utilisateur associés à la connexion. Lorsque des profils sont liés, cette propriété contient les renseignements utilisateur associés aux comptes secondaires.
        </ResponseField>
