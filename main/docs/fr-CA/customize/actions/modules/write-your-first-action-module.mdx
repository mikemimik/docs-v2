---
mode: wide
description: Explique comment utiliser les Action Modules d’Auth0
...
Utilisez le Quickstart pour créer un Action Module et l’utiliser dans une Action afin d’envoyer des logs vers un service externe.

<Steps>
  <Step title="Créer un module d’action" titleSize="h3" stepNumber={1}>
    Pour utiliser un Action Module dans une Action précise, vous devez d’abord créer l’Action Module, puis l’ajouter à une Action et l’utiliser.
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        1. Accédez à [Auth0 Dashboard &gt; Actions &gt; Bibliothèque](https://manage.auth0.com/#/actions/library), puis sélectionnez **Modules**.
  ...
  <Step title="Ajouter un Secret" titleSize="h3" stepNumber={2}>
    Chaque Action Module peut contenir des paires clé/valeur secrètes. Utilisez-les pour stocker des renseignements sensibles, comme des clés API, des certificats et des valeurs qui peuvent varier d’un environnement à l’autre.
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        Stockons l’URL du service externe en tant que Secret.
        ...
        3. Entrez `SERVICE_URL` dans le champ **Name**.
        4. Entrez l’URL du service externe dans le champ **Value**.
    ...
    **Étape supplémentaire :** Répétez les étapes pour enregistrer un `API_KEY` en tant que Secret, que vous enverrez dans les en-têtes au service externe à chaque requête.
    ...
    <Warning>
      Chaque **Action Module Secret** est indépendant des autres **Actions Modules** et des **Actions Secrets**.
    </Warning>

    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      Utilisez les **Secrets** dans l’Action Module en tapant `actions.secrets.[secret_key]`.
    </Callout>
  </Step>

  <Step title="Ajouter une logique personnalisée" titleSize="h3" stepNumber={3}>
    Ajoutez maintenant le code suivant à votre Action Module.
  ...
  <Step title="Publier le module d’action" titleSize="h3" stepNumber={5}>
    Une fois que le code de l’Action Module vous convient, il est temps de le **publier**.
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
          La publication d’un Action Module en crée un instantané à ce moment précis et l’enregistre en tant que **version d’Action Module**.
        </Callout>
    ...
    <Warning>
      Chaque **Action** doit faire explicitement référence à la **version d’Action Module**, ce qui empêche les mises à niveau automatiques pouvant introduire des erreurs inattendues.
    </Warning>
  </Step>
  ...
  <Step title="Utiliser le module Action dans l’Action" titleSize="h3" stepNumber={7}>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        <Frame>![Actions - Utiliser un Action Module](/docs/images/customize/actions/modules/actions-modules-use-module.png)</Frame>
      </Tab>
...
<Check>
  **Vérification**
  ...
  Vous devriez maintenant disposer d’un **Action Module** entièrement fonctionnel utilisé par une **Action**.
</Check>
...
<Accordion title="Dépendances NPM">
  ...
  <Tabs>
    ...
    <Tab title="Dashboard">
      ...
      3. Entrez le **Name** du **package NPM**.
      4. Entrez la **Version** du **package NPM**.
      5. Sélectionnez **Create**.
  ...
  <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
    Utilisez les **Dependencies** répertoriées dans l’Action Module en les important avec `require('[package-name]')`.
  </Callout>

  <Warning>
    Lorsque vous enregistrez cette Action, Auth0 récupère la plus récente version de votre dépendance et la remplace par un numéro de version précis afin d’éviter que de futures mises à jour du package ne brisent votre Action.

    Vous pouvez aussi indiquer une version précise au lieu de `latest`.
  </Warning>
</Accordion>
