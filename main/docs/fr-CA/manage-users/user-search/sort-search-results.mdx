---
description: Découvrez comment trier les résultats de recherche en passant une valeur `field:order` au paramètre sort.
...
Pour trier les résultats de recherche d’utilisateurs, transmettez une valeur `field:order` au paramètre `sort` lorsque vous envoyez votre requête. Le `field` correspond au nom du champ à utiliser pour le tri, tandis que `order` peut être défini à `1` pour l’ordre croissant et à `-1` pour l’ordre décroissant.
