---
description: Apprenez à trier les résultats de recherche en passant une valeur champ:ordre au paramètre sort.
...
Pour trier les résultats de recherche d’utilisateurs, transmettez une valeur `field:order` au paramètre `sort` lors de l’envoi de votre requête. Le `field` correspond au nom du champ à utiliser pour le tri, tandis que `order` peut être défini à `1` pour l’ordre croissant et à `-1` pour l’ordre décroissant.
