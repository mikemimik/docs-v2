...
Pour trier les résultats de recherche d’utilisateurs, transmettez une valeur `field:order` au paramètre `sort` lorsque vous envoyez votre requête. Le `field` correspond au nom du champ servant au tri, tandis que `order` peut être défini à `1` pour un ordre croissant et à `-1` pour un ordre décroissant.

Le tri selon `app_metadata` ou `user_metadata` n’est pas pris en charge.
