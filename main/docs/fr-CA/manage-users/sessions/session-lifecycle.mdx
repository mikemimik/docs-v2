---
description: Découvrez le cycle de vie de la session.
title: Cycle de vie de la session
---

Le cycle de vie d’une session Auth0 commence lorsqu’un utilisateur se connecte, reste actif tant que l’utilisateur interagit avec le système et se termine lorsque la session expire ou est explicitement fermée.

Auth0 considère qu’une session est active lorsqu’un utilisateur interagit avec le serveur d’autorisation.

Par exemple :

* Appels au point de terminaison [/authorize](/docs/fr-CA/api/authentication/authorization-code-flow/authorize-application)
* Effectuer une [authentification silencieuse](/docs/fr-CA/authenticate/login/configure-silent-authentication)
* Se connecter à une nouvelle application à l’aide de l’[authentification unique](/docs/fr-CA/authenticate/single-sign-on)

Une session peut être persistante ou non persistante; cela vous permet de configurer le comportement d’un cookie de session dans le navigateur.

* **Les sessions persistantes** stockent des cookies avec une date d’expiration ultérieure.
* **Les sessions non persistantes** stockent des cookies avec `Expires=0`. Cela indique au navigateur de supprimer le cookie à sa fermeture.

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Le comportement de persistance des cookies de session dépend de la mise en œuvre du navigateur. Auth0 ne peut pas garantir que tous les navigateurs supprimeront les cookies non persistants comme prévu.
  Pour en savoir plus, consultez [Cookies](https://auth0.com/docs/manage-users/cookies).
</Callout>

<div id="session-lifetime">
  ## Durée de vie de la session
</div>

Les sessions persistantes et non persistantes peuvent être configurées avec leur propre durée d’inactivité et leur durée absolue (maximale) afin de définir l’expiration d’une session.

<div id="idle-timeout">
  ### Délai d’expiration d’inactivité
</div>

Le délai d’expiration d’inactivité définit la durée maximale d’inactivité autorisée. Chaque fois qu’un utilisateur interagit avec Auth0 au moyen de l’authentification unique (SSO), de l’authentification silencieuse ou de /authorize, le compteur d’inactivité est réinitialisé. S’il n’y a aucune interaction avec Auth0 pendant la période configurée, la session expire.

<div id="absolute-timeout-maximum">
  ### Délai d’expiration absolu (maximum)
</div>

Le délai d’expiration absolu définit la durée maximale pendant laquelle une session peut rester valide, quelle que soit l’activité de l’utilisateur. Une fois ce délai atteint, la session expire et l’utilisateur devra se réauthentifier.

Découvrez comment [configurer la durée de vie de la session](/docs/fr-CA/manage-users/sessions/configure-session-lifetime).

<div id="session-lifetime-limits">
  ## Limites de durée de vie de la session
</div>

La durée de vie de la session comporte les limites suivantes :

| Type de session | Type de délai d’expiration         | Description                                                                                       | Maximum (forfaits libre-service) | Maximum (forfaits Enterprise) |
| --------------- | ---------------------------------- | ------------------------------------------------------------------------------------------------- | -------------------------------- | ----------------------------- |
| Non persistante | Délai d’expiration pour inactivité | Période après laquelle une session expire en l’absence d’interaction avec le serveur d’autorisation | 3 jours                          | 100 jours                     |
|                 | Connexion requise après            | Durée de vie maximale de la session, peu importe l’activité                                       | 30 jours                         | 365 jours                     |
| Persistante     | Délai d’expiration pour inactivité | Période après laquelle une session expire en l’absence d’interaction avec le serveur d’autorisation | 3 jours                          | 100 jours                     |
|                 | Connexion requise après            | Durée de vie maximale de la session, peu importe l’activité                                       | 30 jours                         | 365 jours                     |

<div id="next-steps">
  ## Prochaines étapes
</div>

* Pour en savoir plus sur l’utilisation d’Actions pour configurer le cycle de vie des sessions, consultez [Sessions avec Actions](/docs/fr-CA/manage-users/sessions/manage-sessions-actions).

* Pour en savoir plus sur les points de terminaison de gestion des sessions de l’Auth0 Management API, consultez [Sessions avec Management API](/docs/fr-CA/manage-users/sessions/manage-user-sessions-with-auth0-management-api).