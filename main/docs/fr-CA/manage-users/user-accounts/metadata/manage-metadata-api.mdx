...
Pour mettre à jour `user_metadata` et ajouter l’adresse résidentielle de l’utilisateur comme propriété de deuxième niveau :
...
Vous effectueriez l’appel `PATCH` suivant :
...
Le profil de l’utilisateur s’affichera désormais comme suit :
...
<Warning>
  Lorsque vous envoyez un appel `PATCH` dans lequel la valeur d’une propriété est définie à `null` (par exemple, `{user_metadata: {color: null}}`), Auth0 **supprime** la paire propriété-valeur de la base de données. De plus, si vous appliquez le correctif aux métadonnées elles-mêmes avec un objet vide, les métadonnées sont entièrement supprimées.
</Warning>
...
Seules les propriétés au niveau racine sont fusionnées dans l’objet. Toutes les propriétés de niveau inférieur sont remplacées.

Par exemple, pour ajouter l’adresse professionnelle d’un utilisateur comme sous-propriété supplémentaire, vous devrez inclure l’intégralité du contenu de la propriété `addresses`. Puisque l’objet `addresses` est une propriété au niveau racine, il sera fusionné dans l’objet JSON final représentant l’utilisateur, mais pas ses sous-propriétés.
...
Par conséquent, l’appel `PATCH` à l’API serait :
...
Appliquer une mise à jour partielle aux métadonnées avec un objet vide supprime complètement les métadonnées. Par exemple, l’envoi de ce corps supprime tout dans `app_metadata` :
