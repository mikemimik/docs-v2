...
<Warning>
  **Auth0 ne bascule pas automatiquement vers le bon** **utilisateur** principal après la liaison de comptes; ce changement doit donc être effectué dans le code des Actions une fois la liaison réussie.

  **Chaque liaison manuelle de comptes devrait inviter l’utilisateur à saisir ses identifiants**. Votre tenant devrait demander une authentification pour les deux comptes avant la liaison afin d’éviter que des personnes malveillantes puissent accéder à des comptes d’utilisateur légitimes.
</Warning>

Les étapes suivantes illustrent un exemple de mise en œuvre :

1. L’Action identifie les comptes d’utilisateur susceptibles d’être liés (s’ils existent).

2. L’Action redirige l’utilisateur vers une application externe de liaison avec un payload de jeton contenant les identités d’utilisateur candidates :
...
4. L’application externe de liaison redirige l’utilisateur vers l’Action avec un payload de jeton contenant les identités d’utilisateur principale et secondaire :
...
7. L’Action bascule vers l’utilisateur principal si celui-ci ne correspond pas à `event.user.user_id`.
...
* Liaison de comptes côté client initiée par l’utilisateur au moyen de <Tooltip tip="Jeton d’accès : information d’autorisation, sous la forme d’une chaîne opaque ou d’un JWT, utilisée pour accéder à une API." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=access+tokens">jetons d’accès</Tooltip> avec la portée `update:current_user_identities`.
* Liaison de comptes côté serveur au moyen de jetons d’accès avec la portée `update:users`.

<div id="user-initiated-client-side-account-linking">
  ### Liaison de comptes côté client initiée par l’utilisateur
</div>

Pour une liaison de comptes côté client initiée par l’utilisateur, il vous faut un jeton d’accès dont la charge utile contient les éléments suivants :

* la portée `update:current_user_identites`
* le `user_id` du compte principal dans l’URL
* le <Tooltip tip="jeton ID : justificatif destiné au client lui-même, plutôt que pour accéder à une ressource." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=ID+token">jeton ID</Tooltip> du compte secondaire, signé avec RS256 et comprenant une revendication `aud` qui identifie le client et correspond à la valeur de la revendication `azp` du jeton d’accès ayant servi à la requête.

Un jeton d’accès qui contient la portée `update:current_user_identities` ne peut être utilisé que pour mettre à jour les renseignements de l’utilisateur actuellement connecté. Cette méthode convient donc aux scénarios où l’utilisateur lance lui-même le processus de liaison.
...
Pour effectuer une liaison de comptes côté serveur, vous avez besoin d’un jeton d’accès qui contient les éléments suivants dans la charge utile :

* la portée `update:users`
* le `user_id` du compte principal dans l’URL
* le `user_id` du compte secondaire
* le jeton ID du compte secondaire, signé avec RS256 et comprenant une revendication `aud` qui identifie le client et correspond à la valeur de la revendication `azp` du jeton d’accès à l’origine de la demande.

Les jetons d’accès qui contiennent la portée `update:users` peuvent servir à mettre à jour les renseignements de n’importe quel utilisateur. Par conséquent, cette méthode est destinée uniquement au code côté serveur.
...
Le `user_id` et le `provider` du compte utilisateur secondaire peuvent être déterminés à partir de son identifiant unique. Par exemple, pour l’identifiant `google-oauth2|108091299999329986433` :

* `provider` est `google-oauth2`
* `user_id` est `108091299999329986433`

Vous pouvez aussi envoyer le jeton ID du compte secondaire au lieu du `provider` et du `user_id` :
