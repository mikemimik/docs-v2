...
1. Ouvrez une session pour l’utilisateur dans votre application.<br />
   ...
   L’utilisateur s’authentifie dans votre application au moyen de [Universal Login](/docs/fr-CA/authenticate/login/auth0-universal-login). Pour en savoir plus, consultez le [Quickstart d’application web traditionnelle](/docs/fr-CA/quickstart/webapp), qui explique comment demander un jeton pour l’audience de l’Auth0 Management API (audience=`https://{yourDomain}/api/v2/`).
...
5. Votre application appelle le [point de terminaison Link a User Account](https://auth0.com/docs/api/v2#!/Users/post_identities) de l’API de gestion Auth0 à l’aide d’un [jeton d’accès à l’API de gestion](/docs/fr-CA/secure/tokens/access-tokens/management-api-access-tokens) avec la portée `update:users`.
