...
title: Exportations en bloc d’utilisateurs
---
...
Pour obtenir la liste des champs de profil utilisateur qui peuvent être exportés, consultez [Structure du profil utilisateur](/docs/fr-CA/manage-users/user-accounts/user-profiles/user-profile-structure).
...
Mettez à jour `connection_id` avec l’ID de votre connexion à la base de données, ou supprimez-le pour exporter tous les utilisateurs du tenant.
...
<Warning>
  Vous ne pouvez pas exporter l’ensemble de `app_metadata` ou de `user_metadata` en CSV. Vous devez préciser explicitement les champs des objets de métadonnées.

  Pour exporter `app_metadata` ou `user_metadata` comme objets uniques, utilisez le format compatible avec JSON et incluez le champ souhaité dans le paramètre `fields` du corps de la requête. Par exemple :

  `{"name":"app_metadata"}`

  Comme vous ne pouvez exporter qu’un maximum de 30 champs, il est recommandé d’utiliser le format JSON si les données utilisateur contiennent de nombreux champs.
</Warning>
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Dans les fichiers CSV d’exportation des utilisateurs, nous échappons les données de type String conformément aux normes de l’OWASP pour atténuer les risques d’injection CSV :
...
Avant de pouvoir importer des utilisateurs à l&#39;aide d&#39;une exportation générée par Auth0, vous devrez convertir le fichier du format NDJSON en JSON à l&#39;aide de la bibliothèque de votre choix (comme [jq](https://stedolan.github.io/jq/)).
...
Une fois la tâche créée pour exporter vos utilisateurs, vous pouvez en vérifier le statut à l’aide de l’[endpoint Get a Job](https://auth0.com/docs/api/management/v2#!/Jobs/get_jobs_by_id).

Fournissez l’ID de la tâche (reçu dans la réponse au moment de sa création). Si vous utilisez l’exemple de requête ci-dessous, remplacez l’espace réservé `{yourJobId}` par la valeur de l’ID.
...
Le lien de téléchargement est valide pendant 60 secondes. Si ce délai expire, vous avez 24 heures pour refaire la requête avant que la tâche n’expire.

<div id="job-cleanup">
  ## Nettoyage des tâches
</div>

Toutes les données associées à vos tâches sont automatiquement supprimées après 24 heures et ne sont plus accessibles par la suite. **Nous vous recommandons donc fortement d’enregistrer les résultats des tâches à l’aide du mécanisme de stockage de votre choix**.

<div id="filter-example">
  ## Exemple de filtre
</div>

Vous souhaiterez peut-être filtrer votre fichier `.csv` exporté pour n’en conserver qu’un sous-ensemble de données, comme la date de la dernière connexion des utilisateurs à votre application.

1. Faites une requête `POST` au endpoint [Create export user&#39;s tâche](https://auth0.com/docs/api/management/v2/jobs/post-users-exports) de la Management API pour exporter les utilisateurs dans un fichier `.csv` :
...
2. Accédez au fichier dans le dossier Téléchargements de votre ordinateur local.
