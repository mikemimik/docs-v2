...
Vous pouvez utiliser le endpoint `POST /api/v2/jobs/users-exports` pour créer une tâche qui exporte tous les utilisateurs associés à une [connexion](/docs/fr-CA/authenticate/identity-providers) ou tous les utilisateurs du tenant.

Pour obtenir la liste des champs du profil utilisateur qui peuvent être exportés, consultez [User Profile Structure](/docs/fr-CA/manage-users/user-accounts/user-profiles/user-profile-structure).
