...
title: Exportation en bloc des utilisateurs
---
...
Vous pouvez utiliser l’endpoint `POST /api/v2/jobs/users-exports` pour créer une tâche qui exporte tous les utilisateurs associés à une [connexion](/docs/fr-CA/authenticate/identity-providers), ou tous les utilisateurs du tenant.

Pour obtenir la liste des champs du profil utilisateur qui peuvent être exportés, consultez la [Structure du profil utilisateur](/docs/fr-CA/manage-users/user-accounts/user-profiles/user-profile-structure).

Lorsque vous créez votre tâche, vous devrez fournir :

* l’[ID de la connexion](/docs/fr-CA/authenticate/identity-providers/locate-the-connection-id) à partir de laquelle vous voulez exporter les utilisateurs (facultatif)
* le format du fichier d’exportation (CSV ou compatible avec JSON)
* le nombre maximal d’enregistrements d’utilisateurs à exporter (facultatif; tous les enregistrements seront exportés s’il est omis)
* les champs associés à l’utilisateur (comme l’ID de l’utilisateur ou le nom) que vous voulez inclure dans l’exportation

Vous aurez aussi besoin d’un [jeton d’accès de la Management API](https://auth0.com/docs/api/management/v2/tokens).

<div id="create-a-request-body">
  ## Créer un corps de la requête
</div>

Au besoin, repérez le `connection_id` ainsi que le nom de domaine de votre locataire Auth0 dans le <Tooltip tip="Auth0 Dashboard : principal produit d'Auth0 pour configurer vos services." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip>. Créez un fichier texte contenant le corps de la requête ci-dessous :
...
Mettez à jour le `connection_id` en y indiquant l’ID de votre connexion de base de données, ou supprimez-le pour exporter tous les utilisateurs du tenant.
...
<div id="include-user-metadata-in-exported-csv">
  ## Inclure les métadonnées utilisateur dans le fichier CSV exporté
</div>
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Dans les fichiers CSV d’exportation des utilisateurs, nous appliquons un échappement aux types de données String conformément aux normes OWASP visant à atténuer les risques d’injection CSV :
...
Si vous exportez les données dans un format compatible avec JSON, vous n’avez qu’à fournir la propriété racine ; vous n’avez pas à nommer chaque propriété interne individuellement, puisqu’elles seront incluses automatiquement.

Les fichiers d’exportation d’Auth0 utilisent le format [NDJSON](https://github.com/ndjson/ndjson-spec) en raison de leur grande taille, tandis que la fonction d’importation exige un fichier JSON.

Avant de pouvoir importer des utilisateurs à l’aide d’une exportation générée par Auth0, vous devrez convertir le fichier de NDJSON en JSON à l’aide de la bibliothèque de votre choix (comme [jq](https://stedolan.github.io/jq/)).
...
Indiquez l’ID de la tâche (que vous avez reçu dans la réponse lors de la création de la tâche). Si vous utilisez l’exemple de requête ci-dessous, remplacez l’espace réservé `{yourJobId}` par la valeur de l’ID.
...
Le lien de téléchargement est valide pendant 60 secondes. Une fois ce délai écoulé, vous avez 24 heures pour y accéder de nouveau avant l’expiration de la tâche.

<div id="job-cleanup">
  ## Nettoyage des tâches
</div>

Toutes les données liées à vos tâches sont automatiquement supprimées après 24 heures et ne sont plus accessibles ensuite. Par conséquent, **nous vous recommandons fortement d’enregistrer les résultats des tâches à l’aide du mécanisme de stockage de votre choix**.

<div id="filter-example">
  ## Exemple de filtre
</div>

Vous voudrez peut-être filtrer le fichier .csv exporté pour n’en conserver qu’un sous-ensemble précis, par exemple la date de dernière connexion des utilisateurs à votre application.

1. Effectuez un appel `POST` à l’endpoint [Create export user&#39;s tâche](https://auth0.com/docs/api/management/v2/jobs/post-users-exports) de la Management API pour exporter les utilisateurs dans un fichier `.csv` :
