...
* Les permissions sont choisies parmi des valeurs prédéfinies. Si votre liste de rôles est vide, assurez-vous d’[ajouter des permissions](/docs/fr-CA/get-started/apis/add-api-permissions) à votre API.

<div id="dashboard">
  ## Dashboard
</div>

Il existe deux façons d’attribuer une permission à un utilisateur. Vous pouvez sélectionner un utilisateur dans la liste **Users**, puis lui attribuer une permission, ou accéder à la page **User Details** (profil utilisateur) d’un utilisateur en particulier et choisir la permission à attribuer dans l’onglet **Permissions**.

<div id="assign-permission-in-user-list">
  ### Attribuer des permissions dans la liste des utilisateurs
</div>
...
Vous pouvez aussi attribuer des permissions à des utilisateurs à partir de la page de leur profil individuel.

1. Accédez à [Dashboard &gt; User Management &gt; Users](https://manage.auth0.com/#/users), puis cliquez sur le nom de l’utilisateur à afficher.
2. Cliquez sur l’onglet **Permissions**, puis sur **Assign Permissions**.
3. Sélectionnez l’API à partir de laquelle vous voulez attribuer des permissions, puis sélectionnez les permissions à attribuer à l’utilisateur et cliquez sur **Add Permissions**.

<div id="management-api">
  ## Management API
</div>

Effectuez une requête `POST` vers le [point de terminaison Assign User Permissions](https://auth0.com/docs/api/management/v2#!/Users/post_permissions). Assurez-vous de remplacer les valeurs de l’espace réservé `USER_ID`, `MGMT_API_ACCESS_TOKEN`, `API_IDENTIFIER` et `PERMISSION_NAME` par votre ID utilisateur, le <Tooltip tip="Jeton d’accès : information d’authentification, sous la forme d’une chaîne opaque ou d’un JWT, utilisée pour accéder à une API." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=access+token">jeton d’accès</Tooltip> de la Management API, le ou les identifiants de l’API et le ou les noms de permission, respectivement.
...
| Valeur                  | Description                                                                                                            |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `USER_ID`               | L’ID de l’utilisateur auquel vous souhaitez attribuer des permissions.                                               |
...
| `API_IDENTIFIER`        | Identifiant(s) de la ou des API associées aux permissions que vous souhaitez attribuer à l’utilisateur indiqué.      |
| `PERMISSION_NAME`       | Nom(s) des permissions que vous souhaitez attribuer à l’utilisateur indiqué.                                         |
