...
<Warning>
  ...
  Les nouveaux tenants doivent suivre les [instructions d’authentification Passwordless avec un fournisseur de téléphone SMS](/docs/fr-CA/authenticate/passwordless/authentication-methods/sms-otp#configure-the-phone-provider).
</Warning>
...
La réponse de l’endpoint sera un tableau d’objets. Chaque objet représente une connexion associée à votre tenant.
4. Repérez l’ID de votre connexion. Vous pouvez trouver l’ID associé à votre connexion Passwordless en consultant le tableau d’objets renvoyé par l’[endpoint GET Connections](https://auth0.com/docs/api/management/v2#!/Connections/get_connections) à l’étape 2.
Pour trouver l’objet correspondant à votre connexion Passwordless, vous pouvez rechercher la propriété `"name": "sms"`. Notez que la connexion affiche actuellement les renseignements Twilio que vous avez fournis pendant la configuration.
...
5. Mettez à jour la connexion. Vous pouvez le faire en envoyant un appel PATCH au [point de terminaison Update a Connection](https://auth0.com/docs/api/management/v2#!/Connections/patch_connections_by_id). Plus précisément, vous allez mettre à jour l’objet `options` de la connexion afin de fournir des informations sur la passerelle SMS.

   <Warning>
     Vous devez envoyer l’objet `options` au complet avec chaque appel; sinon, vous écraserez les données existantes qui ne sont pas incluses dans les appels suivants.
   </Warning>

   Apportez les modifications suivantes :

   * Supprimez les paramètres `twilio_sid` et `twilio_token`
   * Ajoutez le paramètre `provider` et définissez-le sur `sms_gateway`
   * Ajoutez le paramètre `gateway_url` et définissez-le sur l’URL de votre passerelle SMS. Auth0 doit pouvoir joindre cette URL pour utiliser votre passerelle afin d’envoyer des messages en votre nom.
...
Si votre secret est encodé en base64-url, définissez `secret_base64_encoded` à `true`.
...
Si vous définissez la propriété `forward_req_info` de l’`objet` `options` à `true`, la passerelle recevra aussi des renseignements provenant de la requête HTTP qui a lancé le processus Passwordless. Cela comprend l’adresse IP du client qui appelle `/passwordless/start` et son agent utilisateur.
...
Si la passerelle SMS renvoie un code HTTP autre que 200, le endpoint `/passwordless/start` renverra un code HTTP 400 ainsi qu’une réponse semblable à la suivante :
...
Si la passerelle SMS renvoie un code HTTP 401, la valeur de `error_description` sera **Authentication failed while calling the SMS gateway: 401**. (Veuillez noter que le libellé de la description d’erreur peut changer à tout moment.)

Auth0 impose un délai d’expiration de 30 secondes pour les appels HTTP vers des passerelles SMS personnalisées. Si la passerelle SMS ne répond pas dans ce délai, l’endpoint `/passwordless/start` renverra également un code HTTP 400. La réponse aura le format indiqué ci-dessus et le champ `error_description` aura pour valeur **Timeout while calling the SMS gateway: &lt;Timeout Code&gt;**. (Encore une fois, le libellé de la description d’erreur peut changer à tout moment.)