---
description: Décrit la mise en œuvre de l’authentification sans mot de passe avec connexion intégrée dans les applications Web traditionnelles.
title: Connexion sans mot de passe intégrée dans les applications Web traditionnelles
---
...
Pour utiliser les API Embedded <Tooltip tip="Passwordless : forme d’authentification qui ne repose pas sur un mot de passe comme premier facteur." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Passwordless">Passwordless</Tooltip> dans les Regular Web Applications, assurez-vous d’activer le type d’octroi **Passwordless OTP** dans [Auth0 Dashboard &gt; Applications &gt; Applications](https://manage.auth0.com/#/applications), sous les paramètres de votre application dans **Advanced Settings** &gt; **Grant Types**.

L’authentification sans mot de passe pour les Regular Web Applications se déroule en deux étapes :
...
Voici quelques extraits de code pouvant être utilisés pour appeler ces points de terminaison d’API dans différents scénarios.
...
**Authentifier un utilisateur utilisant Email**
...
<div id="setting-the-auth0-forwarded-for-header-for-rate-limit-purposes">
  ## Définir l’en-tête auth0-forwarded-for pour la limitation du nombre de requêtes
</div>

Le point de terminaison `/passwordless/start` a une [limite de requêtes](/docs/fr-CA/troubleshoot/customer-support/operational-policies/rate-limit-policy) de 50 requêtes par heure et par adresse IP. Si vous appelez l’API côté serveur, l’adresse IP de votre backend peut facilement atteindre ces limites. Pour savoir comment résoudre ce problème, consultez la section sur la limitation du nombre de requêtes dans les points de terminaison Passwordless de [Utilisation des API Passwordless](/docs/fr-CA/authenticate/passwordless/implement-login/embedded-login/relevant-api-endpoints).
...
Personnalisez <Tooltip tip="Authentification multifacteur (MFA) : processus d’authentification de l’utilisateur qui utilise un facteur en plus du nom d’utilisateur et mot de passe, comme un code envoyé par SMS." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=MFA">MFA</Tooltip> dans des flux intégrés. Utilisez l’API MFA pour permettre aux utilisateurs d’enregistrer les facteurs pris en charge par votre application de leur choix et de répondre aux demandes de vérification avec ceux-ci.

Lorsque vous utilisez [Lock for Web](/docs/fr-CA/libraries/lock#2-authenticating-and-getting-user-info), le point de terminaison `oauth/token` renvoie l’erreur `mfa_required` et inclut le `mfa_token` nécessaire pour utiliser l’API MFA, ainsi que le paramètre `mfa_requirements` contenant la liste des authentificateurs actuellement pris en charge par votre application :
