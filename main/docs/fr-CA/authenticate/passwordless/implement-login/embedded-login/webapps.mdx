...
title: Connexion sans mot de passe intégrée dans les applications Web régulières
---
...
Pour utiliser les API <Tooltip tip="Passwordless : forme d’authentification qui ne repose pas sur un mot de passe comme premier facteur." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Passwordless">Passwordless</Tooltip> intégrées dans les applications Web régulières, assurez-vous d’activer l’autorisation **Passwordless OTP** dans [Auth0 Dashboard &gt; Applications &gt; Applications](https://manage.auth0.com/#/applications), dans les paramètres de votre application sous **Advanced Settings** &gt; **Grant Types**.

L’authentification sans mot de passe pour les applications Web régulières se déroule en deux étapes :

1. Recueillez l’identifiant utilisateur dans votre application (l’adresse courriel ou le numéro de téléphone de l’utilisateur), puis appelez le point de terminaison `/passwordless/start` pour lancer le processus sans mot de passe. L’utilisateur recevra un courriel, un SMS contenant un code à usage unique ou un lien magique.
...
Notez que si vous utilisez des liens magiques, vous n’avez pas besoin d’appeler `/oauth/token`. L’utilisateur cliquera sur le lien magique et sera redirigé vers l’URL de rappel de l’application.

Voici quelques extraits de code que vous pouvez utiliser pour appeler ces points de terminaison d’API dans différents scénarios.
...
Lorsque vous envoyez un lien magique, vous n’avez pas besoin d’appeler une API pour authentifier l’utilisateur. Les utilisateurs cliqueront sur le lien et seront redirigés vers l’URL de rappel.

<div id="setting-the-auth0-forwarded-for-header-for-rate-limit-purposes">
  ## Définir l’en-tête auth0-forwarded-for pour l’application des limites de débit
</div>

Le point de terminaison `/passwordless/start` a une [limite de débit](/docs/fr-CA/troubleshoot/customer-support/operational-policies/rate-limit-policy) de 50 requêtes par heure par IP. Si vous appelez l’API côté serveur, l’IP de votre back-end peut facilement atteindre ces limites de débit. Pour savoir comment résoudre ce problème, consultez la section Limitation du débit des points de terminaison Passwordless de [Utilisation des API Passwordless](/docs/fr-CA/authenticate/passwordless/implement-login/embedded-login/relevant-api-endpoints).
...
Personnalisez <Tooltip tip="Authentification multifacteur (MFA) : processus d’authentification de l’utilisateur qui utilise un facteur en plus du nom d’utilisateur et du mot de passe, comme un code envoyé par SMS." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=MFA">MFA</Tooltip> avec des flux intégrés. Utilisez la MFA API pour permettre aux utilisateurs d’enregistrer les facteurs pris en charge par votre application et de les utiliser pour la vérification, selon leur choix.

Lors de l’utilisation de [Lock for Web](/docs/fr-CA/libraries/lock#2-authenticating-and-getting-user-info), le point de terminaison `oauth/token` renvoie l’erreur `mfa_required` et inclut le `mfa_token` que vous devez utiliser avec la MFA API, ainsi que le paramètre `mfa_requirements` contenant une liste des authenticateurs actuellement pris en charge par votre application :
...
Pour imposer la vérification MFA, appelez le point de terminaison [`request/mfa/challenge`](https://auth0.com/docs/api/authentication/muti-factor-authentication/request-mfa-challenge).

Personnalisez davantage votre flux MFA grâce à Auth0 Actions. Pour en savoir plus, consultez [Déclencheurs d’Actions : post-challenge - objet API](/docs/fr-CA/customize/actions/explore-triggers/password-reset-triggers/post-challenge-trigger/post-challenge-api-object).