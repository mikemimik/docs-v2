---
description: Décrit l’implémentation de l’authentification sans mot de passe avec connexion intégrée dans les applications natives.
...
Pour utiliser les API <Tooltip tip="Passwordless : forme d’authentification qui ne repose pas sur un mot de passe comme premier facteur." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Passwordless">Passwordless</Tooltip> intégrées dans les applications natives, assurez-vous d’activer le grant **Passwordless OTP** dans [Auth0 Dashboard &gt; Applications &gt; Applications](https://manage.auth0.com/#/applications), dans les paramètres de votre application sous **Advanced Settings** &gt; **Grant Types**.

L’authentification sans mot de passe pour les applications natives se fait en deux étapes :

* Recueillez l’identifiant de l’utilisateur dans votre application (son adresse courriel ou son numéro de téléphone), puis appelez le point de terminaison `/passwordless/start` pour lancer le flux sans mot de passe. L’utilisateur recevra un courriel ou un SMS contenant un mot de passe à usage unique.
* Demandez ensuite à l’utilisateur de saisir le code à usage unique, puis appelez le point de terminaison `/oauth/token` pour obtenir des jetons d’authentification.

Voici quelques extraits de code qui peuvent servir à appeler ces API endpoints dans différents scénarios.
...
**Envoyez un mot de passe à usage unique par SMS**
...
Si vous préférez, vous pouvez utiliser les SDKs Android ou iOS, qui encapsulent ces API de façon adaptée à la plateforme :
...
<div id="customize-mfa-with-embedded">
  ## Personnaliser MFA avec intégré
</div>
...
Personnalisez <Tooltip tip="Multi-factor authentication (MFA) : processus d’authentification de l’utilisateur qui utilise un facteur en plus du nom d’utilisateur et du mot de passe, comme un code envoyé par SMS." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=MFA">MFA</Tooltip> avec des flux intégrés. Utilisez la MFA API pour permettre aux utilisateurs d’enregistrer les facteurs de leur choix et de répondre aux demandes de vérification prises en charge par votre application.

Lorsque votre application demande un <Tooltip tip="Access Token : identifiant d’autorisation, sous la forme d’une chaîne opaque ou d’un JWT, utilisé pour accéder à une API." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=access+token+at+the">jeton d’accès au</Tooltip> point de terminaison `oauth/token`, la réponse renvoyée inclut le `mfa_token` à utiliser avec la MFA API ainsi que le paramètre `mfa_requirements`, accompagné d’une liste des authentificateurs actuellement pris en charge par votre application :
