---
description: Décrit la mise en œuvre de l’authentification Passwordless avec connexion Embedded pour les applications natives.
title: Connexion Passwordless Embedded pour les applications natives
---
...
Pour utiliser les API <Tooltip tip="Sans mot de passe : forme d’authentification qui ne repose pas sur un mot de passe comme premier facteur." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Passwordless">Passwordless</Tooltip> Embedded dans les applications natives, assurez-vous d’activer l’octroi **Passwordless OTP** dans [Auth0 Dashboard &gt; Applications &gt; Applications](https://manage.auth0.com/#/applications), dans les paramètres de votre application sous **Advanced Settings** &gt; **Grant Types**.

L’authentification sans mot de passe pour les applications natives comporte deux étapes :

* Recueillez l’identifiant utilisateur dans votre application (l’adresse courriel ou le numéro de téléphone de l’utilisateur), puis appelez le point de terminaison `/passwordless/start` pour lancer le flux sans mot de passe. L’utilisateur recevra un courriel ou un SMS contenant un mot de passe à usage unique.
...
Personnalisez la <Tooltip tip="Multi-factor authentication (MFA) : Processus d’authentification de l’utilisateur qui utilise un facteur en plus du nom d’utilisateur et mot de passe, comme un code par SMS." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=MFA">MFA</Tooltip> avec des flux intégrés. Utilisez la MFA API pour permettre aux utilisateurs d’inscrire les facteurs pris en charge par votre application de leur choix et de les utiliser pour la vérification.

Lorsque votre application demande un <Tooltip tip="Access Token : Identifiant d’autorisation, sous la forme d’une chaîne opaque ou d’un JWT, utilisé pour accéder à une API." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=access+token+at+the">jeton d’accès au</Tooltip> point de terminaison `oauth/token`, la réponse renvoyée comprend le `mfa_token` à utiliser avec la MFA API ainsi que le paramètre `mfa_requirements` contenant une liste des authentificateurs actuellement pris en charge par votre application :
...
Utilisez le `mfa_token` pour appeler le point de terminaison [`mfa/authenticator`](https://auth0.com/docs/api/authentication/muti-factor-authentication/list-authenticators) afin d’obtenir la liste de tous les facteurs auxquels l’utilisateur s’est déjà inscrit et de repérer le même `type` que votre application prend en charge. Vous devez aussi obtenir la valeur `authenticator_type` correspondante pour lancer des demandes de vérification :
