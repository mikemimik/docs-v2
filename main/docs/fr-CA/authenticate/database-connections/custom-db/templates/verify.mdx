---
description: Décrit les modèles de scripts d’action de base de données personnalisée pour la vérification du courriel de l’utilisateur.
title: Modèles de script Verify
---

Le script Verify implémente la fonction exécutée pour indiquer l’état de vérification de l’adresse courriel d’un utilisateur dans une base de données externe. Nous recommandons de nommer cette fonction `verify`.

Ce script est utilisé uniquement dans les scénarios d’authentification héritée et il est nécessaire pour assurer la vérification de l’adresse courriel de l’utilisateur. Une adresse courriel vérifiée est essentielle à plusieurs scénarios de workflow dans Auth0, et l’implémentation de ce script offrira ce soutien prêt à l’emploi.
...
* Mettre à jour, dans la base de données externe, l’attribut `email_verified` (ou son équivalent) du profil de l’utilisateur.
...
| `callback` | Fonction | Sert à transmettre une erreur ou des données de profil dans le pipeline. |

### Exemple

Voici un exemple en pseudo-Javascript qui montre comment vous pourriez implémenter la fonction `login`. Pour voir des exemples propres à chaque langage, consultez [exemples de scripts par langage](/docs/fr-CA/authenticate/database-connections/custom-db/templates/verify#language-specific-script-examples).
...
La fonction `callback` sert à transmettre les données de profil utilisateur ou les données d’erreur à travers le pipeline.
...
| `verified` | Booléen | Facultatif  | Contient une valeur représentant le statut de vérification de l’utilisateur dans la base de données externe (`true` ou `false`). Obligatoire seulement si la valeur est `true`. |

<div id="return-a-success">
  ### Renvoyer une réussite
</div>

Si l&#39;état de vérification de l&#39;utilisateur a été mis à jour avec succès dans la base de données externe, transmettez une valeur `null` pour le paramètre `error` et une valeur `true` pour le paramètre `verified`.
...
Si une erreur survient, le paramètre `error` doit contenir des renseignements pertinents sur l’origine du problème.
...
function verify(email, callback) {
  ...
  connection.on('connect', function(err) {
    ...
    verifyMembershipUser(email, function(err, wasUpdated) {
      if (err) return callback(err); // ceci retournera une erreur 500
