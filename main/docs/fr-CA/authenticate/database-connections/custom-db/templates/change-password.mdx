---
description: Décrit les modèles de scripts d’action pour base de données personnalisée servant à modifier le mot de passe d’un utilisateur.
title: Modèles de scripts Change Password
---
...
<Warning>
  Évitez de journaliser, de stocker ou de transmettre l’information d’authentification associée au mot de passe sous une forme non chiffrée.
</Warning>

Pour éviter tout risque de fuite de données, hachez la valeur du mot de passe à l’aide d’une bibliothèque de hachage cryptographique comme `bcrypt`.
...
Si l’opération de changement de mot de passe a réussi, appelez la fonction `callback` en transmettant une valeur `null` comme paramètre `error` et un objet comme paramètre `profile`. Si l’attribut `last_password_reset` est fourni dans l’objet, il sera mis à jour dans le profil de l’utilisateur.
...
S&#39;il se produit une erreur, appelez la fonction `callback` et transmettez-lui les renseignements pertinents dans le paramètre `error`.
...
function changePassword(email, newPassword, callback) {
   ...
   * @callback    {[function]}  fonction de rappel à appeler après le hachage du mot de passe
   */
  ...
  connection.on('connect', function (err) {
    ...
    updateMembershipUser(email, newPassword, function(err, wasUpdated) {
      if (err) {
        return callback(err); // ceci retournera une erreur 500
      }
      callback(null, wasUpdated);
    });
  });
...
function changePassword (email, newPassword, callback) {
  ...
  //plus d'info ici : http://tediousjs.github.io/tedious/
  const bcrypt = require('bcrypt');
