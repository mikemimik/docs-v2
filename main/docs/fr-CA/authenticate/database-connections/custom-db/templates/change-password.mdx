---
description: Décrit les modèles de script d’action de base de données personnalisée pour modifier le mot de passe d’un utilisateur.
title: Modèles de script Change Password
---

Le script Change Password implémente la fonction définie pour modifier le mot de passe de l’utilisateur spécifié dans la base de données externe. Nous recommandons d’appeler cette fonction `changePassword`.

Le script est utilisé uniquement dans un scénario d’[authentification héritée](/docs/fr-CA/authenticate/database-connections/custom-db/overview-custom-db-connections) et il est requis si vous souhaitez modifier le mot de passe d’un utilisateur dans la base de données externe. Il s’exécute lorsqu’un utilisateur suit un [processus de réinitialisation du mot de passe](/docs/fr-CA/customize/login-pages/classic-login/customize-password-reset-page), ou lorsqu’un processus de changement de mot de passe est lancé depuis le <Tooltip tip="Auth0 Dashboard : le principal produit d’Auth0 pour configurer vos services." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip> ou l’Auth0 <Tooltip tip="Management API : un produit permettant aux clients d’effectuer des tâches administratives." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Management+API">Management API</Tooltip>.
...
La fonction `changePassword` prend trois paramètres et renvoie une fonction `callback` :
...
| ------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `email`       | String   | Adresse courriel de l’utilisateur dans Auth0 et la base de données externe.                                                                                                                                                                     |
| `newPassword` | String   | Valeur à définir comme nouveau mot de passe de l’utilisateur dans la base de données externe. Cette valeur est transmise en texte brut à la fonction et [devrait être chiffrée](#Encryption) avant d’être envoyée à la base de données externe. |
| `callback`    | Function | Utilisé pour transmettre des données ou le résultat d’une opération dans le pipeline.                                                                                                                                                           |
...
<Warning>
  Évitez d’enregistrer dans les journaux, de stocker ou de transmettre les données d’authentification par mot de passe en clair.
</Warning>

Pour éviter toute fuite potentielle de données, hachez la valeur du mot de passe à l’aide d’une bibliothèque de hachage cryptographique comme `bcrypt`.
...
Si l’opération de changement de mot de passe a réussi, appelez la fonction `callback` en transmettant la valeur `null` pour le paramètre `error` et la valeur `true` pour le paramètre `operationResult`.
...
<div id="return-a-success-and-update-last_password_reset-attribute">
  ### Retourner un succès et mettre à jour l’attribut last_password_reset
</div>

Si l’opération de changement de mot de passe réussit, retournez la fonction `callback` en lui passant la valeur `null` comme paramètre `error` et une valeur de type objet comme paramètre `profile`. Si l’attribut `last_password_reset` est fourni dans l’objet, il sera mis à jour dans le profil de l’utilisateur.
...
Si l’opération de changement de mot de passe a échoué, appelez la fonction `callback` en passant une valeur `null` pour le paramètre `error` et une valeur `false` pour le paramètre `operationResult`.
...
Si une erreur survient, appelez la fonction `callback` et transmettez les renseignements pertinents sur l’erreur dans le paramètre `error`.
...
<div id="aspnet-membership-provider-mvc3-universal-providers">
  ### ASP.NET Membership Provider (MVC3 - fournisseurs Universal)
</div>
...
function changePassword(email, newPassword, callback) {
  ...
  connection.on('connect', function (err) {
    ...
    updateMembershipUser(email, newPassword, function(err, wasUpdated) {
      if (err) {
        return callback(err); // retourne un 500
      }
...
function changePassword(email, newPassword, callback) {
  ...
  connection.on('debug', function(text) {
      // si vous avez des problèmes de connexion, décommentez ceci pour obtenir plus d'informations
      //console.log(text);
  }).on('errorMessage', function(text) {
...
* [Modèles de script Login](/docs/fr-CA/authenticate/database-connections/custom-db/templates/login)
