...
Le script Delete implémente la fonction prévue pour supprimer un utilisateur donné d’une base de données externe. Nous recommandons d’appeler cette fonction `deleteUser`.

Le script est utilisé uniquement dans un [scénario d’authentification héritée](/docs/fr-CA/authenticate/database-connections/custom-db/overview-custom-db-connections) et il est requis si vous souhaitez supprimer un utilisateur d’Auth0 et de votre base de données externe dans la même opération.
...
La fonction `callback` prend un paramètre et renvoie une fonction.
...
Si l’utilisateur a bien été supprimé de votre base de données externe, appelez la fonction `callback` et passez une valeur `null` pour le paramètre `error`.
...
S&#39;il se produit une erreur, appelez la fonction `callback` et transmettez au paramètre `error` les renseignements pertinents sur l&#39;erreur.
