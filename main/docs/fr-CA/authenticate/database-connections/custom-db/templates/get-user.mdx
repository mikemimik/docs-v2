...
* Renvoyer les données de profil de l’utilisateur si celui-ci est trouvé.
* Renvoyer une erreur s’il est impossible de déterminer si l’utilisateur existe ou non.
...
Voici un pseudo-exemple en Javascript montrant comment vous pourriez implémenter la fonction `getUser`. Pour des exemples selon le langage, consultez [Exemples de scripts par langage](#language-specific-script-examples).
...
<div id="return-no-user-profile-user-not-found">
  ### Ne retournez aucun profil utilisateur (utilisateur introuvable)
</div>
...
Si une erreur se produit, transmettez au paramètre `error` les données d’erreur accompagnées des renseignements pertinents sur la nature du problème. Pour en savoir plus, consultez [Résoudre les problèmes liés aux Custom Databases](/docs/fr-CA/authenticate/database-connections/custom-db/error-handling).
