---
description: Décrit les modèles de scripts d’action de base de données personnalisée pour Get User.
title: Modèles de script Get User
---

Le script Get User implémente la fonction exécutée pour vérifier si un utilisateur existe actuellement. Nous recommandons de nommer cette fonction `getUser`.

Ce script est requis pour la [migration automatique](/docs/fr-CA/manage-users/user-migration/configure-automatic-migration-from-your-database) et, selon les opérations configurées pour la connexion, peut aussi être requis pour l’authentification héritée. Auth0 recommande de définir un `user_id` permanent dans le profil utilisateur renvoyé afin d’éviter la création d’utilisateurs en double.
...
La fonction `getUser` accepte deux paramètres et retourne une fonction `callback` :
...
| `callback` | Function | Permet de transmettre l’erreur ou les données de profil dans le pipeline |

### Exemple

Voici un pseudo-exemple en Javascript montrant comment vous pourriez implémenter la fonction `getUser`. Pour des exemples propres à chaque langage, consultez [Exemples de scripts par langage](#language-specific-script-examples).
...
Si une erreur se produit, transmettez les données d’erreur au paramètre `error` avec les renseignements pertinents sur la cause du problème. Pour en savoir plus, consultez [Résoudre les problèmes liés aux bases de données personnalisées](/docs/fr-CA/authenticate/database-connections/custom-db/error-handling).
...
function getByEmail(email, callback) {
  ...
  // Il est utilisé pour vérifier si un utilisateur existe avant d'exécuter des flows qui ne
  ...
  // 1. Un utilisateur a été trouvé avec succès. Le profil doit être dans le format suivant :
...
async function getUserAsync(email, callback) {
  //devrait être mis à jour au fur et à mesure que de nouvelles versions d'axios sont disponibles (https://auth0-extensions.github.io/canirequire/#axios)
  ...
  try {
    response = await axios.post(
      ...
      },
      {
        ...
        headers: {
          //sécuriser l'appel d'API avec l'apiKey stockée dans les paramètres de connexion.
    ...
    if (e.response.status === 404) {
      //supposant que l'API retourne 404 lorsqu'aucun utilisateur avec l'adresse courriel/nom d'utilisateur spécifié n'est trouvé
