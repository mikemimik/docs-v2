...
title: Modèles de scripts Create
---
...
1. [Get User](/docs/fr-CA/authenticate/database-connections/custom-db/templates/get-user) : Vérifie que l’utilisateur n’existe pas déjà dans Auth0 ni dans la base de données externe.
...
* Retourner une erreur si l’opération de création de l’utilisateur a échoué.
...
| ---------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `user`     | Objet. Contient les [données du profil utilisateur](#user-object-parameter) issues du processus de création de l’utilisateur. |
| `callback` | Fonction. Utilisée pour transmettre les données d’erreur dans le pipeline.                                                    |

### Exemple

Voici un exemple en pseudo-JavaScript montrant comment vous pourriez implémenter la fonction `create`. Pour des exemples de scripts propres à chaque langage, consultez [Exemples de scripts propres à chaque langage](#language-specific-script-examples).
...
<Warning>
  Évitez de journaliser, de stocker ou de transporter les données d’authentification par mot de passe, où que ce soit, sous une forme non chiffrée.
</Warning>

Chiffrez la valeur du mot de passe à l’aide d’une bibliothèque de hachage cryptographique comme `bcrypt` afin d’éviter toute fuite potentielle de données.
...
La fonction `callback` accepte un paramètre et retourne une fonction.
...
<div id="return-a-success">
  ### Indiquer une réussite
</div>

Si l’opération de création de l’utilisateur a réussi, retournez la fonction `callback` et transmettez une valeur `null` au paramètre `error`.
...
Si une erreur se produit, retournez la fonction callback et transmettez les renseignements pertinents sur l’erreur dans le paramètre `error`.

<div id="validationerror-type-object">
  #### Objet de type `ValidationError`
</div>

Le type d’erreur personnalisé `ValidationError` vous permet de transmettre des données qui s’afficheront dans les [journaux du tenant](/docs/fr-CA/deploy-monitor/logs).
...
Si vous retournez une erreur avec la valeur `user_exists` pour le paramètre `errorCode`, Auth0 consignera un événement `fs` dans le journal du tenant.
...
| `password`      | Le mot de passe de l’utilisateur en clair.                                                                                                                                                                                       |
| `username`      | Le nom d’utilisateur de l’utilisateur. Obligatoire seulement si le paramètre [**Requires Username**](/docs/fr-CA/authenticate/database-connections/require-username) est activé pour la connexion à une base de données personnalisée. |
...
<div id="user-and-app-metadata">
  ### Métadonnées utilisateur et de l’application
</div>

Les propriétés `user_metadata` et `app_metadata` n’ont pas à être stockées dans votre base de données externe. Auth0 stocke automatiquement ces valeurs dans le profil utilisateur créé à l’interne.
...
function create(user, callback) {
  ...
  // * client_id : le ID client de l'application où l'utilisateur s'est inscrit, ou
  //              clé API s'il a été créé via la Management API ou le Auth0 Dashboard
...
function create(user, callback) {
  ...
  function hashPassword(password, salt) {
    // l'implémentation par défaut utilise HMACSHA256 et comme la longueur de la clé est de 64
