...
Voici un exemple en pseudo-JavaScript montrant comment vous pourriez implémenter la fonction `create`. Pour des exemples propres à chaque langage, consultez [Exemples de scripts propres à chaque langage](#language-specific-script-examples).
...
Chiffrez la valeur du mot de passe à l’aide d’une bibliothèque de hachage cryptographique comme `bcrypt` pour éviter tout risque de fuite de données.
...
Le `callback` prend un paramètre et retourne une fonction.
...
Si l’opération de création de l’utilisateur a réussi, appelez la fonction `callback` et transmettez une valeur `null` comme paramètre `error`.
...
Si une erreur se produit, renvoyez-la par le callback et transmettez les renseignements pertinents au paramètre `error`.

<div id="validationerror-type-object">
  #### Objet de type ValidationError
</div>

L’objet d’erreur personnalisé `ValidationError` vous permet de transmettre des données qui s’afficheront dans vos [journaux du tenant](/docs/fr-CA/deploy-monitor/logs).
...
| `message`   | (Facultatif) Chaîne. Contient de l’information sur l’erreur. |

<div id="return-error-that-user-already-exists">
  #### Renvoyer une erreur indiquant que l’utilisateur existe déjà
</div>

Si vous renvoyez une erreur avec la valeur `user_exists` pour le paramètre `errorCode`, Auth0 enregistrera un événement `fs` dans le journal du tenant.
...
Le paramètre de l’objet `user` contient un ensemble prédéfini de propriétés issues du processus de création de l’utilisateur :

| Propriété       | Description                                                                                                                                                                                                  |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `client_id`     | L’ID client de l’application Auth0 si l’utilisateur s’est inscrit au moyen de Universal Login, ou la clé API si l’utilisateur a été créé dans l’Auth0 Dashboard ou par la Management API.                    |
...
| `username`      | Le nom d’utilisateur. Obligatoire seulement si le paramètre [**Nécessite un nom d’utilisateur**](/docs/fr-CA/authenticate/database-connections/require-username) est activé pour la connexion de base de données personnalisée. |
| `connection`    | Le nom de la connexion Auth0.                                                                                                                                                                                |
| `user_metadata` | Contient les propriétés de l’objet `user_metadata` du profil Auth0 de l’utilisateur, si cet objet existe.                                                                                                    |
| `app_metadata`  | Contient les propriétés de l’objet `app_metadata` du profil Auth0 de l’utilisateur, si cet objet existe.                                                                                                     |

<div id="username-property">
  ### Propriété username
</div>

Si le paramètre [Nécessite un nom d’utilisateur](/docs/fr-CA/authenticate/database-connections/require-username) est activé pour votre connexion de base de données personnalisée, les scripts [Login](/docs/fr-CA/authenticate/database-connections/custom-db/templates/login) et [Get User](/docs/fr-CA/authenticate/database-connections/custom-db/templates/get-user) doivent prendre en charge la propriété `username`; vous devriez donc la stocker dans votre base de données externe.

<div id="user-and-app-metadata">
  ### Métadonnées utilisateur et d’application
</div>

Les propriétés `user_metadata` et `app_metadata` n’ont pas besoin d’être stockées dans votre base de données externe. Auth0 stocke automatiquement ces valeurs dans le profil utilisateur qu’il crée à l’interne.

<div id="custom-sign-up-fields">
  ### Champs personnalisés à l’inscription
</div>

Si vous [créez et utilisez des champs personnalisés](/docs/fr-CA/libraries/custom-signup#using-the-api) pendant l’inscription, ils seront inclus dans l’objet `user`.
...
Si vous ne parvenez pas à créer un utilisateur dans votre ancienne base de données ou dans Auth0 :

1. Vérifiez les appels `console.log()` dans les [journaux en temps réel d’Actions](/docs/fr-CA/customize/actions/actions-real-time-logs).
2. Trouvez l’utilisateur dans votre ancienne base de données et supprimez-le au besoin. Si un état utilisateur partiel se trouve dans Auth0, utilisez le point de terminaison [Supprimer un utilisateur](https://auth0.com/docs/api/management/v2#!/Users/delete_users_by_id) ou [Supprimer un utilisateur d’une connexion](https://auth0.com/docs/api/management/v2#!/Connections/delete_users_by_email) de la Management API.
