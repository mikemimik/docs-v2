...
Ce script est requis à la fois pour l’authentification héritée et pour la [migration automatique](/docs/fr-CA/manage-users/user-migration/configure-automatic-migration-from-your-database). Si la migration automatique est configurée pour cette connexion, le processus de migration se déclenche après la première connexion réussie de l’utilisateur. Auth0 recommande de définir un `user_id` permanent dans le profil utilisateur renvoyé afin d’éviter la création d’utilisateurs en double.
...
* Envoyer à l’API de la base de données externe les identifiants fournis par l’utilisateur.
...
<Warning>
  Évitez de journaliser, de stocker ou de transmettre les données d’authentification du mot de passe sous forme non chiffrée.
</Warning>
...
Si une erreur se produit, le paramètre `error` doit contenir les renseignements pertinents sur la nature du problème.

<div id="wrongusernameorpassworderror-type-object">
  #### Objet de type WrongUsernameOrPasswordError
</div>

L’objet d’erreur personnalisée `WrongUsernameOrPasswordError` vous permet de transmettre des données qui s’afficheront dans vos [journaux du tenant](/docs/fr-CA/deploy-monitor/logs).
...
| Paramètre         | Type   | Obligatoire | Description                                                                |
| ----------------- | ------ | ----------- | -------------------------------------------------------------------------- |
| `userNameOrEmail` | String | Obligatoire | Contient le nom d’utilisateur ou l’adresse courriel, ou une valeur `null`. |
...
Si vous retournez une erreur avec la valeur `null` dans le champ `userNameOrEmail`, Auth0 consignera un événement `fu` dans le journal du tenant.
...
| ----------------------------------------- | ---------------------------------------------------------------- |
| **Code**                                  | `fu`                                                             |
| **Événement**                             | Échec de connexion (adresse courriel/nom d’utilisateur invalide) |
| **Description**                           | `Mon message d'erreur personnalisé`                              |
...
Si vous n’activez pas ce paramètre, les valeurs renvoyées par la base de données externe lors de la première connexion de l’utilisateur continueront d’être utilisées lors des connexions suivantes, même si elles ont changé dans la base de données externe.
...
function login(email, password, callback) {
  ...
  // Il est exécuté lorsqu'un utilisateur tente de se connecter ou immédiatement après
  // son inscription (à titre de vérification que l'inscription a réussi).
  //
...
function login(email, password, callback) {
   ...
   * du fournisseur d'adhésion de l'utilisateur, les hachages de mot de passe et le sel
   *
   ...
   *                                    et le deuxième argument sera un objet `user`
   */
...
async function loginAsync(email, password, callback) {
  ...
  try {
    response = await axios.post(
      ...
      //les identifiants de l'utilisateur sont transmises dans le corps de la requête
      {
        email: email,
        password: password,
      },
  ...
  try {
    ...
    //cela garantit l'unicité des identifiants utilisateur dans toutes les connexions de base de données
    return callback(null, {
...
function login(username, password, callback) {
  ...
  // Ajoutez votre ID client et Secret d'API Stormpath
  var apiCredentials = {
