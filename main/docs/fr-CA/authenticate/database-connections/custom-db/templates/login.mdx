...
* Renvoyer les données de profil de l’utilisateur si l’authentification réussit.
...
| ----------------- | -------- | ------------------------------------------------------------------------ |
| `userNameOrEmail` | String   | Le nom d’utilisateur ou l’adresse courriel.                              |
...
Voici un exemple en pseudo-JavaScript montrant comment vous pourriez implémenter la fonction `login`. Pour des exemples par langage, consultez [Exemples de scripts par langage](#language-specific-script-examples).
...
<Warning>
  Évitez de journaliser, de stocker ou de transmettre l’information d’authentification du mot de passe sous une forme non chiffrée.
</Warning>

Chiffrez la valeur `password` à l’aide d’une bibliothèque de hachage cryptographique comme `bcrypt` afin d’éviter toute fuite potentielle de données.
...
La fonction `callback` sert à transmettre des données de profil utilisateur ou des données d’erreur dans le pipeline.
...
L’objet d’erreur personnalisée `WrongUsernameOrPasswordError` vous permet de transmettre des données qui s’afficheront dans vos [logs du tenant](/docs/fr-CA/deploy-monitor/logs).
...
Si vous renvoyez une erreur contenant une valeur pour le champ `userNameOrEmail`, Auth0 consignera un événement `fp` dans le journal du tenant.
...
<div id="return-error-without-username-or-email">
  #### Retourner une erreur sans nom d’utilisateur ou adresse courriel
</div>

Si vous retournez une erreur avec une valeur `null` dans le champ `userNameOrEmail`, Auth0 consignera un événement `fu` dans le journal du tenant.
...
Si vous n’activez pas ce paramètre, les valeurs renvoyées par la base de données externe lors de la première connexion de l’utilisateur resteront enregistrées lors des connexions suivantes, même si elles ont changé dans la base de données externe.
...
function login(email, password, callback) {
  ...
  // Il est exécuté lorsqu’un utilisateur tente de se connecter ou immédiatement après son inscription
  // (pour vérifier que l’utilisateur a bien été inscrit).
  //
  // Tout ce que ce script renvoie sera ajouté au profil utilisateur
  ...
  // 1. Les identifiants de l’utilisateur sont valides. Le profil utilisateur renvoyé doit respecter
...
function login(email, password, callback) {
  ...
  // Mise en œuvre du Membership Provider utilisée avec le NuGet Microsoft.AspNet.Providers
  /**
   * getMembershipUser
   *
...
function login(email, password, callback) {
   ...
   * et le salt du mot de passe original depuis la base de données, et effectue un hachage HMAC SHA256.
   *
   * @password      {[string]}      le mot de passe saisi par l'utilisateur
   * @originalHash  {[string]}      le mot de passe original haché depuis la base de données
   *                                (incluant le salt).
   ...
   * getMembershipUser
   *
   * Cette fonction reçoit un nom d'utilisateur ou une adresse courriel et retourne les informations de l'utilisateur, les hachages de mot de passe et le salt
   *
...
async function loginAsync(email, password, callback) {
  ...
  try {
    ...
    if (e.response.status === 404) {
      //on suppose que l'API retourne 404 lorsque l'adresse courriel, le nom d'utilisateur ou le mot de passe est invalide
      return callback(
  ...
  request({
    ...
    request({
      ...
      return callback(null, {
        ...
        // était valide dans Stormpath, il a déjà vérifié son courriel
