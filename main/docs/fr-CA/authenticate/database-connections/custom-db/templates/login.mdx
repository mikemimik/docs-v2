---
description: Décrit les modèles de scripts d’action de base de données personnalisée pour la connexion des utilisateurs.
title: Modèles de script Login
---
...
Ce script est requis à la fois pour l’authentification héritée et pour la [migration automatique](/docs/fr-CA/manage-users/user-migration/configure-automatic-migration-from-your-database). Si la migration automatique est configurée pour la connexion, le processus de migration se déclenche après la première connexion réussie de l’utilisateur. Auth0 recommande de définir un `user_id` permanent dans le profil utilisateur renvoyé afin d’éviter la création d’utilisateurs en double.
...
* Envoyer les identifiants fournis par l’utilisateur à l’API de la base de données externe.
...
| `password`        | String   | Le mot de passe de l’utilisateur en clair.                                |
| `callback`        | Function | Sert à transmettre les erreurs ou les données de profil dans le pipeline. |

### Exemple

Voici un pseudo-exemple en JavaScript montrant comment vous pourriez implémenter la fonction `login`. Pour des exemples propres à chaque langage, consultez [Exemples de scripts propres à chaque langage](#language-specific-script-examples).
...
<Warning>
  Évitez de consigner, de stocker ou de transporter un identifiant par mot de passe sous une forme non chiffrée, où que ce soit.
</Warning>

Chiffrez la valeur `password` à l’aide d’une bibliothèque de hachage cryptographique comme `bcrypt` afin d’éviter toute fuite de données.
...
<div id="return-the-user-profile">
  ### Renvoyer le profil utilisateur
</div>

<Warning>
  Les données du profil renvoyées par le script Login pour un utilisateur doivent correspondre aux données du profil renvoyées par le script Get User.
</Warning>

Si l’utilisateur s’authentifie avec succès, les données de son profil doivent être renvoyées dans l’objet `profile`, sous une [forme normalisée](/docs/fr-CA/manage-users/user-accounts/user-profiles/normalized-user-profile-schema). En plus des champs standard, vous pouvez inclure les champs `user_metadata`, `app_metadata` et `mfa_factors`.
...
L’objet d’erreur personnalisé `WrongUsernameOrPasswordError` vous permet de transmettre des données qui s’afficheront dans vos [journaux du tenant](/docs/fr-CA/deploy-monitor/logs).

<div id="constructor">
  ##### Constructeur
</div>

Le constructeur `WrongUsernameOrPasswordError` prend jusqu’à deux paramètres :
...
| Paramètre         | Type   | Obligatoire | Description                                                                                |
| ----------------- | ------ | ----------- | ------------------------------------------------------------------------------------------ |
| `userNameOrEmail` | String | Obligatoire | Contient le nom d’utilisateur ou l’adresse courriel de l’utilisateur, ou la valeur `null`. |
| `message`         | String | Facultatif  | Contient des renseignements sur l’erreur.                                                  |

<div id="return-error-with-username-or-email">
  #### Retourner une erreur avec un nom d’utilisateur ou une adresse courriel
</div>

Si vous retournez une erreur en fournissant une valeur pour le champ `userNameOrEmail`, Auth0 consignera un événement `fp` dans le journal du tenant.
...
Si vous retournez une erreur avec une valeur `null` pour le champ `userNameOrEmail`, Auth0 consignera un événement du journal du tenant `fu`.
...
| Champ de l’événement dans le journal du tenant | Valeur                                                      |
| ---------------------------------------------- | ----------------------------------------------------------- |
| **Code**                                       | `fu`                                                        |
...
Si vous n’activez pas ce paramètre, les valeurs renvoyées par la base de données externe lors de la première connexion de l’utilisateur seront conservées lors des connexions suivantes, même si elles ont changé dans la base de données externe.
...
function login(email, password, callback) {
  // Ce script doit authentifier un utilisateur par rapport aux identifiants stockés dans
  // votre base de données.
  // Il est exécuté lorsqu'un utilisateur tente de se connecter ou immédiatement après son
  // inscription (comme vérification que l'utilisateur s'est bien inscrit).
  //
  // Tout ce que ce script retourne sera inclus dans le profil de l'utilisateur
  // et sera visible par tous les administrateurs du tenant. Évitez d'ajouter des attributs
  ...
  // 1. Les identifiants de l'utilisateur sont valides. Le profil utilisateur retourné doit être dans
  // le format suivant : https://auth0.com/docs/users/normalized/auth0/normalized-user-profile-schema
  //     var profile = {
  ...
  //
  // La liste des modules Node.js pouvant être référencés est disponible ici :
  //
...
function login(email, password, callback) {
  ...
  function hashPassword(password, salt) {
    // l'implémentation par défaut utilise HMACSHA256; comme la longueur de la clé est de 64
  ...
  connection.on('debug', function(text) {
    ...
    // affiche les erreurs survenant lors de la connexion à la base de données SQL ou dans les instructions SQL
  ...
  // Implémentation du Membership Provider utilisée avec le NuGet Microsoft.AspNet.Providers
  /**
   * getMembershipUser
   *
   ...
   *
   * @usernameOrEmail  {[string]}       le nom d'utilisateur ou le courriel; la méthode effectuera
   *                                    une requête sur les deux avec un OR
   *
...
function login(email, password, callback) {
   ...
   *
   * @password      {[string]}      le mot de passe saisi par l'utilisateur
   * @originalHash  {[string]}      le mot de passe original haché provenant de la base de données
   *                                (incluant le salt).
   * @return        {[bool]}        vrai si le mot de passe est validé
   */
...
async function loginAsync(email, password, callback) {
  ...
  try {
    ...
    if (e.response.status === 404) {
      //en supposant que l'API retourne 404 lorsque l'adresse courriel, le nom d'utilisateur ou le mot de passe est invalide
      return callback(
  ...
  try {
    ...
    //si vous utilisez plusieurs connexions de base de données personnalisées dans votre tenant, préfixez
    //le user_id avec une clé propre à la connexion, ex. : "connName|" + user.user_id
