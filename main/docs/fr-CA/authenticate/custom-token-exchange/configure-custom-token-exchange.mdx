...
Avant de configurer l’Échange de jetons personnalisé, assurez-vous que votre application remplit les conditions préalables suivantes :
...
<div id="enable-custom-token-exchange-for-your-application">
  ## Activer échange de jetons personnalisé pour votre application
</div>

Pour activer échange de jetons personnalisé, [créez une nouvelle application](/docs/fr-CA/get-started/auth0-overview/create-applications) ou mettez à jour une application existante dans l’Auth0 Dashboard ou à l’aide de la Management API. Vous pouvez créer plusieurs applications pour utiliser échange de jetons personnalisé.

Lorsque vous créez une nouvelle application :

1. Par défaut, échange de jetons personnalisé est désactivé. Pour activer échange de jetons personnalisé, utilisez la Management API pour effectuer un appel `POST` à [Create a Client](https://auth0.com/docs/api/management/v2/clients/post-clients) ou un appel `PATCH` à [Update a Client](https://auth0.com/docs/api/management/v2/clients/patch-clients-by-id). Définissez l’attribut `allow_any_profile_of_type` dans `token_exchange` sur `["custom_authentication"]` :
...
2. Activez la connexion que vous souhaitez utiliser avec Échange de jetons personnalisé pour l’application.
3. Assurez-vous que votre application est indiquée comme [de première partie](/docs/fr-CA/get-started/applications/first-party-and-third-party-applications) et qu’elle est configurée pour être conforme à OIDC dans **[Dashboard &gt; Applications &gt; Advanced Settings &gt; OAuth](/docs/fr-CA/get-started/applications/application-settings#oauth)**.
...
Une fois l’application créée, prenez note du `client_id` et du `client_secret` pour usage ultérieur lors de l’appel du endpoint `/oauth/token`.

<div id="configure-custom-token-exchange-profile">
  ## Configurer le profil d’échange de jetons personnalisé
</div>

Chaque profil d’échange de jetons personnalisé établit une correspondance un à un entre un `subject_token_type` et une Action, qui contient la logique de code pour un cas d&#39;utilisation précis.

Les requêtes échange de jetons personnalisé envoyées à l&#39;endpoint `/oauth/token` avec une valeur `subject_token_type` précise seront associées au profil de jeton personnalisé correspondant et acheminées vers l&#39;Action liée pour traitement.
...
* Décode et valide le `subject_token` en fonction du `subject_token_type`. Vous obtiendrez ainsi des renseignements sur l’utilisateur pour la transaction.
* Applique toute politique d’autorisation nécessaire à la transaction.

Une fois que vous êtes certain que la transaction peut se poursuivre, définissez l’utilisateur. Auth0 émettra ensuite des jetons d’accès, d’ID et d’actualisation pour cet utilisateur comme forme d’authentification de l’utilisateur.

Pour voir des exemples d’Actions de échange de jetons personnalisé, consultez [Exemples de cas d’utilisation et exemples de code](/docs/fr-CA/authenticate/custom-token-exchange/cte-example-use-cases).
...
Après avoir déployé l’Action, copiez l’ID de l’Action qu’Auth0 lui a attribué. Vous devez encore ajouter votre logique personnalisée à l’Action. Commencez par obtenir l’ID de l’Action pour créer le profil d’échange de jetons personnalisé.

6. Pour obtenir l’ID de l’Action dans l’Auth0 Dashboard, consultez l’URL affichée dans la fenêtre du navigateur. L’ID de l’Action devrait correspondre à la dernière partie de l’URL, comme illustré dans l’image suivante :
...
Vous pouvez aussi obtenir l’ID de l’Action au moyen de la Management API. Commencez par obtenir un Management API Token pour appeler l’API. Ensuite, effectuez la requête `GET` suivante vers l’endpoint `/actions` :
...
Vous devriez recevoir l’ID de l’Action dans le corps de la réponse, à `actions[0].id.` Vous avez besoin de l’ID de l’Action pour créer le profil d’échange de jetons personnalisé.

<div id="create-custom-token-exchange-profile">
  ### Créer un profil d’échange de jetons personnalisé
</div>

Pour créer un profil d’échange de jetons personnalisé, utilisez la Management API pour envoyer une requête `POST` au point de terminaison `/token-exchange-profiles` avec les paramètres suivants :
...
Si vous avez créé avec succès un profil d’échange de jetons personnalisé, vous devriez recevoir une réponse semblable à ce qui suit :
...
Pour supprimer un profil d’échange de jetons personnalisé, effectuez la requête `DELETE` suivante à l’endpoint `/token-exchange-profiles` :
