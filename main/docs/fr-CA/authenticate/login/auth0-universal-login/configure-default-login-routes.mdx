...
Dans certains cas (décrits ci-dessous), Auth0 შეიძლება devoir rediriger vers le point de terminaison Login Initiation de l&#39;application, en utilisant l’ouverture de session OIDC initiée par un tiers. Pour en savoir plus, consultez [Initiating Login from a Third Party](https://openid.net/specs/openid-connect-core-1_0.html#ThirdPartyInitiatedLogin) sur le site de la [OpenID Foundation](https://openid.net).

Vous pouvez configurer ces URI dans le Dashboard, sous [Application Settings](https://manage.auth0.com/#/applications/settings) ou [Tenant Advanced Settings](https://manage.auth0.com/#/tenant/advanced), ou au moyen de la <Tooltip tip="Management API : un produit qui permet aux clients d’effectuer des tâches administratives." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Management+API">Management API</Tooltip>.
...
Conformément à la spécification OIDC Third Party Initiated Login, le paramètre `iss` contenant l’identifiant de l’émetteur sera ajouté à `login_url` en tant que paramètre de la chaîne de requête avant la redirection.

<div id="dynamic-login-uris-with-metadata-placeholders">
  ## URI de connexion dynamiques avec des espaces réservés de métadonnées
</div>

Si vous utilisez [Multiple Custom Domains](/docs/fr-CA/customize/custom-domains/multiple-custom-domains) ou [Organizations](/docs/fr-CA/manage-users/organizations/organizations-overview), vous pouvez configurer le `initiate_login_uri` au niveau de l’application pour y inclure des espaces réservés de métadonnées. Ces espaces réservés de métadonnées sont remplacés dynamiquement à l’exécution.
...
<Note>
  Les espaces réservés de métadonnées sont pris en charge uniquement dans `initiate_login_uri` au niveau de l’application, et non dans `default_redirection_uri` au niveau du tenant.
</Note>
...
Auth0 utilise le `default_redirection_uri` au niveau du tenant si un espace réservé ne peut pas être résolu. Par exemple, si l’organisation est inconnue ou si la clé de métadonnée n’existe pas. Si le `default_redirection_uri` n’est pas non plus configuré, Auth0 affiche une page d’erreur. Nous recommandons de configurer un `default_redirection_uri` au niveau du tenant comme solution de repli fiable.
...
<div id="users-bookmark-login-page">
  ### Les utilisateurs mettent en signet la page de connexion
</div>
...
Si vous utilisez Organizations et que l’utilisateur final met en signet l’invite de connexion de l’organisation, Auth0 inclut aussi le paramètre `organization` lorsqu’il redirige l’utilisateur vers la route de connexion par défaut.

Il arrive que des utilisateurs mettent en signet la page de connexion et que, lorsqu’ils accèdent à l’URL `/login` enregistrée dans leurs signets, l’enregistrement de transaction n’y soit plus; Auth0 ne peut alors pas poursuivre le flux de connexion. Dans ce cas, Auth0 redirige vers l’URL par défaut du client si elle est configurée, ou vers l’URL au niveau du tenant dans le cas contraire. Si aucune URL de connexion par défaut n’est définie, Auth0 affichera une page d’erreur.

<div id="complete-password-reset-flow">
  ### Processus complet de réinitialisation du mot de passe
</div>

Après avoir terminé le processus de réinitialisation du mot de passe, si l’URI par défaut de l’application ou du tenant est configuré, les utilisateurs verront un bouton leur permettant de retourner à la page de connexion.

Ce comportement se produit uniquement lorsque vous activez l’expérience <Tooltip tip="Universal Login : votre application redirige vers Universal Login, hébergé sur l’Authorization Server d’Auth0, afin de vérifier l’identité d’un utilisateur." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Universal+Login">Universal Login</Tooltip>. Avec Classic Login, vous devez configurer l’URL de redirection dans le modèle Change Password. Pour en savoir plus, consultez [Personnaliser les modèles d’e-mails](/docs/fr-CA/customize/email/email-templates).

Pour les tenants qui utilisent Universal Login, le point de terminaison [`/post-password-change`](https://auth0.com/docs/api/management/v2/#!/Tickets/post_password_change) permet de rediriger les utilisateurs vers une application précise. Lorsque `client_id` est spécifié et que l’URI de connexion de l’application est définie, les utilisateurs verront un bouton les ramenant vers l’application après avoir terminé la réinitialisation du mot de passe.
...
Lorsque des utilisateurs sont invités à se joindre à une [Organisation](/docs/fr-CA/manage-users/organizations/organizations-overview), ils reçoivent un lien d’invitation par courriel. S’ils cliquent sur le lien, ils sont redirigés vers la route de connexion par défaut configurée, avec des paramètres propres à l’invitation ajoutés.

Par exemple, si vous avez une application compatible avec les organisations et qu’un **URI de connexion de l’application** est défini comme `https://myapp.com/login`, le lien envoyé dans le courriel d’invitation que reçoit un utilisateur final sera : `https://myapp.com/login?invitation={invitation_ticket_id}&organization={organization_id}&organization_name={organization_name}`.

Ainsi, la route de votre application doit accepter les paramètres `invitation` et `organization` dans la chaîne de requête. Pour lancer la transaction d’acceptation de l’invitation, elle doit transmettre ces deux paramètres ainsi que l’utilisateur final à votre point de terminaison Auth0 `/authorize`.

<div id="disabled-cookies">
  ### Cookies désactivés
</div>

Si un utilisateur accède à `https://{yourDomain}/authorize` alors que les cookies sont désactivés dans son navigateur, Auth0 redirige l’utilisateur vers l’URI de connexion de l’application. Si l’URI de connexion de l’application n’est pas définie, la redirection est alors envoyée vers l’URI de connexion du tenant.

Renvoyer l’utilisateur vers la page de connexion peut entraîner une boucle de redirection. Pour éviter ce problème, utilisez une page de destination pour vérifier si les cookies sont disponibles ; s’ils sont désactivés, avertissez l’utilisateur qu’il doit les activer s’il souhaite continuer.
