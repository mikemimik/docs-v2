...
Dans certains cas (décrits ci-dessous), Auth0 შეიძლება devoir rediriger vers l’point de terminaison Login Initiation de l’application, en utilisant un login initié par un tiers OIDC. Pour en savoir plus, consultez [Initiating Login from a Third Party](https://openid.net/specs/openid-connect-core-1_0.html#ThirdPartyInitiatedLogin) sur le site de la [OpenID Foundation](https://openid.net).
...
Le `login_url` doit pointer vers une route de l’application qui redirige ultimement vers le point de terminaison `/authorize` d’Auth0, par ex. `https://mycompany.org/login`. Notez qu’il doit utiliser `https` et qu’il ne peut pas pointer vers `localhost`. `login_url` peut inclure des paramètres de requête et un fragment URI.

Conformément à la spécification OIDC Third Party Initiated Login, le paramètre `iss` contenant l’identifiant de l’émetteur sera ajouté à `login_url` sous forme de paramètre de la chaîne de requête avant la redirection.

<div id="dynamic-login-uris-with-metadata-placeholders">
  ## URI de connexion dynamiques avec des espaces réservés de métadonnées
</div>

Si vous utilisez [Multiple Custom Domains](/docs/fr-CA/customize/custom-domains/multiple-custom-domains) ou [Organizations](/docs/fr-CA/manage-users/organizations/organizations-overview), vous pouvez configurer l’`initiate_login_uri` au niveau de l’application pour y inclure des espaces réservés de métadonnées. Ces espaces réservés de métadonnées sont résolus dynamiquement à l’exécution.

<div id="supported-placeholders">
  ### Espaces réservés pris en charge
</div>

| Espace réservé                 | Origine                                                     | Exemple                                                   |
| ------------------------------ | ----------------------------------------------------------- | --------------------------------------------------------- |
...
Les clés de métadonnées doivent commencer par `public_` (par exemple, `public_app_host`). Les deux types d’espaces réservés peuvent être combinés dans le même URI si la requête comprend à la fois un domaine personnalisé et un contexte d’organisation.
...
<div id="fallback-behavior">
  ### Comportement de solution de secours
</div>

Auth0 utilise le `default_redirection_uri` à l’échelle du tenant comme solution de secours si un espace réservé ne peut pas être résolu. Par exemple, si l’organisation est inconnue ou si la clé de métadonnées n’existe pas. Si le `default_redirection_uri` n’est pas configuré non plus, Auth0 affiche une page d’erreur. Nous recommandons de configurer un `default_redirection_uri` à l’échelle du tenant comme solution de secours fiable.
...
<div id="users-bookmark-login-page">
  ### Les utilisateurs mettent la page de connexion dans leurs favoris
</div>
...
Si vous utilisez Organizations et que l&#39;utilisateur final met l&#39;invite de connexion de l&#39;organisation dans ses favoris, Auth0 inclut aussi le paramètre `organization` lorsqu&#39;il redirige l&#39;utilisateur vers la route de connexion par défaut.

Il arrive que des utilisateurs mettent la page de connexion dans leurs favoris et que, lorsqu&#39;ils accèdent à l&#39;URL `/login` enregistrée, l&#39;enregistrement de transaction n&#39;existe plus et qu&#39;Auth0 ne puisse pas poursuivre le flux de connexion. Dans ce cas, Auth0 redirige vers l&#39;URL client par défaut si elle est configurée, ou vers l&#39;URL au niveau du tenant dans le cas contraire. Si aucune URL de connexion par défaut n&#39;est définie, Auth0 affiche une page d&#39;erreur.

<div id="complete-password-reset-flow">
  ### Flux complet de réinitialisation du mot de passe
</div>

Après avoir terminé le flux de réinitialisation du mot de passe, si l’URI par défaut de l’application ou du tenant est configuré, les utilisateurs verront un bouton leur permettant de retourner à la page de connexion.
...
Pour les tenants qui utilisent Universal Login, le point de terminaison [`/post-password-change`](https://auth0.com/docs/api/management/v2/#!/Tickets/post_password_change) permet de rediriger les utilisateurs vers une application précise. Lorsque `client_id` est spécifié et que l’URI de connexion de l’application est définie, les utilisateurs verront un bouton pour retourner à l’application après avoir réinitialisé leur mot de passe.
...
Par exemple, si vous avez une application pour laquelle les organisations sont activées et qu’un **URI de connexion de l’application** est défini à `https://myapp.com/login`, le lien envoyé dans le courriel d’invitation que recevra l’utilisateur final sera : `https://myapp.com/login?invitation={invitation_ticket_id}&organization={organization_id}&organization_name={organization_name}`.

La route de votre application doit donc accepter les paramètres `invitation` et `organization` dans la chaîne de requête. Pour démarrer la transaction d’acceptation de l’invitation, elle doit transmettre ces deux paramètres, ainsi que l’utilisateur final, à votre point de terminaison Auth0 `/authorize`.

<div id="disabled-cookies">
  ### Cookies désactivés
</div>

Si un utilisateur accède à `https://{yourDomain}/authorize` alors que les cookies sont désactivés dans son navigateur, Auth0 le redirige vers l’URI de connexion de l’application. Si l’URI de connexion de l’application n’est pas définie, la redirection est envoyée à l’URI de connexion du tenant.

Le renvoi de l’utilisateur vers la page de connexion peut entraîner une boucle de redirection. Pour éviter ce problème, utilisez une page d’atterrissage pour vérifier si les cookies sont disponibles ; s’ils sont désactivés, avertissez l’utilisateur qu’il doit les activer s’il souhaite continuer.
