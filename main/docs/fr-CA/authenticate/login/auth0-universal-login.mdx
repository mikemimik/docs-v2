...
* Tous les écrans du <Tooltip tip="Flux d’autorisation : mécanisme d’autorisation (ou workflow) précisé dans le cadre OAuth 2.0." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=authorization+flow">flux d’autorisation</Tooltip> ont un titre de page clair et distinct.
...
Si vous décidez d’utiliser Classic Login, vous pouvez personnaliser les options de base liées à l’image de marque dans l’Auth0 Dashboard. Vous pouvez ensuite configurer des personnalisations avancées à l’aide du SDK utilisé pour créer votre flux de connexion. Pour en savoir plus, consultez [Customize Classic Login](/docs/fr-CA/customize/login-pages/classic-login).
...
Pour en savoir plus sur la manière de configurer Universal Login pour votre application, consultez les [guides Quickstart](/docs/fr-CA/quickstarts). Choisissez l’approche qui convient le mieux à vos technologies et suivez le Quickstart pour obtenir des instructions pas à pas pour la mise en œuvre.