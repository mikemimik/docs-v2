---
description: Décrit comment configurer l’authentification de l’AD/LDAP Connector avec Kerberos.
title: Configurer l’authentification de l’AD/LDAP Connector avec Kerberos
---

Vous pouvez utiliser la fédération avec Active Directory au moyen de l’AD/LDAP Connector si vous utilisez Classic Login. L’AD/LDAP Connector permet à vos utilisateurs de s’authentifier lorsqu’ils utilisent une machine jointe à un domaine au sein du réseau d’entreprise.

<div id="configure-active-directory">
  ## Configurer Active Directory
</div>

1. Dans [Auth0 Dashboard &gt; Authentication &gt; Enterprise &gt; Active Directory/LDAP](https://manage.auth0.com/#/connections/enterprise/ad), sélectionnez la connexion que vous souhaitez configurer.
...
2. Nous recommandons de redémarrer le service Windows qui héberge l’AD Connector chaque fois que ce paramètre est modifié. Ainsi, les changements prendront effet immédiatement.

<div id="authentication-flow">
  ## Flux d’authentification
</div>

Selon l’emplacement des utilisateurs, le flux d’authentification sera différent lorsque des plages d’adresses IP sont définies. Prenons l’exemple de Fabrikam : puisqu’elle utilise la version SaaS d’Auth0, elle a configuré son adresse IP publique (`24.12.34.56/32`) dans la connexion. Les utilisateurs qui se connectent depuis l’intérieur de l’immeuble proviendront tous de `24.12.34.56` (comme configuré dans la connexion). Lorsqu’ils s’authentifient, ils peuvent suivre le flux natif AD/LDAP et profiter d’une expérience <Tooltip tip="Authentification unique (SSO) : service qui, après qu’un utilisateur s’est connecté à une application, connecte automatiquement cet utilisateur à d’autres applications." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=SSO">SSO</Tooltip> transparente.

Pour que cela fonctionne, le réseau doit permettre aux utilisateurs de se connecter à l’AD/LDAP Connector sur le port configuré dans le fichier `config.json`. Dans les déploiements du Connector à haute disponibilité, l’adresse à laquelle les utilisateurs se connectent est celle du répartiteur de charge réseau placé devant toutes les instances du connecteur.

<Frame>
  ![Configurer l’authentification de l’AD/LDAP Connector avec le schéma du flux Kerberos](/docs/images/cdy7uua7fh8z/1EUZhFx2w5FjDeIoMySERC/fc50869b131faf3257cfe429c96f68d1/2023-09-22_14-28-30.png)
</Frame>
...
À l’inverse, lorsque les utilisateurs ne sont pas sur le réseau d’entreprise (par exemple, chez un client ou en télétravail sans VPN), ils ne pourront pas accéder directement à l’AD/LDAP Connector. Ils devront saisir leur nom d’utilisateur et leur mot de passe, et Auth0 validera ces identifiants auprès de l’AD/LDAP Connector (qui utilisera ensuite Active Directory pour les valider).

<Frame>
  ![Configurer l’authentification de l’AD/LDAP Connector avec le schéma du flux des identifiants Kerberos](/docs/images/cdy7uua7fh8z/7xo1cIdF6GmSowlVB8kjQn/4b55f555977742af3f3df94c5b3a0a9b/2023-09-22_14-29-00.png)
</Frame>
...
Lorsqu’une application utilise Lock 10 ou 11 dans la Login Page hébergée par Auth0 (généralement utilisée pour les protocoles <Tooltip tip="Security Assertion Markup Language (SAML) : protocole normalisé permettant à deux parties d’échanger des renseignements d’authentification sans mot de passe." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=SAML">SAML</Tooltip>/WS-Federation et les intégrations Single Sign-On (SSO)), un bouton permet aux utilisateurs de s’authentifier à l’aide de &quot;Windows Authentication&quot;.

Dans certains cas, il peut être nécessaire de connecter automatiquement l’utilisateur si Kerberos est possible (en fonction de l’adresse IP de l’utilisateur final). Les modifications suivantes peuvent être ajoutées à la Auth0 Login Page pour connecter automatiquement l’utilisateur si Kerberos est possible :
...
2. Supprimez tout message d’avertissement, puis saisissez `negotiate` dans la zone de recherche.
...
1. Configurez un proxy inverse et exposez l’AD/LDAP Connector sur un domaine HTTPS. Vous pouvez utiliser le paramètre `SERVER_URL` (URL visible de l’extérieur) pour publier l’emplacement public où l’AD/LDAP Connector écoutera les requêtes entrantes.
2. Mappez le `SERVER_URL` dans le proxy inverse vers toutes les instances internes des Connectors déployés.