...
4. Ouvrez la connexion en ignorant tout avertissement de certificat affiché par le client Remote Desktop. Si on vous invite à rechercher des PC, des appareils et du contenu sur le réseau local, choisissez **No**.
...
6. Promouvez le serveur en contrôleur de domaine pour le FQDN `mycompany.local`.
...
8. Ajoutez des groupes et des utilisateurs de test. Connectez-vous à la VM à l’aide du client Remote Desktop, puis ouvrez l’invite de commandes PowerShell. Exécutez le script suivant :
...
1. Accédez à [Auth0 Dashboard &gt; Authentication &gt; Enterprise](https://manage.auth0.com/#/connections/enterprise) et créez une nouvelle **connexion** **Active Directory/LDAP** nommée `auth0-test-ad`. Assurez-vous de copier l’**URL du ticket** générée à la fin de ces instructions.
...
4. Suivez les instructions dans le navigateur pour télécharger, installer et configurer le **Connector**. Lorsque le compte de service LDAP vous est demandé, utilisez le compte admin que vous avez créé pour la VM : **Username**: `mycompany\ad-admin,` **Pas**sword: (comme précédemment).
...
7. Ouvrez une session avec l’un des utilisateurs de test créés dans l’annuaire :
