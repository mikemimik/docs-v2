...
* Point de terminaison : `https://{yourDomain}/login/callback` ou `https://<YOUR CUSTOM DOMAIN>/login/callback`, si vous utilisez un [domaine personnalisé](/docs/fr-CA/customize/custom-domains).

<Card title="Federation Metadata">
  Le fichier Federation Metadata contient des renseignements sur les certificats du serveur ADFS. Si le point de terminaison Federation Metadata (`/FederationMetadata/2007-06/FederationMetadata.xml`) est activé dans ADFS, Auth0 peut vérifier périodiquement (une fois par jour) si la configuration a changé, par exemple si un nouveau certificat de signature a été ajouté en prévision d’un remplacement. C’est pourquoi il est préférable d’activer le point de terminaison Federation Metadata plutôt que de fournir un fichier de métadonnées autonome. Si vous fournissez un fichier de métadonnées autonome, nous vous aviserons par courriel lorsque les certificats approcheront de leur date d’expiration.
</Card>

Vous pouvez utiliser un script pour configurer la connexion ou l’établir manuellement.
...
Pour une intégration automatisée, ce script utilise le [composant logiciel enfichable PowerShell d’ADFS](http://technet.microsoft.com/en-us/library/adfs2-powershell-basics.aspx) pour créer et configurer une **<Tooltip tip="Partie de confiance : entité (comme un service ou une application) qui dépend d’un fournisseur d’identité tiers pour authentifier un utilisateur." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Relying+Party">partie de confiance</Tooltip>** qui émettra, pour l’utilisateur authentifié, les claims suivants : **courriel**, **upn**, **prénom** et **nom de famille**.

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Si vous utilisez la fonctionnalité [de domaines personnalisés](/docs/fr-CA/customize/custom-domains), vous devrez remplacer la valeur `$webAppEndpoint` par `https://<YOUR CUSTOM DOMAIN>/login/callback`.
</Callout>
...
<div id="manual-setup-part-1-add-a-relying-party-trust">
  ## Configuration manuelle, partie 1 : ajouter une Relying Party Trust
</div>

1. Ouvrez la console de gestion ADFS.
2. Sur le côté droit de la console, cliquez sur **Add Relying Party Trust***
3. Cliquez sur **Start**.
...
12. Laissez la valeur par défaut `Permit all users...`, puis cliquez sur **Next**.
...
1. Si vous utilisez Windows Server 2019, la boîte de dialogue Edit Claim Issuance Policy s’ouvre automatiquement lorsque vous terminez l’assistant Add Relying Party Trust. Si vous utilisez Windows 2012 ou 2016, suivez les étapes ci-dessous :
...
3. Conservez l’option par défaut `Send LDAP Attributes as Claims`.
4. Donnez à la règle un nom qui décrit ce qu’elle fait.
5. Sous Attribute Store, sélectionnez **Active Directory**.
6. Sélectionnez les mappages ci-dessous sous `Mapping of LDAP attributes to outgoing claim types`, puis cliquez sur **Finish**.
...
3. Si le type de claim que vous recherchez n&#39;existe pas, vous avez deux options :
   ...
   1. Saisissez un [nom qualifié avec un espace de noms](/docs/fr-CA/secure/tokens/json-web-tokens/create-custom-claims) pour le nouveau claim (par exemple, `http://schemas.xmlsoap.org/ws/2005/05/identity/claims/department`).
