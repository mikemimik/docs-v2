...
* Certains IdP ne prennent pas en charge les paramètres en amont. Vérifiez auprès de l’IdP concerné avant de procéder à votre mise en œuvre.
* Les IdP <Tooltip tip="Security Assertion Markup Language (SAML) : protocole normalisé permettant à deux parties d’échanger des renseignements d’authentification sans mot de passe." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=SAML">SAML</Tooltip> ne prennent pas en charge les paramètres en amont.
...
Appelez le point de terminaison [Get a connection](https://auth0.com/docs/api/management/v2#!/Connections/get_connections_by_id) de la Management API pour récupérer les valeurs existantes de l’objet `options` :
...
Copiez l’objet `options` existant, puis ajoutez l’objet `upstream_params` avec le champ `blog` en tant qu’attribut :
...
Appelez le point de terminaison [Mettre à jour une connexion](https://auth0.com/docs/api/management/v2/#!/Connections/patch_connections_by_id) de la Management API en plaçant l’objet `options` dans le corps :
...
Utilisez des paramètres dynamiques pour configurer votre connexion afin d’envoyer à l’IdP, lorsque l’utilisateur se connecte, un ensemble de paramètres dont les valeurs lui sont propres.

Pour configurer des paramètres dynamiques, appelez le point de terminaison [Create a connection](https://auth0.com/docs/api/management/v2/#!/Connections/post_connections) ou [mettre à jour une connexion](https://auth0.com/docs/api/management/v2/#!/Connections/patch_connections_by_id) de l’Auth0 Management API, transmettez l’objet `upstream_params` dans l’objet `options` avec les paramètres que vous souhaitez envoyer à l’IdP, et indiquez, à l’aide de l’attribut `alias`, le champ auquel le paramètre est mappé.
...
X vous permet de transmettre un paramètre facultatif `screen_name` à son point de terminaison d’autorisation OAuth. Le paramètre `screen_name` préremplit le champ du nom d’utilisateur sur l’écran de connexion avec la valeur fournie. Pour en savoir plus, consultez la [documentation de référence de l’API de X](https://docs.x.com/fundamentals/authentication/api-reference#get-oauth%2Fauthorize).

Pour suivre cet exemple, vous aurez besoin d’une [connexion sociale X](https://marketplace.auth0.com/integrations/twitter-social-connection) active.

#### Obtenir la connexion

Appelez l’endpoint de la Management API [Get a connection](https://auth0.com/docs/api/management/v2#!/Connections/get_connections_by_id) pour récupérer les valeurs actuelles de l’objet `options` :
...
Copiez l’objet `options` existant, ajoutez l’objet `upstream_params` avec le champ `screen_name` comme attribut, puis attribuez à l’attribut `alias` la valeur `login_hint` :
...
Lorsque vous appelez le [point de terminaison de connexion](https://auth0.com/docs/api/authentication#login) de l’Authentication API pour un utilisateur, vous pouvez transmettre son adresse courriel dans le paramètre `login_hint` :
