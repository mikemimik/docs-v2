...
* Les IdPs ne prennent pas tous en charge les paramètres transmis en amont. Vérifiez auprès de l’IdP concerné avant de poursuivre votre mise en œuvre.
* Les IdPs <Tooltip tip="Security Assertion Markup Language (SAML) : protocole normalisé permettant à deux parties d’échanger des informations d’authentification sans mot de passe." cta="Consulter le glossaire" href="/docs/fr-CA/glossary?term=SAML">SAML</Tooltip> ne prennent pas en charge les paramètres transmis en amont.

<div id="static-parameters">
  ## Paramètres statiques
</div>

Utilisez des paramètres statiques pour configurer votre connexion afin d’envoyer à l’IdP un ensemble standard de paramètres lorsqu’un utilisateur se connecte.
...
WordPress vous permet de transmettre un paramètre `blog` facultatif à son <Tooltip tip="OAuth 2.0 : framework d’autorisation qui définit les protocoles et les processus d’autorisation." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=OAuth+2.0">OAuth 2.0</Tooltip> point de terminaison d’autorisation, et de demander automatiquement l’accès à un blogue précis pour les utilisateurs lorsqu’ils se connectent. Pour en savoir plus, consultez la [documentation OAuth 2.0 de WordPress](https://developer.wordpress.com/docs/oauth2/).

Pour suivre cet exemple, vous aurez besoin d’une [connexion sociale WordPress](https://marketplace.auth0.com/integrations/wordpress-social-connection) configurée et fonctionnelle.

<div id="get-the-connection">
  #### Récupérer la connexion
</div>

Appelez l’endpoint [Obtenir une connexion](https://auth0.com/docs/api/management/v2#!/Connections/get_connections_by_id) de la Management API pour récupérer les valeurs existantes de l’objet `options` :
...
Appelez le point de terminaison [Mettre à jour une connexion](https://auth0.com/docs/api/management/v2/#!/Connections/patch_connections_by_id) de la Management API en plaçant l’objet `options` dans le corps de la requête :
...
X vous permet de transmettre un paramètre facultatif `screen_name` à son point de terminaison d’autorisation OAuth. Le paramètre `screen_name` préremplit le champ du nom d’utilisateur dans l’écran de connexion avec la valeur fournie. Pour en savoir plus, consultez la [documentation de référence de l’API de X](https://docs.x.com/fundamentals/authentication/api-reference#get-oauth%2Fauthorize).

Pour suivre cet exemple, vous aurez besoin d’une [connexion sociale X active](https://marketplace.auth0.com/integrations/twitter-social-connection).

<div id="get-the-connection">
  #### Récupérer la connexion
</div>

Appelez le endpoint [Obtenir une connexion](https://auth0.com/docs/api/management/v2#!/Connections/get_connections_by_id) de la Management API pour obtenir les valeurs actuelles de l’objet `options` :
...
L’objet `options` ressemblera à quelque chose comme ceci :
...
Copiez l’objet `options` existant, ajoutez l’objet `upstream_params` en utilisant le champ `screen_name` comme attribut, puis définissez l’attribut `alias` sur `login_hint` :
...
<div id="call-the-login-endpoint">
  #### Appeler le point de terminaison Login
</div>

Lorsque vous appelez le point de terminaison [Login](https://auth0.com/docs/api/authentication#login) de l’Authentication API pour un utilisateur, vous pouvez passer son adresse courriel dans le paramètre `login_hint` :
...
Cette valeur sera ensuite transmise au point de terminaison d’autorisation X dans le paramètre `screen_name` :
