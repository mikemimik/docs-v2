---
description: Découvrez comment ajouter n’importe quel fournisseur OAuth2 au moyen des Connexions sociales personnalisées d’Auth0.
...
Les [fournisseurs d&#39;identité (IdP)](/docs/fr-CA/authenticate/identity-providers) les plus courants sont disponibles dans le [Auth0 Dashboard](https://manage.auth0.com/#) et dans le [Auth0 Marketplace](https://marketplace.auth0.com/features/social-connections). Vous pouvez toutefois ajouter n&#39;importe quel fournisseur <Tooltip tip="OAuth 2.0 : Framework d'autorisation qui définit les protocoles et workflows d'autorisation." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=OAuth+2.0">OAuth 2.0</Tooltip> comme **connexion sociale personnalisée** dans le <Tooltip tip="Auth0 Dashboard : Le produit principal d'Auth0 pour configurer vos services." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip>.
...
* **Script de récupération du profil de l’utilisateur** : Script Node.js utilisé pour appeler une URL userinfo avec le jeton d&#39;accès fourni. Pour en savoir plus sur ce script, consultez [Script de récupération du profil de l’utilisateur](#fetch-user-profile-script).
* **Objectif** : Active la connexion sociale pour l&#39;authentification, Connected Accounts for Token Vault, ou les deux. Pour en savoir plus, consultez [User authentication vs Connected Accounts](/docs/fr-CA/secure/tokens/token-vault/connected-accounts-for-token-vault#user-authentication-vs-connected-accounts).

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Lors de la configuration du fournisseur d&#39;identité personnalisé, utilisez l&#39;URL de rappel `https://{yourDomain}/login/callback`.
</Callout>

Une fois la connexion personnalisée créée, la vue **Applications** s&#39;affichera et votre connexion sera assujettie à la [Rate Limit Policy](/docs/fr-CA/troubleshoot/customer-support/operational-policies/rate-limit-policy) d&#39;Auth0. Vous pourrez alors activer ou désactiver les applications pour lesquelles vous souhaitez que la connexion soit affichée.

<div id="update-authentication-flow">
  ## Mettre à jour le flux d’authentification
</div>

Lorsque vous créez une connexion, le type d’autorisation OAuth 2.0 par défaut attribué à la connexion est le flux de code d’autorisation. Si vous avez une application publique qui ne peut pas stocker de Client Secret, comme une application monopage ou une application native, vous pouvez utiliser la <Tooltip tip="Management API : un produit qui permet aux clients d’effectuer des tâches administratives." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Management+API">Management API</Tooltip> pour mettre à jour la connexion afin qu’elle utilise le [flux de code d’autorisation + PKCE](/docs/fr-CA/get-started/authentication-and-authorization-flow/authorization-code-flow-with-pkce). Pour en savoir plus sur les <Tooltip tip="Flux d’autorisation : mécanisme d’autorisation (ou workflow) spécifié dans le framework OAuth 2.0." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=authorization+flows">flux d’autorisation</Tooltip>, consultez [Quel flux OAuth 2.0 devrais-je utiliser ?](/docs/fr-CA/get-started/authentication-and-authorization-flow/which-oauth-2-0-flow-should-i-use)
...
<div id="fetch-user-profile-script">
  ## Script de récupération du profil de l’utilisateur
</div>

Le script de récupération du profil de l’utilisateur est appelé une fois que l’utilisateur s’est connecté auprès du fournisseur OAuth2. Auth0 exécute ce script pour appeler l’API du fournisseur OAuth2 et obtenir le profil de l’utilisateur :
...
Vous pouvez filtrer, ajouter ou supprimer tout élément du profil renvoyé par le fournisseur. Toutefois, il est recommandé de garder ce script aussi simple que possible. Une manipulation plus poussée des données utilisateur peut être effectuée au moyen de [Rules](/docs/fr-CA/customize/rules). L’un des avantages des Rules est qu’elles s’appliquent à toute connexion.

<div id="log-in-using-the-custom-connection">
  ## Se connecter à l’aide de la connexion personnalisée
</div>

Vous pouvez utiliser n’importe lequel des mécanismes standards d’Auth0 pour ouvrir une session pour un utilisateur avec votre connexion personnalisée. Un lien direct ressemblerait à ceci :
...
Dans certaines circonstances, vous pourriez vouloir transmettre une valeur dynamique à un fournisseur d’identité OAuth 2.0. Dans ce cas, vous pouvez utiliser l’élément `authParamsMap` de `options` pour définir un mapping entre l’un des paramètres supplémentaires existants acceptés par le [endpoint Auth0 `/authorize`](https://auth0.com/docs/api/authentication#social) et le paramètre accepté par le fournisseur d’identité.

En reprenant le même exemple ci-dessus, supposons que vous souhaitez transmettre le paramètre `custom_param` au point de terminaison d’autorisation, mais que vous voulez préciser la valeur réelle du paramètre au moment d’appeler l’endpoint Auth0 `/authorize`.
...
Dans certains cas, vous devrez transmettre des en-têtes supplémentaires au <Tooltip tip="Point de terminaison de jeton : point de terminaison du serveur d’autorisation utilisé pour demander des jetons par programmation." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Token+endpoint">point de terminaison de jeton</Tooltip> d’un fournisseur OAuth 2.0. Pour configurer des en-têtes supplémentaires, ouvrez les Settings de la connexion, puis, dans le champ **En-têtes personnalisés**, indiquez un objet JSON contenant les en-têtes personnalisés sous forme de paires clé-valeur :
...
où `[your credentials]` sont les données d’authentification réelles à envoyer au fournisseur d’identité.
