...
<Tabs>
  <Tab title="Dashboard">
    ...
    | URL de connexion                                                    | Saisissez l’**URL de connexion** que vous avez obtenue auprès de l’IdP.                                                                                                                                                                                                                                                                                                                                                         |
    | Certificat de signature X509                                        | Téléversez le fichier de certificat de signature X509 (au format `.pem` ou `.cer`) que vous avez obtenu auprès de l’IdP.                                                                                                                                                                                                                                                                                                        |
    | Activer la déconnexion                                              | Activez le champ **URL de déconnexion**.                                                                                                                                                                                                                                                                                                                                                                                        |
    | URL de déconnexion                                                  | Saisissez l’**URL de déconnexion** obtenue auprès de l’IdP.                                                                                                                                                                                                                                                                                                                                                                     |
    | Attribut d’ID utilisateur                                           | Saisissez l’attribut du token SAML qui sera mappé à la propriété `user_id` dans Auth0. S’il n’est pas défini, `user_id` sera récupéré à partir des attributs suivants (dans l’ordre indiqué) :<ul><li>`http://schemas.xmlsoap.org/ws/2005/05/identity/claims/nameidentifier`</li><li>`http://schemas.xmlsoap.org/ws/2005/05/identity/claims/upn`</li><li>`http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name`</li></ul> |
    | Mode de débogage                                                    | Activez le **mode de débogage** pour une journalisation plus détaillée.                                                                                                                                                                                                                                                                                                                                                         |
    ...
    | Résumé de l’algorithme de signature de la requête                   | Dans le menu déroulant, sélectionnez l’algorithme à utiliser pour vérifier la validité de l’assertion.                                                                                                                                                                                                                                                                                                                          |
  ...
  <Tab title="Management API">
    ...
    | `signingCert`        | String  | Obligatoire   | Contenu du certificat de signature X509 (au format `.pem` ou `.cer`) codé en Base64.                                                                                                                                                                                                                                                                                                                                                                                                   |
    | `debug`              | Booléen | Facultatif    | Active ou désactive le **mode de débogage**, ce qui permet une journalisation plus détaillée pendant le processus d’authentification. Réglez cette valeur à `false` pour les connexions utilisées dans des environnements de production.                                                                                                                                                                                                                                               |
    | `destinationUrl`     | Chaîne  | Facultatif    | L’URL à laquelle Auth0 enverra les requêtes d’authentification SAML. Requise uniquement si vous utilisez une passerelle proxy.                                                                                                                                                                                                                                                                                                                                                         |
    ...
    | `recipientUrl`       | String  | Facultatif    | L’URL à laquelle l’IdP enverra les réponses d’authentification SAML. Requise uniquement en cas d’utilisation d’une passerelle proxy.                                                                                                                                                                                                                                                                                                                                                   |
    ...
    | `signSAMLRequest`    | Booléen | Facultatif    | Permet d’activer ou de désactiver la signature des requêtes d’authentification envoyées par Auth0.                                                                                                                                                                                                                                                                                                                                                                                     |
    | `user_id_attribute`  | String  | Facultatif    | Le nom de l’attribut dans les réponses d’authentification à faire correspondre à la propriété ID utilisateur. Ce champ remplacera tout mappage de la propriété `user_id` dans l’objet `fieldsMap`.                                                                                                                                                                                                                                                                                     |
...
<div id="configure-saml-connection-for-proxy-gateways">
  ### Configurer une connexion SAML pour des passerelles proxy
</div>
...
1. Récupérez la configuration actuelle de votre connexion SAML à l’aide de l’endpoint [Get a connection](https://auth0.com/docs/api/management/v2/connections/get-connections-by-id) de la Management API.
...
4. Appelez l’endpoint [Update a connection](https://auth0.com/docs/api/management/v2/connections/patch-connections-by-id) de la Management API avec l’objet `options` mis à jour au complet dans le corps de la requête.
...
Auth0 prend en charge tous les IdP SAML conformes au protocole SAML 1.1 ou SAML 2.0. Vous trouverez ci-dessous des instructions détaillées pour configurer certains fournisseurs précis.
...
* Effacez l’historique, les cookies et le cache de votre navigateur avant chaque test. Sinon, le navigateur pourrait ne pas prendre en compte les plus récents renseignements de configuration, ou il pourrait contenir des cookies périmés qui nuisent à l’exécution.
