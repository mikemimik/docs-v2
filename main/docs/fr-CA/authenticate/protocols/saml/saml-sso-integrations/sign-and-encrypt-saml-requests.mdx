---
description: Décrit le scénario de configuration particulier pour signer et chiffrer les requêtes SAML
title: Signer et chiffrer les requêtes SAML
---

import {AuthLink} from "/snippets/AuthLink.jsx";

Pour renforcer la sécurité de vos transactions, vous pouvez signer ou chiffrer à la fois vos requêtes et vos réponses à l’aide du protocole <Tooltip tip="Security Assertion Markup Language (SAML) : protocole normalisé permettant à deux parties d’échanger des informations d’authentification sans mot de passe." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=SAML">SAML</Tooltip>. Dans cet article, vous trouverez des configurations pour des scénarios précis, réparties selon deux cas d’utilisation :

* Auth0 comme fournisseur de services SAML (par exemple, une connexion SAML)
* Auth0 comme <Tooltip tip="Identity Provider (IdP) : service qui stocke et gère les identités numériques." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=identity+provider">fournisseur d’identité</Tooltip> SAML (par exemple, une application configurée avec le module complémentaire SAML Web App)
...
4. Téléchargez le certificat sous le commutateur **Sign Request** et remettez-le à l’IdP pour qu’il puisse valider la signature.
...
Pour désactiver l’encodage deflate, vous pouvez faire un [appel PATCH au point de terminaison Update a Connection de la Management API](https://auth0.com/docs/api/management/v2#!/Connections/patch_connections_by_id) et définir l’option `deflate` à `false`.
...
Par défaut, Auth0 utilise la clé privée du tenant pour signer les requêtes SAML (lorsque la bascule **Sign Request** est activée). Vous pouvez également fournir votre propre paire de clés privée et publique pour signer les requêtes provenant d’une connexion particulière.
...
Pour savoir comment obtenir la clé privée et le certificat sous forme de chaîne JSON à utiliser dans le payload, consultez [utiliser des certificats et des clés sous forme de chaînes](/docs/fr-CA/authenticate/protocols/saml/saml-sso-integrations/work-with-certificates-and-keys-as-strings).

<div id="receive-signed-saml-authentication-responses">
  ### Recevoir des réponses SAML d’authentification signées
</div>

Si Auth0 est le **fournisseur de services** SAML, toutes les réponses SAML de votre fournisseur d’identité devraient être signées pour indiquer qu’elles n’ont pas été altérées par un tiers non autorisé.

Vous devrez configurer Auth0 pour valider les signatures des réponses en obtenant un certificat de signature auprès du fournisseur d’identité et en chargeant ce certificat dans votre connexion Auth0 :
...
Auth0 peut accepter une réponse signée pour l’assertion, pour la réponse ou pour les deux.

<div id="receive-encrypted-saml-authentication-assertions">
  ### Recevoir des assertions SAML d’authentification chiffrées
</div>

Si Auth0 est le fournisseur de services SAML, il peut devoir recevoir des assertions chiffrées d’un fournisseur d’identité. Pour ce faire, vous devez fournir à l’IdP le certificat de clé publique du tenant. L’IdP chiffre l’assertion SAML à l’aide de la clé publique et l’envoie à Auth0, qui la déchiffre à l’aide de la clé privée du tenant.
...
Par défaut, Auth0 prend automatiquement en charge les algorithmes répertoriés dans le plus récent [profil d’algorithme](/docs/fr-CA/authenticate/protocols/saml/saml-sso-integrations/algorithm-profiles) pour déchiffrer les assertions SAML.

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Si l’assertion est chiffrée à l’aide d’un algorithme qui ne figure pas dans la liste, Auth0 la rejettera.
</Callout>

Pour préciser un profil différent ou utiliser un algorithme qui ne figure pas dans la liste, vous devez mettre à jour la connexion à l’aide du point de terminaison [Update a Connection](https://auth0.com/docs/api/management/v2/connections/patch-connections-by-id) et modifier la propriété `assertion_decryption_settings`, comme l’illustre l’exemple de charge utile ci-dessous.

Lorsque vous mettez à jour l’objet `options` d’une connexion, la nouvelle configuration remplace l’ensemble de l’objet `options`. Pour conserver les options de connexion existantes, récupérez l’objet `options` actuel et ajoutez-y de nouvelles paires clé-valeur.
...
Comme indiqué ci-dessus, Auth0 utilisera par défaut la paire de clés privée/publique de votre tenant pour gérer le chiffrement. Vous pouvez aussi fournir votre propre paire de clés publique/privée si un scénario avancé l’exige.

Il n’est pas possible de modifier dans l’interface du Dashboard la paire de clés utilisée pour chiffrer et déchiffrer les requêtes de la connexion; vous devrez donc utiliser le point de terminaison [Update a Connection](https://auth0.com/docs/api/management/v2#!/Connections/patch_connections_by_id) de la Management API v2 et ajouter une propriété `decryptionKey` à l’objet `options`, comme illustré dans l’exemple de charge utile ci-dessous.
...
Ce scénario s’applique lorsqu’Auth0 est le fournisseur d’identité SAML d’une application. Dans le Dashboard, cela est représenté par une **Application** pour laquelle le SAML Web App Addon est activé.
...
Par défaut, Auth0 utilisera la paire de clés privée/publique attribuée à votre tenant pour signer les réponses SAML ou les assertions. Dans certains cas très précis, vous pourriez vouloir fournir votre propre paire de clés. Vous pouvez le faire au moyen d’une règle comme celle-ci :
...
exports.onExecutePostLogin = async (event, api) => {
    ...
    // pour les instructions de formatage : vous commencez avec un certificat au format PEM et
...
5. Repérez la clé **`"signingCert"`**. Supprimez sa mise en commentaire (ou ajoutez-la, au besoin), puis attribuez-lui comme valeur le certificat que vous avez téléchargé auprès du fournisseur de services. La configuration devrait ressembler à ceci :
...
Vous devez obtenir le certificat et la clé publique auprès du fournisseur de services. Si vous n’avez reçu que le certificat, vous pouvez en extraire la clé publique à l’aide de `openssl`. En supposant que le fichier de certificat s’appelle `certificate.pem`, vous pouvez exécuter :

`openssl x509 -in certificate.pem -pubkey -noout > public_key.pem`

Une fois que vous avez obtenu les fichiers du certificat et de la clé publique, vous devez [les convertir en chaînes de caractères](/docs/fr-CA/authenticate/protocols/saml/saml-sso-integrations/work-with-certificates-and-keys-as-strings) pour les utiliser dans une Action. L’Action ressemblera à ceci :
...
<Warning>
  Auth0 prévoit de mettre à jour l’algorithme de chiffrement par défaut pour utiliser `aes256-gcm`.
