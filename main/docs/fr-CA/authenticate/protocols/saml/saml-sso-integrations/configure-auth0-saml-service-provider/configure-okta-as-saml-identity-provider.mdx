...
<div id="create-app-integration">
  ### Créer l’intégration d’application
</div>
...
Le flux de connexion est maintenant redirigé vers la page **Sign On** de l’application nouvellement créée.

1. Sélectionnez **View SAML Setup Instructions**.
2. Notez l’**Identity Provider Single Sign-On URL**.
3. Téléchargez le **X.509 Certificate** au format PEM ou CER.
...
Vous pouvez créer une connexion d’entreprise SAML dans le <Tooltip tip="Auth0 Dashboard : le principal produit d’Auth0 pour configurer vos services." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip>.
...
| Certificat de signature X.509 | Certificat de signature avec clé publique du tenant Okta.<br /><br />Téléversez le **certificat X.509** que vous avez téléchargé précédemment.                                             | `myOktaTenantSigningCertificate.pem`                                                                                                |
...
<div id="enable-saml-enterprise-connection-when-not-using-organizations">
  ### Activer la connexion d’entreprise SAML si vous n’utilisez pas Organizations
</div>
...
<Warning>
  Le bouton **Try** fonctionne uniquement pour les utilisateurs connectés à l’Auth0 Dashboard. Vous ne pouvez pas l’utiliser pour un utilisateur anonyme, comme un client.
