...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Si vous avez configuré un [Custom Domain](/docs/fr-CA/customize/custom-domains), vous devez utiliser le CNAME de votre domaine personnalisé au lieu du domaine Auth0. Pour en savoir plus, consultez [Configure Features to Use Custom Domains](/docs/fr-CA/customize/custom-domains/configure-features-to-use-custom-domains#configure-saml-identity-providers).
</Callout>
...
Vous pouvez récupérer la valeur de l’ID d’entité personnalisé à l’aide du point de terminaison Get a Connection :
...
Aussi appelée **liaison de protocole**, elle est envoyée à l’IdP par Auth0. Si possible, définissez la valeur dynamiquement en fonction de `connection.options.protocolBinding` :
...
<div id="saml-response-binding">
  ### liaison de la SAML Response
</div>

Méthode par laquelle Auth0 reçoit le token SAML de l’IdP, définie sur `HTTP-Post`.
...
L’assertion SAML et la réponse SAML peuvent être signées individuellement ou simultanément.
...
Utilisez les liens suivants pour obtenir la clé publique dans les différents formats suivants :
...
Certains fournisseurs d’identité SAML permettent d’importer directement les métadonnées contenant tous les renseignements nécessaires. Vous pouvez accéder aux métadonnées de votre connexion dans Auth0 ici :
