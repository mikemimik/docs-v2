...
2. Désactivez **Sync user profile attributes at each login**, à moins de vouloir [synchroniser des attributs supplémentaires à la connexion](/docs/fr-CA/authenticate/protocols/scim/configure-inbound-scim).
...
5. Passez en revue les **Additional Mappings** pour vous assurer que les attributs SCIM étendus sont mappés à vos attributs Auth0 préférés; consultez le [mappage d’attributs](/docs/fr-CA/authenticate/protocols/scim/configure-inbound-scim#attribute-mapping) pour en savoir plus. Si vous recevez des numéros de téléphone et des adresses postales, notez qu’Okta Workforce les envoie par défaut sous la forme `phoneNumbers[primary eq true].value` et `addresses[primary eq true].*`.
