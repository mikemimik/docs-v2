...
1. Accédez à [Dashboard &gt; Applications &gt; Applications](https://manage.auth0.com/#/applications) et créez une nouvelle application. Donnez-lui un nom évocateur, comme **GitHub**. Vous pouvez choisir n’importe quel type d’application, mais le type **Regular Web App** est recommandé.
...
4. Collez le code suivant dans la zone de texte **Settings**, puis cliquez sur **Debug**.
   ...
   2. Si vous utilisez une seule connexion, vous préférerez peut-être utiliser le `nickname` ou un autre identificateur unique comme **Name Identifier** plus convivial, mais assurez-vous que la propriété choisie est unique.
...
Suivez les instructions dans [Utiliser SAML sur Enterprise Server - Aide GitHub](https://help.github.com/en/enterprise/admin/user-management/using-saml#configuring-saml-settings) pour terminer la configuration dans GitHub Enterprise Cloud. Vous aurez besoin des renseignements suivants :
...
| Certificat public    | Téléchargez-le à partir de `https://{yourDomain}/pem`. Ouvrez le fichier téléchargé dans un éditeur de texte, copiez son contenu et collez-le dans la zone de texte de GitHub. |
