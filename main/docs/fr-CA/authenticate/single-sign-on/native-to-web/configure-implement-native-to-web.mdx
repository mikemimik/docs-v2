...
title: Configurer et mettre en œuvre Native to Web SSO
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  ...
  La prise en charge de Native to Web SSO est disponible dans les outils suivants : [Auth0 Deploy CLI](/docs/fr-CA/deploy-monitor/deploy-cli-tool). [Auth0 Terraform Provider](/docs/fr-CA/deploy-monitor/auth0-terraform-provider) et [Auth0 CLI](https://auth0.github.io/auth0-cli/).
...
* Associer la session à l’appareil d’un utilisateur à l’aide d’une adresse IP ou d’un ASN
...
* Mettez à jour votre application native à l’aide du <Tooltip tip="Auth0 Dashboard : principal produit d’Auth0 pour configurer vos services." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip> :
  ...
  5. Sélectionnez **Require Cascade Revocation** pour révoquer automatiquement la session de l’application web et les jetons qui y sont associés.
...
Native to Web Single SSO offre une expérience utilisateur fluide en assurant le passage des utilisateurs authentifiés de votre application native à votre application web.

Pour ce faire, votre application native doit échanger un jeton d’actualisation contre un jeton de transfert de session et transmettre le jeton de transfert de session, au moyen d’une URL ou d’un cookie, à votre application web afin d’autoriser la session.

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  * Si `allow_refresh_token` est désactivé dans le client, mais que l’application demande `offline_access`, Auth0 n’émet pas de `refresh_token`, mais l’authentification fonctionne quand même.

  * Si la rotation du jeton d’actualisation est activée, Auth0 renvoie un nouveau `refresh_token` dans l’appel d’échange de jeton. L’échange du jeton d’actualisation doit avoir lieu immédiatement avant que votre code ouvre l’application web.
</Callout>
...
Le tenant Auth0 renvoie un session&#95;transfer&#95;token à usage unique et de courte durée (valide pendant 1 minute).
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Si la rotation du jeton d’actualisation est activée, l’échange retournera aussi un jeton d’actualisation.

  Si vous avez demandé un ID Token lors de l’authentification, cet appel retournera aussi un ID Token.
</Callout>

<div id="step-2-send-the-session-transfer-token-through-a-url-or-cookie">
  #### Étape 2 : Envoyer le jeton de transfert de session par URL ou par cookie
</div>
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Lorsque le jeton de transfert de session est envoyé sous forme de cookie, aucune configuration supplémentaire n’est nécessaire, puisque le navigateur envoie le cookie dans la requête vers le point de terminaison `/authorize`.
</Callout>

Implémentez le SSO Native to Web pour application web dans votre application web à l’aide de paramètres d’URL comme suit :
...
Depuis l’URL de connexion de l’application, redirigez vers le point de terminaison `/authorize` lorsque le `session_transfer_token` est transmis comme paramètre d’URL.
...
<div id="option-2-add-the-session-transfer-token-to-web-applications-using-auth0-sdks">
  #### Option 2 : Ajouter le jeton de transfert de session aux applications web à l’aide des SDKs Auth0
</div>

Les SDKs Auth0 ne prennent pas automatiquement en charge Native to Web Single SSO et n’incluent pas le `session_transfer_token` dans la requête envoyée au point de terminaison `/authorize`.

Voici des exemples d’applications web utilisant les SDKs Auth0 pour transmettre le `session_transfer_token` dans la requête envoyée au point de terminaison `/authorize` :

<div id="node-expressjs">
  ##### Node (Express.js)
</div>

Si votre application web utilise [Express.js](https://expressjs.com/) ou le [SDK Auth0 Express](https://github.com/auth0/express-openid-connect), vous pouvez utiliser le code ci-dessous pour que le middleware prenne en charge `session_transfer_token`.
...
// Récupérer le session_transfer_token depuis les paramètres de requête de l'URL (transmis depuis l'application native)
...
Native to Web SSO prend en charge [Organizations](/docs/fr-CA/manage-users/organizations). Lorsqu’un utilisateur s’authentifie avec une organisation dans votre application native, le jeton de transfert de session inclut le contexte de l’organisation.
...
Si l’organisation indiquée dans la requête `/authorize` ne correspond pas à celle du jeton de transfert de session, ce jeton est rejeté et l’utilisateur est redirigé vers la page de connexion afin de s’authentifier de nouveau. Un avertissement est consigné dans les journaux de votre tenant avec la description d’événement &quot;Échec de l’authentification unique : non-correspondance de l’organisation détectée dans le jeton de transfert de session.&quot;

<div id="session-transfer-token-with-actions">
  ## Session Transfer Token avec Actions
</div>

L’utilisation de `session_transfer_token` avec [Actions](/docs/fr-CA/customize/actions) vous permet de configurer des mécanismes de détection et de réponse aux risques après l’authentification afin de mieux protéger les utilisateurs.

À cette fin, l’objet **event.session&#95;transfer&#95;token** de l’Action post-login fournit des renseignements pertinents, notamment un `client_id` unique, le `scope`, des renseignements sur la `request` comme `ip`, `asn`, `user_agent`, ainsi que des renseignements de `geoip` comme `cityName` et `countryCode`, entre autres. Pour en savoir plus, consultez [Actions Triggers: post-login - Event Object](/docs/fr-CA/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-event-object).

Le code Action ci-dessous vous permet de rejeter dynamiquement une transaction selon les renseignements de géolocalisation :

```javascript javascript lines
/**
* Gestionnaire qui sera appelé durant l'exécution d'un flux PostLogin.
*
...
Auth0 rend ces renseignements accessibles dans les Actions Post Login par l’intermédiaire de l’objet `event.session.session_transfer.parent_refresh_token.metadata`. Cela permet une transmission sécurisée et normalisée des métadonnées entre les plateformes.

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  **Exemple de cas d’utilisation :** Une application mobile recueille des données de risque (score de l’appareil, localisation, etc.) et les stocke dans les métadonnées du jeton d’actualisation. Lorsque l’application native lance un flux Native to Web SSO, ces métadonnées sont automatiquement accessibles dans la session Web correspondante via les Actions.
</Callout>
...
Vous pouvez surveiller l’activité de Native to Web SSO en consultant les [journaux du tenant](/docs/fr-CA/deploy-monitor/logs).
...
* `fertft`: Échec de l’échange de jeton d’actualisation. Ce journal correspond à un échange Native to Web SSO lorsque le champ `audience` est `"audience": "urn:$auth0Domain:session_transfer"`

<div id="session-transfer-validation-warning-logs">
  ### Logs d’avertissement liés à la validation du transfert de session
</div>

Auth0 génère des logs d’avertissement (`w`) lorsque la validation du jeton de transfert de session échoue pendant la requête `/authorize`. Ces logs vous aident à résoudre les problèmes liés à Native to Web SSO :
...
| `Single Sign-On failed: Session Transfer Token organization mismatch detected.`                                          | Le paramètre `organization` de la requête `/authorize` ne correspond pas à l’organisation du jeton de transfert de session.                   |
