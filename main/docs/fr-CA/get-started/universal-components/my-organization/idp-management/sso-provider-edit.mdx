...
Le composant `SsoProviderEdit` offre une interface unifiée pour modifier les fournisseurs [Single Sign-On](/docs/fr-CA/authenticate/enterprise-connections/self-service-enterprise-configuration).
...
<Tabs>
  <Tab title="React">
    ...
    Les props obligatoires sont essentielles au fonctionnement du composant. `SsoProviderEdit` requiert l&#39;ID du fournisseur pour charger et modifier le bon fournisseur.
    ...
    * `icon` - composant d’icône personnalisé de Lucide (facultatif, valeur par défaut : ArrowLeft)
    ...
    * `onAfter(provider, result)` - S’exécute une fois le `provider` mis à jour avec succès. Servez-vous-en pour afficher une notification ou faire le suivi de l’événement.
    ...
    ***
    ...
    Le composant `SsoProviderEdit` est composé de sous-composants et de hooks plus petits. Vous pouvez les importer individuellement pour créer des workflows personnalisés d&#39;édition de fournisseurs si vous utilisez shadcn.
  ...
  <Tab title="Next.js">
    ...
    * `onAfter(provider, result)` - S’exécute une fois que le fournisseur a été mis à jour avec succès. Utilisez-le pour afficher une notification ou consigner l’événement.
    ...
    ***
    ...
    Le composant `SsoProviderEdit` est composé de sous-composants et de hooks de plus petite taille. Vous pouvez les importer individuellement pour créer des workflows personnalisés d&#39;édition de fournisseurs.
  ...
  <Tab title="shadcn">
    ...
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      **Configuration d’Auth0 requise** - Assurez-vous que votre tenant est configuré avec le
      My Organization API. [Voir le guide de configuration
      →](/docs/fr-CA/get-started/universal-components/my-organization/build-delegated-admin#configure-auth0-dashboard)
    </Callout>
    ...
    * `onClick` - gestionnaire de clic permettant la navigation
    ...
    * `disabled` - Désactive le bouton Enregistrer
    * `onBefore(provider)` - S’exécute avant la mise à jour. Retournez `false` pour empêcher l’enregistrement (p. ex., pour afficher une boîte de dialogue de confirmation).
    * `onAfter(provider, result)` - S’exécute une fois le provider mis à jour avec succès. Utilisez cette option pour afficher une notification ou consigner l’événement.
    ...
    ***
    ...
    Pour les cas d&#39;utilisation avancés, vous pouvez importer des composants d&#39;onglets individuels pour créer des interfaces d&#39;édition de Custom Provider.
