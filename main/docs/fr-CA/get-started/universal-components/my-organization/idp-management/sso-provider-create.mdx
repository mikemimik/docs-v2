      ...
      export default function App() {
        ...
        return (
          <Auth0Provider
            ...
            interactiveErrorHandler='popup' // Requis pour gérer les challenges d’authentification step-up dans la fenêtre popup Universal Login
          >
...
La prop `createAction` est requise, car elle détermine vers où les utilisateurs sont dirigés après la création réussie d’un fournisseur. Sans elle, le composant ne saurait pas quoi faire ensuite.

**Propriétés :**

* `disabled` - Désactive le bouton de création (par exemple, lorsqu’une autre opération est en cours)
* `onBefore(data)` - S’exécute avant la création du fournisseur. Retournez `false` pour empêcher la création (par exemple, pour afficher d’abord une boîte de dialogue de confirmation).
* `onAfter(data, result)` - S’exécute après la création réussie du fournisseur. Utilisez cette fonction pour rediriger l’utilisateur vers une autre page ou enregistrer l’événement.
...
Les Props d’affichage déterminent la façon dont le composant s’affiche, sans modifier son comportement. Utilisez-les pour masquer certaines sections ou activer le mode lecture seule.
...
Les props de l’action gèrent les interactions utilisateur au-delà du flux principal de création. Elles contrôlent la navigation et le comportement des étapes de l’assistant.
...
* `onClick` - Handler de clic pour la navigation
...
Permet de contrôler la navigation entre les étapes de l’assistant. Ces fonctions de rappel sont appelées lorsque l’utilisateur clique sur Next ou Previous. Retournez `false` pour empêcher la navigation.

**Cas d’utilisation :**

* Valider les données de l’étape avant de poursuivre
* Enregistrer des données analytiques sur la fin d’une étape
* Ignorer certaines étapes selon des conditions
...
Personnalisez tous les textes et toutes les traductions. Tous les champs sont facultatifs et prennent les valeurs par défaut s’ils ne sont pas fournis.
...
Personnalisez l’apparence à l’aide de variables CSS et de redéfinitions de classes. Prend en charge le styling adapté au thème.
...
<Accordion title="Options de styling disponibles">
  ...
  **Classes** - Redéfinitions des classes de composants
...
Le composant `SsoProviderCreate` se compose de sous-composants et de hooks. Vous pouvez les importer individuellement pour créer des flux de travail personnalisés de création de fournisseurs SSO si vous utilisez shadcn.

<div id="available-subcomponents">
  ### Sous-composants disponibles
</div>

Pour les cas d’utilisation avancés, vous pouvez importer des sous-composants individuels afin de créer des workflows personnalisés de création de fournisseurs SSO. C’est utile lorsque vous devez intégrer une seule étape dans une autre UI ou personnaliser le flux de l’assistant au-delà de ce que permettent les props.
...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>UI d’assistant en plusieurs étapes</td>
    </tr>
  </tbody>
</table>
