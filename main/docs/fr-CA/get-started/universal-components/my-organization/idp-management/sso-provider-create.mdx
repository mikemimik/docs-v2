...
description: Assistant en plusieurs étapes pour créer des fournisseurs SSO avec sélection du fournisseur, configuration des paramètres et configuration de l’authentification
---

import Spinner from "/snippets/Spinner.jsx";
import { ComponentLoader } from "/snippets/ComponentLoader.jsx";

Le composant `SsoProviderCreate` fournit une interface unifiée pour ajouter de nouveaux fournisseurs [d’authentification unique](/docs/fr-CA/authenticate/enterprise-connections/self-service-enterprise-configuration).
      ...
      export default function App() {
        ...
        return (
          <Auth0Provider
            ...
            interactiveErrorHandler='popup' // Requis pour gérer les challenges d’authentification renforcée au moyen de la fenêtre popup Universal Login
          >
      ...
      export default function App() {
        ...
        return (
          <Auth0Provider
            ...
            clientId="your-client-id"
            authorizationParams={{ redirect_uri: window.location.origin }}
            interactiveErrorHandler='popup' // Requis pour gérer les défis d’authentification renforcée par l’intermédiaire de la fenêtre contextuelle Universal Login
          >
            <Auth0ComponentProvider domain={domain}>
              <CreateSsoProviderPage />
            </Auth0ComponentProvider>
...
Les props requises sont essentielles au fonctionnement du composant. Pour `SsoProviderCreate`, il n&#39;y a qu&#39;une seule prop principale, qui détermine ce qui se passe après la création réussie d&#39;un fournisseur d&#39;identité.
...
<table class="table">
  ...
  <tbody>
    <tr>
      ...
      <td>
        <strong>Obligatoire.</strong> Contrôle ce qui se passe après la création.
      </td>
    </tr>
  </tbody>
</table>
...
La prop `createAction` est requise, car c&#39;est elle qui détermine vers où les utilisateurs sont redirigés après la création réussie d&#39;un fournisseur d&#39;identité. Sans elle, le composant ne saurait pas quoi faire ensuite.

**Propriétés :**

* `disabled` - Désactive le bouton de création (p. ex., pendant qu&#39;une autre opération est en cours)
* `onBefore(data)` - S&#39;exécute avant la création du fournisseur d&#39;identité. Retournez `false` pour empêcher la création (p. ex., pour afficher d&#39;abord une boîte de dialogue de confirmation).
* `onAfter(data, result)` - S&#39;exécute après la création réussie du fournisseur d&#39;identité. Utilisez ceci pour rediriger l&#39;utilisateur vers une autre page ou faire le suivi de l&#39;événement.
...
Les props d’affichage déterminent comment le composant est rendu sans en modifier le comportement. Utilisez-les pour masquer certaines sections ou activer le mode lecture seule.
...
Contrôle la navigation entre les étapes de l’assistant. Ces callbacks sont appelés lorsque l’utilisateur clique sur Next ou Previous. Retournez `false` pour empêcher la navigation.

**Cas d’utilisation :**

* Valider les données de l’étape avant de continuer
* Consigner des données analytiques lorsque l’étape est terminée
...
Personnalisez tout le texte et les traductions. Tous les champs sont facultatifs; s’ils ne sont pas fournis, les valeurs par défaut seront utilisées.
...
Le composant `SsoProviderCreate` est constitué de sous-composants et de hooks. Vous pouvez les importer individuellement pour créer des workflows personnalisés de création de fournisseurs SSO si vous utilisez shadcn.

<div id="available-subcomponents">
  ### Sous-composants disponibles
</div>

Pour les cas d’utilisation avancés, vous pouvez importer des sous-composants individuels afin de créer des workflows personnalisés pour la création de fournisseurs SSO. C’est utile lorsque vous devez intégrer une seule étape dans une autre interface utilisateur ou personnaliser le flux de l’assistant au-delà de ce que permettent les props.
...
<table class="table">
  ...
  <tbody>
    <tr>
      ...
      <td>Étape de sélection de la stratégie avec des icônes de fournisseur</td>
    </tr>
    ...
    <tr>
      ...
      <td>Assistant à plusieurs étapes</td>
    </tr>
  </tbody>
</table>
...
Ces hooks fournissent la logique sous-jacente, sans UI intégrée. Utilisez-les pour créer des interfaces entièrement personnalisées tout en tirant parti de l’intégration Auth0 API.
...
<table class="table">
  ...
  <tbody>
    <tr>
      ...
      <td>Logique de création du fournisseur et intégration API</td>
    </tr>
  </tbody>
</table>