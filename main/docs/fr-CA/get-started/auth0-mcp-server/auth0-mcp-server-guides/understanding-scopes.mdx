---
description: Découvrez comment utiliser les scopes dans le protocole de contexte de modèle
...
Selon le type d’opération de gestion ou d’outil que vous souhaitez utiliser, vous devrez peut-être demander des scopes supplémentaires pour exécuter correctement l’opération MCP sur votre client. Voici les scopes disponibles.
...
<Accordion title="Scopes des opérations de gestion">
  ...
  | Obtenir les détails d’une application Auth0 précise          | `auth0_get_application`        | `read:clients`            | Permet de consulter des détails de configuration d’application potentiellement sensibles                                           |
  | Créer une nouvelle application Auth0                         | `auth0_create_application`     | `create:clients`          | Permet de créer de nouvelles applications qui pourraient être utilisées dans des flux d’authentification                           |
  ...
  | Obtenir les détails d’un serveur de ressources Auth0 précis  | `auth0_create_resource_server` | `create:resource_servers` | Révèle les détails de configuration de l’API, y compris les algorithmes de signature et les paramètres du jeton                    |
  ...
  | Lister toutes les Actions du tenant Auth0                    | `auth0_list_actions`           | `read:actions`            | Révèle tout le code personnalisé qui s’exécute pendant les flux d’authentification                                                 |
  ...
  | Lister tous les formulaires du tenant Auth0                  | `auth0_list_forms`             | `read:forms`              | Révèle les configurations de formulaires personnalisés pour la connexion, l’inscription et la réinitialisation du mot de passe     |
  | Obtenir les détails d’un formulaire Auth0 précis             | `auth0_get_form`               | `read:forms`              | Expose la configuration détaillée du formulaire, y compris la validation des champs et le design                                   |
  ...
  | Mettre à jour un formulaire Auth0 existant                   | `auth0_update_form`            | `update:forms`            | Peut modifier les formulaires visibles par les utilisateurs, ce qui pourrait affecter l’expérience d’authentification              |
