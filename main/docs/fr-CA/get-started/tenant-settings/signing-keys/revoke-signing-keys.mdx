---
description: Apprenez à révoquer la clé de signature d’application de votre tenant au moyen de l’Auth0 Dashboard ou de la Management API.
...
2. Dans la section **List of Valid Keys**, repérez la clé **Previously Used**, sélectionnez le menu des options supplémentaires (**...**), puis sélectionnez **Revoke Key**.
   La section **List of Valid Keys** répertorie la clé de signature actuelle utilisée par votre tenant, ainsi que la prochaine clé de signature qui sera attribuée si vous choisissez de faire la rotation de vos clés de signature. Si vous avez déjà fait la rotation de vos clés de signature, cette section répertorie aussi les clés utilisées précédemment.
...
<div id="rotate-and-revoke-signing-key">
  ### Effectuer la rotation et révoquer la clé de signature
</div>
...
<Warning>
  Vous pouvez seulement révoquer la clé de signature utilisée précédemment.
</Warning>

1. Pour obtenir la liste des clés de signature, envoyez une requête `GET` à l’[endpoint Get all Application Signing Keys](https://auth0.com/docs/api/management/v2#!/Keys/get_signing_keys).
2. Envoyez une requête `PUT` à l’[endpoint Revoke an Application Signing Key by its Key ID](https://auth0.com/docs/api/management/v2#!/Keys/put_signing_keys). Assurez-vous de remplacer les valeurs fictives `{yourKeyId}` et `{yourMgmtApiAccessToken}` par l’ID de votre clé de signature et votre jeton d’accès de la Management API, respectivement.
