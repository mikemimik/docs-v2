...
Grâce à [Rich Authorization Requests (RAR)](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-rar), les clients peuvent demander et obtenir des données d’<Tooltip tip="Autorisation fine (FGA) : produit Auth0 permettant aux utilisateurs individuels d’accéder à des objets ou à des ressources spécifiques." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=fine-grained+authorization">autorisation fine</Tooltip> auprès des <Tooltip tip="Propriétaire de ressource : entité (comme un utilisateur ou une application) capable d’accorder l’accès à une ressource protégée." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=resource+owners">propriétaires de ressources</Tooltip>, comme les utilisateurs finaux, pendant le [flux du code d’autorisation avec des requêtes d’autorisation poussées](/docs/fr-CA/get-started/authentication-and-authorization-flow/authorization-code-flow/authorization-code-flow-with-par) et le [flux d’authentification par canal secondaire initié par le client](/docs/fr-CA/get-started/authentication-and-authorization-flow/client-initiated-backchannel-authentication-flow).

Rich Authorization Requests utilisent le paramètre `authorization_details`. Ce paramètre accepte un tableau JSON d’objets pouvant inclure des renseignements détaillés sur l’autorisation demandée, par exemple les ressources précises ou les actions auxquelles le client veut accéder au nom de l’utilisateur.

Vous pouvez afficher à l’utilisateur les `authorization_details` demandés à l’aide de l’un ou l’autre des éléments suivants :

* le navigateur, en utilisant une [invite de consentement personnalisée](#set-customized-consent-prompt)
* une application mobile lorsque vous utilisez des notifications poussées avec le [SDK Auth0 Guardian ou l’application Auth0 Guardian](/docs/fr-CA/secure/multi-factor-authentication/auth0-guardian)

Pour configurer Rich Authorization Requests pour un <Tooltip tip="Serveur de ressources : serveur hébergeant des ressources protégées. Les serveurs de ressources acceptent les requêtes visant des ressources protégées et y répondent." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=resource+server">serveur de ressources</Tooltip>, vous devez :
...
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------- |
| `type`        | Indique le type de demande d’autorisation :<br /><ul><li>`urn:auth0:schemas:authorization-details` : l’URN Auth0 indique que la requête utilisera le schéma Auth0.</li><li>`v1` : la version du schéma.</li><li>`user-profile` : valeur fournie par le client indiquant que la requête concerne le renseignement du profil utilisateur.</li></ul>                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `urn:auth0:schemas:authorization-details:v1:user-profile`                                                                         |
| `instruction` | Message compréhensible par l’utilisateur l’invitant à approuver la requête.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `Please approve the request.`                                                                                                     |
| `properties`  | Objet JSON contenant les attributs utilisateur ou les assertions demandés. Chaque clé (par ex. `email`, `full_name`) représente un champ précis du profil utilisateur :<br /><ul><li>`display` : valeur booléenne qui détermine si la propriété doit être affichée à l’utilisateur dans la boîte de dialogue de consentement. Si `true`, elle sera affichée; si `false`, il s’agit d’une propriété réservée à l’interne qui n’est pas destinée à être vue par l’utilisateur.</li><li>`name` : nom compréhensible par l’utilisateur pour la propriété (par ex. &quot;adresse courriel&quot;).</li><li>`display_order` : entier qui détermine l’ordre dans lequel les propriétés seront affichées dans la boîte de dialogue de consentement.</li><li>`description` : courte explication facultative de l’objectif de la propriété.</li><li>`value` : valeur réelle des données de la propriété (par ex. &quot;user@example.com&quot;, &quot;John Doe&quot;). Le type de données peut varier (chaîne, entier, booléen, etc.).</li></ul> | `"properties": { "stringPropertyForDisplay": { "display": true, "name": "A String:", "display_order": "1", "value": "Value 1"} }` |
...
Vous devez enregistrer les types `authorization_details` pour un serveur de ressources, ce qui s’apparente à l’enregistrement des scopes autorisés. Vous pouvez enregistrer les types `authorization_details` dans le [Auth0 Dashboard](https://manage.auth0.com/) ou au moyen de la [Management API](https://auth0.com/docs/api/management/v2).

<Tabs>
  <Tab title="Auth0 Dashboard">
    Pour ajouter des `authorization_details` dans le Auth0 Dashboard :

    1. Accédez à [Auth0 Dashboard &gt; Applications &gt; APIs](https://manage.auth0.com/#/apis).
    2. Sélectionnez l’onglet **Permissions**.
    3. Sous **Add an Authorization Details type**, vous pouvez ajouter plusieurs types `authorization_details` pour votre serveur de ressources. Entrez un type `authorization_details`.
...
[API Access Policies for Applications](/docs/fr-CA/get-started/apis/api-access-policies-for-applications) déterminent à quelles API vos applications peuvent accéder et quels scopes ou types `authorization_details` elles sont autorisées à utiliser.

Vous pouvez vérifier la politique actuelle de votre API à l’aide de la Management API. Effectuez une requête `GET` au point de terminaison [Get a resource server](https://auth0.com/docs/api/management/v2/resource-servers/get-resource-servers-by-id) et vérifiez la propriété `subject_type_authorization` dans la réponse :
...
* Si la valeur de la politique est `allow_all`, les applications ou les utilisateurs peuvent demander tous les types `authorization_details` enregistrés pour l’API.
* Si la valeur de la politique est `require_client_grant`, chaque type de `authorization_details` doit être explicitement autorisé par le client grant de cette application.
* Si la valeur de la politique est `deny_all`, aucune application ni aucun utilisateur ne peut demander quelque type `authorization_details` que ce soit enregistré pour l’API.
...
Vous pouvez afficher les `authorization_details` d’une demande d’autorisation enrichie dans l’invite de consentement. Pour ce faire, configurez l’invite `customized-consent` avec les partials de modèle appropriés.
...
Pour configurer les partials de consentement personnalisé, exécutez la commande `auth0 ul customize` avec les indicateurs appropriés dans votre terminal :

```bash lines
auth0 ul customize
```

Pour en savoir plus, consultez la [documentation d’Auth0 sur la personnalisation de l’expérience Universal Login](https://auth0.github.io/auth0-cli/auth0_universal-login_customize.html).

<div id="management-api">
  ### Management API
</div>

Pour configurer les partials de consentement personnalisé, effectuez une requête `PUT` au point de terminaison `/prompts/customized-consent/partials` :
...
Le modèle de consentement personnalisé affiche les `authorization_details` dans l’écran de consentement suivant qu’Auth0 présente à l’utilisateur final :
...
La politique de consentement du serveur de ressources détermine si Auth0 stocke les valeurs `authorization_details` et les rend accessibles aux applications mobiles lorsqu’une notification poussée est envoyée.
...
| Authorization Code Flow with PAR                 | Oui                          | Aucune invite de consentement ne s’affiche. La solution du client doit afficher le consentement au moyen de sa propre interface utilisateur. Auth0 attribuera un ID unique à la requête et le rendra accessible à la Post-Login Action sous la forme `event.transaction.linking_id`, ainsi que `event.transaction.requested_authorization_details`.<br />Si l’application Auth0 Guardian est utilisée, les `authorization_details` ne seront PAS affichés.<br />Si une application mobile personnalisée est utilisée, la notification poussée comprendra le `linking_id`, ce qui permettra aux développeurs d’applications de récupérer les `authorization_details` à partir de leurs propres API, au besoin. |
| flux d’authentification par canal secondaire initié par le client | Tous                         | Le flux CIBA n’est pas pris en charge avec la politique de consentement `transactional-authorization-with-mfa`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
