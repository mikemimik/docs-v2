---
description: La mise en œuvre sur Android de l’API pour le scénario d’architecture Mobile + API
title: Mise en œuvre de l’application mobile Android (applications mobiles + API)
---

Ce document fait partie du scénario d’architecture Mobile + API et explique comment mettre en œuvre l’application mobile sur Android. Consultez le scénario pour en savoir plus sur la solution mise en œuvre.
...
Téléchargez un projet d’exemple propre à ce tutoriel pour commencer.
...
* [OkHttp](http://square.github.io/okhttp/) : ce package fournit un client HTTP pour envoyer des requêtes à l’API Node.JS.
* [JWTDecode.Android](https://github.com/auth0/JWTDecode.Android) : ce package aidera à décoder les <Tooltip tip="JSON Web Token (JWT) : format standard d’ID Token (et souvent d’Access Token) utilisé pour représenter des claims de façon sécurisée entre deux parties." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=JWTs">JWTs</Tooltip>.
...
Nous mettrons aussi à jour les détails de l’application afin d’utiliser le widget Toolbar :
...
Définissez le <Tooltip tip="Client ID : valeur d’identification attribuée à votre ressource enregistrée par Auth0." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Client+ID">Client ID</Tooltip> Auth0, le domaine Auth0 et l’URL de l’API dans le fichier de ressources `strings.xml` situé dans `/res/values/strings.xml` :
...
Le `LoginActivity` gérera l’autorisation de l’utilisateur et sera le premier écran affiché aux utilisateurs. Nous créerons une méthode `login()` pour initialiser un `WebAuthProvider` et lancer l’authentication. Assurez-vous de fournir le bon schéma, la bonne <Tooltip tip="Audience : identifiant unique de l’audience d’un token émis. Nommée aud dans un token, sa valeur contient l’ID soit d’une application (Client ID) pour un ID Token, soit d’une API (API Identifier) pour un Access Token." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=audience">audience</Tooltip> et le bon scope au `WebAuthProvider`. Pour cette mise en œuvre, nous utiliserons :
...
Avant d’initialiser le `WebAuthProvider` dans la méthode `login()`, nous pouvons créer le `CredentialsManager`. Le passage d’un `AuthenticationAPIClient` au `CredentialsManager` lui permet d’actualiser les <Tooltip tip="Jeton d’accès : information d’autorisation, sous la forme d’une chaîne opaque ou d’un JWT, utilisée pour accéder à une API." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Access+Tokens">jetons d’accès</Tooltip> s’ils ont expiré.
...
Maintenant, mettez à jour la méthode `login()` pour que les identifiants soient stockés à l’aide de `CredentialsManager` après une authentification réussie.
...
Créez un modèle User simple qui sera utilisé par `UserProfileManager` et `UserActivity`.
...
Pour gérer l’enregistrement des renseignements du profil utilisateur, nous allons créer une classe de gestion `UserProfileManager`. Le `UserProfileManager` utilisera [SharedPreferences](https://developer.android.com/reference/android/content/SharedPreferences.html) pour stocker les données.
...
Ensuite, mettez à jour la méthode `login()` dans `LoginActivity` pour récupérer l’<Tooltip tip="ID Token : informations d’identification destinées au client lui-même, plutôt qu’à l’accès à une ressource." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=ID+Token">ID Token</Tooltip> et extraire le profil utilisateur du token à l’aide de la bibliothèque JWTDecode.Android. Enregistrez ensuite le profil utilisateur avec `UserProfileManager`.
...
<div id="step-4-display-ui-elements-conditionally-based-on-scope">
  ## Étape 4. Afficher conditionnellement des éléments de l’interface utilisateur en fonction du scope
</div>

Pour déterminer si un utilisateur a les permissions requises pour effectuer certaines actions, nous pouvons examiner la `scope` qui lui a été accordée pendant le processus d’authentification. La `scope` contiendra une chaîne regroupant tous les scopes accordés à un utilisateur. Pour déterminer si un scope particulier a été accordé, il suffit donc de vérifier si la chaîne des scopes contient la sous-chaîne correspondant à ce scope.
...
N&#39;oubliez pas non plus de mettre à jour le `UserProfileManager` pour enregistrer le champ supplémentaire :
...
Nous pouvons maintenant afficher certains éléments de l’interface utilisateur selon que l’utilisateur s’est vu accorder ou non un scope précis. Par exemple, nous avons un élément de menu d’approbation qui ne devrait être visible que pour les utilisateurs ayant obtenu le scope `approve:timesheets`.

Ci-dessous, vous pouvez voir le code de la classe `BaseActivity` qui vérifie si un utilisateur possède le scope `approve:timesheets` et, en fonction de cela, règle la visibilité de l’élément de menu qui affiche l’activité d’approbation :
...
<div id="create-the-timesheets-activity-layouts">
  ### Créez les fichiers de mise en page de l’activité Timesheets
</div>

Ensuite, créez `timesheet_activity.xml`, le fichier de mise en page de `TimeSheetsActivity` :
...
Le widget `ListView` contiendra des entrées individuelles, représentées par le layout `item_entry.xml` :
...
Créez un modèle pour utiliser les données de feuilles de temps dans nos vues :
...
Le `TimeSheetAdapter` est une classe utilitaire qui prend un tableau d’entrées de feuille de temps et les affiche dans le `ListView` de `TimeSheetActivity`.
...
* La valeur `@string/api_url` est définie à `http://10.0.2.2:8080/timesheets` afin que l’émulateur Android puisse se connecter à l’API Node.JS exécutée à l’adresse `http://localhost:8080`.
...
Pour afficher le profil de l’utilisateur connecté, nous allons créer `UserActivity`, le fichier de mise en page correspondant `user_activity.xml`, ainsi que `user_action_menu.xml` pour la navigation de la Toolbar. Cette vue affichera le nom de l’utilisateur, son courriel et sa photo de profil.

### Mettre à jour le Manifest

Ouvrez le fichier `AndroidManifest.xml` de l’application et ajoutez-y `UserActivity` :
...
<div id="create-the-user-activity-layouts">
  #### Créer la mise en page de l’activité utilisateur
</div>

Ensuite, créez `user_activity.xml`, la mise en page de `UserActivity`, avec une `ImageView` pour la photo de profil et des `TextViews` pour le nom et le courriel de l’utilisateur.
...
<div id="create-the-user-activity">
  ### Créer l’activité User activité
</div>

Dans la méthode `onCreate()`, nous récupérerons les informations de l’utilisateur à partir de `UserProfileManager` et définirons les valeurs dans la vue. Comme précédemment, les méthodes `onCreateOptionsMenu()` et `onOptionsItemSelected()` gèrent la fonctionnalité de navigation de la Toolbar.
...
public class UserActivity extends AppCompatActivity {
    ...
    @Override
    public boolean onCreateOptionsMenu(Menu menu) {
        // Gonfler les éléments de menu pour utilisation dans la barre d'action
        MenuInflater inflater = getMenuInflater();
...
Créez ensuite la `FormActivity` et la mise en page pour permettre la création de nouvelles entrées de feuille de temps.

### Mettre à jour le Manifest

Ouvrez le `AndroidManifest.xml` de l’application et ajoutez le `FormActivity` :
...
<div id="create-the-form-activity-layouts">
  ### Créez les mises en page de la Form activité
</div>

Créez le `form_activity.xml` avec des `EditText` pour le nom du projet et le nombre d’heures travaillées, ainsi qu’un `DatePicker` pour la date travaillée.
...
* `@string/api_url` est définie sur `http://10.0.2.2:8080/timesheets` afin que l’émulateur Android puisse se connecter à l’API Node.JS exécutée sur `http://localhost:8080`.
* La méthode `onCreate()` initialise le formulaire et recueille les données saisies pour la méthode `postAPI()` lorsque l’utilisateur appuie sur le bouton de soumission.
* La méthode `postAPI()` envoie à l’API Node.JS, au format JSON, les données saisies par l’utilisateur et récupérées à partir du formulaire.
...
Avant de continuer, assurez-vous d’avoir [implémenté l’API Node.JS](/docs/fr-CA/get-started/architecture-scenarios/mobile-api/api-implementation-nodejs).
...
4. Une fois l’application mobile chargée dans l’émulateur, vous pouvez connecter l’utilisateur, puis créer et consulter des entrées de feuille de temps à partir de l’API Node.JS en cours d’exécution.
