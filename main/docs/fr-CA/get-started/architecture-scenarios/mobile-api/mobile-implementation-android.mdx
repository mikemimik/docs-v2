---
description: La mise en œuvre de l’API sur Android pour le scénario d’architecture Mobile + API
title: Mise en œuvre de l’application mobile Android (Applications mobiles + API)
---

Ce document fait partie du scénario d’architecture Mobile + API et explique comment mettre en œuvre l’application mobile sur Android. Veuillez consulter le scénario pour en savoir plus sur la solution mise en œuvre.
...
Téléchargez le projet d’exemple correspondant à ce tutoriel pour commencer.
...
* [JWTDecode.Android](https://github.com/auth0/JWTDecode.Android) : ce package aidera à décoder les <Tooltip tip="JSON Web Token (JWT) : format standard d’ID Token (et souvent d’Access Token) utilisé pour représenter de façon sécurisée des claims entre deux parties." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=JWTs">JWTs</Tooltip>.
* AppCompat : ce package nous permet d’utiliser la barre d’outils pour la navigation dans nos activités.
...
Pour cette mise en œuvre, créez dans le package de l’application des répertoires pour les activités, les modèles et les utilitaires.
...
Ouvrez le `AndroidManifest.xml` de l’application et ajoutez-y `LoginActivity` :
...
<div id="create-the-login-activity">
  ### Créer l’activité Login
</div>

Le `LoginActivity` gérera l’autorisation des utilisateurs et sera le premier écran qu’ils verront. Nous créerons une méthode `login()` pour initialiser un `WebAuthProvider` et lancer l’authentification. Assurez-vous de fournir le schéma, <Tooltip tip="Audience : identifiant unique de l’audience d’un token émis. Appelée aud dans un token, sa valeur contient l’ID d’une application (Client ID) pour un ID Token ou d’une API (API Identifier) pour un Access Token." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=audience">audience</Tooltip> et le scope appropriés au `WebAuthProvider`. Pour cette mise en œuvre, nous utiliserons :
...
Pour stocker les informations d’identification reçues après la connexion, nous utiliserons le `CredentialsManager` de la bibliothèque Auth0.Android ainsi que [SharedPreferences](https://developer.android.com/reference/android/content/SharedPreferences.html) comme mécanisme de stockage.

Avant d’initialiser le `WebAuthProvider` dans la méthode `login()`, nous pouvons créer le `CredentialsManager`. En lui passant un `AuthenticationAPIClient`, le `CredentialsManager` peut actualiser les <Tooltip tip="Jeton d’accès : justificatif d’autorisation, sous la forme d’une chaîne opaque ou d’un JWT, utilisé pour accéder à une API." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Access+Tokens">jetons d’accès</Tooltip> s’ils sont expirés.
...
Mettez maintenant à jour la méthode `login()` pour que les informations d’identification soient enregistrées à l’aide de `CredentialsManager` une fois l’authentification réussie.
...
<div id="step-4-display-ui-elements-conditionally-based-on-scope">
  ## Étape 4. Afficher des éléments de l’UI de façon conditionnelle selon le scope
</div>

Pour déterminer si un utilisateur dispose des permissions nécessaires pour effectuer certaines actions, nous pouvons examiner le `scope` qui lui a été accordé pendant le processus d’authentification. Le `scope` contient une chaîne comprenant tous les scopes accordés à un utilisateur; pour déterminer si un scope en particulier a été accordé, il suffit donc de vérifier si la chaîne des scopes contient la sous-chaîne correspondant à ce scope précis.
...
Nous pouvons maintenant afficher certains éléments de l’UI selon que l’utilisateur s’est vu accorder ou non un scope particulier. Par exemple, nous avons un élément de menu d’approbation qui ne devrait être visible que pour les utilisateurs ayant reçu le scope `approve:timesheets`.

Ci-dessous, vous pouvez voir le code de la classe `BaseActivity` qui vérifie si un utilisateur possède le scope `approve:timesheets` et qui, en conséquence, définit la visibilité de l’élément de menu affichant l’Activity d’approbation :
...
<div id="update-the-manifest">
  ### Mettez à jour le fichier Manifest
</div>

Ouvrez le `AndroidManifest.xml` de l’application et ajoutez `TimeSheetActivity` :
...
Et pour la navigation de la Toolbar dans `TimeSheetActivity`, nous créerons la ressource de menu `timesheet_action_menu.xml` (`/res/menu/`) :
...
Créez un modèle pour manipuler les données de feuille de temps dans nos vues :
...
* La valeur de `@string/api_url` est définie sur `http://10.0.2.2:8080/timesheets` pour que l’émulateur Android puisse se connecter à l’API Node.JS qui s’exécute sur `http://localhost:8080`.
...
<div id="create-the-user-activity-layouts">
  #### Créer les mises en page de l’activité `UserActivity`
</div>
...
<div id="create-the-user-activity">
  ### Créer l’activité User
</div>

Dans la méthode `onCreate()`, nous allons récupérer les informations de l’utilisateur à partir de `UserProfileManager` et renseigner les valeurs dans la vue. Comme précédemment, les méthodes `onCreateOptionsMenu()` et `onOptionsItemSelected()` gèrent la fonctionnalité de navigation du widget Toolbar.
...
<div id="step-7-form-for-new-timesheets">
  ## Étape 7. Formulaire pour de nouvelles feuilles de temps
</div>

Créez ensuite le `FormActivity` et la mise en page nécessaires pour créer de nouvelles entrées de feuille de temps.

### Mettez à jour le Manifest

Ouvrez le `AndroidManifest.xml` de l’application et ajoutez `FormActivity` :
...
<div id="create-the-form-activity-layouts">
  ### Créer les mises en page de l’activité Form
</div>

Créez la mise en page `form_activity.xml` avec des `EditText` pour le nom du projet et le nombre d’heures travaillées, ainsi qu’un `DatePicker` pour le jour travaillé.
...
Et créez le `form_actions_menu.xml` pour le `Toolbar` de `FormActivity` :
...
<div id="create-the-form-activity">
  ### Créer l’activité Form
</div>

* Le `@string/api_url` est défini sur `http://10.0.2.2:8080/timesheets` afin que l’émulateur Android puisse se connecter à l’API Node.JS exécutée sur `http://localhost:8080`.
* La méthode `onCreate()` initialise le formulaire et recueille les données saisies pour la méthode `postAPI()` lorsqu’on appuie sur le bouton d’envoi.
...
* La méthode `clearForm()` efface le contenu des champs du formulaire.
* Les méthodes `onCreateOptionsMenu()` et `onOptionsItemSelected()` gèrent la navigation du widget Toolbar.
...
Avant de continuer, assurez-vous d’avoir [mis en place l’API Node.JS](/docs/fr-CA/get-started/architecture-scenarios/mobile-api/api-implementation-nodejs).

1. Démarrez l’API en vous plaçant dans le répertoire de l’API dans votre terminal, puis en entrant la commande `node server`.
