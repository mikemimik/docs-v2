...
Le scénario d’ExampleCo ne comprend qu’une seule application : l’application Web de feuilles de temps. Nous devons donc configurer une application dans Auth0.
...
Lorsque vous cliquez sur **Create**, vous serez redirigé vers la [vue Quickstart](https://manage.auth0.com/#/applications/\{yourClientId}/quickstart). Vous pourrez y choisir la technologie que vous comptez utiliser pour créer votre application, et le guide de démarrage rapide correspondant s’affichera.
...
<Info>
  ...
  Auth0 prend en charge une grande variété de protocoles et de fournisseurs d’identité :
  ...
  * Connexions de base de données : créez votre propre référentiel d’utilisateurs en configurant une nouvelle connexion de base de données, puis authentifiez vos utilisateurs à l’aide d’une adresse courriel ou d’un nom d’utilisateur et d’un mot de passe. Les informations d’identification peuvent être stockées de façon sécuritaire soit dans le référentiel d’utilisateurs d’Auth0, soit dans votre propre base de données.
...
2. Sélectionnez **+ Create DB Connection**. Dans la boîte de dialogue, entrez un nom pour la connexion et configurez les paramètres supplémentaires au besoin.
3. Vérifiez les renseignements dans la boîte de dialogue, puis sélectionnez **Create**.

Après avoir créé la connexion de base de données, vous devez activer vos applications pour cette connexion. Une application doit être activée pour la connexion afin que les utilisateurs puissent accéder à cette application à l’aide de cette connexion. Pour activer vos applications, accédez à l’onglet **Applications** de la nouvelle connexion et activez la bascule pour chaque application qui doit utiliser la connexion.
...
Une fois la connexion et le connecteur configurés, assurez-vous d’activer cette connexion AD / LDAP pour votre application :

<Info>
  ### Prise en charge de Kerberos

  Le connecteur AD/LDAP prend en charge Kerberos afin de faciliter l’authentification de vos utilisateurs lorsqu’ils utilisent une machine jointe à un domaine au sein du réseau d’entreprise. Pour activer Kerberos dans Active Directory, il vous suffit d’activer l’option dans le Dashboard. Après avoir activé Kerberos, vous pourrez aussi configurer les **plages d’adresses IP**. Lorsque les utilisateurs proviennent de ces plages d’adresses IP, cette information sera exposée dans le endpoint SSO, ce qui signifie que des SDK client comme auth0.js et Lock pourront détecter la prise en charge de Kerberos et permettre l’Integrated Windows Authentication.
