...
Ce flux ne peut être utilisé que pour les applications confidentielles (comme les Applications Web traditionnelles), car les méthodes d’authentification de l’application sont incluses dans l’échange et doivent demeurer sécurisées.
...
3. Le serveur d’autorisation Auth0 redirige l’utilisateur vers l’écran de connexion et l’écran d’autorisation.
...
6. Le SDK d’Auth0 envoie le code d’autorisation, le client ID de l’application et les identifiants d’authentification de l’application, comme le client secret ou Private Key JWT, au serveur d’autorisation Auth0 (point de terminaison [`/oauth/token`](https://auth0.com/docs/api/authentication/authorization-code-flow/get-token)).
7. Le serveur d’autorisation Auth0 vérifie le code d’autorisation, le client ID de l’application et les identifiants d’authentification de l’application.
8. Le serveur d’autorisation Auth0 répond avec un ID token et un access token (et, au besoin, un refresh token).
9. L’application peut utiliser l’access token pour appeler une API afin d’accéder à des renseignements sur l’utilisateur.
...
La façon la plus simple d’implémenter le flux de code d’autorisation est de suivre nos [Quickstarts pour les applications Web traditionnelles](/docs/fr-CA/quickstart/webapp).
