...
[Client-Initiated Backchannel Authentication (CIBA)](/docs/fr-CA/get-started/authentication-and-authorization-flow/client-initiated-backchannel-authentication-flow) est une spécification <Tooltip tip="OAuth 2.0 : Cadre d’autorisation qui définit les protocoles et les workflows d’autorisation." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=OAuth+2.0">OAuth 2.0</Tooltip> qui permet à une application cliente de lancer un processus d’authentification et/ou un <Tooltip tip="OAuth 2.0 : Cadre d’autorisation qui définit les protocoles et les workflows d’autorisation." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=authorization+flow">flux d’autorisation</Tooltip> sans nécessiter d’interaction directe de l’utilisateur dans l’application à l’origine de la demande. [Rich Authorization Requests (RAR)](/docs/fr-CA/get-started/apis/configure-rich-authorization-requests) est une extension OAuth 2.0 qui permet aux applications clientes de demander, dans une requête d’autorisation, des permissions plus complexes que les scopes OAuth 2.0 standard.

Vous pouvez utiliser CIBA avec RAR pour transmettre des données d’<Tooltip tip="Fine-grained Authorization (FGA) : Produit Auth0 permettant aux utilisateurs individuels d’accéder à des objets ou à des ressources précis." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=fine-grained+authorization">autorisation fine</Tooltip> au <Tooltip tip="Fine-grained Authorization (FGA) : Produit Auth0 permettant aux utilisateurs individuels d’accéder à des objets ou à des ressources précis." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=authorization+server">serveur d’autorisation</Tooltip> dans une requête backchannel. Le paramètre `authorization_details` contient les détails de la requête que vous pouvez personnaliser dans un écran de consentement pour les présenter à l’utilisateur.
...
* [Notifications par courriel](/docs/fr-CA/get-started/authentication-and-authorization-flow/client-initiated-backchannel-authentication-flow/email-notifications-with-ciba), qui exigent la configuration d’un écran de consentement personnalisé.

<div id="common-use-cases">
  ## Cas d’utilisation courants
</div>

Utilisez RAR avec le CIBA flow pour les cas d’utilisation qui nécessitent un contrôle plus précis de l’accès aux ressources. Voici quelques cas d’utilisation courants :
...
2. Un agent d’IA présente à l’utilisateur les détails d’un rendez-vous chez le médecin qui a été reporté. Les `authorization_details` peuvent être personnalisés pour afficher la nouvelle date et la nouvelle heure.

<div id="how-it-works">
  ## Fonctionnement
</div>

Le flux d’autorisation de l’utilisateur avec CIBA est semblable au flux d’authentification de l’utilisateur avec CIBA. La prise en charge de RAR permet aux clients de transmettre les `authorization_details` au serveur d’autorisation par l’entremise du point de terminaison `/bc-authorize`.
...
* [Étape 2 : le tenant Auth0 confirme la requête CIBA](#step-2%3A-auth0-tenant-acknowledges-the-ciba-request)
...
Pour amorcer une requête CIBA avec Auth0, vous devez :
...
* [Configure Rich Authorization Requests](/docs/fr-CA/get-started/apis/configure-rich-authorization-requests) pour votre <Tooltip tip="Serveur de ressources : serveur qui héberge des ressources protégées. Les serveurs de ressources acceptent les requêtes d’accès aux ressources protégées et y répondent." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=resource+server">serveur de ressources</Tooltip>, ce qui comprend l’enregistrement de vos types `authorization_details`.
...
Une fois que vous avez l’ID utilisateur de l’utilisateur qui autorise la demande, utilisez l’[Authentication API](https://auth0.com/docs/api/authentication/login/start-back-channel-login) pour envoyer une requête CIBA avec les `authorization_details` au point de terminaison `/bc-authorize` :
...
| `client_secret`         | Méthode d’authentification du client utilisée pour l’authentification de l’utilisateur avec CIBA, par exemple secret client, Private Key JWT ou l’authentification mTLS. Si vous utilisez Private Key JWT ou mTLS, vous n’avez pas besoin d’inclure le secret client.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `requested_expiry`      | La durée d’expiration demandée pour le CIBA flow doit être comprise entre 1 et 259200 secondes (72 heures), et la valeur par défaut est de 300 secondes. Incluez le paramètre `requested_expiry` pour définir une durée d’expiration personnalisée pour le CIBA flow.<br /><br />Le paramètre `requested_expiry` aide à déterminer quel canal de notification CIBA utilise :<ul><li>Si vous définissez `requested_expiry` à 300 secondes ou moins, CIBA utilise le canal de notification mobile push s’il est activé. Si la MFA n’est pas configurée pour votre tenant, la requête CIBA échoue.</li><li>Si vous définissez `requested_expiry` à une valeur comprise entre 301 et 259200 secondes, CIBA utilise le canal de notification par courriel s’il est activé.</li></ul> |
| `binding_message`       | Message lisible par l’humain utilisé pour lier le CIBA flow entre les appareils d’authentification et de consommation. Le message de liaison est obligatoire et peut comporter jusqu’à 64 caractères. Utilisez uniquement des caractères alphanumériques et `+-_.,:#`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `audience`              | Identifiant unique de l’audience du jeton émis.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `authorization_details` | Tableau JSON facultatif d’objets qui décrit les permissions à autoriser. Vous devriez enregistrer la valeur `type` de chaque objet sur le serveur de ressources à l’aide du paramètre `authorization_details` de celui-ci. Pour en savoir plus, consultez [Configurer les Rich Authorization Requests](/docs/fr-CA/get-started/apis/configure-rich-authorization-requests).                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
<div id="step-3-client-application-polls-for-a-response">
  ## Étape 3 : L’application cliente vérifie périodiquement si une réponse est disponible
</div>
...
Il faut attendre environ cinq secondes entre chaque interrogation périodique. Si vous interrogez trop souvent, vous recevrez la réponse suivante, où la description varie selon l’intervalle de temporisation exponentielle :
...
Pour résoudre l’erreur, attendez jusqu’au prochain intervalle (en secondes) avant de réinterroger le point de terminaison `/token`.
...
À cette étape, l’utilisateur peut accepter ou refuser la demande d’autorisation.
...
Une fois que l’utilisateur a accepté ou refusé la demande d’autorisation, l’appareil d’authentification renvoie sa réponse à Auth0 :
...
L’application cliente met fin à l’interrogation périodique après avoir reçu une réponse du point de terminaison `/token`. Un flux CIBA exige toujours une réponse — approbation ou refus — de l’utilisateur qui donne l’autorisation, et les autorisations existantes ne sont pas vérifiées. Cela signifie qu’Auth0 traite chaque requête CIBA comme une nouvelle autorisation de la part de l’utilisateur qui donne l’autorisation.
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Le `refresh_token` n’est présent que si la portée `offline_access` a été incluse dans la requête initiale à `/bc-authorize`.
</Callout>
...
Au moment de la compilation, vous pouvez interroger le type et les objets de `authorization_details` à partir des détails du consentement de façon fortement typée, comme vous le feriez en interrogeant dynamiquement du JSON :
...
`filterAuthorizationDetailsByType()` renvoie uniquement les objets correspondant au type `authorization_details` spécifié. Par conséquent, votre application mobile devrait présenter à l’utilisateur tous les `authorization_details` pertinents pour le consentement, peu importe leur type, afin d’assurer une compréhension complète de la demande
...
| `client_id`     | Défini sur le client ID de l’application.                                                                    |
...
Lorsque l’utilisateur autorisant approuve la requête, Auth0 reçoit la réponse de l’utilisateur, et le flux CIBA se termine en renvoyant un jeton d’accès et le tableau `authorization_details` :
...
* La publication des types RAR pour que les clients puissent les découvrir, ce qui signifie que vous devez préenregistrer les clients avec les types `authorization_details` qu’ils peuvent envoyer.
* La validation des objets RAR au-delà de la vérification qu’ils comportent une propriété `type` correspondant aux types autorisés pour l’API. Votre serveur de ressources est responsable de la validation granulaire du contenu d’`authorization_details`. Pour en savoir plus, consultez [Configurer RAR](/docs/fr-CA/get-started/apis/configure-rich-authorization-requests).
