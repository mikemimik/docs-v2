...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Pour utiliser les fonctionnalités de Highly Regulated Identity, vous devez avoir un plan Enterprise avec le module complémentaire Highly Regulated Identity. Consultez [Auth0 Pricing](https://auth0.com/pricing/) pour plus de détails.
</Callout>
...
L’un des avantages de PAR est la validation en amont. Dans d’autres flux OAuth 2.0, comme le [flux du code d’autorisation](/docs/fr-CA/get-started/authentication-and-authorization-flow/authorization-code-flow), les utilisateurs finaux sont redirigés vers le serveur d’autorisation pour que la validation s’y fasse. Avec PAR, les paramètres de la requête sont validés au début de la requête d’autorisation, avant que l’utilisateur final ne soit redirigé. Il n’est pas souhaitable de rediriger les utilisateurs simplement pour leur afficher une page d’erreur.

PAR transmet aussi les requêtes d’autorisation par le back-channel. Les communications en front-channel reposent sur un intermédiaire (p. ex. un navigateur), au moyen de paramètres de requête HTTPS ajoutés (GET, POST). Les messages ne sont pas envoyés directement. Les communications en back-channel sont transmises dans le corps d’une requête backend authentifiée, ce qui constitue une approche plus directe.
...
* Les limites de longueur d’URL ne sont plus un facteur contraignant.

<div id="limitations">
  ## Limitations
</div>

* La taille maximale de la charge utile de la requête est de 10 KB.
...
PAR ignore les paramètres d’autorisation personnalisés additionnels. Les paramètres d’autorisation personnalisés ne sont pas disponibles dans les [Auth0 Actions](/docs/fr-CA/customize/actions) ni dans les [Logs](/docs/fr-CA/deploy-monitor/logs).
...
* Le `expires_in` correspond au nombre de secondes pendant lesquelles le `request_uri` est valide. Après cette période, le `request_uri` expire s’il n’est pas utilisé. Le délai d’expiration de trente secondes est une valeur statique et ne peut pas être configuré.
...
<div id="call-the-authorization-endpoint">
  ## Appelez le point de terminaison d’autorisation
</div>
...
Dans le cas d’un `request_uri` valide, le reste du <Tooltip tip="Flux d’autorisation : mécanisme d’autorisation (ou workflow) spécifié dans le framework OAuth 2.0." cta="Consulter le glossaire" href="/docs/fr-CA/glossary?term=authorization+flow">flux d’autorisation</Tooltip> reste le même.
...
* Une requête non-PAR est rejetée si PAR est requis au niveau du tenant ou du client.
