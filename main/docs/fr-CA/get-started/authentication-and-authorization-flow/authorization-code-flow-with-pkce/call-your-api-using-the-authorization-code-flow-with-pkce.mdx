---
description: "Découvrez comment appeler votre API à partir d’une application native, mobile ou monopage au moyen du flux du code d’autorisation avec la clé de preuve pour l’échange de code (PKCE)."
title: "Appeler votre API au moyen du flux du code d’autorisation avec PKCE"
---
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Ce tutoriel vous montre comment appeler votre propre API à partir d’une application native, mobile ou monopage à l’aide de l’Authorization Code Flow with PKCE. Pour comprendre le fonctionnement de ce flux et pourquoi vous devriez l’utiliser, consultez [Authorization Code Flow with Proof Key for Code Exchange (PKCE)](/docs/fr-CA/get-started/authentication-and-authorization-flow/authorization-code-flow-with-pkce). Pour savoir comment ajouter la connexion à votre application native, mobile ou monopage, consultez [Add Login Using Authorization Code Flow with PKCE](/docs/fr-CA/get-started/authentication-and-authorization-flow/authorization-code-flow-with-pkce/add-login-using-the-authorization-code-flow-with-pkce).
</Callout>

Auth0 permet à votre application de mettre en œuvre facilement l’Authorization Code Flow with Proof Key for Code Exchange (PKCE) à l’aide de :

* [Auth0 Mobile SDKs](/docs/fr-CA/libraries) et [Auth0 Single-Page App SDK](/docs/fr-CA/libraries/auth0-single-page-app-sdk) : la façon la plus simple de mettre en œuvre ce flux, car ils effectuent l’essentiel du travail pour vous. Nos [Mobile Quickstarts](/docs/fr-CA/quickstart/native) et [Single-Page App Quickstarts](/docs/fr-CA/quickstart/spa) vous guideront tout au long du processus.
...
* [Enregistrez l’application dans Auth0](/docs/fr-CA/get-started/auth0-overview/create-applications/native-apps).
  ...
  * Assurez-vous que les **types d’octroi** de l’application comprennent le **code d’autorisation**. Pour savoir comment faire, consultez [Mettre à jour les types d’octroi](/docs/fr-CA/get-started/applications/update-grant-types).
  * Si vous voulez que votre application puisse utiliser des **jetons d’actualisation**, assurez-vous que les **types d’octroi** de l’application comprennent **Jeton d’actualisation**. Pour savoir comment faire, consultez [Mettre à jour les types d’octroi](/docs/fr-CA/get-started/applications/update-grant-types). Pour en savoir plus sur les **jetons d’actualisation**, consultez [Jetons d’actualisation](/docs/fr-CA/secure/tokens/refresh-tokens).
...
Créez un `code_verifier`, c’est-à-dire une clé générée aléatoirement de façon cryptographiquement sécuritaire et encodée en Base64, qui sera ensuite envoyée à Auth0 pour demander des jetons.
...
Une fois que vous avez créé le `code_verifier` et le `code_challenge`, vous devez obtenir l’autorisation de l’utilisateur. Techniquement, il s’agit du début du <Tooltip tip="Flux d’autorisation : mécanisme d’autorisation (ou workflow) spécifié dans le framework OAuth 2.0." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=authorization+flow">flux d’autorisation</Tooltip>, et cette étape peut comprendre un ou plusieurs des processus suivants :

* Authentifier l’utilisateur ;
* Rediriger l’utilisateur vers un <Tooltip tip="Identity Provider (IdP) : service qui stocke et gère les identités numériques." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Identity+Provider">fournisseur d’identité</Tooltip> afin d’effectuer l’authentification ;
...
Pour autoriser l’utilisateur, votre application doit le rediriger vers l’[URL d’autorisation](https://auth0.com/docs/api/authentication#authorization-code-grant-pkce-), en y incluant le `code_challenge` généré à l’étape précédente ainsi que la méthode utilisée pour le générer.
...
* devez inclure un paramètre <Tooltip tip="Audience : identifiant unique de l'audience d'un jeton émis. Nommée aud dans un jeton, sa valeur contient l'ID soit d'une application (ID client) pour un ID Token, soit d'une API (API Identifier) pour un Access Token." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=audience">audience</Tooltip>
* pouvez inclure des scopes supplémentaires pris en charge par l’API cible

| Nom du paramètre        | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `response_type`         | Indique le type d’informations d’identification qu’Auth0 renverra (`code` ou `token`). Pour ce flux, la valeur doit être `code`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `state`                 | (recommandé) Une chaîne alphanumérique opaque et arbitraire que votre application ajoute à la requête initiale et qu’Auth0 inclut lors de la redirection vers votre application. Pour savoir comment utiliser cette valeur afin de prévenir les attaques de falsification de requête intersites (CSRF), consultez [Atténuer les attaques CSRF avec les paramètres state](/docs/fr-CA/secure/attack-protection/state-parameters).                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
À titre d’exemple, votre extrait HTML pour votre URL d’autorisation lors de l’appel à une API pourrait ressembler à ceci :
...
<Warning>
  Les jetons d’actualisation doivent être conservés en toute sécurité, puisqu’ils permettent à un utilisateur de rester authentifié pratiquement indéfiniment.
</Warning>
...
Pour appeler votre API à partir d’une application native ou mobile, l’application doit transmettre le jeton d’accès récupéré comme jeton Bearer dans l’en-tête Authorization de la requête HTTP.
...
* inclus la portée `offline_access` lorsque vous avez lancé la requête d’authentification par l’entremise du [point de terminaison authorize](https://auth0.com/docs/api/authentication/reference#authorize-application).

Vous pouvez utiliser le <Tooltip tip="jeton d’actualisation : jeton utilisé pour obtenir un nouveau jeton d’accès sans obliger les utilisateurs à se reconnecter." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Refresh+Token">jeton d’actualisation</Tooltip> pour obtenir un nouveau jeton d’accès. En général, un utilisateur n’aura besoin d’un nouveau jeton d’accès qu’après l’expiration du précédent ou lorsqu’il accède à une nouvelle ressource pour la première fois. Il est déconseillé d’appeler le point de terminaison pour obtenir un nouveau jeton d’accès chaque fois que vous appelez une API, et Auth0 applique des limites de débit qui restreignent le nombre de requêtes au point de terminaison pouvant être exécutées avec le même jeton à partir de la même adresse IP.
...
| `scope`          | (facultatif) Une liste de portées demandées, séparées par des espaces. S’il n’est pas envoyé, les portées d’origine seront utilisées; sinon, vous pouvez demander un ensemble réduit de portées. Notez que cette valeur doit être encodée dans l’URL. |

<div id="response">
  #### Réponse
</div>

Si tout se passe bien, vous recevrez une réponse `HTTP 200` avec une charge utile contenant un nouvel `access_token`, sa durée de validité en secondes (`expires_in`), les valeurs de `scope` accordées et le `token_type`. Si le `scope` du token initial comprenait `openid`, la réponse inclura également un nouvel `id_token` :
...
Vous pouvez utiliser Actions pour modifier les scopes renvoyés par les Access Tokens et/ou ajouter des claims aux Access Tokens et aux ID Tokens. (Pour en savoir plus sur Actions, consultez [Auth0 Actions](/docs/fr-CA/customize/actions).) Pour ce faire, ajoutez l’Action suivante, qui s’exécutera après l’authentification de l’utilisateur :
...
<Warning>
  Auth0 renvoie les informations du profil sous forme de claims structurés, comme le définit la [spécification OpenID Connect (OIDC)](https://openid.net/specs/openid-connect-core-1_0.html#StandardClaims). Cela signifie que les claims personnalisés ajoutés aux jetons ID ou aux jetons d’accès doivent [respecter certaines directives et restrictions](/docs/fr-CA/secure/tokens/json-web-tokens/create-custom-claims) afin d’éviter d’éventuels conflits.
</Warning>

<div id="view-sample-application-mobile-app-api">
  #### Voir l’exemple d’application : application mobile + API
</div>

Pour un exemple de mise en œuvre, consultez le scénario d’architecture [Mobile + API](/docs/fr-CA/get-started/architecture-scenarios/mobile-api). Cette série de tutoriels s’accompagne d’un [exemple de code auquel vous pouvez accéder sur GitHub](https://github.com/auth0-samples/auth0-pnp-exampleco-timesheets).
