...
Pour réussir les <Tooltip tip="OpenID : norme ouverte d’authentification qui permet aux applications de vérifier l’identité des utilisateurs sans recueillir ni stocker leurs informations de connexion." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=OpenID">OpenID</Tooltip> FAPI Conformance Tests, commencez par configurer les éléments suivants :
...
* Configurez soit [mTLS](/docs/fr-CA/get-started/applications/configure-mtls) (y compris les [alias mTLS](/docs/fr-CA/get-started/applications/configure-mtls/configure-mtls-for-a-tenant#enable-mtls-aliases)), soit [Private Key JWT](/docs/fr-CA/get-started/applications/configure-private-key-jwt)
...
Par défaut, Auth0 autorise les requêtes sans portée et suppose alors la portée `openid` si aucune portée n’est indiquée. Auth0 autorise également les requêtes sans `redirect_uri,` que vous pouvez définir dans [Actions](/docs/fr-CA/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/redirect-with-actions). Cependant, les FAPI Conformance Tests exigent qu’Auth0 applique des règles plus strictes.
