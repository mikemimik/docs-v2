...
<Warning>
  Les nouveaux secrets peuvent prendre jusqu’à trente secondes avant d’être pris en compte pendant la rotation. Pour minimiser le temps d’arrêt, nous vous suggérons de stocker le nouveau secret client dans le code ou la configuration système de votre application comme solution de secours au secret précédent. Ainsi, si la requête de l’application cliente ne fonctionne pas avec l’ancien secret, votre application utilisera le nouveau secret.
