...
'og:title': 'Auth0 .NET MAUI SDK Quickstart : ajoutez la connexion à votre application .NET MAUI'
'twitter:title': 'Auth0 .NET MAUI SDK Quickstart : ajoutez la connexion à votre application .NET MAUI'
mode: wide
---
...
<Steps>
  <Step title="Configurez votre application Auth0" stepNumber={1}>
    Configurez votre application Auth0 pour disposer des identifiants nécessaires à votre application MAUI.

    1. Accédez à **[Auth0 Dashboard](https://manage.auth0.com/dashboard/) &gt; Applications &gt; Applications**
    2. Sélectionnez **Create Application**
    3. Saisissez un nom pour votre application (par exemple, &quot;My MAUI App&quot;), sélectionnez **Native** comme type d’application, puis sélectionnez **Create**
    4. Accédez à l’onglet **Settings** sur la page des détails de l’application
    5. Notez les valeurs **Domain** et **Client ID** — vous en aurez besoin plus tard
    ...
    **URLs de déconnexion autorisées :**
    ...
    <Check>
      Vous avez une application native dans Auth0, et vous avez noté votre **Domaine** et votre **Client ID**, ainsi que configuré les URL de rappel et de déconnexion.
    </Check>
  </Step>

  <Step title="Créez votre projet MAUI" stepNumber={2}>
    Si vous avez déjà un projet .NET MAUI, passez à l’étape 3. Sinon, créez-en un en utilisant le CLI .NET :
  ...
  <Step title="Installer le SDK Auth0 pour MAUI" stepNumber={3}>
  ...
  <Step title="Configurer la gestion du callback selon la plateforme" stepNumber={4}>
    ...
    <Tabs>
      <Tab title="Android">
        ...
        <Note>
          La valeur `CALLBACK_SCHEME` doit correspondre exactement au schéma indiqué dans votre `RedirectUri` et dans les **Allowed Callback URLs** d’Auth0.
        </Note>
      </Tab>

      <Tab title="Windows">
        Sous Windows, le flux de rappel d’Auth0 repose sur l’activation de protocole URI, ce qui exige que votre application soit **packagée** (MSIX). Le modèle .NET MAUI par défaut crée une application packagée; aucun changement n’est donc nécessaire si vous avez utilisé `dotnet new maui` ou le modèle MAUI de Visual Studio.

        <Info>
          Si votre `.csproj` contient `<WindowsPackageType>None</WindowsPackageType>`, votre application n’est **pas packagée** et l’activation de protocole ne fonctionnera pas. Supprimez cette ligne ou remplacez-la par `<WindowsPackageType>MSIX</WindowsPackageType>` pour utiliser une application packagée. Pour en savoir plus, consultez la [documentation .NET MAUI sur le packaging Windows](https://learn.microsoft.com/en-us/dotnet/maui/windows/deployment-overview).
        </Info>

        Deux modifications sont nécessaires : enregistrer le protocole et gérer l’activation du protocole.

        **1. Enregistrez le protocole** dans `Platforms/Windows/Package.appxmanifest`. Ajoutez le bloc `<Extensions>` dans l’élément `<Application>` existant :
  ...
  <Step title="Ajouter la connexion et la déconnexion" stepNumber={5}>
      ...
      <Tab title="MainPageViewModel.cs">
        ...
        public class MainPageViewModel : INotifyPropertyChanged
        {
            ...
            private async Task LoginAsync()
            {
                ...
                // Lire les claims du profil utilisateur dans l’ID token
    ...
    <Check>
      Votre projet contient maintenant un ViewModel avec des commandes de connexion et de déconnexion, une page XAML liée aux données et un code-behind raccordé.
    </Check>
  </Step>

  <Step title="Enregistrer les services et instancier le client Auth0" stepNumber={6}>
    Maintenant, enregistrez `Auth0Client`, le ViewModel et la page dans `MauiProgram.cs` à l’aide de l’injection de dépendances. Cela relie le tout afin que le client Auth0 soit injecté dans le ViewModel, puis que le ViewModel soit injecté dans la page :
  ...
  <Step title="Exécutez votre application" stepNumber={7}>
    ...
    **Flux attendu :**
    ...
    5. L’application affiche votre nom et votre adresse courriel, ainsi qu’un bouton **Se déconnecter**
  </Step>
</Steps>
...
<AccordionGroup>
  <Accordion title="Non-correspondance de l’URL de rappel">
    ...
    **Correctif :**
    ...
    2. Videz le champ **Allowed Callback URLs** et retapez `myapp://callback` manuellement — le copier-coller ajoute souvent des espaces de fin ou des sauts de ligne invisibles
    ...
    5. Vérifiez la barre d’adresse du navigateur pour le paramètre de requête `redirect_uri` afin de voir ce que votre application envoie réellement
  </Accordion>
  ...
  <Accordion title="Windows : la connexion semble bloquée">
    **Symptôme** : Le navigateur s’ouvre, mais une deuxième instance de l’application s’ouvre au lieu de reprendre la session en cours.

    **Correctif :** Assurez-vous que `Auth0.OidcClient.Platforms.Windows.Activator.Default.CheckRedirectionActivation()` est appelé comme **toute première ligne** du constructeur `App` dans `Platforms/Windows/App.xaml.cs`, et que le nom du protocole dans `Package.appxmanifest` correspond au schéma de votre callback URI.
  </Accordion>
  ...
  <Accordion title="Windows : l’activation du protocole ne fonctionne pas">
    ...
    **Correctif :** Votre application doit être une application **empaquetée** (MSIX). Vérifiez dans votre fichier `.csproj` s’il y a un élément `<WindowsPackageType>` :

    * S’il est défini sur `None`, l’activation du protocole n’est pas offerte. Supprimez la ligne ou remplacez-la par `<WindowsPackageType>MSIX</WindowsPackageType>`.
...
Vous disposez maintenant d’une intégration Auth0 fonctionnelle dans votre application .NET MAUI. Explorez ces sujets pour approfondir votre mise en œuvre :

<Accordion title="Jetons d’actualisation">
  Le SDK Auth0 MAUI prend en charge les jetons d’actualisation pour renouveler les sessions en arrière-plan, sans redemander quoi que ce soit à l’utilisateur.
...
<Accordion title="Appeler une API protégée">
  Pour obtenir un jeton d’accès dont la portée correspond à votre API, définissez `Scope` et transmettez un paramètre `audience` à `LoginAsync()` :
  ...
  // Le jeton d’accès a maintenant une portée correspondant à votre API
...
<CardGroup cols={3}>
  ...
  <Card title="Flux PKCE" icon="shield" href="/docs/fr-CA/get-started/authentication-and-authorization-flow/authorization-code-flow-with-pkce">
    Comment les applications natives s’authentifient de manière sécurisée
  </Card>
