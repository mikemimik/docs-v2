...
<Accordion title="Utiliser l’IA pour intégrer Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  Si vous utilisez un assistant IA de codage comme Claude Code, Cursor ou GitHub Copilot, vous pouvez ajouter automatiquement l’authentification Auth0 en quelques minutes grâce à [Agent Skills](https://agentskills.io/home).
...
<Steps>
  <Step title="Créer un nouveau projet Expo" stepNumber={1}>
    ...
    **Dans votre terminal :**
    ...
    <Tip>
      Cela crée une application Expo minimale avec la plus récente version du SDK, prête pour l’intégration de modules natifs. L’option `--template blank` fournit un point de départ épuré, sans code standard supplémentaire.
    </Tip>
  ...
  <Step title="Installer le SDK Auth0" stepNumber={2}>
    ...
    <Tip>
      L’utilisation de `npx expo install` garantit la compatibilité avec votre version du SDK Expo. Le SDK se configure automatiquement au moyen du système de plugins d’Expo.
    </Tip>
  </Step>
  ...
  <Step title="Configurer le plugin Expo" stepNumber={3}>
    ...
    <Warning>
      **Important** : Vous devez définir `bundleIdentifier` pour iOS et `package` pour Android dans votre fichier `app.json`. Ces identifiants sont requis pour que le SDK Auth0 puisse configurer correctement les projets natifs. Si vous ne précisez pas de schéma personnalisé, le SDK utilisera l’identifiant du bundle comme schéma d’URL.
    </Warning>
  ...
  <Step title="Configurer votre application Auth0" stepNumber={4}>
    ...
    6. Mettez à jour la valeur `domain` dans la configuration du plugin de votre `app.json` en y indiquant votre domaine Auth0
    ...
    **URL de déconnexion autorisées :**
    ...
    <Warning>
      **Important** : Assurez-vous que le `customScheme` dans vos URL de rappel correspond exactement à la valeur indiquée dans la configuration du plugin de votre `app.json`, et que le chemin contient bien votre véritable `bundleIdentifier` (iOS) ou `package` (Android). Des valeurs qui ne correspondent pas feront échouer l’authentification.
    </Warning>
  </Step>
  ...
  <Step title="Configuration du composant d’application" stepNumber={5}>
    ...
    <Tabs>
      <Tab title="Basé sur Hooks (avec Provider)">
        Remplacez le contenu de `App.js` et encapsulez votre application dans le composant `Auth0Provider` :
        ...
        <Tip>
          `Auth0Provider` initialise le SDK et fournit le contexte d’authentification à tous les composants enfants via le hook `useAuth0`. Le paramètre `customScheme` doit correspondre à la valeur définie dans la configuration du plugin de votre fichier `app.json`.
        </Tip>
      </Tab>
      ...
      <Tab title="Fondé sur des classes (sans Provider)">
        ...
        <Tip>
          L’approche basée sur les classes ne nécessite pas `Auth0Provider`, puisqu’elle utilise directement l’instance de `Auth0`. L’option `customScheme` doit être fournie comme deuxième paramètre à `authorize()` et à `clearSession()`.
        </Tip>
      </Tab>
    </Tabs>

    <Info>
      La méthode `authorize()` ouvre Universal Login d’Auth0 dans un navigateur sécurisé (ASWebAuthenticationSession sur iOS, Chrome Custom Tabs sur Android). La méthode `clearSession()` déconnecte l’utilisateur et efface la session du navigateur ainsi que les identifiants stockés. Le paramètre `customScheme` doit correspondre à la valeur indiquée dans la configuration du plugin `app.json`.
    </Info>
  </Step>

  <Step title="Lancez votre application" stepNumber={6}>
    Créez et lancez votre application Expo sur un appareil ou un émulateur.
    ...
    **Exécutez ensuite sur votre plateforme cible :**
...
<Check>
  **Vérification**
  ...
  Vous devriez maintenant disposer d’une expérience de connexion Auth0 entièrement fonctionnelle sur votre appareil ou votre émulateur. L’application utilise une authentification sécurisée dans le navigateur et gère automatiquement les identifiants dans le stockage sécurisé de l’appareil.
</Check>
...
<Accordion title="Problèmes courants et solutions">
  ...
  **Correctif :**
  ...
  Sur iOS, les utilisateurs voient une boîte de dialogue de permission : *&quot;App Name&quot; Wants to Use &quot;auth0.com&quot; to Sign In*. Il s’agit du comportement attendu de `ASWebAuthenticationSession`. Les utilisateurs doivent appuyer sur **Continue** pour poursuivre.
...
<Accordion title="Récupération des jetons d’accès">
  Utilisez la méthode `getCredentials()` pour récupérer des jetons à utiliser dans vos appels d’API :
  ...
  <Tip>
    Ajoutez le scope `offline_access` pendant la connexion afin de recevoir un refresh token : `authorize({customScheme: 'auth0sample', scope: 'openid profile email offline_access'})`. Cela permet l’actualisation automatique du jeton.
  </Tip>
</Accordion>
...
<Accordion title="Déploiement en production avec EAS Build">
  ...
  **Créez le build de production :**
  ...
  # Ou build pour une plateforme précise
  ...
  * Prévoyez une gestion adéquate des erreurs en cas de panne réseau
  ...
  <Tip>
    Pour les applications de production, envisagez d’utiliser des URL de rappel HTTPS avec les Universal Links (iOS) et les App Links (Android) plutôt que des schémas personnalisés pour une sécurité accrue.
  </Tip>
</Accordion>
