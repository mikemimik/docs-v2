...
description: Ce guide explique comment intégrer Auth0 à une application Ionic Angular avec Capacitor à l’aide de l’Auth0 Angular SDK.
mode: wide
---
...
<Accordion title="Utiliser l’IA pour intégrer Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  Si vous utilisez un assistant IA de programmation comme Claude Code, Cursor ou GitHub Copilot, vous pouvez ajouter l’authentification Auth0 automatiquement en quelques minutes à l’aide des [agent skills](https://agentskills.io/home).
...
Ce Quickstart explique comment ajouter l’authentification Auth0 à une application Ionic Angular avec Capacitor. Vous créerez une application adaptée aux appareils mobiles avec des fonctionnalités de login, de logout et de profil utilisateur à l’aide du Auth0 Angular SDK et des plugins de navigateur natifs de Capacitor.
...
<Steps>
  ...
  <Step title="Installez le SDK Auth0 Angular et les plugins Capacitor" stepNumber={2}>
    Installez le SDK Auth0 Angular ainsi que les plugins Browser et App de Capacitor :
    ...
    * [`@capacitor/app`](https://capacitorjs.com/docs/apis/app) - gère les URI de rappel de lien profond d’Auth0 après l’authentification
  ...
  <Step title="Configurez votre Auth0 App" stepNumber={3}>
    ...
    <Tabs>
      <Tab title="Configuration rapide (recommandée)">
        Créez une Auth0 App Native et copiez le fichier d’environnement prérempli avec les valeurs de configuration appropriées.
        ...
        Après avoir créé votre application, mettez à jour les **Allowed Callback URLs** et les **Allowed Logout URLs** dans l’onglet **Settings** du [Auth0 Dashboard](https://manage.auth0.com/dashboard/). Remplacez `YOUR_PACKAGE_ID` par la valeur de `appId` dans votre `capacitor.config.ts` (par défaut : `io.ionic.starter`) :
      ...
      <Tab title="CLI">
        Exécutez la commande suivante dans le répertoire racine de votre projet pour créer une application Native d’Auth0 et générer un fichier d’environnement. Mettez `PACKAGE_ID` à jour si votre `capacitor.config.ts` utilise un `appId` différent :
      ...
      <Tab title="Dashboard">
        ...
        3. Saisissez un nom pour votre application (p. ex. `Ionic Angular App`), sélectionnez **Native** comme type d’application, puis cliquez sur **Create**
        4. Ouvrez l’onglet **Settings** et prenez en note les valeurs de **Domain** et de **Client ID**
        5. Remplacez `YOUR_AUTH0_APP_DOMAIN` et `YOUR_AUTH0_APP_CLIENT_ID` dans `src/environments/environment.ts` par vos valeurs

        <Info>
          Dans ce Quickstart, `YOUR_PACKAGE_ID` correspond au champ `appId` de votre fichier `capacitor.config.ts`. La valeur par défaut est `io.ionic.starter`. Consultez le [schéma de configuration de Capacitor](https://capacitorjs.com/docs/config#schema) pour en savoir plus.
        </Info>
        ...
        **Origines autorisées :**
        ...
        <Info>
          ...
          **Allowed Logout URLs** sont essentielles pour offrir une expérience utilisateur fluide lors de la déconnexion. Sans URL correspondante, les utilisateurs obtiendront une erreur au moment de se déconnecter.
  ...
  <Step title="Configurer le module Auth0" stepNumber={4}>
    ...
    * `useRefreshTokens: true` — **obligatoire** pour les appareils mobiles. Les applications Capacitor ne peuvent pas utiliser l’authentification silencieuse basée sur des iframes; les refresh tokens servent donc à renouveler les sessions.
    * `useRefreshTokensFallback: false` — **obligatoire** pour les appareils mobiles. Empêche le SDK d’utiliser l’authentification silencieuse basée sur des iframes comme solution de secours, puisqu’elle n’est pas offerte dans les applications natives.
    * `authorizationParams.redirect_uri` — utilise le schéma d’URL personnalisé de votre application pour rediriger l’utilisateur après l’authentification.

    <Warning>
      Pour enregistrer l’authentification après la fermeture puis la réouverture de l’application, vous pouvez définir `cacheLocation` sur `localstorage`, mais veuillez prendre connaissance des [risques liés au stockage de jetons dans localstorage](https://auth0.com/docs/libraries/auth0-single-page-app-sdk#change-storage-options). De plus, `localstorage` doit être considéré comme **transitoire** dans les applications Capacitor. Veuillez consulter les [directives sur le stockage dans la documentation de Capacitor](https://capacitorjs.com/docs/guides/storage#why-cant-i-just-use-localstorage-or-indexeddb).
    </Warning>
  </Step>
  ...
  <Step title="Créer des composants Login, Logout et Profil" stepNumber={5}>
    ...
    <Info>
      Le callback de l’événement `appUrlOpen` dans le composant App est encapsulé dans `this.ngZone.run()`. C’est **obligatoire**, car les callbacks du plugin Capacitor s’exécutent à l’extérieur de la zone d’Angular et, sans cela, Angular ne détectera pas les changements d’état d’authentification après la connexion. Consultez [Utiliser Angular avec Capacitor](https://capacitorjs.com/docs/guides/angular) pour en savoir plus.
    </Info>
  </Step>

  <Step title="Lancez votre application" stepNumber={6}>
    Essayez d’abord dans le navigateur :
    ...
    <Warning>
      Lorsque vous exécutez l’application dans le navigateur avec `ionic serve`, la redirection vers le schéma d’URL personnalisé ne fonctionnera pas, car les navigateurs ne prennent pas en charge les schémas d’URL personnalisés. Pour tester dans le navigateur, remplacez temporairement `redirect_uri` par `http://localhost:8100` dans `src/main.ts`, puis ajoutez `http://localhost:8100` à **Allowed Callback URLs** et à **Allowed Logout URLs** de votre application Auth0 dans le Dashboard. N’oubliez pas d’annuler cette modification avant de générer l’application pour Native.
    </Warning>
    ...
    <Info>
      Vous devez ajouter les plateformes natives avec `npx cap add` avant de pouvoir les exécuter. Vous n’avez à le faire qu’une seule fois par plateforme. Assurez-vous que votre schéma d’URL personnalisé est enregistré (voir Utilisation avancée ci-dessous).
    </Info>
  </Step>
</Steps>
...
<Accordion title="Utiliser l’approche NgModule traditionnelle">
  ...
  <Note>
    Lorsque vous utilisez des NgModules, injectez `AuthService` au moyen du constructeur (`constructor(public auth: AuthService)`) plutôt qu’avec `inject()`. Utilisez `*ngIf="auth.user$ | async as user"` dans les gabarits au lieu de la syntaxe de contrôle de flux `@if`. Les composants doivent être déclarés dans le module plutôt que définis avec `standalone: true`.
  </Note>
</Accordion>
...
<Accordion title="Configuration d’un schéma d’URL personnalisé">
  ...
  Remplacez `YOUR_PACKAGE_ID` par la valeur `appId` de `capacitor.config.ts`. Pour en savoir plus, consultez [Defining a Custom URL Scheme](https://developer.apple.com/documentation/xcode/defining-a-custom-url-scheme-for-your-app).
  ...
  Remplacez `YOUR_PACKAGE_ID` par la valeur `appId` de `capacitor.config.ts`. Pour en savoir plus, consultez [Create Deep Links to App Content](https://developer.android.com/training/app-links/deep-linking).
...
<Accordion title="Appeler des API protégées">
  Configurez l’intercepteur HTTP pour qu’il ajoute automatiquement des jetons d’accès aux appels d’API :
  ...
  Effectuez ensuite des appels d’API à l’aide du `HttpClient` d’Angular : l’intercepteur ajoute automatiquement le jeton Bearer :
...
<Accordion title="Problèmes courants et solutions">
  ...
  **Solution :** Assurez-vous que le callback de l’événement `appUrlOpen` est exécuté dans `this.ngZone.run()`. Sinon, Angular ne détectera pas les changements d’état déclenchés par `handleRedirectCallback`. Consultez [Using Angular with Capacitor](https://capacitorjs.com/docs/guides/angular).
  ...
  **Correctif :**
  ...
  Activez `cacheLocation: 'localstorage'` dans la configuration `provideAuth0` pour enregistrer les jetons entre les redémarrages de l’application. Gardez à l’esprit les [implications sur le plan de la sécurité](https://auth0.com/docs/libraries/auth0-single-page-app-sdk#change-storage-options) et les [limitations du stockage de Capacitor](https://capacitorjs.com/docs/guides/storage#why-cant-i-just-use-localstorage-or-indexeddb).

  ### L’Observable ne s’exécute jamais

  Toutes les méthodes `AuthService` renvoient des Observables cold. Vous devez appeler `.subscribe()` pour qu’elles s’exécutent. Si `loginWithRedirect()` ou `logout()` semble ne rien faire, vérifiez que `.subscribe()` est bien chaîné à la fin.
</Accordion>
...
* [Exemple d’application](https://github.com/auth0-samples/auth0-ionic-samples/tree/main/angular) — Une application Ionic Angular complète avec une intégration Auth0
* [Documentation du Auth0 Angular SDK](https://github.com/auth0/auth0-angular) — Référence complète du SDK et exemples
