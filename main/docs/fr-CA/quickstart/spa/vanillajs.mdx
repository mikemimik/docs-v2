---
mode: wide
description: Ce guide montre comment intégrer Auth0, ajouter l’authentification et afficher les renseignements du profil utilisateur dans une application monopage (SPA) qui utilise du JavaScript pur, à l’aide de l’Auth0 SPA SDK.
...
<Accordion title="Requête à l’IA" defaultOpen icon="microchip-ai" iconType="sharp-solid">
  **Vous utilisez l&#39;IA pour intégrer Auth0 ?** Ajoutez cette invite à Cursor, Windsurf, Copilot, Claude Code ou votre IDE IA préféré pour accélérer le développement.
  ...
  # Vérifiez s’il existe déjà un projet
  ...
  async function initAuth0() {
    try {
      ...
      // Vérifier si l'utilisateur revient après l'authentification
  ...
  * {
  ...
  Problème : L'agent IA ne parvient pas à exécuter `npm run dev` parce que le terminal n'est pas dans le répertoire auth0-vanillajs, même lorsque pwd affiche le bon chemin
  Solution : Utilisez toujours un changement de répertoire explicite avec le chemin absolu avant d'exécuter les commandes npm :
...
<Note>
  ...
  **Outil de compilation :** Ce guide de démarrage rapide utilise **Vite** pour le développement. Vous pouvez aussi utiliser le SDK par l’entremise d’un CDN pour une configuration sans outil de compilation.
</Note>
...
Ce guide de démarrage rapide montre comment ajouter l’authentification Auth0 à une application JavaScript sans framework. Vous créerez une application monopage moderne dotée d’une fonctionnalité de connexion sécurisée à l’aide de JavaScript pur et de l’Auth0 SPA SDK.
...
<Steps>
  ...
  <Step title="Créer la structure HTML et la logique de l’application" stepNumber={4}>
...
<Accordion title="Gérer la connexion avec une fenêtre contextuelle">
  Pour une expérience utilisateur plus fluide, vous pouvez utiliser une connexion au moyen d’une fenêtre contextuelle :
