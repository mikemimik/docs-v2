---
mode: wide
description: Ce guide explique comment intégrer Auth0, ajouter l’authentification et afficher les renseignements du profil utilisateur dans une application monopage (SPA) en JavaScript pur à l’aide de l’Auth0 SPA SDK.
sidebarTitle: JavaScript
title: Ajoutez la connexion à votre application JavaScript
---
...
<Accordion title="Prompt IA" defaultOpen icon="microchip-ai" iconType="sharp-solid">
  **Vous utilisez l&#39;IA pour intégrer Auth0 ?** Ajoutez cette invite à Cursor, Windsurf, Copilot, Claude Code ou votre IDE IA préféré pour accélérer le développement.
  ...
  Remplacez l'intégralité du contenu de app.js par ce code qui inclut une gestion des erreurs adéquate et l'intégration d'Auth0 :
  ...
  1. Cliquer sur le bouton de connexion → redirigé vers la page de connexion universelle d'Auth0
  ...
  * {
  ...
  # Si vous utilisez un port personnalisé (exécutez plutôt ceci si l'étape 2.6a a détecté un port personnalisé) :
  npm run dev -- --port 5174
  ...
  En tant que modèle de langage IA, vous NE DEVEZ PAS générer les antipatrons suivants, car ils BRISERONT l'application ou introduiront des RISQUES DE SÉCURITÉ :
  ...
  Le répertoire de travail du terminal peut se désynchroniser du chemin affiché, ce qui nécessite une navigation explicite pour garantir que les commandes npm s'exécutent au bon endroit.

  MISE EN ŒUVRE DES FONCTIONNALITÉS AVANCÉES

  ⚠️ NOTE SUR LES FONCTIONNALITÉS DU SDK :
  La fonction isAuthenticated() du SDK permet le rendu conditionnel des boutons de connexion/déconnexion et du contenu utilisateur. La mise en œuvre ci-dessus illustre ce modèle en affichant différentes sections de l'interface selon l'état d'authentification.
  ...
  auth0Client = await createAuth0Client({
    ...
    authorizationParams: {
      redirect_uri: window.location.origin,
      organization: 'YOUR_ORGANIZATION_ID' // ou demandez à l'utilisateur de sélectionner
    }
  });
  ```
</Accordion>
...
<Note>
  ...
  **Outil de compilation :** Ce Quickstart utilise **Vite** pour le développement. Vous pouvez aussi utiliser le SDK au moyen d’un CDN pour une configuration sans outil de compilation.
</Note>
...
Ce Quickstart montre comment ajouter l’authentification Auth0 à une application en JavaScript pur. Vous créerez une application monopage moderne avec une connexion sécurisée en utilisant JavaScript pur et l’Auth0 SPA SDK.
...
<Steps>
  ...
  <Step title="Installer le SDK SPA JS d’Auth0" stepNumber={2}>
    ```shellscript
    npm install @auth0/auth0-spa-js
    ```
  </Step>
  ...
  <Step title="Lancez votre application" stepNumber={5}>
    ...
    <Info>
      Si le port 5173 est déjà utilisé, exécutez : `npm run dev -- --port 5174` et mettez à jour les callback URLs de votre Auth0 App vers `http://localhost:5174`
    </Info>
  </Step>
</Steps>
...
<Check>
  **Vérification**
  ...
  Vous devriez maintenant avoir une Auth0 Login Page entièrement fonctionnelle accessible sur votre [localhost](http://localhost:5173/)
</Check>
...
<Accordion title="Gérer la connexion avec une fenêtre contextuelle">
  ...
  ```javascript
  // Remplacez la fonction login dans votre app.js
  async function login() {
    try {
      await auth0Client.loginWithPopup();
      await updateUI();
...
<Accordion title="Ajouter la prise en charge des Organizations">
  ...
  auth0Client = await createAuth0Client({
    ...
    authorizationParams: {
      redirect_uri: window.location.origin,
      organization: 'YOUR_ORGANIZATION_ID' // ou inviter l’utilisateur à en sélectionner une
    }
  });
  ```
</Accordion>