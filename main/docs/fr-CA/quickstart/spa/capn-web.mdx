...
<Accordion title="Instruction à l’IA" defaultOpen icon="microchip-ai" iconType="sharp-solid">
  **Vous utilisez l&#39;IA pour intégrer Auth0 ?** Ajoutez cette invite à Cursor, Windsurf, Copilot, Claude Code ou votre IDE propulsé par l&#39;IA préféré pour accélérer le développement.
  ...
  - Créer une application Auth0 (de type SPA)
  ...
  Cela vous permettra de déterminer si vous êtes dans le répertoire principal ou un sous-répertoire, et si le projet a été créé dans le répertoire courant ou dans un nouveau sous-répertoire.
  ...
  Étape 3.3 : Afficher les instructions de configuration manuelle
    ...
    echo "8. Mettez à jour le fichier .env avec votre domaine, Client ID et audience API"
  ...
  EXIGENCES DE SÉCURITÉ ET BONNES PRATIQUES
...
Ce Quickstart montre comment ajouter l’authentification Auth0 à une application Cap&#39;n Web. Vous créerez une application web moderne fondée sur RPC avec une fonctionnalité de connexion sécurisée à l’aide du framework Javascript de Cap&#39;n Web et de l’Auth0 SPA SDK.
...
<Steps>
  <Step title="Créer un nouveau projet" stepNumber={1}>
    ...
    Initialisez le projet et configurez-le pour qu’il utilise les modules ES :
      ...
      <Tab title="Dashboard">
        ...
        **Origines Web autorisées :**
        ...
        <Info>
          ...
          Les **Allowed Logout URLs** sont essentielles pour offrir une user experience fluide au moment de la déconnexion. Sans URL correspondante, les utilisateurs ne seront pas redirigés vers votre application après la déconnexion et resteront plutôt sur une page Auth0 générique.
  ...
  <Step title="Créer le serveur" stepNumber={5}>
    ...
    const server = http.createServer(async (req, res) => {
      ...
      // Servir le SDK JS Auth0 SPA depuis node_modules
      if (req.url === '/@auth0/auth0-spa-js') {
        const modulePath = join(__dirname, '../node_modules/@auth0/auth0-spa-js/dist/auth0-spa-js.production.esm.js');
        const js = readFileSync(modulePath, 'utf8');
        res.setHeader('Content-Type', 'application/javascript');
...
<Accordion title="Gestion des connexions WebSocket">
  Gérez correctement les connexions WebSocket avec une reconnexion automatique :
  ...
  // Se connecter une fois l’authentification réussie
  if (isAuthenticated) {
...
<Accordion title="Intégration de base de données">
  Remplacez le stockage en mémoire par une base de données pour une utilisation en production :
