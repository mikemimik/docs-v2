---
mode: wide
description: Ce guide explique comment intégrer Auth0, ajouter l’authentification et afficher les renseignements du profil utilisateur dans une application monopage (SPA) qui utilise Cap'n Web RPC, à l’aide du SDK Auth0 SPA.
...
<Callout icon="pencil" color="#FFC107" iconType="solid">
  Ce Quickstart est actuellement en **Beta**. Nous serions ravis de recevoir vos commentaires !
</Callout>

<Accordion title="Prompt IA" defaultOpen icon="microchip-ai" iconType="sharp-solid">
  **Vous utilisez l&#39;IA pour intégrer Auth0 ?** Ajoutez ce prompt à Cursor, Windsurf, Copilot, Claude Code ou votre IDE IA préféré pour accélérer le développement.
  ...
  1. VÉRIFIER LE PROJET EXISTANT EN PREMIER : Avant de créer un nouveau projet, vérifiez si le répertoire courant contient déjà un projet Cap'n Web (package.json avec les dépendances capnweb).
  ...
  - Implémenter des points de terminaison sécurisés pour la gestion du profil
  ...
  - Ne JAMAIS accepter des appels RPC non authentifiés
  ...
  2. TOUJOURS vérifier le répertoire courant avec `pwd` avant de continuer
  3. Si vous travaillez avec un projet existant : Restez dans le répertoire courant
  ...
  Cela vous aidera à déterminer si vous êtes dans le répertoire principal ou un sous-répertoire, et si le projet a été créé dans le répertoire courant ou dans un nouveau sous-répertoire.
  ...
  Étape 4 : Implémenter le serveur WebSocket sécurisé avec validation JWT
  ...
  Remplacez l'intégralité du contenu par l'implémentation sécurisée du serveur WebSocket :
  ...
  - S'assurer que les paramètres de l'Auth0 application correspondent à vos URL locales
...
Ce Quickstart montre comment ajouter l’authentification Auth0 à une application Cap&#39;n Web. Vous créerez une application Web moderne basée sur RPC avec une fonctionnalité de connexion sécurisée à l’aide du framework Javascript de Cap&#39;n Web et du SDK Auth0 SPA.
...
<Steps>
  <Step title="Créer un nouveau projet" stepNumber={1}>
    ...
    Initialisez le projet et configurez-le pour utiliser des modules ES :
  ...
  <Step title="Installer les dépendances" stepNumber={2}>
    Installez Cap&#39;n Web et ses dépendances principales :
      ...
      <Tab title="Dashboard">
        ...
        **Origines Web autorisées :**
        ...
        <Info>
          ...
          **Allowed Logout URLs** est essentiel pour offrir une user experience fluide au moment de la déconnexion. Sans URL correspondante, les utilisateurs ne seront pas redirigés vers votre application après la déconnexion et resteront plutôt sur une page Auth0 générique.
        ...
        **Obligatoire : Créer une Auth0 API**
        ...
        1. Dans le Auth0 Dashboard, allez à **APIs** &gt; **Create API**
        ...
        <Warning>
          Sans créer d’Auth0 API, vous obtiendrez une erreur &quot;access&#95;denied&quot; pendant le login. L’identificateur de l’API doit correspondre exactement à votre variable d’environnement `AUTH0_AUDIENCE`.
        </Warning>
      </Tab>
    </Tabs>
  </Step>
...
<Accordion title="Gestion des connexions WebSocket">
  Mettez en place une gestion appropriée des connexions WebSocket avec reconnexion automatique :
  ...
  const connectionManager = new RpcConnectionManager(
    `${protocol}//${host}/api`,
    {
      ...
      onReconnect: async (api) => {
        // Actualiser l'interface ou recharger les données après la reconnexion
