...
sidebarTitle: React
description: Ce guide explique comment intégrer Auth0, ajouter l’authentification et afficher les renseignements du profil utilisateur dans une application monopage (SPA) utilisant React, à l’aide de l’Auth0 React SDK.
mode: wide
---
...
<Note>
  ...
  **Compatibilité des versions de React :** Ce Quickstart fonctionne avec **React 18.x** et les versions ultérieures.
</Note>
...
Ce Quickstart explique comment ajouter l’authentification Auth0 à une application React. Vous créerez une application monopage sécurisée avec des fonctionnalités de login, de logout et de profil utilisateur à l’aide du Auth0 React SDK.
...
<Steps>
  <Step title="Créer un nouveau projet" stepNumber={1}>
    Créez un nouveau projet React pour suivre ce Quickstart
  ...
  <Step title="Installer le SDK Auth0 React" stepNumber={2}>
    ```shellscript
    ...
    <Tabs>
      ...
      <Tab title="CLI">
        ...
        <Note>
          ...
          2. Créer une application Auth0 Single Page Application configurée pour `http://localhost:5173`
...
<Check>
  **Vérification**
  ...
  Vous devriez maintenant avoir une page de connexion Auth0 pleinement fonctionnelle sur votre [localhost](http://localhost:5173/)
</Check>
...
<Accordion title="Protéger les routes avec React Router">
...
<Accordion title="Appeler des API protégées">
  Configurez votre Auth0Provider pour inclure l’audience d’une API et utilisez la méthode `getAccessTokenSilently` :
  ...
  Ensuite, effectuez des appels d’API authentifiés :
...
<Accordion title="Utiliser Auth0 avec des hooks personnalisés">
  Créez des hooks personnalisés réutilisables pour les schémas d’authentification courants :
