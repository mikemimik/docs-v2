---
title: Ajouter la fonctionnalité de connexion à votre application Angular
sidebarTitle: Angular
description: Ce guide montre comment intégrer Auth0, ajouter l’authentification et afficher les informations du profil utilisateur dans n’importe quelle application Angular à l’aide du SDK Auth0 Angular.
mode: wide
---
...
<Accordion title="Utiliser l’IA pour intégrer Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  Si vous utilisez un assistant IA de codage comme Claude Code, Cursor ou GitHub Copilot, vous pouvez ajouter l’authentification Auth0 automatiquement en quelques minutes grâce à [Agent Skills](https://agentskills.io/home).
  ...
  **Ensuite, demandez à votre assistant IA :**
  ...
  ```text
  Ajoute l’authentification Auth0 à mon application Angular
  ```

  Votre assistant IA créera automatiquement votre application Auth0, récupérera les identifiants, installera `@auth0/auth0-angular`, créera des gardes de routes et des intercepteurs HTTP, puis configurera votre environnement. [Documentation complète d’Agent Skills →](/docs/fr-CA/quickstart/agent-skills)
</Accordion>
...
Ce Quickstart montre comment ajouter l’authentification Auth0 à une application Angular. Vous créerez une application monopage sécurisée offrant des fonctionnalités de connexion et de déconnexion à l’aide du système d’injection de dépendances d’Angular et du SDK Auth0 Angular.
...
<Steps>
  <Step title="Créer un nouveau projet" stepNumber={1}>
    Créez un nouveau projet Angular dans le cadre de ce Quickstart
  ...
  <Step title="Configurez votre Auth0 App" stepNumber={3}>
    ...
    <Tabs>
      <Tab title="Configuration rapide (recommandée)">
        Créez une Auth0 App, puis copiez le fichier `.env` prérempli avec les bonnes valeurs de configuration.
      ...
      <Tab title="Dashboard">
        Avant de commencer, créez un fichier d’environnement pour votre projet
        ...
        4. Allez à l’onglet **Settings** sur la page Détails de l’application
        ...
        **Origines Web autorisées :**
        ...
        <Info>
          **Allowed Callback URLs** constituent une mesure de sécurité essentielle pour garantir que les utilisateurs reviennent en toute sécurité à votre application après l’authentification. Sans URL correspondante, le processus de connexion échouera, et les utilisateurs verront une page d’erreur Auth0 au lieu d’accéder à votre application.

          **Allowed Logout URLs** sont essentielles pour offrir une expérience utilisateur fluide lors de la déconnexion. Sans URL correspondante, les utilisateurs ne seront pas redirigés vers votre application après la déconnexion et se retrouveront plutôt sur une page générique d’Auth0.
  ...
  <Step title="Configurer le module Auth0" stepNumber={4}>
    Une fois votre fichier d’environnement créé à l’étape précédente, ajoutez `provideAuth0` au tableau providers dans votre `app.config.ts` :
    ...
    <Note>
      ...
      Les projets Angular 20+ incluent aussi `provideBrowserGlobalErrorListeners()` dans ce fichier — laissez-le dans le tableau aux côtés de `provideAuth0`. Aucune modification de `main.ts` n’est nécessaire.
    </Note>
  </Step>
  ...
  <Step title="Créer des composants Login, Logout et Profil" stepNumber={5}>
    ...
    <Tabs>
      <Tab title="Composant d’application">
        Remplacez le contenu du fichier composant de votre application (`src/app/app.ts` dans Angular 20+, ou `src/app/app.component.ts` dans Angular 19) :
...
<Accordion title="Utiliser l’approche NgModule traditionnelle">
  Si vous avez créé votre projet avec `--standalone=false` ou si vous préférez utiliser des NgModules plutôt que des composants autonomes, voici comment configurer le SDK dans des projets générés par la CLI d’Angular 20+ :
  ...
  <Note>
    Si vous utilisez **Angular 19**, vos fichiers générés sont généralement `app.component.ts`, `app.module.ts` et `app-routing.module.ts`, et `main.ts` utilise `platformBrowserDynamic` au lieu de `platformBrowser`.
    ...
    **Angular 20+** génère `app.ts`, `app-module.ts` et `app-routing-module.ts` (sans points dans les noms de fichier), et `main.ts` utilise `platformBrowser`, comme illustré ci-dessus.
    ...
    Dans tous les cas, les projets NgModule s’amorcent avec `bootstrapModule()` plutôt qu’avec l’approche `bootstrapApplication()` présentée dans le Quickstart principal.
  </Note>
</Accordion>
...
<Accordion title="Appeler des API protégées">
  Configurez l’interceptor HTTP pour ajouter automatiquement des jetons aux appels d’API. Ajoutez `provideHttpClient` avec l’interceptor Auth0 et une `audience` à votre `app.config.ts` :
