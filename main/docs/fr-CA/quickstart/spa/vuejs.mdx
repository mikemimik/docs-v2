...
sidebarTitle: Vue
description: Ce guide montre comment intégrer Auth0, ajouter l’authentification et afficher les renseignements du profil utilisateur dans n’importe quelle application Vue à l’aide de l’Auth0 Vue SDK.
mode: wide
---
...
Ce Quickstart montre comment intégrer l’authentification Auth0 à une application Vue.js 3. Vous créerez une application monopage adaptative avec une authentification des utilisateurs sécurisée à l’aide de l’API de composition de Vue et du Auth0 Vue SDK.
...
<Steps>
  <Step title="Créer un nouveau projet" stepNumber={1}>
    Créez un nouveau projet Vue 3 dans le cadre de ce Quickstart
  ...
  <Step title="Installez le SDK Auth0 Vue" stepNumber={2}>
    ```shellscript
  ...
  <Step title="Configurez votre Auth0 App" stepNumber={3}>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        3. Dans la popup, entrez un nom pour votre application, sélectionnez `Single Page Web Application` comme type d’application, puis cliquez sur **Create**
        ...
        **Allowed Web Origins :**
        ...
        <Info>
          Les **Allowed Callback URLs** constituent une mesure de sécurité essentielle pour garantir que les utilisateurs sont redirigés vers votre application en toute sécurité après l’authentication. Sans URL correspondante, le processus de login échouera et les utilisateurs seront bloqués sur une page d’erreur Auth0 au lieu d’accéder à votre application.
  ...
  <Step title="Lancez votre application" stepNumber={6}>
    ...
    <Info>
      Si le port 5173 est déjà utilisé, exécutez : `npm run dev -- --port 5174` et mettez à jour les URL de callback de votre Auth0 App à `http://localhost:5174`
    </Info>
  </Step>
</Steps>
...
<Accordion title="Appeler des API protégées">
  ...
  Effectuez ensuite des appels d’API authentifiés dans vos composants :
...
<Accordion title="Utiliser des composables pour la logique d’authentification">
  Créez des composables réutilisables pour des scénarios d’authentification courants :
