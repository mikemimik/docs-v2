---
mode: wide
description: Ce guide montre comment intégrer Auth0, ajouter l’authentification et afficher les renseignements du profil utilisateur dans une application monopage (SPA) qui utilise Svelte, à l’aide du SDK Auth0 SPA JS.
sidebarTitle: Svelte
...
<Callout icon="pencil" color="#FFC107" iconType="solid">
  Ce Quickstart est actuellement en **bêta**. N’hésitez pas à nous faire part de vos commentaires!
</Callout>

<Accordion title="Consigne d’IA" defaultOpen icon="microchip-ai" iconType="sharp-solid">
  **Vous utilisez l’IA pour intégrer Auth0 ?** Ajoutez cette invite dans Cursor, Windsurf, Copilot, Claude Code ou votre EDI préféré propulsé par l’IA pour accélérer le développement.

  ```markdown expandable
  Integrate the Auth0 SPA JS SDK into a Svelte app

  PERSONA IA ET OBJECTIF PRINCIPAL
  Vous êtes un assistant utile pour l'intégration du SDK Auth0. Votre fonction principale est d'exécuter des commandes pour configurer un environnement de développement Auth0 avec Svelte. Votre fonction secondaire est de modifier les fichiers créés par ces commandes shell.

  INSTRUCTIONS COMPORTEMENTALES CRITIQUES
  1. VÉRIFIER D'ABORD LE PROJET EXISTANT : Avant de créer un nouveau projet, vérifiez si le répertoire courant contient déjà une application Svelte (package.json avec des dépendances Svelte). Si c'est le cas, ignorez la création du projet et travaillez avec le projet existant.
  ...
  5. CRÉER UNE BELLE INTERFACE : Vous DEVEZ créer une interface de connexion visuellement attrayante et moderne avec un style approprié, des animations et l'image de marque Auth0.
  ...
  APRÈS l'exécution réussie de la commande à l'étape 1, vous effectuerez les opérations de fichiers suivantes dans le répertoire du projet.
  ...
  - Créer une application monopage configurée pour http://localhost:5173
  ...
  2. N'omettez JAMAIS authorizationParams avec redirect_uri lors de la configuration du client Auth0. Sans cela, le flux de connexion échouera.
...
<Note>
  ...
  **Compatibilité des versions de Svelte :** Ce Quickstart fonctionne avec **Svelte 5.x** et les versions ultérieures.
</Note>
...
Ce Quickstart montre comment ajouter l’authentification Auth0 à une application Svelte. Vous créerez une application à page unique sécurisée avec des fonctionnalités de connexion, de logout et de profil utilisateur à l’aide du SDK Auth0 SPA JS.
...
<Steps>
  <Step title="Créer un nouveau projet" stepNumber={1}>
    Créez un projet Svelte pour ce Quickstart
  ...
  <Step title="Installez l’Auth0 SPA SDK" stepNumber={2}>
    ```shellscript
  ...
  <Step title="Créer la boutique d’Auth0" stepNumber={4}>
...
<Accordion title="La connexion ne fonctionne pas">
  Si vous cliquez sur le bouton de connexion et qu’il ne se passe rien :
...
<Tip>
  Vous êtes toujours bloqué? Consultez la [Auth0 Community](https://community.auth0.com/) ou le [Discord de SvelteKit](https://svelte.dev/chat) pour obtenir de l’aide.
</Tip>