---
mode: wide
description: "Ce guide montre comment intégrer Auth0, ajouter l’authentification et afficher les renseignements du profil utilisateur dans une application monopage (SPA) utilisant Svelte, à l’aide du SDK Auth0 SPA JS."
sidebarTitle: Svelte
...
<Accordion title="Requête à l’IA" defaultOpen icon="microchip-ai" iconType="sharp-solid">
  **Vous utilisez l’IA pour intégrer Auth0 ?** Ajoutez cette consigne dans Cursor, Windsurf, Copilot, Claude Code ou votre IDE préféré propulsé par l’IA pour accélérer le développement.
  ...
  APRÈS l'exécution réussie de la commande à l'étape 1, effectuez les opérations de fichiers suivantes dans le répertoire du projet.
  ...
  1. NE JAMAIS coder en dur le domaine Auth0 ou le Client ID directement dans le code. Il s'agit d'une faille de sécurité majeure.
  2. NE JAMAIS omettre authorizationParams avec redirect_uri lors de la configuration du client Auth0. Sans cela, le flux de connexion échouera.
  3. NE JAMAIS suggérer de gérer manuellement les jetons ou d'appeler les API Auth0 avec fetch pour l'authentification de base. Le SDK est conçu pour gérer cette complexité de manière sécurisée.
  ...
  4. Vous DEVEZ démontrer les fonctionnalités en utilisant les stores Svelte pour la gestion de l'état.
  ```
</Accordion>
...
<Note>
  ...
  **Compatibilité des versions de Svelte :** Ce Quickstart fonctionne avec **Svelte 5.x** et les versions plus récentes.
</Note>
...
Ce Quickstart montre comment ajouter l’authentification Auth0 à une application Svelte. Vous créerez une application monopage sécurisée avec des fonctionnalités de connexion, de logout et de profil utilisateur à l’aide du SDK Auth0 SPA JS.
...
<Steps>
  ...
  <Step title="Installer Auth0 SPA SDK" stepNumber={2}>
    ```shellscript
      ...
      <Tab title="Dashboard">
        ...
        **Allowed Web Origins:**
        ...
        <Info>
          ...
          **Allowed Logout URLs** sont essentielles pour offrir une expérience utilisateur fluide au moment de la déconnexion. Sans URL correspondante, les utilisateurs ne seront pas redirigés vers votre application après la déconnexion et se retrouveront plutôt sur une page Auth0 générique.
  ...
  <Step title="Créer le store d’Auth0" stepNumber={4}>
    ...
    Ajoutez le code suivant pour gérer l’état d’authentification
...
<Check>
  **Vérification**
  ...
  Vous devriez maintenant avoir une page de connexion Auth0 entièrement fonctionnelle sur [localhost](http://localhost:5173/)
</Check>
...
<Accordion title="La connexion ne fonctionne pas">
  ...
  4. Assurez-vous que `VITE_AUTH0_DOMAIN` contient uniquement le domaine (p. ex. `tenant.auth0.com`), sans `https://`
</Accordion>
...
<Tip>
  Vous êtes toujours bloqué? Consultez [Auth0 Community](https://community.auth0.com/) ou [SvelteKit Discord](https://svelte.dev/chat) pour obtenir de l’aide.
</Tip>