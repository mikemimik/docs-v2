---
mode: wide
description: Ce guide montre comment intégrer Auth0 à une application Web Flutter à l’aide de l’Auth0 Flutter SDK.
sidebarTitle: Flutter (Web)
title: Ajouter l’ouverture de session à votre application Web Flutter
---
...
Ce Quickstart montre comment ajouter l’authentification Auth0 à une application Web Flutter. Vous créerez une application monopage sécurisée avec des fonctionnalités de connexion, de déconnexion et de profil d’utilisateur à l’aide du SDK Auth0 Flutter.
...
<Steps>
  ...
  <Step title="Installez le SDK Auth0 Flutter" stepNumber={2}>
    ...
    Le SDK nécessite que la bibliothèque Auth0 SPA JS soit chargée dans votre application Web. Ajoutez la balise `<script>` suivante dans votre fichier `web/index.html`, avant la balise fermante `</body>` :
    ...
    <Warning>
      Le script Auth0 SPA JS est requis pour que le SDK Flutter Web fonctionne. Sans lui, l’authentification ne fonctionnera pas.
    </Warning>
  </Step>
    ...
    <Tabs>
      <Tab title="CLI">
        ...
        **Windows (PowerShell) :**
        ...
        Copiez les valeurs **domain** et **client&#95;id** figurant dans la sortie. Vous les utiliserez à l’étape suivante.
      ...
      <Tab title="Dashboard">
        ...
        4. Sélectionnez **Single Page Web Applications** et cliquez sur **Create**
        ...
        <Info>
          Les **Allowed Callback URLs** constituent une mesure de sécurité essentielle pour garantir que les utilisateurs sont redirigés de façon sécuritaire vers votre application après l’authentification.

          Les **Allowed Logout URLs** sont essentielles pour offrir une expérience utilisateur fluide au moment de la déconnexion.
  ...
  <Step title="Configurer le SDK" stepNumber={4}>
    Créez une instance de la classe `Auth0Web` à l’aide de vos valeurs Auth0 **domain** et **client ID**.
    ...
    <Tip>
      Définir `cacheLocation: CacheLocation.localStorage` permet de conserver des sessions persistantes d’un rechargement de page à l’autre.
    </Tip>
  </Step>
  ...
  <Step title="Créer la vue principale" stepNumber={5}>
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildInfoRow('Courriel', credentials.user.email ?? 'S/O'),
                      _buildInfoRow('Nom', credentials.user.name ?? 'S/O'),
                      _buildInfoRow('Pseudonyme', credentials.user.nickname ?? 'S/O'),
                      _buildInfoRow('ID utilisateur', credentials.user.sub),
                      const SizedBox(height: 24),
                      const Text(
                        'Raw User Object',
                        style: TextStyle(
    ...
    * `onLoad()` est appelé dans `initState()` pour gérer le callback d’authentification
    ...
    * Les renseignements du profil utilisateur sont accessibles via `credentials.user`
  </Step>

  <Step title="Lancez votre application" stepNumber={6}>
    Exécutez votre application Web Flutter sur le port 3000 :
...
<Check>
  **Checkpoint**
  ...
  Vous devriez maintenant avoir une page de connexion d’Auth0 entièrement fonctionnelle accessible à [http://localhost:3000](http://localhost:3000). Lorsque vous :
  ...
  3. cliquez sur « Voir le profil » - vos renseignements d’utilisateur s’affichent
  4. cliquez sur « Se déconnecter » - vous êtes déconnecté de votre application et d’Auth0
</Check>
...
<Accordion title="Paramètres de connexion personnalisés">
  Passez des paramètres supplémentaires au flux de connexion :
...
<AccordionGroup>
  <Accordion title="Problèmes courants et solutions">
    ...
    **Solution :** Assurez-vous que l’URL de rappel dans votre code correspond exactement à celle indiquée dans le Auth0 Dashboard :
    ...
    3. L’URL doit correspondre exactement (sans barre oblique finale, sauf si vous l’incluez dans le code)
    ...
    **Solutions :**
    ...
    1. Assurez-vous que **Allowed Web Origins** inclut `http://localhost:3000` dans le Auth0 Dashboard
    ...
    **Problème :** Le `state` ne correspond pas pendant le rappel d’authentification.

    **Solutions :**

    1. Videz le cache et le stockage local du navigateur
    ...
    **Solution :**
    ...
    1. Ajoutez `http://localhost:3000` à **Allowed Web Origins** dans le Auth0 Dashboard
    2. Assurez-vous d’utiliser le port 3000 (comme dans votre configuration)
  </Accordion>
</AccordionGroup>
...
Maintenant que l’authentification fonctionne, pensez aussi à explorer ce qui suit :

* **[Quickstart d’application native Flutter](/docs/fr-CA/quickstart/native/flutter/interactive)** - Créez une application mobile native avec Auth0
* **[Appeler des API protégées](https://auth0.com/docs/secure/tokens/access-tokens)** - Utilisez des jetons d’accès pour appeler vos API backend
...
* **[Ajouter des Social Connections](https://auth0.com/docs/connections/social)** - Activez Google, GitHub et d’autres connexions sociales
* **[Implement MFA](https://auth0.com/docs/secure/multi-factor-authentication)** - Ajoutez l’authentification multifactorielle
