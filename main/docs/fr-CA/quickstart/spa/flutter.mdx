...
Ce Quickstart montre comment ajouter l’authentification Auth0 à une application Web Flutter. Vous créerez une application monopage sécurisée avec des fonctionnalités de connexion, de déconnexion et de profil utilisateur à l’aide du SDK Auth0 Flutter.
...
<Steps>
    ...
    <Tabs>
      <Tab title="CLI">
        ...
        **Windows (PowerShell) :**
        ...
        Copiez les valeurs **domain** et **client&#95;id** affichées dans la sortie. Vous les utiliserez à l’étape suivante.
      ...
      <Tab title="Dashboard">
        ...
        3. Saisissez un nom pour votre application (p. ex., &quot;My Flutter Web App&quot;)
        ...
        <Info>
          ...
          Les **Allowed Logout URLs** sont essentielles pour offrir une expérience utilisateur fluide lors de la déconnexion.

          **Allowed Web Origins** est essentiel à l’authentification silencieuse. Sans ce paramètre, les utilisateurs seront déconnectés lorsqu’ils actualiseront la page ou reviendront plus tard dans votre application.
        </Info>
      </Tab>
    </Tabs>
  </Step>
  ...
  <Step title="Configurer le SDK" stepNumber={4}>
    Créez une instance de la classe `Auth0Web` avec les valeurs **domain** et **client ID** de votre instance Auth0.
    ...
    <Warning>
      Remplacez `YOUR_AUTH0_DOMAIN` par le domaine de votre tenant Auth0 (p. ex., `dev-abc123.us.auth0.com`) et `YOUR_AUTH0_CLIENT_ID` par le Client ID de votre application dans le tableau de bord.
    </Warning>
  ...
  <Step title="Créer la vue principale" stepNumber={5}>
    ...
    * `onLoad()` est appelée dans `initState()` pour gérer le callback d’authentification
  ...
  <Step title="Exécutez votre application" stepNumber={6}>
    ...
    <Info>
      Flutter 3.24.0 et les versions ultérieures prennent en charge la compilation WASM pour améliorer les performances :
...
<Check>
  **Checkpoint**
  ...
  Vous devriez maintenant avoir une page de connexion Auth0 entièrement fonctionnelle sur [http://localhost:3000](http://localhost:3000). Lorsque vous :

  1. Cliquez sur « Se connecter » - vous êtes redirigé vers la page Universal Login d’Auth0
  2. Terminez l’authentication - vous êtes redirigé vers votre application
...
<Accordion title="Authentification silencieuse">
  ...
  Future<void> silentLogin() async {
    try {
      // Tenter d’obtenir un jeton de façon silencieuse
...
<AccordionGroup>
  <Accordion title="Problèmes courants et solutions">
    ...
    **Problème :** La URL de rappel ne correspond pas à celle configurée dans Auth0.

    **Solution :** Assurez-vous que la URL de rappel dans votre code correspond exactement à celle configurée dans Auth0 Dashboard :
    ...
    **Solution :** Vérifiez que le script Auth0 SPA JS est chargé dans `web/index.html` :
    ...
    Ce script doit être présent avant la balise de fermeture `</body>`.

    ### L’utilisateur est déconnecté après le rechargement de la page

    **Problème :** La session de l’utilisateur ne persiste pas lors du rechargement de la page.
    ...
    **Problème :** Le paramètre state ne correspond pas pendant le callback d’authentification.
    ...
    **Problème :** Erreurs de partage de ressources entre origines multiples (CORS).
...
* **[Quickstart Flutter pour application native](/docs/fr-CA/quickstart/native/flutter/interactive)** - Créez une application mobile native avec Auth0
* **[Appeler des API protégées](https://auth0.com/docs/secure/tokens/access-tokens)** - Utilisez des jetons d’accès pour appeler vos API de backend
...
* **[Flutter Documentation](https://docs.flutter.dev/)** - En savoir plus sur Flutter
