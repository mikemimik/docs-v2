...
<Steps>
  ...
  <Step title="Ajouter Okta Spring Boot Starter" stepNumber={2}>
    Ajoutez la dépendance Okta Spring Boot Starter à votre projet. Cela inclut la prise en charge de la connexion OAuth2 de Spring Security avec une autoconfiguration propre à Auth0/Okta.
  ...
  <Step title="Configurer Auth0" stepNumber={3}>
    ...
    <Tabs>
      <Tab title="CLI">
        Exécutez la commande shell suivante à la racine de votre projet pour créer une application Auth0 et mettre à jour votre fichier `application.yml` :
      ...
      <Tab title="Dashboard">
        ...
        7. Remplacez les valeurs d’espace réservé dans `application.yml`.

        <Info>
          La **Callback URL** doit correspondre exactement. OAuth2 login de Spring Security utilise le chemin `/login/oauth2/code/okta` par défaut.
  ...
  <Step title="Configurer l’authentification" stepNumber={4}>
    Créez une configuration de sécurité qui active l’ouverture de session OAuth2 et gère la déconnexion d’Auth0. Les utilisateurs non authentifiés sont automatiquement redirigés vers la page de connexion d’Auth0.
  ...
  <Step title="Créer des contrôleurs et des vues" stepNumber={5}>
    ...
    <AuthCodeGroup>
      ...
      <head>
          <meta charset="UTF-8">
          <title>Connexion Auth0 Spring Boot</title>
      </head>
      <body>
          <h1>Exemple de connexion Auth0 Spring Boot</h1>
  ...
  <Step title="Lancez votre application" stepNumber={6}>
    ...
    Votre application est maintenant en cours d’exécution à l’adresse `http://localhost:3000`. Accédez à `http://localhost:3000/profile` pour déclencher le flux de connexion Auth0.
  </Step>
</Steps>

<Check>
  Vous devriez maintenant avoir une application Web Spring Boot entièrement fonctionnelle avec la connexion Auth0, accessible sur votre [localhost](http://localhost:3000/). La page d’accueil est publique, et l’accès à `/profile` redirige les utilisateurs non authentifiés vers la page de connexion Auth0.
</Check>
...
<Accordion title="Role-Based Access Control">
  Vous pouvez restreindre l’accès aux pages selon les rôles Auth0. Commencez par ajouter les rôles à l’ID token à l’aide d’une Action Auth0, puis utilisez `hasAuthority()` dans votre configuration de sécurité.

  ### Ajouter des rôles aux jetons

  1. Accédez à [Auth0 Dashboard](https://manage.auth0.com) → **Actions** → **Flows** → **Login**.
  2. Créez une Action personnalisée qui ajoute les rôles comme custom claim à l’ID token :
...
<Accordion title="Mapping personnalisé des autorités">
  Le starter Okta prend en charge le mapping personnalisé des autorités par l’intermédiaire de l’interface `AuthoritiesProvider`. Enregistrez un bean pour ajouter des objets `GrantedAuthority` personnalisés en fonction des attributs de l’utilisateur ou de sources de données externes.
  ...
  public class AuthoritiesConfig {
      ...
      AuthoritiesProvider customAuthoritiesProvider() {
          return (user, userRequest) -> {
              ...
              // Mapper le claim de rôles personnalisé vers les autorités Spring Security
...
<AccordionGroup>
  <Accordion title="Échec de la redirection vers la connexion - URL de rappel non valide">
    ...
    1. Accédez au [Auth0 Dashboard](https://manage.auth0.com) → **Applications** → votre application → **Settings**.
    2. Sous **Allowed Callback URLs**, ajoutez : `http://localhost:3000/login/oauth2/code/okta`.
    3. Cliquez sur **Save Changes**.
  </Accordion>

  <Accordion title="Issuer non valide au démarrage">
    L’application ne démarre pas, ou login échoue en raison d’une discordance de l’issuer.

    La valeur `okta.oauth2.issuer` doit être l’URL complète du tenant Auth0, y compris `https://` et un `/` à la fin.
  ...
  <Accordion title="Valeurs de configuration introuvables">
    ...
    Assurez-vous que votre fichier `application.yml` utilise la bonne indentation YAML sous l’espace de noms `okta.oauth2` :
...
<CardGroup cols={3}>
  ...
  <Card title="Référence Spring Security" icon="shield" href="https://docs.spring.io/spring-security/reference/servlet/oauth2/login.html">
    Documentation de Spring Security sur OAuth2 Login
  </Card>
...
Une application exemple complète qui montre le login, l’affichage du profil et le logout avec Auth0 est offerte dans le dépôt d’exemples Auth0.

<Card title="Exemple de login MVC" icon="github" href="https://github.com/auth0-samples/auth0-spring-boot-login-samples/tree/master/mvc-login">
  Comprend le login, le logout et une page de profil avec une intégration OAuth2 d’Auth0
</Card>
