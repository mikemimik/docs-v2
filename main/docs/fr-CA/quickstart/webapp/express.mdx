...
<Tabs>
  ...
  <Tab title="Dashboard">
    ...
    | Paramètre             | Valeur                  |
    | --------------------- | ----------------------- |
    | Allowed Rappel URLs | `http://localhost:3000` |
...
<AccordionGroup>
  ...
  <Accordion title="Utilisation de l’autorisation fondée sur les revendications">
    ...
    <Note>
      Des claims comme `role` doivent être ajoutées à vos jetons au moyen d’Auth0 Rules ou d’Actions. [En savoir plus sur l’ajout de claims personnalisés](https://auth0.com/docs/customize/actions/flows-and-triggers/login-flow/add-user-roles-to-id-and-access-tokens).
    </Note>
  </Accordion>
...
* **[Appeler des API protégées](https://auth0.com/docs/secure/tokens/access-tokens/get-access-tokens)** - Utilisez des jetons d’accès pour appeler vos API côté serveur
