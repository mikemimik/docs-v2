...
<Note>
  ...
  * **[Python](https://www.python.org/downloads/)** 3.9 ou une version ultérieure (3.11+ recommandé)
  * **[pip](https://pip.pypa.io/)** 21+ ou **[Poetry](https://python-poetry.org/)** 1.2+
  * **[OpenSSL](https://www.openssl.org/)** - Nécessaire pour générer les secrets de session
...
<Steps>
  <Step title="Créer un nouveau projet" stepNumber={1}>
    Créez un nouveau répertoire pour votre projet et configurez un environnement virtuel :
  ...
  <Step title="Configurez votre Auth0 App" stepNumber={3}>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        3. Dans la fenêtre popup, entrez un nom pour votre application, sélectionnez `Regular Web Application` comme type d’application, puis cliquez sur **Create**
        4. Ouvrez l’onglet **Settings** sur la page Details de l’Application
        5. Remplacez les valeurs du fichier `.env` par celles de **Domain**, **Client ID** et **Client Secret** du dashboard
        6. Générez un secret de session en exécutant : `openssl rand -hex 64`, puis utilisez-le pour `SESSION_SECRET`

        Enfin, dans l’onglet **Settings** de la page Details de votre Application, configurez les URL suivantes :
        ...
        **Origines Web autorisées :**
        ...
        <Info>
          ...
          Les **Allowed Logout URLs** sont essentielles pour offrir une expérience utilisateur fluide au moment de la déconnexion. Sans URL correspondante, les utilisateurs ne seront pas redirigés vers votre application après la déconnexion et se retrouveront plutôt sur une page générique d’Auth0.
  ...
  <Step title="Configurer Auth0 FastAPI SDK" stepNumber={4}>
    Créez un fichier `main.py` dans le répertoire racine de votre projet et ajoutez le code suivant :
    ...
    <Info>
      **SESSION&#95;SECRET** sert à chiffrer les cookies de session et doit être cryptographiquement sûr. Sans un secret robuste (minimum de 32 octets), les sessions de votre application peuvent être compromises. Générez un secret sécurisé à l’aide de `openssl rand -hex 64` et ne l’ajoutez jamais au contrôle de version.

      **SessionMiddleware** doit être ajouté avant d’utiliser le SDK. Sans lui, FastAPI ne peut ni lire ni définir des cookies, et toutes les tentatives d’authentification échoueront sans message d’erreur.
  ...
  <Step title="Créer des routes et afficher le profil de l’utilisateur" stepNumber={5}>
    ...
    * Un endpoint d’API protégé (`/profile`) qui renvoie les données utilisateur en JSON et nécessite une authentification
...
<Accordion title="Utiliser Redis pour le stockage de sessions avec état">
  ...
  * **Prise en charge du logout par canal arrière** - Gérez les événements de déconnexion d’Auth0
...
<Accordion title="Les sessions ne sont pas conservées / Les utilisateurs sont déconnectés à répétition">
  ...
  **Causes possibles et solutions** :
  ...
  2. **HTTP en production avec des cookies sécurisés**
     ...
     Les cookies sécurisés nécessitent HTTPS. Si vous faites des tests en local sur HTTP, vous pouvez désactiver temporairement les cookies sécurisés (non recommandé en production) :
...
<Accordion title="Erreur Auth0 : l'URL de rappel ne correspond pas">
  ...
  **Solution** :
  ...
  1. Accédez à [Auth0 Dashboard](https://manage.auth0.com/) → Applications → Your App → Settings
...
<Accordion title="HTTPS requis en production">
  ...
  **Solution** :
  ...
  3. **Assurez-vous qu&#39;APP&#95;BASE&#95;URL utilise HTTPS** :
     ```
