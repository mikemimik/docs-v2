---
mode: wide
description: Ce guide explique comment intégrer Auth0, ajouter l’authentication et afficher les informations du profil utilisateur dans une application Python à l’aide de l’Auth0 FastAPI SDK.
sidebarTitle: FastAPI
title: Ajouter la fonctionnalité de connexion à votre application FastAPI
---
...
<Accordion title="Instruction à l’IA" defaultOpen icon="microchip-ai" iconType="sharp-solid">
  **Vous utilisez l&#39;IA pour intégrer Auth0 ?** Ajoutez ce prompt à Cursor, Windsurf, Copilot, Claude Code ou votre IDE propulsé par l&#39;IA préféré pour accélérer le développement.
  ...
  Solution : Mettre le nom du paquet entre guillemets : pip install "uvicorn[standard]"

  Problème 3 : Les sessions ne sont pas enregistrées
  Problème : SessionMiddleware est absent ou le secret est incorrect
  Solution : Ajouter SessionMiddleware avec un secret_key approprié depuis les variables d'environnement
...
<Note>
  ...
  * **[Python](https://www.python.org/downloads/)** 3.9 ou une version ultérieure (3.11+ recommandée)
...
Ce guide de démarrage rapide montre comment ajouter l’authentication Auth0 à une application Web Python FastAPI. Vous créerez une application Web sécurisée offrant des fonctionnalités de connexion, de logout et de profil utilisateur à l’aide du [Auth0 FastAPI SDK](https://github.com/auth0/auth0-fastapi).
...
<Steps>
  ...
  <Step title="Configurez votre Auth0 App" stepNumber={3}>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        4. Ouvrez l’onglet **Settings** sur la page de détails de l’application
        ...
        Enfin, dans l’onglet **Settings** de la page de détails de votre application, configurez les URL suivantes :
        ...
        **Origines Web autorisées :**
        ...
        <Info>
          ...
          **Allowed Logout URLs** sont essentielles pour offrir une expérience utilisateur fluide au moment de la déconnexion. Sans URL correspondante, les utilisateurs ne seront pas redirigés vers votre application après la déconnexion et resteront plutôt sur une page générique d’Auth0.
  ...
  <Step title="Configurer le SDK Auth0 FastAPI" stepNumber={4}>
    ...
    <Info>
      **SESSION&#95;SECRET** sert à chiffrer les cookies de session et doit être sécurisé sur le plan cryptographique. Sans secret robuste (minimum de 32 octets), les sessions de votre application risquent d’être compromises. Générez un secret sûr à l’aide de `openssl rand -hex 64` et ne l’ajoutez jamais au contrôle de version.

      **SessionMiddleware** doit être ajouté avant d’utiliser le SDK. Sans lui, FastAPI ne peut ni lire ni écrire les cookies, et toutes les tentatives d’authentification échoueront sans bruit.

      **HTTPS en Production** est requis pour les cookies sécurisés (`secure=True`). Sans HTTPS, les cookies de session ne seront pas envoyés par les navigateurs, et les utilisateurs seront continuellement déconnectés à chaque requête.
    </Info>
  </Step>
  ...
  <Step title="Créer des routes et afficher le profil utilisateur" stepNumber={5}>
    ...
    # Ajouter le middleware de session - nécessaire pour la gestion des cookies
    app.add_middleware(SessionMiddleware, secret_key=os.getenv("SESSION_SECRET"))
    ...
    config = Auth0Config(
        ...
        authorization_params={
          "scope": "openid profile email", # Nécessaire pour obtenir les informations du profil utilisateur
        }
    )
    ...
    * Un endpoint d’API protégé (`/profile`) qui renvoie les données de l’utilisateur en JSON et nécessite une authentification
...
<Accordion title="Protection des routes d’API avec des dépendances personnalisées">
...
<Accordion title="Utilisation de Redis pour le stockage de sessions avec état">
  ...
  # Le transmettre à AuthClient
  auth_client = AuthClient(config, state_store=state_store)
  ```

  Avantages des sessions avec état :

  * **Aucune limite de taille pour les cookies** - Stockez un volume illimité de données de session
  * **Invalidation immédiate** - Supprimez les sessions côté serveur
  * **Prise en charge du backchannel logout** - Gérez les événements de déconnexion d’Auth0
...
<Accordion title="Les sessions ne sont pas enregistrées / Les utilisateurs sont régulièrement déconnectés">
  ...
  **Causes possibles et solutions** :
  ...
  2. **HTTP en production avec des cookies sécurisés**
     ...
     Les cookies sécurisés exigent HTTPS. Si vous faites des tests localement en HTTP, vous pouvez désactiver temporairement les cookies sécurisés (non recommandé en production) :
...
<Accordion title="Erreur Auth0 : l’URL de rappel ne correspond pas">
  ...
  **Solution** :
  ...
  1. Allez dans [Auth0 Dashboard](https://manage.auth0.com/) → Applications → Votre application → Settings
...
<Accordion title="HTTPS requis en production">
  ...
  **Cause** : Les cookies sécurisés exigent HTTPS en production. L’indicateur `secure=True` empêche l’envoi de cookies sur des connexions HTTP non chiffrées.
