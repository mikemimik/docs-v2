...
title: Ajouter l’ouverture de session à votre application FastAPI
---
...
<Accordion title="Instruction à l’IA" defaultOpen icon="microchip-ai" iconType="sharp-solid">
  **Vous utilisez l&#39;IA pour intégrer Auth0 ?** Ajoutez cette invite à Cursor, Windsurf, Copilot, Claude Code ou votre IDE préféré propulsé par l&#39;IA pour accélérer le développement.
  ...
  Replace or create main.py with this complete, production-ready code:
    ...
    # Ajouter le SessionMiddleware - requis pour la gestion des cookies
        ...
        if session:
            ...
            <body>
                <div class="container">
                    ...
                    <h1>Bienvenue dans Auth0 FastAPI</h1>
  ...
  Issue 2: Erreur d'expansion glob dans le shell avec uvicorn[standard]
  ...
  Issue 3: Les sessions ne persistent pas
  Problème : SessionMiddleware est absent ou le secret est incorrect
  Solution : Ajouter SessionMiddleware avec un secret_key approprié issu de l'environnement

  Issue 4: HTTPS requis en production
...
<Note>
  ...
  * **[OpenSSL](https://www.openssl.org/)** - Obligatoire pour générer des secrets de session

  **Compatibilité des versions de FastAPI :** Ce Quickstart nécessite **FastAPI 0.115.11+** et **Pydantic 2.12.5+**.
</Note>
...
Ce Quickstart montre comment ajouter l’authentification Auth0 à une application Web FastAPI en Python. Vous créerez une application Web sécurisée avec des fonctionnalités de connexion, de déconnexion et de profil utilisateur à l’aide du [Auth0 FastAPI SDK](https://github.com/auth0/auth0-fastapi).

<Steps>
  <Step title="Créer un nouveau projet" stepNumber={1}>
    Créez un nouveau répertoire pour votre projet et mettez en place un environnement virtuel :
  ...
  <Step title="Installez le SDK Auth0 FastAPI" stepNumber={2}>
    <Tabs>
      <Tab title="pip">
        ```shellscript
        pip install auth0-fastapi "uvicorn[standard]" python-dotenv itsdangerous
  ...
  <Step title="Configurez votre Auth0 App" stepNumber={3}>
    ...
    <Tabs>
      <Tab title="CLI">
        Exécutez la commande shell suivante à partir du répertoire racine de votre projet pour créer une Auth0 App et générer un fichier `.env` :
      ...
      <Tab title="Dashboard">
        ...
        4. Ouvrez l’onglet **Settings** sur la page Détails de l’application
        ...
        Enfin, dans l’onglet **Settings** de la page Détails de votre application, configurez les URL suivantes :
  ...
  <Step title="Configurer le SDK Auth0 FastAPI" stepNumber={4}>
    Créez un fichier `main.py` dans le répertoire racine de votre projet, puis ajoutez le code suivant :
    ...
    <Info>
      **SESSION&#95;SECRET** sert à chiffrer les cookies de session et doit être cryptographiquement sûr. Sans un secret robuste (minimum de 32 octets), les sessions de votre application peuvent être compromises. Générez un secret sûr à l’aide de `openssl rand -hex 64` et ne l’ajoutez jamais au contrôle de version.

      **SessionMiddleware** doit être ajouté avant d’utiliser le SDK. Sans cela, FastAPI ne peut ni lire ni définir de cookies, et toutes les tentatives d’authentification échoueront sans message d’erreur.

      **HTTPS en production** est requis pour les cookies sécurisés (`secure=True`). Sans HTTPS, les cookies de session ne seront pas envoyés par les navigateurs et les utilisateurs seront constamment déconnectés après chaque requête.
    </Info>
  </Step>
  ...
  <Step title="Créer des routes et afficher le profil utilisateur" stepNumber={5}>
    ...
    * Un point de terminaison d’API protégé (`/profile`) qui renvoie les données utilisateur en JSON et nécessite une authentification
...
<Accordion title="Appeler des API protégées avec des jetons d’accès">
  Configurez le SDK pour demander des jetons d’accès pour votre API et les utiliser dans des appels en aval :
  ...
  config = Auth0Config(
      ...
      secret=os.getenv("SESSION_SECRET"),
      audience="https://api.example.com",  # Votre identifiant d’API
      authorization_params={
...
<Accordion title="Utiliser Redis pour le stockage des sessions avec état">
  Mettez votre application à l’échelle en stockant les sessions dans Redis au lieu de cookies chiffrés :
  ...
  * **Aucune limite de taille pour les cookies** - Stockez une quantité illimitée de données de session
  * **Invalidation immédiate** - Supprimez les sessions côté serveur
  * **Prise en charge de la déconnexion en backchannel** - Gérez les événements de déconnexion d’Auth0
...
<Accordion title="Les sessions ne sont pas conservées / Les utilisateurs sont déconnectés à répétition">
  ...
  **Causes possibles et solutions** :
  ...
  2. **HTTP en production avec des cookies sécurisés**
     ...
     Les cookies sécurisés nécessitent HTTPS. Si vous faites des tests localement en HTTP, vous pouvez désactiver temporairement les cookies sécurisés (non recommandé en production) :
...
<Accordion title="Erreur Auth0 : l'URL de rappel ne correspond pas">
  **Problème** : Après avoir cliqué sur « Se connecter », Auth0 affiche l&#39;erreur suivante : « L&#39;URL de rappel ne correspond pas »
...
<Accordion title="ImportError ou problèmes avec async/await">
  ...
  **Cause** : FastAPI est un framework asynchrone et toutes les méthodes du SDK doivent être appelées avec `await`.

  **Solution** : Assurez-vous que toutes les fonctions de route sont async et que les méthodes du SDK sont correctement appelées avec `await` :
...
<Accordion title="HTTPS requis en production">
  **Problème** : Les sessions fonctionnent localement, mais pas en production.
