---
mode: wide
description: "Ajoutez l’authentification Auth0 à une application Web Flask avec connexion, routes protégées et profils d’utilisateur"
sidebarTitle: Flask
...
<Note>
  ...
  **Compatibilité avec les versions de Flask :** Ce Quickstart utilise **Flask 2.0+** avec l’extra `[async]` pour la prise en charge de l’asynchrone.
</Note>
...
Ce Quickstart montre comment ajouter l’authentification Auth0 à une application Flask. Vous créerez une application Web sécurisée avec une fonctionnalité de connexion, des routes protégées et un accès au profil utilisateur à l’aide du [Auth0 WebApp Python SDK](https://github.com/auth0/auth0-server-python).
...
<Steps>
  ...
  <Step title="Créer la configuration d’Auth, les routes et les modèles" stepNumber={4}>
    Créer des fichiers
...
<Accordion title="Configuration du stockage personnalisé">
  Par défaut, le SDK utilise un stockage fondé sur les cookies. Pour les environnements de Production ayant des besoins particuliers (mise à l’échelle horizontale, partage de sessions entre services), vous pouvez configurer des systèmes de stockage personnalisés comme Redis ou PostgreSQL.
  ...
  **Exemple Redis :**
  ...
  # Initialiser le stockage d’état personnalisé
  ...
  <Info>
    Pour la plupart des applications, le stockage par défaut fondé sur les cookies suffit. Le stockage personnalisé exige la mise en œuvre de l’interface `StateStore`. Consultez les [exemples du SDK](https://github.com/auth0/auth0-server-python/blob/main/examples/ConfigureStore.md) pour voir des mises en œuvre détaillées.
  </Info>
</Accordion>
...
<AccordionGroup>
  <Accordion title="MissingRequiredArgumentError">
    **Problème :** Vous voyez &quot;MissingRequiredArgumentError: secret&quot; au lancement de l’application
  ...
  <Accordion title="Erreur : URL de rappel invalide">
    ...
    **Solution :**
    ...
    2. Allez dans [Auth0 Dashboard](https://manage.auth0.com/dashboard/) → Applications → Votre application → Settings
  ...
  <Accordion title="Erreurs de la boucle d’événements AsyncIO">
    ...
    **Cause :** La prise en charge asynchrone de Flask 2.0+ peut entraîner des problèmes avec certaines configurations.
  ...
  <Accordion title="Erreurs de module introuvable">
    **Problème :** Le message &quot;ModuleNotFoundError: No module named &#39;auth0&#95;server&#95;python&#39;&quot; ou un message semblable s’affiche
  ...
  <Accordion title="ClaimDecodingFailed Error">
    ...
    * Algorithmes de signature non concordants
    ...
    **Solution :**
    ...
    2. Vérifiez que l’heure de votre système est synchronisée (NTP) :
       ...
       ```
    3. Supprimez les cookies du navigateur et relancez l’authentification
    4. Vérifiez que `AUTH0_DOMAIN` n’inclut pas le préfixe `https://`
    5. Vérifiez que Auth0 Dashboard → Applications → Votre application → Settings → Advanced → OAuth → JsonWebToken Signature Algorithm correspond à la configuration de votre SDK
  </Accordion>
  ...
  <Accordion title="Jeton expiré ou invalide">
    ...
    **Solution :**
    ...
    4. Mettez en place une gestion adéquate des erreurs pour rediriger les utilisateurs vers le login lorsque les jetons expirent
  </Accordion>

  <Accordion title="CORS et erreurs liées aux requêtes inter-origines">
    ...
    **Solution :**
    ...
    1. Ajoutez l’origine de votre application dans Auth0 Dashboard → Applications → Votre application → Settings:
       * **Allowed Web Origins**: `http://localhost:5000`
       * **Allowed Callback URLs**: `http://localhost:5000/callback`
       * **Allowed Logout URLs**: `http://localhost:5000`
    2. En Production, ajoutez vos URL de production :
       ...
       ```
    3. Assurez-vous que votre application Flask est correctement configurée pour CORS si vous appelez les API Auth0 à partir du frontend Javascript :
       ```python
  ...
  <Accordion title="Limite de requêtes dépassée">
    ...
    **Solution :**
    ...
    1. Consultez Auth0 Dashboard → Monitoring → Logs pour connaître les détails des limites de débit
    2. Mettez en œuvre une temporisation exponentielle pour les nouvelles tentatives :
       ```python
       import time
    ...
    4. Optimisez le flux d’authentification afin de réduire les requêtes de jeton inutiles
