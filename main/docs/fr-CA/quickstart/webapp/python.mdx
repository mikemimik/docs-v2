...
sidebarTitle: Flask
title: "Ajoutez la connexion à votre application Flask"
---
...
<Note>
  ...
  **Compatibilité des versions de Flask :** Ce Quickstart utilise **Flask 2.0+** avec l’extra `[async]` pour la prise en charge des fonctionnalités asynchrones.
</Note>
...
Ce Quickstart montre comment ajouter l’authentification Auth0 à une application Flask. Vous créerez une application web sécurisée avec une fonction de connexion, des routes protégées et un accès au profil utilisateur à l’aide du [Auth0 WebApp Python SDK](https://github.com/auth0/auth0-server-python).
...
<Steps>
  ...
  <Step title="Installer les dépendances" stepNumber={2}>
    ...
    Le fichier `requirements.txt` recense toutes les dépendances du projet. Pour les installer, exécutez :
  ...
  <Step title="Configurez votre Auth0 App" stepNumber={3}>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        3. Donnez un nom à votre application (p. ex. « My Flask App ») et sélectionnez **Regular Web Application**
        ...
        7. Remplacez `YOUR_AUTH0_DOMAIN` dans `.env` par votre **Domain** de l’onglet **Settings** (p. ex. `your-tenant.auth0.com`)
  ...
  <Step title="Créer la configuration d’authentification, les routes et les gabarits" stepNumber={4}>
    Créer des fichiers
    ...
    <Info>
      **Développement seulement :** Cet exemple utilise de simples classes de stockage en mémoire (`MemoryStateStore` et `MemoryTransactionStore`) à des fins de démonstration. Ces mécanismes de stockage perdront toutes les données de session lorsque votre application redémarrera et ne fonctionneront pas dans les déploiements multi-instances.

      **Pour les applications de production**, vous devez utiliser un stockage persistant. Le SDK est indépendant du `framework` et vous oblige à fournir des implémentations personnalisées de `StateStore` et `TransactionStore`. Consultez les [exemples officiels de stockage du SDK](https://github.com/auth0/auth0-server-python/blob/main/examples/ConfigureStore.md) pour obtenir des indications détaillées sur l’implémentation de Redis, PostgreSQL ou d’autres systèmes de stockage persistants.
    </Info>
  </Step>
...
<Check>
  **Vérification**
  ...
  Vous devriez maintenant avoir une page de connexion Auth0 entièrement fonctionnelle sur votre [localhost](http://localhost:5000/)
</Check>
...
<Accordion title="Accéder aux jetons d’API">
  ...
  async def api_call():
      try:
          # Récupérer un jeton d’accès pour votre API
          ...
          return f"Jeton d’accès récupéré : {access_token[:20]}..."
      except Exception as e:
          return f"Erreur lors de la récupération du jeton d’accès : {str(e)}", 500
  ```
...
<Accordion title="Configuration du stockage personnalisé">
  Par défaut, le SDK utilise un stockage fondé sur des cookies. Pour les environnements de production ayant des besoins particuliers (mise à l’échelle horizontale, partage des sessions entre services), vous pouvez configurer des mécanismes de stockage personnalisés comme Redis ou PostgreSQL.

  **Quand utiliser un stockage personnalisé :**

  * Vous devez partager des sessions entre plusieurs serveurs
  * Vos données de session sont trop volumineuses pour les limites de taille des cookies
  * Vous avez besoin d’une gestion centralisée des sessions pour le logout backchannel
  ...
  **Exemple avec Redis :**
  ...
  <Info>
    Pour la plupart des applications, le stockage par défaut fondé sur des cookies est suffisant. Le stockage personnalisé exige d’implémenter l’interface `StateStore`. Consultez les [exemples du SDK](https://github.com/auth0/auth0-server-python/blob/main/examples/ConfigureStore.md) pour voir des implémentations détaillées.
  </Info>
</Accordion>
...
<AccordionGroup>
  <Accordion title="MissingRequiredArgumentError">
    ...
    **Solution :**
    ...
    3. Générez une nouvelle valeur secrète au besoin : `openssl rand -hex 64`
    4. Ajoutez-la à `.env` : `AUTH0_SECRET=your_generated_secret`
  ...
  <Accordion title="Erreur : URL de rappel invalide">
    ...
    **Cause :** Le Redirect URI dans votre code ne correspond pas à celui qui est enregistré dans Auth0 Dashboard.
  ...
  <Accordion title="Erreurs de la boucle d’événements AsyncIO">
    ...
    **Solution :**
    ...
    Ensuite, redémarrez votre application Flask.
  </Accordion>

  <Accordion title="Erreurs « Module introuvable »">
    **Problème :** Vous voyez &quot;ModuleNotFoundError: No module named &#39;auth0&#95;server&#95;python&#39;&quot; ou un message du genre
  ...
  <Accordion title="Erreur ClaimDecodingFailed">
    ...
    **Cause :** L’ID token ou l’access token reçu d’Auth0 n’a pas pu être décodé correctement, souvent pour l’une des raisons suivantes :
    ...
    **Solution :**
    ...
    2. Vérifiez que l’heure de votre système est synchronisée (NTP) :
       ...
       ```
    3. Effacez les cookies du navigateur et recommencez l’authentification
  ...
  <Accordion title="Le jeton a expiré ou est invalide">
    **Problème :** Vous voyez les erreurs &quot;Token has expired&quot; ou &quot;invalid&#95;token&quot;

    **Cause :** Le jeton d’accès ou l’ID token a dépassé sa durée de vie, ou la session a expiré.
    ...
    **Solution :**
    ...
    2. Pour les API, assurez-vous de demander de nouveaux jetons :
       ...
       ```
    3. Ajustez la durée de vie des jetons dans Auth0 Dashboard → Applications → Your App → Settings → Advanced → OAuth
    4. Prévoyez une gestion adéquate des erreurs pour rediriger les utilisateurs vers la page de login lorsque les jetons expirent
  </Accordion>
  ...
  <Accordion title="CORS et erreurs d’origine croisée">
    ...
    **Cause :** L’origine de votre application n’est pas correctement configurée dans Auth0.
    ...
    **Solution :**
    ...
    2. En production, ajoutez vos URL de production :
       ...
       ```
    3. Assurez-vous que votre application Flask a une configuration CORS adéquate si vous appelez des API Auth0 à partir du frontend en JavaScript :
       ```python
  ...
  <Accordion title="Limite de requêtes dépassée">
    **Problème :** Vous voyez les erreurs &quot;Trop de requêtes&quot; ou &quot;Limite de débit dépassée&quot;
    ...
    **Solution :**
    ...
    2. Mettez en place un backoff exponentiel pour les nouvelles tentatives :
       ```python
       import time
           ...
           for attempt in range(max_retries):
               try:
                   ...
                   if e.status_code == 429 and attempt < max_retries - 1:
                       ...
                       raise
       ```
    3. Vérifiez les limites de votre abonnement Auth0
    4. Optimisez le flux d’authentification pour réduire les requêtes de jeton inutiles
    5. Mettez les jetons en cache de manière appropriée au lieu d’en demander fréquemment de nouveaux
  </Accordion>
</AccordionGroup>