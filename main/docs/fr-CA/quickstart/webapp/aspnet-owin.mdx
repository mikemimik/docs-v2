...
<Steps>
  <Step title="Configurer votre application Auth0">
    ...
    <Tabs>
      <Tab title="CLI">
        Exécutez la commande suivante depuis le répertoire racine de votre projet. Elle crée une application Auth0 et génère un extrait de `Web.config` prêt à être collé, avec vos identifiants déjà renseignés :
        ...
        Copiez les deux lignes `<add>` du résultat dans la section `<appSettings>` de votre `Web.config` :
