...
Auth0 vous permet d’ajouter l’authentication à votre application ASP.NET OWIN en quelques minutes. Ce guide vous montre comment ajouter la connexion, le logout et l’affichage du profil utilisateur à une application ASP.NET OWIN classique.
...
<Steps>
  <Step title="Configurez votre application Auth0">
    Toute application qui utilise Auth0 doit être enregistrée dans le Auth0 Dashboard. Auth0 génère un **Client ID** et un **Domain** que votre application utilise pour communiquer avec Auth0.
  ...
  <Step title="Installer des packages NuGet">
    ...
    <Info>
      L’utilisation simultanée du middleware de cookies OWIN et des cookies `System.Web` peut causer des problèmes. Si vous rencontrez des problèmes de cookies en double, consultez la documentation sur [System.Web cookie integration issues](https://github.com/aspnet/AspNetKatana/wiki/System.Web-response-cookie-integration-issues).
    </Info>
  </Step>

  <Step title="Configurer le middleware OWIN">
    Le middleware OWIN est enregistré dans une classe de démarrage. Si votre projet a déjà une classe de démarrage OWIN (généralement `App_Start/Startup.Auth.cs`), mettez à jour sa méthode `ConfigureAuth`. Sinon, créez le fichier maintenant.
    ...
    2. Middleware OpenID Connect - gère le flux de connexion et de déconnexion Auth0
    ...
    public partial class Startup
    {
        public void ConfigureAuth(IAppBuilder app)
        {
            ...
            // Le cookie middleware doit être enregistré en premier
    ...
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      `AuthenticationType` est défini à `"Auth0"`. Cette chaîne est utilisée à l’étape suivante pour déclencher le défi de connexion. La notification `RedirectToIdentityProvider` intercepte les requêtes de déconnexion et construit l’URL de déconnexion Auth0 appropriée.
    </Callout>
  </Step>

  <Step title="Ajouter des actions de connexion, de déconnexion et de profil">
  ...
  <Step title="Ajouter une vue de profil">
    ...
    La vue reçoit un `UserProfileViewModel` alimenté par les claims que le middleware OIDC extrait lorsqu’Auth0 renvoie l’ID token.

    <Note>
      ##### Point de contrôle

      Après vous être connecté, accédez à `/Account/UserProfile`. Vous devriez voir votre nom, votre adresse courriel et votre photo de profil. Si le nom ou l’adresse courriel semble vide, vérifiez que le `Scope` dans votre `OpenIdConnectAuthenticationOptions` inclut `"openid profile email"`.
    </Note>
  </Step>

  <Step title="Ajoutez des liens de connexion et de déconnexion à votre layout">
    Mettez à jour `Views/Shared/_Layout.cshtml` pour afficher des liens de connexion et de déconnexion selon l’état d’authentification de l’utilisateur :
    ...
    Ajoutez ceci à l’intérieur de l’élément `<nav>`, là où vos liens de navigation apparaissent dans la mise en page.

    <Note>
      ##### Checkpoint

      Lancez votre application. Vous devriez voir un lien **Se connecter** dans la navigation. Après vous être connecté, il devrait être remplacé par votre nom (avec un lien vers votre profil), ainsi qu’un lien **Se déconnecter**. En cliquant sur **Se déconnecter**, vous devriez être déconnecté et redirigé vers la page d’accueil.
    </Note>
  </Step>
</Steps>
...
<AccordionGroup>
  <Accordion title="Incompatibilité de l’URI de redirection après la connexion">
  ...
  <Accordion title="Boucle de connexion — l’application redirige constamment vers Auth0">
  ...
  <Accordion title="L’utilisateur n’est pas redirigé après le logout">
    **Problème :** En cliquant sur **Log out**, l’utilisateur est déconnecté d’Auth0, mais n’est pas redirigé vers votre application.

    **Solution :** Ajoutez un paramètre de requête `returnTo` à l’URL de logout Auth0 dans la notification `RedirectToIdentityProvider`. L’URL de retour doit aussi figurer dans **Allowed Logout URLs** dans les paramètres de votre application Auth0 :
  ...
  <Accordion title="La photo de profil ou l’adresse courriel est vide">
    ...
    **Solution :** Vérifiez que `Scope` dans `OpenIdConnectAuthenticationOptions` inclut `"openid profile email"`. Le scope `profile` fournit le nom et l’image de profil; le scope `email` fournit l’adresse courriel.
  ...
  <Accordion title="Les valeurs de Domain ou de Client ID sont nulles au démarrage">
    **Problème :** L’application génère une exception de référence nulle ou une exception de configuration au démarrage.

    **Solution :** Vérifiez que `auth0:Domain` et `auth0:ClientId` sont bien présents dans `<appSettings>` dans `Web.config`, et que vous exécutez la bonne configuration de build (Debug/Release), celle qui charge la bonne transformation `Web.config`.
...
<Accordion title="Personnaliser les paramètres de connexion">
  ...
  RedirectToIdentityProvider = notification =>
  {
      if (notification.ProtocolMessage.RequestType == OpenIdConnectRequestType.Authentication)
      {
          ...
          // Définir une locale d’interface précise
...
<CardGroup cols={2}>
  ...
  <Card title="Documentation Katana / OWIN" icon="book" href="https://learn.microsoft.com/en-us/aspnet/aspnet/overview/owin-and-katana/">
    Documentation de référence officielle de Microsoft sur OWIN/Katana
  </Card>
