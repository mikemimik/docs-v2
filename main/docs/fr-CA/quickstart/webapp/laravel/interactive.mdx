---
title: Ajoutez la connexion à votre application Laravel
sidebarTitle: Laravel
description: Ce guide montre comment intégrer Auth0 à une application Laravel, nouvelle ou existante, à l’aide de l’Auth0 Laravel SDK.
mode: wide
---
...
<Accordion title="Utiliser l’IA pour intégrer Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **Demandez ensuite à votre assistant IA :**
  ...
  Votre assistant IA créera automatiquement votre application Auth0, récupérera les identifiants, installera `auth0/login`, configurera les variables d’environnement et enregistrera les routes d’authentification. [Documentation complète sur Agent Skills →](/docs/fr-CA/quickstart/agent-skills)
</Accordion>
...
Ce guide de démarrage rapide montre comment ajouter l’authentification Auth0 à une application Laravel. Vous configurerez une connexion sécurisée, la déconnexion, des routes protégées et l’accès au profil utilisateur à l’aide du [Auth0 Laravel SDK](https://github.com/auth0/laravel-auth0).
...
<Steps>
  ...
  <Step title="Installer le SDK Auth0 pour Laravel" stepNumber={2}>
  ...
  <Step title="Ajouter des routes d’authentification" stepNumber={4}>
    Le SDK Auth0 enregistre automatiquement les routes suivantes pour votre application — aucune configuration de route supplémentaire n’est nécessaire :

    | Route       | Description                                        |
    | ----------- | -------------------------------------------------- |
    | `/login`    | Démarre le flux de connexion Auth0                 |
    | `/logout`   | Déconnecte l’utilisateur et le redirige vers Auth0 |
    | `/callback` | Traite le callback d’authentification Auth0        |

    <Note>
      Si votre application utilise Laravel Breeze, Fortify ou Jetstream, les routes `/login`, `/logout` et `/callback` du SDK peuvent entrer en conflit avec les routes enregistrées par ces packages. Consultez le [README du SDK](https://github.com/auth0/laravel-auth0) pour obtenir des instructions sur l’enregistrement manuel des routes.
    </Note>
  ...
  <Step title="Protéger les routes à l’aide d’un middleware" stepNumber={5}>
    Utilisez le middleware `auth` de Laravel pour exiger l’authentification sur n’importe quelle route. Vous pouvez aussi imposer des [permissions](https://auth0.com/docs/manage-users/access-control/rbac) précises à l’aide du middleware `can` :
    ...
    <Info>
      Les autorisations sont définies dans les paramètres de votre API Auth0 et attribuées aux utilisateurs au moyen de rôles. Consultez [le contrôle d’accès basé sur les rôles](https://auth0.com/docs/manage-users/access-control/rbac) pour plus de détails.
    </Info>
  </Step>
  ...
  <Step title="Lancez votre application" stepNumber={6}>
    ...
    <Info>
      Votre application fonctionne à l’adresse [http://localhost:8000](http://localhost:8000). Si le port 8000 est déjà utilisé, exécutez `php artisan serve --port=8001`, puis mettez à jour les **Allowed Callback URLs** et **Allowed Logout URLs** de votre application Auth0 afin d’utiliser le nouveau port.
    </Info>
  </Step>
</Steps>
...
<AccordionGroup>
  <Accordion title="Erreur de non-correspondance de l’URL de rappel">
    ...
    **Correctif :**
    ...
    Assurez-vous qu’il n’y a pas de barre oblique finale et que le port correspond à celui de votre serveur en cours d’exécution.
  </Accordion>

  <Accordion title="Erreur invalid state / CSRF après la connexion">
    **Cause :** Mauvaise configuration de la session ou du cookie — la session Laravel ne conserve pas le paramètre state entre la redirection de connexion et le callback.

    **Correctif :**

    1. Assurez-vous que `SESSION_DRIVER` dans `.env` est défini sur `file`, `database` ou `redis` (et non `array`)
  ...
  <Accordion title="404 sur /login, /logout ou /callback">
    **Cause :** Les routes du SDK ne sont pas enregistrées, généralement parce que le fournisseur de services n’a pas été chargé.

    **Correctif :**

    1. Confirmez que vous avez exécuté `php artisan vendor:publish --tag auth0`
    2. Vérifiez que la découverte automatique du package est activée (consultez `composer.json` pour `"dont-discover": []`)
...
<AccordionGroup>
  <Accordion title="Management API — mettre à jour les métadonnées utilisateur">
    Vous pouvez mettre à jour les renseignements d’un utilisateur à l’aide de l’[Auth0 Management API](https://github.com/auth0/laravel-auth0/blob/main/docs/Management.md). Tous les points de terminaison de la Management API sont accessibles au moyen de la méthode `Auth0::management()` du SDK.
    ...
    **Avant d’effectuer des appels à la Management API**, autorisez votre application à accéder à la Management API :
    ...
    Route::get('/colors', function () {
        ...
        // Mettre à jour les métadonnées de l'utilisateur authentifié avec une couleur préférée aléatoire
        $users->update(
            id: auth()->id(),
            body: [
                'user_metadata' => [
        ...
        // Récupérer et afficher les métadonnées mises à jour
    ...
    La documentation du SDK contient une référence complète de toutes les méthodes de la Management API : [SDK documentation](https://github.com/auth0/laravel-auth0/blob/main/docs/Management.md).
  </Accordion>

  <Accordion title="Modèles utilisateur et référentiels personnalisés">
    Le SDK prend en charge les modèles utilisateur et les référentiels personnalisés, ce qui vous permet de stocker et de récupérer les utilisateurs dans votre propre base de données tout en conservant Auth0 comme fournisseur d’identité.

    Consultez [User Repositories and Models](https://github.com/auth0/laravel-auth0/blob/main/docs/Users.md) pour le guide complet de mise en œuvre.
  </Accordion>

  <Accordion title="Écouter les événements du SDK">
    Le SDK déclenche des événements à des étapes clés du cycle de vie de l’authentification — à la connexion, à la déconnexion, à l’actualisation du token, et plus encore — ce qui vous permet de personnaliser entièrement le comportement sans modifier le code de base du SDK.
  ...
  <Accordion title="Role-Based Access Control (RBAC)">
    Utilisez le RBAC d’Auth0 pour attribuer des permissions aux utilisateurs au moyen de rôles, puis appliquez-les dans Laravel à l’aide du middleware `can` :
...
* [Auth0 Dashboard](https://manage.auth0.com/) — Configurer et gérer votre tenant Auth0 et vos applications
* [laravel-auth0 SDK](https://github.com/auth0/laravel-auth0) — Documentation complète du SDK et intégrations avancées
* [Role-Based Access Control](https://auth0.com/docs/manage-users/access-control/rbac) — Attribuer des permissions aux utilisateurs à l’aide de rôles
* [Auth0 Marketplace](https://marketplace.auth0.com/) — Découvrir des intégrations pour étendre les fonctionnalités d’Auth0