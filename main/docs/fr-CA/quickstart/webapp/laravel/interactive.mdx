...
description: Ce guide explique comment intégrer Auth0 à une application Laravel, nouvelle ou existante, à l’aide du Auth0 Laravel SDK.
mode: wide
---
...
<Steps>
  <Step title="Créer un nouveau projet Laravel" stepNumber={1}>
    Si vous avez déjà une application Laravel, passez directement à l’étape 2.
  ...
  <Step title="Installez le SDK Auth0 pour Laravel" stepNumber={2}>
  ...
  <Step title="Ajouter des routes d’authentification" stepNumber={4}>
    ...
    | `/callback` | Gère le callback d’authentification Auth0          |

    <Note>
      Si votre application utilise Laravel Breeze, Fortify ou Jetstream, les routes `/login`, `/logout` et `/callback` du SDK peuvent entrer en conflit avec les routes enregistrées par ces packages. Consultez le [README du SDK](https://github.com/auth0/laravel-auth0) pour savoir comment enregistrer les routes manuellement.
    </Note>
  ...
  <Step title="Protéger les routes à l’aide d’un middleware" stepNumber={5}>
    ...
    // Requiert l'authentification ET la permission 'read:messages'
  ...
  <Step title="Lancez votre application" stepNumber={6}>
    ...
    <Info>
      Votre application est accessible à l’adresse [http://localhost:8000](http://localhost:8000). Si le port 8000 est déjà utilisé, exécutez `php artisan serve --port=8001`, puis mettez à jour les **Allowed Callback URLs** et **Allowed Logout URLs** de votre application Auth0 pour utiliser le nouveau port.
    </Info>
  </Step>
</Steps>
...
<Check>
  ...
  * [http://localhost:8000/logout](http://localhost:8000/logout) — ferme la session et retourne à la page d’accueil
</Check>
...
<AccordionGroup>
  <Accordion title="Erreur de non-correspondance de l’URL de rappel">
    ...
    **Correctif :**
    ...
    1. Allez dans le [Auth0 Dashboard](https://manage.auth0.com/) → **Applications &gt; Applications** → votre application → **Settings**
    2. Ajoutez `http://localhost:8000/callback` à **Allowed Callback URLs**
    3. Cliquez sur **Save Changes**

    Assurez-vous qu’il n’y a pas de barre oblique à la fin et que le port correspond bien à celui de votre serveur en cours d’exécution.
  </Accordion>

  <Accordion title="Erreur Invalid state / CSRF après le login">
    **Cause :** Mauvaise configuration de la session ou du cookie — la session Laravel ne conserve pas l’état entre la redirection de login et le callback.
  ...
  <Accordion title="404 sur /login, /logout ou /callback">
    ...
    **Correctif :**
    ...
    1. Vérifiez que vous avez exécuté `php artisan vendor:publish --tag auth0`
...
<AccordionGroup>
  <Accordion title="Management API — mettre à jour les métadonnées de l’utilisateur">
    Vous pouvez mettre à jour les informations d’un utilisateur au moyen de l’[Auth0 Management API](https://github.com/auth0/laravel-auth0/blob/main/docs/Management.md). Tous les points de terminaison de la Management API sont accessibles à partir de la méthode `Auth0::management()` du SDK.
    ...
    **Avant d’effectuer des appels à la Management API**, autorisez votre application à accéder à la Management API :
    ...
    Route::get('/colors', function () {
        ...
        // Met à jour les métadonnées de l’utilisateur authentifié avec une couleur préférée choisie au hasard
        $users->update(
            id: auth()->id(),
            body: [
                'user_metadata' => [
  ...
  <Accordion title="Modèles et référentiels d’utilisateurs personnalisés">
    Le SDK prend en charge les modèles et référentiels d’utilisateurs personnalisés, ce qui vous permet de stocker et de récupérer des utilisateurs à partir de votre propre base de données tout en conservant Auth0 comme fournisseur d’identité.
  ...
  <Accordion title="Surveiller les événements du SDK">
    Le SDK déclenche des événements à des moments clés du cycle de vie de l’authentication — au login, au logout, à l’actualisation du token, et plus encore — ce qui vous permet de personnaliser entièrement le comportement sans modifier le code de base du SDK.
  ...
  <Accordion title="Contrôle d’accès basé sur les rôles (RBAC)">
    ...
    2. Ajoutez les autorisations dont votre application a besoin (p. ex., `admin:dashboard`, `read:messages`)
...
Maintenant que l’authentification fonctionne dans votre application Laravel, découvrez d’autres fonctionnalités d’Auth0 :
