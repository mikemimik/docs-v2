...
<Steps>
  <Step title="Créer un nouveau projet" stepNumber={1}>
    Créez un nouveau projet Blazor Server pour suivre ce Quickstart
  ...
  <Step title="Configurez votre application Auth0" stepNumber={3}>
    ...
    * **Origines Web autorisées**: `http://localhost:5000`
    ...
    <Info>
      **Important :** Assurez-vous de [configurer les connexions](https://auth0.com/docs/get-started/applications/set-up-database-connections) et de les activer pour votre application dans Auth0 Dashboard, sous l’onglet **Connections**.
    </Info>
  </Step>
  ...
  <Step title="Ajouter les pages Login et Logout" stepNumber={5}>
    Créez des pages Login et Logout pour permettre aux utilisateurs de s’authentifier.
  ...
  <Step title="Créer la page de profil et mettre à jour le layout" stepNumber={6}>
    Créez une page de profil utilisateur personnalisée pour afficher le nom de l’utilisateur et les claims, puis mettez à jour le layout pour y ajouter des liens de connexion et de déconnexion.

    Pour commencer, créez le composant Profile :
    ...
    Ajoutez les extraits de code suivants. Veillez à ajouter le code `MainLayout` dans la section supérieure de votre layout, en laissant toutes les autres parties intactes.
  ...
  <Step title="Lancez votre application" stepNumber={7}>
    ...
    Ouvrez votre navigateur et allez à `http://localhost:5000`. Cliquez sur le lien **Connexion** dans le menu de navigation. Vous serez redirigé vers la page de connexion d’Auth0.
...
<Check>
  **Checkpoint**
  ...
  Vous devriez maintenant disposer d’une application Blazor Server entièrement fonctionnelle et protégée par Auth0, accessible à l’adresse [http://localhost:5000](http://localhost:5000). Les utilisateurs peuvent se connecter, consulter leur profil et se déconnecter.
</Check>
...
<Accordion title="Configurer les organisations">
  ...
  Ou indiquez l’organisation à la connexion :
...
<AccordionGroup>
  <Accordion title="Impossible d'obtenir la configuration">
    ...
    * Aucun slash final dans la valeur du Domain
  ...
  <Accordion title="Valeurs de configuration introuvables">
    ...
    **Solution :** Assurez-vous que `appsettings.json` contient la section Auth0 avec les valeurs Domain, ClientId et ClientSecret. Vérifiez que la configuration est correctement lue :
...
Vous trouverez ci-dessous un exemple d’application illustrant l’intégration d’Auth0 avec ASP.NET Core MVC.:
