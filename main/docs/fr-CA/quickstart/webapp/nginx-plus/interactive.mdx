...
<Recipe>
  <Content>
    ...
    <Section id={sections[0].id} title={sections[0].title} stepNumber="1">
      Ce tutoriel et le projet de départ ont été testés avec les éléments suivants :
    ...
    <Section id={sections[1].id} title={sections[1].title} stepNumber="2">
      ...
      Une fois l’installation terminée, vous devez l’activer pour NGINX en ajoutant la ligne suivante près du haut de votre fichier
    ...
    <Section id={sections[2].id} title={sections[2].title} stepNumber="3">
      Clonez le repository `nginx-openid-connect`. Ce repository inclut une configuration modèle.
    ...
    <Section id={sections[3].id} title={sections[3].title} stepNumber="4">
      ...
      Ensuite, ajoutez l’[URL de logout de votre tenant](https://auth0.com/docs/api/authentication#auth0-logout) à votre fichier
      `openid_connect_configuration.conf`.
    </Section>

    <Section id={sections[4].id} title={sections[4].title} stepNumber="5">
      Ajoutez le header `Accept-Encoding` et définissez sa valeur à `gzip` dans votre fichier
    ...
    <Section id={sections[7].id} title={sections[7].title} stepNumber="8">
      Ajoutez des headers supplémentaires provenant du ID token (JWT) à la cible en amont dans votre fichier
      `/etc/nginx/conf.d/frontend.conf`.
    </Section>

    ## Prochaines étapes

    Excellent travail ! Si vous vous êtes rendu jusque-là, vous devriez maintenant avoir le login, le logout et les renseignements du profil utilisateur qui fonctionnent dans votre application.
