---
mode: wide
description: Ce guide explique comment intégrer Auth0 à toute application ASP.NET MVC, nouvelle ou existante, à l’aide du SDK Auth0.AspNetCore.Authentication.
...
<Note>
  ...
  * **[.NET SDK](https://dotnet.microsoft.com/download)** 8.0 ou une version plus récente
...
Auth0 vous permet d’ajouter rapidement l’authentification et d’avoir accès aux renseignements du profil utilisateur dans votre application. Ce guide montre comment intégrer Auth0 à n’importe quelle application ASP.NET MVC, nouvelle ou existante, à l’aide du SDK `Auth0.AspNetCore.Authentication`.

<Steps>
  <Step title="Créer un nouveau projet" stepNumber={1}>
    Créez un nouveau projet ASP.NET Core MVC dans le cadre de ce Quickstart
  ...
  <Step title="Mettez à jour votre layout" stepNumber={7}>
    ...
    <Info>
      **Important :** Remplacez uniquement l’élément `<nav>`. Conservez toutes les autres parties de &#95;Layout.cshtml intactes, en particulier l’appel `@RenderBody()`, qui est nécessaire pour afficher le contenu de la page.
    </Info>
  </Step>
...
<Check>
  **Checkpoint**
  ...
  Vous devriez maintenant avoir une application MVC protégée par Auth0 entièrement fonctionnelle à l’adresse [http://localhost:5000](http://localhost:5000). Les utilisateurs peuvent se connecter, consulter leur profil et se déconnecter.
</Check>
...
<Accordion title="Accéder aux renseignements du profil utilisateur">
  Vous pouvez accéder aux renseignements du profil utilisateur à l’aide de la propriété `User` dans vos contrôleurs ou vos vues :
...
<Accordion title="Gérer les événements d’authentification">
  Personnalisez le comportement d’authentification en traitant les événements :
...
<AccordionGroup>
  <Accordion title="Impossible d’obtenir la configuration">
    ...
    **Solution :** Vérifiez que votre `Domain` est correct et n’inclut pas `https://`. La bibliothèque construit automatiquement l’autorité.
    ...
    Assurez-vous aussi de ce qui suit :
...
<Card title="Application Playground ASP.NET Core MVC" icon="github" href="https://github.com/auth0/auth0-aspnetcore-authentication/tree/main/playground/Auth0.AspNetCore.Authentication.Playground">
