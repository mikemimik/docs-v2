---
mode: wide
description: Ce guide explique comment intégrer Auth0 à une application ASP.NET MVC, nouvelle ou existante, à l’aide du SDK Auth0.AspNetCore.Authentication.
...
Auth0 vous permet d’ajouter rapidement l’authentification et d’accéder aux renseignements du profil utilisateur dans votre application. Ce guide montre comment intégrer Auth0 à toute application ASP.NET MVC, nouvelle ou existante, à l’aide du SDK `Auth0.AspNetCore.Authentication`.

<Steps>
  <Step title="Créer un nouveau projet" stepNumber={1}>
    Créez un nouveau projet MVC ASP.NET Core pour ce Quickstart
  ...
  <Step title="Configurez votre application Auth0" stepNumber={3}>
    ...
    * **Allowed Web Origins**: `http://localhost:5000`
    ...
    <Info>
      **Important :** Assurez-vous de [configurer les connections](https://auth0.com/docs/get-started/applications/set-up-database-connections) et de les activer pour votre application dans l’Auth0 Dashboard, sous l’onglet **Connections**.
    </Info>
  </Step>

  <Step title="Configurer l’authentification" stepNumber={4}>
    Mettez à jour votre `Program.cs` afin de configurer l’authentification Auth0 :
  ...
  <Step title="Ajouter les fonctionnalités Login et Logout" stepNumber={5}>
    Créez le fichier `AccountController.cs` dans le dossier `Controllers` :
  ...
  <Step title="Vue de profil Create" stepNumber={6}>
  ...
  <Step title="Mettez à jour votre fichier de mise en page" stepNumber={7}>
    ...
    <Info>
      **Important :** Remplacez uniquement l’élément `<nav>`. Laissez toutes les autres parties de &#95;Layout.cshtml intactes, en particulier l’appel `@RenderBody()`, qui est nécessaire pour afficher le contenu de la page.
    </Info>
  </Step>
...
<Check>
  **Checkpoint**
  ...
  Vous devriez maintenant avoir une application MVC protégée par Auth0 pleinement fonctionnelle à l’adresse [http://localhost:5000](http://localhost:5000). Les utilisateurs peuvent se connecter, consulter leur profil et se déconnecter.
</Check>
...
<Accordion title="Accéder aux renseignements du profil utilisateur">
  ...
  Les claims de l’utilisateur contiennent des renseignements OIDC standards :
...
<Accordion title="Stocker les jetons pour les appels d’API">
  Si vous devez effectuer des appels d’API externes au nom de l’utilisateur, vous pouvez récupérer et stocker des jetons :
...
<Accordion title="Gérer les événements d’authentification">
  Personnalisez le comportement lié à l’authentification en gérant les événements :
...
<AccordionGroup>
  <Accordion title="Impossible d’obtenir la configuration">
    ...
    **Solution :** Vérifiez que votre valeur `Domain` est correcte et qu’elle n’inclut pas `https://`. La bibliothèque construit automatiquement l’autorité.

    ```json
    {
      "Auth0": {
        "Domain": "your-tenant.auth0.com"  // Correct - sans protocole
      }
    }
    ```
    ...
    * il n’y a pas de barre oblique à la fin de la valeur du domaine
...
Vous trouverez une application exemple avec le code source du SDK :

<Card title="Application de démonstration ASP.NET Core MVC Playgroud" icon="github" href="https://github.com/auth0/auth0-aspnetcore-authentication/tree/main/playground/Auth0.AspNetCore.Authentication.Playground">
