...
description: Ce guide explique comment intégrer Auth0 à n’importe quelle application Next.js, nouvelle ou existante, à l’aide du SDK Auth0 Next.js v4.
mode: wide
...
<Accordion title="Utiliser l’IA pour intégrer Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **Puis, demandez à votre assistant IA :**

  ```text
  Ajoute l’authentification Auth0 à mon application Next.js
  ```

  Votre assistant IA créera automatiquement votre application Auth0, récupérera les informations d’identification, installera `@auth0/nextjs-auth0`, créera des routes d’API et configurera les variables d’environnement. [Documentation complète d’Agent Skills →](/docs/fr-CA/quickstart/agent-skills)
</Accordion>
...
Ce guide de démarrage rapide explique comment ajouter l’authentification Auth0 à une application Next.js 16. Vous créerez une application Web full stack avec rendu côté serveur, une fonctionnalité de connexion sécurisée et des routes protégées à l’aide du SDK Auth0 Next.js.
...
<Steps>
  ...
  <Step title="Installez le SDK Auth0 Next.js" stepNumber={2}>
  ...
  <Step title="Configurez votre Auth0 App" stepNumber={4}>
    ...
    <Tabs>
      ...
      <Tab title="CLI">
        Exécutez la commande suivante à la racine de votre projet pour créer une Auth0 App et générer un fichier `.env` :
      ...
      <Tab title="Dashboard">
        ...
        **Générez un AUTH0&#95;SECRET sécurisé :**
        ...
        Copiez la sortie et remplacez `YOUR_LONG_RANDOM_SECRET_HERE` dans `.env.local`. Elle doit contenir exactement 64 caractères hexadécimaux.
        ...
        4. Passez à l’onglet **Settings** dans la page Application Details
        ...
        <Warning>
          **Important :** Si vous modifiez `AUTH0_SECRET` après avoir lancé l’application, vous devez effacer les cookies de votre navigateur pour localhost. Les anciens session cookies chiffrés avec le secret précédent provoqueront une erreur `JWEDecryptionFailed`.
        </Warning>
        ...
        **Allowed Web Origins :**
        ...
        <Info>
          Les **Allowed Callback URLs** constituent une mesure de sécurité essentielle pour s’assurer que les utilisateurs sont redirigés de façon sécuritaire vers votre application après l’authentication. Sans URL correspondante, le login process échouera et les utilisateurs seront bloqués sur une page d’erreur Auth0 au lieu d’accéder à votre application.
  ...
  <Step title="Ajouter un proxy" stepNumber={6}>
    ...
    <Info>
      Ce proxy monte automatiquement les routes d’authentification suivantes :

      * `/auth/login` - route de login
      * `/auth/logout` - route de logout
      * `/auth/callback` - route de callback
      * `/auth/profile` - route de profil utilisateur
      * `/auth/access-token` - route du jeton d’accès
      * `/auth/backchannel-logout` - route de logout en canal arrière
    </Info>
  </Step>

  <Step title="Créer des composants Login, Logout et Profil" stepNumber={7}>
  ...
  <Step title="Mettre à jour le layout avec Auth0Provider" stepNumber={9}>
    Mettez à jour `src/app/layout.tsx` pour charger la police Inter et envelopper votre application avec `Auth0Provider` :
    ...
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      En v4, Auth0Provider est facultatif. Vous n’en avez besoin que si vous souhaitez transmettre un utilisateur initial lors du rendu côté serveur afin qu’il soit disponible dans le hook useUser().
    </Callout>
  </Step>
  ...
  <Step title="Lancez votre application" stepNumber={11}>
    ...
    <Info>
      Votre application sera accessible à l’adresse http://localhost:3000. Le SDK Auth0 v4 configure automatiquement les routes d’authentification sous `/auth/*` (et non `/api/auth/*`, comme dans la v3).

      Si le port 3000 est déjà utilisé, exécutez : `npm run dev -- --port 3001` et mettez à jour les URL de rappel de votre application Auth0 pour `http://localhost:3001`
    </Info>
  </Step>
</Steps>
...
<Accordion title="Erreur JWEDecryptionFailed">
  ...
  **Solution :**
  ...
  1. Générez un nouveau secret à l&#39;aide de :
  ...
  3. **Supprimez les cookies de votre navigateur** pour `localhost:3000` :
     ...
     * Safari : menu Develop → Show Web Inspector → onglet Storage → Cookies → Supprimer tout
...
<Accordion title="Erreur 404 sur /auth/login">
  Si cliquer sur connexion vous mène vers une page 404, vérifiez les points suivants :
  ...
  4. **Vérification des imports :** Assurez-vous que le chemin `import { auth0 } from "./lib/auth0"` est correct
</Accordion>

<Accordion title="Erreurs « Module Not Found »">
  Si vous voyez « Cannot find module &#39;@/components/LoginButton&#39; » ou des erreurs semblables :
  ...
  4. **Vérifiez les imports :** Assurez-vous d&#39;utiliser `@/components/*` (et non `~/components/*`)
</Accordion>
...
<Accordion title="Changements importants de la v4">
  Ce guide de démarrage rapide utilise **Auth0 Next.js SDK v4**, qui comporte des changements importants par rapport à la v3 :
  ...
  * **Nouveaux chemins de route** - Les routes se trouvent à `/auth/*` plutôt qu’à `/api/auth/*`
  * **Proxy requis** - Toute la fonctionnalité d’authentification passe par `proxy.ts`
  * **Utilisez des balises `<a>`** - La navigation doit utiliser `<a href="/auth/login">` au lieu de boutons avec onClick
  ...
  | Route                      | Objectif                           |
  | -------------------------- | ---------------------------------- |
  | `/auth/login`              | Démarrer la connexion              |
  ...
  | `/auth/backchannel-logout` | Traiter le logout backchannel      |
  ...
  <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
    ...
    2. Le proxy est correctement configuré avec le motif `matcher` montré à l’étape 6
...
<Accordion title="Utilisation de jetons Auth0 avec des backends tiers">
  Si vous utilisez un service backend tiers (comme Convex, Supabase ou Firebase) qui nécessite des jetons d’authentification Auth0, vous devrez transmettre le jeton d’accès de votre application Next.js au client backend.
  ...
  **Côté client :**
  ...
  <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
    Assurez-vous que votre application Auth0 est configurée avec une audience d’API si votre backend l’exige. Vous pouvez la définir dans le Auth0 Dashboard sous Applications → APIs, ou ajouter `AUTH0_AUDIENCE` à votre fichier `.env.local` et configurer le SDK en conséquence.
  </Callout>

  ### Dépannage des problèmes de jeton

  Si `ctx.auth.getUserIdentity()` renvoie `null` dans votre backend :

  1. **Vérifiez que le jeton est bien transmis :** consultez l’onglet Network des outils de développement du navigateur pour confirmer que le jeton est inclus dans les requêtes
  2. **Vérifiez le format du jeton :** assurez-vous de transmettre `accessToken`, et non `idToken`
  3. **Vérifiez la configuration du backend :** confirmez que votre backend utilise le bon domaine Auth0 et le bon client ID
