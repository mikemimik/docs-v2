...
description: Ce guide montre comment intégrer Auth0 à n’importe quelle application Next.js, nouvelle ou existante, à l’aide du SDK Auth0 Next.js v4.
mode: wide
...
<Steps>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        **Générez un AUTH0&#95;SECRET sécurisé :**
        ...
        1. Ouvrez le [Auth0 Dashboard](https://manage.auth0.com/dashboard/)
        ...
        <Warning>
          **Important :** Si vous modifiez `AUTH0_SECRET` après le démarrage de l&#39;application, vous devez effacer les cookies de votre navigateur pour localhost. Les anciens session cookies chiffrés avec le secret précédent provoqueront une erreur `JWEDecryptionFailed`.
        </Warning>
        ...
        **Allowed Web Origins :**
        ...
        <Info>
          **Allowed Callback URLs** constituent une mesure de sécurité essentielle pour garantir que les utilisateurs sont redirigés en toute sécurité vers votre application après l&#39;authentication. Sans URL correspondante, le processus de connexion échouera et les utilisateurs verront une page d&#39;erreur Auth0 au lieu d&#39;accéder à votre application.

          **Allowed Logout URLs** sont essentielles pour offrir une expérience utilisateur fluide à la déconnexion. Sans URL correspondante, les utilisateurs ne seront pas redirigés vers votre application après le logout et resteront plutôt sur une page Auth0 générique.
  ...
  <Step title="Ajouter un proxy" stepNumber={6}>
    ...
    <Info>
      Ce proxy configure automatiquement les routes d’authentication suivantes :
      ...
      * `/auth/access-token` - Route de jeton d’accès
      * `/auth/backchannel-logout` - Route de Logout par canal arrière
    </Info>
  </Step>

  <Step title="Créer des composants Login, Logout et de profil" stepNumber={7}>
    Ajoutez le code du composant dans les fichiers créés à l’étape 3 :
  ...
  <Step title="Mettre à jour le layout avec Auth0Provider" stepNumber={9}>
    Mettez à jour `src/app/layout.tsx` pour y charger la police Inter et encapsuler votre application dans `Auth0Provider` :
    ...
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      Avec la v4, Auth0Provider est facultatif. Vous n’en avez besoin que si vous souhaitez transmettre un utilisateur initial lors du rendu côté serveur pour le rendre accessible au hook useUser().
    </Callout>
  </Step>
  ...
  <Step title="Exécutez votre application" stepNumber={11}>
    ```shellscript
    npm run dev
    ```

    <Info>
      Votre application sera accessible à l’adresse http://localhost:3000. Le SDK Auth0 v4 configure automatiquement les routes d’authentification sur `/auth/*` (et non `/api/auth/*` comme dans la v3).
...
<Accordion title="Erreur JWEDecryptionFailed">
  Si vous voyez l’erreur `JWEDecryptionFailed: decryption operation failed`, c’est généralement parce que `AUTH0_SECRET` est invalide ou qu’un ancien cookie de session a été chiffré avec un autre secret.
...
<Accordion title="Erreur 404 sur /auth/login">
  Si le clic sur login vous mène à une page 404, vérifiez les points suivants :
...
<Accordion title="Changements importants de la v4">
  ...
  * **Aucun handler de route dynamique requis** - Le proxy monte automatiquement les routes d’authentification
  ...
  * **Proxy requis** - Toute la logique d’authentification passe par `proxy.ts`
  * **Utilisez des balises `<a>`** - La navigation doit se faire avec `<a href="/auth/login">` plutôt qu’avec des boutons utilisant onClick
  ...
  Le SDK monte automatiquement ces routes au moyen du proxy :
...
<Accordion title="Utilisation des jetons Auth0 avec des backends tiers">
  Si vous utilisez un service backend tiers (comme Convex, Supabase ou Firebase) qui exige des jetons d’authentification Auth0, vous devrez transmettre le jeton d’accès de votre application Next.js au client du backend.
  ...
  **Côté client :**
  ...
  4. **Vérifiez l’audience :** si vous utilisez l’Auth0 API, assurez-vous que `AUTH0_AUDIENCE` est défini et correspond à votre identifiant d’API
