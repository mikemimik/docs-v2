...
<Accordion title="Utiliser l’IA pour intégrer Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  Si vous utilisez un assistant IA de programmation comme Claude Code, Cursor ou GitHub Copilot, vous pouvez ajouter automatiquement l’authentification avec Auth0 API en quelques minutes grâce à [Agent Skills](https://agentskills.io/home).
  ...
  **Ensuite, demandez à votre assistant IA :**
  ...
  Votre assistant IA créera automatiquement votre Auth0 API, récupérera les identifiants, installera `@auth0/auth0-fastify-api`, configurera le plugin et protégera vos points de terminaison d’API avec la validation JWT. [Documentation complète sur Agent Skills →](/docs/fr-CA/quickstart/agent-skills)
</Accordion>
...
<Note>
  ...
  **Compatibilité des versions de Fastify :** Ce guide de démarrage rapide fonctionne avec **Fastify 5.x** et les versions plus récentes.
</Note>
...
<Steps>
  ...
  <Step title="Configurez votre API Auth0" stepNumber={3}>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        3. Entrez un nom pour votre API (par exemple, &quot;My Fastify API&quot;)
        4. Définissez **Identifier** (par exemple, `https://my-fastify-api.example.com`)
        ...
        <Warning>
          Remplacez `YOUR_AUTH0_DOMAIN` par le domaine de votre tenant Auth0 (par exemple, `dev-abc123.us.auth0.com`) et `YOUR_API_IDENTIFIER` par l’identifiant de votre API dans le dashboard (par exemple, `https://my-fastify-api.example.com`).
        </Warning>
      </Tab>
    </Tabs>
  ...
  <Step title="Configurer le plugin de l’API Auth0" stepNumber={4}>
    ...
    * Enregistre le plugin Auth0 API pour votre domaine Auth0 et l’audience de l’API
  ...
  <Step title="Créer des routes d’API" stepNumber={5}>
    ...
    * `request.user` contient les claims JWT décodés pour les requêtes authentifiées
  ...
  <Step title="Testez votre API" stepNumber={7}>
    ...
    Testez l’endpoint protégé sans token (devrait échouer) :
    ...
    1. Créer une application cliente (application Web ou mobile) qui sert à authentifier les utilisateurs
...
<Check>
  **Vérification**
  ...
  4. Fournit les renseignements sur l’utilisateur à partir des claims du jeton dans `request.user`
</Check>
...
<Accordion title="Claims de jeton personnalisés avec TypeScript">
  Étendez l’interface Token pour ajouter un typage sécurisé à vos claims personnalisés dans vos jetons d’accès :
  ...
  <Note>
    Les claims personnalisés doivent utiliser des URL dotées d’un espace de noms (par exemple, `https://myapp.com/roles`), sauf s’il s’agit de claims OIDC standard. [En savoir plus sur les claims personnalisés](https://auth0.com/docs/secure/tokens/json-web-tokens/create-custom-claims).
  </Note>
</Accordion>
...
<Accordion title="Autorisation basée sur les rôles">
  Mettez en place un contrôle d’accès basé sur les rôles à l’aide de claims personnalisées :
...
<Accordion title="Gestion des erreurs">
  ...
  fastify.setErrorHandler((error, request, reply) => {
    ...
    // Gérer les erreurs de permission/rôle
...
Maintenant que vous avez une API protégée, explorez aussi :
