...
<Accordion title="Utiliser l’IA pour intégrer Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  Si vous utilisez un assistant de codage IA comme Claude Code, Cursor ou GitHub Copilot, vous pouvez ajouter automatiquement l’authentification pour votre API avec Auth0 en quelques minutes à l’aide d’[Agent Skills](https://agentskills.io/home).
  ...
  **Demandez ensuite à votre assistant IA :**
  ...
  Votre assistant IA créera automatiquement votre API Auth0, récupérera les identifiants, installera `@auth0/auth0-fastify-api`, configurera le plugin et protégera vos points de terminaison API avec la validation JWT. [Documentation complète d’Agent Skills →](/docs/fr-CA/quickstart/agent-skills)
</Accordion>
...
<Note>
  ...
  **Compatibilité des versions de Fastify :** Ce Quickstart fonctionne avec **Fastify 5.x** et les versions plus récentes.
</Note>
...
Ce guide Quickstart montre comment protéger des points de terminaison d’API Fastify à l’aide de jetons d’accès JWT. Vous créerez une API sécurisée qui valide les jetons d’accès Auth0 et autorise l’accès à des ressources protégées.
...
<Steps>
  ...
  <Step title="Installer le SDK d’API Auth0 pour Fastify" stepNumber={2}>
    ...
    Mettez à jour votre `package.json` afin d’ajouter des scripts de démarrage :
  ...
  <Step title="Configurer le plugin de l’API Auth0" stepNumber={4}>
    Créez votre serveur Fastify et enregistrez le plugin Auth0 API :
    ...
    * Enregistre le plugin de l’API Auth0 à l’aide de votre domaine Auth0 et de l’audience de l’API
    * Configure la validation du JWT pour les requêtes entrantes
    * Met le preHandler `requireAuth()` à votre disposition pour protéger les routes
  </Step>
  ...
  <Step title="Créer des routes d’API" stepNumber={5}>
    ...
    * `request.user` contient les claims JWT décodés des requêtes authentifiées
  ...
  <Step title="Testez votre API" stepNumber={7}>
    ...
    Testez le endpoint protégé sans token (échec attendu) :
    ...
    Pour faire un Test avec un jeton valide, vous devez :
...
<Accordion title="Claims de jeton personnalisées avec TypeScript">
  Étendez l’interface Token pour ajouter la sécurité de type à vos claims personnalisées dans vos jetons d’accès :

  ```typescript server.ts
  import '@auth0/auth0-fastify-api';

  // Étendez l’interface Token avec vos claims personnalisées
  ...
  TypeScript reconnaîtra alors vos claims personnalisées :
  ...
  <Note>
    Les claims personnalisées doivent utiliser des URL qualifiées par un espace de noms (par ex. `https://myapp.com/roles`), sauf s’il s’agit de claims OIDC standard. [En savoir plus sur les claims personnalisées](https://auth0.com/docs/secure/tokens/json-web-tokens/create-custom-claims).
  </Note>
</Accordion>
...
<Accordion title="Autorisation basée sur les permissions">
  ...
  <Note>
    Les permissions doivent être configurées dans les paramètres de votre API Auth0 et accordées aux clients. [En savoir plus sur les permissions d’API](https://auth0.com/docs/manage-users/access-control/configure-core-rbac/rbac-users/assign-permissions-users).
  </Note>
</Accordion>

<Accordion title="Autorisation basée sur les rôles">
  Mettez en œuvre le contrôle d&#39;accès basé sur les rôles à l&#39;aide de claims personnalisés :
  ...
  // Route réservée au gestionnaire ou à l'administrateur
...
<Accordion title="Limitation du débit">
  Protégez votre API contre les abus grâce à la limitation du débit :
  ...
  await fastify.register(rateLimit, {
    ...
    errorResponseBuilder: (request, context) => {
      return {
        ...
        message: `Limite de requêtes dépassée. Réessayez dans ${context.after}`,
  ...
  fastify.get('/api/expensive-operation', {
    ...
    return { result: 'résultat de l’opération coûteuse' };
  });
  ```
</Accordion>
...
<AccordionGroup>
  <Accordion title="Problèmes courants et solutions">
    ...
    **Solution :** Installez et configurez `@fastify/cors` :
    ...
    await fastify.register(cors, {
      origin: 'http://localhost:3000', // URL de votre interface Web
      credentials: true
    });
    ```
  </Accordion>
...
* **[Quickstart Fastify pour une application web](/docs/fr-CA/quickstart/webapp/fastify)** - Créez une application web qui appelle votre API
