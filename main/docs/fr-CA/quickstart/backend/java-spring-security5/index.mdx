...
<Accordion title="Utiliser l’IA pour intégrer Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **Ensuite, demandez à votre assistant IA :**
  ...
  Votre assistant IA créera automatiquement votre API Auth0, récupérera les identifiants, ajoutera la dépendance Auth0 Spring Boot API SDK, configurera `application.yml` et mettra en place un `SecurityFilterChain` avec validation JWT et points de terminaison protégés. [Documentation complète sur Agent Skills →](/docs/fr-CA/quickstart/agent-skills)
</Accordion>
...
<Note>
  ...
  * L’IDE de votre choix (IntelliJ IDEA, Eclipse ou VS Code avec support Java)
...
Ce Quickstart montre comment ajouter l’authentification JWT d’Auth0 à une API Spring Boot. Vous créerez une API sécurisée avec des points de terminaison protégés à l’aide de l’Auth0 Spring Boot API SDK.
  ...
  <Step title="Configurez votre API Auth0" stepNumber={3}>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        5. Remplacez `YOUR_AUTH0_DOMAIN` dans `application.yml` par votre **Domain** depuis l’onglet Test (par exemple, `your-tenant.auth0.com`)
...
<Accordion title="Appel d’API protégées">
  ...
  **1. Obtenez un jeton d’accès** auprès d’Auth0 à l’aide du flux Client Credentials :
  ...
  <Info>
    ...
    application Machine to Machine dans l’[Auth0 Dashboard](https://manage.auth0.com/#/applications)
  ...
  **3. Testez le point de terminaison protégé sans authentication** (devrait renvoyer 401 Unauthorized) :
...
<Accordion title="Autorisation basée sur les scopes">
  Implémentez un contrôle d’accès granulaire à l’aide des scopes JWT pour une sécurité renforcée.

  **1. Définissez des scopes dans votre API Auth0 :**

  Dans l’[Auth0 Dashboard](https://manage.auth0.com) → APIs → Your API → Permissions, ajoutez des scopes :
...
<AccordionGroup>
  <Accordion title="401 Non autorisé - audience invalide">
    ...
    **Solution :** Assurez-vous que `auth0.audience` correspond exactement à l’identifiant de votre API Auth0. La valeur d’audience dans le jeton doit correspondre à cette valeur.
  ...
  <Accordion title="Valeurs de configuration introuvables">
    ...
    **Solution :** Vérifiez la structure de `application.yml` et les noms des propriétés. Assurez-vous que la section `auth0` contient les valeurs de Domain et d’Audience.
  ...
  <Accordion title="Problèmes de connectivité réseau">
    **Problème :** Échec de récupération de JWKS ou délai d’attente de connexion.

    **Solution :** Il se peut que le pare-feu de votre entreprise bloque les points de terminaison Auth0. Ajoutez les domaines Auth0 à la liste d’autorisation pour l’accès HTTPS :
  ...
  <Accordion title="Les portées ne fonctionnent pas dans les politiques d’autorisation">
    ...
    **Solution :** Assurez-vous que votre jeton d’accès inclut les portées requises. Lorsque vous demandez un jeton, précisez les portées :
    ...
    Vérifiez aussi que les portées sont définies dans les paramètres de votre API Auth0 (Dashboard → APIs → Votre API → Permissions).
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={3}>
  ...
  <Card title="Documentation DPoP" icon="shield" href="https://auth0.com/docs/secure/sender-constraining/demonstrating-proof-of-possession-dpop">
    Découvrez le renforcement de la sécurité par Proof-of-Possession
  </Card>
...
Une application d’exemple complète qui présente toutes les fonctionnalités est offerte dans le repository du SDK.
...
<Card title="Application de démonstration" icon="github" href="https://github.com/auth0/auth0-auth-java/tree/main/auth0-springboot-api-playground">
  Comprend des points de terminaison publics et protégés, le soutien de DPoP et des
  exemples détaillés
</Card>
