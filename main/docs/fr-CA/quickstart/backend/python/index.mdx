...
<Note>
  ...
  **Compatibilité des versions de Flask :** Ce Quickstart nécessite **Flask 3.0** ou version ultérieure pour la prise en charge native de l’asynchrone.
</Note>
...
<Steps>
  ...
  <Step title="Configurez votre API Auth0" stepNumber={3}>
    L’étape suivante consiste à créer une nouvelle API dans votre tenant Auth0 et à configurer votre application.
    ...
    <Tabs>
      <Tab title="Dashboard">
        ...
        <Info>
          ...
          L’**Audience** (API Identifier) est un identificateur unique pour votre API et peut être n’importe quel URI valide.
        </Info>
      </Tab>
  ...
  <Step title="Configurer le client Auth0" stepNumber={5}>
    ...
    <Warning>
      Remplacez `your-tenant.us.auth0.com` par votre domaine Auth0 réel et mettez à jour `API_IDENTIFIER` pour qu’il corresponde à l’identifiant d’API de votre Dashboard.
    </Warning>
  ...
  <Step title="Créer des routes protégées" stepNumber={5}>
    Ajoutez un décorateur pour protéger les routes et créer des points de terminaison publics et privés :
  ...
  <Step title="Lancez votre API" stepNumber={6}>
    ...
    Votre API est maintenant accessible à l’adresse `http://localhost:5000`.
  </Step>
</Steps>
...
<Check>
  **Vérification**
  ...
  Vous devriez maintenant avoir une API Flask entièrement fonctionnelle protégée par Auth0 qui s’exécute sur votre localhost et comporte trois points de terminaison :
...
Remplacez `YOUR_ACCESS_TOKEN` par le token que vous avez copié depuis l’Auth0 Dashboard.
...
<Accordion title="Authentification DPoP">
  Pour une sécurité renforcée, activez DPoP (Demonstrating Proof-of-Possession). DPoP améliore OAuth 2.0 en liant les jetons d’accès à des clés cryptographiques.
  ...
  api_client = ApiClient(ApiClientOptions(
      ...
      dpop_required=False  # Par défaut - permet à la fois les jetons Bearer et DPoP
  ))
...
<Accordion title="Autorisation basée sur les scopes">
  Créez un décorateur pour vérifier la présence de scopes précis :
...
<Accordion title="Bonnes pratiques de gestion des erreurs">
  Prévoyez une gestion complète des erreurs avec des types d’erreurs précis :
  ...
  def handle_token_verification_error(error):
      """Gère précisément les échecs de vérification de jeton"""
  ...
  def handle_generic_error(error):
      """Intercepte toute erreur inattendue"""
      app.logger.error(f"Unexpected error: {str(error)}", exc_info=True)
      return jsonify({"error": "Internal server error"}), 500
  ```

  <Info>
    Toutes les erreurs d’authentification héritent de `BaseAuthError`, qui fournit des méthodes comme `get_status_code()`, `get_headers()` et `get_error_code()` pour retourner des réponses HTTP appropriées avec des en-têtes WWW-Authenticate.
  </Info>
</Accordion>
...
<AccordionGroup>
  <Accordion title="401 Non autorisé - audience invalide">
    ...
    **Solution** :
    ...
    1. Vérifiez que `AUTH0_AUDIENCE` dans votre fichier `.env` correspond exactement à l’identificateur d’API Auth0
  ...
  <Accordion title="Erreurs de prise en charge async dans Flask">
    ...
    **Cause** : Vous utilisez des routes async sans Flask 3.0+ ou vous mélangez mal sync et async
  ...
  <Accordion title="Erreurs de jeton expiré">
    ...
    **Solution** :
    ...
    1. Demandez un nouveau jeton dans l’onglet Test du Auth0 Dashboard
...
Consultez le [dépôt d’exemples de l’API Python d’Auth0](https://github.com/auth0-samples/auth0-python-api-samples) pour découvrir des exemples complets et prêts à l’emploi avec Flask.