---
title: 'API Flask'
description: 'Ce guide explique comment intégrer Auth0 à toute API Python, nouvelle ou existante, créée avec Flask.'
---
...
Ce guide explique comment intégrer Auth0 à n’importe quelle API Python, nouvelle ou existante, développée avec [Flask](https://flask.palletsprojects.com/).
...
<Steps>
  ...
  <Step title="Configurer votre API Auth0" stepNumber={3}>
    ...
    <Tabs>
      <Tab title="Dashboard">
        ...
        6. Copiez l’**Identifier** que vous venez de créer (c’est votre audience)

        <Info>
          Votre **Domain** ne doit pas inclure `https://` — utilisez uniquement le nom de domaine (par ex. : `your-tenant.auth0.com`).

          L’**Audience** (API Identifier) est un identifiant unique pour votre API et peut être n’importe quel URI valide.
        </Info>
      </Tab>
  ...
  <Step title="Configurer le client Auth0" stepNumber={5}>
    ...
    <Warning>
      Remplacez `your-tenant.us.auth0.com` par votre domaine Auth0 et mettez à jour `API_IDENTIFIER` pour qu’il corresponde à l’identifiant de votre API dans le dashboard.
    </Warning>
...
<Check>
  **Point de contrôle**
  ...
  Vous devriez maintenant avoir une API Flask protégée par Auth0, entièrement fonctionnelle, en cours d’exécution sur votre machine locale, avec trois points de terminaison :

  * `/api/public` - Accessible sans authentification
  * `/api/private` - Nécessite un jeton d’accès Auth0 valide
  * `/api/private-scoped` - Nécessite l’authentification et la permission `read:messages`
</Check>
...
Testez le point de terminaison public (aucun jeton n’est requis) :

```bash
curl http://localhost:5000/api/public
```

Testez le point de terminaison protégé (token requis) :
...
Remplacez `YOUR_ACCESS_TOKEN` par le token que vous avez copié à partir de l’Auth0 Dashboard.
...
<Accordion title="Validation des claims personnalisés">
  Exigez la présence de claims spécifiques dans le jeton d’accès :
  ...
  try:
      ...
      return jsonify({"error": "Claims requis manquants"}), 401
  ```
</Accordion>
...
<Accordion title="Meilleures pratiques de gestion des erreurs">
  Mettez en place une gestion complète des erreurs avec des types d’erreurs précis :
  ...
  def handle_token_verification_error(error):
      """Gérer spécifiquement les échecs de validation de jeton"""
  ...
  <Info>
    Toutes les erreurs d’authentification dérivent de `BaseAuthError`, qui fournit des méthodes comme `get_status_code()`, `get_headers()` et `get_error_code()` afin de générer des réponses HTTP appropriées avec des en-têtes WWW-Authenticate.
  </Info>
</Accordion>
...
<Accordion title="Utiliser le middleware before_request">
  ...
  async def protected_data():
      # Les claims sont automatiquement accessibles via g.user_claims
...
<AccordionGroup>
  <Accordion title="401 Non autorisé - audience invalide">
    ...
    **Solution** :
    ...
    1. Vérifiez que `AUTH0_AUDIENCE` dans votre fichier `.env` correspond exactement à l’identifiant de votre API Auth0
  ...
  <Accordion title="Valeurs de configuration introuvables">
    ...
    **Solution** :
    ...
    3. Vérifiez que les noms de variables correspondent exactement (sensibles à la casse)
  </Accordion>

  <Accordion title="Erreurs liées à la prise en charge de l’asynchrone dans Flask">
    **Symptôme** : `RuntimeError: This event loop is already running` ou erreurs async similaires

    **Cause** : Utilisation de routes async sans Flask 3.0+ ou mélange incorrect de code sync et async
  ...
  <Accordion title="Erreurs de jeton expiré">
    ...
    **Cause** : Le jeton d’accès a expiré

    **Solution** :

    1. Demandez un nouveau jeton dans l’onglet Test de l’Auth0 Dashboard
...
<CardGroup cols={3}>
  ...
  <Card title="Guide d’authentification des API" icon="shield" href="https://auth0.com/docs/secure/tokens/access-tokens">
    Découvrez les jetons d’accès et la sécurité des API
  </Card>

  <Card title="Documentation DPoP" icon="lock" href="https://auth0.com/docs/secure/sender-constraining/demonstrating-proof-of-possession-dpop">
    Découvrez la sécurité Proof-of-Possession
  </Card>
