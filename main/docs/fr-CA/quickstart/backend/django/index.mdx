...
Ce tutoriel montre comment ajouter la gestion des autorisations à une API Django REST Framework. Nous vous recommandons de vous connecter pour suivre ce Quickstart avec des exemples configurés pour votre compte.
...
<Info>
  **Vous débutez avec Auth0 ?** Découvrez [comment fonctionne Auth0](/docs/fr-CA/get-started/auth0-overview) et apprenez-en davantage sur [la mise en œuvre de l’authentification et de l’autorisation des API](/docs/fr-CA/get-started/authentication-and-authorization-flow) à l’aide du framework OAuth 2.0.
</Info>
...
Vous devez définir une façon de mapper le nom d’utilisateur du contenu du jeton d’accès à l’utilisateur du système d’authentification de Django.
...
Ajoutez l’application `rest_framework` à l’entrée `INSTALLED_APPS`.
...
Définissez `JWT_AUDIENCE` avec l’identifiant de votre API et `JWT_ISSUER` avec votre domaine Auth0. Par défaut, ces valeurs sont récupérées depuis le fichier `.env`.
...
Créez la fonction permettant de récupérer le JWKS de votre compte Auth0 afin de vérifier et de décoder le jeton d’accès reçu.
...
* `GET /api/private` : accessible aux requêtes authentifiées contenant un jeton d’accès sans scopes supplémentaires
...
Utilisez le décorateur `requires_scope` dans les méthodes qui exigent certains scopes. La méthode ci-dessous exige le scope `read:messages`.
...
Django possède un [répartiteur d’URL](https://docs.djangoproject.com/en/2.2/topics/http/urls/) qui permet d’associer des modèles d’URL à des vues.
...
Le projet Django contient également un fichier `urls.py`. Ajoutez une référence au fichier `urls.py` de votre application.
