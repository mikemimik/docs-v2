...
description: "Ajouter l’authentification JWT d’Auth0 à une ASP.NET Core Web API avec des points de terminaison protégés"
---
...
Ce Quickstart montre comment ajouter l’authentification JWT d’Auth0 à une ASP.NET Core Web API. Vous créerez une API sécurisée avec des points de terminaison protégés à l’aide du SDK d’API ASP.NET Core d’Auth0.
...
<Steps>
  ...
  <Step title="Configurez votre API Auth0" stepNumber={3}>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        5. Remplacez `YOUR_AUTH0_DOMAIN` dans `appsettings.json` par votre **Domain** de l’onglet **Test** (par ex., `your-tenant.auth0.com`)
        6. Remplacez `YOUR_AUTH0_API_IDENTIFIER` dans `appsettings.json` par votre **Identifier** (par ex., `https://my-api`)
  ...
  <Step title="Lancez votre API" stepNumber={6}>
    ...
    Votre API s’exécute maintenant à l’adresse `https://localhost:7190` (ou à une adresse semblable - vérifiez la sortie de la console pour connaître l’URL exacte).
  </Step>
</Steps>
...
<Accordion title="Utiliser des points de terminaison avec contrôleurs">
  Pour les API plus volumineuses, utilisez des contrôleurs plutôt que des API minimales.

  **1. Ajoutez le soutien des contrôleurs :**
...
<Accordion title="Protéger les routes avec une autorisation basée sur les scopes">
...
<Accordion title="Utilisation de DPoP pour Enhanced Security">
  DPoP (Demonstration of Proof-of-Possession) lie les jetons d’accès à des clés cryptographiques, ce qui prévient le vol de jetons et les attaques par rejeu.
...
<Accordion title="Implémenter des politiques d’autorisation personnalisées">
  ...
  **3. Enregistrez la politique et utilisez-la :**
...
<Accordion title="Accès aux renseignements de l’utilisateur">
  Récupérez les renseignements de l’utilisateur à partir du jeton d’authentification.
...
<Accordion title="Validation personnalisée des jetons">
  Personnalisez les paramètres de validation des jetons JWT selon des exigences particulières.
...
<CardGroup cols={3}>
  ...
  <Card title="Exemples de code" icon="code" href="https://github.com/auth0/aspnetcore-api/blob/master/EXAMPLES.md">
    Exemples de code et patrons complets
  </Card>

  <Card title="documentation DPoP d’Auth0" icon="shield" href="https://auth0.com/docs/secure/sender-constraining/demonstrating-proof-of-possession-dpop">
...
<AccordionGroup>
  <Accordion title="401 Unauthorized - audience invalide">
    ...
    **Solution :** Assurez-vous que `Audience` dans `appsettings.json` correspond exactement à l’identifier de votre Auth0 API. La claim `audience` du jeton doit correspondre à cette valeur.
    ...
    {
      "Auth0": {
        "Audience": "https://my-api"  // Doit correspondre à l’identifier de l’API Auth0
      }
    }
    ```
  </Accordion>
  ...
  <Accordion title="401 Unauthorized - issuer invalide">
    **Problème :** La validation du jeton échoue en raison d’une erreur d’issuer.
...
Un exemple d’application complet présentant toutes les fonctionnalités se trouve dans le dépôt du SDK.
...
<Card title="Application de démonstration" icon="github" href="https://github.com/auth0/aspnetcore-api/tree/master/Auth0.AspNetCore.Authentication.Api.Playground">
  Comprend des points de terminaison publics et protégés, la prise en charge de DPoP, l’intégration à Swagger UI et une collection Postman
</Card>
