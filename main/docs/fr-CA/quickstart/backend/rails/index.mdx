...
Ce tutoriel explique comment ajouter l’autorisation à une API Ruby on Rails. Nous vous recommandons de vous connecter pour suivre ce Quickstart avec des exemples configurés pour votre compte.
...
<Info>
  **Vous découvrez Auth0 ?** Découvrez [comment fonctionne Auth0](/docs/fr-CA/get-started/auth0-overview) et apprenez [comment mettre en œuvre l’authentification et l’autorisation pour les API](/docs/fr-CA/get-started/authentication-and-authorization-flow) à l’aide du framework OAuth 2.0.
</Info>
...
<Info>
  Nous vous recommandons d’utiliser l’algorithme de [signing](/docs/fr-CA/get-started/applications/signing-algorithms) RS256 par défaut pour votre API. Si vous devez utiliser l’algorithme HS256, consultez l’[exemple d’intégration HS256](https://github.com/auth0-samples/auth0-rubyonrails-api-samples/tree/OIDC/02-Authentication-HS256).
</Info>
...
Ce tutoriel montre comment valider un jeton d’accès à l’aide de la gem [**jwt**](https://github.com/jwt/ruby-jwt) dans une classe `Auth0Client` personnalisée. Un Concern appelé `Secured` sert à autoriser les points de terminaison qui nécessitent une authentification au moyen d’un jeton d’accès entrant.
...
Créez une classe appelée `Auth0Client` qui décode et vérifie le jeton d’accès reçu dans l’en-tête `Authorization` de la requête. Vous pouvez récupérer la clé publique de votre tenant Auth0 pour vérifier le jeton.
...
La méthode `Auth0Client.validate_token` ci-dessus vérifie que le jeton d’accès inclus dans la requête est valide; toutefois, elle ne comprend pas encore de mécanisme pour vérifier que le jeton a le **scope** nécessaire pour accéder aux ressources demandées.

Pour rechercher un `scope` précis dans un jeton d’accès, créez une nouvelle struct dans votre classe `Auth0Client` appelée `Token` et définissez-y une nouvelle méthode, `validate_permissions`, qui, à partir d’un tableau de scopes requis, vérifiera s’ils sont présents dans le payload du jeton.
...
* `GET /api/private` : disponible pour les requêtes authentifiées contenant un jeton d’accès sans autre scope
...
Pour vérifier que votre jeton d’accès dispose des autorisations requises, appelez la méthode `validate_permissions` comme suit dans l’action `private-scoped` :
