...
<Info>
  **Vous débutez avec Auth0 ?** Découvrez [comment fonctionne Auth0](/docs/fr-CA/get-started/auth0-overview) et [comment mettre en œuvre l’authentification et l’autorisation des API](/docs/fr-CA/get-started/authentication-and-authorization-flow) à l’aide du framework OAuth 2.0.
</Info>
...
Dans la section [APIs](https://manage.auth0.com/#/apis) du tableau de bord Auth0, cliquez sur **Créer une API**. Saisissez un nom et un identifiant pour votre API, par exemple `https://quickstarts/api`. Vous utiliserez cet identifiant plus tard comme `audience`, lorsque vous configurerez la vérification du jeton d’accès. Laissez **l’algorithme de signature** à **RS256**.
...
Ce tutoriel montre comment valider un jeton d’accès à l’aide de la gem [**jwt**](https://github.com/jwt/ruby-jwt) dans une classe `Auth0Client` personnalisée. Un Concern nommé `Secured` sert à autoriser les points de terminaison qui nécessitent une authentification au moyen d’un jeton d’accès entrant.
...
Créez une classe appelée `Auth0Client` qui décode et vérifie le jeton d’accès reçu, extrait de l’en-tête `Authorization` de la requête. La clé publique de votre tenant Auth0 peut être récupérée afin de vérifier le jeton.
...
Créez un Concern appelé `Secured` qui recherche le jeton d’accès dans l’en-tête `Authorization` d’une requête reçue. Si le jeton est présent, il doit être transmis à `Auth0Client.validate_token`.
...
La méthode `Auth0Client.validate_token` ci-dessus vérifie que le jeton d’accès inclus dans la requête est valide; cependant, elle ne comprend pas encore de mécanisme permettant de vérifier que le jeton dispose du **scope** nécessaire pour accéder aux ressources demandées.

Pour vérifier la présence d’un `scope` particulier dans un jeton d’accès, créez une nouvelle structure dans votre classe `Auth0Client` appelée `Token` et définissez-y une nouvelle méthode, `validate_permissions`, qui, à partir d’un tableau de scopes requis, vérifiera s’ils sont présents dans le payload du jeton.
...
Ensuite, dans le concern `Secured`, définissez une nouvelle constante d’erreur `INSUFFICIENT_PERMISSIONS` pour renvoyer un message d’erreur approprié lorsqu’une ressource est demandée sans les permissions requises. Mettez ensuite à jour la valeur de retour de l’appel `Auth0Client.validate_token`, puis créez une nouvelle méthode `validate_permissions` pour vérifier si le token possède les bonnes permissions; sinon, renvoyez un code d’état `403 FORBIDDEN` avec le message d’erreur `INSUFFICIENT_PERMISSIONS`.
...
* `GET /api/private-scoped` : accessible pour les requêtes authentifiées contenant un jeton d’accès avec le scope `read:messages`
...
Pour vérifier que votre jeton d’accès a les permissions appropriées, appelez la méthode `validate_permissions` comme suit dans l’action `private-scoped` :
