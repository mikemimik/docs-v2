...
<Warning>
  Les métadonnées du jeton d’actualisation sont actuellement en accès anticipé pour les clients d’entreprise uniquement. En utilisant cette fonctionnalité, vous acceptez les modalités d’essai gratuit applicables énoncées dans le [Master Subscription Agreement](https://www.okta.com/legal) d’Okta. Pour en savoir plus sur le cycle de lancement des produits d’Auth0, consultez [les étapes de lancement du produit](https://auth0.com/docs/troubleshoot/product-lifecycle/product-release-stages).
</Warning>

Pour configurer les métadonnées du jeton d’actualisation, vous pouvez utiliser une [Action Post-Login](/docs/fr-CA/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger) Auth0 et la [Management API](https://auth0.com/docs/api/management/v2).

<div id="auth0-post-login-actions">
  ## Actions post-login d’Auth0
</div>

Vous pouvez gérer les opérations CRUD sur les métadonnées du jeton d’actualisation à l’aide des objets `api.refreshToken` dans une [Action post-login](/docs/fr-CA/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger). Vous pouvez ainsi gérer les métadonnées du jeton d’actualisation selon une logique propre à l’utilisateur ou au contexte.
...
Les modifications sont immédiatement disponibles dans l’objet `event.refresh_token` des Actions suivantes.
...
* `api.refreshToken.deleteMetadata("key")` supprime les métadonnées du jeton d’actualisation spécifiées
...
Vous pouvez gérer les requêtes CRUD (création, remplacement, mise à jour et suppression) des métadonnées des jetons d’actualisation à l’aide de la Management API :

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Les appels au point de terminaison `/api/v2/refresh-tokens/{id}` nécessitent un [Management API access token](/docs/fr-CA/secure/tokens/access-tokens) doté de la portée `update:refresh_tokens`.
</Callout>
...
Effectuez une requête `PATCH` au point de terminaison [`/api/v2/refresh-tokens/{id}`](https://auth0.com/docs/api/management/v2/refresh-tokens/patch-refresh-token) :
...
<div id="use-case-store-and-consume-organization-context">
  ## Cas d’utilisation : stocker et exploiter le contexte d’organisation
</div>

Vous pouvez utiliser les métadonnées du jeton d’actualisation pour stocker le contexte d’organisation lors de l’authentification initiale, puis l’exploiter plus tard lors des échanges de jeton d’actualisation. C’est utile pour les systèmes en aval, comme les pipelines d’audit, d’analyse et de révocation.
...
Lors d’un échange de jeton d’actualisation, l’objet `event.refresh_token` existe et vous pouvez lire les métadonnées précédemment stockées :
...
Vous pouvez aussi interroger les métadonnées du jeton d’actualisation à l’aide du point de terminaison [/api/v2/refresh-tokens/{id}](https://auth0.com/docs/api/management/v2/refresh-tokens/get-refresh-token) :
...
<div id="use-case-track-and-validate-device-information">
  ## Cas d’utilisation : Suivi et validation des renseignements sur l’appareil
</div>

Vous pouvez utiliser les métadonnées du jeton d’actualisation pour recueillir des renseignements sur l’appareil lors de l’authentification initiale et les valider lors des échanges subséquents de jetons d’actualisation à des fins de sécurité.

<div id="set-device-information-at-initial-authentication">
  ### Définir les renseignements de l’appareil lors de l’authentification initiale
</div>
...
<div id="validate-device-information-during-refresh-token-exchange">
  ### Valider les renseignements sur l’appareil lors de l’échange du jeton d’actualisation
</div>
...
Vous pouvez consulter les événements de journal des métadonnées du jeton d’actualisation en accédant à [Dashboard &gt; Monitoring &gt; Logs](https://manage.auth0.com/#/logs), ou récupérer des journaux à l’aide du endpoint [Management API logs](/docs/fr-CA/api/management/v2/logs/get-logs).

* Si une erreur se produit lors de l’ajout ou de la mise à jour des métadonnées du jeton d’actualisation avec Actions, la transaction d’authentification échoue et une erreur est renvoyée à l’URL de callback.
...
* Si une erreur survient lors de la gestion des métadonnées du jeton d’actualisation au moyen de l’Auth0 Management API, l’API renvoie une erreur `HTTP status: 400` ainsi que le message correspondant :
