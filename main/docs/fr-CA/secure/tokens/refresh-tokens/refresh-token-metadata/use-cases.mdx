...
Les métadonnées du jeton d’actualisation et les métadonnées de session vous permettent, ensemble, de créer des revendications personnalisées persistantes afin d’ajouter de l’information aux jetons [ID](/docs/fr-CA/secure/tokens#id-tokens) et [d’accès](/docs/fr-CA/secure/tokens/access-tokens).

Grâce aux revendications personnalisées persistantes, vous pouvez accéder à des données propres à l’application, comme :
...
Configurez un déclencheur [post-login](/docs/fr-CA/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger#login-/-post-login) [Action](/docs/fr-CA/customize/actions) pour créer des revendications personnalisées persistantes et les enregistrer dans les métadonnées du jeton d’actualisation à l’aide de l’objet `api.refreshToken.setMetadata()`.
...
exports.onExecutePostLogin = async (event, api) => {
  ...
  // --- Fonction d'aide pour simuler le calcul des revendications personnalisées ---
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Une même action `post-login` peut gérer différents scénarios de `grant_type` à l’aide de l’objet `event.request.body.grant_type` afin d’assurer la persistance des revendications. L’objet `event.refresh_token` est accessible en lecture seule pendant les échanges de jetons d’actualisation.
</Callout>
...
Les métadonnées du jeton d’actualisation et les métadonnées de session vous permettent de créer un ID de session unique afin de mettre en place un identifiant de session persistant, conservé pendant toute la durée de la session d’un utilisateur, y compris lors des [rotations de jeton d’actualisation](docs/secure/tokens/refresh-tokens/refresh-token-rotation).
...
* Offrir des expériences UX cohérentes sur plusieurs cycles de vie de jetons.

Configurez un déclencheur d’Action [post-login](/docs/fr-CA/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger#login-/-post-login) pour créer un ID de session unique et l’attribuer à la session de l’utilisateur au moyen des objets `api.session.setMetadata()` et `api.refreshToken.setMetadata()`.

Ajoutez l’ID de session unique comme revendication personnalisée aux jetons d’ID et d’accès au moyen des objets `api.idToken.setCustomClaim()` et `api.accessToken.setCustomClaim()`.
...
exports.onExecutePostLogin = async (event, api) => {
  ...
  // Si elle n'est pas trouvée dans les métadonnées de session, vérifier si elle est disponible dans les métadonnées du jeton d'actualisation.
...
Lors d’un échange de jeton d’actualisation, un déclencheur d’action post-login subséquent peut accéder à ces revendications personnalisées au moyen de l’objet `event.refresh_token.metadata` et les appliquer aux nouveaux jetons d’actualisation émis à l’aide des objets `api.idToken.setCustomClaim()` et `api.accessToken.setCustomClaim()`.

<div id="create-a-tenant-identifier">
  ## Créer un identifiant de tenant
</div>

Ensemble, les métadonnées du jeton d’actualisation et les métadonnées de session vous permettent de créer un identifiant de tenant persistant pour gérer des applications multi-tenant, où une seule instance d’application sert plusieurs organisations clientes, et qui est conservé pendant toute la durée de la session d’un utilisateur.
...
* Créer une expérience utilisateur adaptée afin d’offrir du contenu et des fonctionnalités pertinents pour le contexte de tenant actuel de l’utilisateur
* Simplifier la logique multi-tenant en centralisant l’identification du tenant et sa propagation dans Auth0
...
* Améliorer l’évolutivité en réduisant le besoin d’effectuer des requêtes répétées vers la base de données ou d’avoir recours à une logique complexe pour déterminer le contexte de tenant à chaque appel d’API ou actualisation de jeton
...
Une fois le tenant identifié, attribuez la valeur de l’identifiant de tenant à la session de l’utilisateur à l’aide des objets `api.session.setMetadata()` et `api.refreshToken.setMetadata()`, puis ajoutez-la en tant que revendication personnalisée aux jetons ID et d’accès à l’aide des objets `api.idToken.setCustomClaim()` et `api.accessToken.setCustomClaim()`.
...
Les métadonnées du jeton d’actualisation et les métadonnées de session vous permettent de gérer, tout au long de la session d’un utilisateur, les données transitoires et contextuelles provenant des IDP en amont, sans les enregistrer de façon permanente dans le profil Auth0 de l’utilisateur.
...
* Accroître la flexibilité en prenant en charge des exigences de données variées provenant de différents IDP, sans imposer de modification du schéma ni alourdir les profils utilisateur persistants
...
* Réduire la charge de développement en simplifiant le traitement des données IDP transitoires, puisque Auth0 Actions et les métadonnées gèrent le cycle de vie des données

Configurez un déclencheur d’action [post-login](/docs/fr-CA/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger#login-/-post-login) pour identifier les données de profil utilisateur contenues dans les objets `event.request`, `event.user` et `event.context`.

Déterminez quelles données sont transitoires ou contextuelles et attribuez-les à la session de l’utilisateur à l’aide des objets `api.session.setMetadata()` et `api.refreshToken.setMetadata()`, puis ajoutez les données transitoires comme revendication personnalisée aux jetons d’identification et d’accès à l’aide des objets `api.idToken.setCustomClaim()` et `api.accessToken.setCustomClaim()`.
...
exports.onExecutePostLogin = async (event, api) => {
  ...
  // Ceci est illustratif ; la prise d'empreinte réelle de l'appareil pourrait être plus complexe
  ...
  // Facultativement, ajouter ces éléments comme revendications aux jetons d'accès si nécessaire pour les API
  // L'utilisation d'espaces de noms personnalisés est une bonne pratique pour les revendications spécifiques à l'application.
...
Lors d’un échange de jeton d’actualisation, un déclencheur d’Action `post-login` ultérieur peut accéder à ces revendications personnalisées à l’aide de l’objet `event.refresh_token.metadata`, puis les appliquer aux jetons d’actualisation nouvellement émis au moyen de l’objet `api.accessToken.setCustomClaim()`.

<div id="enhance-security-and-fraud-detection">
  ## Renforcer la sécurité et la détection des fraudes
</div>

Les métadonnées du jeton d’actualisation et les métadonnées de session vous permettent ensemble de mettre en place une sécurité adaptative en assurant le suivi et la comparaison d’informations contextuelles tout au long de la session d’un utilisateur, y compris les rotations de jeton d’actualisation et les requêtes d’[authentification silencieuse](/docs/fr-CA/authenticate/login/configure-silent-authentication).

La mise en place d’une sécurité adaptative vous permet de :

* Mettre en œuvre une détection proactive des menaces en identifiant automatiquement les changements suspects dans les données de contexte utilisateur et en y répondant, ce qui réduit le risque de détournement de session et d’accès non autorisé.
* Réduire les frictions pour les utilisateurs légitimes en ne demandant la MFA ou une vérification supplémentaire que lorsqu’une véritable anomalie est détectée, ce qui améliore l’expérience utilisateur par rapport à des exigences systématiques de MFA.

Configurez un déclencheur d’Action [post-login](/docs/fr-CA/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger#login-/-post-login) pour identifier les données contextuelles utilisateur, qui peuvent inclure l’empreinte de l’appareil, l’emplacement géographique, les attributs réseau et les attributs comportementaux. Stockez ces données contextuelles dans la session de l’utilisateur à des fins de comparaison, à l’aide de l’objet `api.session.setMetadata()`.
...
exports.onExecutePostLogin = async (event, api) => {
  ...
  if (event.session && !event.session.metadata) {
    ...
    // Comparer l'empreinte stockée avec l'empreinte reçue 
