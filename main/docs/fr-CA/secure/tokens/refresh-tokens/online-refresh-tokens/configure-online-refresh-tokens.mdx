...
Pour activer les ORT, effectuez une requête `PATCH` au point de terminaison [Mettre à jour le serveur de ressources](/docs/fr-CA/api/management/v2/resource-servers/patch-resource-servers-by-id) :
...
Pour vérifier la configuration, effectuez une requête `GET` auprès du serveur de ressources :
