...
Si le jeton d’actualisation est associé à une session déjà authentifiée, le journal contiendra une référence à cette session authentifiée dans l’attribut `session_id`.
...
* **api.refreshToken.setIdleExpiresAt(idle)** vous permet de définir une nouvelle date d’expiration en cas d’inactivité pour un jeton d’actualisation donné.

Vous pouvez utiliser ces méthodes pour personnaliser dynamiquement la durée de vie du jeton d’actualisation et les politiques d’inactivité en fonction de ce qui suit :
...
* Les jetons d’actualisation sans expiration ou les jetons d’actualisation qui n’ont pas été échangés ne contiendront pas la propriété `event.refresh_token.last_exchanged_at`.

* Pour des raisons de sécurité, les délais d’expiration d’inactivité et absolus ne peuvent pas être définis au-delà des paramètres de jeton d’actualisation de l’application indiqués dans [les expirations des jetons d’actualisation](/docs/fr-CA/secure/tokens/refresh-tokens/configure-refresh-token-expiration). Si vous essayez de définir une date au-delà de ces paramètres d’expiration, les méthodes d’API effectueront la mise à jour jusqu’aux valeurs définies dans [les expirations des jetons d’actualisation](/docs/fr-CA/secure/tokens/refresh-tokens/configure-refresh-token-expiration) et consigneront un événement d’avertissement (`w`) dans les journaux du tenant.

* `api.refreshToken.setExpiresAt()` et `api.refreshToken.setIdleExpiresAt()` peuvent uniquement raccourcir leur durée de vie respective à partir des valeurs actuelles. Elles ne peuvent ni la prolonger ni l’augmenter.
...
Dans cet exemple, une vérification est effectuée au début de l’Action pour confirmer que `event.authentication.ImpossibleTravel.code` est égal à la propriété `impossible_travel_from_last_login`. Si la valeur est `true`, l’Action appelle `api.refreshToken.revoke()` pour :
...
<div id="revoke-refresh-tokens-due-to-ip-binding">
  ### Révoquer des jetons d’actualisation en raison de la liaison à l’adresse IP
</div>

Si vous utilisez les propriétés de l’objet post-login `event.refresh_token.device.initial_ip` et `event.request.ip` pour vous assurer qu’une transaction de jeton d’actualisation reste liée à la même adresse IP pendant toute sa durée. Dans ce scénario, tout changement d’adresse IP est considéré comme un risque, et un nouveau jeton d’actualisation est requis.
...
Dans cet exemple, une vérification est effectuée au début de l’Action pour faire le suivi des adresses IP à l’aide des propriétés `event.refresh_token.device.initial_ip` et `event.request.ip`. L’Action détermine si l’adresse IP de la transaction a changé. Si c’est le cas, l’Action appelle `api.refreshToken.revoke()` pour :
...
Autrement, pour une Action moins restrictive, vous pouvez faire le suivi des propriétés event.`request.asn` et `event.refresh_token.device.initial_asn` afin de surveiller les changements d’ASN plutôt que les changements d’IP.
...
```js lines
exports.onExecutePostLogin = async (event, api) => {
  // Métadonnées du délai d'expiration du jeton d'actualisation (en millisecondes) configurées dans les Organizations
...
Dans cet exemple, si un délai d’expiration absolu précis est défini pour une organisation, l’Action règle le délai d’expiration absolu du jeton d’actualisation pour qu’il soit égal à :
...
exports.onExecutePostLogin = async (event, api) => {
  ...
  // Si l'application a un délai d'inactivité spécifique défini, appliquer le délai d'expiration
