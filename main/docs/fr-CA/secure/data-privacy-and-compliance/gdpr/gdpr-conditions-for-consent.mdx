---
description: Décrit les fonctionnalités d’Auth0 qui peuvent vous aider à respecter les exigences du GDPR relatives aux conditions du consentement.
title: 'GDPR : conditions du consentement'
---
...
Conformément à l’article 7 du GDPR, vous devez obtenir le consentement des utilisateurs au traitement de leurs données personnelles sous une forme claire et facilement accessible. Vous devez également être en mesure de démontrer que l’utilisateur a donné son consentement et offrir un moyen simple de le retirer à tout moment.

Cet article explique comment utiliser les fonctionnalités d’Auth0 pour mettre en œuvre ces exigences.

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Le contenu de ces documents ne constitue pas un avis juridique et ne doit pas non plus être considéré comme un substitut à une assistance juridique. La responsabilité finale de comprendre le GDPR et de s’y conformer vous incombe, bien qu’Auth0 vous aide, dans la mesure du possible, à satisfaire aux exigences du GDPR.
</Callout>
...
<Warning>
  Les métadonnées d’Auth0 ne constituent pas un espace de stockage sécurisé et ne doivent pas être utilisées pour stocker des renseignements sensibles, comme des secrets à haut risque et des renseignements personnels identifiables (PII), par exemple des numéros d’assurance sociale ou de carte de crédit. Les clients Auth0 sont fortement encouragés à évaluer les données stockées dans les métadonnées et à n’y conserver que celles qui sont nécessaires à la gestion des identités et des accès.
</Warning>
...
Vous pouvez personnaliser l’interface utilisateur de Lock pour afficher des liens vers vos pages de conditions générales et/ou de politique de confidentialité, ainsi qu’une case de consentement que l’utilisateur doit cocher pour pouvoir s’inscrire. Cela peut se faire avec l’option Lock `mustAcceptTerm`s. Cette propriété, lorsqu’elle est définie sur `true`, affiche une case à cocher à côté des conditions générales, qui doit être cochée avant l’inscription. Les conditions générales peuvent être définies à l’aide de l’option languageDictionary. Pour en savoir plus, consultez [Options de configuration de Lock](/docs/fr-CA/libraries/lock/lock-configuration).

Une fois que l’utilisateur a accepté et s’est inscrit, enregistrez les renseignements sur le consentement dans le `user_metadata` à l’aide d’une règle qui s’exécutera lors de la première connexion. Pour en savoir plus sur les règles, consultez [Règles Auth0](/docs/fr-CA/customize/rules).

Si vous souhaitez obtenir plus d’information auprès des utilisateurs pendant l’inscription, et que vous authentifiez les utilisateurs avec une connexion de base de données, vous pouvez ajouter des champs personnalisés à l’interface utilisateur de Lock. Cela peut se faire avec l’option Lock additionalSignUpFields. Tous les champs personnalisés sont automatiquement ajoutés au `user_metadata`.

Si vous utilisez des connexions sociales, l’ajout de champs personnalisés n’est pas possible, mais vous pouvez rediriger l’utilisateur vers une autre page où vous demandez le consentement et tout renseignement supplémentaire, puis le rediriger pour terminer la transaction d’authentification. Cela peut se faire avec des règles de redirection. Pour en savoir plus, consultez [Rediriger les utilisateurs depuis les Rules](/docs/fr-CA/customize/rules/redirect-users). Une fois le processus d’inscription terminé, enregistrez les renseignements sur le consentement dans le `user_metadata` en appelant la <Tooltip tip="Management API : Un produit permettant aux clients d'effectuer des tâches administratives." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Management+API">Management API</Tooltip> [point de terminaison **Update User**](https://auth0.com/docs/api/management/v2#!/Users/patch_users_by_id).
...
Autrement, si vous utilisez Auth0.js à partir d’une SPA, vous pouvez [utiliser la méthode `signup`](/docs/fr-CA/libraries/auth0js) pour créer l’utilisateur dans Auth0 et enregistrer les renseignements sur le consentement dans le `user_metadata`.
...
* Déterminer si un nouveau consentement est nécessaire pour vos utilisateurs, selon vos anciennes conditions générales et vos certifications antérieures en matière de protection des renseignements personnels.
...
<div id="track-consent">
  ## Assurer le suivi du consentement
</div>

Conformément au GDPR, vous devriez être en mesure de démontrer que l’utilisateur a consenti au traitement de ses données personnelles.

Avec Auth0, vous pouvez enregistrer les renseignements relatifs au consentement de l’utilisateur dans le `user_metadata`. Vous pouvez soit enregistrer uniquement un indicateur montrant si l’utilisateur a consenti ou non, soit un ensemble de renseignements et de préférences liés au consentement (y compris, par exemple, la date à laquelle l’utilisateur a donné son consentement, les conditions auxquelles il a consenti, etc.). Vous pourrez ensuite accéder à ces renseignements et les gérer à l’aide de la Management API.

La Management API offre également plusieurs options pour la recherche d’utilisateurs, ainsi que des points de terminaison permettant de mettre à jour les métadonnées utilisateur ou d’exporter des utilisateurs en lot.
...
Définissez le paramètre de requête **fields** à `user_metadata` afin de limiter les champs renvoyés. Ainsi, seules les données `user_metadata` seront renvoyées au lieu du profil utilisateur complet.
...
Définissez le paramètre de requête **fields** sur `user_metadata` afin de limiter les champs renvoyés. Ainsi, seul `user_metadata` sera renvoyé au lieu du profil complet de l’utilisateur.
...
Pour mettre à jour le `user_metadata` d’un utilisateur, utilisez le point de terminaison [**Update a User** endpoint](https://auth0.com/docs/api/management/v2#!/Users/patch_users_by_id).

La structure de votre requête dépend de la façon dont vos métadonnées sont structurées : à la racine ou dans des propriétés internes.
...
Cela ajoutera une nouvelle propriété au profil de l’utilisateur, **user&#95;metadata.consentDate**, qui contiendra la date à laquelle le client a donné son consentement. La réponse renverra le profil complet de l’utilisateur. Les métadonnées mises à jour ressembleront à ceci :
...
Ajoutons une propriété imbriquée pour la date de consentement et attribuons-lui la valeur `01/23/2018`.
...
Cela ajoutera une nouvelle propriété au profil de l’utilisateur, **user&#95;metadata.consent.date,** qui contiendra la date à laquelle le client a donné son consentement. La réponse contiendra le profil de l’utilisateur complet. Les métadonnées mises à jour ressembleront à ceci :
...
L’utilisateur devrait pouvoir retirer son consentement dans votre application. Cette option devrait être facile d’accès et clairement identifiable. Une fois que l’utilisateur décide de retirer son consentement, vous devriez agir.

D’abord, vous devez décider comment vous allez gérer le retrait du consentement : allez-vous supprimer les utilisateurs ou les marquer comme supprimés ?

<div id="delete-user">
  ### Supprimer un utilisateur
</div>

Pour supprimer un utilisateur, utilisez le [**point de terminaison Delete a User**](https://auth0.com/docs/api/management/v2#!/Users/delete_users_by_id).
...
Le corps de la réponse de ce point de terminaison est vide. Donc, si vous voulez confirmer que l’utilisateur a bien été supprimé, essayez de le récupérer à l’aide de son adresse courriel. Si le point de terminaison renvoie une erreur, cela signifie que votre appel de suppression a réussi.
...
Cela vous permet de conserver un enregistrement des utilisateurs supprimés pour une utilisation future.

<div id="flag-the-profile">
  #### Marquer le profil
</div>

Pour marquer un utilisateur comme supprimé, utilisez **app&#95;metadata**. Dans l’exemple suivant, nous vous montrons comment ajouter une propriété appelée **deleted** au champ **app&#95;metadata**. Vous pouvez ainsi configurer le processus d’authentification pour que tout utilisateur dont cette propriété est définie à true soit traité comme supprimé.
...
* Configurer dans l’application la zone où les clients retireront leur consentement.
