...
description: Configurez Akamai comme proxy inverse pour envoyer Supplemental Signals à votre tenant Auth0.
---
...
Pour qu’Akamai puisse savoir si les tentatives de connexion réussissent ou échouent, vous devrez enregistrer une API dans Akamai, puis définir un point de terminaison pour chaque flux de connexion Auth0 que vous souhaitez prendre en charge.
...
<AccordionGroup>
  ...
  <Accordion title="ROPG">
    ...
    * `/oauth/token`
    ...
    Le processus est le même pour tous les endpoints, mais les valeurs de configuration diffèrent légèrement.
...
<div id="step-2-forward-bot-results-from-akamai-to-auth0">
  # Étape 2 : Transmettre les résultats des bots d’Akamai à Auth0
</div>

Pour configurer Akamai afin qu’il transmette les résultats des bots à Auth0, consultez [Forward Bot Results to Origin on Akamai TechDocs](https://techdocs.akamai.com/content-protector/docs/forward-bot-results-to-origin).

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  L’en-tête `akamai-user-risk` peut ne pas être présent dans chaque requête de connexion, car Akamai n’envoie cet en-tête que lorsqu’il établit un score de risque utilisateur.
</Callout>
...
<Tabs>
  <Tab title="Dashboard">
    ...
    1. Accédez à **[Auth0 Dashboard &gt; Sécurité &gt; Attack Protection](https://manage.auth0.com/dashboard/#/security/attack-protection)**.
    2. Sélectionnez **Supplemental Signals**.
    3. Activez la bascule **Enable Akamai headers**.

    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      Si vous ne voyez pas l’option **Supplemental Signals** dans l’Auth0 Dashboard, vous devez demander l’**Attack Protection Add-on**. Pour en savoir plus, communiquez avec [Auth0 Sales](https://auth0.com/get-started?place=header\&type=button\&text=talk%20to%20sales).
    </Callout>
  </Tab>
  ...
  <Tab title="Management API">
    ...
    1. [Obtenez un jeton d’accès pour la Management API](/docs/fr-CA/secure/tokens/access-tokens/management-api-access-tokens/get-management-api-access-tokens-for-production) avec le scope `update:attack_protection`.
    2. Appelez le point de terminaison de la Management API [Mettre à jour la configuration de Supplemental Signals pour un tenant](/docs/fr-CA/api/management/v2/supplemental-signals/patch-supplemental-signals) avec le corps suivant :
...
* Les en-têtes `akamai-user-risk` et `akamai-bot` figureront dans les journaux de votre tenant pour les événements correspondants.
