---
description: Découvrez comment utiliser l’Auth0 Management API pour gérer les méthodes d’authentification multifacteur (MFA) de vos utilisateurs.
...
Utilisez le point de terminaison [Gets a list of authentication methods](https://auth0.com/docs/api/management/v2#!/Users/get_authentication_methods) pour obtenir la liste de toutes les méthodes d’authentification qu’un utilisateur a configurées, en tout ou en partie.
...
Pour chaque requête valide, la <Tooltip tip="Management API : Un produit permettant aux clients d’effectuer des tâches administratives." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Management+API">Management API</Tooltip> renverra une réponse au format JSON.
...
<div id="get-a-single-authentication-method">
  ## Obtenir une méthode d’authentification précise
</div>

Utilisez le point de terminaison [Obtient une méthode d’authentification par ID](https://auth0.com/docs/api/management/v2#!/Users/get_authentication_methods_by_authentication_method_id) pour obtenir une méthode d’authentification précise pour un utilisateur, à partir de l’ID de la méthode d’authentification.
...
Envoyez aux utilisateurs un OTP par SMS; ils seront ensuite invités à le saisir avant de pouvoir terminer leur authentification.
...
Pour chaque requête valide, la Management API retournera une réponse au format JSON.
...
Envoyez aux utilisateurs un OTP par courriel, qu’ils devront ensuite saisir pour terminer l’authentification. Le facteur courriel n’est pris en charge que lorsqu’un utilisateur ne dispose d’aucun autre facteur.
...
La requête suivante crée une méthode d’authentification WebAuthn with Security Keys pour un utilisateur.
...
Utilisez le point de terminaison [Met à jour toutes les méthodes d’authentification en les remplaçant par celles fournies](https://auth0.com/docs/api/management/v2#!/Users/put_authentication_methods) pour remplacer toutes les méthodes d’authentification existantes par celles que vous fournissez.
...
Utilisez le point de terminaison [Mettre à jour une méthode d’authentification](https://auth0.com/docs/api/management/v2#!/Users/patch_authentication_methods_by_authentication_method_id) pour mettre à jour une méthode d’authentification précise pour un utilisateur.

Ce point de terminaison nécessite le scope suivant : `update:authentication_methods`.

### Exemples

La requête suivante met à jour une seule méthode d’authentification d’un utilisateur à partir de l’ID de la méthode d’authentification en question.
...
Utilisez le point de terminaison [Supprimer toutes les méthodes d’authentification de l’utilisateur indiqué](https://auth0.com/docs/api/management/v2#!/Users/delete_authentication_methods) pour supprimer toutes les méthodes d’authentification d’un utilisateur.
...
Pour chaque requête valide, la Management API renverra une réponse avec un code d’état `204` et un corps de la réponse vide.

<div id="delete-a-single-authentication-method">
  ## Supprimer une seule méthode d’authentification
</div>

Utilisez le point de terminaison [Supprimer une méthode d’authentification par ID](https://auth0.com/docs/api/management/v2#!/Users/delete_authentication_methods_by_authentication_method_id) pour supprimer une méthode d’authentification pour un utilisateur.
