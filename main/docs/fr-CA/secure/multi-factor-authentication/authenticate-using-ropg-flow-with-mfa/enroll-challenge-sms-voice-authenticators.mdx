---
description: Décrit comment créer vos propres parcours MFA en utilisant les SMS et/ou les appels vocaux comme facteur d’authentification.
title: Inscrire et soumettre à un défi les authentificateurs SMS et vocaux
---
...
* Si vous effectuez l’inscription pendant l’authentification, consultez [S’authentifier à l’aide du Resource Owner Password Grant et de la MFA](/docs/fr-CA/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa).
...
Faites une requête `POST` au point de terminaison MFA Associate pour inscrire l’authentificateur de l’utilisateur. Le bearer token requis par ce point de terminaison est le jeton MFA obtenu à l’étape précédente.

Pour s’inscrire avec SMS ou appel vocal, l’utilisateur inscrit un numéro de phone qui peut recevoir un challenge par SMS ou appel vocal. Indiquez les paramètres ci-dessous pour appeler le point de terminaison. Le paramètre `oob_channels` indique comment vous voulez envoyer le code à l’utilisateur (SMS ou appel vocal).
...
Si vous obtenez une erreur `User is already enrolled`, c’est que l’utilisateur a déjà un facteur MFA inscrit. Avant d’associer un autre facteur à l’utilisateur, vous devez lui envoyer une demande de vérification à l’aide du facteur existant.

S’il s’agit de la première fois que l’utilisateur associe un authentificateur, vous remarquerez que la réponse comprend `recovery_codes`. Les codes de récupération servent à accéder au compte de l’utilisateur s’il perd l’accès au compte ou à l’appareil utilisé pour son authentification à deux facteurs. Ces codes ne peuvent être utilisés qu’une seule fois, et de nouveaux codes sont générés au besoin.
...
Pour terminer l’inscription, effectuez une requête `POST` vers le <Tooltip tip="Point de terminaison des jetons : endpoint du serveur d’autorisation utilisé pour demander des jetons par programmation." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=OAuth">OAuth</Tooltip> <Tooltip tip="OAuth 2.0 : framework d’autorisation qui définit les protocoles et les processus d’autorisation." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=Token+endpoint">point de terminaison des jetons</Tooltip>. Vous devez inclure le `oob_code` renvoyé dans la réponse précédente, ainsi que le `binding_code` avec la valeur reçue dans le message.
...
Pour lancer la demande de défi, vous avez besoin de l’`authenticator_id` du facteur que vous voulez défier. Vous pouvez obtenir la liste de tous les authentificateurs inscrits à l’aide du point de terminaison MFA Authenticators :
...
Pour déclencher une demande de défi, envoyez une requête `POST` au point de terminaison MFA Challenge en utilisant l’`authenticator_id` correspondant et le `mfa_token`.
...
**Remarque :** Les SMS et les retours de code invalide sont soumis à une limite de fréquence. Les codes SMS peuvent être envoyés 10 fois et se rechargent à raison d’une fois par heure, tandis que les codes invalides peuvent être retournés 10 fois et se rechargent à raison d’une fois toutes les six minutes.
...
La MFA personnalisable permet aux utilisateurs d’inscrire les facteurs pris en charge par votre application de leur choix et de les utiliser pour le défi.

Lors de l’authentification au endpoint `oauth/token`, la réponse renvoie l’erreur `mfa_required`, qui comprend le `mfa_token` à utiliser avec la MFA API ainsi que le paramètre `mfa_requirements`, avec une liste d’authentificateurs :
...
Exigez la demande de défi MFA en appelant le point de terminaison [`request/mfa/challenge`](/docs/fr-CA/api/authentication/muti-factor-authentication/request-mfa-challenge).
...
* [Inscrire et soumettre à un défi des authentificateurs par courriel](/docs/fr-CA/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa/enroll-and-challenge-email-authenticators)
* [Inscrire et soumettre à un défi des authentificateurs OTP](/docs/fr-CA/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa/enroll-and-challenge-otp-authenticators)
* [Inscrire et soumettre à un défi des authentificateurs push](/docs/fr-CA/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa/enroll-and-challenge-push-authenticators)