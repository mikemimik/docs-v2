---
description: Décrit comment installer, configurer les options et utiliser les SDKs Guardian.
...
var auth0GuardianJS = require('auth0-guardian-js')({
  ...
  // Pour les tenants JP : https://{name}.guardian.jp.auth0.com
	...
	issuer: {
		// Le nom de l'émetteur à afficher dans les applications de génération de mots de passe à usage unique
		label: "{{ userData.tenantFriendlyName }}",
		name: "{{ userData.tenant }}",
	},

	// Le libellé du compte à afficher dans les applications de génération de mots de passe à usage unique
...
L’utilisation de `requestToken` ou de `ticket` dépend de la méthode d’authentification. Le `ticket` correspond à un [ticket d’enrôlement](/docs/fr-CA/secure/multi-factor-authentication/multi-factor-authentication-developer-resources/create-custom-enrollment-tickets) généré précédemment.

<div id="use-a-custom-domain">
  ### Utiliser un domaine personnalisé
</div>

Si vous avez configuré un <Tooltip tip="Domaine personnalisé : domaine tiers avec un nom spécialisé ou personnalisé." cta="Consulter le glossaire" href="/docs/fr-CA/glossary?term=custom+domain">domaine personnalisé</Tooltip> pour votre tenant Auth0, vous devrez mettre à jour la propriété `serviceUrl` afin qu’elle pointe vers le point de terminaison Guardian :
