...
1. [Configurer Connected Accounts for Token Vault](/docs/fr-CA/secure/tokens/token-vault/configure-token-vault#configure-connected-accounts-for-token-vault) pour une connexion sociale ou d’entreprise prise en charge.
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Si vous avez déjà défini votre politique MFA sur **Always** dans l’Auth0 Dashboard, vous devez la définir sur **Never** pour récupérer un jeton d’accès depuis Token Vault. Sinon, vous recevrez une erreur. Pour en savoir plus sur les différentes politiques MFA, consultez [Activer MFA dans l’Auth0 Dashboard](/docs/fr-CA/secure/multi-factor-authentication/enable-mfa#enable-mfa-in-the-auth0-dashboard).
...
Connected Accounts for Token Vault gère un profil utilisateur Auth0 unifié lié à plusieurs comptes externes. Votre application récupère ensuite les identifiants stockés dans Token Vault pour interagir avec des API externes au nom de l’utilisateur.

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Si votre application utilise [Organizations](/docs/fr-CA/manage-users/organizations), configurez la connexion, puis authentifiez l’utilisateur auprès de l’organization cible avant de lancer le flux Connected Accounts. Le compte connecté et les jetons stockés demeurent associés au profil Auth0 de l’utilisateur.
</Callout>

Vous pouvez configurer Connected Accounts pour les connexions sociales et d’entreprise prises en charge. Pour en savoir plus, consultez [Configure Connected Accounts](/docs/fr-CA/secure/tokens/token-vault/connected-accounts-for-token-vault#configure-connected-accounts).
...
* Authorization Code : permet à votre application d’effectuer la connexion initiale de l’utilisateur en échangeant un code d’autorisation temporaire contre un jeton d’accès Auth0, un refresh token et un ID token.
* Refresh token : permet à votre application d’utiliser un Auth0 refresh token de longue durée pour demander un nouvel jeton d’accès Auth0 sans que l’utilisateur ait à se reconnecter.
...
* [Créer une API backend](#create-backend-api) pour laquelle la SPA peut demander un jeton d’accès Auth0 en la précisant comme audience.
* [Créer un client d’API personnalisé](#create-custom-api-client) qui est lié à l’API backend, avec le type d’octroi Token Vault activé.

<div id="configure-your-spa">
  #### Configurez votre SPA
</div>

Configurez votre SPA pour utiliser le type d’octroi `authorization_code`. Cela permet à la SPA de demander au serveur d’autorisation Auth0 un jeton d’accès Auth0 ayant pour portée l’API backend.
...
<Tabs>
  <Tab title="Auth0 Dashboard">
    ...
    * Pour créer votre API, suivez les instructions dans [Enregistrer des API](/docs/fr-CA/get-started/auth0-overview/set-up-apis). **Remarque :** Une fois que vous avez défini un identifiant pour votre API, vous ne pouvez plus le modifier.
    * Cliquez sur **Create**.
  ...
  <Tab title="Management API">
    Pour créer une API backend à l’aide de la Management API, effectuez une requête `POST` au point de terminaison `/resource-servers` :
...
Pour l’échange de jeton d’accès, vous devez créer un client API personnalisé lié à l’API backend. La SPA pourra demander un jeton d’accès pour l’API backend en la spécifiant comme `audience` dans la requête d’autorisation envoyée au serveur d’autorisation Auth0. Le client API personnalisé a le même identifiant que votre API backend et le type d’octroi Token Vault est activé.

Lorsque l’API backend effectue l’échange de jeton d’accès, elle s’authentifie en transmettant les informations d’identification du client API personnalisé au serveur d’autorisation Auth0, ce qui prouve qu’il s’agit de la même entité enregistrée dans l’Auth0 Dashboard.
...
<Tabs>
  <Tab title="Auth0 Dashboard">
    ...
    * Cliquez sur **Add**. Une fois l’application créée, cliquez sur **Configure Application** et faites défiler jusqu’à **Application Properties**. Le **Application Type** est un client API personnalisé.
  ...
  <Tab title="Management API">
    ...
    | `app_type`                   | Le type d’application de votre client API personnalisé. Pour enregistrer le client comme serveur de ressources, définissez cette valeur sur `resource_server`.                                                  |
    ...
    | `grant_types`                | Les types d’octroi activés pour votre client API personnalisé. Définissez cette valeur sur le type d’octroi Token Vault : `urn:auth0:params:oauth:grant-type:token-exchange:federated-connection-access-token`. |
  </Tab>
</Tabs>

Une fois le client API personnalisé créé, l’utilisateur y sera redirigé après la connexion, plutôt que vers la SPA.