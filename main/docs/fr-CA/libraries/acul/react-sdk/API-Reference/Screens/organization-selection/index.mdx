...
description: "Décrit tous les hooks et toutes les méthodes disponibles pour personnaliser l’écran `organization-selection` de Universal Login."
---

L’écran `organization-selection` s’affiche lorsqu’un utilisateur doit saisir le nom de son organisation pour poursuivre le flux d’authentification. Il lui permet d’entrer le nom de son organisation et de continuer.
...
Des hooks propres à l’écran qui fournissent un accès en lecture seule aux données de contexte d’Auth0 sur l’écran `organization-selection`. Importez-les depuis `@auth0/auth0-acul-react/organization-selection`.
...
<ParamField body="useBranding" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/BrandingMembers">BrandingMembers</a></span>}>
  Ce hook fournit les configurations d’image de marque, comme le logo, les couleurs et les paramètres du thème affichés sur l’écran `organization-selection`.
...
<ParamField body="useClient" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/ClientMembers">ClientMembers</a></span>}>
  Ce hook fournit les configurations liées au client, comme `id`, `name` et `logoUrl`, pour l’écran `organization-selection`.
...
<ParamField body="useOrganization" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/OrganizationMembers">OrganizationMembers</a></span>}>
  Ce hook fournit des renseignements sur l’organisation de l’utilisateur si la sélection est propre à l’organisation. Il renvoie `null` lorsqu’aucun contexte d’organisation n’est présent.
...
<ParamField body="useUntrustedData" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/UntrustedDataMembers">UntrustedDataMembers</a></span>}>
  Ce hook gère les données non fiables transmises à l’écran, comme les paramètres préremplis provenant de chaînes de requête d’URL.
...
<ParamField body="useOrganizationSelection" type={<a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/OrganizationSelectionMembers">OrganizationSelectionMembers</a>}>
  Ce hook renvoie toutes les méthodes et tout le contexte disponibles sur l’écran `organization-selection`.
</ParamField>
