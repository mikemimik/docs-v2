...
description: "Décrit tous les hooks et toutes les méthodes disponibles pour personnaliser l’écran `customized-consent` d’Universal Login."
---

L’écran `customized-consent` s’affiche pendant le flux d’autorisation lorsqu’un utilisateur doit accorder ou rejeter des permissions à une application avec une expérience de consentement personnalisée. Il présente les scopes demandés et permet à l’utilisateur d’accepter ou de rejeter la demande de consentement.
...
* L’utilisation de l’importation racine vous permet de charger tous les écrans à partir d’un seul bundle, ce qui est utile si vous voulez une compilation unifiée capable de gérer tous les écrans possibles.
...
<ParamField body="useBranding" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/BrandingMembers">BrandingMembers</a></span>}>
  Ce hook fournit les paramètres d’image de marque, comme le logo, les couleurs et les réglages du thème affichés sur l’écran `customized-consent`.
...
<ParamField body="useTransaction" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/TransactionMembers">TransactionMembers</a></span>}>
  Ce hook fournit des données propres à la transaction pour l’écran `customized-consent`, comme les connections actives et l’état actuel du flux.
...
<ParamField body="deny" type="Promise<void>">
  Cette méthode rejette la demande de consentement et révoque les permissions demandées par l’application.
