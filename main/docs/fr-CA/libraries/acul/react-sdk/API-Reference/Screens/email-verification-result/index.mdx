...
description: "Décrit tous les hooks et toutes les méthodes disponibles pour personnaliser l’écran `email-verification-result` dans Universal Login."
---

L’écran `email-verification-result` s’affiche une fois qu’un utilisateur a terminé le processus de vérification de l’adresse courriel. Il indique si la vérification a réussi ou non.
...
* L’utilisation d’un **partial import** vous permet d’inclure uniquement le code nécessaire à votre cas d’utilisation.
* L’utilisation d’un **root import** vous permet de charger tous les écrans à partir d’un seul bundle, ce qui est utile si vous voulez qu’un build unifié puisse gérer tous les écrans possibles.
...
Hooks associés à l’écran qui fournissent un accès en lecture seule aux données de contexte Auth0 sur l’écran `email-verification-result`. Importez-les depuis `@auth0/auth0-acul-react/email-verification-result`.
...
<ParamField body="useBranding" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/BrandingMembers">BrandingMembers</a></span>}>
  Ce hook fournit les configurations de Branding, comme le logo, les couleurs et les paramètres de thème affichés sur l’écran `email-verification-result`.
...
<ParamField body="useClient" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/ClientMembers">ClientMembers</a></span>}>
  Ce hook fournit les configurations liées au client, comme `id`, `name` et `logoUrl`, pour l’écran `email-verification-result`.
...
<ParamField body="useTransaction" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/TransactionMembers">TransactionMembers</a></span>}>
  Ce hook fournit des données propres à la transaction pour l’écran `email-verification-result`, comme les connections actives et l’état actuel du flux.
...
<ParamField body="useEmailVerificationResult" type={<a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/EmailVerificationResultMembers">EmailVerificationResultMembers</a>}>
  Ce hook renvoie toutes les méthodes et tout le contexte disponibles pour l’écran `email-verification-result`.
</ParamField>
