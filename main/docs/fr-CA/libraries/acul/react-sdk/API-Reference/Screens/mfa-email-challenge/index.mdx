...
description: "Décrit tous les hooks et toutes les méthodes disponibles pour personnaliser l’écran `mfa-email-challenge` du Universal Login."
---
...
Hooks associés à l’écran qui fournissent un accès en lecture seule aux données de contexte Auth0 sur l’écran `mfa-email-challenge`. Importez-les depuis `@auth0/auth0-acul-react/mfa-email-challenge`.
...
<ParamField body="useBranding" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/BrandingMembers">BrandingMembers</a></span>}>
  Ce hook fournit des configurations d’image de marque, comme le logo, les couleurs et les paramètres de thème affichés sur l’écran `mfa-email-challenge`.
...
<ParamField body="useOrganization" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/OrganizationMembers">OrganizationMembers</a></span>}>
  Ce hook fournit des renseignements sur l’Organization de l’utilisateur si le flux AMF est associé à une Organization. Renvoie `null` lorsqu’aucun contexte Organization n’est présent.
...
<ParamField body="useMfaEmailChallenge" type={<a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/MfaEmailChallengeMembers">MfaEmailChallengeMembers</a>}>
  Ce hook renvoie toutes les méthodes et le contexte disponibles sur l’écran `mfa-email-challenge`.
</ParamField>
...
<ParamField body="continueMethod" type="Promise<void>">
  Cette méthode soumet le code à usage unique saisi par l’utilisateur pour terminer l’étape de vérification AMF par courriel.
...
<ParamField body="pickEmail" type="Promise<void>">
  Cette méthode redirige vers l’écran de sélection de l’adresse courriel si plusieurs adresses courriel sont disponibles pour cette vérification.
...
<ParamField body="tryAnotherMethod" type="Promise<void>">
  Cette méthode redirige vers l’écran de sélection de la méthode d’AMF afin que l’utilisateur puisse choisir un facteur d’authentification différent.
...
<ParamField body={<a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Hooks/useResend">useResend</a>} type="Hooks">
  Ce hook gère l’état et la logique du délai d’attente pour le renvoi sur les écrans qui permettent le renvoi de code.
</ParamField>
