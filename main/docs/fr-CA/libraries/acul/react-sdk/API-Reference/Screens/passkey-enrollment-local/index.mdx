...
description: "Décrit tous les points d’extension et toutes les méthodes disponibles pour personnaliser l’écran `passkey-enrollment-local` de Universal Login."
---

L’écran `passkey-enrollment-local` s’affiche lorsqu’un utilisateur est invité à inscrire une clé d’accès sur l’appareil qu’il utilise. Il permet à l’utilisateur de poursuivre l’inscription de la clé d’accès ou d’abandonner et, au besoin, de ne plus afficher les invites à l’avenir.
...
Hooks associés à l’écran qui offrent un accès en lecture seule aux données de contexte Auth0 dans l’écran `passkey-enrollment-local`. Importez-les depuis `@auth0/auth0-acul-react/passkey-enrollment-local`.
...
<ParamField body="useBranding" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/BrandingMembers">BrandingMembers</a></span>}>
  Ce hook fournit la configuration de l’image de marque, comme le logo, les couleurs et les paramètres de thème affichés dans l’écran `passkey-enrollment-local`.
...
<ParamField body="useClient" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/ClientMembers">ClientMembers</a></span>}>
  Ce hook fournit la configuration liée au client, comme `id`, `name` et `logoUrl`, pour l’écran `passkey-enrollment-local`.
...
<ParamField body="useOrganization" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/OrganizationMembers">OrganizationMembers</a></span>}>
  Ce hook fournit des renseignements sur l’Organization de l’utilisateur si l’inscription de la clé d’accès est associée à une Organization. Retourne `null` lorsqu’aucun contexte d’Organization n’est présent.
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    ...
    <ParamField body="doNotShowAgain" type="boolean">
      Lorsque `true`, supprime les futures demandes d’inscription de clé d’accès pour cet utilisateur.
    </ParamField>
  </Expandable>
</ParamField>
...
<ParamField body={<a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Hooks/useErrors">useErrors</a>} type="Hooks">
  Ce hook lit et gère les erreurs côté serveur, côté client et liées au développeur à l’écran.
</ParamField>