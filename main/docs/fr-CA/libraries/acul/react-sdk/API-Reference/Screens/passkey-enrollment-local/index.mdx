...
description: "Décrit tous les hooks et les méthodes disponibles pour personnaliser l’écran `passkey-enrollment-local` de Universal Login."
---

L’écran `passkey-enrollment-local` s’affiche lorsqu’un utilisateur est invité à enregistrer une clé d’accès sur son appareil actuel. Il permet à l’utilisateur de poursuivre l’inscription de la clé d’accès, ou d’abandonner et, au besoin, de ne plus afficher les invites futures.

<Frame>
  <img style={{maxHeight:"400px"}} src="/docs/images/cdy7uua7fh8z/7FZiX1waboFh7cd7HmKSwX/df095b993e61cb453b3e8db8dec81ce3/Passkey_Enrollment_Local.png" alt="ACUL Enregistrement local de clé d’accès" />
</Frame>
...
<ParamField body="useBranding" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/BrandingMembers">BrandingMembers</a></span>}>
  Ce hook fournit les configurations d’image de marque, comme le logo, les couleurs et les paramètres de thème affichés sur l’écran `passkey-enrollment-local`.
...
<ParamField body="useClient" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/ClientMembers">ClientMembers</a></span>}>
  Ce hook fournit les configurations liées au client, comme `id`, `name` et `logoUrl`, pour l’écran `passkey-enrollment-local`.
...
<ParamField body="useOrganization" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/OrganizationMembers">OrganizationMembers</a></span>}>
  Ce hook fournit des renseignements sur l’Organization de l’utilisateur si l’inscription de la clé d’accès est limitée à une Organization. Retourne `null` lorsqu’aucun contexte d’Organization n’est présent.
...
<ParamField body="continuePasskeyEnrollment" type="Promise<void>">
  Cette méthode démarre le processus d’inscription de la clé d’accès sur l’appareil actuel et poursuit le flux d’authentification.
...
<ParamField body="abortPasskeyEnrollment" type="Promise<void>">
  Cette méthode annule le processus d’inscription de la clé d’accès et poursuit le flux d’authentification sans enregistrer de passkey.
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    ...
    <ParamField body="doNotShowAgain" type="boolean">
      Lorsque `true`, empêche l’affichage des futurs prompts d’inscription de la clé d’accès pour cet utilisateur.
    </ParamField>
  </Expandable>
</ParamField>
