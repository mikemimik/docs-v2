...
description: "Décrit tous les hooks et toutes les méthodes disponibles pour personnaliser l’écran `passkey-enrollment` dans Universal Login."
---

L’écran `passkey-enrollment` s’affiche lorsqu’un utilisateur est invité à enregistrer une clé d’accès comme méthode d’authentification. Il permet à l’utilisateur de poursuivre l’inscription de la clé d’accès ou d’interrompre le processus.

<Frame>
  <img style={{maxHeight:"400px"}} src="/docs/images/cdy7uua7fh8z/7haU4kXN2y0zikB9kanO8J/f32a48edc265d906aa063de96e9f3b89/Passkey_Enrollment.png" alt="ACUL Inscription de clé d’accès" />
</Frame>
...
* L’importation à la racine vous permet de charger tous les écrans à partir d’un seul bundle, ce qui est utile si vous souhaitez qu’une version unifiée gère tous les écrans possibles.
...
Hooks associés à l’écran qui fournissent un accès en lecture seule aux données de contexte d’Auth0 sur l’écran `passkey-enrollment`. Importez-les depuis `@auth0/auth0-acul-react/passkey-enrollment`.
...
<ParamField body="useBranding" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/BrandingMembers">BrandingMembers</a></span>}>
  Ce hook fournit les configurations de Branding, comme le logo, les couleurs et les paramètres de thème affichés sur l’écran `passkey-enrollment`.
...
<ParamField body="useOrganization" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/OrganizationMembers">OrganizationMembers</a></span>}>
  Ce hook fournit des renseignements sur l’Organization de l’utilisateur si l’inscription de la clé d’accès est limitée à l’Organization. Il renvoie `null` lorsqu’aucun contexte d’Organization n’est présent.
...
<ParamField body="useScreen" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/ScreenMembersOnPasskeyEnrollment">ScreenMembersOnPasskeyEnrollment</a></span>}>
  Ce hook contient des détails propres à l’écran `passkey-enrollment`, notamment sa configuration et son contexte.
...
<ParamField body="usePasskeyEnrollment" type={<a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/PasskeyEnrollmentMembers">PasskeyEnrollmentMembers</a>}>
  Ce hook renvoie toutes les méthodes et tout le contexte disponibles sur l’écran `passkey-enrollment`.
</ParamField>
...
<ParamField body="doNotShowAgain" type="Promise<void>">
  Cette méthode saute l’inscription de la clé d’accès et enregistre la préférence de l’utilisateur pour qu’on ne lui propose plus d’inscrire une clé d’accès.
