...
description: "Décrit tous les hooks et les méthodes permettant de personnaliser l’écran `mfa-recovery-code-challenge` de Universal Login."
---

L’écran `mfa-recovery-code-challenge` invite l’utilisateur à saisir son code de récupération pour vérifier son identité lorsque les autres méthodes MFA ne sont pas disponibles.
...
Des hooks associés à l’écran qui fournissent un accès en lecture seule aux données de contexte Auth0 sur l’écran `mfa-recovery-code-challenge`. Importez-les depuis `@auth0/auth0-acul-react/mfa-recovery-code-challenge`.
...
<ParamField body="useBranding" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/BrandingMembers">BrandingMembers</a></span>}>
  Ce hook fournit les configurations de branding, comme le logo, les couleurs et les paramètres de thème affichés sur l’écran `mfa-recovery-code-challenge`.
...
<ParamField body="useClient" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/ClientMembers">ClientMembers</a></span>}>
  Ce hook fournit les configurations liées au client, comme `id`, `name` et `logoUrl`, pour l’écran `mfa-recovery-code-challenge`.
...
<ParamField body="continueMethod" type="Promise<void>">
  Cette méthode soumet le code de récupération de l’utilisateur pour terminer l’étape de vérification de la MFA.
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    ...
    <ParamField body="code" type="string" required>
      Le code de récupération saisi par l’utilisateur pour la vérification.
    </ParamField>
  </Expandable>
</ParamField>

<ParamField body="tryAnotherMethod" type="Promise<void>">
  Cette méthode ouvre l’écran de sélection des méthodes de MFA afin que l’utilisateur puisse choisir un autre facteur d’authentification.
...
<ParamField body={<a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Hooks/useAuth0Themes">useAuth0Themes</a>} type="Hooks">
  Ce hook récupère les options de Theme actuelles avec une configuration aplatie à partir du contexte de branding.
</ParamField>
