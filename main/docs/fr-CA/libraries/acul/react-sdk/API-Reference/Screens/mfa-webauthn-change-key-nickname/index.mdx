...
L’écran `mfa-webauthn-change-key-nickname` permet à l’utilisateur de définir ou de mettre à jour le nom d’affichage d’une clé de sécurité WebAuthn déjà inscrite.
...
* L’importation depuis la racine vous permet de charger tous les écrans à partir d’un seul bundle, ce qui est utile si vous souhaitez qu’une version unifiée gère tous les écrans possibles.
...
Des hooks propres à l’écran qui fournissent un accès en lecture seule aux données de contexte d’Auth0 sur l’écran `mfa-webauthn-change-key-nickname`. Importez-les depuis `@auth0/auth0-acul-react/mfa-webauthn-change-key-nickname`.
...
<ParamField body="useBranding" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/BrandingMembers">BrandingMembers</a></span>}>
  Ce hook fournit la configuration de l’image de marque, comme le logo, les couleurs et les paramètres du thème affichés sur l’écran `mfa-webauthn-change-key-nickname`.
...
<ParamField body="useTransaction" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/TransactionMembers">TransactionMembers</a></span>}>
  Ce hook fournit des données propres à la transaction pour l’écran `mfa-webauthn-change-key-nickname`, comme l’état actuel du flux d’AMF.
...
<ParamField body="useUntrustedData" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/UntrustedDataMembers">UntrustedDataMembers</a></span>}>
  Ce hook gère les données non fiables transmises à l’écran, par exemple des valeurs préremplies issues de paramètres d’URL.
...
<ParamField body="useMfaWebAuthnChangeKeyNickname" type={<a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/MfaWebAuthnChangeKeyNicknameMembers">MfaWebAuthnChangeKeyNicknameMembers</a>}>
  Ce hook renvoie toutes les méthodes et tout le contexte disponibles sur l’écran `mfa-webauthn-change-key-nickname`.
</ParamField>
...
<ParamField body="continueWithNewNickname" type="Promise<void>">
  Cette méthode envoie le nouveau surnom de la clé de sécurité WebAuthn déjà inscrite.
