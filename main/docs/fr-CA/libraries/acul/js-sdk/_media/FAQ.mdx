...
Vous trouverez ci-dessous une série de questions ou d’enjeux soulevés par l’utilisation du SDK.
...
<div id="where-can-i-see-errors-related-to-my-screen-and-how-to-handle-them">
  ## Où puis-je consulter les erreurs liées à mon écran et comment les gérer
</div>
...
1. **Méthode `getErrors` pour les erreurs propres à l’écran**
   La fonction getter de premier niveau `getErrors` peut être utilisée pour récupérer les erreurs d’authentification détaillées lorsque la validation du backend échoue.
   ```javascript
      import Login from "@auth0/auth0-acul-js/login";
...
2. **Erreurs propres à l’écran**
   Chaque écran comprend une propriété `transaction.errors` qui renvoie une liste d’erreurs propres à cet écran. Ces erreurs sont générées après la validation finale de la requête, ce qui vous aide à repérer les problèmes qui demandent votre attention.

En utilisant ces méthodes, vous pouvez facilement suivre et corriger les erreurs tout au long du flow du SDK.

<div id="can-i-use-this-sdk-from-a-cdn">
  ## Puis-je utiliser ce SDK depuis un CDN ?
</div>

À l’heure actuelle, ce SDK est offert uniquement par installation via le registre npm et ne peut pas être utilisé directement depuis un CDN.
...
Bien que le SDK soit écrit en Vanilla JavaScript, il est entièrement compatible avec tous les frameworks basés sur JavaScript, notamment React, Angular, Vue et d’autres. Vous pouvez l’utiliser sans problème dans vos composants ou hooks React.