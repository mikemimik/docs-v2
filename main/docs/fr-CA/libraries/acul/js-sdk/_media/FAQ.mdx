...
Vous trouverez ci-dessous quelques questions ou problèmes qui sont survenus lors de l’utilisation du SDK.
...
1. **méthode `getErrors` pour les erreurs propres à l’écran**
   Une fonction d’accès de premier niveau, `getErrors`, peut être utilisée pour récupérer des erreurs d’authentification détaillées lorsque la validation backend échoue.
   ```javascript
      import Login from "@auth0/auth0-acul-js/login";
...
2. **Erreurs propres à l’écran**
   Chaque écran comprend une propriété `transaction.errors`, qui renvoie une liste d’erreurs propres à cet écran. Ces erreurs sont générées après la validation finale de la requête, ce qui vous aide à repérer les problèmes qui nécessitent votre attention.
...
Actuellement, ce SDK n’est disponible qu’au moyen du registre npm et ne peut pas être utilisé directement depuis un CDN.
...
Même si le SDK est écrit en JavaScript pur, il est entièrement compatible avec tous les cadres JavaScript, notamment React, Angular, Vue et d’autres. Vous pouvez l’utiliser en toute simplicité dans vos composants React ou vos hooks.