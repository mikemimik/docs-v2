...
description: "Décrit toutes les propriétés et méthodes disponibles pour personnaliser l’écran `mfa-webauthn-platform-challenge` d’Universal Login."
---
...
Créez une instance du gestionnaire de l’écran MFA WebAuthn Platform Challenge :
...
<ParamField body="untrustedData" type={<span><a href="/docs/fr-CA/libraries/acul/js-sdk/Screens/interfaces/UntrustedDataMembers">UntrustedDataMembers</a></span>}>
  Gère les données non fiables transmises au SDK, comme les saisies de l’utilisateur pendant le défi de plateforme WebAuthn d’AMF.
</ParamField>
...
<ParamField body="verify" type="Promise<void>">
  Cette méthode lance le défi au moyen de l’authentificateur de plateforme WebAuthn.
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    ...
    <ParamField body="rememberDevice?" type="boolean">
      Lorsque la valeur est `true`, le navigateur est mémorisé pour les futurs défis d’AMF. S’applique seulement si `screen.showRememberDevice` est `true`.
    </ParamField>
  </Expandable>
</ParamField>