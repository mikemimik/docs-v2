...
La classe Consent implémente la fonctionnalité de l’écran `consent`. Cet écran présente les permissions demandées à l’utilisateur et recueille sa décision d’accepter ou de rejeter l’accès.
...
<ParamField body="untrustedData" type={<span><a href="/docs/fr-CA/libraries/acul/js-sdk/Screens/interfaces/UntrustedDataMembers">UntrustedDataMembers</a></span>}>
  Contient les données non fiables transmises au SDK, comme la saisie de l’utilisateur pendant le flux de consentement.
</ParamField>
...
<ParamField body="accept" type="Promise<void>">
  Cette méthode soumet la décision de l’utilisateur d’accepter (d’accorder) les permissions demandées.
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    ...
    <ParamField body="persist?" type="&#x22;session&#x22;">
      Lorsqu’elle est définie sur `'session'`, la langue sélectionnée est enregistrée pendant toute la session.
    </ParamField>
  </Expandable>
</ParamField>

<ParamField body="deny" type="Promise<void>">
  Cette méthode soumet la décision de l’utilisateur de rejeter les permissions demandées.
...
<ParamField body="getErrors" type={<span><a href="/docs/fr-CA/libraries/acul/js-sdk/Screens/interfaces/Error">Error</a>[]</span>}>
  Cette méthode récupère le tableau des erreurs de la transaction dans le contexte, ou un tableau vide s’il n’y en a aucune.
</ParamField>