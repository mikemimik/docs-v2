...
<ParamField body="showError" type="Promise<void>">
  Cette méthode transmet à Auth0 une erreur WebAuthn côté navigateur, comme une annulation par l’utilisateur (`NotAllowedError`) ou un délai d’expiration de `navigator.credentials.get()`.
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    ...
    <ParamField body="error" type={<span><a href="/docs/fr-CA/libraries/acul/js-sdk/Screens/interfaces/WebAuthnErrorDetails">WebAuthnErrorDetails</a></span>} required>
      L’objet d’erreur provenant de l’API WebAuthn (`navigator.credentials.get()`) à transmettre.
    ...
    <ParamField body="rememberDevice?" type="boolean">
      Lorsque `true`, conserve la préférence de mémorisation de l’appareil avec le signalement de l’erreur.
    </ParamField>
  </Expandable>
</ParamField>

<ParamField body="tryAnotherMethod" type="Promise<void>">
  Cette méthode permet à l’utilisateur de sélectionner une autre méthode d’AMF pour terminer l’étape de vérification.
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    ...
    <ParamField body="rememberDevice?" type="boolean">
      Lorsque `true`, conserve la préférence de mémorisation de l’appareil lors du changement de méthode.
    </ParamField>
  </Expandable>
</ParamField>

<ParamField body="useSecurityKey" type="Promise<void>">
  Cette méthode lance la vérification par clé de sécurité WebAuthn et envoie les informations d’authentification obtenues pour terminer l’étape d’AMF pendant le processus de réinitialisation du mot de passe. Si le navigateur génère une `DOMException` (par exemple, si l’utilisateur annule l’opération), interceptez l’erreur et appelez `showError` pour en aviser Auth0.
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    ...
    <ParamField body="rememberDevice?" type="boolean">
      Lorsque `true`, marque l’appareil comme fiable afin que l’utilisateur ne soit pas invité de nouveau à utiliser l’AMF sur cet appareil.
    </ParamField>
  </Expandable>
</ParamField>