...
description: "Décrit toutes les propriétés et méthodes permettant de personnaliser l’écran `customized-consent` de l’expérience Universal Login."
---

La classe CustomizedConsent implémente les fonctionnalités de l’écran `customized-consent`. Cet écran invite l’utilisateur à accepter ou à rejeter les permissions et les détails d’autorisation demandés par une application cliente.
...
<ParamField body="client" type={<span><a href="/docs/fr-CA/libraries/acul/js-sdk/Screens/interfaces/ClientMembers">ClientMembers</a></span>}>
  Fournit des paramètres liés au client, comme `id`, `name` et `logoUrl`, pour l’écran `customized-consent`.
</ParamField>
...
<ParamField body="accept" type="Promise<void>">
  Cette méthode soumet la décision de l’utilisateur d’accepter les permissions demandées et le détail d’autorisation.
...
<ParamField body="deny" type="Promise<void>">
  Cette méthode soumet la décision de l’utilisateur de rejeter les permissions demandées et le détail d’autorisation.
...
<ParamField body="getErrors" type={<span><a href="/docs/fr-CA/libraries/acul/js-sdk/Screens/interfaces/Error">Error</a>[]</span>}>
  Cette méthode récupère dans le contexte le tableau des erreurs de transaction, ou un tableau vide s’il n’en existe aucune.
</ParamField>