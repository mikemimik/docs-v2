...
description: "Décrit toutes les propriétés et méthodes permettant de personnaliser l’écran `mfa-webauthn-roaming-challenge` dans Universal Login."
---
...
Créez une instance du gestionnaire de l’écran MFA WebAuthn Roaming Challenge :
...
<ParamField body="untrustedData" type={<span><a href="/docs/fr-CA/libraries/acul/js-sdk/Screens/interfaces/UntrustedDataMembers">UntrustedDataMembers</a></span>}>
  Gère les données non fiables transmises au SDK, comme la saisie de l’utilisateur pendant un challenge MFA WebAuthn avec authentificateur itinérant.
</ParamField>
...
<ParamField body="verify" type="Promise<void>">
  Cette méthode amorce le challenge WebAuthn à l’aide de la clé de sécurité.
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    ...
    <ParamField body="rememberDevice?" type="boolean">
      Lorsque `true`, mémorise le navigateur pour les prochaines vérifications MFA. S’applique uniquement si `screen.showRememberDevice` vaut `true`.
    </ParamField>
  </Expandable>
</ParamField>