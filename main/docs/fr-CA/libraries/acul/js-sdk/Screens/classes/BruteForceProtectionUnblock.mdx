...
La classe BruteForceProtectionUnblock implémente la fonctionnalité de l’écran `brute-force-protection-unblock`. Cet écran s’affiche lorsque le compte d’un utilisateur a été bloqué à la suite de plusieurs tentatives de connexion infructueuses et qu’il doit soumettre une demande de déblocage.
...
<ParamField body="untrustedData" type={<span><a href="/docs/fr-CA/libraries/acul/js-sdk/Screens/interfaces/UntrustedDataMembers">UntrustedDataMembers</a></span>}>
  Traite les données non fiables transmises au SDK, comme les saisies de l’utilisateur pendant le flux de déblocage de compte.
</ParamField>
...
<ParamField body="unblockAccount" type="Promise<void>">
  Cette méthode envoie la requête de déblocage du compte.
