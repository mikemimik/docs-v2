---
title: "LoginPassword"
description: "Décrit toutes les propriétés et méthodes permettant de personnaliser l’écran `login-password` de Universal Login."
---

La classe LoginPassword implémente les fonctionnalités de l’écran `login-password`. Cet écran recueille l’identifiant et le mot de passe de l’utilisateur.
...
Créez une instance du gestionnaire d’écran LoginPassword :
...
<ParamField body="screen" type={<span><a href="/docs/fr-CA/libraries/acul/js-sdk/Screens/interfaces/ScreenMembersOnLoginPassword">ScreenMembersOnLoginPassword</a></span>}>
  Contient des détails propres à l’écran `login-password`, notamment sa configuration et son contexte.
</ParamField>
...
<ParamField body="federatedLogin" type="Promise<void>">
  Cette méthode redirige l’utilisateur vers le fournisseur d’identité social ou d’entreprise (IdP) pour s’authentifier.
...
<ParamField body="switchConnection" type="Promise<void>">
  Cette méthode permet de basculer entre une connexion de base de données (mot de passe) et une connexion Passwordless (OTP par courriel/SMS).
