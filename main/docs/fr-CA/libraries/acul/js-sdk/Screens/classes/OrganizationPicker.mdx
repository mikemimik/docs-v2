...
description: "Décrit toutes les propriétés et méthodes disponibles pour personnaliser l’écran `organization-picker` d’Universal Login."
---

La classe OrganizationPicker prend en charge la fonctionnalité de l’écran `organization-picker`. Cet écran permet aux utilisateurs de sélectionner une organisation dans une liste d’organisations disponibles.
...
<ParamField body="untrustedData" type={<span><a href="/docs/fr-CA/libraries/acul/js-sdk/Screens/interfaces/UntrustedDataMembers">UntrustedDataMembers</a></span>}>
  Gère les données non fiables transmises au SDK, comme les entrées de l’utilisateur pendant le flux de sélection d’organisation.
</ParamField>

<ParamField body="user" type={<span><a href="/docs/fr-CA/libraries/acul/js-sdk/Screens/interfaces/UserMembers">UserMembers</a></span>}>
  Contient des renseignements sur l’utilisateur actif, y compris `username`, `email` et `roles`.
</ParamField>
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    ...
    <ParamField body="persist?" type="&#x22;session&#x22;">
      Lorsque défini à `'session'`, la langue sélectionnée est conservée pendant toute la durée de la session.
    </ParamField>
  </Expandable>
</ParamField>
...
<ParamField body="skipOrganizationSelection" type="Promise<void>">
  Cette méthode envoie l’action permettant d’ignorer la sélection d’organisation.
