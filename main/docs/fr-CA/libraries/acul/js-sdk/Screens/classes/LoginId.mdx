...
La classe LoginId implémente la fonctionnalité de l’écran `login-id`. Cet écran permet de saisir l’identifiant et le mot de passe de l’utilisateur. Selon votre tenant, cet identifiant peut être une adresse courriel, un numéro de téléphone ou un nom d’utilisateur.
...
<ParamField body="pickCountryCode" type="Promise<void>">
  Cette méthode redirige l’utilisateur vers la liste de sélection de l’indicatif du pays, où il peut modifier l’indicatif de pays de son numéro de téléphone.
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    <ParamField body="inputId?" type="string">
      ID facultatif de l’élément `<input>` du nom d’utilisateur (sans `#`). Exemple : `"username"`. S’il est omis, le développeur doit s’assurer manuellement que les bons attributs `autocomplete` sont définis sur le champ de saisie.
    </ParamField>
  </Expandable>
</ParamField>