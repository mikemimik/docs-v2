...
description: "Décrit toutes les propriétés et méthodes offertes pour personnaliser l’écran `login-email-verification` de Universal Login."
---

La classe LoginEmailVerification implémente la fonctionnalité de l’écran `login-email-verification`. Cet écran invite l’utilisateur à saisir un code à usage unique envoyé à son adresse courriel afin de vérifier son identité dans le cadre du processus de connexion.
...
<ParamField body="getErrors" type={<span><a href="/docs/fr-CA/libraries/acul/js-sdk/Screens/interfaces/Error">Error</a>[]</span>}>
  Cette méthode récupère le tableau des erreurs de transaction à partir du contexte, ou un tableau vide s’il n’y en a aucune. Il s’agit d’un tableau d’objets d’erreur provenant du contexte de transaction.
</ParamField>
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    ...
    <ParamField body="onStatusChange?" type={<span><a href="/docs/fr-CA/libraries/acul/js-sdk/Screens/type-aliases/OnStatusChangeCallback">OnStatusChangeCallback</a></span>}>
      Fonction de rappel invoquée chaque fois que le status du renvoi change; elle retourne le nombre de secondes restantes et indique si la fonctionnalité de renvoi est actuellement désactivée.
    </ParamField>
