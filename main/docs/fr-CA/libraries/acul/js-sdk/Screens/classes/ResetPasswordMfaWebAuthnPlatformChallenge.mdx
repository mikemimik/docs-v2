...
description: "Décrit l’ensemble des propriétés et des méthodes offertes pour personnaliser l’écran `reset-password-mfa-webauthn-platform-challenge` de Universal Login."
---

La classe ResetPasswordMfaWebAuthnPlatformChallenge implémente les fonctionnalités de l’écran `reset-password-mfa-webauthn-platform-challenge`. Cet écran invite l’utilisateur à vérifier son identité à l’aide d’un authentificateur de plateforme (comme Touch ID ou Windows Hello) dans le cadre de l’AMF pendant un processus de réinitialisation du mot de passe.
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    ...
    <ParamField body="persist?" type="&#x22;session&#x22;">
      Lorsque cette option est définie à `'session'`, la langue sélectionnée est conservée pendant toute la durée de la session.
    </ParamField>
  </Expandable>
</ParamField>

<ParamField body="continueWithPasskey" type="Promise<void>">
  Cette méthode lance le défi de l’authentificateur de plateforme WebAuthn et soumet les informations d’authentification obtenues pour terminer l’AMF pendant le processus de réinitialisation du mot de passe. Si le navigateur renvoie une `DOMException` (comme une annulation par l’utilisateur), interceptez l’erreur et appelez `reportBrowserError` pour en aviser Auth0.
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    ...
    <ParamField body="rememberDevice?" type="boolean">
      Lorsque `true`, marque l’appareil comme fiable afin que l’utilisateur n’ait plus à effectuer l’AMF sur cet appareil.
    </ParamField>
  </Expandable>
</ParamField>
...
<ParamField body="tryAnotherMethod" type="Promise<void>">
  Cette méthode permet à l’utilisateur de sélectionner une autre méthode d’AMF pour répondre au défi.
