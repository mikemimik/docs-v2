...
description: "Décrit toutes les propriétés et méthodes disponibles pour personnaliser l’écran `reset-password-mfa-email-challenge` de Universal Login."
---

La classe ResetPasswordMfaEmailChallenge implémente la fonctionnalité de l’écran `reset-password-mfa-email-challenge`. Cet écran invite l’utilisateur à saisir le code de vérification envoyé à son adresse courriel dans le cadre de l’étape MFA du flux de réinitialisation du mot de passe.
...
<ParamField body="untrustedData" type={<span><a href="/docs/fr-CA/libraries/acul/js-sdk/Screens/interfaces/UntrustedDataMembers">UntrustedDataMembers</a></span>}>
  Gère les données non fiables transmises au SDK, comme les entrées de l’utilisateur dans le cadre du flux de défi AMF.
</ParamField>
...
<ParamField body="continue" type="Promise<void>">
  Cette méthode envoie le code de vérification par courriel pour poursuivre le défi AMF pendant le flux de réinitialisation du mot de passe.
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    ...
    <ParamField body="rememberDevice?" type="boolean">
      Indique s’il faut mémoriser l’appareil pour les prochains défis AMF.
    </ParamField>
  </Expandable>
</ParamField>
