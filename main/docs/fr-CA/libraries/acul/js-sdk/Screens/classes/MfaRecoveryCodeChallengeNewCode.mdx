...
La classe MfaRecoveryCodeChallengeNewCode implémente la fonctionnalité de l’écran `mfa-recovery-code-challenge-new-code`. Cet écran affiche le code de récupération AMF pour que vous puissiez le conserver en lieu sûr.
...
<ParamField body="untrustedData" type={<span><a href="/docs/fr-CA/libraries/acul/js-sdk/Screens/interfaces/UntrustedDataMembers">UntrustedDataMembers</a></span>}>
  Gère les données non fiables transmises au SDK, comme les entrées de l’utilisateur lors de l’étape de code de récupération AMF.
</ParamField>
...
<ParamField body="continue" type="Promise<void>">
  Cette méthode confirme que l’utilisateur a bien enregistré le nouveau code de récupération et poursuit le flux d’authentification.
