  ...
  <Expandable title="Paramètres">
    <ParamField body="options" type={<span><a href="/docs/fr-CA/libraries/acul/js-sdk/Screens/type-aliases/MfaPollingOptions">MfaPollingOptions</a></span>}>
      <ParamField body="MfaPollingOptions" type="object">
        Options de configuration pour gérer un processus de scrutation des notifications poussées d’AMF.

        Fournissez ces options au démarrage d’une session de scrutation pour contrôler la fréquence
        à laquelle le point de terminaison du défi est interrogé et la façon dont les réussites et les échecs sont gérés.
        ...
        <Expandable title="Propriétés">
          ...
          <ParamField body="onCompleted()?" type="() => void">
            Fonction de rappel facultative exécutée une fois que le défi push d’AMF
            ...
            * Utile pour faire avancer le flux de connexion (par exemple, en appelant `continue()`).
          </ParamField>
          ...
          <ParamField body="onError()?" type="(error) => void">
            ...
            * Appelée une fois par événement d’erreur; la scrutation peut être relancée selon le type d’erreur.
