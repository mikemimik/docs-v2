...
Interface définissant les méthodes et les propriétés disponibles pour l’écran mfa-webauthn-roaming-enrollment.
...
<ParamField body="showError" type="Promise<void>">
  ...
  const webauthnEnrollment = new MfaWebAuthnRoamingEnrollment();
  // Exemple de gestionnaire d’erreur pour un appel à l’API WebAuthn du navigateur
  const handleError = async (error: any) => {
