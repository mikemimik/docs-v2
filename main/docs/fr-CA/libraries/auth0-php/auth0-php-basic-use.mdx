...
4. Si l’authentication a réussi, la valeur `state` est validée.
5. Si `state` est valide, la valeur `code` est échangée avec Auth0 contre un ID Token et/ou un access token.
...
* [PHP : Résoudre les problèmes de votre intégration Auth0-PHP](/docs/fr-CA/libraries/auth0-php/troubleshoot-auth0-php-library)