...
フォームを作成したら、`api.prompt.render()` メソッドを使用して [Actions](/docs/ja/customize/actions) でレンダリングできます。
...
[Actions Code editor](/docs/ja/customize/actions/write-your-first-action#create-an-action) では、Form をいつ、どのようにレンダリングするかを決めるビジネスロジックを定義できます。
...
* [Event Object](/docs/ja/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-event-object): Event オブジェクトとそのプロパティについて説明します。
* [API object](/docs/ja/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-api-object): API オブジェクトとそのメソッドについて説明します。
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  第2引数で参照しているフィールド `ID` が、[nodes and components](/docs/ja/customize/forms/nodes-and-components) でクライアント側から利用できることを確認してください。そうでない場合、値は設定されません。
</Callout>
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  ...
  `{{vars.*}}` プロパティを使うと、フォームやそれにリンクされたフローで共有変数を参照できます。詳しくは、[Variables](/docs/ja/customize/forms/variables) をご覧ください。
</Callout>
...
* 同じ Action 内でユーザーを[リダイレクト](/docs/ja/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/redirect-with-actions)しながらフォームをレンダリングすることはできません。両方を使用する必要がある場合は、別々の Action を使用することを検討してください。
...
* `api.prompt.render()` メソッドは、次のトリガーで使用できます。
  ...
  * [post-login](/docs/ja/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-api-object)
  * [post-challenge](/docs/ja/customize/actions/explore-triggers/password-reset-triggers/post-challenge-trigger/post-challenge-api-object)