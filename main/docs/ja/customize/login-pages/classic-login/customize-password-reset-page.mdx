...
<Warning>
  現在、Auth0 の開発は Universal Login に注力しており、クラシックログインは今後更新されません。ログインページをカスタマイズする場合は、Auth0 は [Advanced Customizations for Universal Login](/docs/ja/customize/login-pages/advanced-customizations) を推奨しています。
</Warning>
...
<Card title="パスワードをリセットする場合">
  ...
  * 他のユーザーのパスワードをリセットするテナント管理者の場合: [Change Users&#39; Passwords](/docs/ja/authenticate/database-connections/password-change)
  * 自分のパスワードをリセットする場合: [Reset Auth0 Account Password](/docs/ja/troubleshoot/customer-support/reset-account-passwords)
</Card>
...
パスワードリセットページは、デフォルトですべての Auth0 ユーザーに対して有効になっており、カスタマイズは不要です。ただし、パスワードリセットページのブランディングやその他の要素を変更したい場合は、[高度なカスタマイズ](/docs/ja/customize/login-pages/classic-login/customization-classic)を有効にできます。

<Tooltip tip="Auth0 Dashboard: サービスを設定するための Auth0 の主要製品。" cta="用語集を見る" href="/docs/ja/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip>から高度なカスタマイズを有効にするには:
...
* [クラシックログインページをカスタマイズ](/docs/ja/customize/login-pages/classic-login/customization-classic)
* [クラシックログイン向けのMFAをカスタマイズ](/docs/ja/secure/multi-factor-authentication/customize-mfa/customize-mfa-classic-login)