...
Delegated Administration Extension で [複数のカスタムドメイン](/docs/ja/customize/custom-domains/multiple-custom-domains) を使用する場合、**カスタムドメイン Hook** を使うと、ユーザーの作成、パスワードのリセット、メールアドレスの確認、ユーザーのブロックといった操作に使用するカスタムドメインを指定できます。
...
* **callback(error, result)**: カスタムドメインの設定を返すためのコールバック関数。
  ...
  * **result**: カスタムドメインの動作を指定するオブジェクト:
    ...
    * テナント の[デフォルトドメイン](/docs/ja/customize/custom-domains/multiple-custom-domains)に委譲するには、空のオブジェクト `{}` を返します。
...
* 空のオブジェクト `{}` を返すと、ドメインの選択はテナントの[デフォルトのカスタムドメイン](/docs/ja/customize/custom-domains/multiple-custom-domains)設定に委ねられます。
