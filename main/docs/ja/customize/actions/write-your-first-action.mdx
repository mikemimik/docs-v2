...
このガイドでは、<Tooltip tip="Auth0 Dashboard: サービスを設定するためのAuth0の主要製品。" cta="用語集を見る" href="/docs/ja/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip>を使って、初めてのActionを作成し、デプロイする方法を順を追って説明します。読み終える頃には、すべてのActionsで使用する基本的な機能を理解し、プログラミングモデルにも慣れているはずです。ここで学ぶ内容は、目的やフローにかかわらず、あらゆるActionを作成するための基礎となります。
...
各 Action にはシークレット値を含めることができ、<Tooltip tip="アクセストークン: API へのアクセスに使用される、不透明な文字列または JWT 形式の Authorization 認証情報。" cta="用語集を見る" href="/docs/ja/glossary?term=Access+Tokens">アクセストークン</Tooltip>や API キーなどの機密情報を保持するのに適しています。Slack の Webhook URL をシークレットとして保存しましょう。
...
すべての Action には、関連する トリガー に応じた読み取り専用のコンテキスト情報を含む `event` オブジェクトがあります。Editor で `event.` と入力し始めると、`event` で利用できるすべてのプロパティが表示されます。各 トリガー で利用できる情報について詳しくは、[Explore Flows and Triggers](/docs/ja/customize/actions/explore-triggers) を参照してください。
...
2. [ログインフロー](/docs/ja/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger) を選択し、`my-slack-action` を フロー にドラッグします。
...
これで、この Action はテナント内の実際のユーザートラフィックに対して実行されるようになります。より本格的なテスト環境が必要な場合は、[複数の Auth0 環境を設定する](/docs/ja/get-started/auth0-overview/create-tenants/set-up-multiple-environments)ことを検討してください。
...
最初の Action を作成したら、[Flows と Triggers を確認する](/docs/ja/customize/actions/explore-triggers)を参照して、Actions でよくあるタスクを実行する方法を学びましょう。