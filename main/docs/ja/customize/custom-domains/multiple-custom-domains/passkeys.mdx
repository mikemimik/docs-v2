...
[パスキー](/docs/ja/secure/multi-factor-authentication/fido-authentication-with-webauthn) は、WebAuthn を使用して、フィッシング耐性のあるパスワードレス認証を提供します。[複数のカスタムドメイン](/docs/ja/customize/custom-domains/multiple-custom-domains) を使用する場合、WebAuthn のセキュリティモデルにより、パスキーはドメインごとに登録されます。
