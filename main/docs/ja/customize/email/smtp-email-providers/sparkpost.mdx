...
* 特定の [Auth0 IP アドレス](/docs/ja/secure/security-guidance/data-security/allowlist) からの接続を許可するように、プロバイダーを設定する必要があります。
...
SparkPost に <Tooltip tip="カスタムドメイン: 特別な名前、またはバニティ名を持つサードパーティのドメイン。" cta="用語集を見る" href="/docs/ja/glossary?term=custom+domain">カスタムドメイン</Tooltip> をまだ登録していない場合は、`sparkpostbox.com` ドメインから送信できるテストメールは最大 5 通です。詳細については、SparkPost の [Transmissions: The Sandbox Domain](https://developers.sparkpost.com/api/transmissions.html#header-the-sandbox-domain) を参照してください。
