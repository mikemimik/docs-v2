...
* Auth0 <Tooltip tip="Management API: 顧客が管理タスクを実行するための製品。" cta="用語集を表示" href="/docs/ja/glossary?term=Management+API">Management API</Tooltip>
* auth0.js ライブラリ
...
* `update:current_user_identities` スコープを持つ <Tooltip tip="アクセストークン: API へのアクセスに使用される認可資格情報で、不透明な文字列または JWT の形式を取ります。" cta="用語集を見る" href="/docs/ja/glossary?term=access+tokens">アクセストークン</Tooltip> を使った、ユーザー主導のクライアントサイドでのアカウントリンク。
...
* RS256 で署名され、リクエスト元のアクセストークンの `azp` クレームの値と一致するクライアントを識別する `aud` クレームを含む、セカンダリアカウントの <Tooltip tip="ID トークン: リソースへのアクセスではなく、クライアント自身のための認証情報です。" cta="用語集を見る" href="/docs/ja/glossary?term=ID+token">ID トークン</Tooltip>
...
Auth0.js ライブラリを使用して、クライアントサイドでアカウントリンクを行えます。詳しくは、[Auth0.js リファレンス &gt; ユーザー管理](/docs/ja/libraries/auth0js#user-management)を参照してください。
