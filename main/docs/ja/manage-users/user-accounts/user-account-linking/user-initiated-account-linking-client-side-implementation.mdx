...
1. ユーザーをアプリケーションにログインさせます。<br /><br />
   ...
   ユーザーは [Universal Login](/docs/ja/authenticate/login/auth0-universal-login) を使用して SPA で認証を行い、[Management API 用のアクセストークン](https://auth0.com/docs/api/management/v2/get-access-tokens-for-spas) をリクエストします。<br /><br />

   一般的な SPA のログインでは、コールバックは同じページ上でクライアント側で処理され、認証に成功すると JWT を受け取ります。詳しくは、[Single-Page App Quickstart](/docs/ja/quickstart/spa) をご覧ください。
...
* [ユーザーアカウントのリンク解除](/docs/ja/manage-users/user-accounts/user-account-linking/unlink-user-accounts)
