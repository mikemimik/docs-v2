---
description: sort パラメータに field:order の値を指定して、検索結果を並べ替える方法を説明します。
title: 検索結果の並べ替え
---
...
ユーザー検索結果を並べ替えるには、リクエスト時に `sort` パラメータに `field:order` 形式の値を指定します。`field` は並べ替えの基準となるフィールド名で、order には昇順の場合は `1`、降順の場合は `-1` を設定できます。

`app_metadata` または `user_metadata` での並べ替えはサポートされていません。

たとえば、`created_at` フィールドでユーザーを昇順に並べ替えるには、`sort` パラメータに `created_at:1` を指定します。
...
`sort` やその他のパラメータについて詳しくは、[Management API Explorer](https://auth0.com/docs/api/management/v2#!/Users/get_users) のドキュメントをご覧ください。