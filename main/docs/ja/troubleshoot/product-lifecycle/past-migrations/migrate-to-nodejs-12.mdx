...
ルール、Hooks、カスタムデータベーススクリプト、カスタムソーシャル接続については、[Management API](/docs/ja/api) を使用して確認できます。これにより、テスト目的で項目を本番テナントから開発テナントへ移しやすくなります。

<Tooltip tip="Management API: お客様が管理タスクを実行できるようにする製品です。" cta="用語集を見る" href="/docs/ja/glossary?term=Management+API">Management API</Tooltip> の Connections エンドポイントを使用すると、カスタムデータベーススクリプトは `options.customScripts` を使って取得または更新できます。同様に、カスタムソーシャル接続は `options.scripts.fetchUserProfile` で確認できます。
