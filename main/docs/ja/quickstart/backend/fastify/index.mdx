...
<Accordion title="AI を使って Auth0 を統合する" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **次に、AI アシスタントに次のように依頼します:**
  ...
  AI アシスタントが、Auth0 API の作成、資格情報の取得、`@auth0/auth0-fastify-api` のインストール、プラグインの設定、JWT バリデーションによる API エンドポイントの保護まで自動的に行います。[agent skills の完全なドキュメント →](/docs/ja/quickstart/agent-skills)
</Accordion>
...
* **[Fastify Web App クイックスタート](/docs/ja/quickstart/webapp/fastify)** - APIを呼び出すWeb アプリケーションを構築する
