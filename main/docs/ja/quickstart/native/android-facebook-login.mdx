...
2. **Facebook Native 用に Auth0 を設定する** - Auth0 アプリケーションで Facebook Native Sign In を使用するよう設定します。詳しくは、[ネイティブアプリに Facebook Login を追加する](/docs/ja/authenticate/identity-providers/social-identity-providers/facebook-native) を参照してください。
  ...
  <Step title="Auth0 SDKをインストールして設定する" stepNumber={2}>
    ...
    **ActivityでAuth0 SDKを初期化します：**
    ...
    <Note>
      ...
      Web Authentication をサポートする予定がある場合は、[Universal Login を使用した認証](/docs/ja/libraries/auth0-android#authentication-via-universal-login)を参照してください。
    </Note>
  </Step>
...
<CardGroup cols={2}>
  <Card title="他のIDプロバイダーを設定する" icon="key" href="/docs/ja/authenticate/identity-providers" iconType="solid">
  ...
  <Card title="Actions" icon="bolt" href="/docs/ja/customize/actions/actions-overview" iconType="solid">
    サーバーレスコードで認証フローをカスタマイズする
  </Card>

  <Card title="Android SDK リファレンス" icon="android" href="/docs/ja/libraries/auth0-android" iconType="solid">
