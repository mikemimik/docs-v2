...
<Accordion title="AI を使って Auth0 を統合する" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **次に、AI アシスタントにこう依頼します。**
  ...
  AI アシスタントが Auth0 アプリケーションの作成、資格情報の取得、Auth0.swift SDK の依存関係の追加、Auth0.plist の設定、コールバックURLの設定、ログイン／ログアウトフローの実装を自動的に行います。[agent skills の完全なドキュメント →](/docs/ja/quickstart/agent-skills)
</Accordion>
...
<Accordion title="カスタムドメインの設定">
  [カスタムドメイン](/docs/ja/customize/custom-domains) を使用している場合は、すべての箇所で Auth0ドメインの代わりにその値を使用してください。
...
<Accordion title="本番デプロイ">
  ...
  * Auth0 API 証明書をピン留めしては**いけません** - [Auth0 はこの方法を推奨していません](/docs/ja/troubleshoot/product-lifecycle/past-migrations#avoid-pinning-or-fingerprinting-tls-certificates-for-auth0-endpoints)
