...
[Server + API Architecture Scenario](/docs/ja/get-started/architecture-scenarios/server-application-api)の一環として、Node.jsでTimesheets APIを実装します。実装するソリューションの詳細については、このシナリオを参照してください。
...
* <Tooltip tip="アクセストークン: API へのアクセスに使用される、opaque string または JWT の形式の認可資格情報。" cta="用語集を見る" href="/docs/ja/glossary?term=access+token">アクセストークン</Tooltip>を検証するミドルウェア関数を作成します。
...
* アクセストークンを取得します。取得方法の詳細については、[Get an Access Token](/docs/ja/get-started/architecture-scenarios/server-application-api#get-an-access-token) を参照してください
