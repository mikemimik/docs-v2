...
* [JWTDecode.Android](https://github.com/auth0/JWTDecode.Android): このパッケージは、<Tooltip tip="JSON Web Token (JWT): 2者間でクレームを安全に表現するために使われる標準的な ID トークン形式（多くの場合、アクセストークン形式としても使われます）。" cta="用語集を見る" href="/docs/ja/glossary?term=JWTs">JWT</Tooltip>のデコードに役立ちます。
...
`/res/values/strings.xml` にある `strings.xml` リソースで、Auth0 の <Tooltip tip="Client ID: Auth0 から登録済みリソースに付与される識別値。" cta="用語集を見る" href="/docs/ja/glossary?term=Client+ID">Client ID</Tooltip>、Auth0 ドメイン、API の URL を設定します。
...
`LoginActivity` ではユーザーの認可を処理し、ユーザーに最初に表示される画面として使用します。`WebAuthProvider` を初期化して認証を開始する `login()` メソッドを作成します。`WebAuthProvider` には、正しい scheme、<Tooltip tip="Audience: 発行されたトークンの対象者を一意に識別する識別子です。トークンでは aud という名前で表され、その値には ID Token の場合はアプリケーション（Client ID）、Access Token の場合は API（API Identifier）の ID が含まれます。" cta="用語集を見る" href="/docs/ja/glossary?term=audience">audience</Tooltip>、および scope を必ず指定してください。この実装では、次を使用します。

* **scheme**: `demo`
* **audience**: `https://api.exampleco.com/timesheet`s (Node.JS API)
* **response&#95;type**: `code`
* **scope**: `create:timesheets read:timesheets openid profile email offline_access`。これらのスコープにより、Node.JS API に対して `POST` と `GET` を実行できるほか、ユーザープロフィールと <Tooltip tip="Refresh Token: ユーザーに再度ログインを求めることなく、新しい Access Token を取得するために使用されるトークンです。" cta="用語集を見る" href="/docs/ja/glossary?term=Refresh+Token">Refresh Token</Tooltip> も取得できます。
...
`login()` メソッドで `WebAuthProvider` を初期化する前に、`CredentialsManager` を作成しておけます。`AuthenticationAPIClient` を `CredentialsManager` に渡すと、有効期限が切れた <Tooltip tip="アクセストークン: API へのアクセスに使用される、opaque string または JWT の形式の認可資格情報。" cta="用語集を見る" href="/docs/ja/glossary?term=Access+Tokens">アクセストークン</Tooltip> を更新できるようになります。
...
次に、`LoginActivity` の `login()` メソッドを更新して、<Tooltip tip="ID Token: リソースへのアクセス用ではなく、クライアント自身のための認証情報です。" cta="用語集を見る" href="/docs/ja/glossary?term=ID+Token">ID Token</Tooltip> を取得し、JWTDecode.Android ライブラリを使ってトークンからユーザープロファイルを取得します。続いて、そのユーザープロファイルを `UserProfileManager` に保存します。
...
先に進む前に、[Node.JS API を実装](/docs/ja/get-started/architecture-scenarios/mobile-api/api-implementation-nodejs)していることを確認してください。
