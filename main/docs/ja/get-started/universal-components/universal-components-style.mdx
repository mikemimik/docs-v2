...
<Warning>
  Auth0 Universal Componentsは現在、早期アクセス段階です。これを使用することで、[Oktaのマスターサブスクリプション契約](https://www.okta.com/agreements/)に記載された適用中の無料トライアル条件に同意したものとみなされます。詳しくは、[製品リリース段階](/docs/ja/troubleshoot/product-lifecycle/product-release-stages)をご覧ください。
</Warning>

Auth0 Universal Componentsは、[Tailwind CSS](https://tailwindcss.com/docs/installation/using-vite)と[Radix UI primitives](https://www.radix-ui.com/primitives/docs/overview/introduction)を活用した柔軟なデザインシステムを基盤に構築されています。スタイル設定は、[`Auth0ComponentProvider`](/docs/ja/get-started/universal-components/auth0-component-provider)によって一元的に制御され、高レベルのプリセットを使うことも、CSS変数を細かく上書きすることもできます。
