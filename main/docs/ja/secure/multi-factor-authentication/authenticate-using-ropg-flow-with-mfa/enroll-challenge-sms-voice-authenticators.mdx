...
Auth0 には、[Universal Login](/docs/ja/authenticate/login/auth0-universal-login) を使用した、組み込みの <Tooltip tip="多要素認証（MFA）: SMS によるコードなど、ユーザー名とパスワードに加えて認証要素を使用するユーザー認証プロセス。" cta="用語集を見る" href="/docs/ja/glossary?term=MFA">MFA</Tooltip> の登録および認証フローが用意されています。ですが、独自のユーザーインターフェースを作成したい場合は、[MFA API](/docs/ja/secure/multi-factor-authentication/multi-factor-authentication-developer-resources/mfa-api) を使って実現することもできます。
...
* Auth0 Dashboard または [Management API](https://auth0.com/docs/api/management/v2#!/Guardian/put_factors_by_name) を使用して、[電話を認証要素として設定する](/docs/ja/secure/multi-factor-authentication/multi-factor-authentication-factors/configure-sms-voice-notifications-mfa)。
...
登録を開始するタイミングに応じて、MFA API の利用に必要な <Tooltip tip="アクセストークン: API へのアクセスに使用する、opaque な文字列または JWT 形式の認可資格情報。" cta="用語集を見る" href="/docs/ja/glossary?term=access+token">アクセストークン</Tooltip> は、いくつかの方法で取得できます。
...
登録を完了するには、<Tooltip tip="Token Endpoint: 認可サーバー上でプログラムからトークンを要求するために使用されるエンドポイント。" cta="用語集を見る" href="/docs/ja/glossary?term=OAuth">OAuth</Tooltip> <Tooltip tip="OAuth 2.0: 認可プロトコルとワークフローを定義する認可フレームワーク。" cta="用語集を見る" href="/docs/ja/glossary?term=Token+endpoint">トークンエンドポイント</Tooltip> に `POST` リクエストを送信します。前のレスポンスで返された `oob_code` と、メッセージで受け取った値を含む `binding_code` を指定する必要があります。
...
[MFA を使用して Resource Owner Password グラントで認証する](/docs/ja/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa) の手順に従って、MFAトークンを取得します。
...
`mfa_token` を使って [`mfa/authenticator`](/docs/ja/api/authentication/muti-factor-authentication/list-authenticators) エンドポイントを呼び出し、ユーザーが登録済みのすべての認証要素の一覧を取得したうえで、アプリケーションがサポートしている種類と一致するものを特定します。さらに、チャレンジを発行するには、対応する `authenticator_type` も取得する必要があります:
...
Auth0 Actions を使用すると、MFAフローをさらに柔軟にカスタマイズできます。詳しくは、[Actions トリガー: post-challenge - API Object](/docs/ja/customize/actions/explore-triggers/password-reset-triggers/post-challenge-trigger/post-challenge-api-object) をご覧ください。
...
* [Push 認証器の登録とチャレンジ](/docs/ja/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa/enroll-and-challenge-push-authenticators)