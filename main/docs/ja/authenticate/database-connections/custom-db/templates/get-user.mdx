...
このスクリプトは、[自動移行](/docs/ja/manage-users/user-migration/configure-automatic-migration-from-your-database) では必須であり、レガシー認証では接続に設定された操作に応じて条件付きで必須になります。Auth0 では、重複ユーザーの作成を避けるため、返されるユーザープロファイルに固定の `user_id` を設定することを推奨しています。
...
ユーザーが見つかった場合は、`error` パラメータに `null` を渡し、`profile` パラメータには[正規化された形式](/docs/ja/manage-users/user-accounts/user-profiles/normalized-user-profile-schema)のユーザーのプロファイルデータを渡します。標準フィールドに加えて、`user_metadata`、`app_metadata`、`mfa_factors` フィールドを含めることもできます。
...
エラーが発生した場合は、問題の内容がわかる関連情報とともに、エラーデータを `error` パラメータに渡します。詳しくは、[カスタムデータベースのトラブルシューティング](/docs/ja/authenticate/database-connections/custom-db/error-handling)を参照してください。
