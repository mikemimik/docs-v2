...
このスクリプトは任意です。有効にすると、ユーザーが <Tooltip tip="Universal Login: ユーザーのアイデンティティを確認するために、アプリケーションは Auth0 の認可サーバーでホストされている Universal Login にリダイレクトされます。" cta="用語集を見る" href="/docs/ja/glossary?term=Universal+Login">Universal Login</Tooltip> からサインアップした場合、または <Tooltip tip="Auth0 Dashboard: サービスを構成するための Auth0 の主要製品です。" cta="用語集を見る" href="/docs/ja/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip> か Auth0 <Tooltip tip="Auth0 Dashboard: サービスを構成するための Auth0 の主要製品です。" cta="用語集を見る" href="/docs/ja/glossary?term=Management+API">Management API</Tooltip> を通じて作成された場合に、Auth0 は外部データベース内に対応するユーザーレコードを作成するため、このスクリプトを実行します。

Auth0 でユーザーが作成されると、Auth0 は一連のスクリプトを呼び出します。

1. [Get User](/docs/ja/authenticate/database-connections/custom-db/templates/get-user): ユーザーが Auth0 または外部データベースにまだ存在しないことを確認します。
2. Create: 外部データベースにユーザーを作成します。
3. [Login](/docs/ja/authenticate/database-connections/custom-db/templates/login): ユーザーが正常に作成されたことを確認します。
...
`ValidationError` はカスタムエラー型のオブジェクトで、[テナントログ](/docs/ja/deploy-monitor/logs)に表示されるデータを渡せます。
...
| `username`      | ユーザーのユーザー名です。[**Requires Username**](/docs/ja/authenticate/database-connections/require-username) 設定がカスタムデータベース接続で有効になっている場合にのみ必須です。 |
...
カスタムデータベース接続で [Requires Username](/docs/ja/authenticate/database-connections/require-username) 設定が有効になっている場合、[Login](/docs/ja/authenticate/database-connections/custom-db/templates/login) スクリプトと [Get User](/docs/ja/authenticate/database-connections/custom-db/templates/get-user) スクリプトは `username` プロパティに対応している必要があるため、これを外部データベースに保存しておく必要があります。
...
サインアップ時に[カスタムフィールドを作成して使用する](/docs/ja/libraries/custom-signup#using-the-api)と、それらは`user`オブジェクトに含まれます。
