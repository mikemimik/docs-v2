...
OIDC および Okta Workforce エンタープライズ接続向けの Private Key JWT Client Authentication は、<Tooltip tip="Auth0 Dashboard: サービスを設定するための Auth0 の主要な製品です。" cta="用語集を表示" href="/docs/ja/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip> または <Tooltip tip="Auth0 Dashboard: サービスを設定するための Auth0 の主要な製品です。" cta="用語集を表示" href="/docs/ja/glossary?term=Management+API">Management API</Tooltip> のいずれかから設定できます。
...
* OpenID プロバイダーの <Tooltip tip="Audience: 発行されたトークンの audience の一意な識別子。トークンでは aud という名前で表され、その値には、ID トークンの場合はアプリケーション (Client ID) の ID、Access Token の場合は API (API 識別子) の ID が含まれます。" cta="用語集を表示" href="/docs/ja/glossary?term=token+endpoint">トークンエンドポイント</Tooltip> を識別する `aud` (<Tooltip tip="Audience: 発行されたトークンの audience の一意な識別子。トークンでは aud という名前で表され、その値には、ID トークンの場合はアプリケーション (Client ID) の ID、Access Token の場合は API (API 識別子) の ID が含まれます。" cta="用語集を表示" href="/docs/ja/glossary?term=audience">audience</Tooltip>) 。
* 1 回限りの使用やリプレイ攻撃対策を可能にする `jti` (JWT ID) 。
* トークンの有効期間を制限する `exp` (有効期限) 。
* <Tooltip tip="Client ID: Auth0 から登録済みリソースに付与される識別値です。" cta="用語集を表示" href="/docs/ja/glossary?term=client+ID">client ID</Tooltip> を識別する `sub` と `iss`。
...
* [Auth0アプリケーションを Okta Workforce Enterprise 接続に接続する](/docs/ja/authenticate/identity-providers/enterprise-identity-providers/okta)
* [OpenID Connect の IDプロバイダーに接続する](/docs/ja/authenticate/identity-providers/enterprise-identity-providers/oidc)