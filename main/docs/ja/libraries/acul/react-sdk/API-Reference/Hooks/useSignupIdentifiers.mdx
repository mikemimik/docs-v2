...
<ParamField body="useSignupIdentifiers()" type={<span>[] | <a href="/docs/ja/libraries/acul/react-sdk/API-Reference/Types/interfaces/Identifier">Identifier[]</a></span>}>
  有効になっている識別子 (メールアドレス、電話番号、またはユーザー名) の一覧を返します。各識別子には `required` ステータスが含まれ、
  現在の画面のサインアップ識別子に基づいて決まります。

  ## 戻り値

  [] | [`Identifier`](/docs/ja/libraries/acul/react-sdk/API-Reference/Types/interfaces/Identifier)[]

  各要素に `type` (識別子の種類) と、サインアップ時に必須かどうかを示す `required` フラグを含む、[Identifier](/docs/ja/libraries/acul/react-sdk/API-Reference/Types/interfaces/Identifier) オブジェクトの配列。
