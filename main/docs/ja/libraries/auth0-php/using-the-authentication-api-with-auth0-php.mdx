...
以下のドキュメントは、[Installation と Getting started セクション](/docs/ja/libraries/auth0-php)の手順を完了し、そこで示されているコードの続きから進めることを前提としています。

<div id="authorization-code-flow">
  ## 認可コードフロー
</div>

[認可コードフロー](/docs/ja/get-started/authentication-and-authorization-flow/authorization-code-flow)は、ユーザーにアプリケーションへのアクセスを許可するための基本的な方法です。このフローは、[Auth0-PHP Basic Use page](/docs/ja/libraries/auth0-php/auth0-php-basic-use)で使用されているものと同じです。ログインやコールバックの処理をより細かく制御する必要がある場合は、このセクションで Authentication API を直接使用する方法を説明します。
...
2. `state` 値が存在すること、およびそれがこちらで生成したものと一致することを確認します。[これは CSRF 攻撃を防ぐために重要です。](/docs/ja/secure/attack-protection)
...
[クライアント認証情報フロー](/docs/ja/get-started/authentication-and-authorization-flow/client-credentials-flow) では、Auth0 Dashboard で設定したスコープに基づいて、アプリケーションに特定の API へのアクセス権を付与できます。これにより、たとえばアプリケーションから <Tooltip tip="Management API: お客様が管理タスクを実行できるようにするプロダクト。" cta="用語集を見る" href="/docs/ja/glossary?term=Management+API">Management API</Tooltip> を呼び出せます。認証が成功すると、リクエストした API 用の <Tooltip tip="Access Token: API へのアクセスに使用される認可資格情報で、不透明な文字列または JWT の形式を取ります。" cta="用語集を見る" href="/docs/ja/glossary?term=Access+Token">Access Token</Tooltip> が発行されます。
...
このアクセストークンの使用方法について詳しくは、[Auth0-PHP で Management API を使用する](/docs/ja/libraries/auth0-php/using-the-management-api-with-auth0-php)を参照してください。
...
* [PHP: Auth0-PHP を使い始める](/docs/ja/libraries/auth0-php)
* [PHP: Auth0-PHP でのログイン、ログアウト、ユーザープロファイルの取得](/docs/ja/libraries/auth0-php/auth0-php-basic-use)
* [PHP: Auth0-PHP で Management API を使用する](/docs/ja/libraries/auth0-php/using-the-management-api-with-auth0-php)
* [PHP: Auth0-PHP で JWT (JSON Web トークン) を検証する](/docs/ja/libraries/auth0-php/validating-jwts-with-auth0-php)
