...
class AuthenticationControllerProvider {
    ...
    // si plusieurs fils d'exécution peuvent appeler ceci, synchronisez cette méthode et envisagez le double verrouillage
    static AuthenticationController getInstance(ServletConfig config) throws UnsupportedEncodingException {
        if (INSTANCE == null) {
            ...
            // JwkProvider requis pour les jetons RS256. Si vous utilisez HS256, ne pas utiliser.
