...
export const SignUpForm = () => {
  ...
  /** Start LoggedInForm */
  /**
   * Demo de flujos de autenticación (sin librerías de UI externas)
   * - Solo Tailwind, estructura lista para producción
   * - Lista de aplicaciones respaldada por LocalStorage + configuración de URL por aplicación
   * - 3 flujos:
   *   1) Menú: Crear | Integrar | Ejemplo
   *   2) Crear aplicación -> agrega a la lista y redirige al flujo de integración preseleccionado
   *   3) Integrar con existente: seleccionar aplicación + guardar URLs -> notificación
   */
    ...
    function IntegrateForm({ apps, selected, onSelect, saved, onSave, onCancel }) {
      ...
      return (
        <div className="space-y-6">
          ...
          >
            <Input id="callbacks" name="callbacks" label="URLs de devolución de llamada" placeholder="http://localhost:3000" value={callbacks} onChange={setCallbacks} />
  ...
  const CreateApplicationForm = () => {
    ...
    return (
      <Card className="flex-col items-start p-4 gap-3.75">
        <span className="font-medium text-gray-900 dark:text-gray-200">Crear Auth0 App</span>
