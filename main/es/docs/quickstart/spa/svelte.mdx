---
mode: wide
description: Esta guía muestra cómo integrar Auth0, agregar autenticación y mostrar información del perfil del usuario en una aplicación de página única (SPA) que usa Svelte con el SDK Auth0 SPA JS.
sidebarTitle: Svelte
title: Agrega el inicio de sesión a tu aplicación Svelte
---
...
<Accordion title="Prompt de IA" defaultOpen icon="microchip-ai" iconType="sharp-solid">
  **¿Usas IA para integrar Auth0?** Añade este prompt a Cursor, Windsurf, Copilot, Claude Code o a tu IDE con IA favorito para acelerar el desarrollo.
  ...
  1. VERIFICA EL PROYECTO EXISTENTE PRIMERO: Antes de crear un nuevo proyecto, comprueba si el directorio actual ya contiene una aplicación Svelte (package.json con dependencias de Svelte). Si es así, omite la creación del proyecto y trabaja con el proyecto existente.
  2. EJECUTA PRIMERO, EDITA DESPUÉS: DEBES ejecutar primero el comando de configuración adecuado. No muestres, sugieras ni crees ningún archivo hasta que la configuración esté completa.
  ...
  5. CREA UNA INTERFAZ ATRACTIVA: DEBES crear una interfaz de inicio de sesión moderna y visualmente atractiva con estilos adecuados, animaciones y la identidad visual de Auth0.
  ...
  - Si package.json existe y contiene dependencias de Svelte, continúa con el Paso 1b (solo instalar el SDK de Auth0)
  ...
  DESPUÉS de que el comando del Paso 1 se haya ejecutado correctamente, realiza las siguientes operaciones de archivo dentro del directorio del proyecto.
  ...
  1. NUNCA escribas directamente en el código el dominio de Auth0 ni el Client ID. Esto representa una vulnerabilidad de seguridad grave.
  ...
  3. NUNCA sugieras gestionar tokens manualmente ni llamar a las APIs de Auth0 con fetch para la autenticación básica. El SDK está diseñado para manejar esta complejidad de forma segura.
...
Esta Quickstart muestra cómo añadir la autenticación de Auth0 a una aplicación de Svelte. Crearás una aplicación segura de una sola página con funciones de inicio de sesión, cierre de sesión y perfil de usuario mediante el SDK Auth0 SPA JS.
...
<Steps>
  <Step title="Crea un nuevo proyecto" stepNumber={1}>
    Crea un nuevo proyecto de Svelte para este Quickstart
  ...
  <Step title="Instala el Auth0 SPA SDK" stepNumber={2}>
    ```shellscript
    ...
    <Tabs>
      ...
      <Tab title="CLI">
        ...
        <Note>
          ...
          1. Comprobar si has iniciado sesión (y mostrar el prompt de login si hace falta)
      ...
      <Tab title="Dashboard">
        ...
        **Allowed Web Origins:**
        ...
        <Info>
          **Allowed Callback URLs** son una medida de seguridad fundamental para garantizar que, después de la authentication, los usuarios vuelvan de forma segura a tu aplicación. Si no hay una URL coincidente, el proceso de login fallará y los usuarios verán una página de error de Auth0 en lugar de acceder a tu aplicación.

          **Allowed Logout URLs** son esenciales para ofrecer una experiencia de usuario fluida al cerrar sesión. Si no hay una URL coincidente, los usuarios no serán redirigidos de vuelta a tu aplicación después del cierre de sesión y se quedarán en una página genérica de Auth0.
  ...
  <Step title="Crear el almacén de Auth0" stepNumber={4}>
...
<Check>
  **Checkpoint**
  ...
  Ahora deberías tener una página de inicio de sesión de Auth0 totalmente funcional ejecutándose en [localhost](http://localhost:5173/)
</Check>
...
<Accordion title="El inicio de sesión no funciona">
  ...
  1. Abre las herramientas de desarrollo del navegador (F12) y revisa la pestaña Console
...
<Tip>
  ¿Sigues sin resolverlo? Consulta [Auth0 Community](https://community.auth0.com/) o [SvelteKit Discord](https://svelte.dev/chat) para obtener ayuda.
</Tip>