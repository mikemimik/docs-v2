---
mode: wide
description: Esta guía muestra cómo integrar Auth0, añadir autenticación y mostrar información del perfil de usuario en una aplicación de una sola página (SPA) que utiliza Cap'n Web RPC con el Auth0 SPA SDK.
sidebarTitle: Cap'n Web
title: Añadir inicio de sesión a tu aplicación Cap'n Web
---
...
<Accordion title="Prompt para IA" defaultOpen icon="microchip-ai" iconType="sharp-solid">
  **¿Usas IA para integrar Auth0?** Añade este prompt a Cursor, Windsurf, Copilot, Claude Code o tu IDE favorito con IA para acelerar el desarrollo.
  ...
  2. EXECUTE FIRST, EDIT SECOND: DEBES ejecutar primero el comando de configuración adecuado. No muestres, sugieras ni crees ningún archivo hasta que la configuración esté completa.
  3. NO PLANNING: NO propongas una estructura de directorios. Tu primera acción debe ser ejecutar el comando correspondiente.
  ...
  Step 1: Crear el proyecto Cap'n Web
  ...
  Step 3: Configurar la aplicación de Auth0 (usa el comando de CLI del Step 3 del quickstart)

  Step 4: Configurar la aplicación y la API de Auth0
  - Crear una aplicación de Auth0 (tipo SPA)
  - Crear una API de Auth0 con los scopes requeridos
  - Configurar las URLs de devolución de llamada y los orígenes

  Step 5: Implementar el servidor con validación JWT
  ...
  - Implementar endpoints seguros de gestión de perfil

  Step 6: Implementar el cliente con integración de Auth0
  - Inicializar el cliente Auth0 SPA con tokens de actualización habilitados
  ...
  Step 3: Configurar la aplicación y la API de Auth0
  DESPUÉS de que los comandos de los Steps 1 y 2 se hayan ejecutado correctamente, realizarás la configuración de Auth0.
  ...
  Step 3.1: Navegar al directorio del proyecto (si es necesario) y configurar Auth0:
  ...
  Step 3.2: Crear la plantilla manual de .env (si la configuración automática falla)
  ...
  Step 3.3: Mostrar las instrucciones de configuración manual
    ...
    echo "7. Agrega scopes: read:profile, write:profile"
    echo "8. Actualiza el archivo .env con tu dominio, ID de cliente y audiencia de la API"

  Step 4: Implementar el servidor WebSocket seguro con validación JWT
  ...
  5.3: Create HTTP server and WebSocket server:
    ...
    const server = http.createServer(async (req, res) => {
      ...
      // Servir archivos HTML, JS y módulos de npm
      if (req.url === '/' || req.url === '/index.html') {
        const html = readFileSync(join(__dirname, '../client/index.html'), 'utf8');
  ...
  Usa newWebSocketRpcSession de capnweb y el Auth0 SDK con las importaciones de módulos ES adecuadas:
    ...
    async function initializeApp() {
      try {
        ...
        if (isAuthenticated) {
          // Conectarse al WebSocket solo si está autenticado
...
Esta guía de inicio rápido muestra cómo añadir la autenticación de Auth0 a una aplicación web de Cap&#39;n Web. Crearás una aplicación web moderna basada en RPC con inicio de sesión seguro utilizando el framework JavaScript de Cap&#39;n Web y el Auth0 SPA SDK.
...
<Steps>
  ...
  <Step title="Instalar las dependencias" stepNumber={2}>
    ...
    Instala los SDK de Auth0 para autenticación y verificación de tokens:
    ...
    <Info>
      **@auth0/auth0-spa-js** se usa en el cliente para gestionar la autenticación de usuarios, los flujos de inicio de sesión y los tokens en el navegador.

      **@auth0/auth0-api-js** se usa en el servidor para verificar tokens de acceso y validar firmas JWT con el JWKS de Auth0.
    </Info>
  ...
  <Step title="Configura tu Auth0 App" stepNumber={3}>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        Por último, en la pestaña **Settings** de la página de detalles de tu aplicación, configura las siguientes URLs:
        ...
        **Obligatorio: crea una API de Auth0**
        ...
        Debes crear una API de Auth0 para que la validación del token funcione:
        ...
        <Warning>
          Si no creas una API de Auth0, obtendrás un error de &quot;access&#95;denied&quot; durante el inicio de sesión. El identificador de la API debe coincidir exactamente con tu variable de entorno `AUTH0_AUDIENCE`.
        </Warning>
      </Tab>
    </Tabs>
  </Step>
  ...
  <Step title="Crear el servidor" stepNumber={5}>
    ...
    // Inicializa el cliente de la API de Auth0 para la verificación de tokens
    const auth0ApiClient = new ApiClient({
      domain: AUTH0_DOMAIN,
      audience: AUTH0_AUDIENCE
    });
...
<Accordion title="Seguridad de RPC en el servidor">
  Refuerza la seguridad añadiendo validación adicional y limitación de tasa a tus métodos RPC:
  ...
  class ProfileService extends RpcTarget {
    constructor() {
      super();
      this.rateLimiter = new Map(); // Limitación de tasa sencilla en memoria
    }
  ...
  const connectionManager = new RpcConnectionManager(
    `${protocol}//${host}/api`,
    {
      ...
      onReconnect: async (api) => {
        // Actualiza la interfaz o recarga los datos tras la reconexión
        await displayProfile(api);
      }
    }
  );

  // Conectarse cuando el usuario esté autenticado
  if (isAuthenticated) {
...
<Accordion title="Integración de base de datos">
  Reemplaza el almacenamiento en memoria por una base de datos para usarla en producción:
