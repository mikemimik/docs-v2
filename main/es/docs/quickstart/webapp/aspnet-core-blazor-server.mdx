---
mode: wide
description: Esta guía muestra cómo integrar Auth0 en cualquier aplicación de Blazor Server, nueva o existente, mediante el SDK Auth0.AspNetCore.Authentication.
sidebarTitle: Blazor Server
title: Añade el inicio de sesión a tu aplicación Blazor Server
---
...
Auth0 te permite añadir rápidamente autenticación y acceder a la información del perfil de usuario en tu aplicación. Esta guía muestra cómo integrar Auth0 con cualquier aplicación nueva o existente de Blazor Server mediante el SDK `Auth0.AspNetCore.Authentication`.
...
<Steps>
  ...
  <Step title="Agregar las páginas de Login y cierre de sesión" stepNumber={5}>
    Cree páginas de Login y cierre de sesión para que los usuarios puedan autenticarse.
  ...
  <Step title="Crear la página de perfil y actualizar el diseño" stepNumber={6}>
    Crea una página personalizada de perfil de usuario para mostrar el nombre y los claims del usuario, y actualiza el layout para agregar enlaces de inicio y cierre de sesión.
    ...
    Agrega los siguientes fragmentos de código. Ten en cuenta que debes agregar el código de `MainLayout` en la sección superior de tu layout, manteniendo intacto todo lo demás.
  ...
  <Step title="Ejecuta tu aplicación" stepNumber={7}>
    ...
    Después de iniciar sesión, volverás a tu aplicación y deberías ver tu nombre en el menú de navegación.
  </Step>
</Steps>
...
<Check>
  **Checkpoint**
  ...
  Ahora deberías tener una aplicación de Blazor Server protegida con Auth0, completamente funcional y ejecutándose en [http://localhost:5000](http://localhost:5000). Los usuarios pueden iniciar sesión, ver su perfil y cerrar sesión.
</Check>
...
<Accordion title="Guardar tokens para llamadas a la API">
  Si necesitas llamar a APIs externas en nombre del usuario, puedes recuperar y guardar tokens:
...
<Accordion title="Configurar Organizations">
  Configura el soporte para Organizations en escenarios B2B:
...
<AccordionGroup>
  <Accordion title="No se puede obtener la configuración">
    ...
    **Solución:** Verifica que el valor de Domain sea correcto y no incluya `https://`. La biblioteca construye automáticamente la autoridad.
    ...
    * Que tu aplicación tenga acceso a Internet para acceder a Auth0
...
A continuación, se presenta una aplicación de ejemplo que muestra la integración de Auth0 con ASP.NET Core MVC.:

<Card title="Aplicación en Blazor para ASP.NET Core" icon="github" href="https://github.com/auth0-samples/auth0-aspnetcore-blazor-server-samples/tree/main/Quickstart/Sample">
