---
mode: wide
description: Esta guía muestra cómo integrar Auth0, agregar autenticación y mostrar información del perfil del usuario en una aplicación de Python con Auth0 FastAPI SDK.
sidebarTitle: FastAPI
title: Agregar inicio de sesión a tu aplicación FastAPI
---
...
<Accordion title="Prompt de IA" defaultOpen icon="microchip-ai" iconType="sharp-solid">
  **¿Usas IA para integrar Auth0?** Añade este prompt a Cursor, Windsurf, Copilot, Claude Code o tu IDE favorito con IA para acelerar el desarrollo.
  ...
  5.  NUNCA omitas la activación del entorno virtual. Los comandos pip fallarán o contaminarán el entorno Python global.
  ...
  Issue 2: Error de expansión de glob en shell con uvicorn[standard]
  ...
  Problema: Las cookies seguras no funcionan por HTTP
...
<Note>
  ...
  **Compatibilidad de versiones de FastAPI:** Este Quickstart requiere **FastAPI 0.115.11+** y **Pydantic 2.12.5+**.
</Note>
...
Esta guía de inicio rápido muestra cómo agregar la autenticación de Auth0 a una aplicación web de Python con FastAPI. Crearás una aplicación web segura con funciones de inicio de sesión, cierre de sesión y perfil de usuario usando el [Auth0 FastAPI SDK](https://github.com/auth0/auth0-fastapi).
...
<Steps>
  ...
  <Step title="Configura tu Auth0 App" stepNumber={3}>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        4. Ve a la pestaña **Settings** en la página Detalles de la aplicación
        ...
        Por último, en la pestaña **Settings** de la página Detalles de la aplicación, configura las siguientes URL:
  ...
  <Step title="Configurar el SDK de Auth0 para FastAPI" stepNumber={4}>
    ...
    <Info>
      **SESSION&#95;SECRET** se usa para cifrar las cookies de sesión y debe ser criptográficamente seguro. Sin un secreto sólido (mínimo 32 bytes), las sesiones de su aplicación pueden verse comprometidas. Genere un secreto seguro con `openssl rand -hex 64` y nunca lo incluya en el control de versiones.
      ...
      **HTTPS en producción** es obligatorio para las cookies seguras (`secure=True`). Sin HTTPS, los navegadores no enviarán las cookies de sesión y los usuarios cerrarán sesión repetidamente después de cada solicitud.
    </Info>
  </Step>
  ...
  <Step title="Crear rutas y mostrar el perfil del usuario" stepNumber={5}>
    ...
    * Una página de inicio (`/`) que muestra un botón de inicio de sesión cuando el usuario no ha iniciado sesión, o su perfil cuando sí la ha iniciado
    * Un endpoint de API protegido (`/profile`) que devuelve los datos del usuario en JSON y requiere autenticación
...
<Check>
  **Comprobación**
  ...
  Ahora ya deberías tener una Auth0 Login Page completamente funcional en ejecución en tu [localhost](http://localhost:3000/).
</Check>
...
<Accordion title="Protección de rutas de API con dependencias personalizadas">
  Crea dependencias personalizadas de FastAPI para el control de acceso basado en roles:
  ...
  async def require_roles(required_roles: List[str]):
      """Factoría de dependencias para el control de acceso basado en roles"""
  ...
  async def delete_user(
      ...
      return {"message": f"Se eliminó el usuario {user_id}"}
  ```
</Accordion>

<Accordion title="Llamar a API protegidas con tokens de acceso">
  Configura el SDK para solicitar tokens de acceso para tu API y usarlos en llamadas posteriores:

  ```python
  from auth0_fastapi.config import Auth0Config
  import httpx

  # Configurar Auth0 con la audience de la API
  config = Auth0Config(
      domain=os.getenv("AUTH0_DOMAIN"),
      client_id=os.getenv("AUTH0_CLIENT_ID"),
      client_secret=os.getenv("AUTH0_CLIENT_SECRET"),
      ...
      secret=os.getenv("SESSION_SECRET"),
      audience="https://api.example.com",  # El identificador de tu API
      authorization_params={
...
<Accordion title="Usar Redis para el almacenamiento de sesiones con estado">
  Escala tu aplicación almacenando las sesiones en Redis en lugar de usar cookies cifradas:
  ...
  * **Sin límites de tamaño para las cookies** - Almacena una cantidad ilimitada de datos de sesión
  * **Invalidación inmediata** - Elimina sesiones del lado del servidor
  * **Compatibilidad con logout por backchannel** - Gestiona eventos de logout desde Auth0
...
<Accordion title="Las sesiones no persisten / los usuarios cierran sesión repetidamente">
  **Problema**: Los usuarios inician sesión, pero las sesiones no se mantienen entre peticiones.
  ...
  **Posibles causas y soluciones**:
  ...
  2. **Uso de HTTP en producción con cookies seguras**
...
<Accordion title="Error de Auth0: URL de devolución de llamada mismatch">
  ...
  **Solución**:
  ...
  2. Agrega tu URL de devolución de llamada a **Allowed Callback URLs**:
     ...
     http://localhost:3000/auth/callback
     ```
  3. Para producción, agrega la URL de producción:
     ```
...
<Accordion title="Problemas con ImportError o async/await">
...
<Accordion title="HTTPS requerido en producción">
  **Problema**: Las sesiones funcionan localmente, pero no en producción.

  **Causa**: Las cookies seguras requieren HTTPS en producción. La marca `secure=True` impide que las cookies se envíen a través de conexiones HTTP sin cifrar.
