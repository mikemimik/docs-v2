    ...
    <ResponseField name="riskAssessment" type="dictionary" post={["optional"]}>
      ...
      <Expandable title="propiedades de riskAssessment" defaultOpen>
        <ResponseField name="supplemental" type="dictionary" post={["optional"]}>
          Supplemental Signals enviados por proveedores externos para ayudar en las evaluaciones de riesgo.
...
<ResponseField name="event.client" type="dictionary" post={["optional"]}>
  ...
  <Expandable title="propiedades del cliente" defaultOpen>
    ...
    <ResponseField name="metadata" type="dictionary">
      Un objeto que almacena otras propiedades de la aplicación.
    </ResponseField>
...
<ResponseField name="event.security_context" type="object" post={["optional"]}>
  Un objeto que contiene firmas de huellas digitales. Solo estará disponible si el cliente usa Cloudflare. La huella digital JA3/JA4 puede ser nula o estar vacía en algunos casos. El caso más común ocurre con las solicitudes HTTP, porque JA3 y JA4 se calculan en TLS. También puede estar vacía porque el Worker envía solicitudes dentro de la misma zona o a una zona que no pasa por proxy (o a un tercero).
...
<ResponseField name="event.transaction" type="dictionary" post={["optional"]}>
  ...
  <Expandable title="propiedades de la transacción" defaultOpen>
    ...
    <ResponseField name="locale" type="string">
      La configuración regional que se utilizará para esta transacción, determinada al comparar los idiomas solicitados por el navegador con la configuración de idioma del tenant.
    </ResponseField>
    ...
    <ResponseField name="prompt" type="array of strings" post={["optional"]}>
      Lista de instrucciones que indica si se le puede solicitar al usuario una nueva autenticación y consentimiento.
    </ResponseField>
    ...
    <ResponseField name="requested_scopes" type="array of strings">
      Los scopes solicitados (si corresponde) al iniciar este flujo de autenticación.
    </ResponseField>
    ...
    <ResponseField name="correlation_id" type="string" post={["optional"]}>
      El ID de correlación puede proporcionarse en la solicitud de autenticación inicial cuando la aplicación redirige a Universal Login. Puedes usar este valor para correlacionar registros y solicitudes de tu código de Action con el flujo del usuario.
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="event.user" type="dictionary">
  ...
  <Expandable title="propiedades del usuario" defaultOpen>
    ...
    <ResponseField name="user_metadata" type="dictionary" post={["optional"]}>
      Campos personalizados que almacenan información sobre un usuario que no afecta a lo que puede o no puede acceder, como la dirección del trabajo, el domicilio o las preferencias del usuario.
    </ResponseField>
