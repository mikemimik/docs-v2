...
Configura el <Tooltip tip="Client ID: Valor de identificación proporcionado por Auth0 a tu recurso registrado." cta="Ver glosario" href="/es/docs/glossary?term=Client+ID">Client ID</Tooltip>, el dominio de Auth0 y la URL de la API en el recurso `strings.xml`, ubicado en `/res/values/strings.xml`:
...
Antes de inicializar `WebAuthProvider` en el método `login()`, podemos crear `CredentialsManager`. Al pasar un `AuthenticationAPIClient` a `CredentialsManager`, este puede renovar los <Tooltip tip="Token de acceso: credencial de autorización, en forma de una cadena opaca o JWT, utilizada para acceder a una API." cta="Ver glosario" href="/es/docs/glossary?term=Access+Tokens">tokens de acceso</Tooltip> si han expirado.
...
Antes de continuar, asegúrate de haber [implementado la API de Node.JS](/es/docs/get-started/architecture-scenarios/mobile-api/api-implementation-nodejs).
