---
description: Describe la implementación de la autenticación Passwordless con inicio de sesión integrado en aplicaciones web clásicas.
...
Para usar las API de <Tooltip tip="Passwordless: Forma de autenticación que no depende de una contraseña como primer factor." cta="Ver glosario" href="/es/docs/glossary?term=Passwordless">Passwordless</Tooltip> integradas en aplicaciones web clásicas, asegúrate de habilitar la concesión **Passwordless OTP** en [Auth0 Dashboard &gt; Applications &gt; Applications](https://manage.auth0.com/#/applications), dentro de la configuración de tu aplicación, en **Advanced Settings** &gt; **Grant Types**.

La autenticación passwordless para aplicaciones web clásicas consta de dos pasos:

1. Captura el identificador del usuario en tu aplicación (su correo electrónico o número de teléfono) e invoca el endpoint `/passwordless/start` para iniciar el flujo passwordless. El usuario recibirá un correo electrónico, un SMS con un código de un solo uso o un Magic Link.
...
**Envía un Magic Link por correo electrónico**
...
**Autenticar a un usuario con un Magic Link**

Cuando envías un Magic Link, no necesitas hacer una llamada a la API para autenticar al usuario. Los usuarios harán clic en el enlace y serán redirigidos a la URL de devolución de llamada.
...
El endpoint `/passwordless/start` tiene un [límite de frecuencia](/es/docs/troubleshoot/customer-support/operational-policies/rate-limit-policy) de 50 solicitudes por hora por IP. Si llamas a la API desde el servidor, la IP de tu backend puede alcanzar fácilmente estos límites de frecuencia. Para saber cómo solucionar este problema, consulta la sección Limitación de frecuencia en endpoints de Passwordless de [Using Passwordless APIs](/es/docs/authenticate/passwordless/implement-login/embedded-login/relevant-api-endpoints).

<div id="customize-mfa">
  ## Personaliza MFA
</div>

<ReleaseStageNotice feature="MFA personalizable con los flujos de concesión de contraseña del propietario del recurso, integrados o de token de actualización" stage="ea" terms="true" contact="equipo de soporte de Auth0" />

Personaliza <Tooltip tip="Autenticación multifactor (MFA): proceso de autenticación de usuarios que usa un factor además del nombre de usuario y la contraseña, como un código por SMS." cta="Ver glosario" href="/es/docs/glossary?term=MFA">MFA</Tooltip> con flujos integrados. Usa la API de MFA para permitir que los usuarios registren y verifiquen su identidad con los factores compatibles con tu aplicación que prefieran.

Al usar [Lock for Web](/es/docs/libraries/lock#2-authenticating-and-getting-user-info), el endpoint `oauth/token` devuelve el error `mfa_required` e incluye el `mfa_token` que necesitas para usar la API de MFA y el parámetro `mfa_requirements`, con una lista de autenticadores que tu aplicación admite actualmente:
...
Usa el `mfa_token` para llamar al endpoint [`mfa/authenticator`](https://auth0.com/docs/api/authentication/muti-factor-authentication/list-authenticators) y enumerar todos los factores que el usuario ya tiene registrados, y hacer coincidir el mismo `type` que admite tu aplicación. También necesitas obtener el `authenticator_type` correspondiente para emitir desafíos:
...
Personalice aún más su flujo de MFA con Auth0 Actions. Para obtener más información, lea [Actions Triggers: post-challenge - objeto de API](/es/docs/customize/actions/explore-triggers/password-reset-triggers/post-challenge-trigger/post-challenge-api-object).