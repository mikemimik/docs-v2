---
description: Proporciona un script de devolución de llamada personalizado para quienes estén integrando Auth0 con IBM DB2.
title: Script de Login para IBM DB2
---

Si está integrando Auth0 con [IBM DB2](https://www.ibm.com/analytics/us/en/technology/db2), puede usar el siguiente script en su proceso de inicio de sesión.
...
function login (email, password, callback) {
  ...
  ibmdb.open(credentials, function(err, conn) {
    ...
    conn.query("select * from =<user_name>.USERS where email='" + email + "'", function(err, data) {
      ...
      //mapear atributos al perfil – ejemplo a continuación
      var profile = {
        user_id: data[0].ID,
