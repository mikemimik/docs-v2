...
Para conectar tu aplicación a un <Tooltip tip="Proveedor de identidad (IdP): servicio que almacena y administra identidades digitales." cta="Ver glosario" href="/es/docs/glossary?term=Identity+Provider">proveedor de identidad</Tooltip> OIDC, debes:
...
<Card title="Encuentra tu nombre de dominio de Auth0 para las redirecciones">
  Si tu nombre de dominio de Auth0 no aparece arriba y no estás usando nuestra función de [dominios personalizados](/es/docs/customize/custom-domains), tu nombre de dominio se compone del nombre de tu tenant, tu subdominio regional y `auth0.com`, separados por el símbolo de punto (`.`).
...
Durante este proceso, tu proveedor de identidad OIDC generará un identificador único para la API registrada, normalmente llamado **<Tooltip tip="ID de cliente: Valor de identificación asignado a tu recurso registrado por Auth0." cta="Ver glosario" href="/es/docs/glossary?term=Client+ID">ID de cliente</Tooltip>** o **ID de la aplicación**. Toma nota de este valor; lo necesitarás más adelante.

<div id="create-an-enterprise-connection-in-auth0">
  ## Crear una conexión empresarial en Auth0
</div>

Antes de configurar una conexión empresarial de OIDC en Auth0, asegúrate de tener el **ID de la aplicación (cliente)** y el **<Tooltip tip="Secreto de cliente: secreto que usa un cliente (aplicación) para autenticarse con el servidor de autorización; solo deben conocerlo el cliente y el servidor de autorización, y debe ser lo suficientemente aleatorio como para que no se pueda adivinar." cta="Ver glosario" href="/es/docs/glossary?term=Client+secret">secreto de cliente</Tooltip>** que se generan al configurar tu aplicación en el proveedor de OIDC.
...
| **Perfil de la conexión**                    | Para entender cómo cambiar el perfil de tu conexión, lee [Configurar PKCE y la asignación de claims para conexiones OIDC](/es/docs/authenticate/identity-providers/enterprise-identity-providers/configure-pkce-claim-mapping-for-oidc).                                                                                                                       |
...
| **Sincronizar perfiles de usuario usando SCIM**                           | Cuando está habilitado, Auth0 permite sincronizar los datos del perfil del usuario mediante SCIM. Para obtener más información, consulta <a href="/es/docs/authenticate/protocols/scim/configure-inbound-scim">configurar SCIM entrante</a>. |

5. En la vista **Experiencia de inicio de sesión**, configura cómo los usuarios inician sesión con esta conexión.

| Campo                         | Descripción                                                                                                                                                                                                                                                        |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Home Realm Discovery**      | Compara el dominio de correo electrónico de un usuario con los dominios del proveedor de identidad proporcionados. Para obtener más información, lee [Configurar Identifier First Authentication](/es/docs/authenticate/login/auth0-universal-login/identifier-first) |
...
Para usar la nueva conexión empresarial, primero debe [habilitar la conexión](/es/docs/authenticate/identity-providers/enterprise-identity-providers/enable-enterprise-connections) para sus aplicaciones de Auth0.
...
La conexión empresarial de <Tooltip tip="OpenID: Estándar abierto de autenticación que permite a las aplicaciones verificar la identidad de los usuarios sin recopilar ni almacenar información de inicio de sesión." cta="Ver glosario" href="/es/docs/glossary?term=OpenID">OpenID</Tooltip> Connect es útil para federarse con otro tenant de Auth0. Introduce la URL de tu tenant de Auth0 (por ejemplo, `https://<tenant>.us.auth0.com`) en el campo **Issuer** e introduce el ID de cliente de cualquier aplicación del tenant al que quieras federarte en el campo **ID de cliente**.
