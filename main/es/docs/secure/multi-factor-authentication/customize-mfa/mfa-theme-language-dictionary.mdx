---
description: Describe las opciones de configuración de la página alojada de MFA para personalizar las propiedades del tema de las páginas de MFA.
...
El `languageDictionary` se puede usar para reemplazar el texto en muchas partes del proceso de <Tooltip tip="Autenticación multifactor (MFA): Proceso de autenticación del usuario que utiliza un factor adicional al nombre de usuario y la contraseña, como un código enviado por SMS." cta="Ver glosario" href="/docs/es/glossary?term=MFA">MFA</Tooltip> en la experiencia de Classic Login. Para hacerlo, usa el siguiente formato:
