---
description: Explica cómo crear tus propios flujos de MFA con SMS o voz como factor de autenticación.
title: Registrar y verificar autenticadores de Email
---
...
Auth0 ofrece un flujo integrado de inscripción en <Tooltip tip="Autenticación multifactor (MFA): Proceso de autenticación de usuarios que utiliza un factor adicional al nombre de usuario y la contraseña, como un código por SMS." cta="Ver glosario" href="/docs/es/glossary?term=MFA">MFA</Tooltip> y autenticación con [Universal Login](/docs/es/authenticate/login/auth0-universal-login). Sin embargo, si quieres crear tu propia interfaz de usuario, puedes usar la [API de MFA](/docs/es/secure/multi-factor-authentication/multi-factor-authentication-developer-resources/mfa-api) para hacerlo.

Cuando el correo electrónico está habilitado como factor, todos los usuarios con correos electrónicos verificados podrán usarlo para completar la MFA.

<Card title="La disponibilidad varía según el plan de Auth0">
  Tanto tu implementación específica de login como tu plan de Auth0 o tu acuerdo personalizado afectan la disponibilidad de esta función. Para obtener más información, consulta [Precios](https://auth0.com/pricing).
</Card>
...
Antes de poder usar las API de MFA, deberás habilitar el tipo de concesión MFA para tu aplicación. Ve a [Auth0 Dashboard &gt; Applications &gt; Advanced Settings &gt; Grant Types](https://manage.auth0.com/#/applications) y selecciona **MFA**.

* [Configura Email](/docs/es/secure/multi-factor-authentication/multi-factor-authentication-factors/configure-email-notifications-for-mfa) como factor en el Dashboard o con la [Management API](https://auth0.com/docs/api/management/v2#!/Guardian/put_factors_by_name).

<div id="enroll-with-email">
  ## Registrar correo electrónico
</div>

Para permitir que los usuarios registren direcciones de correo electrónico además del correo electrónico verificado de su identidad principal, debes completar los siguientes pasos.

<div id="get-mfa-token">
  ### Obtener un token de MFA
</div>

Según cuándo actives la inscripción, puedes obtener un <Tooltip tip="Access Token: Credencial de autorización, en forma de una cadena opaca o JWT, utilizada para acceder a una API." cta="Ver glosario" href="/docs/es/glossary?term=access+token">token de acceso</Tooltip> para usar la API de MFA de diferentes maneras:
...
Haz una solicitud `POST` al endpoint de MFA Associate para registrar el autenticador del usuario. El bearer token que requiere este endpoint es el token de MFA obtenido en el paso anterior.
...
Si obtienes un error de `User is already enrolled`, el usuario ya tiene un factor de MFA inscrito. Antes de asociar otro factor con el usuario, debes solicitarle una verificación con el factor existente.

Si esta es la primera vez que el usuario asocia un autenticador, verás que la respuesta incluye `recovery_codes`. Los códigos de recuperación se usan para acceder a la cuenta del usuario en caso de que pierda el acceso a la cuenta o al dispositivo que usa para la autenticación de segundo factor. Son códigos de un solo uso y, cuando es necesario, se generan otros nuevos.

<div id="confirm-email-enrollment">
  ### Confirmar la inscripción por correo electrónico
</div>

El usuario debería recibir un correo electrónico con el código de 6 dígitos, que puede proporcionar a la aplicación.

Para completar la inscripción, realiza una solicitud `POST` al endpoint [**`/oath/token`**](https://auth0.com/docs/api/authentication#get-token). Incluye el `oob_code` devuelto en la respuesta anterior y el `binding_code` con el valor incluido en el mensaje de correo electrónico.
...
Puedes comprobar en cualquier momento si un autenticador se ha confirmado llamando al endpoint MFA Authenticators. Si el autenticador está confirmado, el valor que devuelve `active` es `true`.

Opcionalmente, puedes personalizar los correos electrónicos que reciben los usuarios. Consulta [Personalizar las plantillas de correo electrónico](/docs/es/customize/email/email-templates) para obtener más información.
...
Obtén un token de MFA siguiendo los pasos descritos en [Autenticación con la concesión de contraseña del propietario del recurso y MFA](/docs/es/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa).

<div id="retrieve-enrolled-authenticators">
  ### Obtener autenticadores inscritos
</div>

Para solicitar un desafío al usuario, necesitas el `authenticator_id` del factor que quieres verificar. Puedes listar todos los autenticadores inscritos mediante el endpoint MFA Authenticators:
...
Para activar un desafío por correo electrónico, envía una solicitud `POST` al endpoint MFA Desafío con el `authenticator_id` correspondiente y el `mfa_token`.
...
Tu aplicación debe solicitar al usuario el código y enviarlo como parte de la solicitud en el parámetro `binding_code` en la siguiente llamada al endpoint `oauth``/token`:
...
```json
{
  ...
  "mfa_requirements": {
    "challenge": [
      ...
      { "type": "phone" },
      { "type": "recovery-code" }
      { "type": "email"} //solo puede funcionar con desafío
    ]
  }
}
```

Usa el `mfa_token` para llamar al endpoint [`mfa/authenticator`](/docs/es/api/authentication/muti-factor-authentication/list-authenticators) y obtener una lista de todos los factores que el usuario tiene inscritos, y luego buscar el mismo tipo que admite tu aplicación. También necesitas obtener el `authenticator_type` correspondiente para emitir desafíos:
...
Personaliza aún más tu flujo de MFA con Auth0 Actions. Para obtener más información, consulta [Triggers de Actions: post-challenge - objeto de API](/docs/es/customize/actions/explore-triggers/password-reset-triggers/post-challenge-trigger/post-challenge-api-object).
