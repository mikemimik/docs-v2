...
Auth0 ofrece un flujo integrado de inscripción en <Tooltip tip="Autenticación multifactor (MFA): Proceso de autenticación de usuarios que utiliza un factor adicional al nombre de usuario y la contraseña, como un código por SMS." cta="Ver glosario" href="/es/docs/glossary?term=MFA">MFA</Tooltip> y autenticación con [Universal Login](/es/docs/authenticate/login/auth0-universal-login). Sin embargo, si quieres crear tu propia interfaz de usuario, puedes usar la [API de MFA](/es/docs/secure/multi-factor-authentication/multi-factor-authentication-developer-resources/mfa-api) para hacerlo.
...
* [Configura Email](/es/docs/secure/multi-factor-authentication/multi-factor-authentication-factors/configure-email-notifications-for-mfa) como factor en el Dashboard o con la [Management API](https://auth0.com/docs/api/management/v2#!/Guardian/put_factors_by_name).
...
Según cuándo actives la inscripción, puedes obtener un <Tooltip tip="Access Token: Credencial de autorización, en forma de una cadena opaca o JWT, utilizada para acceder a una API." cta="Ver glosario" href="/es/docs/glossary?term=access+token">token de acceso</Tooltip> para usar la API de MFA de diferentes maneras:

* Si realizas la inscripción durante la autenticación, consulta [Autenticar con la concesión de contraseña del propietario del recurso y MFA](/es/docs/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa).
...
Opcionalmente, puedes personalizar los correos electrónicos que reciben los usuarios. Consulta [Personalizar las plantillas de correo electrónico](/es/docs/customize/email/email-templates) para obtener más información.
...
Obtén un token de MFA siguiendo los pasos descritos en [Autenticación con la concesión de contraseña del propietario del recurso y MFA](/es/docs/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa).
...
Usa el `mfa_token` para llamar al endpoint [`mfa/authenticator`](/es/docs/api/authentication/muti-factor-authentication/list-authenticators) y obtener una lista de todos los factores que el usuario tiene inscritos, y luego buscar el mismo tipo que admite tu aplicación. También necesitas obtener el `authenticator_type` correspondiente para emitir desafíos:
...
* [Registrar y desafiar autenticadores de SMS y voz](/es/docs/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa/enroll-challenge-sms-voice-authenticators)