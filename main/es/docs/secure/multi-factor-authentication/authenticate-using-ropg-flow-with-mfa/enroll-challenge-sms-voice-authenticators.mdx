...
* [Configura phone como factor](/es/docs/secure/multi-factor-authentication/multi-factor-authentication-factors/configure-sms-voice-notifications-mfa) en el Dashboard o con la [Management API](https://auth0.com/docs/api/management/v2#!/Guardian/put_factors_by_name).
...
Según el momento en que inicies la inscripción, puedes obtener un <Tooltip tip="Token de acceso: credencial de autorización, en forma de una cadena opaca o JWT, utilizada para acceder a una API." cta="Ver glosario" href="/es/docs/glossary?term=access+token">token de acceso</Tooltip> para usar la API de MFA de distintas maneras:

* Si realizas la inscripción durante la autenticación, consulta [Autenticarse con la concesión de contraseña del propietario del recurso y MFA](/es/docs/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa).
...
Obtén un token de MFA siguiendo los pasos que se describen en [Autenticarse con la concesión de contraseña del propietario del recurso y MFA](/es/docs/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa).
...
Usa el `mfa_token` para llamar al endpoint [`mfa/authenticator`](/es/docs/api/authentication/muti-factor-authentication/list-authenticators) y obtener una lista de todos los factores que el usuario haya registrado, y encontrar uno del mismo tipo que admite tu aplicación.  También debes obtener el `authenticator_type` correspondiente para emitir desafíos:
...
Exija el desafío de MFA llamando al endpoint [`request/mfa/challenge`](/es/docs/api/authentication/muti-factor-authentication/request-mfa-challenge).

Personalice aún más el flujo de MFA con Auth0 Actions. Para obtener más información, lea [Triggers de Actions: post-challenge - objeto de API](/es/docs/customize/actions/explore-triggers/password-reset-triggers/post-challenge-trigger/post-challenge-api-object).
...
* [Configurar notificaciones de SMS y voz para MFA](/es/docs/secure/multi-factor-authentication/multi-factor-authentication-factors/configure-sms-voice-notifications-mfa)
...
* [Registrar y desafiar autenticadores de correo electrónico](/es/docs/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa/enroll-and-challenge-email-authenticators)
* [Registrar y desafiar autenticadores OTP](/es/docs/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa/enroll-and-challenge-otp-authenticators)
* [Registrar y desafiar autenticadores push](/es/docs/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa/enroll-and-challenge-push-authenticators)