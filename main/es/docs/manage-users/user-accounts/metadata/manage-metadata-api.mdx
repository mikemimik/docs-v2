...
title: Administra los metadatos con la Management API
---
...
Realiza la siguiente llamada `POST` al endpoint [`/post_users`](https://auth0.com/docs/api/management/v2#!/Users/post_users) de la <Tooltip tip="Management API: Un producto que permite a los clientes realizar tareas administrativas." cta="Ver glosario" href="/docs/es/glossary?term=Management+API">Management API</Tooltip> para crear el usuario y establecer los valores de las propiedades:
...
<div id="update-user-metadata">
  ### Actualizar los metadatos de usuario
</div>

Puedes actualizar los metadatos de usuario de un usuario mediante una solicitud `PATCH` al endpoint [`/patch_users_by_id`](https://auth0.com/docs/api/management/v2#!/Users/patch_users_by_id) de la Management API.

Supongamos que creaste un usuario con los siguientes valores de metadatos:
...
Para actualizar `user_metadata` y agregar la dirección del domicilio del usuario como una propiedad de segundo nivel:
...
Harías la siguiente solicitud `PATCH`:
...
<Warning>
  Cuando envías una llamada `PATCH` en la que estableces el valor de una propiedad en `null` (por ejemplo, `{user_metadata: {color: null}}`), Auth0 **elimina** la propiedad o el valor de la base de datos. Además, si aplicas un parche a los metadatos con un objeto vacío, los metadatos se eliminan por completo.
</Warning>

<div id="merge-user-metadata">
  ### Combinar metadatos de usuario
</div>
...
Por lo tanto, la llamada correspondiente a la API mediante `PATCH` sería:
...
<div id="delete-user-metadata">
  ### Eliminar los metadatos de usuario
</div>
...
Si actualizas los metadatos con un objeto vacío, se eliminan por completo. Por ejemplo, si envías este cuerpo, se elimina todo lo que hay en `app_metadata`:
...
Se puede incluir un objeto `clientMetadata` al crear una nueva aplicación a través del endpoint de aplicaciones `POST /api/v2/`.
...
Las claves de los metadatos del cliente se pueden eliminar mediante una solicitud PATCH, como se describe arriba en &quot;Actualizar app&#95;metadata&quot;, pero proporcionando `null` como valor de la clave. Este comportamiento coincide con el de las propiedades `user_metadata` y `app_metadata` en el endpoint `PATCH` [/api/v2/users/{id}](https://auth0.com/docs/api/management/v2#!/Users/patch_users_by_id).
