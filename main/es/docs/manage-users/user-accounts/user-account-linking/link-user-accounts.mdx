...
* <Tooltip tip="token de ID: Credencial pensada para el propio cliente, en lugar de para acceder a un recurso." cta="Ver glosario" href="/es/docs/glossary?term=ID+token">token de identidad</Tooltip> de la cuenta secundaria, firmado con RS256 e incluido con un claim `aud` que identifique al cliente y coincida con el valor del claim `azp` del token de acceso solicitante.
...
Puedes usar la biblioteca Auth0.js para realizar la vinculación de cuentas del lado del cliente. Consulta [Referencia de Auth0.js &gt; Administración de usuarios](/es/docs/libraries/auth0js#user-management) para obtener más información.
