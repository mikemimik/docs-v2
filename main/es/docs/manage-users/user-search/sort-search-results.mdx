---
description: Aprende a ordenar los resultados de búsqueda pasando un valor `field:order` en el parámetro sort.
...
Para ordenar los resultados de búsqueda de usuarios, pasa un valor `field:order` al parámetro `sort` al realizar tu solicitud. `field` es el nombre del campo por el que se ordenará, mientras que `order` puede establecerse en `1` para orden ascendente y en `-1` para orden descendente.
