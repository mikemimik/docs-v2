...
export interface ScreenMembersOnResetPasswordMfaPhoneChallenge extends ScreenMembers {
   ...
   * @property {string} data.phoneNumber - Le numéro de téléphone (souvent masqué pour des raisons de sécurité) associé à ce challenge MFA lors de la réinitialisation du mot de passe.
   */
  data: {
    /** Le numéro de téléphone (souvent masqué) associé à ce challenge MFA. */
    phoneNumber: string;
  } | null;
}
```
