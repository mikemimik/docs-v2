...
description: "Décrit toutes les propriétés et méthodes disponibles pour personnaliser l’écran `reset-password-mfa-webauthn-platform-challenge` dans Universal Login."
---

La classe ResetPasswordMfaWebAuthnPlatformChallenge implémente la fonctionnalité de l’écran `reset-password-mfa-webauthn-platform-challenge`. Cet écran invite l’utilisateur à vérifier son identité à l’aide d’un authentificateur de plateforme (comme Touch ID ou Windows Hello) dans le cadre de l’étape MFA d’un processus de réinitialisation du mot de passe.
...
<ParamField body="continueWithPasskey" type="Promise<void>">
  Cette méthode lance le challenge du WebAuthn authentificateur de plateforme et soumet l’identifiant résultant pour terminer l’étape MFA pendant le processus de réinitialisation du mot de passe. Si le navigateur génère une `DOMException` (comme une annulation par l’utilisateur), interceptez l’erreur et appelez `reportBrowserError` pour en aviser Auth0.
