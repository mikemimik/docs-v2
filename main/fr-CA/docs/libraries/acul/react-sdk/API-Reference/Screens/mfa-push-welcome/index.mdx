...
description: "Décrit tous les hooks et les méthodes disponibles pour personnaliser l’écran `mfa-push-welcome` de Universal Login."
---

L’écran `mfa-push-welcome` présente à l’utilisateur l’AMF par notification push et l’invite à l’activer ou à choisir un autre authentificateur.
...
* L’importation racine vous permet de charger tous les écrans à partir d’un seul bundle, ce qui est pratique si vous souhaitez qu’une build unifiée prenne en charge tous les écrans possibles.
...
Des hooks propres à l’écran qui fournissent un accès en lecture seule aux données de contexte d’Auth0 sur l’écran `mfa-push-welcome`. Importez-les depuis `@auth0/auth0-acul-react/mfa-push-welcome`.
...
<ParamField body="useBranding" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/BrandingMembers">BrandingMembers</a></span>}>
  Ce hook fournit la configuration de l’image de marque, comme le logo, les couleurs et les paramètres de thème affichés sur l’écran `mfa-push-welcome`.
...
<ParamField body="useUser" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/UserMembers">UserMembers</a></span>}>
  Ce hook fournit des renseignements sur l’utilisateur actif, notamment son `username`, son `email` et les authentication methods disponibles.
...
<ParamField body="enroll" type="Promise<void>">
  Cette méthode lance l’inscription aux notifications push pour l’utilisateur.
...
<ParamField body="pickAuthenticator" type="Promise<void>">
  Cette méthode redirige vers l’écran de sélection de l’authentificateur afin que l’utilisateur puisse choisir une autre méthode MFA.
