...
description: "Décrit tous les hooks et toutes les méthodes disponibles pour personnaliser l’écran `brute-force-protection-unblock-failure` de Universal Login."
---

L’écran `brute-force-protection-unblock-failure` s’affiche lorsqu’une tentative de déblocage du compte d’un utilisateur échoue et l’informe que la demande de déblocage n’a pas pu être traitée.
...
Des hooks propres à l’écran qui fournissent un accès en lecture seule aux données de contexte Auth0 sur l’écran `brute-force-protection-unblock-failure`. Importez-les depuis `@auth0/auth0-acul-react/brute-force-protection-unblock-failure`.
...
<ParamField body="useOrganization" type={<span>() =&gt; <a href="/docs/fr-CA/libraries/acul/react-sdk/API-Reference/Types/interfaces/OrganizationMembers">OrganizationMembers</a></span>}>
  Ce hook fournit des renseignements sur l’Organization de l’utilisateur si le flux de déblocage est limité à l’Organization. Il retourne `null` lorsqu’aucun contexte d’Organization n’est présent.
