...
export interface ScreenMembersOnMfaWebAuthnEnrollmentSuccess extends ScreenMembers {
  /**
   * Données propres à l'écran contenant les détails de l'inscription WebAuthn réussie.
  ...
  * Déclaration de type : `{ nickname: string; webauthnType: [WebAuthnType]; }`
  ...
  <Expandable title="propriétés">
    <ParamField body="nickname" type="string">
      Le surnom que l’utilisateur (ou le système) a attribué à l’authentificateur WebAuthn qui vient d’être inscrit.
      Cela aide l’utilisateur à reconnaître cet authentificateur plus tard.
