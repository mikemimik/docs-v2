...
Pour configurer AWS pour le <Tooltip tip="authentification unique (SSO) : service qui, après qu’un utilisateur s’est connecté à une application, connecte automatiquement cet utilisateur à d’autres applications." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=SSO">SSO</Tooltip>, vous devez suivre les étapes suivantes :
...
1. Configurez un fournisseur d’identité externe dans AWS à l’aide du guide AWS [Connect to your External Identity Provider](https://docs.aws.amazon.com/singlesignon/latest/userguide/manage-your-identity-source-idp.html), avec une seule modification.
...
Pour utiliser le fournisseur, vous devez créer un rôle IAM en utilisant ce fournisseur dans la politique de confiance du rôle.
...
2. Sur la page suivante, vous devrez sélectionner le type d’entité de confiance. Sélectionnez **SAML 2.0 Federation**.
3. Lorsque vous y êtes invité, définissez le fournisseur créé ci-dessus comme **SAML provider**. Sélectionnez **Allow programmatic and AWS Management Console access**. Cliquez sur **Next** pour continuer.
4. Sur la page **Attach Permission Policies**, sélectionnez les politiques appropriées à associer au rôle. Elles définissent les permissions dont disposeront les utilisateurs à qui ce rôle est attribué dans AWS. Par exemple, pour accorder à vos utilisateurs un accès en lecture seule à IAM, filtrez et sélectionnez la politique `IAMReadOnlyAccess`. Une fois terminé, cliquez sur **Next Step**.
...
6. Sur la page **Review**, définissez le **Role Name** et vérifiez vos paramètres. Fournissez des valeurs pour les paramètres suivants :

   | Champ                | Description                                 |
   | -------------------- | ------------------------------------------- |
   | **Role name**        | Nom descriptif de votre rôle.               |
   | **Role description** | Description de l’utilisation de votre rôle. |
7. Passez en revue les informations **Trusted entities** et **Policies**, puis cliquez sur **Create Role**.
...
Les **rôles AWS** indiqués seront associés à une **politique IAM** qui définit le type d’accès autorisé à une ressource, y compris aux consoles AWS. Pour en savoir plus sur les rôles et les politiques, consultez [Creating IAM Roles](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_create.html).

1. Pour associer un rôle AWS à un utilisateur, créez une [règle](/fr-CA/docs/customize/rules) :
   ...
   1. Votre règle peut obtenir ces deux valeurs de plusieurs façons. Vous pouvez les récupérer dans la console IAM en sélectionnant, dans la barre latérale gauche, les éléments que vous avez créés dans AWS aux étapes précédentes. Le fournisseur d’identité et le rôle que vous avez créés ont tous deux un ARN que vous pouvez copier si vous les sélectionnez dans la console.
...
Par exemple, supposons que vous avez des groupes Active Directory avec la structure suivante :
...
Votre règle pourrait donc se présenter ainsi :
...
Pour prolonger le délai avant l’expiration de la session AWS (3600 secondes), utilisez une [règle personnalisée](/fr-CA/docs/customize/rules). Votre règle définit l’[attribut **SessionDuration**](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_providers_create_saml_assertions.html), ce qui modifie la durée de la session.
...
3. Dans l’onglet **Usage**, repérez l’**Identity Provider Login URL**. Vous devriez être redirigé vers la page de login du compte Auth0. Si la connexion réussit, vous serez redirigé vers AWS.
