...
La configuration Office 365 par défaut comprend Active Directory et les services DirSync/Azure AD Sync, qui synchronisent et provisionnent les utilisateurs AD présents dans votre Azure AD pour le <Tooltip tip="Authentification unique (SSO) : service qui, après qu’un utilisateur se connecte à une application, le connecte automatiquement à d’autres applications." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=SSO">SSO</Tooltip>. Dans cette configuration, Auth0 est le <Tooltip tip="Authentification unique (SSO) : service qui, après qu’un utilisateur se connecte à une application, le connecte automatiquement à d’autres applications." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=identity+provider">fournisseur d’identité</Tooltip> et nous fournissons l’authentification unique (SSO) à ces utilisateurs.

Mais que faire si vous souhaitez permettre à des sous-traitants, à des partenaires ou même à des clients d’accéder à votre environnement Office 365 (p. ex., SharePoint) ? Dans ce cas, l’approche par défaut n’est pas idéale, car ces utilisateurs devraient être créés dans votre environnement AD. Vous devez plutôt provisionner de façon personnalisée des utilisateurs Azure AD à l’aide des [Règles Auth0](/fr-CA/docs/customize/rules).

Le provisionnement personnalisé vous permet de créer des utilisateurs dans Azure AD (et donc, dans les faits, dans Office 365) au moment même où ils se connectent à partir de toute connexion disponible dans Auth0. (Dans ce cas, votre règle prend en charge la tâche de DirSync pour tout type de connexion avec lequel DirSync ne fonctionnerait pas.) Cette configuration vous permet d’offrir diverses options de connexion (notamment Facebook, LinkedIn et Google Workspace) pour votre environnement Office 365.
...
* [Configurer Office 365](https://marketplace.auth0.com/integrations/office-365-sso) : enregistrer un <Tooltip tip="Domaine personnalisé : domaine tiers portant un nom distinctif ou personnalisé." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=custom+domain">domaine personnalisé</Tooltip> et configurer Office 365 comme une application tierce dans Auth0.
...
5. Remplissez le formulaire :
   ...
   3. Saisissez une URL de connexion. Vous pouvez entrer n’importe quelle URL valide ; elle ne sera pas réellement utilisée.
6. L’application nouvellement créée apparaîtra dans la liste **App registrations**. Sélectionnez-la.
7. Dans le panneau **Settings** (Microsoft appelle ces sections des « blades »), choisissez **Keys**.
8. Saisissez une **Description** (comme `Auth0 Provision`) et choisissez une **Duration** pour la nouvelle clé. Si vous choisissez d’émettre une clé non permanente, notez sa date d’expiration et créez un rappel pour la remplacer avant qu’elle n’expire.
9. Cliquez pour enregistrer la clé, puis copiez la **App Key**. Cette clé ne sera affichée qu’une seule fois et elle est nécessaire pour la règle Auth0.
10. Choisissez **Required permissions**, puis cliquez sur **Add** dans le nouveau panneau.
11. Sélectionnez l’API **Microsoft Graph**, puis cochez `Read and write directory data` sous **Application Permissions**.
12. De retour dans **Required permissions**, cliquez sur le bouton **Grant Permissions**, puis sur **Yes** pour accorder les permissions demandées.
...
Le nom d’utilisateur est généré par la fonction `createAzureADUser`, qui, par défaut, crée un nom d’utilisateur au format `auth0-c3fb6eec-3afd-4d52-8e0a-d9f357dd19ab@fabrikamcorp.be`. Vous pouvez le modifier à votre guise; assurez-vous simplement que cette valeur est unique pour tous vos utilisateurs.

Assurez-vous de définir les bonnes valeurs pour `AUTH0_OFFICE365_CLIENT_ID`, `AAD_CUSTOM_DOMAIN`, `AAD_DOMAIN`, `AAD_APPLICATION_ID` et `AAD_APPLICATION_API_KEY` dans votre objet de configuration afin que ces valeurs soient accessibles dans le code de votre règle. Pour en savoir plus, consultez [Store Configuration for Rules](/fr-CA/docs/customize/rules/configuration).
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Ce code illustre le processus de provisionnement d’un nouvel utilisateur, mais vous pouvez aussi l’adapter pour synchroniser les métadonnées des utilisateurs existants.
</Callout>
...
La façon la plus simple pour vos utilisateurs externes de s’authentifier consiste à utiliser une connexion initiée par le fournisseur d’identité.
...
Cela leur affichera la page de connexion Auth0, après quoi ils seront redirigés vers Office 365. Il sera important d’expliquer aux utilisateurs externes qu’il s’agit de la seule façon pour eux de s’authentifier, puisque la page de connexion Office 365 ne prend pas en charge Home Realm Discover pour ces utilisateurs. Cela signifie aussi que, lorsqu’ils essaieront d’ouvrir un lien, ils devront d’abord passer par le smart link avant de pouvoir accéder au lien qu’ils tentaient d’ouvrir.
...
Certaines mises en œuvre peuvent nécessiter une liaison profonde (par exemple vers SharePoint Online). Dans ce cas, il faut créer un lien intelligent qui commence sur la page de connexion d’Office 365 :

```http wrap lines
https://login.microsoftonline.com/login.srf?wa=wsignin1.0&whr={yourCustomDomain}&wreply={deepLink}
```

Le premier paramètre, `{yourCustomDomain}`, doit être le domaine que vous avez configuré dans Azure AD pour l’<Tooltip tip="Service qui, après qu’un utilisateur se connecte à une application, le connecte automatiquement à d’autres applications." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=Single+Sign-on+%28SSO%29">authentification unique (SSO)</Tooltip> (par exemple, `travel0.com`). En l’indiquant comme valeur de `whr`, Azure AD saura qu’il doit rediriger l’utilisateur vers Auth0 au lieu d’afficher la page de connexion.
