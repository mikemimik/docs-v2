...
<ResponseField name="reason" type="string" post={["optional"]}>
  Une explication claire du rejet de l’octroi du jeton d’accès.
</ResponseField>
...
<ResponseField name="key" type="string">
  Nom de la revendication (notez qu’il peut être nécessaire qu’il s’agisse d’une URL pleinement qualifiée).
</ResponseField>
...
Supprime l’enregistrement correspondant à une valeur mise en cache associée à la
...
Stocke ou met à jour une valeur de type string dans le cache à la clé indiquée.
...
valeurs `ttl` ou `expires_at` indiquées. Si aucune durée de vie n’est précisée, une durée de vie
...
[Accès anticipé] Ajoute un scope à l’ensemble des scopes cibles. Les scopes ajoutés sont ensuite croisés avec le client grant une fois toutes les
Actions terminées. Les scopes qui ne figurent pas dans le grant
sont retirés silencieusement du jeton d’accès final.
...
[Accès anticipé] Supprime un scope de l’ensemble cible de scopes.
...
[Early Access] Remplace l’ensemble complet des scopes cibles. Les nouvelles scopes sont mises en correspondance avec
le client grant une fois toutes les Actions terminées. Les scopes qui ne figurent pas dans le
grant sont supprimées discrètement du jeton d’accès final.
...
[Accès anticipé] Supprime toutes les scopes de l’ensemble des scopes cibles.
