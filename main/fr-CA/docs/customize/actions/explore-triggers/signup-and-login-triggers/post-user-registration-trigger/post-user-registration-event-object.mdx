...
title: "Actions Triggers : post-user-registration - objet Event"
        ...
        <ResponseField name="supplemental" type="dictionary" post={["optional"]}>
          ...
          <Expandable title="propriétés de supplemental">
            <ResponseField name="akamai" type="dictionary" post={["optional"]}>
              &#91;Early Access&#93; Évaluation supplémentaire des risques. Cette fonction est offerte uniquement si Akamai Account Protector est activé et qu’Akamai transmet les en-têtes pour la transaction.
                ...
                <ResponseField name="akamaiBot" type="object" post={["optional"]}>
                  ...
                  <Expandable title="propriétés de akamaiBot">
                    ...
                    <ResponseField name="botnetId" type="string" post={["optional"]}>
                      L’identifiant du botnet dans les résultats d’Akamai Bot Manager.
                    </ResponseField>
                  </Expandable>
                </ResponseField>
                ...
                <ResponseField name="akamaiUserRisk" type="dictionary" post={["optional"]}>
                  ...
                  <Expandable title="propriétés de akamaiUserRisk">
                    ...
                    <ResponseField name="requestid" type="string" post={["optional"]}>
                      L’identifiant de la requête de l’utilisateur.
                    </ResponseField>
...
<ResponseField name="event.request" type="dictionary" post={["optional"]}>
  ...
  <Expandable title="propriétés de la requête" defaultOpen>
    ...
    <ResponseField name="user_agent" type="string" post={["optional"]}>
      La valeur de l’en-tête `User-Agent` reçue au moment où la transaction est lancée.
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="event.security_context" type="object" post={["optional"]}>
  Un objet contenant des signatures d’empreinte. Il n’est disponible que si le client utilise Cloudflare. L’empreinte JA3/JA4 peut être nulle ou vide dans certains cas. Le cas le plus courant concerne les requêtes HTTP, puisque JA3 et JA4 sont calculées au niveau de TLS. Elle peut aussi être vide parce que le Worker envoie des requêtes dans la même zone ou vers une zone qui n’est pas relayée par proxy (ou vers un tiers).
...
<ResponseField name="event.tenant" type="object">
  Informations sur le tenant associé à la transaction en cours.
...
<ResponseField name="event.transaction" type="dictionary" post={["optional"]}>
  ...
  <Expandable title="propriétés de la transaction" defaultOpen>
    ...
    <ResponseField name="requested_scopes" type="array of strings">
      Les scopes demandés (le cas échéant) au début de ce flux d&#39;authentification.
    </ResponseField>
...
<ResponseField name="event.user" type="dictionary">
  ...
  <Expandable title="propriétés de l’utilisateur" defaultOpen>
    <ResponseField name="app_metadata" type="dictionary">
      Champs personnalisés stockant des renseignements sur l’utilisateur qui influencent ses droits d’accès, comme son forfait d’assistance, ses rôles de sécurité ou ses groupes de contrôle d’accès.
    </ResponseField>
    ...
    <ResponseField name="user_metadata" type="dictionary">
      Champs personnalisés stockant des renseignements sur l’utilisateur qui n’ont pas d’incidence sur ses droits d’accès, comme son adresse professionnelle, son adresse personnelle ou ses préférences.
    </ResponseField>
