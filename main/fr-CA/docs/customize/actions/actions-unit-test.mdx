...
Le package **Actions NPM (`@auth0/actions`)** permet d’utiliser TypeScript dans des projets externes, ce qui aide les développeurs à suivre les pratiques exemplaires et à améliorer leurs tests unitaires grâce aux définitions TypeScript.
...
Suivez les directives d’installation et d’importation décrites ici : [Fonctionnement d’Actions NPM](/fr-CA/docs/customize/actions/actions-npm#how-it-works).
...
Les tests unitaires peuvent être exécutés dans un environnement local, avec le contrôle de version ou dans un processus CI/CD, ce qui améliore l’assurance qualité globale et les vérifications avant que des changements n’affectent les tenants Auth0.
...
Les exemples suivants servent à valider une série de scénarios en simulant les objets nécessaires.

<Note>
  Les exemples utilisent **Jest** (`https://www.npmjs.com/package/jest`), mais vous pouvez utiliser n’importe quelle bibliothèque de test.
</Note>
...
<Tabs>
  ...
  <Tab title="TypeScript">
    ...
    Dans votre fichier `jest.config.js`, définissez les préréglages d’environnement de `Jest` :
...
<div id="pre-user-registration-access-control-and-user-metadata-setup">
  ## Configuration du contrôle d’accès et des métadonnées utilisateur de Pre-User Registration
</div>

L’Action d’exemple suivante vérifie si l’adresse courriel de l’utilisateur appartient à un domaine interdit et appelle `api.access.deny()` si c’est le cas. Sinon, elle vérifie si le nom complet a été fourni au moyen des champs supplémentaires de Custom Prompts, puis l’enregistre dans le `user_metadata` du profil utilisateur; autrement, elle envoie une erreur de validation à Universal Login.
...
L’exemple d’Action suivant tente d’envoyer un message au moyen d’une requête HTTP à un service externe, en gérant une éventuelle erreur de requête afin d’interrompre la notification. Il utilise des secrets pour l’URL du service externe et la clé API d’autorisation.
...
<Tabs>
  <Tab title="Javascript">
    ...
    /**
    * Handler à exécuter pendant l’envoi d’une notification par courriel
    *
