---
mode: wide
description: Décrit comment utiliser les modules d’Action Auth0
sidebarTitle: Créez votre premier module d’Action
title: Créez votre premier module d’Action
---
...
Utilisez le Quickstart pour rédiger un module d’Action et l’utiliser dans une Action afin d’envoyer des logs vers un service externe.

<Steps>
  <Step title="Créer un module d’action" titleSize="h3" stepNumber={1}>
    Pour utiliser un module d’action dans une Action donnée, vous devez d’abord créer le module d’action, puis l’ajouter et l’utiliser dans une Action.
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        <Frame>![Actions Modules - Nouveau module d’action](/docs/images/customize/actions/modules/actions-modules-new-module.png)</Frame>
      </Tab>
  ...
  <Step title="Ajouter un Secret" titleSize="h3" stepNumber={2}>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        Stockons l’URL du service externe comme Secret.
    ...
    **Étape supplémentaire :** Répétez les étapes pour stocker `API_KEY` comme Secret, que vous enverrez dans les en-têtes au service externe avec chaque requête.
    ...
    <Warning>
      Chaque **Secret de module d’action** est indépendant des autres **Actions Modules** et des **Actions Secrets**.
    </Warning>

    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      Utilisez les **Secrets** dans le module d’action en tapant `actions.secrets.[secret_key]`.
    </Callout>
  </Step>
  ...
  <Step title="Ajouter une logique personnalisée" titleSize="h3" stepNumber={3}>
    ...
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      ...
      * [Limitations](/fr-CA/docs/customize/actions/limitations#actions-modules)
    </Callout>
  </Step>
  ...
  <Step title="Enregistrer le brouillon" titleSize="h3" stepNumber={4}>
    ...
    Cette opération enregistre votre Action Module sans publier de nouvelle version; elle n’a donc aucun effet sur les Actions qui l’utilisent.
  </Step>
  ...
  <Step title="Publier le module d’action" titleSize="h3" stepNumber={5}>
    ...
    <Warning>
      Chaque **Action** doit faire explicitement référence à la **version d’Action Module**, ce qui évite les mises à niveau automatiques susceptibles d’entraîner des erreurs inattendues.
    </Warning>
  </Step>
  ...
  <Step title="Ajouter l’Action Module à une Action" titleSize="h3" stepNumber={6}>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        1. Accédez à [Auth0 Dashboard &gt; Actions &gt; Bibliothèque](https://manage.auth0.com/#/actions/library), puis sélectionnez une **Action** en particulier.
  ...
  <Step title="Utiliser le module Action dans l’Action" titleSize="h3" stepNumber={7}>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        2. Ajoutez l’appel à `logger.sendLog` dans la **fonction Action**, à l’endroit où vous voulez envoyer les logs.

        ```javascript lines
        await logger.sendLog('logger_success', 'Your Action was able to use the Logger Action Module');
        ```

        3. Sélectionnez **Deploy** pour l’Action.
...
<Check>
  **Vérification**
  ...
  Vous devriez maintenant disposer d’un **module d’Action** entièrement fonctionnel, utilisé par une **Action**.
</Check>
...
<Accordion title="Dépendances NPM">
  Chaque **module d’Action** peut ajouter ou utiliser des **[dépendances NPM](https://www.npmjs.com/)**.
  ...
  <Tabs>
    ...
    <Tab title="Dashboard">
      ...
      Auth0 ajoute la dépendance à la liste des dépendances du module d’Action.
    </Tab>
  ...
  <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
    Utilisez les **dépendances** répertoriées dans le module d’Action en les important avec `require('[package-name]')`.
  </Callout>

  <Warning>
    Lorsque vous enregistrez cette Action, Auth0 résout la version la plus récente de votre dépendance et la remplace par un numéro de version précis afin d’éviter que de futures mises à jour du package ne compromettent votre Action.
