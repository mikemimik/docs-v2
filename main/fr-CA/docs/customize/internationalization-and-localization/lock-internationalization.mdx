...
Vous pouvez changer la langue de Lock à l’aide de l’option de configuration `language`. Le fichier de langue correspondant sera alors extrait du répertoire `i18n` de Lock.
...
Ensuite, définissez votre objet `options` et ajoutez l’option `language`. L’option `language` doit être une chaîne de caractères qui correspond au nom du fichier dans le répertoire `i18n` [de Lock](https://github.com/auth0/lock/tree/master/src/i18n). Puis, instanciez Lock.
...
Les données de traduction de Lock proviennent de fichiers de langue contenant des paires clé-valeur pour les différentes traductions. Pour certaines langues, il peut manquer certaines valeurs; dans ce cas, vous verrez l’avertissement suivant : `language does not have property <missing prop>`. Nous vous encourageons à soumettre une [pull request](https://github.com/auth0/lock/tree/master/src/i18n) afin d’ajouter ces valeurs manquantes. Sinon, vous pouvez définir les valeurs manquantes dans les `options` de Lock (voir ci-dessous).

<div id="replace-dictionary-terms">
  ## Remplacer des termes du dictionnaire
</div>

Vous pouvez aussi personnaliser certaines entrées précises de votre propre dictionnaire à l’aide de l’option `languageDictionary`. C’est utile si vous utilisez l’une des langues prises en charge, mais souhaitez modifier le libellé de quelques éléments précis. Par exemple, vous pourriez reformuler le `title` ou changer la façon dont d’autres libellés s’affichent pour l’utilisateur, tout en laissant inchangé le reste du texte du widget.
...
Pour voir des exemples de noms de propriétés `languageDictionary` disponibles et la façon de structurer un fichier de langue, consultez le [fichier dictionary anglais pour Lock](https://github.com/auth0/lock/blob/master/src/i18n/en.js).

L’option `languageBaseUrl`, qui accepte une valeur de type string (une URL), remplace l’URL source des traductions fournies par Auth0. Par défaut, elle utilise l’URL du CDN d’Auth0 `https://cdn.auth0.com`, puisque c’est là que sont stockées les traductions fournies. En indiquant une autre valeur, vous pouvez utiliser votre propre source pour les traductions, selon les besoins de vos applications. Votre source de langue doit être un fichier Javascript.