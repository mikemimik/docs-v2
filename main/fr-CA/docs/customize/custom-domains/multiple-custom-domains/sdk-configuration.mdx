...
title: Configuration des SDK Auth0 pour plusieurs domaines personnalisés
---

import {AuthCodeBlock} from "/snippets/AuthCodeBlock.jsx";
import {AuthCodeGroup} from "/snippets/AuthCodeGroup.jsx";

Si vous utilisez [plusieurs domaines personnalisés](/fr-CA/docs/customize/custom-domains/multiple-custom-domains), vous devez configurer vos SDK Auth0 pour qu’ils utilisent le domaine personnalisé approprié pour l’authentification. Ce guide explique la configuration des SDK pour différentes plateformes et divers scénarios.
...
Tous les SDK Auth0 exigent un paramètre `domain` qui indique quel domaine Auth0 utiliser pour l’authentification. Si vous utilisez des domaines personnalisés, définissez ce paramètre sur votre domaine personnalisé plutôt que sur votre domaine Auth0 canonique.
...
Lorsque vous utilisez un domaine personnalisé, le claim `iss` (émetteur) des jetons aura pour valeur votre domaine personnalisé :
...
Vous devez configurer la validation de votre jeton pour accepter votre domaine personnalisé comme émetteur valide.
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Lorsque vous utilisez MCD, le client est responsable de fournir et de valider tous les domaines personnalisés. Lorsque vous configurez des SDKs pour résoudre les domaines personnalisés du tenant à l’aide des fonctions de résolution de domaine, vous devez vous assurer que tous les domaines résolus sont fiables. Une mauvaise configuration du résolveur de domaine peut permettre de contourner l’authentification au niveau de la partie de confiance ou rendre l’application vulnérable à une falsification de requêtes côté serveur. Si vos domaines et vos serveurs proxy ne sont pas configurés correctement, cela peut entraîner des vulnérabilités de sécurité critiques dont Okta ne peut être tenue responsable.
</Callout>
...
Pour les applications monopages utilisant le [Auth0 SPA SDK](https://github.com/auth0/auth0-spa-js) :
...
<AuthCodeGroup>
  ...
  // Déterminer le domaine selon le contexte de l’application
  function getAuth0Domain() {
    const hostname = window.location.hostname;

    // Associer les domaines d’application aux domaines personnalisés Auth0
...
<AuthCodeGroup>
  ...
  const auth0 = new Auth0Client({
    domain: ({ headers }) => {
      // Associe les hôtes d’application aux domaines personnalisés Auth0
  ...
  // Associe les identifiants de tenant aux domaines personnalisés (depuis la base de données, le cache, etc.)
  async function resolveTenantDomain(host: string): Promise<string> {
    // Extrait l’identifiant du tenant à partir du sous-domaine
    const subdomain = host.split('.')[0];

    // Recherche le domaine personnalisé pour ce tenant (exemple : dans la base de données)
...
* **Mise en cache des instances** : Le SDK met automatiquement en cache les instances `Auth0Client` par domaine au moyen d’un cache LRU de taille limitée (maximum de 100 entrées) pour améliorer les performances.
* **Isolation des sessions** : Les sessions créées par l’intermédiaire d’un domaine personnalisé sont isolées à ce domaine et ne peuvent pas être utilisées de façon interchangeable avec les sessions d’un autre domaine.
* **Paramètre d’URL** : Le paramètre `url` du résolveur est `undefined` dans les Server Components et les Server Actions; il est seulement disponible dans le middleware ou les routes API.
* **Réglage du cache de découverte** : Configurez la mise en cache des métadonnées OIDC avec l’option `discoveryCache` :
  ```typescript
  const auth0 = new Auth0Client({
    // ... autre configuration
    discoveryCache: {
      ttl: 600,      // Met en cache pendant 10 minutes (par défaut)
...
Pour les scénarios multidomaines :
...
app.use((req, res, next) => {
  ...
  // Associer le tenant au domaine personnalisé
  const domainMap = {
...
À l’aide de [Auth0.swift](https://github.com/auth0/Auth0.swift) :
...
Pour sélectionner le domaine de façon dynamique :
...
Pour la prise en charge multidomaine :
...
class AuthManager(private val context: Context) {
    ...
    private fun getAuth0Domain(): String {
        // Récupérer depuis les préférences partagées ou la configuration de l'application
...
Les SDK de gestion permettent d’interagir avec l’Auth0 Management API. Lorsque vous utilisez des domaines personnalisés, vous devrez peut-être inclure l’en-tête `auth0-custom-domain` ou utiliser le [domaine par défaut](/fr-CA/docs/customize/custom-domains/multiple-custom-domains/default-domain).
...
// Lister les utilisateurs (endpoint sur liste blanche - l'en-tête est envoyé automatiquement)
const users = await management.users.getAll();

// Remplacement par requête (a priorité sur le paramètre global)
const reqOptions = {
...
<div id="python-management-sdk">
  ### SDK Python de gestion
</div>

```python
from auth0.management import ManagementClient, CustomDomainHeader

# Domaine personnalisé global (envoyé sur toutes les requêtes en liste blanche)
...
# Lister les utilisateurs (endpoint en liste blanche - l'en-tête est envoyé automatiquement)
...
// Niveau client : applique automatiquement l'en-tête de domaine personnalisé aux endpoints sur liste blanche
...
Lorsque vous utilisez des domaines personnalisés, mettez à jour la validation du jeton pour accepter le domaine personnalisé comme émetteur.

<div id="nodejs-express">
  ### Node.js (Express)
</div>

À l’aide de [express-jwt](https://github.com/auth0/express-jwt) ou de [jose](https://github.com/panva/jose) :
...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      <td>Échec de récupération du JWKS</td>
    ...
    <tr>
      <td>Erreurs cross-origin (CORS)</td>
      <td>Le domaine personnalisé n’est pas inclus dans les origines autorisées</td>
...
2. **Validez plusieurs émetteurs** : si vous utilisez plusieurs domaines personnalisés, configurez la validation des jetons pour qu’ils soient tous acceptés comme émetteurs valides
...
5. **Surveillez les émetteurs des jetons** : consignez et surveillez le claim `iss` dans les jetons afin de vous assurer que le bon domaine personnalisé est utilisé
6. **Documentez les correspondances entre domaines** : tenez une documentation claire indiquant quelles applications utilisent quels domaines personnalisés
7. **Gérez les échecs adéquatement** : prévoyez une gestion appropriée des erreurs en cas d’échec de l’authentification
8. **Mettez JWKS en cache** : mettez les données JWKS en cache pour améliorer les performances et réduire les requêtes
