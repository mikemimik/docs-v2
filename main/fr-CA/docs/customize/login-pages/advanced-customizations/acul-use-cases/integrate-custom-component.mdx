...
2. Lancez le serveur de développement local d’ACUL pour modifier et prévisualiser les mises à jour de votre écran.
...
4. Suivez les instructions du CLI pour créer le fichier `components.json`, qui contiendra la configuration de votre projet, ainsi que le fichier `src/lib/utils.ts`.
...
6. Intégrez le composant :
   ...
   b. Importez les composants InputOTP et remplacez le champ de saisie actuel. Vous devez également gérer l’état du code OTP et utiliser le bon hook du SDK.
...
9. Suivez les étapes pour générer vos ressources locales, démarrer le serveur de développement local et mettre à jour la configuration ACUL dans votre tenant.
