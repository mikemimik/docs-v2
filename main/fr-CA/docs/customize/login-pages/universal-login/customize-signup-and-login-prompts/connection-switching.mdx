---
description: Découvrez comment configurer la commutation flexible des connexions avec Universal Login
title: Configurer la commutation flexible des connexions
---
...
Le basculement flexible entre les connexions est une fonctionnalité facultative qui permet aux utilisateurs de choisir leur méthode d’authentification au moment de se connecter à une application. Lorsqu’elle est mise en place, l’écran de connexion de votre application offre aux utilisateurs la possibilité de s’authentifier soit à l’aide d’identifiants de base de données traditionnels, soit au moyen d’une connexion <Tooltip tip="" cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=passwordless">sans mot de passe</Tooltip>. Les utilisateurs qui sélectionnent une [connexion sans mot de passe](/fr-CA/docs/authenticate/passwordless) reçoivent un mot de passe à usage unique (OTP) par courriel ou par SMS, qu’ils peuvent utiliser pour se connecter à votre application.

Le basculement flexible entre les connexions utilise des [prompts Universal Login personnalisés](/fr-CA/docs/customize/login-pages/universal-login/customize-signup-and-login-prompts) afin d’offrir aux utilisateurs une expérience d’authentification plus autonome.

<div id="before-you-begin">
  ## Avant de commencer
</div>

Avant de pouvoir utiliser le basculement flexible entre les connexions, assurez-vous que les exigences suivantes sont respectées :
...
* Activez les éléments suivants pour votre application :
  ...
  * [authentification sans mot de passe par courriel ou SMS](/fr-CA/docs/authenticate/login/auth0-universal-login/passwordless-login/email-or-sms)

<div id="implement-flexible-connection-switching">
  ## Mettre en œuvre le basculement flexible entre les connexions
</div>

Pour mettre en œuvre cette fonctionnalité, utilisez l’[Auth0 Management API](https://auth0.com/docs/api/management/v2) pour configurer des partials personnalisés pour les prompts d’inscription et de connexion. Les partials sont des extraits de code personnalisé insérés à un point d’entrée dans un écran de prompt, comme l’écran de connexion. Pour en savoir plus, consultez [Personnaliser les prompts d’inscription et de connexion](/fr-CA/docs/customize/login-pages/universal-login/customize-signup-and-login-prompts).
...
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- |
| `state`      | Affiche la valeur `state` de la page actuelle, qui est opaque et utilisée à des fins de sécurité.<br /><br />Pour en savoir plus sur les renseignements de l’écran actuel, consultez [Personnaliser les modèles de page Universal Login](/fr-CA/docs/customize/login-pages/universal-login/customize-templates#current-screen-information). | `<input type='hidden' name='state' value='{{state}}'>`  |
| `connection` | Nom et type de la connexion.<br /><br />Pour les connexions Passwordless, la valeur est soit `email`, soit `sms`.                                                                                                                                                                                                                     | `<input type='hidden' name='connection' value='email'>` |
...
Vous pouvez configurer le prompt `signup-password` en utilisant le point de terminaison [Set partials for a prompt](https://auth0.com/docs/api/management/v2/prompts/put-partials) :
...
Par conséquent, un bouton **Envoyer un code sécurisé par courriel** est ajouté à l’écran `signup-password`. Lorsqu’un utilisateur sélectionne ce bouton, il soumet le corps du formulaire contenant le paramètre d’état de connexion et le nom de la connexion souhaitée.

<div id="configure-login-prompts">
  ### Configurer les prompts de connexion
</div>

Pour de meilleurs résultats, nous vous recommandons de configurer le prompt de connexion pour les connexions avec mot de passe et les connexions sans mot de passe.

Vous pouvez configurer le prompt `login-password` à l’aide du point de terminaison [Set partials for a prompt](https://auth0.com/docs/api/management/v2/prompts/put-partials) :
...
Par conséquent, un **bouton Send a secure code by email** est ajouté à l’écran `login-password`. Lorsqu’un utilisateur sélectionne ce bouton, le formulaire soumis contient le paramètre d’état de connexion ainsi que le nom de la connexion souhaitée.
