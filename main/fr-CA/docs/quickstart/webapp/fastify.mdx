...
description: Ce guide explique comment intégrer Auth0, ajouter l’authentification et afficher les informations du profil utilisateur dans une application web Fastify à l’aide de l’Auth0 Fastify SDK.
mode: wide
---
...
<Accordion title="Utiliser l’IA pour intégrer Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **Demandez ensuite à votre assistant IA :**
  ...
  Votre assistant IA créera automatiquement votre application Auth0, récupérera les identifiants, installera `@auth0/auth0-fastify`, configurera le plugin et créera toutes les routes et vues nécessaires. [Documentation complète sur Agent Skills →](/fr-CA/docs/quickstart/agent-skills)
</Accordion>
...
Ce Quickstart montre comment ajouter l’authentification Auth0 à une application Fastify. Vous créerez une application web sécurisée avec des fonctionnalités de login, de logout et de profil utilisateur à l’aide de l’Auth0 Fastify SDK.

<Steps>
  <Step title="Créer un nouveau projet" stepNumber={1}>
    Créez un nouveau répertoire pour votre application Fastify, puis initialisez un projet Node.js.
  ...
  <Step title="Installez le SDK Fastify d’Auth0" stepNumber={2}>
    ...
    <Tabs>
      ...
      <Tab title="CLI">
        ...
        <Note>
          ...
          2. crée une application Auth0 de type **Regular Web Application** configurée pour `http://localhost:3000`;
          3. génère un fichier `.env` contenant `AUTH0_DOMAIN`, `AUTH0_CLIENT_ID`, `AUTH0_CLIENT_SECRET`, `SESSION_SECRET` et `APP_BASE_URL`
        </Note>
      </Tab>
      ...
      <Tab title="Dashboard">
        ...
        3. Saisissez un nom pour votre application (par ex. &quot;My Fastify App&quot;)
        ...
        <Warning>
          Remplacez `YOUR_AUTH0_DOMAIN` par le domaine de votre tenant Auth0 (par ex. `dev-abc123.us.auth0.com`), `YOUR_CLIENT_ID` par le **Client ID** de votre application et `YOUR_CLIENT_SECRET` par le **Client Secret** de votre application dans le dashboard.
        </Warning>
  ...
  <Step title="Configurer le plugin Auth0" stepNumber={4}>
    ...
    * Enregistre le moteur de rendu pour afficher les gabarits HTML
    * Configure le plugin Auth0 avec vos identifiants
    * Crée automatiquement des routes pour `/auth/login`, `/auth/logout` et `/auth/callback`
    * Gère les sessions à l’aide de cookies chiffrés
  </Step>

  <Step title="Créer des gabarits de vue" stepNumber={5}>
    Créez un répertoire `views` et ajoutez des fichiers de modèle :
    ...
    <body>
      <div class="container">
        ...
        <h3>Objet utilisateur complet</h3>
        <div class="user-data">
  ...
  <Step title="Créer des routes" stepNumber={6}>
    ...
    * La route d’accueil vérifie le status d’authentication et le transmet au gabarit
    * La route de profil utilise un `preHandler` pour protéger cette route
...
<Check>
  **Vérification**
  ...
  3. allez à &quot;/profile&quot; - vous voyez les renseignements de votre profil
...
<Accordion title="Chemins de routes personnalisés">
  ...
  <Note>
    N’oubliez pas de mettre à jour vos **Allowed Callback URLs** dans l’Auth0 Dashboard pour y inclure votre URL de rappel personnalisée.
  </Note>
</Accordion>
...
<Accordion title="Utilisation de TypeScript">
  Convertissez votre projet à TypeScript pour bénéficier d’une meilleure vérification des types :
...
<AccordionGroup>
  <Accordion title="Problèmes courants et solutions">
    ...
    **Problème :** Le `state` ne correspond pas entre la `request` d’`authentication` et le `rappel`.

    **Solutions :**

    1. Assurez-vous que les `cookie` sont correctement définis (et non bloqués par le navigateur)
    2. Vérifiez que l’`URL de rappel` correspond exactement dans Auth0 Dashboard (y compris `/auth/callback`)
    ...
    **Solution :**
    ...
    ```javascript
    // Débogage : consigner les valeurs de configuration (à retirer en production !)
...
* **[Authentification d’API Fastify](/fr-CA/docs/quickstart/backend/fastify)** - Protégez vos points de terminaison d’API grâce à la validation JWT
...
* **[Configurer MFA](https://auth0.com/docs/secure/multi-factor-authentication)** - Ajoutez l’authentification multifacteur
