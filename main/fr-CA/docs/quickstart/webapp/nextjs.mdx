---
title: Ajoutez la fonctionnalité de connexion à votre application Next.js
...
<Accordion title="Utiliser l’IA pour intégrer Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **Ensuite, demandez à votre assistant IA :**
  ...
  Votre assistant IA créera automatiquement votre application Auth0, récupérera les informations d’identification, installera `@auth0/nextjs-auth0`, créera les routes d’API et configurera les variables d’environnement. [Documentation complète sur Agent Skills →](/fr-CA/docs/quickstart/agent-skills)
</Accordion>
...
Ce Quickstart montre comment intégrer l’authentification Auth0 à une application Next.js 16. Vous créerez une application Web complète avec rendu côté serveur, une fonctionnalité de connexion sécurisée et des routes protégées grâce au SDK Auth0 Next.js.
...
<Steps>
  <Step title="Créer un nouveau projet" stepNumber={1}>
    Créez un nouveau projet Next.js dans le cadre de ce Quickstart
  ...
  <Step title="Installez le SDK Auth0 pour Next.js" stepNumber={2}>
    ...
    <Tabs>
      ...
      <Tab title="CLI">
        ...
        <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
          ...
          3. Générer un fichier `.env` avec `AUTH0_DOMAIN`, `AUTH0_CLIENT_ID`, `AUTH0_CLIENT_SECRET`, `AUTH0_SECRET` et `APP_BASE_URL`
        </Callout>
      </Tab>
      ...
      <Tab title="Dashboard">
        ...
        **Générez un AUTH0&#95;SECRET sécurisé :**
        ...
        Copiez le résultat et remplacez `YOUR_LONG_RANDOM_SECRET_HERE` dans `.env.local`. Il doit contenir exactement 64 caractères hexadécimaux.
        ...
        <Warning>
          **Critique :** Si vous modifiez `AUTH0_SECRET` après avoir démarré l’application, vous devez effacer les cookies de votre navigateur pour localhost. Les anciens session cookies chiffrés avec le secret précédent provoqueront une erreur `JWEDecryptionFailed`.
        </Warning>
        ...
        **Allowed Web Origins:**
        ...
        <Info>
          **Allowed Callback URLs** est une mesure de sécurité essentielle pour garantir que les utilisateurs sont redirigés de façon sécuritaire vers votre application après l’authentication. Sans URL correspondante, le login process échouera et les utilisateurs verront une page d’erreur Auth0 au lieu d’accéder à votre application.
  ...
  <Step title="Créez la configuration d’Auth0" stepNumber={5}>
  ...
  <Step title="Ajouter un proxy" stepNumber={6}>
    Ajoutez le code du proxy à `src/proxy.ts` :
    ...
    <Info>
      ...
      * `/auth/access-token` - Route du access token
      * `/auth/backchannel-logout` - Route de logout backchannel
    </Info>
  </Step>

  <Step title="Créer les composants Login, Logout et Profil" stepNumber={7}>
  ...
  <Step title="Mettez à jour votre page principale" stepNumber={8}>
  ...
  <Step title="Mettre à jour le layout à l’aide d’Auth0Provider" stepNumber={9}>
    Mettez à jour `src/app/layout.tsx` pour charger la police Inter et englober votre application dans `Auth0Provider` :
    ...
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      Avec la v4, Auth0Provider est facultatif. Vous n’en avez besoin que si vous voulez transmettre un utilisateur initial lors du rendu côté serveur pour qu’il soit accessible au hook useUser().
    </Callout>
  </Step>
  ...
  <Step title="Lancez votre application" stepNumber={11}>
    ...
    <Info>
      Votre application sera accessible à l’adresse http://localhost:3000. Le SDK Auth0 v4 configure automatiquement les routes d’authentification à `/auth/*` (et non `/api/auth/*`, comme dans la v3).
...
<Accordion title="Erreur JWEDecryptionFailed">
  Si vous voyez l’erreur `JWEDecryptionFailed: decryption operation failed`, c’est généralement dû à un `AUTH0_SECRET` invalide ou à un ancien cookie de session chiffré avec un autre secret.

  **Solution :**

  1. Générez un nouveau secret comme suit :
  ...
  3. **Supprimez les cookies de votre navigateur** pour `localhost:3000` :
     ...
     * Safari : menu Develop → Show Web Inspector → onglet Storage → Cookies → Tout supprimer
  ...
  Le secret doit contenir exactement 32 octets (64 caractères hexadécimaux). Cette erreur se produit lorsque l’application tente de déchiffrer un cookie de session existant qui a été chiffré avec un autre secret.
</Accordion>

<Accordion title="Erreur 404 sur /auth/login">
  Si le fait de cliquer sur le bouton de connexion vous mène à une page 404, vérifiez les points suivants :
...
<Accordion title="Erreurs Module Not Found">
  ...
  4. **Vérification des imports :** Assurez-vous d’utiliser `@/components/*` (et non `~/components/*`)
</Accordion>
...
<Accordion title="Changements importants de v4">
  Ce Quickstart utilise **Auth0 Next.js SDK v4**, qui apporte des changements importants par rapport à v3 :

  * **Aucun gestionnaire de route dynamique n’est nécessaire** - Les routes d’authentification sont montées automatiquement par le proxy
  * **Configuration du client simplifiée** - `new Auth0Client()` lit automatiquement les variables d’environnement
  * **Nouveaux chemins de routes** - Les routes se trouvent à `/auth/*` au lieu de `/api/auth/*`
  * **Proxy requis** - Toutes les fonctionnalités d’authentification passent par `proxy.ts`
  * **Utilisez les balises `<a>`** - La navigation doit utiliser `<a href="/auth/login">` au lieu de boutons avec onClick
  ...
  | `/auth/profile`            | Récupérer le profil utilisateur  |
  | `/auth/access-token`       | Récupérer le jeton d’accès       |
  ...
  <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
    ...
    2. Le proxy est correctement configuré avec le motif `matcher` illustré à l’étape 6
...
<Accordion title="Authentification côté serveur">
  Auth0 Next.js SDK v4 prend en charge les architectures App Router et Pages Router. Voici quelques modèles courants côté serveur :
...
<Accordion title="Protection des routes API">
  Pour protéger les routes API, utilisez la méthode `withApiAuthRequired` :
...
<Accordion title="Utiliser des jetons Auth0 avec des backends tiers">
  ...
  **Côté client :**
  ...
  export function ConvexClientProvider({ children }: { children: React.ReactNode }) {
    return (
      <ConvexProviderWithAuth0
        client={convex}
        useAuth0={() => ({
          // Récupère le jeton d’accès depuis votre route API Next.js
  ...
  <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
    Assurez-vous que votre application Auth0 est configurée avec une audience d’API si votre backend l’exige. Vous pouvez la définir dans l’Auth0 Dashboard sous Applications → APIs, ou ajouter `AUTH0_AUDIENCE` à votre fichier `.env.local` et configurer le SDK en conséquence.
  </Callout>
