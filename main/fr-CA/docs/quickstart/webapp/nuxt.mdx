...
description:  Ce guide explique comment intégrer Auth0, ajouter l’authentification et afficher les renseignements du profil utilisateur dans une application monopage (SPA) utilisant Nuxt.js, à l’aide du SDK Auth0 pour Nuxt.js.
mode: wide
---
...
<Callout icon="pencil" color="#FFC107" iconType="solid">
  Ce Quickstart est actuellement en version **Beta**. Nous serions ravis d’avoir vos commentaires !
</Callout>
...
<Accordion title="Utiliser l’IA pour intégrer Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **Demandez ensuite à votre assistant IA :**
  ...
  Votre assistant IA créera automatiquement votre application Auth0, récupérera les identifiants, installera `@auth0/auth0-nuxt`, configurera le module et mettra en place vos routes. [Documentation complète sur Agent Skills →](/fr-CA/docs/quickstart/agent-skills)
</Accordion>
...
<Note>
  ...
  **Compatibilité des versions de Nuxt :** Ce quickstart fonctionne avec **Nuxt 3.x** tel quel. Pour **Nuxt 4.x**, assurez-vous d’utiliser Nuxt 4.2+.
</Note>
...
Ce Quickstart montre comment ajouter l’authentification Auth0 à une application Nuxt.js. Vous créerez une application monopage sécurisée avec des fonctionnalités de login, de logout et de profil utilisateur à l’aide du SDK Nuxt d’Auth0.

<Steps>
  <Step title="Créer un nouveau projet" stepNumber={1}>
    Créez un nouveau projet Nuxt pour ce Quickstart
  ...
  <Step title="Installer le SDK Auth0 pour Nuxt" stepNumber={2}>
    ```shellscript
  ...
  <Step title="Configurez votre Auth0 App" stepNumber={3}>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        6. Générez un secret de session en exécutant : `openssl rand -hex 64` et utilisez-le pour `NUXT_AUTH0_SESSION_SECRET`
        ...
        **Origines Web autorisées :**
        ...
        <Info>
          Les **URL de rappel autorisées** constituent une mesure de sécurité essentielle pour garantir que les utilisateurs reviennent à votre application en toute sécurité après l’authentification. Sans URL correspondante, le processus de connexion échouera et une page d’erreur Auth0 empêchera les utilisateurs d’accéder à votre application.

          Les **URL de déconnexion autorisées** sont essentielles pour offrir une expérience utilisateur fluide au moment de la déconnexion. Sans URL correspondante, les utilisateurs ne seront pas redirigés vers votre application après la déconnexion et se retrouveront plutôt sur une page Auth0 générique.

          Les **origines Web autorisées** sont essentielles à l’authentification silencieuse. Sans elles, les utilisateurs seront déconnectés lorsqu’ils actualiseront la page ou reviendront plus tard dans votre application.
        </Info>
      </Tab>
    </Tabs>
  </Step>
