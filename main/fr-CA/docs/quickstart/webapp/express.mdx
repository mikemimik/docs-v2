---
mode: wide
description: Ce guide montre comment intégrer Auth0, ajouter l’authentification et afficher les informations du profil utilisateur dans une application web Express.js à l’aide du SDK express-openid-connect.
...
<Accordion title="Utiliser l’IA pour intégrer Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **Ensuite, demandez à votre assistant IA :**
  ...
  Votre assistant IA créera automatiquement votre application Auth0, récupérera les identifiants, installera `express-openid-connect`, configurera le middleware et mettra en place vos routes. [Documentation complète sur Agent Skills →](/fr-CA/docs/quickstart/agent-skills)
</Accordion>
...
<div id="2-install-the-auth0-express-sdk">
  ## 2. Installez le SDK Express d’Auth0
</div>

Installez `express-openid-connect` ainsi qu’Express et dotenv pour la gestion des variables d’environnement.
...
Mettez à jour votre `package.json` pour y ajouter les scripts `start` :
...
* `auth0Logout: true` fait en sorte que les utilisateurs soient aussi déconnectés d’Auth0, en plus de votre application
* Le middleware fournit automatiquement les routes `/login`, `/logout` et `/callback`
* La session de l’utilisateur est stockée dans un cookie chiffré

***

<div id="5-create-login-logout-and-profile-routes">
  ## 5. Créez les routes login, logout et profil
</div>

Ajoutez maintenant des routes pour afficher des liens de login/logout et une page de profil protégée.
...
<Check>
  **Checkpoint**
  ...
  3. Accédez à « /profile » - vous voyez les informations de votre profil
...
<AccordionGroup>
  <Accordion title="Protéger certaines routes avec requiresAuth()">
    Utilisez le middleware `requiresAuth()` pour protéger les routes individuelles qui nécessitent une authentification :
  ...
  <Accordion title="Utilisation de l’autorisation fondée sur les revendications">
    ...
    <Note>
      Les claims comme `role` doivent être ajoutées à vos jetons à l’aide d’Auth0 Rules ou d’Actions. [En savoir plus sur l’ajout de claims personnalisées](https://auth0.com/docs/customize/actions/flows-and-triggers/login-flow/add-user-roles-to-id-and-access-tokens).
    </Note>
  </Accordion>

  <Accordion title="Stockage de sessions personnalisé (Redis)">
    Pour les environnements de production ou si vous exécutez plusieurs instances du serveur, utilisez un magasin de sessions personnalisé :
    ...
    * Les données de session dépassent la taille maximale d’un cookie (~4 KB)
    * Besoin de conserver les sessions entre les redémarrages du serveur
    * Utilisation de la déconnexion par back-channel
  </Accordion>
  ...
  <Accordion title="Gestion des erreurs">
    ...
    app.use(auth({
      ...
      clientID: process.env.CLIENT_ID,
      issuerBaseURL: process.env.ISSUER_BASE_URL,
      errorOnRequiredAuth: true,  // Retourner 401 au lieu de rediriger pour les routes API
    }));
...
<AccordionGroup>
  <Accordion title="Problèmes courants et solutions">
    ...
    **Solutions :**
    ...
    3. Vérifiez que la callback URL correspond exactement à celle configurée dans l&#39;Auth0 Dashboard

    ### `req.oidc` n&#39;est pas défini
    ...
    **Solution :** Assurez-vous que `app.use(auth(config))` est appelé avant toute route qui accède à `req.oidc` :
    ...
    ### Session trop volumineuse / erreurs de témoin

    **Problème :** Les données de session de l&#39;utilisateur dépassent la taille maximale des témoins.
    ...
    **Solution :**
    ...
    1. Accédez à votre Auth0 Dashboard → Applications → Votre application → Settings
    ...
    **Solution :**
    ...
    ```javascript
    // Débogage : consignez les valeurs de configuration (à retirer en Production !)
...
* **[Ajouter l’autorisation](https://auth0.com/docs/manage-users/access-control/rbac)** - Mettre en place un contrôle d’accès basé sur les rôles
* **[Appeler des API protégées](https://auth0.com/docs/secure/tokens/access-tokens/get-access-tokens)** - Utiliser des jetons d’accès pour appeler vos API côté serveur
* **[Personnaliser Universal Login](https://auth0.com/docs/customize/universal-login-pages)** - Personnaliser votre expérience de connexion
* **[Ajouter des connexions sociales](https://auth0.com/docs/connections/social)** - Activer Google, GitHub et d’autres options de connexion sociale
* **[Implement MFA](https://auth0.com/docs/secure/multi-factor-authentication)** - Ajouter l’authentification multifacteur
