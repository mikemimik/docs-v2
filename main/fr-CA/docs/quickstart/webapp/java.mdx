---
mode: wide
description: "Ce Quickstart montre comment ajouter l’authentification Auth0 à une application Java Servlet. Vous créerez une application web sécurisée avec des fonctionnalités de login, de logout et de profil utilisateur à l’aide du SDK Auth0 Java MVC Commons."
sidebarTitle: Java
title: Ajouter le login à votre application Java Servlet
---
...
<Note>
  ...
  * **Compte Auth0** : [Inscrivez-vous gratuitement](https://auth0.com/signup) si vous n’en avez pas encore un
</Note>
...
<Accordion title="Utiliser l’IA pour intégrer Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **Ensuite, demandez à votre assistant IA :**
  ...
  Votre assistant IA créera automatiquement votre application Auth0, récupérera les identifiants, ajoutera la dépendance du SDK Auth0 Java MVC Commons, configurera votre `web.xml` et implémentera les flux de connexion et de déconnexion avec des filtres de servlet. [Documentation complète d’Agent Skills →](/fr-CA/docs/quickstart/agent-skills)
</Accordion>
...
<Steps>
  <Step title="Créer un nouveau projet Java Web" stepNumber={1}>
  ...
  <Step title="Configurer le SDK Auth0" stepNumber={4}>
    Configurez votre application servlet pour utiliser le SDK Auth0 en configurant `web.xml` avec les identifiants Auth0 générés ci-dessus.

    Créez ou mettez à jour `src/main/webapp/WEB-INF/web.xml` et remplacez les valeurs fictives par les paramètres réels de votre application Auth0 :
  ...
  <Step title="Créer des composants d’authentification et des filtres" stepNumber={5}>
  ...
  <Step title="Créer des pages d’interface utilisateur" stepNumber={6}>
    Créez les pages JSP et les fichiers HTML pour votre application.
  ...
  <Step title="Compiler et exécuter votre application" stepNumber={7}>
    Vous êtes maintenant prêt à générer et à exécuter votre application.
    ...
    <Tabs>
      <Tab title="Maven">
        ...
        Déployez l’application sur Tomcat :
...
<Accordion title="Améliorez votre application">
  Maintenant que l’authentification de base fonctionne, envisagez les améliorations suivantes :

  * **Informations du profil utilisateur** : Décodez l’ID token pour afficher les informations de l’utilisateur
  * **Appels d’API** : Utilisez le jeton d’accès pour appeler la Management API d’Auth0 ou vos propres API
  * **Accès basé sur les rôles** : Mettez en œuvre l’autorisation à l’aide des rôles et des permissions d’Auth0
  * **Single Sign-On** : Configurez le SSO dans plusieurs applications
</Accordion>

<Accordion title="Ressources supplémentaires">
  * [Documentation d’Auth0 Java MVC Commons](https://github.com/auth0/auth0-java-mvc-common) - [Dashboard
    d’Auth0](https://manage.auth0.com/) - Gérez vos applications Auth0 -
    [Dépôt d’exemples Auth0 Servlet](https://github.com/auth0-samples/auth0-servlet-sample) - Exemples
    complets - [Documentation du SDK Java d’Auth0](https://auth0.com/docs/libraries/auth0-java) - Utilisation
    avancée du SDK
</Accordion>
...
<Accordion title="Dépannage">
  ...
  * Vérifiez que HTTPS est utilisé dans les environnements de Production
  ...
  Pour obtenir du soutien supplémentaire, consultez la [Auth0 Community](https://community.auth0.com/).
</Accordion>