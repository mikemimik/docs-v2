...
<Accordion title="Utiliser l’IA pour intégrer Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **Ensuite, demandez à votre assistant IA :**
  ...
  Votre assistant IA créera automatiquement votre Auth0 API, récupérera les identifiants, installera le SDK Auth0 ASP.NET Core Authentication API, configurera l’authentification JWT Bearer et implémentera des points de terminaison d’API protégés. [Documentation complète sur les Agent Skills →](/fr-CA/docs/quickstart/agent-skills)
</Accordion>

<Note>
  **Prérequis :** Avant de commencer, assurez-vous d’avoir installé ce qui suit :

  * **[.NET 8.0 SDK](https://dotnet.microsoft.com/download/dotnet/8.0)** ou une version plus récente
  * L’IDE de votre choix (Visual Studio 2022, VS Code ou Rider)

  **Compatibilité des versions .NET :** Ce Quickstart fonctionne avec **.NET 8.0** et les versions ultérieures.
</Note>
...
Ce guide de démarrage rapide montre comment ajouter l’authentification JWT d’Auth0 à une API Web ASP.NET Core. Vous créerez une API sécurisée avec des points de terminaison protégés à l’aide du SDK d’API ASP.NET Core d’Auth0.

<Steps>
  <Step title="Créer un nouveau projet" stepNumber={1}>
    Créez un projet Web API ASP.NET Core pour ce Quickstart
  ...
  <Step title="Configurez votre API Auth0" stepNumber={3}>
    ...
    <Tabs>
      <Tab title="CLI">
        Exécutez la commande shell suivante dans le répertoire racine de votre projet pour créer une API Auth0 et mettre à jour votre fichier `appsettings.json` :
      ...
      <Tab title="Dashboard">
        Avant de commencer, ajoutez la configuration d’Auth0 dans votre fichier `appsettings.json`
        ...
        <Info>
          ...
          L’**Audience** (API Identifier) est un identifiant unique pour votre API et peut être n’importe quel URI valide. Il n’est pas nécessaire qu’il s’agisse d’une URL accessible publiquement.
        </Info>
      </Tab>
    </Tabs>
  </Step>
  ...
  <Step title="Créer des points de terminaison publics et protégés" stepNumber={5}>
    Ajoutez des endpoints pour tester l’authentification. Ajoutez le code suivant dans `Program.cs` avant `app.Run()` :
  ...
  <Step title="Lancez votre API" stepNumber={6}>
    ...
    Votre API s’exécute maintenant à l’adresse `https://localhost:7190` (ou une adresse semblable — consultez la sortie de la console pour connaître l’URL exacte).
  </Step>
</Steps>
...
<Check>
  **Vérification**
  ...
  Vous devriez maintenant avoir une API protégée par Auth0 entièrement fonctionnelle sur votre [localhost](https://localhost:7190/)
</Check>
...
<Accordion title="Appeler des API protégées">
  Testez vos points de terminaison protégés avec un access token.

  **1. Obtenez un access token** d’Auth0 à l’aide du flux Client Credentials :
...
<Accordion title="Protéger des routes avec une autorisation basée sur des scopes">
  Protégez les points de terminaison en fonction de scopes précis dans l’access token.
  ...
  **3. Appliquez les politiques aux points de terminaison :**
  ...
  Lorsque vous demandez un access token, incluez le scope requis :
...
<Accordion title="Utilisation de DPoP pour Enhanced Security">
  ...
  **Modes DPoP :**
  ...
  Accepter à la fois les jetons DPoP et les jetons Bearer (par défaut) :
  ...
  .WithDPoP(dpopOptions =>
  {
      ...
      dpopOptions.Leeway = 30;      // Tolérance de 30 secondes au décalage d’horloge
  });
  ```
...
<Accordion title="Mise en œuvre de politiques d’autorisation personnalisées">
...
<Accordion title="Accès aux renseignements de l’utilisateur">
  Extrayez les renseignements de l’utilisateur à partir du jeton d’authentification.
...
<CardGroup cols={3}>
  ...
  <Card title="Exemples de code" icon="code" href="https://github.com/auth0/aspnetcore-api/blob/master/EXAMPLES.md">
    Exemples de code et approches détaillés
  </Card>
...
<AccordionGroup>
  <Accordion title="401 Unauthorized - audience invalide">
    **Problème :** La validation du jeton échoue en raison d’une non-correspondance de l’audience.

    **Solution :** Assurez-vous que `Audience` dans `appsettings.json` correspond exactement à l’identifier de votre Auth0 API. La claim `audience` du jeton doit correspondre à cette valeur.
  ...
  <Accordion title="401 Unauthorized - issuer invalide">
    **Problème :** La validation du jeton échoue avec une erreur d’issuer.

    **Solution :** Vérifiez que votre Domain est correct et qu’il n’inclut pas `https://`. La library construit automatiquement l’autorité sous la forme `https://{Domain}`.
  ...
  <Accordion title="Problèmes d’ordre des middleware">
    **Problème :** L’authentication ne fonctionne pas malgré une configuration correcte.
  ...
  <Accordion title="Les scopes ne fonctionnent pas dans les politiques d’autorisation">
    ...
    **Solution :** Assurez-vous que votre access token inclut les scopes requis. Lorsque vous demandez un token, précisez les scopes :
    ...
    Vérifiez également que les scopes sont définis dans les paramètres de votre Auth0 API (Dashboard → APIs → Your API → Permissions).
  </Accordion>
</AccordionGroup>
...
Une application exemple complète qui présente toutes les fonctionnalités est disponible dans le dépôt du SDK.
