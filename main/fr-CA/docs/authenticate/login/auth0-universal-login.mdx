...
Avec Universal Login, vous n’avez aucun travail d’intégration à effectuer pour prendre en charge différentes méthodes d’authentification. Vous pouvez commencer par un flux d’authentification simple, puis ajouter des fonctionnalités supplémentaires, comme la connexion sans mot de passe, la connexion sociale ou l’<Tooltip tip="Authentification multifacteur (MFA) : processus d’authentification de l’utilisateur qui utilise un facteur en plus du nom d’utilisateur et du mot de passe, comme un code envoyé par SMS." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=multi-factor+authentication">authentification multifacteur</Tooltip> (MFA), afin de personnaliser une expérience de connexion sécurisée et conviviale.

La configuration d’Universal Login est dynamique et ne nécessite aucune modification au niveau de l’application, puisque les pages Web hébergées sur le <Tooltip tip="Serveur d’authentification : serveur qui confirme ou refuse l’identité d’un utilisateur." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=authentication+server">serveur d’authentification</Tooltip> centralisé d’Auth0 gèrent entièrement le flux de connexion. De plus, vous n’avez pas besoin de mettre à jour manuellement votre code pour que les applications profitent des améliorations apportées par Auth0 à Universal Login.
...
Pour en savoir plus sur les différences entre Universal Login et Embedded Login, consultez [Universal Login centralisé ou Embedded Login](/fr-CA/docs/authenticate/login/universal-vs-embedded-login).

<div id="configure-universal-login">
  ## Configurer Universal Login
</div>

Dans votre Auth0 Dashboard, vous pouvez choisir l’expérience à utiliser pour les pages de connexion par défaut dans [Branding &gt; Universal Login &gt; Options avancées](https://manage.auth0.com/#/universal-login/customizations-new).

Auth0 offre deux expériences de connexion hébergées :

* **Universal Login** propose une expérience simplifiée pour les utilisateurs et ne nécessite pas l’utilisation de JavaScript pour la personnalisation.
* **Classic Login** utilise des contrôles JavaScript pour chaque page du flux de connexion.

À l’heure actuelle, les efforts de développement d’Auth0 sont axés sur Universal Login, et Classic Login ne reçoit plus de mises à jour. À moins que votre cas d’usage particulier n’exige l’expérience Classic, il est recommandé d’adopter Universal Login.
...
Auth0 Universal Login active ces normes par défaut. Voici les éléments qui pourraient avoir une incidence sur les personnalisations apportées par les clients :
...
* Les Forms utilisent la correction automatique.
...
* Les adresses courriel sont validées côté client et suivent les mêmes règles de mise en forme que la validation côté serveur.
* Les libellés HTML sont accessibles et associés à leurs champs de saisie respectifs.
* Les exigences de complexité du mot de passe et les messages d’erreur affichés dans le formulaire sont annoncés par les lecteurs d’écran.
* Tous les écrans du <Tooltip tip="Flux d’autorisation : mécanisme d’autorisation (ou workflow) précisé dans le cadre OAuth 2.0." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=authorization+flow">flux d’autorisation</Tooltip> ont un titre de page clair et distinct.
...
Après avoir activé Universal Login, vous pouvez personnaliser votre page de connexion directement dans l’Auth0 Dashboard. Plus précisément, vous pouvez personnaliser les gabarits de page et les thèmes, les écrans d’inscription et de connexion, ainsi que d’autres éléments de texte. Pour en savoir plus, consultez Customize Universal Login.

Si vous décidez d’utiliser Classic Login, vous pouvez personnaliser les options de base liées à l’image de marque dans l’Auth0 Dashboard. Vous pouvez ensuite configurer des personnalisations avancées à l’aide du SDK utilisé pour créer votre flux de connexion. Pour en savoir plus, consultez [Customize Classic Login](/fr-CA/docs/customize/login-pages/classic-login).

<div id="implement-universal-login">
  ## Mettre en œuvre Universal Login
</div>

Après avoir activé Universal Login ou Classic Login dans votre tenant, suivez les étapes ci-dessous pour mettre en place l’expérience :

1. [Enregistrez votre application](/fr-CA/docs/get-started/auth0-overview/create-applications) auprès d’Auth0.
2. Configurez votre application pour appeler le [point de terminaison Login](https://auth0.com/docs/api/authentication#login) de l’Authentication API afin de déclencher le flux de connexion et de traiter la réponse. Vous pouvez effectuer cette configuration directement ou utiliser l’un des [SDKs](/fr-CA/docs/libraries) d’Auth0.
3. Créez une connexion [Database](/fr-CA/docs/get-started/applications/set-up-database-connections), [Enterprise](/fr-CA/docs/authenticate/enterprise-connections) ou [Social](/fr-CA/docs/authenticate/identity-providers/social-identity-providers), puis activez-la pour votre application.
...
* `state`
  ...
  * Pour mieux comprendre pourquoi cette valeur est requise, consultez [Prévenir les attaques et rediriger les utilisateurs à l’aide des paramètres d’état OAuth 2.0](/fr-CA/docs/secure/attack-protection/state-parameters).

Vous pouvez également inclure le paramètre `connection` pour demander aux utilisateurs de s’authentifier avec la connexion spécifiée.
...
Pour en savoir plus sur la manière de configurer Universal Login pour votre application, consultez les [guides Quickstart](/fr-CA/docs/quickstarts). Choisissez l’approche qui convient le mieux à vos technologies et suivez le Quickstart pour obtenir des instructions pas à pas pour la mise en œuvre.