...
Pour utiliser le <Tooltip tip="Authentification unique (SSO) : service qui, après qu’un utilisateur se connecte à une application, le connecte automatiquement à d’autres applications." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=Single+Sign-On">Single Sign-On</Tooltip> (SSO) Native to Web, configurez vos applications natives et Web (application monopage ou application Web traditionnelle) pour créer et gérer des sessions avec la <Tooltip tip="Management API : produit qui permet aux clients d’effectuer des tâches administratives." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=Management+API">Management API</Tooltip> d’Auth0.

Vous avez besoin d’un [jeton d’accès](/fr-CA/docs/secure/tokens/access-tokens/management-api-access-tokens) pour utiliser la Management API ou Auth0 CLI.
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  ...
  La prise en charge de Native to Web SSO est offerte dans les outils suivants : [Auth0 Deploy CLI](/fr-CA/docs/deploy-monitor/deploy-cli-tool), [Auth0 Terraform Provider](/fr-CA/docs/deploy-monitor/auth0-terraform-provider) et [Auth0 CLI](https://auth0.github.io/auth0-cli/).

  Native to Web SSO prend en charge tout flux d’authentification qui renvoie un jeton d’actualisation, comme le [flux Resource Owner Password](/fr-CA/docs/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa) et le [flux de code d’autorisation avec PKCE](/fr-CA/docs/get-started/authentication-and-authorization-flow/authorization-code-flow-with-pkce).
</Callout>
...
Une fois l’utilisateur authentifié, Auth0 renvoie un [jeton d’accès](/fr-CA/docs/secure/tokens/access-tokens), un [jeton d’ID](/fr-CA/docs/secure/tokens/id-tokens) et, facultativement, un [jeton d’actualisation](/fr-CA/docs/secure/tokens/refresh-tokens).

Vous pouvez configurer votre application native pour échanger un <Tooltip tip="Jeton d’actualisation : jeton utilisé pour obtenir un nouveau jeton d’accès sans obliger les utilisateurs à se reconnecter." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=refresh+token">jeton d’actualisation</Tooltip> contre un Session Transfer Token. Si votre application web ne prend pas en charge l’injection de cookie, votre application native doit aussi configurer l’**URI de connexion** de votre application web afin d’injecter le Session Transfer Token en tant que paramètre d’URI.

* Mettez à jour votre application native à l’aide du <Tooltip tip="Auth0 Dashboard : principal produit d’Auth0 pour configurer vos services." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip> :

  1. Accédez à [Dashboard &gt; Applications](https://manage.auth0.com/dashboard/#/applications) :
  2. Sélectionnez votre application ou [créez-la](/fr-CA/docs/get-started/auth0-overview/create-applications#create-applications).
...
Avant d’activer le **Session Transfer Token**, assurez-vous d’avoir configuré l’**Application Login URI** de votre application web pour prendre en charge des paramètres supplémentaires. Pour en savoir plus sur les URI, consultez [Application Settings](/fr-CA/docs/get-started/applications/application-settings#application-uris).
...
Native to Web SSO prend en charge [Organizations](/fr-CA/docs/manage-users/organizations). Lorsqu’un utilisateur s’authentifie avec une organisation dans votre application native, le Session Transfer Token inclut le contexte de l’organisation.
...
L’utilisation de `session_transfer_token` avec [Actions](/fr-CA/docs/customize/actions) vous permet de configurer des capacités de détection et de réponse aux risques après l’authentification afin de mieux protéger les utilisateurs.
...
Vous pouvez surveiller l’activité Native to Web SSO en consultant les [journaux](/fr-CA/docs/deploy-monitor/logs) du tenant.
