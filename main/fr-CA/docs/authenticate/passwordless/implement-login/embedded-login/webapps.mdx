...
Pour utiliser les API <Tooltip tip="Passwordless : forme d’authentification qui ne repose pas sur un mot de passe comme premier facteur." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=Passwordless">Passwordless</Tooltip> intégrées dans les Regular Web Applications, assurez-vous d’activer l’option **Passwordless OTP** dans [Auth0 Dashboard &gt; Applications &gt; Applications](https://manage.auth0.com/#/applications), dans les paramètres de votre application, sous **Advanced Settings** &gt; **Grant Types**.
...
Le point de terminaison `/passwordless/start` a une [limite de débit](/fr-CA/docs/troubleshoot/customer-support/operational-policies/rate-limit-policy) de 50 requêtes par heure par IP. Si vous appelez l’API côté serveur, l’adresse IP de votre backend peut facilement atteindre ces limites de débit. Pour savoir comment résoudre ce problème, consultez la section Limitation du débit dans les points de terminaison Passwordless de [Utiliser les API Passwordless](/fr-CA/docs/authenticate/passwordless/implement-login/embedded-login/relevant-api-endpoints).
...
Personnalisez <Tooltip tip="Authentification multifacteur (MFA) : processus d’authentification de l’utilisateur qui utilise un facteur en plus du nom d’utilisateur et du mot de passe, comme un code par SMS." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=MFA">MFA</Tooltip> au moyen de flux intégrés. Utilisez la MFA API pour permettre aux utilisateurs d’enregistrer les facteurs de leur choix et d’effectuer la vérification avec des facteurs pris en charge par votre application.

Lorsque vous utilisez [Lock for Web](/fr-CA/docs/libraries/lock#2-authenticating-and-getting-user-info), le point de terminaison `oauth/token` renvoie l’erreur `mfa_required` et inclut le `mfa_token` dont vous avez besoin pour utiliser la MFA API, ainsi que le paramètre `mfa_requirements` contenant la liste des méthodes d’authentification actuellement prises en charge par votre application :
