...
Pour utiliser les API <Tooltip tip="Passwordless : forme d’authentification qui ne repose pas sur un mot de passe comme premier facteur." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=Passwordless">Passwordless</Tooltip> intégrées dans les applications natives, assurez-vous d’activer l’autorisation **Passwordless OTP** dans [Auth0 Dashboard &gt; Applications &gt; Applications](https://manage.auth0.com/#/applications), dans les paramètres de votre application, sous **Advanced Settings** &gt; **Grant Types**.
...
Personnalisez la <Tooltip tip="Authentification multifacteur (MFA) : processus d’authentification de l’utilisateur qui utilise un facteur en plus du nom d’utilisateur et du mot de passe, comme un code envoyé par SMS." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=MFA">MFA</Tooltip> avec des flux Embedded. Utilisez la MFA API pour permettre aux utilisateurs d’inscrire les facteurs de leur choix pris en charge par votre application et de les utiliser pour la vérification.

Lorsque votre application demande un <Tooltip tip="Jeton d’accès : information d’autorisation, sous la forme d’une chaîne opaque ou d’un JWT, utilisée pour accéder à une API." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=access+token+at+the">jeton d’accès au</Tooltip> point de terminaison `oauth/token`, la réponse renvoyée comprend le `mfa_token` à utiliser avec la MFA API ainsi que le paramètre `mfa_requirements`, qui contient une liste des authenticateurs actuellement pris en charge par votre application :
