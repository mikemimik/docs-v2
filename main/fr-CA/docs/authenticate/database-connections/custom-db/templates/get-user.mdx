...
Si une erreur survient, transmettez les informations d’erreur au paramètre `error`, en précisant ce qui s’est mal passé. Pour en savoir plus, consultez [Résoudre les problèmes liés aux bases de données personnalisées](/docs/fr-CA/authenticate/database-connections/custom-db/error-handling).
