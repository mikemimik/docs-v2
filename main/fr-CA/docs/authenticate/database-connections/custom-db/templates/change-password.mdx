---
description: Décrit les modèles de script d’action de base de données personnalisée permettant de changer le mot de passe d’un utilisateur.
title: Modèles de script Change Password
---

Le script Change Password implémente la fonction définie pour changer le mot de passe de l’utilisateur indiqué dans la base de données externe. Nous recommandons de nommer cette fonction `changePassword`.

Le script est utilisé uniquement dans un [scénario d’authentification héritée](/fr-CA/docs/authenticate/database-connections/custom-db/overview-custom-db-connections) et il est requis si vous voulez changer le mot de passe d’un utilisateur dans la base de données externe. Il s’exécute lorsqu’un utilisateur effectue un [workflow de réinitialisation du mot de passe](/fr-CA/docs/customize/login-pages/classic-login/customize-password-reset-page), ou lorsqu’un workflow de changement de mot de passe est lancé depuis l’<Tooltip tip="Auth0 Dashboard : le principal produit d’Auth0 pour configurer vos services." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip> ou l’Auth0 <Tooltip tip="Management API : un produit qui permet aux clients d’effectuer des tâches administratives." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=Management+API">Management API</Tooltip>.
...
* Renvoyer `true` (ou un objet contenant la propriété `last_password_reset`) si l’opération de changement de mot de passe a réussi. Si la propriété `last_password_reset` est présente dans l’objet, elle sera mise à jour dans le profil de l’utilisateur.
* Renvoyer `false` si l’opération de changement de mot de passe a échoué.
* Renvoyer une erreur si la base de données externe est inaccessible.
...
| `newPassword` | String   | Valeur à définir comme nouveau mot de passe de l’utilisateur dans la base de données externe. Cette valeur est envoyée en clair à la fonction et [doit être chiffrée](#Encryption) avant d’être envoyée à la base de données externe. |
| `callback`    | Function | Sert à transmettre des données ou le résultat d’une opération dans le pipeline.                                                                                                                                                       |

### Exemple

Voici un exemple en pseudo-JavaScript montrant comment vous pourriez implémenter la fonction `changePassword`. Pour des exemples propres à chaque langage, consultez [Exemples de scripts par langage](#language-specific-script-examples).
...
<Warning>
  Évitez de consigner, de stocker ou de transmettre le justificatif de mot de passe sous forme non chiffrée.
</Warning>

Pour éviter toute fuite potentielle de données, hachez la valeur du mot de passe à l’aide d’une bibliothèque de hachage cryptographique telle que `bcrypt`.
...
| `resultObj`       | Object  | Facultatif  | Indique que l’opération de changement de mot de passe a réussi. Si la propriété `last_password_reset` est présente, elle sera mise à jour dans le profil de l’utilisateur. |

<div id="return-a-success">
  ### Retourner un résultat positif
</div>

Si l’opération de changement de mot de passe a réussi, appelez la fonction `callback` en transmettant une valeur `null` pour le paramètre `error` et une valeur `true` pour le paramètre `operationResult`.
...
<div id="return-a-success-and-update-last_password_reset-attribute">
  ### Renvoyer un succès et mettre à jour l’attribut last_password_reset
</div>

Si l’opération de changement de mot de passe a réussi, renvoyez la fonction `callback` en transmettant une valeur `null` comme paramètre `error` et une valeur de type objet comme paramètre `profile`. Si l’attribut `last_password_reset` est fourni dans l’objet, il sera mis à jour dans le profil de l’utilisateur.
...
Si l’opération de changement de mot de passe a échoué, appelez la fonction `callback` en passant la valeur `null` pour le paramètre `error` et la valeur `false` pour le paramètre `operationResult`.
...
Si une erreur se produit, appelez la fonction `callback` et transmettez-lui les renseignements pertinents sur l’erreur dans le paramètre `error`.
...
function changePassword(email, newPassword, callback) {
  ...
  connection.on('debug', function(text) {
      // si vous avez des problèmes de connexion, décommentez ceci pour obtenir plus de détails
      //console.log(text);
