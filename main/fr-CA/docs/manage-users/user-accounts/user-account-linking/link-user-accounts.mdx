---
description: Découvrez comment lier des comptes utilisateur provenant de divers fournisseurs d’identité afin que vos utilisateurs puissent s’authentifier avec n’importe lequel de leurs comptes et être reconnus par votre application.
...
<Card title="La disponibilité varie selon le plan Auth0">
  Votre implémentation de connexion ainsi que votre plan Auth0 ou votre entente personnalisée déterminent si cette fonctionnalité est offerte. Pour en savoir plus, consultez [Tarification](https://auth0.com/pricing).
</Card>

Vous pouvez lier des comptes utilisateur de différentes façons :

* Action avec application externe de liaison de comptes
...
Vous pouvez utiliser une Action avec une application externe de liaison de comptes pour lier des comptes d’utilisateur au moyen de la Management API.
...
<Warning>
  ...
  **Toute liaison manuelle de comptes devrait inviter l’utilisateur à saisir ses identifiants**. Votre tenant devrait demander l’authentification des deux comptes avant la liaison afin d’éviter que des acteurs malveillants puissent accéder à des comptes d’utilisateur légitimes.
</Warning>
...
2. L’Action redirige l’utilisateur vers une application externe de liaison de comptes avec une charge utile de jeton contenant les identités d’utilisateur candidates :
...
4. L’application externe de liaison de comptes redirige l’utilisateur vers l’Action avec une charge utile de jeton contenant les identités d’utilisateur principale et secondaire :
...
6. L’Action appelle la Management API pour lier les comptes selon les résultats de l’application externe de liaison de comptes.
...
/**
 * Filtrer et mapper les identités candidates avec une adresse courriel vérifiée
 */
...
// Nous ne traiterons pas les utilisateurs pour la liaison de comptes tant qu'ils n'auront pas vérifié leur adresse courriel.
  ...
  try {
    // Trouver les utilisateurs ayant la même adresse courriel
...
Pour une liaison de comptes côté client initiée par l’utilisateur, vous avez besoin d’un jeton d’accès qui contient les éléments suivants dans la charge utile :

* portée `update:current_user_identites`
* `user_id` du compte principal dans l’URL
* <Tooltip tip="Jeton d’identification : justificatif destiné au client lui-même plutôt qu’à l’accès à une ressource." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=ID+token">jeton d’identification</Tooltip> du compte secondaire, signé avec RS256 et comprenant une revendication `aud` qui identifie le client correspondant à la valeur de la revendication `azp` du jeton d’accès à l’origine de la demande.
...
Pour la liaison de comptes côté serveur, vous avez besoin d’un jeton d’accès contenant les éléments suivants dans la charge utile :

* la portée `update:users`
* le `user_id` du compte principal dans l’URL
* le `user_id` du compte secondaire
* le jeton d’identification du compte secondaire, signé avec RS256, qui comprend une revendication `aud` identifiant le client correspondant à la valeur de la revendication `azp` du jeton d’accès demandeur.

Les jetons d’accès qui contiennent la portée `update:users` peuvent être utilisés pour mettre à jour les renseignements de n’importe quel utilisateur. Cette méthode est donc destinée uniquement au code côté serveur.
...
Le `user_id` et le `provider` du compte utilisateur secondaire peuvent être déduits à partir de son identifiant unique. Par exemple, pour l’identifiant `google-oauth2|108091299999329986433` :

* `provider` est `google-oauth2`
* `user_id` est `108091299999329986433`

Vous pouvez également envoyer le jeton d’identification du compte secondaire au lieu du `provider` et du `user_id` :
...
Vous pouvez utiliser la bibliothèque auth0.js pour effectuer la liaison de comptes côté client. Consultez [Référence auth0.js &gt; Gestion des utilisateurs](/fr-CA/docs/libraries/auth0js#user-management) pour en savoir plus.
