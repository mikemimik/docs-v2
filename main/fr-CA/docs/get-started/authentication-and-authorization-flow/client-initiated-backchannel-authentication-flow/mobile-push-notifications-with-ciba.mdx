...
Utilisez les [API User Search](/docs/fr-CA/manage-users/user-search) pour trouver l’utilisateur qui autorisera la demande pour lequel vous souhaitez lancer une requête CIBA et obtenir son ID utilisateur.

Une fois que vous avez l’ID utilisateur de l’utilisateur qui autorisera la demande, utilisez l’Authentication API ou nos [SDKs](/docs/fr-CA/libraries) pour envoyer une requête CIBA au endpoint `/bc-authorize` :
...
Utilisez l’Authentication API ou nos [SDKs](/docs/fr-CA/libraries) pour appeler le point de terminaison `/token` à l’aide du grant type `urn:openid:params:grant-type:ciba` et de l’`auth_req_id` reçu du point de terminaison `/bc-authorize` :
...
Auth0 envoie une notification push à l’application mobile ou à l’appareil enregistré de l’utilisateur par l’intermédiaire de l’application Auth0 Guardian ou d’une application personnalisée intégrée au [SDK Auth0 Guardian](/docs/fr-CA/secure/multi-factor-authentication/auth0-guardian).

Si vous utilisez une application personnalisée, le [SDK Auth0 Guardian](/docs/fr-CA/secure/multi-factor-authentication/auth0-guardian) fournit des méthodes pour analyser les données reçues de la notification push et renvoyer une instance `Notification` prête à l’emploi. L’instance `Notification` comprend un identifiant de liaison de la transaction, ou `txlinkid`, que l’application mobile utilise pour récupérer les détails du consentement auprès d’Auth0.
