...
Le composant `SsoProviderEdit` fournit une interface unifiée pour modifier les fournisseurs [Single Sign-On](/fr-CA/docs/authenticate/enterprise-connections/self-service-enterprise-configuration).

<ComponentLoader componentSelector="sso-provider-edit" componentPreviewText="Aperçu du composant SSO Provider Edit" />
...
<Tabs>
  <Tab title="React">
    ...
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      **Configuration d’Auth0 requise** - Assurez-vous que votre tenant est configuré avec l’API My Organization. [Voir le guide de configuration
    ...
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      L’exécution de l’une ou l’autre commande installe aussi la dépendance @auth0/universal-components-core pour les utilitaires partagés et l’intégration à Auth0.
    </Callout>
    ...
    Les props obligatoires sont fondamentales au fonctionnement du composant. `SsoProviderEdit` nécessite l&#39;identifiant du provider pour charger et modifier le bon provider.
    ...
    <table class="table">
      ...
      <tbody>
        <tr>
          ...
          <td>
            <strong>Obligatoire.</strong> L’identifiant du Provider SSO à modifier.
          </td>
        </tr>
      </tbody>
    </table>
    ...
    **Type:** `{ icon?: LucideIcon; onClick: (e: MouseEvent) => void }`
    ...
    Configure le bouton de retour dans l&#39;en-tête du composant. Utilisez-le pour revenir à votre liste de fournisseurs.

    **Propriétés :**

    * `icon` - composant d’icône Lucide personnalisé (facultatif, valeur par défaut : ArrowLeft)
    * `onClick` - gestionnaire d’événement de clic pour la navigation
    ...
    **Type:** `ComponentAction<IdentityProvider, IdentityProvider>`
    ...
    Gère l&#39;enregistrement des modifications à la configuration du fournisseur (identifiant client, secrets, certificats, etc.).
    ...
    * `onAfter(provider, result)` - S’exécute une fois le Provider mis à jour avec succès. Utilisez ceci pour afficher une notification ou consigner l’événement.
    ...
    **Type:** `ComponentAction<IdentityProvider>`
    ...
    Contrôle la suppression permanente du fournisseur de votre tenant Auth0.
    ...
    **Type:** `ComponentAction<Domain>`
    ...
    Contrôle la vérification de domaine via un enregistrement DNS TXT.
    ...
    <table class="table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Variables CSS et surcharges de classe.
          </td>
        </tr>
      </tbody>
    </table>
    ...
    **schema**
    ...
    <Accordion title="Champs disponibles du schéma">
      ...
      **provider.*** - Champs de configuration de Provider selon la stratégie

      * Commun : `name`, `displayName`
    ...
    **customMessages**
    ...
    <Accordion title="Messages disponibles">
      ...
      * `title`, `description`
      * `fields.*` - Étiquettes de champs du formulaire selon la stratégie
    ...
    ***
    ...
    Le composant `SsoProviderEdit` est composé de sous-composants et de hooks plus petits. Vous pouvez les importer individuellement pour créer des workflows personnalisés de modification de fournisseur si vous utilisez shadcn.
  ...
  <Tab title="Next.js">
    ...
    Les props de base sont fondamentales au fonctionnement du composant. `SsoProviderEdit` nécessite l&#39;identifiant du provider pour charger et modifier le bon provider.
    ...
    **Type:** `ComponentAction<IdentityProvider, IdentityProvider>`
    ...
    Gère l&#39;enregistrement des modifications apportées à la configuration du fournisseur (identifiant client, secrets, certificats, etc.).
    ...
    * `onAfter(provider, result)` - S’exécute une fois le Provider mis à jour avec succès. Utilisez ceci pour afficher une notification ou faire le suivi de l’événement.
    ...
    **style**
    ...
    Personnalisez l&#39;apparence avec des variables CSS et des remplacements de classes. Prend en charge la stylisation sensible au thème.
    ...
    ***
    ...
    Le composant `SsoProviderEdit` est composé de sous-composants et de hooks plus petits. Vous pouvez les importer individuellement pour créer des workflows personnalisés de modification de fournisseur.
  ...
  <Tab title="shadcn">
    ...
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      L’exécution de la commande shadcn installe également la dépendance @auth0/universal-components-core pour les utilitaires communs et l’intégration d’Auth0.
    </Callout>
    ...
    * `onClick` - fonction appelée au clic pour la navigation
    ...
    <table class="table">
      ...
      <tbody>
        <tr>
          ...
          <td>
            Mettre à jour les paramètres du Provider.
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            Supprimer définitivement le Provider.
          </td>
        </tr>
    ...
    **Type:** `ComponentAction<IdentityProvider, IdentityProvider>`
    ...
    Gère la sauvegarde des modifications apportées à la configuration du fournisseur (identifiant client, secrets, certificats, etc.).

    **Propriétés :**

    * `disabled` - Désactive le bouton Enregistrer
    * `onBefore(provider)` - S’exécute avant la mise à jour. Retournez `false` pour empêcher l’enregistrement (p. ex., pour afficher une boîte de dialogue de confirmation).
    * `onAfter(provider, result)` - S’exécute une fois le provider mis à jour avec succès. Utilisez cette fonction pour afficher une notification ou consigner l’événement.
    ...
    ***
    ...
    La propriété `provisioning` configure les actions pour l&#39;onglet de provisionnement SCIM. Cet onglet gère le provisionnement automatique des utilisateurs via le protocole SCIM.
    ...
    **Type:** `ComponentAction<IdentityProvider, CreateIdPProvisioningConfigResponseContent>`
    ...
    Active le provisionnement SCIM pour le fournisseur. Une fois activé, vous pouvez générer un jeton SCIM à l&#39;intention de votre fournisseur d&#39;identité.
    ...
    **Type:** `ComponentAction<IdentityProvider, CreateIdpProvisioningScimTokenResponseContent>`
    ...
    Génère un nouveau jeton SCIM de type porteur pour que votre fournisseur d&#39;identité s&#39;authentifie auprès d&#39;Auth0.
    ...
    ***
    ...
    Le composant `SsoProviderEdit` est composé de sous-composants et de hooks plus petits. Vous pouvez les importer individuellement pour créer des workflows personnalisés de gestion de fournisseurs SSO si vous utilisez shadcn.

    ### Sous-composants disponibles

    Pour les cas d&#39;utilisation avancés, vous pouvez importer des composants d&#39;onglets individuels pour créer des interfaces d&#39;édition de fournisseurs personnalisées.
