...
Pour configurer les métadonnées des jetons d’actualisation, vous pouvez utiliser une [Action post-login](/fr-CA/docs/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger) d’Auth0 ainsi que la [Management API](https://auth0.com/docs/api/management/v2).
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Les appels au point de terminaison `/api/v2/refresh-tokens/{id}` nécessitent un [jeton d’accès à la Management API](/fr-CA/docs/secure/tokens/access-tokens) assorti de la portée `update:refresh_tokens`.
</Callout>
...
Vous pouvez consulter les événements de journal liés aux métadonnées du jeton d’actualisation en accédant à [Dashboard &gt; Monitoring &gt; Logs](https://manage.auth0.com/#/logs) ou récupérer les journaux à l’aide du point de terminaison [Management API logs](/fr-CA/docs/api/management/v2/logs/get-logs).
