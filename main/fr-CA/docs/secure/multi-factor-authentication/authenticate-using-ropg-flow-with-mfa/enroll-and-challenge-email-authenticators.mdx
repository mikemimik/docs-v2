---
description: Décrit comment créer vos propres flux MFA à l’aide du SMS ou d’un appel vocal comme facteur d’authentification.
...
Auth0 fournit un flux intégré d’inscription et d’authentification <Tooltip tip="MFA : processus d’authentification de l’utilisateur qui utilise un facteur en plus du nom d’utilisateur et du mot de passe, comme un code envoyé par SMS." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=MFA">MFA</Tooltip> avec [Universal Login](/docs/fr-CA/authenticate/login/auth0-universal-login). Toutefois, si vous souhaitez créer votre propre interface utilisateur, vous pouvez utiliser l’[MFA API](/docs/fr-CA/secure/multi-factor-authentication/multi-factor-authentication-developer-resources/mfa-api) pour le faire.

Lorsque le courriel est activé comme facteur, tous les utilisateurs ayant des adresses courriel vérifiées pourront les utiliser pour terminer l’authentification MFA.
...
* [Configurer Email](/docs/fr-CA/secure/multi-factor-authentication/multi-factor-authentication-factors/configure-email-notifications-for-mfa) comme facteur dans le Dashboard ou à l’aide de la [Management API](https://auth0.com/docs/api/management/v2#!/Guardian/put_factors_by_name).

<div id="enroll-with-email">
  ## Inscription par courriel
</div>

Pour permettre aux utilisateurs d’inscrire des adresses courriel en plus de l’adresse courriel vérifiée de leur identité principale, vous devez suivre les étapes suivantes.
...
Envoyez une requête `POST` au point de terminaison MFA Associate pour inscrire l’authentificateur de l’utilisateur. Le bearer token exigé par ce point de terminaison est le jeton MFA obtenu à l’étape précédente.
...
Si vous obtenez une erreur `User is already enrolled`, c’est que l’utilisateur a déjà un facteur MFA inscrit. Avant d’associer un autre facteur à l’utilisateur, vous devez d’abord lui envoyer une demande de vérification avec le facteur existant.
...
L’utilisateur devrait recevoir un courriel contenant le code à 6 chiffres qu’il pourra fournir à l’application.

Pour terminer l’inscription, envoyez une requête `POST` au point de terminaison [**`/oath/token`**](https://auth0.com/docs/api/authentication#get-token). Incluez le `oob_code` renvoyé dans la réponse précédente ainsi que le `binding_code` correspondant à la valeur figurant dans le message du courriel.
...
À ce stade, l’authentificateur est entièrement associé, prêt à être utilisé, et vous disposez des jetons d’authentification de l’utilisateur.
...
Pour soumettre l’utilisateur à une demande de vérification, vous avez besoin de l’`authenticator_id` du facteur que vous souhaitez utiliser. Vous pouvez obtenir la liste de tous les authentificateurs inscrits à l’aide du point de terminaison MFA Authenticators :
...
<div id="challenge-user-with-otp">
  ### Vérifier l’utilisateur au moyen d’un OTP
</div>
...
Votre application doit demander à l’utilisateur de saisir le code et l’envoyer dans la requête, dans le paramètre `binding_code`, lors de l’appel suivant au point de terminaison `oauth``/token` :
...
<Warning>
  La MFA personnalisable avec les flux Resource Owner Password Grant, Embedded ou Refresh Token est en accès anticipé. En utilisant cette fonctionnalité, vous acceptez les conditions d’essai gratuit applicables de l’[entente-cadre d’abonnement](https://www.okta.com/legal/) d’Okta. Pour en savoir plus sur les étapes de lancement d’Auth0, consultez [Product Release Stages](/docs/fr-CA/troubleshoot/product-lifecycle/product-release-stages). Pour participer à l’accès anticipé, communiquez avec [Auth0 Support](https://support.auth0.com/).
</Warning>
