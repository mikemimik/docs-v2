...
title: Gérer les méthodes d’authentification avec la Management API
---
...
<Card title="Avant de commencer">
  * Configurez une [application machine-to-machine (M2M)](/fr-CA/docs/get-started/auth0-overview/create-applications/machine-to-machine-apps) et [autorisez-la à accéder à l&#39;Auth0 Management API](/fr-CA/docs/secure/tokens/access-tokens/management-api-access-tokens/get-management-api-access-tokens-for-production) avec les portées suivantes :
...
L&#39;[Auth0 Management API](https://auth0.com/docs/api/management/v2) fournit plusieurs points de terminaison que vous pouvez utiliser pour gérer les méthodes d&#39;authentification <Tooltip tip="Authentification multifacteur (MFA) : processus d'authentification de l'utilisateur qui utilise un facteur en plus du nom d'utilisateur et du mot de passe, comme un code envoyé par SMS." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=MFA">MFA</Tooltip> de vos utilisateurs.

Cette méthode repose sur l&#39;authentification au moyen d&#39;une application confidentielle. Pour en savoir plus sur les applications confidentielles et les applications publiques, consultez [Confidential and Public Applications](/fr-CA/docs/get-started/applications/confidential-and-public-applications).

<div id="get-all-authentication-methods">
  ## Obtenir toutes les méthodes d’authentification
</div>

Utilisez le point de terminaison [Récupère une liste des méthodes d’authentification](https://auth0.com/docs/api/management/v2#!/Users/get_authentication_methods) pour obtenir la liste de toutes les méthodes d’authentification qu’un utilisateur a entièrement ou partiellement configurées.

Ce point de terminaison exige la portée : `read:authentication_methods`.
...
Utilisez le [point de terminaison Gets an authentication method by ID](https://auth0.com/docs/api/management/v2#!/Users/get_authentication_methods_by_authentication_method_id) pour obtenir une méthode d’authentification unique pour un utilisateur à l’aide de l’ID de la méthode d’authentification.

Ce point de terminaison exige la portée : `read:authentication_methods`.

### Exemples

La requête suivante renvoie une seule méthode d&#39;authentification pour un utilisateur, à partir de l&#39;ID de la méthode d&#39;authentification spécifiée.
...
Utilisez le point de terminaison [Créer une méthode d’authentification pour un utilisateur donné](https://auth0.com/docs/api/management/v2#!/Users/post_authentication_methods) pour créer une méthode d’authentification pour un utilisateur, notamment par SMS, par courriel, par mot de passe à usage unique (OTP) ou avec WebAuthn et des clés de sécurité. Pour en savoir plus sur les facteurs d’authentification MFA offerts, consultez [Facteurs d’authentification multifacteur](/fr-CA/docs/secure/multi-factor-authentication/multi-factor-authentication-factors).

Ce point de terminaison nécessite la portée suivante : `create:authentication_methods`.

<Warning>
  Les méthodes d’authentification créées au moyen de ce point de terminaison seront confirmées automatiquement et disponibles immédiatement. Vérifiez avec l’utilisateur que la méthode d’authentification est correctement configurée et qu’elle est toujours valide.
</Warning>
...
Envoyez aux utilisateurs un OTP par SMS, qu’ils devront ensuite saisir avant de pouvoir terminer leur authentification.
...
Envoyez un OTP aux utilisateurs par courriel; ils seront ensuite invités à le saisir avant de pouvoir finaliser l’authentification. Le facteur Email est pris en charge uniquement lorsqu’un utilisateur n’a aucun autre facteur disponible.
...
Permettez aux utilisateurs d’utiliser une application d’authentification, comme Google Authenticator, sur leur appareil personnel pour générer un OTP qui change périodiquement et qu’ils devront saisir avant la fin du processus d’authentification.
...
Permettez aux utilisateurs d’utiliser des clés de sécurité conformes à la norme FIDO (par exemple, [Yubikey](https://www.yubico.com/) ou [Google Titan](https://cloud.google.com/titan-security-key)) pour l’authentification multifacteur.

<div id="examples">
  #### Exemples
</div>

La requête suivante crée une méthode d’authentification WebAuthn avec des clés de sécurité pour un utilisateur.
...
Utilisez le point de terminaison [Mettre à jour une méthode d’authentification](https://auth0.com/docs/api/management/v2#!/Users/patch_authentication_methods_by_authentication_method_id) pour mettre à jour une méthode d’authentification d’un utilisateur.

Ce point de terminaison nécessite la portée : `update:authentication_methods`.

### Exemples

La requête suivante met à jour une seule méthode d’authentification pour un utilisateur à partir de l’ID de cette méthode d’authentification.
...
Ce point de terminaison nécessite la portée : `delete:authentication_methods`.
...
Pour chaque requête valide, la Management API renvoie une réponse avec un code d’état `204` et un corps de réponse vide.

<div id="delete-a-single-authentication-method">
  ## Supprimer une méthode d’authentification
</div>

Utilisez le point de terminaison [Supprime une méthode d’authentification par ID](https://auth0.com/docs/api/management/v2#!/Users/delete_authentication_methods_by_authentication_method_id) pour supprimer une méthode d’authentification d’un utilisateur.

### Exemples

La requête suivante supprime une méthode d’authentification précise pour un utilisateur, en fonction de l’ID de la méthode d’authentification indiqué.
...
Pour chaque requête valide, la Management API renvoie une réponse avec un code d’état `204` et sans corps.
