...
Si vos utilisateurs ne sont pas inscrits à <Tooltip tip="Authentification multifacteur (AMF) : processus d’authentification de l’utilisateur qui utilise un facteur en plus du nom d’utilisateur et du mot de passe, comme un code par SMS." cta="Consulter le glossaire" href="/docs/fr-CA/glossary?term=MFA">AMF</Tooltip>, vous devriez utiliser la politique par défaut d’Adaptive MFA. Si un utilisateur n’est pas inscrit à l’AMF et que votre Action évalue le risque comme élevé, vos options pour bloquer un <Tooltip tip="Acteur malveillant : entité (une personne ou un groupe) qui représente une menace pour l’entreprise ou l’environnement avec l’intention de causer du tort." cta="Consulter le glossaire" href="/docs/fr-CA/glossary?term=bad+actor">acteur malveillant</Tooltip> sont limitées.
...
Adaptive MFA calcule un score de confiance global à partir de l’analyse de trois évaluations : `NewDevice`, `ImpossibleTravel` et `UntrustedIP`. Pour en savoir plus, consultez [Adaptive MFA: How it works](/docs/fr-CA/secure/multi-factor-authentication/adaptive-mfa).
...
* Le déclencheur d’Action [`api.multifactor.enable`](/docs/fr-CA/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-api-object#api-multifactor) pour gérer à la fois l’inscription et présenter les vérifications AMF configurées à la fin du flux de connexion.
