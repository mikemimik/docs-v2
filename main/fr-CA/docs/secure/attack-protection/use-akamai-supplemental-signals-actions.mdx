...
Une fois Akamai configuré pour utiliser Supplemental Signals, vous pouvez utiliser les données fournies par ces signaux dans les [Auth0 Actions](/docs/fr-CA/customize/actions).
...
<AccordionGroup>
  ...
  <Accordion title="Demander l’authentification multifacteur (MFA) en fonction des résultats d’Akamai Bot Manager">
    ...
    1. **Mettre à jour les [métadonnées d’application](/docs/fr-CA/manage-users/user-accounts/metadata/metadata-fields-data)** : si la propriété `score` dépasse une valeur donnée, consignez que la MFA est requise pour la session.
