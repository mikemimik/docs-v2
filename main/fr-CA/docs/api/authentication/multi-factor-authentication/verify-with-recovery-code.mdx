...
description: "Vérifiez la MFA à l’aide d’un code de récupération lorsque l’appareil enregistré de l’utilisateur n’est pas disponible."
---
...
Vérifie l’authentification multifacteur (MFA) à l’aide d’un code de récupération. Certains fournisseurs d’authentification multifacteur (MFA) (comme Guardian) prennent en charge l’utilisation d’un code de récupération pour ouvrir une session. Utilisez cette méthode pour authentifier l’utilisateur lorsque son appareil enregistré n’est pas disponible, ou qu’il ne peut pas recevoir la demande de validation ni y répondre en raison de problèmes de connectivité.

Pour vérifier la MFA à l’aide d’un code de récupération, votre application doit demander à l’utilisateur de saisir le code de récupération, puis envoyer une requête à `/oauth/token` avec `grant_type=http://auth0.com/oauth/grant-type/mfa-recovery-code`. Incluez le code de récupération saisi ainsi que le `mfa_token` provenant de l’erreur `mfa_required`. Si le code de récupération est accepté, la réponse sera la même que pour les types d’octroi `password` ou `http://auth0.com/oauth/grant-type/password-realm`. Elle peut aussi inclure un champ `recovery_code`, que l’application doit afficher à l’utilisateur final pour qu’il le conserve de façon sécuritaire en vue d’une utilisation future.
...
<ResponseSchema>
  <ResponseField name="access_token" type="string">
    Le jeton d’accès issu d’une vérification réussie.
  </ResponseField>
  ...
  <ResponseField name="recovery_code" type="string">
    Un nouveau code de récupération à conserver en lieu sûr pour une utilisation future.
  </ResponseField>
</ResponseSchema>
