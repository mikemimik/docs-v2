...
例として、[Get User Info endpoint](/ja/user-profile/get-user-info)があります。このシナリオでは、ユーザーを認証するとアクセストークンを取得でき、その後、そのトークンを`Authorization` headerに含めて[Get User Info endpoint](/ja/user-profile/get-user-info)にリクエストを送信し、ユーザーのプロファイルを取得できます。
