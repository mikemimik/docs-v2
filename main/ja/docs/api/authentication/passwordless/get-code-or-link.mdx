...
* メールまたはSMSで確認コードを送信した場合は、コードを受け取った後、`username` に `email` または `phone_number` を、`password` に確認コードを指定して、[/passwordless/verify エンドポイント](/ja/passwordless/authenticate-user)を使用してユーザーを認証する必要があります。
...
* auth0.js のサンプルスクリプトでは、ライブラリのバージョン 8 を使用しています。auth0.js バージョン 7 を使用している場合は、この[リファレンスガイド](/ja/libraries/auth0js/v7)を参照してください。
