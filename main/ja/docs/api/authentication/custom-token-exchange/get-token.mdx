...
* Action が `setActor()` を呼び出すと、発行されたアクセストークンと ID トークンには、[委任チェーン](/docs/ja/secure/call-apis-on-users-behalf/on-behalf-of-token-exchange#the-act-claim) を表す `act` クレームが含まれます。`act` クレームは userinfo レスポンスにも含まれます。
