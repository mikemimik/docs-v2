...
キーワード置換を使って、固定の配列に値を追加したい場面があるかもしれません。このケースを直接サポートする特別な構文はありませんが、適切な値を含む単一の文字列内で二重引用符をエスケープし、`##` キーワード構文で挿入することで実現できます。この手法は、[YAML 形式とディレクトリ形式](/ja/docs/deploy-monitor/deploy-cli-tool/available-resource-configuration-formats)の両方で使用できます。
