...
* [Quickstart](/docs/ja/quickstart/native/ios-swift/interactive): Auth0.swift を iOS / macOS アプリにゼロから組み込む方法を説明します。
...
Auth0.swift が Auth0 と通信するには、Auth0 アプリケーションの **<Tooltip tip="Client ID: Auth0 から登録済みリソースに付与される識別子です。" cta="用語集を見る" href="/docs/ja/glossary?term=Client+ID">Client ID</Tooltip>** と **Domain** が必要です。これらの情報は、Auth0 アプリケーションの設定ページで確認できます。[カスタムドメイン](/docs/ja/customize/custom-domains) を使用している場合は、設定ページの値ではなく、<Tooltip tip="Custom Domain: 特別な、または独自の名前を持つサードパーティのドメイン。" cta="用語集を見る" href="/docs/ja/glossary?term=custom+domain">カスタムドメイン</Tooltip> の値を使用してください。
...
コールバック URL とログアウト URL は、Auth0 がアプリケーションへリダイレクトする際に使用する URL です。Auth0 はユーザーの認証後にコールバック URL を呼び出し、<Tooltip tip="セッション Cookie: 存在すると、ユーザーが認証済みと見なされるようにするエンティティ。" cta="用語集を見る" href="/docs/ja/glossary?term=session+cookie">セッション Cookie</Tooltip> を削除した後にログアウト URL を呼び出します。
...
[Auth0 アプリケーション](https://manage.auth0.com/#/applications/) の設定ページに移動し、アプリケーションのプラットフォームに応じて、対応する URL を **Allowed Callback URLs** と **Allowed Logout URLs** に追加します。[カスタムドメイン](/docs/ja/customize/custom-domains) を使用している場合は、`{yourAuth0Domain}` を設定ページの値ではなく、カスタムドメインの値に置き換えてください。
...
<Warning>
  [**トークンエンドポイント認証方法**](/docs/ja/get-started/applications/confidential-and-public-applications/view-application-type) が `None` に設定されていることを確認してください。
</Warning>
...
次に、**Login** ボタンのアクションで [Universal Login](/docs/ja/authenticate/login/auth0-universal-login) ページを表示します。
...
ユーザーをログアウトするには、<Tooltip tip="Universal Login: アプリケーションは、ユーザーの本人確認を行うために、Auth0 の認可サーバーでホストされている Universal Login にリダイレクトされます。" cta="用語集を見る" href="/docs/ja/glossary?term=Universal+Login">Universal Login</Tooltip> のセッション Cookie をクリアした後、アプリケーションからユーザーの資格情報を削除します。
