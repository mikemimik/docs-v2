...
<ParamField body="resendManager" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/ResendControl">ResendControl</a></span>}>
  このメソッドは、timeout management を備えた resend functionality を提供します。再送をトリガーする `startResend()` メソッドを持つ [`ResendControl`](/ja/docs/libraries/acul/js-sdk/Screens/interfaces/ResendControl) オブジェクトを返し、ユーザーが再度再送できるようになるまでクールダウン時間を適用します。
