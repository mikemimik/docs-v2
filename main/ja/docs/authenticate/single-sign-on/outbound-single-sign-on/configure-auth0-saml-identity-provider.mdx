...
SAML 2.0を使用する<Tooltip tip="IDプロバイダー（IdP）：デジタルアイデンティティを保存・管理するサービス。" cta="用語集を見る" href="/ja/docs/glossary?term=identity+provider">アイデンティティプロバイダー</Tooltip>として、Auth0を<Tooltip tip="IDプロバイダー（IdP）：デジタルアイデンティティを保存・管理するサービス。" cta="用語集を見る" href="/ja/docs/glossary?term=SAML">SAML</Tooltip>構成で利用できます。
...
一部の SSO 連携では、[SAML2 Web App アドオン](/ja/docs/authenticate/protocols/saml/saml-sso-integrations/enable-saml2-web-app-addon) を使用します。

<div id="manually-configure-sso-integrations">
  ## SSO 連携を手動で設定する
</div>

<Tooltip tip="Auth0 Dashboard: サービスを設定するための Auth0 のメイン製品です。" cta="用語集を見る" href="/ja/docs/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip> の SAML2 Web App アドオンを使用すると、SAML <Tooltip tip="シングルサインオン（SSO）: ユーザーが1つのアプリケーションにログインすると、他のアプリケーションにも自動的にログインできるサービスです。" cta="用語集を見る" href="/ja/docs/glossary?term=SSO">SSO</Tooltip> 連携を手動で設定できます。SAML アドオンは、画面上の手順とサービスプロバイダーの設定で確認できる情報を使って設定できます。
...
* [SAMLアサーションをカスタマイズする](/ja/docs/authenticate/protocols/saml/saml-configuration/customize-saml-assertions)
