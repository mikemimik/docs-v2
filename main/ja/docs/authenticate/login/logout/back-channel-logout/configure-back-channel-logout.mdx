...
<Card title="始める前に">
  ...
  * 本番運用向けのアプリケーションでは TLS を使用する必要があります。詳しくは、[TLS (SSL) Versions and Ciphers](/ja/docs/customize/custom-domains/self-managed-certificates/tls-ssl) を参照してください。
...
<Tooltip tip="Auth0 Dashboard: サービスを設定するための Auth0 の主要プロダクト。" cta="用語集を見る" href="/ja/docs/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip> または <Tooltip tip="Auth0 Dashboard: サービスを設定するための Auth0 の主要プロダクト。" cta="用語集を見る" href="/ja/docs/glossary?term=Management+API">Management API</Tooltip> で、ログアウトトークンを受け取るアプリケーションを登録します。
...
<Tabs>
  <Tab title="Auth0 Dashboard">
    ...
    監査のため、サービスはバックチャネルログアウト URL の登録・登録解除を、常に Auth0 テナントログに `API Operation Event` として記録します。詳しくは、[ログ](/ja/docs/deploy-monitor/logs) を参照してください。
  </Tab>
...
この機能は、2023 年 10 月 3 日からすべてのテナントに順次リリースされます。なお、[テナントログのイベントコード](/ja/docs/deploy-monitor/logs/log-event-type-codes)には、`oidc_backchannel_logout_succeeded` の `sslo` または `oidc_backchannel_logout_failed` の `fslo` が引き続き表示される場合があります。
...
1. トークンが標準的な base64 エンコードの JWT であることを確認します。長いパラメーターは、一部の Web サーバーで途中で切り詰められることがあります。詳しくは、[署名アルゴリズム](/ja/docs/get-started/applications/signing-algorithms) を参照してください。
2. 可能であればトークンを取得し、それが JWT であることを確認します。[JWT.IO](http://jwt.io) などの信頼できるソースを使用してください。
3. 検証関数が、[JSON Web Key Sets](/ja/docs/secure/tokens/json-web-tokens/json-web-key-sets) (JWKS) を通じてテナントの署名鍵を動的に取得するようになっていることを確認します。
