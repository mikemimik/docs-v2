...
Auth0 では、[データベース接続](/ja/docs/authenticate/database-connections)でメールおよび電話ベースのパスワードレス認証を直接設定できます。ワンタイムパスワード (OTP) 認証用に別の接続を作成しなくても、データベース接続から直接パスワードレスログインを提供できます。これにより、実装の複雑さが軽減され、エンドユーザーのログイン体験がよりシンプルになります。
...
Auth0 では、ユーザーを OTP 認証で確認する [Identifier-First Authentication](/ja/docs/authenticate/login/auth0-universal-login/identifier-first) アプローチを採用しています。

1. エンドユーザーは、[Auth0 の Universal Login](/ja/docs/authenticate/login/auth0-universal-login) でログインに使用する識別子を入力します。
...
<Card title="始める前に">
  ...
  * 電話ベースの OTP (SMS または Voice) を使用する予定がある場合は、[**Branding &gt; Phone Provider**](https://manage.auth0.com/#/phone/templates/phone/provider) にある [Unified Phone Experience](/ja/docs/customize/phone-messages/unified-phone/configure-unified-phone) 設定を有効にする必要があります。
  * データベース接続で [Attributes](/ja/docs/authenticate/database-connections/activate-and-configure-attributes-for-flexible-identifiers) を有効にする必要があります。
...
詳しくは、[Flexible Identifiers の属性を有効化して設定する](/ja/docs/authenticate/database-connections/activate-and-configure-attributes-for-flexible-identifiers)を参照してください。
...
<Tabs>
  ...
  <Tab title="Management API" icon="code">
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      Management API を使用するには、[Management API access token](/ja/docs/secure/tokens/access-tokens/management-api-access-tokens/get-management-api-access-tokens-for-production) が必要です。
    </Callout>
...
<Tabs>
  ...
  <Tab title="Management API" icon="code">
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      Management API を使用するには、[Management API access token](/ja/docs/secure/tokens/access-tokens/get-management-api-access-tokens-for-production) が必要です。
    </Callout>
...
* [Flexible Identifiers の属性を有効化して設定する](/ja/docs/authenticate/database-connections/activate-and-configure-attributes-for-flexible-identifiers)