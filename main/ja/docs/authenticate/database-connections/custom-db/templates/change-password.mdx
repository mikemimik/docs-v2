...
このスクリプトは、[レガシー認証シナリオ](/ja/docs/authenticate/database-connections/custom-db/overview-custom-db-connections)でのみ使用され、外部データベース内のユーザーのパスワードを変更する場合に必要です。これは、ユーザーが[パスワードリセットのワークフロー](/ja/docs/customize/login-pages/classic-login/customize-password-reset-page)を実行したとき、または <Tooltip tip="Auth0 Dashboard: サービスを設定するための Auth0 の主要製品。" cta="用語集を見る" href="/ja/docs/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip> または Auth0 <Tooltip tip="Management API: お客様が管理タスクを実行できるようにする製品。" cta="用語集を見る" href="/ja/docs/glossary?term=Management+API">Management API</Tooltip> からパスワード変更ワークフローが開始されたときに実行されます。
