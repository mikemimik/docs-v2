...
* [埋め込みログインのネイティブアプリケーション](/docs/ja/authenticate/passwordless/implement-login/embedded-login/native)
* [Android および iOS アプリケーション向け Native パスキー](/docs/ja/authenticate/database-connections/passkeys/native-passkeys-for-mobile-applications)
...
<Tooltip tip="多要素認証（MFA）: SMS で送信されるコードなど、ユーザー名とパスワードに加えて認証要素を使用するユーザー認証プロセス。" cta="用語集を見る" href="/docs/ja/glossary?term=MFA">MFA</Tooltip> が有効になっている場合、設定とリスク評価に応じて、ユーザーはパスキーで認証した後に MFA チャレンジの完了を求められることがあります。
...
詳しくは、[Reduce friction with passkeys](/docs/ja/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger#reduce-friction-with-passkeys) および [Multi-Factor Authentication](/docs/ja/secure/multi-factor-authentication) を参照してください。
...
RP ID をカスタマイズする方法については、[Configure Passkey Policy](/docs/ja/authenticate/database-connections/passkeys/configure-passkey-policy) を参照してください。

<div id="learn-more">
  ## 詳細情報
</div>

* [パスキー認証を設定する](/docs/ja/authenticate/database-connections/passkeys/configure-passkey-policy)
* [テナントログでパスキーイベントを監視する](/docs/ja/authenticate/database-connections/passkeys/monitor-passkey-events-in-tenant-logs)