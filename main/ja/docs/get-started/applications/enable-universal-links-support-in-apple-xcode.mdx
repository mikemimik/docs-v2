...
Universal Links では、ドメインとアプリケーションの間に検証済みの関連付けが確立されるため、Auth0 のアプリケーション設定と iOS アプリケーションの設定が一致している必要があります。そのためには、Apple Team ID と Bundle Identifier を Auth0 に提供する必要があります。<Tooltip tip="Auth0 Dashboard: サービスを設定するための Auth0 の主要プロダクト。" cta="用語集を見る" href="/ja/docs/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip> を使用して、Auth0 に登録したアプリケーションで Universal Links サポートを有効にできます。
