...
<Warning>
  ワイルドカード URL プレースホルダーは、強化されたセキュリティ制御が適用された[サードパーティアプリケーション](/ja/docs/get-started/applications/third-party-applications)ではサポートされていません。コールバック URL、許可するオリジン、Web Origins には正確な URL を使用する必要があります。
</Warning>
...
* **Allows Web Origins**: [Cross-Origin Authentication](/ja/docs/authenticate/login/cross-origin-authentication)、[Device Flow](/ja/docs/get-started/authentication-and-authorization-flow/device-authorization-flow)、および[レスポンスモードとしての web&#95;message](/ja/docs/authenticate/protocols/oauth)を使用する認可リクエストの送信元として使用できる URL の一覧。
...
Auth0 では、このプレースホルダーをアプリケーションのログイン URI (`initiate_login_uri`) でサポートしています。詳しくは、[メタデータプレースホルダーを使用した動的ログイン URI](/ja/docs/authenticate/login/auth0-universal-login/configure-default-login-routes)を参照してください。
...
この機能の詳細な概要については、[複数のカスタムドメイン](/ja/docs/customize/custom-domains/multiple-custom-domains) を参照してください。
