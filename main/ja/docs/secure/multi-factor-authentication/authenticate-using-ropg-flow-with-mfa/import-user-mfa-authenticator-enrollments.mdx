...
[自動移行](/docs/ja/manage-users/user-migration/configure-automatic-migration-from-your-database)および[一括ユーザーインポート](/docs/ja/manage-users/user-migration/bulk-user-imports)を使用して、ユーザーの<Tooltip tip="多要素認証（MFA）：SMSで送信されるコードなど、ユーザー名とパスワードに加えて別の認証要素を使用するユーザー認証プロセス。" cta="用語集を見る" href="/docs/ja/glossary?term=MFA">MFA</Tooltip>登録情報をインポートできます。サポートされる登録タイプは次のとおりです。
...
クラシックログインでは、複数の認証要素を持つユーザーによる認証要素の選択はサポートされていません。複数の登録済み認証要素を持つユーザーをインポートする予定がある場合は、[Universal Login](/docs/ja/authenticate/login/auth0-universal-login) の利用を検討してください。
...
MFA の登録情報は、[自動移行](/docs/ja/authenticate/database-connections/custom-db/overview-custom-db-connections#automatic-migration-scenario)中にインポートすることもできます。これは、カスタム DB の [Login スクリプト](/docs/ja/authenticate/database-connections/custom-db/templates/login) の最後でコールバックに渡されるユーザーの `mfa_factors` フィールドに、既存の登録情報を指定することで実現できます。
...
1. `users.json` ファイルを準備します。詳しくは[一括ユーザーインポート](/docs/ja/manage-users/user-migration/bulk-user-imports)を参照してください。
