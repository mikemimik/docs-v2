...
`languageDictionary` を使用すると、Classic Login エクスペリエンスでの<Tooltip tip="多要素認証（MFA）: SMSで送信されるコードなど、ユーザー名とパスワードに加えて別の要素を使用するユーザー認証プロセス。" cta="用語集を見る" href="/docs/ja/glossary?term=MFA">MFA</Tooltip>プロセス内のさまざまな箇所のテキストを上書きできます。次の形式で指定します。
...
* [Classic Login の MFA のカスタマイズ](/docs/ja/secure/multi-factor-authentication/customize-mfa/customize-mfa-classic-login)