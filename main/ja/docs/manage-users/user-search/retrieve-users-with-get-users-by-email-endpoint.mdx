---
description: get-users-by-email endpointを使用してユーザー一覧を取得する方法を説明します。
title: Get Users by Email Endpointでユーザーを取得する
---
...
[`GET /api/v2/users-by-email` エンドポイント](https://auth0.com/docs/api/management/v2#!/Users_By_Email/get_users_by_email)を使用すると、メールアドレスでユーザーを検索できます。この検索では、指定したメールアドレスとの完全一致のみが対象となり、大文字と小文字は区別されます。

このエンドポイントは**即時整合性**があるため、次の用途で使用することを推奨します。

* 認証プロセス中に実行するユーザー検索。
* アカウントリンク プロセスの一環として実行するユーザー検索。

リクエストには、[Management API access token](/ja/docs/secure/tokens/access-tokens/management-api-access-tokens)を含める必要があります。
