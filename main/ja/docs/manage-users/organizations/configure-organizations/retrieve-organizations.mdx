...
[組織](/docs/ja/manage-users/organizations/organizations-overview)をプログラムから操作する場合、すべての組織の一覧を取得したり、名前または ID を指定して個別の組織を取得したりする必要があることがあります。

<Warning>
  Management API へのアクセスには、confidential なクライアントのみを使用してください。この API の利用には、[Auth0 Management API のレート制限](/docs/ja/troubleshoot/customer-support/operational-policies/rate-limit-policy/management-api-endpoint-rate-limits)が適用されます。
</Warning>
...
<Tooltip tip="Management API: お客様が管理タスクを実行するためのプロダクトです。" cta="用語集を見る" href="/docs/ja/glossary?term=Management+API">Management API</Tooltip> を使用して、テナントのすべての組織を取得できます。
...
`Get Organizations` endpoint に `GET` リクエストを送信します。`MGMT_API_ACCESS_TOKEN` プレースホルダーの値は、必ずご利用の Management API <Tooltip tip="Access Token: API へのアクセスに使用される認可資格情報で、opaque 文字列または JWT の形式です。" cta="用語集を見る" href="/docs/ja/glossary?term=Access+Token">Access Token</Tooltip> に置き換えてください。
