...
* テストの送信元となる IP アドレス
* 使用予定のツール
* 1 秒あたりのリクエスト数 (テストは [Rate Limit Policy](/ja/docs/troubleshoot/customer-support/operational-policies/rate-limit-policy) に準拠している必要があります)
* 対象となる Auth0 テナント
...
* 1秒あたりのテストリクエスト数が、当社の[Rate Limit Policy](/ja/docs/troubleshoot/customer-support/operational-policies/rate-limit-policy)で定義されている上限を超えないこと
...
* テナント や space に対して、いかなる形式のサービス拒否 (DoS) テストや、許可されていない大量トラフィック攻撃も実施してはなりません。詳細は、[負荷テスト](/ja/docs/troubleshoot/customer-support/operational-policies/load-testing-policy)を参照してください。
...
* [負荷テストポリシー](/ja/docs/troubleshoot/customer-support/operational-policies/load-testing-policy)