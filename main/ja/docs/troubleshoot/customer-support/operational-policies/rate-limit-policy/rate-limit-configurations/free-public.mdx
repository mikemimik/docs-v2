...
<AccordionGroup>
  <Accordion title="Authentication API: FreeサブスクリプションにおけるAuthentication APIとAPIエンドポイントのレート制限。">
    ...
    | [On-Behalf-Of Token Exchange](/ja/docs/secure/call-apis-on-users-behalf/on-behalf-of-token-exchange)           | `POST`    | 2                                                                             | 2/秒         | すべてのリクエスト |                                                 |

    *これはデフォルトの上限を示します。Auth0 Dashboard で Signup endpoint の上限を設定できます。詳しくは、[不審な IP スロットリング](/ja/docs/secure/attack-protection/suspicious-ip-throttling)をご覧ください。
  </Accordion>
  ...
  <Accordion title="My Account API: Free サブスクリプションの My Account API エンドポイントに対するレート制限。">
    ...
    | エンドポイント            | メソッド                    | [バースト時のリクエスト制限](/ja/docs/troubleshoot/customer-support/operational-policies/rate-limit-policy#rate-limit-algorithm) | [継続的なリクエスト制限](/ja/docs/troubleshoot/customer-support/operational-policies/rate-limit-policy#rate-limit-algorithm) | 制限タイプ     |
    | ------------------ | ----------------------- | ---------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | --------- |
