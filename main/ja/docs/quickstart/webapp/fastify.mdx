...
<Accordion title="AI を使って Auth0 を統合" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **次に、AI アシスタントにこう依頼します:**
  ...
  AI アシスタントが、Auth0 アプリケーションの作成、資格情報の取得、`@auth0/auth0-fastify` のインストール、プラグインの設定、必要なルートやビューの作成まで自動的に行います。[agent skills の完全なドキュメント →](/docs/ja/quickstart/agent-skills)
</Accordion>
...
* **[Fastify API 認証](/docs/ja/quickstart/backend/fastify)** - JWT の検証で API エンドポイントを保護する
