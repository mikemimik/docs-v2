...
<Info>
  **Auth0は初めてですか？** [Auth0の仕組み](/docs/ja/get-started/auth0-overview)を学び、OAuth 2.0フレームワークを使用した [API の認証および認可の実装方法](/docs/ja/get-started/authentication-and-authorization-flow) についてご確認ください。
</Info>
...
<Steps>
  <Step title="API を作成" stepNumber={1}>
    ...
    デフォルトでは、API はトークンの署名アルゴリズムとして RS256 を使用します。RS256 は秘密鍵と公開鍵のペアを使用するため、Auth0 アカウントの公開鍵を使ってトークンを検証します。公開鍵は [JSON Web Key Set (JWKS)](/docs/ja/secure/tokens/json-web-tokens/json-web-key-sets) 形式で提供されており、[こちら](https://\{yourDomain}/.well-known/jwks.json) から取得できます。
  </Step>
...
<CardGroup cols={3}>
  ...
  <Card title="IDプロバイダー" icon="plug" href="/docs/ja/authenticate/identity-providers">
    他のアイデンティティプロバイダーを設定する
  </Card>

  <Card title="多要素認証" icon="shield" href="/docs/ja/secure/multi-factor-authentication">
    多要素認証を有効にする
  </Card>

  <Card title="攻撃対策" icon="lock" href="/docs/ja/secure/attack-protection">
    攻撃対策について学ぶ
  </Card>

  <Card title="ルール" icon="code" href="/docs/ja/customize/rules">
    カスタムロジックでAuth0を拡張する
  </Card>
