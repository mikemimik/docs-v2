...
<Accordion title="AI を使って Auth0 を統合する" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **次に、AI アシスタントに次のように依頼します。**
  ...
  AI アシスタントが、Auth0 API の作成、資格情報の取得、`express-oauth2-jwt-bearer` のインストール、JWT ミドルウェアの設定、トークンの検証による API エンドポイントの保護を自動的に行います。[Agent Skills の完全なドキュメント →](/ja/docs/quickstart/agent-skills)
</Accordion>
