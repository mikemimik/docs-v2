...
<Accordion title="AI を使って Auth0 を統合する" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **次に、AI アシスタントに次のように依頼します:**
  ...
  AI アシスタントが、Auth0 アプリケーションの作成、資格情報の取得、Auth0 OidcClient MAUI SDK のインストール、コールバック URL の設定、ログイン/ログアウト フローの実装を自動的に行います。[agent skills の完全なドキュメント →](/ja/docs/quickstart/agent-skills)
</Accordion>
...
<Accordion title="Organizations (B2B/enterprise)">
  ...
  詳しくは、[Organizations](/ja/docs/manage-users/organizations) を参照してください。
</Accordion>
...
<Accordion title="Universal Login をカスタマイズする">
  ...
  詳しくは、[Universal Login をカスタマイズする](/ja/docs/customize/universal-login-pages) を参照してください。
</Accordion>
...
<CardGroup cols={3}>
  ...
  <Card title="Token Best Practices" icon="key" href="/ja/docs/secure/tokens/token-best-practices">
