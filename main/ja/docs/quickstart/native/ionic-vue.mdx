...
<Accordion title="AI を使って Auth0 を連携する" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  AI アシスタントが、Auth0 アプリケーションの作成、資格情報の取得、Auth0 Vue SDK と Capacitor プラグインのインストール、ディープリンクの設定、さらにネイティブ ブラウザー連携によるログイン/ログアウト フローの実装まで、自動的に行います。[Auth0 agent skills](/ja/docs/quickstart/agent-skills) の詳細をご覧ください。
</Accordion>
