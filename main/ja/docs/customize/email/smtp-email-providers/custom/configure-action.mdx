...
他の Actions と同様に、[Management API](https://auth0.com/docs/api/management/v2/actions/get-actions) を使用して Action とその[バージョン](/ja/docs/customize/actions/manage-versions)を管理できます。すべての Actions には、同じ[制限事項](/ja/docs/customize/actions/limitations)が適用されます。
...
このコードサンプルでは、関数 `onExecuteCustomEmailProvider` は [`custom-email-provider` イベントオブジェクト](/ja/docs/customize/email/configure-a-custom-email-provider/action-triggers-custom-email-provider-event-object) から 2 つの引数を受け取ります。`event` にはユーザーや通知のコンテキストに関する情報が含まれ、`api` では通知の送信時にカスタム動作を実装するためのヘルパーメソッドが提供されます。
...
Actions の詳細については、[Auth0 Actions の仕組みを理解する](/ja/docs/customize/actions/actions-overview)をお読みください。