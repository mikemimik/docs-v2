...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  オーケストレーションと[相関付け](/docs/ja/customize/events/correlate-identity-changes)の違いは、連携先システムの数にあります。相関付けでは、1 つのイベントを 1 つの外部レコードに対応付けます。オーケストレーションでは、より大きなワークフローの一部として、1 つのイベントを複数のシステムに振り分けます。
</Callout>
...
* Events が有効化された Auth0 テナント。プランの提供状況について詳しくは、[Event Stream を作成する](/docs/ja/customize/events/create-an-event-stream)をご覧ください。
* SCIM プロビジョニングをサポートする企業向け IdP (例: Okta、Microsoft Entra ID) 。
* 該当する接続に対して設定済みの Auth0 Inbound SCIM。詳しくは、[Inbound SCIM](/docs/ja/authenticate/protocols/scim/configure-inbound-scim)をご覧ください。
...
シークレットを使うと、認証情報を Action のソースコードに含めずに済みます。詳しくは、[Action Secrets](/docs/ja/customize/actions/write-your-first-action#add-a-secret) をご覧ください。
...
イベントが届かない場合は、Auth0 Dashboard で Event Stream の配信ログを確認してください。詳しくは、[Event Testing, Observability, and Failure Recovery](/docs/ja/customize/events/event-testing-observability-and-failure-recovery) を参照してください。
...
* [ID の変更を外部システムと関連付ける](/docs/ja/customize/events/correlate-identity-changes)
* [イベントの種類](/docs/ja/customize/events/event-types)
* [Event Stream を作成する](/docs/ja/customize/events/create-an-event-stream)
