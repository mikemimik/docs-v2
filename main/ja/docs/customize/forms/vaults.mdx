...
Vault を使用すると、外部サービスや連携機能へのリクエストを認証するためのシークレットや共通設定 (例: API キー、<Tooltip tip="アクセストークン: API にアクセスするために使用される、不透明な文字列または JWT の形式の認証資格情報。" cta="用語集を見る" href="/docs/ja/glossary?term=access+tokens">アクセストークン</Tooltip>、<Tooltip tip="アクセストークン: API にアクセスするために使用される、不透明な文字列または JWT の形式の認証資格情報。" cta="用語集を見る" href="/docs/ja/glossary?term=refresh+tokens">リフレッシュトークン</Tooltip>) を、Vault 接続として安全に保存できます。
