...
* **[Machine-to-Machine](/ja/docs/get-started/apis/create-m2m-app-test#create-machine-to-machine-applications-for-testing) アプリケーション** (以下の Management API スコープが必要) :
...
トリートメントバリエーションを追加するには、[`/api/v2/experimentation/feature-flags/{feature_flag_id}/variations`](/ja/docs/api/management/v2/experimentation/create-variation) エンドポイントに `POST` リクエストを送信し、`overrides` オブジェクトを設定します。
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  レスポンスには `segment_id` の値が含まれます。セグメントは、実験で `segment` の[割り当て戦略](/ja/docs/api/management/v2/experimentation/create-experiment#body-allocation-strategy)を使用する場合にのみ適用されます。
</Callout>
...
* 実験コンテキストに応じて ACUL コンポーネントのコードを分岐する方法については、[ACUL integration guide](/ja/docs/customize/experiment-center/integrations/acul-integration-guide)を参照してください。
* post-login と registration のトリガーで `event.experiment` を使用する方法については、[Actions integration guide](/ja/docs/customize/experiment-center/integrations/actions-integration-guide)を参照してください。